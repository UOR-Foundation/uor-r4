...
fn test_live_selective_backoff_and_intersection() {
    ...
    println!(
        "Facet Store type index keys: {:?}",
        router.facet_store.type_index.keys()
    );
    println!(
        "Facet Store entity index keys: {:?}",
        router.facet_store.entity_index.keys()
    );
    ...
    for r in &resonances {
        println!(
            "Resonance: sentence='{}', relevance={}",
            r.sentence, r.relevance
        );
    }
    assert!(!resonances.is_empty(), "Should match indexed sentence");
    assert!(
        resonances[0].sentence.contains("fox jumps"),
    ...
    assert!(
        epoch_root.starts_with("blake3:"),
        "Epoch root must be a Blake3 CID"
    );
