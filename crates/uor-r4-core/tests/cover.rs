...
use uor_r4_core::transformerless::cover::{
    self, Cover, CoverConfig, CoverEdge, CoverRegion, Observation, EDGE_KIND_NEIGHBOR,
    EDGE_KIND_REFINEMENT,
};
...
fn t_invariance_and_double_run_identity() {
    ...
    let emit = |induced: &cover::InducedCover| -> Vec<u8> {
        ...
        let edges = cover::build_edges(&induced.cover, &reference, &observations);
...
fn r4g1_artifact_validates_and_reproduces() {
    ...
    let cover = &induced.cover;
    let reference = cover::ReferenceClassifier::freeze(cover);
    let edges = cover::build_edges(cover, &reference, &observations);
    let prior = cover::root_prior(&observations);
...
fn context_window_respects_story_boundaries_and_width() {
    let corpus = Corpus {
        ...
        next: vec![0; 12],
        t_argmax: vec![0; 12],
        top_tokens: vec![[0; 3]; 12],
        top_weights: vec![[0; 3]; 12],
        span_start: vec![0; 12],
        span_end: vec![0; 12],
        byte_start: vec![0; 12],
        byte_end: vec![0; 12],
...
fn split_positions_applies_the_80_20_story_cut() {
    let corpus = Corpus {
        ...
        next: vec![0; 10],
        t_argmax: vec![0; 10],
        top_tokens: vec![[0; 3]; 10],
        top_weights: vec![[0; 3]; 10],
        span_start: vec![0; 10],
        span_end: vec![0; 10],
        byte_start: vec![0; 10],
        byte_end: vec![0; 10],
