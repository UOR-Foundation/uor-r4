...
use super::{
    data::Example,
    runtime::{self, Artifact, Control, UpdateCandidate, FEATURES, MAX_RULES},
};
...
pub fn initialize(
    parent: crate::native_geometric::relative_language::runtime::Artifact,
    train: &[Example],
) -> Result<Artifact> {
    let sources = concat!(
    ...
    Ok(Artifact{schema:1,parent_digest:*blake3::hash(&parent.encode()?).as_bytes(),parent,rules:vec![],data_digest:*blake3::hash(&serde_json::to_vec(train).map_err(|_|Error::Artifact)?).as_bytes(),source_digest:*blake3::hash(sources.as_bytes()).as_bytes(),feature_names:runtime::FEATURE_NAMES.iter().map(|x|x.to_string()).collect(),training:"Greedy bounded monotone conjunction cover (1..3 of8 update-position predicates,<=8rules), requiring unique candidate and no wrong-output exposure; actual candidate byte/EOS emissions provide latent compatibility, no intermediate/gold source/word labels. Suffix utility is measured by actual final generated answer under each candidate substitution. No numbered word-slot or source-ID features and no lexical dictionary; endpoints and relative context-match positions are explicit. Retained learned byte/EOS writer supplies lexical stopping; actual full generation decides acceptance.".into()})
}
pub fn prepare(
    a: &Artifact,
    g: &BoundGeometry,
    ...
    train
        .iter()
        .map(|e| {
            let qs = runtime::questions(&e.prompt)?;
            let first = reader::route(&a.parent, g, m, &e.records, &qs[0], reader::Control::Full)?;
            let value = first.selected.as_ref().ok_or(Error::State)?;
            let candidates =
                runtime::updates(a, g, m, &e.records, &qs[1], &value.bytes, Control::Full)?;
            ...
            for update in &candidates {
                ...
                compatible_outputs.push(!out.exhausted() && out.tokens() == target(e));
            }
            Ok(Prepared {
                candidates,
                compatible_outputs,
...
pub fn fit(
    ...
    for e in train {
        ...
        exact += usize::from(!out.exhausted() && out.tokens() == target(e));
    }
    let fit = Fit {
