//! Bounded offline refinement of an inherited rule over occurrence witnesses.
...
use super::{
    completion, occurrence as runtime, scheduling, span, span_data::Example, span_learning::target,
};
...
fn proposals(base: u32) -> Vec<u32> {
    let bits: Vec<_> = (0..span::FEATURES)
...
pub fn fit(
    initial: &span::Artifact,
    g: &BoundGeometry,
    m: &Metric,
    train: &[Example],
) -> Result<(span::Artifact, Fit)> {
    initial.validate(g)?;
    ...
    a.source_digest = *blake3::hash(
        concat!(
            ...
            include_str!("occurrence_learning.rs"),
            include_str!("span.rs"),
    ...
    .as_bytes();
    a.training = format!("Bounded offline occurrence rule refinement from parent blake3:{}. Every enumerated source/span is supervised by actual retained completion/writer final answer/EOS; expected source, span and intermediate labels are not used. A candidate is admitted by any one complete witness satisfying one rule; features are never combined across witnesses. Proposals are the inherited rule {base} plus zero, one or two existing predicate bits, at most five set bits, sorted by bit count then numeric value. Greedy cover starts empty, rejects every incompatible admission and preserves previously unique coverage; at most eight rules. Context roles, completion, updater, writer and geometric encoding are frozen. Training rows are artifact-bound; development is not an input to this learner.", blake3::hash(&initial.encode()?));
    let mut covered = 0;
    let mut history = Vec::new();
    for _ in 0..span::MAX_RULES {
    ...
    let report = Fit {
        ...
        stopping: if covered == train.len() {"complete_cover"} else if a.rules.len() == span::MAX_RULES {"rule_limit"} else {"no_safe_improving_rule"}.into(),
    };
    Ok((a, report))
}
#[cfg(test)]
