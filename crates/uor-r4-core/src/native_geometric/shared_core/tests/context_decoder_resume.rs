//! Evaluation-only recovery of four saved conditional heads after report storage failure.
...
fn serialized_value(value: &Value) -> RunResult<Value> {
...
#[test]
#[ignore = "requires sealed incomplete four-head attempt; zero training calls"]
fn context_decoder_resume_saved_evaluation() -> RunResult<()> {
    let input = fs::canonicalize(std::env::var("UOR_CONTEXT_REFIT_INPUT")?)?;
    let saved_root = fs::canonicalize(std::env::var("UOR_CONTEXT_REFIT_SAVED")?)?;
    let evidence = input
        .parent()
    ...
    let result = (|| -> RunResult<()> {
        ...
        let failed = read_json(&saved_root.join("failure.json"))?;
        if failed["status"] != "INCOMPLETE"
            || failed["error"] != "report write exceeds storage limit with seal reserve"
            || saved_root.join("result.json").exists()
        {
            return Err("saved attempt not the expected interrupted report write".into());
        }
        for cell in CELLS {
            for suffix in ["head", "fit", "witness", "early-generation"] {
                if !saved_root.join(format!("{cell}-{suffix}.json")).is_file() {
                    return Err("saved four-cell artifact set incomplete".into());
                }
            }
        }
        let design_bytes = fs::read(&path)?;
        if fs::read(saved_root.join("design.json"))? != design_bytes {
            return Err("recovery design differs from original".into());
        }
        ...
        let parent = &priors[0];
        let cap = &priors[2];
        let original_source = json!({"implementation":SharedCore::implementation_digest(),"driver":driver_digest(),"tree_source":digest(include_bytes!("../angular_tree.rs")),"context_driver":digest(include_bytes!("context_augmentation.rs")),"design":design_digest,"corpus":corpus_digest,"dataset":dataset_digest,"context_source":context_digest,"input":input,"dataset_root":data_root,"prior_identities":ids,"prior_artifact_sha256":prior_bytes.iter().map(|b|format!("{:x}",sha2::Sha256::digest(b))).collect::<Vec<_>>(),"cells":CELLS,"state_for_cell":STATE_FOR_CELL,"data_for_cell":DATA_FOR_CELL,"state_proposals":0,"standard_changed_model_artifact_exported":false});
        let saved_source_bytes = fs::read(saved_root.join("source.json"))?;
        let saved_source: Value = serde_json::from_slice(&saved_source_bytes)?;
        if original_source != saved_source {
            return Err("saved source differs from complete original source binding".into());
        }
        let mut recovery_source = original_source;
        recovery_source["recovery_driver"] =
            json!(digest(include_bytes!("context_decoder_resume.rs")));
        recovery_source["recovered_from"] = json!(saved_root);
        recovery_source["saved_source_digest"] = json!(digest(&saved_source_bytes));
        recovery_source["saved_manifest_digest"] =
            json!(digest(&fs::read(saved_root.join("manifest.json"))?));
        recovery_source["saved_failure_digest"] =
            json!(digest(&fs::read(saved_root.join("failure.json"))?));
        recovery_source["fit_calls"] = json!(0);
        recovery_source["preserved_fit_calls"] = json!(4);
        write(&out, "source.json", &recovery_source)?;
        ...
        for (cell_index, cell) in CELLS.iter().enumerate() {
            ...
            let bytes = fs::read(saved_root.join(format!("{cell}-witness.json")))?;
            let witness: Witness = serde_json::from_slice(&bytes)?;
            let head_bytes = fs::read(saved_root.join(format!("{cell}-head.json")))?;
            let head: AngularEmission = serde_json::from_slice(&head_bytes)?;
            let fit_bytes = fs::read(saved_root.join(format!("{cell}-fit.json")))?;
            let fit: AngularTreeReport = serde_json::from_slice(&fit_bytes)?;
            let data = training_digest(docs);
            if serde_json::to_vec(&head)? != serde_json::to_vec(&witness.head)?
                || head_bytes.len() > 131072
            {
                return Err("saved head/witness mismatch or size".into());
            }
            let candidate = witness.replay(
                cap,
                state,
                cell,
            ...
            if fit.parent != witness.ephemeral_cap
                || fit.data_digest != data
                || serde_json::to_value(fit.config)? != serde_json::to_value(config())?
                || fit.after.positions != 2688
                || !fit.after.mean_nll.is_finite()
            {
                return Err("saved fit receipt lineage".into());
            }
            let checked = candidate.evaluate(docs, Intervention::Full)?;
            if !metric_equal(&checked, &fit.after) {
                return Err("saved fit metric replay".into());
            }
            ...
            if cell_index == 0 {
                let baseline = parent
                    ...
                    .as_ref()
                    .ok_or("baseline head")?;
                let expected = serde_json::to_vec(&baseline.branches)?;
                let actual = serde_json::to_vec(&head.branches)?;
                let reproduction = read_json(&saved_root.join("baseline-reproduction.json"))?;
                if expected != actual
                    || reproduction["matched"] != true
                    || reproduction["fit_calls"] != 1
                    || reproduction["expected_branch_payload"] != digest(&expected)
                    || reproduction["actual_branch_payload"] != digest(&actual)
                    || reproduction["witness"] != witness_id
                {
                    return Err("saved baseline reproduction does not verify".into());
                }
                write(
                    &out,
                    "baseline-reproduction.json",
                    &json!({"saved":reproduction,"recovered_from":saved_root,"fit_calls":0,"replayed_exact_branch_payload":true}),
                )?;
            }
            let outputs = generate(&candidate, &prompts, &[Intervention::Full])?;
            let saved_early = read_json(&saved_root.join(format!("{cell}-early-generation.json")))?;
            if saved_early["cell"] != *cell
                || saved_early["witness"] != witness_id
                || saved_early["outputs"] != serde_json::to_value(&outputs)?
            {
                return Err("saved early generation replay".into());
            }
            write(&out, &format!("{cell}-early-generation.json"), &saved_early)?;
            fit_reports.push(json!({"cell":cell,"witness":witness_id,"state":ids[STATE_FOR_CELL[cell_index]],"data_arm":if DATA_FOR_CELL[cell_index]==0{"repeated"}else{"varied"},"fit":fit,"head_bytes":serde_json::to_vec(&head)?.len()}));
            witness_records.push(json!({"cell":cell,"witness":witness_id,"witness_path":saved_root.join(format!("{cell}-witness.json")),"head_path":saved_root.join(format!("{cell}-head.json")),"head_file_digest":digest(&head_bytes),"fit_path":saved_root.join(format!("{cell}-fit.json")),"fit_file_digest":digest(&fit_bytes)}));
            candidates.push(candidate);
            witness_ids.push(witness_id);
            early.push(outputs);
        }
        write(&out, "saved-witnesses.json", &witness_records)?;
        ...
        let prior_behavior = json!({"panels":saved_prior_panels,"generations":prior_outputs,"replayed_rows":prior_rows,"replayed_outputs":replayed_prior_outputs});
        if read_json(&saved_root.join("prior-behavior.json"))? != serialized_value(&prior_behavior)?
        {
            return Err("completed saved prior behavior differs".into());
        }
        write(&out, "prior-behavior.json", &prior_behavior)?;
        ...
        for (cell_index, cell) in CELLS.iter().enumerate() {
            ...
            let behavior = json!({"panels":panels,"outputs":outputs,"probe_panels":probe_panels,"probe_outputs":probe_outputs});
            if cell_index < 3
                && read_json(&saved_root.join(format!("{cell}-behavior.json")))?
                    != serialized_value(&behavior)?
            {
                return Err("completed saved cell behavior differs".into());
            }
            if cell_index == 3 && saved_root.join(format!("{cell}-behavior.json")).exists() {
                return Err("unexpected fourth saved behavior".into());
            }
            write(&out, &format!("{cell}-behavior.json"), &behavior)?;
        ...
        write(
            &out,
            "result.json",
            &json!({"schema":"uor-r4.context-decoder-refit-result/1","decision":if accepted{"PASS_CONTEXT_DECODER_REFIT_DEVELOPMENT_GATE"}else{"FAIL_CONTEXT_DECODER_REFIT_DEVELOPMENT_GATE"},"baseline_branch_reproduction":true,"fit_calls":0,"preserved_fit_calls":4,"recovered_from":saved_root,"recovered_failure":"INCOMPLETE_REPORT_STORAGE_LIMIT","completed_old_cell_reports_replayed":3,"state_proposals":0,"decoder_only_gate":decoder_only,"varied_state_increment_gate":state_increment,"method_gate":method_gate,"contrasts":contrasts,"cells":cell_results,"fit_reports":fit_reports,"prior_identities":ids,"prior_replayed_rows":prior_rows,"prior_replayed_original_outputs":replayed_prior_outputs,"prior_replayed_probe_rows":prior_probe_rows,"prior_replayed_probe_outputs":36,"elapsed_ms":start.elapsed().as_millis(),"promoted":false,"scope":"Evaluation-only recovery of four previously fitted and sealed conditional heads; zero new fits. Original frozen state snapshots and construction data. All seven prior states/artifacts replayed, with fixed states unchanged by head refits. Previously opened recombination probes and actual outputs compared after all four fits; no new state learning, regularization, JEPA, fresh broad holdout, normal changed artifact, promotion or language/energy qualification."}),
        )?;
        guard(start)?;
        Ok(())
    })();
...
#[test]
fn context_decoder_resume_compact_records_preserve_values_and_original_identity() -> RunResult<()> {
    let value = json!({"rows":[{"position":0,"nll":2.8395341889547745,"roots":[1,2,3,4],"source":null}],"fit_calls":0,"preserved_fit_calls":4});
    let compact = serde_json::to_vec(&value)?;
    let pretty = serde_json::to_vec_pretty(&value)?;
    assert!(compact.len() < pretty.len());
    // Compare serialized evidence on both sides, including decimal f64 parsing.
    assert_eq!(
        serialized_value(&value)?,
        serde_json::from_slice::<Value>(&pretty)?
    );
    assert_eq!(
        serde_json::from_slice::<Value>(&compact)?,
        serde_json::from_slice::<Value>(&pretty)?
    );
    assert_eq!(
        driver_digest(),
    ...
    assert_ne!(
        driver_digest(),
        digest(include_bytes!("context_decoder_resume.rs"))
    );
    Ok(())
}
