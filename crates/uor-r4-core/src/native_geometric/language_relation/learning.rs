...
pub fn initialize(
    parent: crate::native_geometric::ordered_state::runtime::Artifact,
    train: &[Example],
) -> Result<Artifact> {
    let sources = concat!(
    ...
    Ok(Artifact{schema:1,parent_digest:*blake3::hash(&parent.encode()?).as_bytes(),parent,rules:vec![],data_digest:*blake3::hash(&serde_json::to_vec(train).map_err(|_|Error::Artifact)?).as_bytes(),source_digest:*blake3::hash(sources.as_bytes()).as_bytes(),training:"Greedy bounded monotone conjunction cover (1..3 of20 structural predicates,<=8rules), requiring unique candidate and no wrong-output exposure; actual candidate byte/EOS emissions provide latent compatibility, no source/word labels. Retained learned byte/EOS writer supplies lexical stopping; actual full generation decides acceptance.".into()})
}
pub fn prepare(
    a: &Artifact,
    g: &BoundGeometry,
    ...
    train
        ...
        .map(|e| {
            ...
            for c in &candidates {
                ...
                for &b in &c.bytes {
                    actual.push(text::symbol(&a.parent.parent.parent, b, true)?);
                }
                actual.push(text::symbol(&a.parent.parent.parent, 0, false)?);
