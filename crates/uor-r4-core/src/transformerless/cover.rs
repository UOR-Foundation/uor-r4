...
pub struct Observation {
    ...
    pub position: u32,
    /// Content address of the context window (`observe::sample_id`).
    pub sample: [u8; 32],
...
pub fn build_observations(
    ...
    for &i in positions {
        let bundle = runtime::bundle_plain(art, &rot, corpus, i);
        ...
        observations.push(Observation {
            position: i as u32,
            sample: observe::sample_id(&context_window(corpus, i)),
            vector,
            sig: runtime::sig_plain(art, &bundle),
            next: corpus.next[i],
        });
    }
    observations
...
pub fn emit_r4g1(
    ...
    // NODE: the root record is all zeros; regions follow creation order.
    ...
    // EMIT: descriptor + the v0 linear-count root prior block.
    ...
    head.extend_from_slice(&DEFAULT_MAX_EMISSION_ENTRIES.to_le_bytes()); // E
