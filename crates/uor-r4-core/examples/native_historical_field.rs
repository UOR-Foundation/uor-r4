//! Actual prompt-only generation for historical typed-field composition.
    ...
    .enumerate()
    {
        ...
        .enumerate()
        {
                ...
                .enumerate()
            {
                ...
                rows.push(json!({"id":format!("{layout}/{query}/{style}"),"prompt":prompt,"actual":generate(&model,&prompt,Control::Full,false,false)?}));
            }
        }
    }
    save(
...
fn prepare(out: &Path, fresh: bool) -> Result<()> {
    ...
    let owners: Vec<(String, String, String)> = if fresh {
        (0..3)
            .map(|i| {
                let h = blake3::hash(format!("historical-field:{draw}:{i}").as_bytes());
                let b = h.as_bytes();
                ...
                for third in [false, true] {
                        ...
                        .enumerate()
                    {
                        cases.push(Case {
                            ...
                            prompt: format!(
                                "{facts}Where was {owner} before? {instruction} Answer:"
                            ),
    ...
    let docs: Vec<_> = cases
        .iter()
        .map(|c| FieldCompositionExample {
            id: c.id.clone(),
            prompt: c.prompt.clone(),
            owner: c.owner.clone(),
            value: c.value.clone(),
            pieces: if c.current_record.is_some() {
                vec![
                    FieldPiece::Bytes(" ".into()),
                    FieldPiece::Owner,
                    FieldPiece::Bytes(" was in ".into()),
                    FieldPiece::Value,
                    FieldPiece::Bytes(".\n".into()),
                ]
            } else {
        ...
        if target {
            ...
            if controls {
                for control in [
                    ...
                    if control == Control::HistoricalFieldCompositionDisabled
                        && reference
                            .as_ref()
...
fn main() -> Result<()> {
    ...
    if a.len() != 5 {
        return Err("usage: prepare/fresh OUT | diagnose MODEL OUT | fit/baseline/evaluate/controls MODEL CASES OUT".into());
    }
    ...
    if a[1] == "fit" {
        ...
        let (candidate, report) = model.fit_historical_field_composition(&docs, config)?;
    ...
    let parent = model.without_historical_field_composition()?;
    save(
