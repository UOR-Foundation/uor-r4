...
Start with the [September 19 takeover review](docs/integration/takeover-review-2026-09-19.md), [latest learning-contract review](docs/integration/learning-contract-review-2026-09-19.md), [current state](docs/integration/current-state.md), [canonical plan](docs/integration/project-track.md) and [next DeepSeek handoff](docs/integration/deepseek-learning-contract-step-2026-09-19.md). These reconcile the earlier work and current mechanism evidence. Live [programme issue #820](https://github.com/UOR-Foundation/uor-r4/issues/820) owns delivery status; dated experiment records retain their original evidence scope.
...
PR #1296 adds `prior_learning.rs` and reports 0.375 accuracy on a small addition fixture, with real-text fitting not run. The [source review](docs/integration/learning-contract-review-2026-09-19.md) finds that training omits the contextual logit scale, the fixture mixes conflicting addition/subtraction labels, and the shared optimizer suppresses small valid gradients. The result does not yet isolate quantization or establish a correct learning instrument.

Next, **repair the shared forward/gradient/optimizer contract, validate a consistent target mask and an explicit low-bit capacity witness, then rerun one small learned gate**. Use a floating comparator only if the corrected gate still fails. When the exported low-bit gate passes, complete schedule/checkpoint/identity checks and continue into the bounded representative prior-only text curve. The [complete DeepSeek prompt](docs/integration/deepseek-learning-contract-step-2026-09-19.md) gives executable conditions and resources.

The roadmap separates this local-context baseline from the later learned geometric state/read and exact-memory integration. A two-token prior cannot distinguish older histories sharing the same tail; stored geometric metadata is not a predictive mechanism. The [canonical dependency sequence](docs/integration/project-track.md#research-dependencies-and-exit-conditions) keeps the baseline connected to the full model goal.
