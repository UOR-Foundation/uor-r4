...
Start with the [September 19 takeover review](docs/integration/takeover-review-2026-09-19.md), the [frozen-prior replay](docs/evidence/native_geometric_frozen_prior_replay_2026-09-20.txt) and the [executed older-prefix pilot](docs/evidence/native_geometric_ordered_prefix_pilot_2026-09-20.txt), [current state](docs/integration/current-state.md), [canonical plan](docs/integration/project-track.md) and the [review after PR #1304](docs/integration/prefix-pilot-review-2026-09-20.md) and [next readout diagnostic](docs/integration/deepseek-readout-diagnostic-step-2026-09-20.md). These reconcile the earlier work and current mechanism evidence. Live [programme issue #820](https://github.com/UOR-Foundation/uor-r4/issues/820) owns delivery status; dated experiment records retain their original evidence scope.
...
The learned older-prefix group-state/read pilot has been **executed and is numerically negative**. Its primary hard-forward CE is **7.575604** bits/target versus the frozen parent's **7.558429** on 36 documents / 288 windows. Final token action IDs changed, but the primary does not demonstrate an advantage over the fitted local-tail control or a positive conditional-history intervention. This scopes one register, palette, surrogate and separable residual reader; it does not establish that older context is unneeded. [Principal review](docs/integration/prefix-pilot-review-2026-09-20.md).

The review also corrects delivery and generation claims: saved prefix JSON files contain hashes rather than complete hard weights; evaluation used in-memory cores; local-tail generation used the wrong state rule; count-reference generation considered only three candidates. The candidates are recoverable from preserved checkpoints. Their numerical loss evidence remains distinct from independently reloaded serving qualification.

The next task recovers those artifacts and diagnostics **without prefix retraining**, then tests empirical versus smoothed conditional-target training of the existing output head on frozen integer context features. Count-reference superiority motivates this bounded diagnostic but does not isolate an output-head defect or make count matching a condition for future geometry. The [complete DeepSeek prompt](docs/integration/deepseek-readout-diagnostic-step-2026-09-20.md) and [canonical dependencies](docs/integration/project-track.md#research-dependencies-and-exit-conditions) retain conditional geometric read/update, exact memory and integrated capability as the larger direction. The new diagnostic is **NOT_RUN**.
