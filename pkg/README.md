...
Next, **implement and execute one learned older-prefix group-state/read pilot**, keeping the current prior frozen. A 120-state exact group channel uses learned eight-way token actions and a width-16 ternary residual reader. Compare it with a fitted fixed-action channel and a fitted local-tail channel, then exchange prefix states only among occurrences with the same local pair. This separates older-history information from extra local prediction capacity. The [principal review](docs/integration/ordered-prefix-review-2026-09-20.md) gives the mathematics and research rationale; the [complete DeepSeek prompt](docs/integration/deepseek-ordered-prefix-step-2026-09-20.md) specifies hard-forward training, independent surrogate checks, actual controls and resources. This pilot is **NOT_RUN**.

The count-reference gap is unrecovered local performance, not a diagnosis of limited capacity or insufficient history. Likewise, the shared `(32,32)` argmax does not explain the frequency of entering that fixed point; count-reference trajectories were not run. The reported `(284,198)` self-loop is only immediate repetition and then exits newline. These corrections preserve the valid replay gates and six observed period-one trajectories of the old model. The [canonical dependency sequence](docs/integration/project-track.md#research-dependencies-and-exit-conditions) leads from a validated local baseline to measured prefix-state contribution, exact occurrence memory and integrated capability. A single finite state is not exact memory or an established language model.
