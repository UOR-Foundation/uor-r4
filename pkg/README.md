...
The owner-adopted plan is a **shared trainable geometric language core**: recurrent geometric state, bounded contextual reads and learned byte/EOS emission trained toward one predictive objective. The first experiment exercises its actual discrete forward path throughout learning and has no inherited additive predictor fallback. It is an isolated prototype; it has not replaced the retained native artifact or established language quality.

The accumulated memory repair is parked with all source, artifacts and evidence preserved. Its bounded historical results remain in their issue records. The retained model is `15baec48`. The experimental addressed-attention path now connects exact memory, typed operations and learned emission; its first bounded learning pilot fails generated-behavior acceptance and has not replaced the retained model. [Current state](docs/integration/current-state.md) records the evidence and remaining diagnosis. Intermediate-version repair is no longer the active next step.

[The canonical plan](docs/integration/project-track.md) owns the sequence and acceptance. [The first-step design](docs/integration/shared-geometric-core-2026-09.md) defines the minimal complete learning path; [current state](docs/integration/current-state.md) owns actual execution results, artifact identities and resource receipts. [#973](https://github.com/UOR-Foundation/uor-r4/issues/973) owns integrated language learning, with [#964](https://github.com/UOR-Foundation/uor-r4/issues/964) covering the complete runtime operation constraint. [#820](https://github.com/UOR-Foundation/uor-r4/issues/820) remains the programme tracker and [ROADMAP.md](ROADMAP.md) provides issue navigation.
