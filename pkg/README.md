...
| Current geometric addressed-memory core | Fixed token-to-element assignment, ordered address pairs, accumulated value memory, ternary readout and optional group kernels | Synthetic recall/composition diagnostics and mechanism controls. The unchanged zero-prior configuration has 85–94% cold reads on the measured internal corpora. A learned local prior plus causal memory is the next experiment; real-text language qualification remains pending. |
...
Implement and test an **always-available learned exact-token context prior plus the existing causal memory residual**, using one shared low-bit readout. Compare it with a separately trained prior-only model, bias/unigram controls and causal interventions on one pinned document/target manifest. This follows the observed cold-route failure; it does not yet establish a distinctive geometric advantage. The [architecture review](docs/integration/cold-context-review-2026-09-19.md) explains the choice and current literature, and the [complete DeepSeek prompt](docs/integration/deepseek-cold-context-step-2026-09-19.md) specifies implementation, corrections, a bounded paired pilot and delivery.
