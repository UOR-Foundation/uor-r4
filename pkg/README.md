...
**Latest result: the separable read is a local query calibration, not history.** The [frozen S attribution](docs/integration/s-attribution-result-2026-09-20.md) is one complete evaluation-only experiment on the retained artifacts. On `Z_S = E + u(q) + u(b)` the older row alone is indistinguishable from E; dropping it **improves** CE by 0.0073 (new positions) / 0.0100 (old) bits with the paired interval entirely below zero on both panels; and a content-free, length-conditioned fit average beats the individual older state by 0.0187 / 0.0236 bits, interval entirely below zero on both. The exact integer identity `Z11+Z00=Z10+Z01` holds with **0 failures** and E/S01/S00/Qonly are *exactly* invariant to the older-prefix donor and reversal. All 16 retained old vectors reproduce with a **0.0** maximum delta. S's 7.032228 therefore remains a valid local emission result, but the 120-state group product is not carrying usable older content at this dose. [Raw receipt](docs/evidence/native_geometric_s_attribution_2026-09-20.txt).

The same change repairs the artifact plumbing the review flagged: production export now requires the real raw tokenizer identity (the legacy zero-digest files load only through a restricted, hash-pinned import), the inference path folds the older prefix once instead of twice (L uses 3 compose calls instead of 65), and an evaluation-only entry point cannot fall into fitting. Corrected descendants differ from the originals in exactly 32 bytes, all inside the tokenizer field. CPQK production continuation remains incomplete and is required before any new fit.

**Next: give the state a mechanism that can make older content addressable** — a bounded selective write/reset over exact 2I state, or exact occurrence/version access — and qualify it against fresh behavioural criteria rather than another score comparison on this panel. No re-fit of S, width/precision/corpus sweep, or reset campaign is selected here. Useful language/coding and full laptop cost remain open.
