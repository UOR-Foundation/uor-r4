...
**Next: execute the [matched geometric query-read prompt](docs/integration/deepseek-geometric-query-step-2026-09-20.md)**, currently NOT_RUN. Freeze corrected E and compare a joint history/query read, a separable history/query read, and a local-only read with the same parameters and read costs. The task includes a learned-query fixture, exact artifacts and gradients, a bounded three-arm fit, causal older-prefix controls, generation and cost. Writing remains the existing ordered group product; selective writing/reset and exact occurrence/version memory are later dependencies. The [canonical plan](docs/integration/project-track.md#research-dependencies-and-exit-conditions) connects this test to useful shared-model conversation and coding.
