...
The [composed operator/output refinement](docs/native_geometric_composed_output_1140.md) retains `866cb92d` over preserved parent `ccbe7f2a`. The shared learned signed-H4 selector now combines two Add operations with sentence or Rust-equality emission, using the first committed result in the second operation. Construction, open, changed-context and post-selection fresh checks pass; all 663 retained answers, earlier panels and exact old outputs/controls are preserved. Actual native API passes 50 checks, and 54 generated Rust equalities compile and evaluate. These finite forms do not qualify general prose or Rust program synthesis. The next step is mixed shared Copy/Add sequences and operand references, preserving exact dependencies and stopping.
