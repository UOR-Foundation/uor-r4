...
**Next: learn geometric selection of exact token occurrences in the current BPE model.** The [comprehensive DeepSeek prompt](docs/integration/deepseek-occurrence-reader-step-2026-09-20.md) freezes the local scorer, reuses bounded occurrence/version contracts, and learns a candidate-relative reader with NoRead and exact payload access. Fresh counterfactual bindings, a competent exact-lookup comparator, actual generated outputs and a bounded raw-text probe determine usefulness. A 120-state reset changes forgetting but does not create addressable payload memory. DeepSeek has discretion over implementation and justified corrections; no further S attribution or width/projection campaign is selected.

[Storage cleanup](docs/integration/storage-cleanup-2026-09-20.md) recovered 23.21GB and left 51.17GB free at its endpoint, preserving all research/model/session data and release binaries. Refresh resources before work; reusable debug build cache is partly cold.
