...
The [current-query refinement](docs/native_geometric_current_query_1140.md) retains `ddc943f9` over the preserved contextual writer parent `79710468`. The shared geometric selector now distinguishes new literal operands from prior computed results on 255 construction, 48 open and 24 post-selection numeric-transfer cases, with 16 harder history checks and all 663 retained answers preserved. Removing the original result changes dependent answers while leaving independent questions correct. The previously failing third independent sum now returns 17 through the actual API and restored checkpoint. These are bounded authored query forms, not general language qualification. The next step is learned transitions within a response and compositional emission from actual state; broader language learning remains unfinished.
