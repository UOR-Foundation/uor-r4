...
Next, **specify and bound one learned ordered prefix-state/read channel** with same-tail/different-prefix, state-disabled, order and matched capacity/cost controls, using the corrected replay's count reference as the comparator. The [principal review](docs/integration/frozen-prior-review-2026-09-20.md) gives source findings, the [complete DeepSeek prompt](docs/integration/deepseek-frozen-prior-step-2026-09-20.md) specified the executed replay, and the [replay receipt](docs/evidence/native_geometric_frozen_prior_replay_2026-09-20.txt) records its artifacts. Refitting this prior or retraining to improve the verdict is not the next step; the loop diagnostic shows generation collapse is shared with the local count statistics at `(32,32)` and is model-specific only at `(284,198)`.

The roadmap then tests one learned older-prefix state/read channel with matched capacity/cost controls, keeping exact occurrence memory distinct. Equal local tails force equal predictions from this two-token prior. The executed diagnostic shows all six bounded trajectories enter a period-1 cycle, that `argmax Z(32,32)=32` is a real fixed point supported by local counts (and by the count reference), and that the model diverges from the reference only at `(284,198)`; the short loop is therefore mostly a property of the data and greedy decoding, not evidence that adding finite geometric state removes it. The corrected comparisons select the next mechanism, without making perfect prior fluency or unchanged retraining a prerequisite. The [canonical dependency sequence](docs/integration/project-track.md#research-dependencies-and-exit-conditions) connects this increment to the full goal.
