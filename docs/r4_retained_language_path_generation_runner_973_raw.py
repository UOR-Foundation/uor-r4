...
def _verify_model_dependencies(parent: Mapping[str, Any]) -> dict[str, Any]:
    ...
    generator_record = artifact_records(
        trainer_root,
        ["src/r4_softmax_trainer/language_path_generation.py"],
    )[0]
...
def _validate_rollout_record(
    ...
    *,
    ...
    if record["prompt"] != prompt.record() or record["seed"] != SEED:
    ...
    if record["input_token_ids"] != expected_input:
    ...
    if record["fed_back_generated_token_ids"] != generated[:-1]:
    ...
    if (
        record["stop_reason"] != expected_stop
        or record["first_eos_offset"] != first_eos_offset
        or record["short_cycle_period"] != short_cycle_period(generated)
    ):
        raise ValueError("generation transcript stop evidence differs from the freeze")
    response_ids = generated[:first_eos_offset] if first_eos_offset is not None else generated
    if record["response_token_ids"] != response_ids:
    ...
    if record["audit"] != expected_audit:
        raise ValueError("generation transcript audit differs from direct retained work")
    if record["audit_cid"] != cid_bytes(canonical_json_bytes(expected_audit)):
...
def _validate_environment(value: Any) -> dict[str, Any]:
    ...
    if set(value) != expected_fields or any(
        value.get(key) != item for key, item in constraints.items()
    ):
...
def _validate_implementation(
    ...
    if observed != expected or not isinstance(observed, Mapping):
...
def _validate_frozen_result(
    ...
    *,
    ...
    _verify_self_cid(result, "result_cid")
    if result.get("inputs") != input_evidence:
        raise ValueError("generation result inputs differ from verified frozen evidence")
    implementation_record = _validate_implementation(result.get("implementation"), implementation)
    ...
    if dict(result) != _with_self_cid(expected_body, "result_cid"):
