...
| Double counting across memberships | **RESOLVED 2026-07-23 (issue #64)**: chain-telescoped Rule 1 (`score_candidates` in `score_runtime.rs`) restricts ΔE to the single deepest covered refinement chain — correlated sibling emissions can no longer stack. D4 EXCT precedence (Rule 2) gates out graph residuals when exact-context evidence is strong. Legacy Σ-over-cloud formula retained as `score_candidates_legacy` for Gate C side-by-side comparison only (test `legacy_formula_double_counts_sibling_regions_new_formula_does_not` demonstrates the contrast). Re-run `r4 transformerless score` on a recompiled fixture graph to measure the new formula's Gate C numbers. |
