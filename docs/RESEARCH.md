...
clean words into word salad. This does not yet prove the architecture works
end-to-end (that needs a fix-and-recompile-and-retest cycle, scoped in
#755), but it substantially undermines reading the word-salad result as
architectural: at least one tested bundle has demonstrably clean training
text and a demonstrably corrupted context-reconstruction step. Separately,
...
and #755's fix-and-retest cycle is now the most direct path to an answer.
...
| #755 | #745 root cause: `load_corpus_bytes`/`history_token` assume per-story-contiguous record order | **Open.** Found by #752. Fix: derive `input[i]`/context from records grouped by `(story, span_start)` rather than physical on-disk order, so correctness does not depend on the observation pipeline's shard/merge ordering discipline. Then recompile `smollm2-135m-instruct` (already-clean corpus text) and re-run `r4 ask` as the decisive before/after test for #745 |
