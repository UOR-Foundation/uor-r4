...
| #496 | VSA: real encoder built — facet-bottlenecked below Spectral | Real encoder **landed** (owner decision was "build it"): `VsaGeometry::ground` now grounds the hypervector from the zeta word-sum content signal (the multiplication-free construction Spectral uses, not a content hash), and scoring re-grounds each candidate for a commensurable VSA-vs-VSA cosine. VSA went from dead (0.0000 on every arm) to **functional** — but `geometry_ablation` (300 stories/30 probes) shows **Spectral still wins decisively, 0.93 vs 0.50 MRR**: VSA recall@20 is only 0.53 because its facet candidate-set drops ~47% of targets. The encoder is fixed; the **facet index is now the bottleneck**, and query-time re-grounding is O(candidates) (needs store-at-index). So even with a real encoder VSA underperforms Spectral (0.83 MRR full-caps), which already serves the semantic path — the open owner call is now build a real facet-index (substantial) vs deprecate, decided on evidence rather than a placeholder. [geometry_ablation_434.md](geometry_ablation_434.md) |
