...
they were trained on. That points the remaining investigation (**#752**) at
the observe/corpus side rather than serving, but does not yet close the
question of a fixable defect vs. a genuine architectural ceiling. Separately,
...
| #745 | Root-cause the degenerate small-vocabulary generation on the best-available compiled bundle | **Open, substantially narrowed.** The #655 sweep asked real questions of every reachable local bundle through `r4 ask`. `smollm2-135m-instruct`/`-chat` both returned empty output; `smollm2-1-7b-chat` — this repo's largest/best-looking local bundle — produced non-grammatical word-salad cycling a small set of nonsense tokens (`tetra`, `gru`, `gulick`, `chall`, `caption`…). Two hypotheses have since been ruled out by direct empirical test, not argument: (1) a real, separate byte-BPE prompt-encoding bug was found and fixed experimentally (tracked as **#751**), but re-running `r4 ask` with the fix applied produced byte-identical word-salad — prompt encoding is not the cause; (2) the recurring tokens were initially suspected to come from a foreign (Llama/SentencePiece) vocabulary bleeding into a SmolLM2-vocabulary decode, but direct inspection confirmed the compiled artifact's declared vocab size (49152, read from the `TLA5` header) matches the tokenizer's own 49152-entry vocab exactly, and decoding the exact predicted token ids through that same tokenizer reproduces `tetra`/`gru`/`chall`/`caption` precisely — these are genuinely native SmolLM2 subword pieces, not a cross-vocabulary artifact. The decisive new finding: instrumenting `runtime::predict_witness_plain`'s resolution-depth witness shows generation is **not** falling back to context-independent unigram selection (depth 0) — most tokens resolve at depth 1-4 with high evidence counts (some >50,000), meaning the compiled tables have learned strong, high-confidence associations favoring this narrow set as the top continuation across unrelated prompts ("Hello, how are you?", "Quantum physics describes...", and "1 2 3 4 5" all produce near-identical output vocabularies). This rules the cause **into** the observe/corpus/training side of the pipeline, not live serving — continuation scoped as **#752** (decode and inspect the bundle's actual observed corpus text) |
...
| #752 | #745 continuation: inspect observed corpus.records content for the smollm2-1-7b-instruct bundle | Scoped follow-up carrying #745's investigation into the corpus/observation side, per the depth-witness finding above |
