...
*Recently landed and closed (GitHub is the source of truth; this table tracks what is still open): **#486/#490** — the serving path compared a routing vector to a content vector; the content-vector query is now the deployed default (+0.1363 MRR), with the serde-default and blast-radius findings recorded on #490. **#487** — corrected #434's Spectral record (lexical, not geometry). **#493** — the VSA switch made honest; its `0.0000` is a scoring category error, not a wiring gap (below). **#458/#459** — interaction information and the estimation ladder, both landed NEGATIVE/count-limited. **#456** — reconstructability certificate + null arm (below).*

| Issue | Question | State |
|---|---|---|
| #488 | Phase timing across the score pipeline | Instrument **landed** (PR #495): `PhaseLog` generalized to the whole `score` pipeline, verified on the 500k fixture (phases sum to wall clock with zero remainder; Gate C scoring dominates at 247s, cover induction is the largest pre-Gate-C stage at 27.9s). **Kept open**: attributing a real 2.11M run's hours needs a corpus at that scale — gone from disk, `fetch_d3_corpus.py` warns it is non-reproducible |
| #457 | IPF-consistent reconstruction operator | **Open, unstarted.** Arm A (Rule 1 + Rule 2 chain-telescoped, EXCT disabled) vs Arm B (IPF-consistent reconstruction over overlapping region marginals) + shuffled-block null, on the pinned D3 fixtures. #459's k≥3 count-limit warns the higher-order consistency gain may be unmeasurable at D3 scale, so the per-region model-vs-empirical KL (the NEGATIVE-branch deliverable) is the important output regardless of direction |
