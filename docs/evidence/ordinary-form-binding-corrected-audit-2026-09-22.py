...
import blake3
P=pathlib.Path;REPO=P('/Users/casey.allard/uor-r4');BASE=REPO/'.uor-models/realtext-prior-2026-09-20';HEAD=sys.argv[2]if len(sys.argv)>2 else '940a9076';ROOT=P(sys.argv[1])if len(sys.argv)>1 else BASE/'ordinary-form-binding-principal-1'
...
out={'schema':'uor-r4.ordinary-form-corrected-independent-audit/1','scope':__doc__,'submitted_head':HEAD,'roots':{}}
for i in range(1,9):
...
def control(arm):return{'lexical_membership_continuation':{'LexicalMembership':{'entities':[list(x.encode())for x in names]}},'membership_continuation':{'Membership':{'entities':persons}},'maximum_two_read':{'ReadCap':{'max_reads':2}},'reads_disabled':'ReadsDisabled'}.get(arm,'Normal')
def expected(w,person,goal,arm='categorical_order'):
 ...
 for _ in range(6):
  ...
  elif arm in['analytical_lexical_membership','lexical_membership_continuation']:emit=q not in[[1]+list(x.encode())for x in names]
...
for name,row,cs,saved_expected,goal,edited_seg in checks:
 ...
 if edited_seg is not None:
  l=next(l for l in labels if l['seg']==edited_seg);start,oldlen=l['object'];oldc=next(c for c in w0['clauses']if c['seg']==edited_seg);c=byseg[edited_seg];delta=len(c['tokens'])-len(oldc['tokens']);newlen=oldlen+delta;l['object']=[start,newlen]
  for part in['subject','marker']:
   if l[part][0]>=start+oldlen:l[part][0]+=delta
  isolated=oldc['tokens'][:start]==c['tokens'][:start]and oldc['tokens'][start+oldlen:]==c['tokens'][start+newlen:]
  editdetails.append({'name':name,'segment':edited_seg,'before':oldc['text'],'after':c['text'],'old_object':decode(oldc['tokens'][start:start+oldlen]),'new_object':decode(c['tokens'][start:start+newlen]),'retained_suffix':decode(c['tokens'][start+newlen:]),'single_object_only':isolated})
...
exe=P(load(ROOT/'attempt.json')['argv'][0]);out['provenance']={'source_matches_submitted':{f['path']:sha(subprocess.check_output(['git','show',HEAD+':'+f['path']],cwd=REPO))==f['sha256']for f in r['running_source']['source_files']},'preserved_executable':str(exe),'executable_matches':sha(exe.read_bytes())==r['running_source']['executable_sha256'],'source_receipt':r['running_source']}
...
# Seal for this corrected replay, separate from the eight immutable historical reports.
...
out['corrected_seal']={'listed':len(listed),'missing':sorted(listed-actual),'unlisted':sorted(actual-listed),'size_errors':[f['path']for f in m['files']if(ROOT/f['path']).stat().st_size!=f['bytes']],'blake3_errors':[f['path']for f in m['files']if blake3.blake3((ROOT/f['path']).read_bytes()).hexdigest()!=f['blake3']]}
original=BASE/'ordinary-form-argument-binding-8';origrows=[json.loads(x)for x in(original/'rows.jsonl').read_text().splitlines()];rowkey=lambda x:(x['split'],x['arm'],x['world'],x['person'],x['goal']);newbykey={rowkey(x):x for x in rows};behavior_fields=['emitted','reads','terminal','selected_segments','selected_roles','correct'];diff=[]
for oldrow in origrows:
 newrow=newbykey.get(rowkey(oldrow))
 if newrow is None or any(oldrow[f]!=newrow[f]for f in behavior_fields):diff.append(rowkey(oldrow))
actual_lexical=out['arms']['exposed_regression/lexical_membership_continuation']
out['original_preservation']={'original_rows':len(origrows),'matched_behavior_rows':len(origrows)-len(diff),'behavior_differences':diff,'model_bytes_identical':{a:b==(original/f'artifacts/ordinary_forms_{a}.json').read_bytes()for a,b in models.items()},'final_panel_repeated':wd['final']==load(original/'worlds.json')['final'],'original_models_unchanged':True}
out['matched_lexical_membership']={'actual_complete':actual_lexical['complete'],'actual_depth_correct':actual_lexical['depth_correct'],'rows':actual_lexical['rows'],'agrees_with_independent_lexical_oracle':actual_lexical['flags_oracles_text_controls_match'],'historical_token_membership_complete':out['arms']['exposed_regression/membership_continuation']['complete'],'scope':'New executed loaded-runtime comparator uses the same exact lexical identity regime as the primary; it intentionally substitutes registry membership for learned role/action semantics.'}
out['saved_evidence_consistency_pass']=out['saved_evidence_consistency_pass']and all(not out['corrected_seal'][k]for k in['missing','unlisted','size_errors','blake3_errors'])and not diff and all(out['original_preservation']['model_bytes_identical'].values())and all(e['single_object_only']for e in editdetails)and actual_lexical['flags_oracles_text_controls_match']
out['qualified_conclusion']='Corrected replay of the authored ordinary-form panel with object-only intervention and actual matched-identity lexical membership comparator. Original models and previous row behaviors preserved. Not a fresh final draw, geometric predictive advantage or broad-language claim.'
P('/tmp/uor-ordinary-corrected-audit.json').write_text(json.dumps(out,indent=2)+'\n')
print(json.dumps({'consistent':out['saved_evidence_consistency_pass'],'arms':out['arms'],'counts':out['counts_before_uncheckpointed_perturbation'],'frame_errors':len(errors),'snapshot_errors':len(snaperrors),'split_scope':out['split_scope'],'membership':{'actual':38,'analytical_lexical':lcorrect},'out':'/tmp/uor-ordinary-corrected-audit.json'},indent=2))
