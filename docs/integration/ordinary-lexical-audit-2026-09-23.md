...
All new evidence is written to newly claimed roots (`olx-audit-1..4`, `olx-channel-probe-1/2`,
...
The decisive record is the sealed root
...
| prose token-conditional bits/target | **7.372198274238398 == 7.372198274238398** |
...
Classification: `decision_tracks_fingerprint = true`, `decision_tracks_feedback = false`,
`no_copy_event_matches_actual = true`, `identity_erased_alias = true`. **The learned post-copy
vocabulary decision is a function of the selected-source evidence fingerprint `m`; it does not depend on
the token actually emitted by `Copy`.** The delivered 2/2 is therefore a content/fingerprint result, not
a demonstrated role for emitted-token feedback. The class words remain declared labels, not semantic
...
- **full-action NLL = token-conditional NLL = 7.3722 bits/target** over 5,376 scored actions; every
  scored development action is a `Generate` (`stop_targets = 0`) because the stratified development
  windows are non-terminal.
- **mean Stop probability 2.28e-4**; `Stop` is never chosen on development. The references are
  token-only; with measured Stop mass this small, the denominator difference is **not** the gap.
- **hidden-state clamp saturation 0.0** and **fingerprint clamp saturation 0.0** — clamping is not the
  cause.
- token-conditional loss **by context frequency** (fit-split count of the exact `(prev, cur)` context):
...
- token-conditional loss **by position** (bucket `i/8`) is roughly flat between 7.10 and 7.59.
- **local identity**: swapping only the penultimate token (same last token) changes the state digest
  **100 %** of the time and the top-1 next token **56 %** of the time; swapping the last token changes
  top-1 93 %. The exact last two token identities are physically present in the 64-coordinate state; the
  readout only partly exposes them, so "the state does not retain local IDs" is **not** the whole story.
...
(two-token − recurrence-only CI `[−0.527, −0.410]`, excludes zero), and both arms remain behind the
count reference. On this matched evidence the principal's "give the shared path a direct bounded
last-two-token channel" hypothesis is **falsified as a sufficient remedy**: the exact local channel is
redundant with the recurrent state and adds optimisation burden.
