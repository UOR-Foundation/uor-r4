...
**Current synthesis:** the [ternary head-projection receipt](../evidence/native_geometric_head_projection_2026-09-20.txt) records the executed experiment. With the parent's embeddings, integer features, bias and `F` frozen and only the output head changed, the existing quantizer **Q0** scores 7.416178 bits/target and one activation-aware dyadic ternary calibration **QG** scores 7.190872, against the incumbent gradient-trained ternary head **E** at 7.170816 and the raw floating head **F** at 6.874144 on the same development panel. **Both conversions fail** the predeclared practical screen against E, while the method-attribution screen **passes** decisively: QG is +0.2253 bits/target better than Q0 [0.200621, 0.250299] and reconstructs F's scores 12.7x better than E does. That measures directly that lower squared-score reconstruction error does not imply lower cross-entropy, and it withdraws the earlier reading that feature rank/capacity had been identified as the dominant bottleneck. E stays the stronger qualified local baseline; a <=4-bit head change, quantization-aware refinement or joint feature training remains a separately justified option rather than an automatic next run. The architectural priority returns to a **query-conditioned geometric read/update** on this strengthened local baseline, with a separable-prefix residual and a matched local-only control. D0-b, prime/zeta/R4/S3/H4/icosian priorities and the absence of a hidden transformer/provider are unchanged; no learned geometric superiority, general language, full-path energy advantage or frontier capability is established.
