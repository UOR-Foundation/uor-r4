...
**September 23 principal update, PRs #1353/#1354:** one shared native Generate/Copy/Stop learner now fits ordinary text and authored grounded cases, but open-development prose remains worse than an exact two-token count reference (7.3722 versus 5.1217 bits/target), and greedy outputs collapse. This is a local-competence/optimization diagnosis, not yet evidence that long-range geometric transport is missing. Post-copy feedback causality is unisolated because pre-copy fingerprint and feedback changed together; broad loaded-artifact parity is unmeasured. First test those boundaries and an equal-data local-ID control, then compare signed H4/shared state against a matched ordinary mechanism for a witnessed structural need. [Principal review](ordinary-lexical-principal-review-2026-09-23.md) · [successor brief](deepseek-ordinary-lexical-successor-2026-09-23.md). Hopf fiber, S7/E8 and harmonic banks remain conditional research tools.
