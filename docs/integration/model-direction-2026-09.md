...
**Current synthesis:** the [frozen S attribution result](s-attribution-result-2026-09-20.md) is the newest executed experiment. S's numerical improvement over E is a **current-token query/emission calibration**, not older-content memory. The older reader row alone is indistinguishable from E (7.171048 vs 7.170816); dropping it improves CE by 0.0073 (new positions) / 0.0100 (old) bits with the paired interval entirely below zero on both panels; and a content-free, length-conditioned fit-average history beats the individual older state by 0.0187 / 0.0236 bits with the interval entirely below zero on both. The identity-anchored constant `u(e)` is harmful. The exact integer identity `Z11+Z00=Z10+Z01` holds with 0 failures and every content-independent condition is exactly invariant to the donor and reversal maps. All 16 retained old vectors reproduce with a 0.0 maximum delta.

**One next step:** implement the smallest mechanism that can make older content **addressable** — a bounded selective write/reset over exact 2I state, or exact occurrence/version access — and qualify it against fresh behavioural criteria. Do not re-fit S, start a width/precision/corpus sweep, or treat the offline `M01` comparator as a serving candidate; repair CPQK production continuation before any new fit.

This closes the *querying* and the *individual-content* hypotheses at their bounded scope and reopens the *writing* hypothesis as the untested part of the state path. One 120-state register holds at most log2(120) ≈ 6.91 bits and bijective transport supplies no forgetting by itself. D0-b and prime/zeta/R4/S3/H4/icosian priorities remain unchanged. No wider head, precision escalation, hidden transformer/provider, full-path energy or frontier claim is selected.
