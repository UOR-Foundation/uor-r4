...
**Delivered:** the [direct read-influence policy](reader-utility-result-2026-09-21.md) is executed and unpromoted. Held-out reader text improves to **−0.0195 bits/token [−0.0329,−0.0055]** (categorical −0.0243) from the frozen parent's **+0.5453**: first net benefit on held-out prose, concentrated on copy-available positions. Absence improves. **`relational_behaviour_preserved = false`** because the expected-cost minimiser reads every candidate at ≈1 nat, so correct payload reads rise while emitted tokens collapse and present-query CE is +7.59 bits/query worse — a contextual-representation limitation, not support or quantization. **Next:** a learned contextual observation that separates reliable from unreliable reads (persisted role/scope or query-relation state). No model promotion.
