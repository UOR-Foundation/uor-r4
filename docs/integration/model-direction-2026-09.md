...
**September 23 principal update, PRs #1353/#1354 and subsequent audit:** one shared native Generate/Copy/Stop learner fits ordinary text and authored grounded cases, and its saved artifact now reproduces every reported panel after fresh-process reload. Open-development prose remains worse than an exact two-token count reference (7.372198 Generate-action versus 5.1217 token-only bits/target); greedy outputs collapse. A separate prose-only full-context recurrence diagnostic reaches 6.2039, but is not a grounded successor, and adding a direct two-token channel worsens that diagnostic to 6.6732. On one crossed pair the post-copy top-1 decision tracks the selected-source fingerprint, while a feedback-only swap does not change it; broader feedback causality remains unverified. This points first to the shared grounded learner's per-position full-context training formulation, retaining Copy/Stop and actual loaded generation, rather than assuming long-range transport is the bottleneck. Compare signed H4/shared state with a matched ordinary mechanism only for a witnessed structural need. [Principal review](ordinary-lexical-principal-review-2026-09-23.md) · [audit and correction](ordinary-lexical-audit-2026-09-23.md) · [research-leader handoff](research-leader-handoff-2026-09-23.md). Hopf fiber, S7/E8 and harmonic banks remain conditional research tools.
