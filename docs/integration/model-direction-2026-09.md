...
**Current synthesis:** the [correcting PR #1314 audit](occurrence-reader-audit-2026-09-20.md) preserves constructed occurrence-selection gains and identifies raw ranking, abstention and overstrong copy integration as the current failure. The raw reader selects correctly in five of ten covered positions and five of 88 total reads. The earlier slot-attribution, causal-control and negligible-overhead claims are narrowed or withdrawn. No candidate is promoted.

**Next mechanism, owner-steered:** learn contextual geometric query/source descriptions and directed H4 relations over exact BPE occurrences. [Single handoff](deepseek-relational-reader-step-2026-09-20.md). A calibrated version of PR #1314 is the comparator; loss-based NoRead/copy-strength learning makes the comparison interpretable. A calibration-only run would leave exact-key admission and shallow equality unchanged. Permit one justified causal candidate-pool correction when those exclude the intended relation, comparing ranking on the same pool. This is a hypothesis to test, not demonstrated H4 advantage.
...
The owner has added **learned role/scope memory followed by evidence-driven reuse of Hopf/fiber or S7 mechanisms** to the research direction. The current memory-utility correction and learning step remains first; [the canonical sequence](project-track.md#structural-memory-and-geometric-representation-follow-up) and [source-linked design](structural-memory-hopf-direction-2026-09-20.md) define the comparisons. Local decay may coexist with event-driven structural retention. Learn roles for occurrences in context and preserve exact payload/version identity; a single subject slot cannot represent nested scopes.
