...
**Current synthesis:** the [PR #1319 review](relational-reader-review-2026-09-20.md) preserves a useful learned finite-reader improvement on construction while withdrawing unique-H4 attribution and completed runtime qualification. The categorical control uses wrong evaluation addresses; source-bank membership can explain intended pair-task selection. Natural-text prediction was not evaluated. No model is promoted.

**Next mechanism:** [competitive query-dependent single reading](deepseek-competitive-reader-step-2026-09-20.md) through one shared causal current-prefix function. Repair identified discrepancies within the constructive run, train against plausible competing sources, and evaluate hard-action utility/generated behavior with a competent learned comparator and bounded natural-text regression. Preserve exact references and directed relations for subsequent query updates. A parity-only replay or immediate two-hop scheduler does not resolve the current uncertainty.
