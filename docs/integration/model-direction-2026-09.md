...
**Current synthesis:** the [prefix-recovery and readout receipt](../evidence/native_geometric_prefix_recovery_readout_2026-09-20.txt) records the executed pair. The three negative prefix candidates were recovered into executable `CPX2` artifacts with an exact, bijection-bound `2I` table (14,400 products verified twice; 0 mismatches against the floating table) and exact reloaded parity, without refitting. The paired fixed-feature output-head diagnostic then shows the **empirical** conditional-target arm at 7.170816 bits/target (**+0.387614** [0.360308, 0.415469] over the frozen parent) and the **smoothed** arm at 7.188585 (+0.369845): the practical screen passes, but smoothed distributional supervision does **not** beat equally dosed empirical refitting on true-label development CE. The conditional floating head on the same features and soft targets reaches 6.874144 with a teacher KL still of 3.2-4.5 bits, against a 5.061160 full-fit reference. The measured bottleneck is therefore the **frozen local feature map**, not the output objective or the target estimator. The next bounded increment is a local representation/rank increase under the same hard integer serving contract and dose; count matching is not a prerequisite. The rejected prefix residual remains separable from the local query, and local-conditioned geometric read/update, selective forgetting and exact memory remain larger architectural obligations. D0-b and geometric priority remain; no useful-language or 2I-superiority claim follows. The [takeover review](takeover-review-2026-09-19.md) preserves the broader history/source map.
