...
**Current synthesis:** the [matched query-read result](query-read-result-2026-09-20.md) is the newest executed experiment. Frozen corrected empirical E remains the numerical incumbent at 7.170816 bits/target and was reproduced exactly. The joint multiplicative read Q beats E by +0.029310 bits but fails the predeclared 0.10-bit practical screen, is **0.109278 bits worse than the equally parameterized separable read S** and 0.040456 worse than the local-only read L, shows no older-content sensitivity under the exact donor map and no demonstrable query-route use. S's query channel is strongly used (+0.190660). All instrument checks pass; every arm's generation remains degenerate. No candidate is promoted. The two small projection source/report defects (the omitted `s0` nearest-seed sweep and the hardcoded `singular:true`) are corrected with focused tests; the complete originally specified projection remains NOT_RUN and no QG campaign was rerun.

**One next step:** continue from the **separable** read S, which is the strongest arm against E so far, and test **selective write/reset**, **exact occurrence/version access** or a **history-capacity** question on it, against a new independent behaviour panel. Do not rebuild the multiplicative read, start a projection/width/precision sweep, or add a fresh corpus. Current state owns the retained artifacts; the negative is scoped to this arrangement and dose, not to geometric language modelling.

This closes the immediate geometric *querying* hypothesis at its bounded scope and reopens the *writing* hypothesis. If selective write/reset or versioned access is useful, then advance toward one model with useful conversation and executed Rust behavior. A single 120-state group register holds at most log2(120) bits and bijective transport supplies no forgetting by itself; the separable read does not change that. D0-b and prime/zeta/R4/S3/H4/icosian priorities remain unchanged. No wider head, precision escalation, hidden transformer/provider, full-path energy or frontier claim is selected.
