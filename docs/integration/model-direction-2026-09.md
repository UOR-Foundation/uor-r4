...
**Current synthesis:** the [principal review after PR #1310](query-read-review-2026-09-20.md) verifies S's numerical improvement to 7.032228 bits/target over E 7.170816, while Q fails its registered screens. S's conditional older-prefix penalty is −0.002298 [−0.014072,+0.008581], so useful older content is not established. Its positive current-token query effect survives the corrected older-present denominator (+0.197210). A better score than fitted Q/L does not isolate the benefit of history, and changing action codes does not exclude optimization limitations.

**One next step:** execute the [frozen S attribution prompt](deepseek-separable-attribution-step-2026-09-20.md), currently NOT_RUN. Separate history, query and constant calibration with exact integer corners/ablations and one offline fit-mean comparator. Repair CPX3's zero tokenizer identity and redundant inference folds; reproduce old vectors and evaluate a fixed nonoverlapping position panel from the existing dev documents. Preserve CPQK as incomplete continuation rather than claim a production resume. No new fit, corpus, width, precision or reset mechanism is selected now.

Geometric state maintenance and exact addressed memory remain the architectural path. A finite right-product register cannot perform learned many-to-one reset; a later translation/reset monoid is a coherent small hypothesis, but does not add capacity or keyed/versioned access. Resolve which part of S is useful before attaching more mechanisms. Integrate with the retained shared-memory/composition path toward useful conversation and executed Rust on one artifact. D0-b and prime/zeta/R4/S3/H4/icosian priorities remain unchanged; no general language, full-path energy or frontier claim follows.
