...
**September 23 formulation milestone:** the recorded per-position full-context question was executed inside the one shared grounded Generate/Copy/Stop learner. Under the served window conditioning — the recurrence the served multi-token path executes — the per-position arm with grounded supervision scores 7.0327 bits/target against the delivered 7.3722 (−0.3395 [−0.4388, −0.2447]) and fully retains the grounded panels (32/32, 3/3, 2/2, 36/36, 4/4); a prose-only window objective reaches 6.6841 (−0.6881 [−0.7686, −0.6091]); a window objective with two grounded draws per step reaches 6.9686 with 31/32, 1/3, 2/2. Because per-position minus window at the same schedule is only +0.0641 [−0.0142, +0.1490], the deficit is a training-curriculum effect, not a formulation or long-range transport effect, and the per-position objective is retired as a necessary change. What remains is local: every arm trails the fit/tune-separated tuned two-token count reference (5.1217) by 1.56–1.91 bits/target, so the 64-coordinate ternary state still under-uses the two tokens available at the decision point, and greedy generation collapses. The next questions are therefore (1) retain the authored panels fully under the served conditioning and (2) close the local two-token deficit before assuming any longer-range mechanism. Compare signed H4/shared state with a matched ordinary mechanism only for a witnessed structural need. [Result](ordinary-lexical-formulation-result-2026-09-23.md) · [audit and correction](ordinary-lexical-audit-2026-09-23.md) · [research-leader handoff](research-leader-handoff-2026-09-23.md). Hopf fiber, S7/E8 and harmonic banks remain conditional research tools.
