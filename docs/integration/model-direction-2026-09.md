...
**Current synthesis:** the [review after PR #1302](ordered-prefix-review-2026-09-20.md) verifies the frozen prior's corrected development gains and preserves the generated repetition. Count-reference superiority on the same inputs is unrecovered local performance, not an isolated capacity or history diagnosis. The next [complete DeepSeek prompt](deepseek-ordered-prefix-step-2026-09-20.md) implements one exact older-prefix group-state channel with learned eight-way actions and a width-16 ternary reader, tested against fixed-action and fitted local-tail controls plus conditional prefix-state exchange. Hard-forward categorical credit is an explicitly biased offline surrogate; actual integer artifacts decide the result. The [canonical dependencies](project-track.md#research-dependencies-and-exit-conditions) then lead toward selective state/read behavior, exact memory and integrated capability. The [takeover review](takeover-review-2026-09-19.md) retains the broader source/history map. D0-b and geometric priority remain; the pilot is NOT_RUN and does not establish 2I superiority or useful language.
