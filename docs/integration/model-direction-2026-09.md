...
**September 23 state-probe diagnostic:** the served local deficit was decomposed instead of assumed. On the same development targets (restricted to the 92.15% covered by the top-1024 target classes, full-set normalisation), the artifact's own served readout scores **6.055956** bits/target, a tuned `unigram + bigram(cur)` count model **5.565123**, the tuned `(prev, cur)` count model **4.784593** and the fit-only unigram **8.669572**; a converged float linear readout of the artifact's **own input** reaches **5.5136** (last-epoch fit delta 0.0119 bits), so `P − A = −0.5423` and at most ~0.14 bits of that is the probe's class-restriction convention — leaving ≥0.40 bits of genuine headroom that the served output layer is not using. A training-free decode recovers the immediately preceding token from the state at **0.9674** top-1 while the token before it sits at **0.0206**, so the state carries first-order local context and not second-order context. The decomposition is therefore: about 0.54 bits output-side and recoverable on the same state, about 0.73 bits second-order structure the state does not supply; the earlier two-regime plateau at ≈6.69 is a shared output-layer ceiling, not a recurrence limit. Post-hoc re-pricing of a fitted readout into the servable alphabets is destructive (7.62–8.97), so only in-class training can judge the served class, and one initialisation discrepancy (epoch-0 6.5954 > `A`, which renormalisation forbids) remains unresolved and is not relied on. The next questions are therefore (1) can an in-class readout refit realise the headroom on the frozen state with the grounded responsibility retained, and (2) does explicit second-order access to the two-token context pay against an equal-information ordinary control. Nothing measured here witnesses a long-range or geometric need. [Result](ordinary-lexical-state-probe-result-2026-09-23.md) · [audit and correction](ordinary-lexical-audit-2026-09-23.md) · [research-leader handoff](research-leader-handoff-2026-09-23.md). Hopf fiber, S7/E8 and harmonic banks remain conditional research tools.
