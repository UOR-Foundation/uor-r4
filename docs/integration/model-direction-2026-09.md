...
**Current synthesis:** the [exact-occurrence reader result](occurrence-reader-result-2026-09-20.md) is the newest executed experiment. On top of the frozen local baseline `z_local = z_E + u_S(b)` a 128-token causal ring admits exact past occurrences of the current token and a ten-scalar signed ≤4-bit selector chooses one or NoRead. On the geometry-only family — where the correct source's role is a *different* token sharing its transported write slot, so exact identity cannot identify it — reader hard accuracy is **0.825** versus **0.000** for `z_local` (interval [0.768, 0.880]), and the transported-slot feature contributes **+0.275** over the geometry-disabled control. Altered-source, causality, staleness, export/reload and cross-process continuation controls all pass.

**Abstention is the failure.** All-position loss regresses **+0.671** bits/token on the fresh synthetic panel (tolerance +0.01) and **+2.789** on the bounded raw-text probe (tolerance +0.02), with **83 false copies in 88 reads**. Coverage on raw text is 10/488, so the admitted-but-uncovered stratum dominates; a selector trained only on a construction where admission nearly implies usefulness cannot learn that a repeated key is usually not a copy opportunity. Three earlier versions are preserved sealed as near misses, including one whose 4-bit threshold landed exactly on the correct candidate's score.

**One next step:** add **one declared abstention signal** learned from natural-text positions where a candidate exists but no exact successor is useful, then re-evaluate the same frozen panels and controls. Do not re-fit this selector, widen the ring or candidate bounds, or sweep width/precision/corpus. CPQK production continuation remains required before any fit that uses QueryTrainer; the occurrence reader's own ten-scalar trainer already has a working cross-process continuation and does not need it.

This confirms that useful contextual access is a *selection-and-abstention* problem, not a capacity problem. One 120-state register holds at most log2(120) ≈ 6.91 bits; the retained ring is a separate, already-exact component. D0-b and prime/zeta/R4/S3/H4/icosian priorities remain unchanged. No wider head, precision escalation, hidden transformer/provider, full-path energy or frontier claim is selected.
