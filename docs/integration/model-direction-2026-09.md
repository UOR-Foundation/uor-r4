...
**Current synthesis:** the [review after #1300](frozen-prior-review-2026-09-20.md) preserves the real-text prior's recorded loss improvement but withdraws its complete gate pending corrected permutation and document aggregation. The next action is evaluation-only replay of the existing final artifact, count references and a diagnostic of actual greedy pair cycles. Refitting a smaller corpus does not repair that result. The [canonical dependency sequence](project-track.md#research-dependencies-and-exit-conditions) then tests one learned older-prefix state/read and exact memory before integrated capability; additional prior training or perfect fluency is not an automatic prerequisite. The [takeover review](takeover-review-2026-09-19.md) retains the broader source/history map. Use newest [current state](current-state.md) and the [complete DeepSeek prompt](deepseek-frozen-prior-step-2026-09-20.md). D0-b overrides historical blanket serving exclusions; this local prior has not established geometric advantage. Recent state-tracking and distance-decoder research informs the scoped comparisons in the review, not a replacement architecture by headline.
