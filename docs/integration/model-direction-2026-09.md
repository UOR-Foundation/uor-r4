...
**Current synthesis:** the [review after #1300](frozen-prior-review-2026-09-20.md) withdrew the real-text prior's complete gate; its **evaluation-only replay has now been executed** and is recorded in the [replay receipt](../evidence/native_geometric_frozen_prior_replay_2026-09-20.txt). The replay reproduces the recorded step-512 losses to ~1e-15 bits/target, repairs the occurrence permutation and the window-as-document bootstrap, recovers the original exposure (4,096 consumed windows = 257,113 targets) and passes the frozen 0.10-bit threshold on both the legacy and a spread-position panel. It also measures the seam for the next increment: an interpolated order-2 count reference on the same `(t-2, t-1)` input reaches 3.9691 bits/target against the model's 7.1425, so the local readout — not exposure — is the bottleneck. The [canonical dependency sequence](project-track.md#research-dependencies-and-exit-conditions) therefore tests **one learned ordered prefix-state/read channel** with equal-tail, state-disabled, order and matched capacity/cost controls, then exact memory, before integrated capability; additional prior training or perfect fluency is not an automatic prerequisite. The [takeover review](takeover-review-2026-09-19.md) retains the broader source/history map. Use newest [current state](current-state.md). D0-b overrides historical blanket serving exclusions; this local prior has not established geometric advantage. Recent state-tracking and distance-decoder research informs the scoped comparisons in the review, not a replacement architecture by headline.
