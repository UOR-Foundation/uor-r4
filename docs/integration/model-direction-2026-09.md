...
**Current synthesis:** the [principal review after PR #1308](geometric-query-review-2026-09-20.md) verifies that corrected empirical E remains the numerical incumbent at 7.170816 bits/target. Executed QG is better than Q0 by 0.225306 bits but worse than E by 0.020057 bits. The complete originally specified search was not executed: the optimized nearest seed at s0 is absent. Reported Gram singularity is unmeasured. Those source/report defects scope the conclusion without invalidating the retained development vectors. E/S tokenizer metadata has been corrected as the sole byte change. Short cyclic generation persists and no useful-language qualification follows.

**One next step:** implement and execute the [matched geometric query-read prompt](deepseek-geometric-query-step-2026-09-20.md), currently NOT_RUN. Frozen E receives either a joint older-state/current-query read, a separable older-state/query read or a matched local-only read. All have identical parameter shapes and two reader accesses. The test isolates nonseparable querying; it does not add history-register capacity or selective write/reset semantics. Real-text improvement over E and both fitted controls, an exact conditional older-prefix intervention and query-route sensitivity are required for a positive result. The small projection-loop/report repair needs focused tests, not another full calibration campaign.

This restores geometric context access as the immediate research focus. If useful querying emerges, advance toward selective writes/reset and exact occurrence/version memory, shared composition and one model with useful conversation and executed Rust behavior. A single 120-state group register and bijective transport do not supply arbitrary memory or semantic advantage. D0-b and prime/zeta/R4/S3/H4/icosian priorities remain unchanged. No wider head, precision escalation, hidden transformer/provider, full-path energy or frontier claim is selected.
