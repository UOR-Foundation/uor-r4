...
**September 23 readout-refit milestone:** the state probe's artefact-initialisation discrepancy is resolved — a softmax **base** convention worth exactly `ln 2` — and the matched reference is now `A_kclass = 5.977039` at K=1024, confirming the published output-side headroom at **0.4634 bits** (`A_kclass − P`; the class restriction itself is worth 0.078917 bits of mass). An **in-class (served-alphabet) readout refit on the frozen state**, with an asserted exact initialisation (deviation 0 over 262,272 logits), delivers a **positive but bounded** result: **served loss 6.5528 against the artifact's 6.6841 (−0.1313 bits)** end-to-end through the artifact's own integer kernel, and **−0.1435** against its matched K-class reference — with the **integer realisation equal to the float surrogate**, so the ternary alphabet is not the constraint and the limit is the states the readout is fit on, not the way they are expressed. The pre-declared ≥0.30-bit bar is **not met**. Two retractions are recorded: this milestone's first-draft "the integer realisation is the limit" conclusion was an artifact of a defective refit initialisation, and the previous record's post-hoc re-pricing figures came from a degenerate quantisation. The next questions are (1) does a **joint** fit with the output layer initialised from the converged refit recover more than the 0.13 bits a frozen-state refit reaches, and (2) does explicit **second-order local access** pay, given that a training-free decode recovers `cur` at 96.7% and `prev` at 2.1%. Nothing measured here witnesses a long-range or geometric need. [Result](ordinary-lexical-readout-refit-result-2026-09-23.md) · [audit and correction](ordinary-lexical-audit-2026-09-23.md) · [research-leader handoff](research-leader-handoff-2026-09-23.md). Hopf fiber, S7/E8 and harmonic banks remain conditional research tools.
