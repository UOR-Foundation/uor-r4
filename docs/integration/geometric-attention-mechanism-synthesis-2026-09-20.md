...
**Current sequencing after PR #1342:** retain learned categorical marker decoding and variable-depth exact-token answers; principal corrections bind the loaded runtime to actual artifacts/observed questions and repair spans, EOS, restore and evidence. [Review](observed-text-session-review-2026-09-22.md). The next milestone is [contextual occurrence roles and exact spans from ordinary text](deepseek-contextual-text-roles-step-2026-09-22.md), not more disjoint marker labels. Shared vocabulary in different roles and changed word order expose the global token-vote representation loss. Reuse full ordered/relative H4, occurrence-role/span learning and owned session interfaces; compare equal-information contextual controls. Constructive Q8 computation, structural banks, retained Hopf fiber/Spin and finite fields remain tools; paired-H4/E8/S7 follows a demonstrated distinction or quality/cost need. Broad prose, durable corrections and executed Rust remain unqualified. Dated next-task statements below are historical.
