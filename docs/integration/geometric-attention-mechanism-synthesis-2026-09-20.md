...
**Current sequencing, September22 principal review of PR #1351:** the seven-slot action recurrence is retained, while transferable lexical learning remains open. [Review](state-conditioned-lexical-review-2026-09-22.md) and [next execution brief](deepseek-transferable-lexical-step-2026-09-22.md) connect exact observed address/value meaning with actual emitted-token state and ordinary-text learning. Signed ordered transport and shared contextual residuals remain preferred candidates with equal-information ordinary controls; broader E8/S7/harmonic state remains available for explicit order/interference/cost needs. The mechanism inventory below remains useful; its older next-task wording is historical.
...
Earlier dated directions below are historical; the canonical plan above owns current sequencing.

**September22 consumed-state principal review:** [review](consumed-geometric-state-review-2026-09-22.md) verifies that retained signed state changes a downstream record and answer, while a competent ordinary finite table ties. This is a concrete projection/information-loss witness, not unique geometric or quantum advantage. The new artifact preserves 17/38 old primary outcomes against migrated parent38/38; next is [combined learned observed ingestion/control/lifecycle](deepseek-observed-computation-lifecycle-step-2026-09-22.md), then E/S language. Primary typed-world flags are supplied, so richer S7/E8/harmonic coordinates do not repair that missing learned interface. Earlier then-next directions below are historical.
