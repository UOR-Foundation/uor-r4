...
**Current sequencing after PR #1335:** [review](contextual-emission-review-2026-09-21.md) preserves the reported noncopy signal and withdraws the capacity-only inference. [Next](deepseek-consistent-emission-step-2026-09-21.md) learns at a consistent hard-forward boundary. The review derives a conditional finite-orbit cancellation obstruction for `h(q1)-h(q0)` and a relative-update alternative `h(inverse(q0)*q1)-h(identity)`. Test that bridge only on an actual witness; it reuses H4 inverse/transport and precedes unnecessary dimension expansion. Exact ownership, retained Hopf fiber and S7/E8/harmonic tools remain available for measured needs. Dated next-task statements below are historical.
