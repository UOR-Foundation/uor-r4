...
**Current sequencing after PR #1337:** the [principal review](geometric-computation-review-2026-09-21.md) retains the corrected learner's improvement and selects a [learned decoder on computed geometric result](deepseek-derived-state-decoder-step-2026-09-21.md). Relative state removes a verified nuisance frame; explicit validity keeps a legitimate identity result distinct from NoRead. A nonlinear difference-of-potentials alone retains finite-orbit cancellation. Preserve query context when meaningful, and separate requested operation from source-compatibility relation if their information needs differ. The corrected composition fixture tests combinations of familiar primitives; old negative results and conditional Hopf/Spin, finite spectral, role/scope and paired-H4/E8/S7 mechanisms remain preserved. Codex retains holistic leadership; DeepSeek has substantive research autonomy. Dated next-task statements below are historical.
