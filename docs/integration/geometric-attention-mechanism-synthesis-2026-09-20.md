...
**Current sequencing after PR #1340:** retain exact observed Q8 factorization and corrected owned-state/fixed dependent chain; advance [learned relational access and content-dependent control](deepseek-learned-relation-control-step-2026-09-21.md). The [principal review](grounded-session-review-2026-09-21.md) separates compact shared computation from serialization/cost claims, actual loaded serving from parameter parity, immutable captured values from current references, and fixed scheduling from learned continuation. Query geometry should select the requested relation among plausible current facts; arbitrary world relations need not be invertible. Signed H4/Q8 and retained Hopf fiber preserve order, structural banks preserve role/scope distinctions, and finite fields can support measured compatibility. Paired-H4/E8/S7 follows a concrete unmet need. Codex retains principal leadership; DeepSeek has substantive research autonomy. Dated next-task statements below are historical.
