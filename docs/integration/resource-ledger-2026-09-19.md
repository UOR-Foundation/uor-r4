...
**Correction at 2026-09-23T01:50:33Z:** the immediately preceding revision used an incorrect unit conversion: 15,000,000 ms is **250 minutes**, not 15 minutes. Less than 19 minutes had elapsed since recovery, so neither the initial 7,200,000-ms estimate nor the 15,000,000-ms revision was close to exhaustion. The proposed +12,000,000-ms allowance is **withdrawn without use**; the live limit is restored to **390,700,000 ms**, cumulative remains **372,836,017 ms** pending actual charge, and the conservative 15,000,000-ms projection remains. No paid compute or deletion occurred. The receipt preserves the mistaken revision and its correction rather than silently rewriting history.

Principal review charged once at 2026-09-23T01:51:17Z: **1,579,623 ms** = 1,159,623 ms measured since the 01:31:58Z recovery clock + 120,000 ms earlier recovery estimate + 300,000 ms protected-delivery allowance. New cumulative **374,415,640 / 390,700,000 ms**. [Charge receipt](../evidence/transferable-lexical-principal-charge-2026-09-23.json) binds the calculation. The reviewed release build and unchanged-artifact replay passed; 48 distinct focused library tests passed, and the six owning issues plus five knowledge records/five edges were synchronized. Free space at charge was **34,188,107,776 bytes**, above the 24 GiB working reserve plus 128 MiB stop margin. No unique evidence was deleted and no paid compute was used. Protected PR merge is verified separately; queue acknowledgements are not these local tests.

### September 23 — ordinary lexical delivery reconciliation

DeepSeek's PR #1353/#1354 handoff reports a **3,500,000-ms** cumulative charge, taking the prior **374,415,640** to **377,915,640 / 390,700,000 ms**. The live local `model-time.json` contains that latter value. The prior ledger text omitted this delivery charge; this entry records it **once**. The handoff does not provide an independently reconstructible step-by-step wall-time breakdown, so the increment is attributed to the operator report and live ledger rather than invented precision. The [principal review](ordinary-lexical-principal-review-2026-09-23.md) preserves the delivered result and qualifies its claims. No local time extension or paid compute is implied by this reconciliation.

The principal review began with **25,114,939,392 bytes** physically free, below the working reserve plus stop margin. It removed only **8,354,021,376 physical bytes** of inactive, reproducible `target/debug` output from the already-merged PR #1354 worktree; physical free rose to **33,468,960,768 bytes** before creation of the isolated review worktree. Sealed roots, source, models and unique evidence were preserved. A [prospective projection](../evidence/ordinary-lexical-principal-projection-2026-09-23.json) governs the subsequent focused validation and its final actual charge will be appended separately.
...
## Causal continuation checkpoint, September24 UTC (September23 New York)

The prospective local allowance extension was5,400,000ms, retaining the prior442,674,193ms cumulative charge and setting the limit to450,100,000ms. The task cap remains5,400,000ms, two CPU workers,2GiB model memory,4GiB compilation memory and2GiB projected new storage; no paid compute. Through2026-09-24T01:02:52Z, complete elapsed work is charged3,765,127ms, bringing the shared ledger to446,439,320/450,100,000ms. Native/server overlap is counted once, including the recovered connector interruption. Protected-delivery tail is accounted separately.

At this checkpoint, physical free space is54,674,399,232bytes, above the25,904,021,504-byte reserve. Investigation allocation is160,518,144bytes and the new retained-candidate root is22,941,696bytes. Complete logical sizes of release-cache files modified during the task total248,712,708bytes, a conservative retained-file count rather than a temporary-peak or net-cache-increment measurement. The existing worktree was reused. Two other clean redundant worktrees were removed only after verifying no unique files and preserving their commit-bearing branches.

See [checkpoint evidence](../evidence/causal-resource-checkpoint-2026-09-24.json), [the result](causal-continuation-result-2026-09-24.md), and its final verification/cleanup receipts. No energy measurement or blanket allocation-free model claim is inferred from these resource counts.

## Prospective KVAR hard-select successor, September 24, 2026

The next bounded milestone is to align KVAR `(c)` training with its hard served selection, reject empty table reads, and evaluate ordinary `(f)` and geometric `(h)` at matched parameter-access cost if the repaired `(c)` is viable. The panel, seeds, control thresholds and geometry margin remain those frozen in `kvar-plan-2026-09-24.md`; exploratory smoke attempts use separate roots and do not replace the frozen held-out panel. The original checkout, prior sealed roots and negative candidates are retained.

Complete projected allowance is **6,000,000 ms**: source/contract review and Rust changes 1,600,000; one reused-cache build and focused checks 900,000; bounded smoke and diagnosis 900,000; two-seed 1,500-step fitted arms plus controls and evaluation 1,600,000; receipt, interpretation and protected delivery 1,000,000. One Cargo process, at most two model threads, peak 4 GiB compilation RAM and 2 GiB model RAM. Incremental temporary/build storage is capped at 700 MiB, new retained evidence at 30 MiB, and the 128 MiB stop margin remains in addition to the 25,904,021,504-byte working reserve. Physical free after verified removal of a clean duplicate checkout is 27,168,153,600 bytes (26,531,400 KiB), so the projected 730 MiB is within the remaining physical headroom. No paid/external compute or artifact deletion. Stop/checkpoint and record a new projection before any increase.

The shared model ledger before execution is **454,839,320 / 458,000,000 ms**. The standing owner authorization is applied prospectively to increase only the local limit by **3,200,000 ms**, to **461,200,000 ms**, leaving 6,360,680 ms available; the cumulative charge is unchanged at projection time. Charge actual complete elapsed work once after the milestone.

**Final complete charge and storage check.** The full local milestone through source review, failed and successful fits, fresh-panel comparison, report repair/replay, focused validation, protected PR #1380 and issue #820 handoff is conservatively charged **2,000,000 ms** once. The shared ledger advances from **454,839,320 to 456,839,320 / 461,200,000 ms**; 4,360,680 ms remain. Thirteen new sealed diagnostic/final roots have a total reported logical allocation of **26,208 KiB**. Physical free after PR and issue handoff is **26,488,896 KiB = 27,124,629,504 bytes**, compared with 26,531,400 KiB before the runs; the net free-space decrease is 42,504 KiB. It remains **1,086,390,272 bytes** above the 25,904,021,504-byte working reserve plus the 128 MiB stop margin. The existing isolated worktree/build cache was reused. No paid compute, deletion of unique artifacts or modification of the owner's tracked checkout occurred. Ordinary `(f)` and geometric `(h)` were not run; the prospective allowance was not spent to force a geometry claim.

## Prospective matched relative-energy pilot, September 24, 2026

The next bounded work is the [frozen KVAR relative-energy pilot](kvar-relative-energy-plan-2026-09-24.md): compare ordinary cyclic-index delta-memory read energy with exact Q8 relative read energy and transport-off, using identical sealed `(c)` parameters and the same KVAR generator. This tests a narrow learned residual; it does not silently relabel the original full `(f)/(h)` arms. The open parent PR #1380 is preserved and this work uses a new stacked branch in the same isolated full worktree to reuse its build cache under the physical storage constraint.

Complete projection is **3,600,000 ms**: source/design and prospective registration 900,000; Rust implementation, focused tests and one reused-cache release build 1,200,000; smoke diagnosis 300,000; one locked two-seed fit plus fresh group-4 evaluation and retained controls 500,000; interpretation, evidence, PR and issue handoff 700,000. One Cargo process at a time, one fitting process and at most two CPU threads. Peak projected RAM is 4 GiB during compilation and 1 GiB for the finite fit. Incremental temporary/build storage is capped at 300 MiB and unique retained reports at 20 MiB. Physical free before this work is **26,384,184 KiB = 27,017,404,416 bytes**, leaving **979,165,184 bytes** above the 25,904,021,504-byte working reserve plus the 128 MiB stop margin; the 320 MiB combined projection fits. Stop/checkpoint and record a new prospective increment if either ceiling would be exceeded. No paid or external compute, and no artifact/worktree deletion.

The live shared ledger is **456,839,320 / 461,200,000 ms** at registration; 4,360,680 ms remain. The 3,600,000-ms projection fits **without an allowance extension**. Cumulative charge and limit remain unchanged until actual work is charged once at completion.

**Final complete charge and storage check.** Live-state/source review, design registration, Rust implementation and focused checks, quick/development and one fresh final fit/evaluation, evidence/report, protected stacked PR #1381 and issue #820 handoff are charged **1,100,000 ms** once. This is conservative wall-clock accounting of the complete milestone, including the pre-branch review. The shared ledger advances from **456,839,320 to 457,939,320 / 461,200,000 ms**; **3,260,680 ms** remain. The three new sealed roots use **5,668 KiB** reported logical allocation. Physical free after delivery is **26,324,756 KiB = 26,956,550,144 bytes**, a **59,428-KiB** decrease from the pre-work snapshot. It remains **918,310,912 bytes** above the 25,904,021,504-byte working reserve plus the 128 MiB stop margin. The existing isolated worktree/build cache was reused. No paid/external compute, unique-artifact deletion or tracked change in the owner's original checkout occurred. The residual failed the pre-registered two-seed geometry gate; original full `(f)/(h)` arms remain unrun.

## Prospective addressed-memory/lexical bridge, September 24, 2026

The [frozen integration gate](addressed-lexical-bridge-plan-2026-09-24.md) couples the existing exact versioned `Memory` to the loaded native `TlModel` through selected owned tokens and truthful typed facts. It adds no training run. Complete projected allowance is **2,500,000 ms**: live state/source and design 500,000; Rust bridge/runner and focused checks 900,000; reused-cache release build and loaded/reloaded causal panel 500,000; receipt, interpretation, resource charge and protected delivery 600,000. One Cargo process, at most two CPU threads, peak projected 4 GiB compilation RAM and 1 GiB execution RAM. Incremental temporary/build storage is capped at 300 MiB and new retained reports at 20 MiB. Physical free before implementation is **26,556,908 KiB = 27,194,273,792 bytes**, leaving **1,156,034,560 bytes** above the 25,904,021,504-byte reserve plus 128 MiB stop margin; the combined 320 MiB projection fits. Stop and record a prospective increment if a ceiling is approached. No paid compute or deletion. The shared ledger is **457,939,320 / 461,200,000 ms**, with 3,260,680 ms available; this projection fits without a limit increase and the cumulative charge is unchanged until completion.

**Build-path correction before further execution.** The first `cargo test --lib addressed_lexical_bridge` attempted a full, optimised debug **lib test harness** and created an incomplete incremental working directory of **1,117,704 KiB**, far above the 300-MiB projection. It was interrupted before tests ran when physical free fell to **25,433,560 KiB**, only **5,592 KiB** above the reserve plus margin. After confirming the compiler was stopped and that the new directory was its incomplete `-working` cache, that directory alone was removed; free recovered to **26,564,584 KiB**. No research artifact, completed binary, sealed root or original-checkout file was removed. This is a failed build attempt, not a passed test. The revised path uses one reused-cache **release binary build** and exercises the focused causal cases through the actual runner, with no lib test-harness build. Before that next build, the additional release build/output projection is **≤300 MiB** from the recovered checkpoint, retained reports **≤20 MiB**, peak RAM **4 GiB**, and the same 2,500,000-ms complete wall-clock cap. The recovered physical headroom is **1,163,894,784 bytes**; the 320-MiB revised increment fits. Stop immediately on renewed unexpected cache growth; no silent physical allowance increase.

**Corrected-attempt projection before recompilation.** The first optimised runner built within the revised disk projection, then sealed `final1` and separate-process `replay1`. Inspection found the parent receipt's premature reload success and an unsafe ungrounded fallback on missing evidence. The [correction](addressed-lexical-bridge-correction-2026-09-24.md) freezes the same panel and repairs reporting/typed abstention in `final2`/`replay2`. One additional reused-cache release compile is projected at **≤200 MiB incremental**, corrected reports **≤10 MiB**, with the same 4-GiB RAM bound; total complete milestone wall time remains below the recorded 2,500,000-ms cap. No limit extension or new external cost. Preserve `final1` and `replay1` as the exposed defective attempt.

**Additional scoped-boundary projection before final3.** `final2`/`replay2` confirmed the corrected 20-row interface. Two directly untested typed branches, `NoHistory` and wrong-scope absence, warrant a final 24-row rerun with the same frozen value pairs. Only the runner changes; the loaded model and bridge source are unchanged. The reused-cache **binary-only** rebuild is projected at ≤100 MiB incremental, new reports ≤5 MiB, peak RAM ≤1 GiB for the binary build, within the original 2,500,000-ms complete cap and current physical reserve. Preserve both prior pairs of roots.

**Binary-reload correction projected before final4.** Review of `final3`/`replay3` found the `Memory::from_bytes` check discarded its result before generation. The [correction](addressed-lexical-bridge-correction-2026-09-24.md) requires each actual row to consume the binary-decoded memory. Before rebuilding, this adds one reused-cache binary-only compile projected at ≤100 MiB incremental, two new sealed roots ≤5 MiB, peak RAM ≤1 GiB for the binary build, and ≤300,000 ms additional wall time; the shared 2,500,000-ms complete task cap and 461,200,000-ms cumulative limit are unchanged. Current physical free is 26,555,012 KiB, leaving 1,154,093,056 bytes above reserve plus margin, sufficient for the incremental projection. Preserve the previous six sealed roots.

**Final complete charge and storage check.** Complete source/history review, pre-registration, the interrupted lib-test build and validated removal of only its incomplete working cache, release builds, all eight sealed attempts, correction/replay, checks, result/plan updates, protected stacked PR #1382 and issue #820 handoff are conservatively charged **2,100,000 ms once**. This is whole-milestone wall-clock accounting, including engineering and delivery, below the prospective 2,500,000-ms task cap. The shared ledger advances from **457,939,320 to 460,039,320 / 461,200,000 ms**; **1,160,680 ms** remain. The eight sealed roots together use **464 KiB** reported allocation. Physical free after issue handoff is **26,520,596 KiB = 27,157,090,304 bytes**, versus 26,556,908 KiB before implementation, a net decrease of 36,312 KiB; free remains **1,118,851,072 bytes** above the 25,904,021,504-byte working reserve plus 128 MiB stop margin. The failed debug lib harness ran no tests. The reused-cache release binary built and executed actual loaded and independent-replay controls. No paid compute, unique-artifact deletion or tracked edit to the owner's original checkout occurred. No general-language, geometric, sparse-serving or energy qualification follows.

## Prospective real-text sparse-recall opportunity gate, September 24, 2026

The [frozen plan](realtext-sparse-recall-plan-2026-09-24.md) asks whether a finite exact-pair cache plus a Tune-fitted dyadic age gate improves source-separated natural-text prediction over the retained count family. It is the next M1-to-M3 architecture decision, not another authored answer panel. Complete projected work is **2,500,000 ms**: live-state/source/literature review and registration 700,000; one Rust runner and focused self-check 800,000; one reused-cache offline release build plus bounded Fit/Tune/Dev evaluation 500,000; interpretation, report, protected PR and issue handoff 500,000. One Cargo process, ≤2 evaluation threads, peak 4 GiB compilation RAM and 2 GiB execution RAM; context is at most 4,096 tokens per Dev document, Fit/Tune token caps 400,000/100,000; temporary build increment ≤200 MiB and retained evidence ≤30 MiB. At registration physical free is **26,524,276 KiB = 27,160,858,624 bytes**, leaving **1,122,619,392 bytes** above the 25,904,021,504-byte working reserve plus the 128 MiB stop margin; the combined 230 MiB fits. Stop before reserve/margin or task cap, and record a new prospective increment if genuinely needed. No external/paid compute or unique-artifact deletion.

The live shared ledger is **460,039,320 / 461,200,000 ms** (1,160,680 ms remaining). Under the standing owner authorization, record a necessary **2,000,000-ms local time allowance extension** before build/evaluation: new limit **463,200,000 ms**, cumulative unchanged, leaving 3,160,680 ms available for the 2,500,000-ms complete projection. Charge actual complete elapsed work once at delivery.

**Final complete charge and storage check.** Live-state/source/history and primary-literature review, registration and the prospective local extension, one Rust runner/offline release build, one sealed Fit/Tune/Dev run, independent read-only row reaggregation, result/plan interpretation, checks, protected stacked PR #1383 and issue #820 handoff are conservatively charged **1,100,000 ms once**, under the recorded 2,500,000-ms complete cap. The shared ledger advances from **460,039,320 to 461,139,320 / 463,200,000 ms**; **2,060,680 ms** remain. The single sealed root uses **7,740 KiB** reported logical allocation. Physical free after issue handoff is **26,496,156 KiB = 27,132,063,744 bytes**, a **28,120-KiB** decrease from the 26,524,276-KiB registration snapshot; free remains **1,093,824,512 bytes** above the 25,904,021,504-byte working reserve plus the 128 MiB stop margin. The isolated worktree/build cache was reused, no model artifact was trained or modified, and no paid/external compute, unique-artifact deletion or tracked owner-checkout edit occurred. This in-domain sparse-recall opportunity result does not qualify the loaded native model, geometry, end-to-end sparse serving or energy.

## Prospective loaded native sparse-read gate, September 24, 2026

The [frozen plan](native-sparse-read-plan-2026-09-24.md) integrates a two-source exact-pair store and a small learned hard read selector with the unchanged loaded native `TlModel`, then evaluates source-separated prose and Rust code with actual generation. Complete projected local work is **4,500,000 ms**: authority/source/data review and registration 600,000; Rust mechanism, gate and focused self-check 1,500,000; one reused-cache release build 600,000; bounded Fit/Tune/Dev scoring and generated controls 500,000; interpretation, receipt, documentation, resource charge and protected delivery 1,300,000. One Cargo process and one runner process at a time, at most two CPU threads, peak 4 GiB compile RAM and 2 GiB evaluation RAM. Incremental temporary build/source storage is capped at **250 MiB**, unique retained source/evidence at **40 MiB**. Physical free at registration is **26,489,800 KiB = 27,125,555,200 bytes**, about **1,087,315,968 bytes** above the 25,904,021,504-byte working reserve plus 128 MiB stop margin; the combined 290 MiB projection fits. Stop/checkpoint and record a prospective increment before exceeding a ceiling. No paid/external compute, unique-artifact deletion or tracked edit to the owner's checkout.

The live shared ledger before execution is **461,139,320 / 463,200,000 ms** (2,060,680 ms remaining). Under the owner's standing authorization for necessary local extensions, the limit is increased **3,000,000 ms to 466,200,000 ms**, cumulative unchanged, leaving **5,060,680 ms** to cover the 4,500,000-ms complete projection. Charge actual whole-milestone time once after protected delivery.

**Final complete charge and storage check.** Complete live-state/source review, prospective registration and local extension, Rust mechanism/runner, one reused-cache release build, the one sealed Fit/Tune/Dev run with actual generation, independent row audit, result/plan/current-state interpretation, focused checks, protected stacked PR #1384 and issue #820 handoff are conservatively charged **1,700,000 ms once**, below the 4,500,000-ms projected cap. The shared cumulative ledger advances from **461,139,320 to 462,839,320 / 466,200,000 ms**; **3,360,680 ms** remain. The retained original book inputs and sealed final report use **700 KiB** and **240 KiB** of reported logical allocation. Physical free after PR/issue handoff is **26,488,828 KiB = 27,124,559,872 bytes**, a **972-KiB** decrease from the 26,489,800-KiB registration snapshot, still **1,086,320,640 bytes** above the 25,904,021,504-byte reserve plus 128 MiB stop margin. The isolated full worktree and warm release cache were reused. No paid/external compute, unique-artifact deletion or tracked edit to the owner's original checkout occurred. The four-bit admission gate failed its matched ordinary-cache comparison and was not tuned after Dev exposure.
