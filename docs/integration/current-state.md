...
## Active: learned exact-occurrence reader — EXECUTED, targeted selection works, abstention fails


The [result record](occurrence-reader-result-2026-09-20.md), [design note](occurrence-reader-design-2026-09-20.md) (written before scoring) and [evidence receipt](../evidence/native_geometric_occurrence_reader_2026-09-20.txt) report one complete constructive learning task from base `1dae322c`, sealed and verified at `.uor-models/realtext-prior-2026-09-20/occurrence-reader-4` (21 files, 3,385,009 bytes). Three earlier roots are preserved sealed as near misses. No output projection, width/precision search or reset campaign was run.

**Local baseline.** `z_local(v) = z_E(v) + u_S(b)` for `i ≥ 2`, else `z_E`, where `b` is the frozen separable artifact's query state. No history row is added and **no history fold is executed**; read-disabled reproduces `z_local` exactly at all 978 checked positions.

**Mechanism.** A 128-token causal ring with session sequence identity; at most 24 candidates admitted per position by exact token equality from the causal prefix only; nine 0/1 features; a ten-scalar signed ≤4-bit selector with an explicit NoRead threshold; the selected occurrence's exact observed successor emitted as `+2^14`. Artifact 130 bytes; export/reload decision mismatches 0.

**What works.** On the fresh `geom_slot_only` family — where the correct source's role is a *different* token sharing its transported write slot, so only the geometric feature can identify it — reader hard accuracy is **0.825** versus **0.000** for `z_local`, paired interval **[0.768, 0.880]**. The latest-occurrence control scores 0.533 and geometry-disabled 0.550, so **the transported-slot feature contributes +0.275 absolute**. 10/10 covered raw-text positions were read correctly. Altered-source, causality, staleness and cross-process continuation controls all pass.

**What fails.** Abstention is uncalibrated. Fresh synthetic all-position CE regresses **+0.6713** bits/token (tolerance +0.01) and the bounded raw-text probe regresses **+2.7891** (tolerance +0.02), with **83 false copies in 88 reads**. The damage is precision on the admitted-but-uncovered stratum that a construction-only training signal cannot separate from a useful recurrence. **No candidate is promoted.**

**Near misses preserved.** Version 1's threshold landed exactly on the correct candidate's score after 4-bit quantization (`f2+f8+f6 = 4+1−4 = 1`, `μ = 1`, strict comparison), so it abstained when it should have read (0.400). A declared **tune-only** threshold sweep fixed that. Version 2 carried a mis-specified altered-source control (it rewrote the target, not the source) and an over-generous automatic label; both are corrected in the retained version.

**Next action:** add one declared abstention signal learned from natural-text positions where a candidate exists but no exact successor is useful, then re-evaluate the same panels and controls before any integration claim. Do **not** re-fit this selector, widen the ring/candidate bounds, or sweep width/precision/corpus. Reset/Continue maintenance, exact occurrence/version memory, shared composition, useful conversation and full laptop cost remain unrun. `#973`/`#820`/`#963`/`#964` stay open.
...
**Resources:** the standing-authorized +10800000 ms extension to 191300000 ms was recorded before use; this step charges 2700000 ms once. Whole-run peak RSS 142,458,880 B; elapsed 110.4 s; physical energy UNAVAILABLE. See [ledger](resource-ledger-2026-09-19.md).
