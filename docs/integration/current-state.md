...
## Active: the refit recovers the relation; coverage is next

**Matched relational learning executed (2026-09-21).** Bounded scorer/descriptor/**refit** alternation is the decisive change: the geometric arm's fresh global-strength loss improves from the parent's **10.7185 to 10.2037 bits/candidate position**, and **relational − exact recurrence = −0.524968 [−0.654447,−0.396747]** where the pre-refit parent measured **−0.010151 [−0.104470,+0.083675]** on the same repaired construction. Descriptor refinement alone *worsened* the exported development loss (−1079.49 → −1040.77 nats) because the scalars reading the new descriptors were stale; the refit repaired and surpassed it (−1354.60). Present-final-query outcomes reach **4.0912 bits/query** with **103/119** correct payload reads and **83/119** correct emitted next tokens (parent: 9.08857 bits, 49, 35). [Result](relational-learning-result-2026-09-21.md), [prospective design](relational-learning-design-2026-09-21.md), [receipt](../evidence/native_geometric_relational_learning_2026-09-21.txt).

**Both preregistered comparisons fail at their margins.** `relational_ctx − relational` is **−0.012093 [−0.023107,+0.003536]**, spanning zero, with **zero** differing decisions across all 119 present final queries — so the parent's −0.257644 strength gain was largely the stale-scalar artefact surfacing as a strength correction; `retained_component_positive = false`. The matched nongeometric `categorical_ctx` is **significantly better** (−0.124149 [−0.227536,−0.022590] overall, −0.890533 [−1.683652,−0.070554] on present queries), so `unique_geometric_benefit = false`. The relation channel is still load-bearing *inside* the geometric arm (lesion −0.537061 [−0.658622,−0.405202]). Document-separated held-out reader text still harms: +0.2709 / +0.2742 / +0.2422 bits/token for global / contextual / categorical-contextual. Generation remains degenerate.

**Diagnosis to act on.** The repaired exact decomposition (ranking + gate + dose = actual − Lpool, verified per position, 0/1813 served sources differing from the ungated top source) puts **coverage first**: only **646 of 1,813** positions admit the correct payload, ranking regret 0.600 vs gate 0.209 vs dose 0.121 nats/position. Reader cost stays unresolved (the uncached local E path dominates; the reader increment is within noise) and physical energy is `UNAVAILABLE`.

**Next constructive task (single successor):** a prospective, matched comparison of **one bounded broadening of the causal admission rule** — same 128-token ring, same ≤24 candidates, same shared path and same alternating fitting schedule — against the current pool, scored on the admitted-correct rate and held-out document loss. Do not widen the ring, sweep widths, replay S attribution, or start a scheduler; do not escalate to S7 or a pair table, which this run does not license.

Delivered sealed root `.uor-models/realtext-prior-2026-09-20/relational-learning-4` (27 files, 0 unlisted). Attempts 1–3 are retained superseded reporting/control defects with a bit-identical measurement: (1) a one-shared-path control compared the contextual arm against the global arm's `choose`; (2) the cost timings vector was omitted; (3) the no-reader-work baseline used a different stream. Base revision `472767dc`; RLR2 v3; `binding.json` carries the full observation digest, splits, seeds, configuration and format versions.
