...
## Active: frozen separable-reader attribution — specified, NOT_RUN

The [principal review after PR #1310](query-read-review-2026-09-20.md) verifies the retained sets, saved loss vectors/intervals, dose and numerical mechanism. Q's registered negative survives. S is the strongest numerical candidate on the reused 36-document/288-window/17342-target panel: E 7.170815718, Q 7.141505532, S 7.032227740, L 7.101049449 bits/target. E remains the established local reference. No model is promoted to useful language.

**The missing attribution:** S's exact-tail older-prefix donor penalty is **−0.002298261 [−0.014071554,+0.008581473]**, with 564 changed reads among 1177 eligible targets. This does not show useful older content. On the correct 16766 older-present targets, S's query-neutralization penalty is **+0.197210288 [0.179469190,0.214992789]**; the previously reported +0.190660 used all targets. Its current-token query channel is useful, but the source of the aggregate gain is unresolved. Hard-map movement does not exclude an optimization bottleneck. The eight-window tune curves do not establish convergence.

**One next task:** execute the [complete frozen S attribution prompt](deepseek-separable-attribution-step-2026-09-20.md). Preserve parameters; compare the four identity-anchored history/query corners, E, direct zero-row ablations and a fit-only length-conditioned mean-history comparator. Repair metadata and redundant inference folds, reproduce the old panel, then evaluate a deterministic nonoverlapping position panel from the existing open-dev documents. New positions are not independent-document or final-held-out qualification. No optimizer updates, new corpus, width/precision change or reset fit. The mean-history comparator is offline analysis only.

**Corrections required before reuse:** all CPX3 tokenizer fields are zero; re-export metadata-only descendants with raw digest a7ac75b6… and verify numerical parity. The report's source is a preimplementation base with evaluator revision unset. CPQK serializes parameters/moments but does not bind/recover the full data order or production continuation; the mini test resumed in-memory bytes in one process. Keep that narrower parity result. Q/S fold older history twice and L does unnecessary history work; microbenchmark outputs were discarded. Cycle claims must distinguish local-pair repetition from sufficient bounded state. Full continuation remains required before a later fit, outside this evaluation task.

The [raw result](query-read-result-2026-09-20.md) and [receipt](../evidence/native_geometric_query_read_2026-09-20.txt) are preserved with this correction. Both complete roots verify; their final Q/S/L artifacts are identical. Saved S artifact SHA256: `9f8cda09bfb562221b5718a8330cc4a296215b8e94d3e0c2b8abe2ce31fae851`. Actual reloaded numerical parity and the projection-loop/singularity repairs remain valid. General conversation/coding and complete-path energy remain unestablished.

**Roadmap:** useful local emission -> attributed geometric state access/maintenance -> exact occurrence/version memory and shared composition -> useful conversation and executed Rust on one artifact with full laptop cost. A later Continue/ResetTo transformation could add noninvertible global reset without changing exact 2I state, but would not add capacity or keyed memory. It is a hypothesis, not the next executed mechanism. #973/#820/#963/#964 stay open.

**Resources:** live JSON 176138565 /178100000 ms, remaining 1961435 ms. Preserve the 5900000-ms mixed measured/estimated debit. This review ran no build/training/model forward and changes no balance. The next task proposes 3600000 ms complete work and +2400000-ms standing-authorized limit extension to 180500000 ms, to refresh and record before use. Preserve all roots and the 128 MiB margin. [Ledger](resource-ledger-2026-09-19.md).
