...
**Next action.** Confirm the NEGLIGIBLE bands and flip rates on a second disjoint slice, then **repair the VSA codebook wiring** (derive it from the learned embeddings, not a fixed random hash) and re-measure — `vsa` is an `enabler` with an identified wiring defect, so its current delta is not a verdict on the mechanism. Repair or remove the two D0-a violators (the JEPA `i64` projection and the `f64` sampler). Reconcile any further ledger charges.
