...
## Active: matched geometric query read — EXECUTED, valid negative

The [result record](query-read-result-2026-09-20.md) and its [evidence receipt](../evidence/native_geometric_query_read_2026-09-20.txt) report one complete executed experiment from source `92240a86`, in the isolated worktree `codex/geometric-query-read`, sealed and verified at `.uor-models/realtext-prior-2026-09-20/query-read-2` (39 files, 35,737,680 bytes).

Frozen corrected E (`565cf98a01273af38425687adb522704b09a89bd7ab976b58b20bfbdaccfa0bf`) reproduced at **7.170815718** bits/target on the exact 36-document / 288-window / 17,342-target panel. Three equally parameterized arms were fitted for 512 batch-8 updates each (64-update R/W warm-up), exported as new `CPX3` artifacts, reloaded and evaluated with 0 parity mismatches over all 17,342 positions.

| Arm | Dev micro CE | Gain over E, nominal 95% paired interval |
| --- | ---: | --- |
| E, corrected empirical incumbent | 7.170815718 | — |
| Q, query-conditioned older read | 7.141505532 | +0.029310 [0.022866, 0.036005] |
...
**Decision: valid negative at this fixed architecture and dose.** Q beats E but fails the predeclared 0.10-bit practical screen, is **0.109278 bits worse than S** and **0.040456 worse than L**, shows no older-content sensitivity under the exact donor map (−0.003830 [−0.012235, +0.004366]) and no demonstrable query-route use (+0.000322 [−0.002913, +0.003358]). S's own query channel *is* strongly used (+0.190660 [0.173466, 0.207857]). No candidate is promoted. Learning moved 1,549–2,241 write codes and 1,544–1,581 query codes per arm, so this is an arrangement result, not an optimizer stall.

Instrument checks all pass: read-disabled equals E exactly in every arm, L is byte-identical under the donor map and older-order reversal, re-export/reload integer parity is exact, and an on-disk split-at-4 checkpoint/resume equals the uninterrupted run in next-batch loss, all losses and artifact bytes. All six retained prompts still degenerate into short cycles for every arm; no language claim follows. Peak RSS 763,002,880 B, complete run 992.7 s, physical energy UNAVAILABLE.

**Scope corrections delivered in the same change:** `project_row` now sweeps the nearest-code seed at every admissible shift including `s0` (focused correlated-Gram regression; the defect was reproduced by temporarily restoring the guard), and the head-projection runner reports `singularity: NOT_MEASURED` with `singular_inputs_supported: true` instead of a hardcoded rank claim. The previously reported reduced-search measurements remain valid; the complete originally specified projection is still **NOT_RUN** and no QG campaign was rerun. Old report bytes are unchanged.

**One instrument defect, found and corrected before the retained run:** the superseded `query-read-1` mis-specified the local-only arm's *reversal* control; the model and losses were unaffected (identical CE) and the root is preserved sealed. See the result record.

**Larger direction:** the multiplicative `q*b` transport adds no demonstrated value over its separable control at this dose, while the separable older+query read is the strongest arm so far against E. The next bounded mechanism should therefore test **selective write/reset or exact occurrence/version access on the separable read**, or ask whether more history capacity (rather than nonseparable querying) is the binding constraint — with a new independent behaviour panel, not another projection campaign. One 120-state register holds at most log2(120) ≈ 6.91 bits; prime/zeta/R4/S3/H4/icosian priorities, D0-b and #973/#820/#963/#964 remain unchanged and open.

**Resources:** the standing-authorized +7200000 ms extension to 178100000 ms was recorded before use; this step charges 5900000 ms once. See [ledger](resource-ledger-2026-09-19.md).
