...
## Active — transferable lexical learner executed; ordinary text remains open

**This task, executed (`state-lexical-v3-9`, reused artifact `state-lexical-v3-8`):** the retained state-conditioned decoder now consumes a **position-resolved, tail-carrying, unknown-explicit** content observation, the **actual emitted token identity** and an **exact 15-coordinate typed causal block** (requested history; derived; prior_differs; committed; key_changed; exact equality / common prefix / exact divergence / lengths / unknown counts) in its recurrence, and **continues after the copied span** when the consumed computation actually changed the address. Executed on the declared source-separated text: learned **31/31** (fitting documents 23/23, held-out source documents **8/8**), teacher-forced **791/791** through the exported integer artifact, retained legacy copy **31/31**, retained comparator **20/31**, recurrence ablation **0/31**, context ablation **6/31**, value-sensitive effect **4/4**, ten in-process restart boundaries plus a separate-process child agree, and the loaded **E/S reference** (454,788 + 53,555 bytes at their pinned SHAs) loads, validates and executes. The version-2 artifact and its recorded results are preserved. [Result](transferable-lexical-result-2026-09-22.md) and [evidence](../evidence/transferable-lexical-2026-09-22.json) own the exact scope; the principal review that set this task is [here](state-conditioned-lexical-review-2026-09-22.md).
...
**Next:** learn vocabulary and continuation from a source-separated ordinary-text corpus through the retained session (a continuation objective over the same low-bit state), then adapt the E/S donor through a new donor-conditioned vocabulary decision over an exact owned span. Broader conversation and executed Rust follow through the same candidate, then whole-model laptop cost and product delivery.
