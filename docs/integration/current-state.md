...
**Next action.** Wire the codebook mode as an artifact-level option (a reserved header byte plus a serde default, keeping the format size-compatible), rebuild the hierarchical codebook at load when selected, and verify end to end with `r4-native-chat` before changing the default — `root` is now measured-better on routing and must not stay an evaluation-only switch. Then obtain a **finer representation-derived code** (needs an artifact section for the continuous embeddings plus a re-export) paired with a richer per-head read, since one scalar distance over one codebook cannot express identity, similarity and role together. Repair or remove the two D0-a violators (the JEPA `i64` projection and the `f64` sampler). Run the `jepa` × `s2_readout` factorial before any further removal.
