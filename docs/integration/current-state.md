...
## Shared geometric language core — emission bottleneck diagnosed, September 12
...
**Next action:** implement a shared calibrated nonlinear geometric byte-branch operator that retains the ordered state components until its decision and can express class priors/constant decisions. Establish representability on the existing development diagnostic before one separately frozen joint-learning experiment. Preserve bounded finite serving without contractions or a complete-state Cartesian lookup. A landmark-only refit or greedy-decoder change is insufficient; calibration alone cannot recover distinctions lost in composition. The opened holdout remains development evidence. General language, reasoning/coding, matrix-free qualification of the inherited full serving path and laptop energy savings remain unestablished. #973 and #964 remain open; #1139 retains its separate contextual contribution obligation.
