...
## Active: read influence now transfers to held-out prose; relational behaviour is traded away

**Direct read-influence policy executed (2026-09-21).** The five frozen parent artifacts are loaded by SHA256 and only *influence* is learned: a 32-bucket direct opcode table over the ungated top source selecting `{NoRead, 0.0625, 1, 8}` nats, fitted by **mean individual action cost** with a declared minimum support, a global fallback and a safe-abstention tie rule; `RLR2` v4. On **document-held-out** reader text (8 disjoint documents, 1,952 positions) `h4_policy` is **−0.0195 bits/token [−0.0329, −0.0055]** and `categorical_policy` **−0.0243 [−0.0369, −0.0126]**, against the frozen parent's **+0.5453 [+0.4038, +0.6847]** and `h4_policy − parent` **−0.5648 [−0.7020, −0.4269]**. This is the **first net benefit on held-out prose** rather than harm, concentrated where causal copying is possible (−0.5649 on copy-available rows versus +0.0204 on copy-unavailable). Absence handling improves too. [Result](reader-utility-result-2026-09-21.md), [prospective design](reader-utility-design-2026-09-21.md), [receipt](../evidence/native_geometric_reader_utility_2026-09-21.txt).

**The honest tradeoff.** `relational_behaviour_preserved = false`: under the declared equal-weight construction/text mixture the expected-cost minimiser reads **every** candidate-bearing position at ≈1 nat (1,665 of 1,822, only 4 of 32 buckets earning their own action), so correct payload reads rise **465 → 517** while correct emitted tokens collapse **362 → 29** and present-query hard CE is **+7.59 bits/query worse** than the parent. The diagnosed cause is a **contextual-representation limitation**: `gap`/`ctx_class`/`margin_bit` cannot separate high-copy-reliability construction buckets from low-reliability text buckets, and expected action cost (fitted) is not emitted-argmax correctness (measured). Full support (2,969 + 944 weighted events), exact artifact round-trip and an unchanged local predictor rule out support, quantization and the local predictor as explanations.

**Repairs integrated this run.** The four-condition selected-source intervention shows **disabled-before == disabled-after on 140/140** (the parent's mis-matched comparison is fixed), 138/140 keep the original source reference, 0 lost support and 0 NoRead. Group identity is a real SHA256 over the group table bytes; `binding.json` carries a **loader-verified `manifest_digest`**; the result field set is validated before the expensive pass; 0 reload failures. Mean admission regret is **0.150 nats** on the final construction and **0.283** on the final text window, so the declared 0.25 trigger fires **on text only** — named, not acted on. Paired interleaved cost puts the reader increment at **+0.7 µs/prediction** against a ~1,352 µs/prediction uncached local path; physical energy `UNAVAILABLE`; whole-path D0-b compliance is not claimed.

**Next constructive task (single successor):** a **learned contextual observation that separates high-copy-reliability from low-reliability reads** — concretely persisted **role/scope or query-relation state** (stage 2 of the plan) — rather than another bucket on the same three observations. The admission complement is second-ranked and applies only at the triggered text scope. Do not launch endless controller refinements, widen the ring, or escalate to S7 or a pair table.

Delivered sealed root `.uor-models/realtext-prior-2026-09-20/reader-utility-2` (0 unlisted files). Attempt `reader-utility-1` is retained and unsealed: all measurements completed and it failed only because the binding manifest was named `manifest.json`, which collides with the seal filename; attempt 2 also fixes per-position document attribution and the read-rate denominator.
