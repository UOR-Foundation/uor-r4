...
## Active next: recover the frozen real-text prior's evidence — review after PR #1300

**Keep the trained artifact; repair and execute its evaluation before selecting another model change.** PR #1300 is merged as `74fef0886ca3b14ff90943c8677d6b7815064198`, with the same tree as reviewed head `00957c4f`. The [principal review](frozen-prior-review-2026-09-20.md) and [complete DeepSeek prompt](deepseek-frozen-prior-step-2026-09-20.md) supersede the earlier request for a new 4 MiB training run. This review ran no Rust build, model forward or training.

| Recorded checkpoint | Unpermuted micro bits/target | Frozen quantized bias | Exact full-fit unigram |
| --- | ---: | ---: | ---: |
| 0 | 9.13163 | 9.13163 | 9.0599 |
| 256 | 8.2182 | 9.13163 | 9.0599 |
| 512 | 7.14247 | 9.13163 | 9.0599 |

**Positive result:** saved losses show a 1.9174-bit improvement over exact unigram and **1.98915761 bits over the frozen quantized bias**. Regrouping saved step-0/512 rows into the actual 32 document clusters gives a nominal 95% interval **[1.84387043, 2.12714975]** for the latter gain. This is arithmetic over retained losses, not a fresh model measurement. Correlated repository documents remain open development data.

**Correction to the original receipt:** the full gate PASS is withdrawn pending replay. The permutation uses a context-keyed replacement map that overwrites repeated occurrences and changes target frequencies; even the context-independent step-zero loss changes under it. Its +2.8744-bit penalty and 5,805 changed-association count are not qualified controls. The claimed document bootstrap samples 96 windows, not documents. Actual evaluation is **32 documents × three opening windows = 96 windows / 6,048 targets**, not 36 evaluated documents or spread-position sampling. All three generation prefixes come from one document; the saved final outputs collapse to token-ID 32 repetition. The [raw receipt](../evidence/native_geometric_realtext_prior_curve_2026-09-20.txt) and sealed results are preserved unchanged.

**Actual data/exposure:** the 850-file archive is 88,016,240 bytes, independently identical to the Git corpus at `e9c04e80`. The runner's eligible input is **429 Markdown documents / 7,609,837 bytes (7.26 MiB)**, still above the requested 4 MiB. Split pools are 355 fit / 38 tune / 36 dev docs. The 38,987 fit windows / 2,446,208 targets supplied the full-fit frozen marginal; the optimizer consumed the first **4,096 distinct shuffled windows**, recorded in its checkpoint at step 512, cursor 4096, pass 0. Their exact target total remains to be reconstructed; 258,048 is only a full-window upper bound.

**Retained root:** `/Users/casey.allard/uor-r4/.uor-models/realtext-prior-2026-09-20/attempt-1`. All seven manifest members verified by BLAKE3/size with zero unlisted files. Final CPL2 is 454,788 bytes, SHA256 `cd5a3aa1b804ccc3582a4a25090e25c862489e0bc4e5bb86c1d1f363a7338a00`; CPCK is 19,192,104 bytes, SHA256 `1c3dfedb2248ec76c21f320b7ecec585c7a522a2d47e35ed5dc9eaca2454bff2`. Only the final model/checkpoint is saved. Earlier curve points used in-memory cores; intermediate artifacts and full integer logits are absent. The legacy composite identity is not a complete data/configuration/training-source binding.

**Implementation scope:** integer trace and normal mid-pass permutation restoration exist; this audit found no new active-path gradient-scale defect. Shared construction validation still needs to call `PriorCore::validate`, and widened ternary scaling should use signed shifts instead of source-level multiplication. Make these value-preserving corrections with exact retained-model score parity. Complete continuation identity and an on-disk resume remain future training obligations, not requirements to update weights during frozen replay. Existing #1298 small-gate evidence remains preserved.

**One next run:** implement and execute evaluation-only replay of the final CPL2 on the exact legacy population plus a separately named spread-position panel. Use unique occurrences, fixed targets, a saved bijection, constant-predictor invariance and true document clusters. Add exact/quantized unigram and tuned count references, consumed-fit/unseen-pair strata, and saved-artifact generation from three genuinely distinct documents with pair-cycle/margin/data-support diagnostics. No retraining or decoder change. Then choose one evidence-supported seam toward learned ordered geometric prefix-state/read and exact-memory integration. The stored modulo-120 table is still unused by this prior; geometric advantage, useful prose/coding and energy savings remain unestablished.

**Resources:** live JSON records **152,238,565 / 154,400,000 ms**, remaining **2,161,435 ms (36.02 minutes)**. Preserve prior charges; no review charge or limit change. The next prompt proposes a complete 1,800,000 ms tranche, to be refreshed and recorded before use. Necessary local extensions remain owner-authorized with prior projection and retained cumulative charges; preserve the 128 MiB margin, original checkout and all unique artifacts. No deletion or paid compute. #973/#820/#963/#964 remain open; no linked project items exist at this review.
