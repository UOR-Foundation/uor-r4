...
**Next within the same integrated model:** connect candidate-conditioned language utility to discriminative contextual addresses, relative ranking, read permission and the output head. Preserve exact raw events; incomplete labels are not negative write evidence. Natural recurrence pointers are weak and sometimes unidentifiable from the query. Address the common write/query descriptor and candidate competition, distinguish rank before the gate from final selection, and train on a sufficiently diverse grounded prose/Rust curriculum together with natural next-token loss. No larger geometry, isolated gate retuning, or fresh final benchmark follows from these results. The [full programme](principal-attention-plan-2026-09-24.md) and [A1 contract](integrated-attention-a1-plan-2026-09-24.md) retain the remaining mechanisms and limits. [Evidence](../evidence/integrated-attention-a1-result-2026-09-24.json).
...
> **Direction review (2026-09-23):** a whole-repository review and staged proposal is at
> [repo-review-direction-2026-09-23](repo-review-direction-2026-09-23.md) (owner decisions requested; not yet
> adopted). It confirms this milestone's negative, corrects the "rank-64 interface" framing (the readout input is
> 147-wide, so the bound is ≤147; the real constraint is linearity/higher-order features), and re-orders the next
> step to a frozen-state adjudication that includes the transport-decode arm before any capacity or geometry build.
...
**Verdict:** the exact oracle criterion is satisfied (`gap 1.562381`, `mean_kl 2.115408`, `b'(1) 0.553026`), so an *in-sample* `λ` can beat `C` — but the effect is not material and not tune-robust. The tune-frozen pool (`λ*=0.8`) is **+0.004928 [−0.026228, +0.040527]** against `C` on the full split (interval includes zero, pool slightly worse), the in-sample oracle gain is only **−0.025181 [−0.040195, −0.007841]**, and on the **422 rare targets outside the top-1024 the pool is ≈+0.552 bits/target worse**. On the K=1024 subpopulation the tune-frozen pool is −0.041691 [−0.068550, −0.010625] — below the standing 0.10-bit threshold. The one-token control fails (`blend(E1,C)` oracle = `C` exactly), so `C` already dominates the information the state carries first-order. **Decision: no exact addressed local read inside the served readout is justified on this evidence.** The ground is **cost and threshold, not absence**: an addressed `(prev,cur)` read is ≈8–16 MiB at 4-bit against the 466,711-byte artifact for only ≈0.04–0.08 bits over serving `C` alone, and on the 92.15 % frequent-token head the tune-frozen pool in fact *beats* `C` by −0.0417 [−0.0686, −0.0106] while losing ≈+0.552 bits/target on the 422 rare targets. A **K3 architecture review** confirmed the decision and required these qualifications; it also registers the unmerged `codex/observer-transport-20260923` two-parameter result as contested evidence (see Next). Full `C` 5.121721, restricted `C` 4.784593 and restricted `E1` 5.565123 reproduce the merged state-probe values exactly; `A` reproduces to ≈2.9e-6 bits under the documented token-conditional (Stop-renormalised) convention. No K3 review (conservative scoped negative); an independent evidence audit required the wording corrections recorded in the result, including that "domination" is not uniform (the pool marginally beats `C` on the frequent head).

**Next:** the K3 architecture review (2026-09-23) confirmed the decision but ordered the **adjudication + transport-decode depth ladder** before any other step: (1) a frozen two-parameter adjudication (full tune split, declared refined grid, vs raw **and** calibrated `C`, rare-tail accounting, fresh-process verification) to settle the unmerged `codex/observer-transport-20260923` **−0.037612 [−0.057254, −0.016462]** two-parameter result; (2) oracle and tune-frozen `λ` per context-frequency class; (3) a transport-decode depth ladder (`x_(t-2)`, `x_(t-3)`, `x_(t-4)`) on the identical recorded states to settle the `prev`-retention contradiction. The longer-context control follows these.

## Previous active — the joint fit does not recover the headroom: training budget is not the constraint, September 23

**Executed milestone (`olx-joint-1`):** the joint fit was run as the decisive test of whether the shared learner is under-trained — the prose-only window-objective recipe of `olx-form-2` at **4,000 steps** (double its 2,000), with everything else identical: same corpus and split, same seed 13, lr 0.02, batch 4, interleaved curriculum, no grounded draws. No code change: the existing `run()` path with the existing declared flags. Base `67f2e350` plus open PRs #1363/#1364/#1365; model source unchanged (`03f83eb8…b250`). [Result](ordinary-lexical-joint-fit-result-2026-09-23.md) · [evidence](../evidence/ordinary-lexical-joint-fit-2026-09-23.json).

**The pre-declared bar is not met and the model has plateaued.** Served development loss **6.708963 bits/target** against the arm's own **6.684105** at 2,000 steps: **+0.0249 [−0.0422, +0.0967]**, a paired document-cluster interval that **includes zero**; and the development probe oscillates between **6.6318 and 6.9169 (median 6.7843)** with no trend instead of descending. So the model has converged operationally and **training budget is not the constraint**. Gains against the retained references: tuned `(prev, cur)` count −1.5872, donor E +0.1612, unigram +2.3737. The pre-declared ≥0.30-bit bar is not met.

**Cumulative picture:** the two cheapest output-side levers are now exhausted at about a quarter of the available headroom — an in-class readout refit on the frozen state recovered **−0.1313 bits** (with an exact, asserted initialisation and the integer realisation equal to the float surrogate), and a longer joint schedule recovers **nothing**. The matched float-readout headroom on the same states is 0.4634 bits in principle. So the remaining deficit against the tuned `(prev, cur)` count reference (5.1217) is an **interface** fact: the readout is a rank-≤64 linear map of a 64-dimensional state. The state's content is already measured: a training-free **raw-embedding** decode recovers the immediately preceding token at **96.7%** and the one before it at **2.1%** — **corrected 2026-09-23:** the second-order *retention* claim is withdrawn for the training-free observer class (a transport-aware decode recovers `prev` at **75.90 %** on byte-identical states; see Next), while 2.1 % remains the raw-embedding and linear-readout result.

**Retractions carried from the previous record:** this programme's first-draft "the integer realisation is the limit" conclusion was an artifact of a defective refit initialisation and is withdrawn, and the previous post-hoc re-pricing figures (8.6058 / 7.6181 / 8.9683) came from a degenerate quantisation and are not evidence about the alphabets.

**Next:** the **count-prior blend** — on the same 5,376 development targets and the served window conditioning, measure the loss of a **log-linear blend of the artifact's served readout with the retained tuned `(prev, cur)` count prior**, tuned on the tune split, with per-document paired intervals, distinguishing **complementarity** (the blend beats both) from **domination** (it collapses to the count prior). It is the decisive, cheap check for second-order local access and it sets the form of any subsequent change: strong complementarity means the local prior belongs inside the served readout as an exact addressed read; domination means the retained comparator is the right local tool. Signed H4/shared geometric transport remains conditional on a witnessed order/role/distant-interference failure against an information- and compute-matched ordinary control.

## Previous active — the served alphabet realises the fit, and the frozen-state gain is 0.13 bits, September 23

**Executed milestone (`olx-sp-e`, `olx-sp-parity`, `olx-sp-parity-serv`; `olx-refit-1` unsealed):** the state probe's artifact-initialisation discrepancy is **resolved exactly** and an in-class served-alphabet readout refit was built and run on the frozen state. Base `67f2e350` plus open PR #1364; model source unchanged (`03f83eb8…b250`). [Result](ordinary-lexical-readout-refit-result-2026-09-23.md) · [evidence](../evidence/ordinary-lexical-readout-refit-2026-09-23.json).

**The discrepancy was a softmax base convention, worth exactly `ln 2`.** The artifact scores `exp2((logit − max)·2^-score_shift)` (base 2 over dyadic logits); the probe used base `e`, so the initialised readout was sharper than the artifact's by `1/ln 2` and its loss came out **above** `A_restricted` — the inequality that looked impossible. Four steps located it (identical logits to 1.1e-14 with different losses, worst row gap 5.65 bits at K=64); multiplying the initialisation by `ln 2` collapsed the gap to **7.1e-15**, and the invariant is now **asserted** for both the probe and the refit (deviation **0** over 262,272 logits in the served scope). Corrected matched reference: **`A_kclass` = 5.977039 dev / 6.018168 fit** at K=1024; the class restriction is worth **0.078917** bits of mass; the pre-fix diagnostic was inflated by **0.6183**.

**The previous record's headroom is confirmed, not corrected:** `A_kclass − P = 0.4634` bits, inside the previously published "≥0.40".

**In-class served-alphabet refit on the frozen state, exact initialisation, sealed:** the served scope (all 4,098 legal rows, all 5,376 development targets) reaches **6.5528 against the artifact's own 6.6841 — a gain of 0.1313 bits** end-to-end through the artifact's integer kernel (top-1 0.2161), and the K-class scope reaches **4.6357 against `A_kclass` 4.7793 — a gain of 0.1435**. In both scopes the **integer realisation equals the float surrogate**, so the ternary codes, per-row power-of-two scales and integer bias are **not** the constraint. The pre-declared **≥0.30-bit bar is not met**: refitting the output layer alone on these states buys about a quarter of the 0.4634 bits those states support.

**Two retractions recorded.** (a) This milestone's own first-draft conclusion that "the fit is not the limit, the integer realisation is" — with a ≈0.27-bit realisation cost — was an artifact of a **defective refit initialisation** (0.14–0.21 bits above the artifact's own readout) and is **withdrawn**; with an exact initialisation the realisation cost vanishes. (b) The previous record's post-hoc re-pricing figures (8.6058 / 7.6181 / 8.9683) came from a **degenerate** quantisation (code density 0.0088, 614 of ~70k codes non-zero) and are not evidence about the alphabets.

**Controls:** kernel cross-check 0.0; frozen-bias ablation 4.6232 / 6.5544 (the learned bias contributes ~0.01 bits, so the gain is in the vocabulary rows); permuted-target null 6.4188 / 8.2744 against floors 4.7793 / 6.6841 with the floor corrected to `min(its own initialisation, the restricted unigram)`; both alphabets' round trips and the straight-through gradient unit-tested; 7 runner tests and 12 `transferable_lexical` tests pass.

**Next:** take the refit out of the frozen state — one **joint** fit on the served conditioning with the output layer initialised from the converged refit, keeping grounded Copy/Stop supervision, pre-declaring served loss below **6.6841 by ≥0.30 bits with preflight A 32/32, held-out 3/3 and class 4/4 retained**, and reporting the matched reference, the float surrogate and the integer realisation so a shortfall is attributable. If that also stalls near 0.13 bits, the next branch is the input side: explicit **second-order local access**, since a training-free **raw-embedding** decode recovers `cur` at 96.7% but `prev` at 2.1% — **corrected 2026-09-23:** `prev` is recoverable through the model's own transport at **75.90 %**, so this premise is revised; see the [state-probe correction](ordinary-lexical-state-probe-result-2026-09-23.md). Signed H4/shared geometric transport remains conditional on a witnessed order/role/distant-interference failure against an information- and compute-matched ordinary control.

## Previous active — the local deficit is in the output layer, not the recurrent state, September 23

**Executed diagnostic (`olx-probe-3`, `olx-probe-4`; preserved `olx-probe-1/2` and four `/tmp/olx-sp-*` smokes):** a new runner-only mode `--state-probe` records the *exact* state vector `h` the served readout receives at every scored prose position (walks mirroring `score_example`), then fits a float linear readout of that state, decodes `cur`/`prev`/`x_(t+1)` from it, and reports the artifact's own readout against a tuned unigram+bigram(`cur`) count model and the tuned `(prev, cur)` count. The model source is unchanged (`03f83eb8…b250`). An independent K3 architecture review corrected two errors that are load-bearing: a linear readout cannot in general reach a full count table (rank ≤ `h_dim`), so the decision rule is `P` versus `A`, and the probe's class-restricted softmax needs the same normalisation as `A`. [Result](ordinary-lexical-state-probe-result-2026-09-23.md) · [evidence](../evidence/ordinary-lexical-state-probe-2026-09-23.json).

**Decision-population results** (K=1024, 4,954 of 5,376 development targets = 92.15%; bits/target; `A`, `E1`, `C`, `U` share the full-set normalisation and are directly comparable): `A` artifact served readout **6.055956**; `P_softmax` converged float linear readout of the same state **5.5136** (top-1 0.2618, last-epoch fit delta 0.0119); `E1` unigram+bigram(`cur`) **5.565123**; `C` tuned `(prev, cur)` **4.784593**; `U` **8.669572**. So `P − A = −0.5423` and `P − E1 = −0.0515`.

**The state carries the last token and not the one before it.** A *training-free* nearest-embedding rule over the artifact's own embedding rows recovers `cur = x_(t-1)` at **0.9674** top-1 (majority 0.0381, chance 0.0087) while `prev = x_(t-2)` sits at **0.0206** and the control `x_(t+1)` at 0.0201. A ridge decode agrees on the ordering (`cur` 4.0495 bits/0.5333, `prev` 6.4473/0.2838). The state-side hypothesis is **rejected for first-order local structure and supported for second-order structure**.

**Decomposition of the 1.27-bit restricted gap to `C`:** about **0.54 bits (≥0.40 after the class-restriction allowance) is output-side** — a converged readout of the artifact's *own input* recovers it, and the artifact sits 0.49 bits behind a one-token count model while the state carries that token at 96.7%; about **0.73 bits is second-order structure** that the current state does not supply to a linear readout. The previous milestone's two-regime plateau at ≈6.69 is therefore a shared **output-layer** ceiling at this state, not evidence of a recurrence limit.

**Instrument negatives recorded:** post-hoc re-pricing of the fitted readout into the servable alphabets is destructive (ternary 8.6058, ternary+offsets 7.6181, 4-bit 8.9683 versus the in-class-trained artifact at 6.055956), so only in-class training can judge the served class; and the artifact-initialised probe's epoch-0 dev loss (6.5954) exceeds `A`, which the renormalisation identity forbids, so that discrepancy is **unresolved and nothing is built on it**.

**Next:** freeze the recurrence and embeddings and train the served readout **in class** on the frozen-state data, pre-declaring served loss below `olx-form-2`'s 6.6841 by ≥0.30 bits with preflight A 32/32, held-out 3/3 and class 4/4 retained and the probe's epoch-0 mapping reconciled. If in-class training cannot recover the headroom, the next branch is second-order access to the two-token context, since the state does not retain `prev`. Signed H4/shared geometric transport remains conditional on a witnessed order/role/distant-interference failure against an information- and compute-matched ordinary control.

## Previous active — the prose deficit was the training curriculum, not the objective, September 23

**Executed milestone (`olx-form-1..5`, `olx-condition-1/2`):** the recorded per-position formulation was tested inside the shared grounded Generate/Copy/Stop learner on `79b5d50c` (Kimi worktree `/Users/casey.allard/uor-r4-kimi`). Five matched arms ran through one **unchanged** learner (`learner/transferable_lexical.rs` sha256 `03f83eb8…b250`); only the runner gained declared options `--objective window|position`, `--curriculum delivered|interleaved`, `--prose-slots`, `--ground-slots`, `--ground-weight` and a read-only `--condition`/`--artifact` scoring mode. The default path reproduces the delivered artifact **bit for bit** (`olx-form-1` sha256 `fe3e8a638cd50a19741065cb00ac787b63bb59f8ad5776fd8bfcb162c37a912a`, development `7.372198274238398`), so every arm is matched to the delivered baseline through one unchanged path. [Result](ordinary-lexical-formulation-result-2026-09-23.md) · [evidence](../evidence/ordinary-lexical-formulation-2026-09-23.json).

**Served-conditioning outcomes** (window conditioning = the recurrence serving actually executes; same 5,376 development targets; paired document-cluster intervals): delivered **7.3722**; prose-only window objective **6.6841, −0.6881 [−0.7686, −0.6091]**, crossing donor E (6.8701) for the first time; window objective with two grounded draws per step at weight 4.0 **6.9686, −0.4036 [−0.4668, −0.3412]**, retaining 31/32 preflight A, 1/3 held-out, 2/2 preflight B, 35/36 authored training and 4/4 class; **per-position objective with 24 grounded draws per step 7.0327, −0.3395 [−0.4388, −0.2447], fully retaining 32/32, 3/3, 2/2, 36/36 and 4/4** with separate-process restore `REPLAY_OK 36 cases`. The per-position arm is **−0.0641 [−0.0142, +0.1490]** versus the served-conditioning window arm — indistinguishable from zero. **The recorded hypothesis that the shared learner needed a different prose objective is not supported**: removing the grounded rehearsal mix and the grounded-only final phase and keeping prose at the full declared batch accounts for the whole recovery. The per-position objective is retired as a necessary training change and retained as a diagnostic conditioning.

**Correction to the retained audit:** its "about 1.17 bits" compared each artifact under its own conditioning. Under the **served** conditioning the per-position prose-only artifact recovers **0.682 [0.581, 0.790]**, not 1.17; roughly 0.49 bits was the conditioning change, and serving enjoys that conditioning only for the first generated token. The audit's data set, artifact and sealed numbers are unchanged.

**Still failing:** the fit/tune-separated tuned two-token count reference at **5.1217** still beats the best arm (6.6841) by **1.56 bits/target**, so the 64-coordinate ternary state still uses local two-token information less effectively than a lookup table — a local-recency/optimization fact, not a long-range transport one. Greedy generation still collapses on every arm; sampling NOT_RUN; energy UNAVAILABLE.

**Next:** keep the **served window conditioning** and raise the grounded presence until the authored panels are fully retained (the configuration `olx-form-3` was one case short of), pre-declaring 32/32 preflight A, 3/3 held-out and 4/4 class as acceptance and measuring served prose between `olx-form-3`'s 6.9686 and `olx-form-2`'s 6.6841; then attack the 1.56-bit local deficit to the tuned count reference. Signed H4/shared relative transport remains conditional on a witnessed order/role/distant-interference failure against an information- and compute-matched ordinary control.

## Previous active — decisive artifact replay and a matched local-channel null result, September 23

**Executed audit (`olx-audit-4`, preserved `olx-audit-1..3`):** a fresh process deserialising the sealed `model.tlx` reproduces **every** panel exactly — authored training 36/36, held-out 3/3, class 4/4, preflight A 32/32 (route-key rule contradicted on 16), preflight B identical, prose token-conditional 7.372198274238398 == stored, generation and the five mixed phases identical. The lifecycle gap that the [principal review](ordinary-lexical-principal-review-2026-09-23.md) identified is closed: the loaded artifact equals the in-memory quantised instance. [Result](ordinary-lexical-audit-2026-09-23.md) · [evidence](../evidence/ordinary-lexical-audit-2026-09-23.json).

**Crossed copied-token intervention:** on one tested token pair, holding the selected-source fingerprint, typed facts, prefix and forced `Copy` action fixed and changing only the token handed to `transition` leaves the post-copy **top-1** decision at `Generate(808)`; changing only the fingerprint moves it to `Generate(1361)`; the no-copy control matches; the identity-erased arms alias. The delivered 2/2 is a content/fingerprint result. Emitted-token feedback may affect lower-ranked logits or other contexts; its learned causal role remains UNVERIFIED.

**Prose diagnosis (same 5,376 dev targets):** the earlier equality of full-action and so-called token-conditional NLL at 7.3722 was tautological: all targets were Generate and the latter still included Stop in its denominator. A corrected fresh-process replay (`olx-audit-5`) measures true Generate-conditional NLL **7.371855** versus Generate-action **7.372198**, a 0.000343-bit difference; Stop normalization is not the material gap. Mean Stop probability is 2.28e-4 and Stop is never chosen; hidden-state and fingerprint clamp saturation are both 0.0. Generate-action loss is 8.49/8.41/7.74/7.05 by `(prev,cur)` context frequency and roughly flat by position. A penultimate-token swap changes sampled state digests 100 % and top-1 choices 56 %; this shows sensitivity, not injective retention of token IDs. Checkpoint before the final grounded-only phase is **UNAVAILABLE**.

**Matched bounded local-channel comparator (`olx-channel-1`):** one learner, one split, one schedule, one seed, one architecture. Recurrence-only **6.2039** bits/target; recurrence + direct two-token channel **6.6732** (two-token minus recurrence-only **+0.469**, correctly oriented paired interval **[+0.410, +0.527]**); both trail the tuned count reference 5.1217. The immutable receipt mislabeled a negative recurrence-minus-two-token interval as its reverse; the [audit](ordinary-lexical-audit-2026-09-23.md) records the correction. This channel is not a sufficient remedy in the tested training configuration; representation redundancy and optimization burden remain alternative causes. The plain full-context recurrence-only formulation recovers about **1.17 bits/target** against the delivered 7.3722, but differs in context event and curriculum and is a prose-only diagnostic, **not** a successor artifact.

**Next:** repair the shared path's training formulation (per-position full-context objective, larger target batch) while keeping the grounded Copy/Stop supervision, confirm the prose recovery and the authored/held-out panels on open development, then close the remaining gap to the count reference. Add signed H4/shared geometric transport only on a witnessed role/order/interference failure against an information- and compute-matched ordinary control. Energy remains UNAVAILABLE.

## Previous active — shared lexical learner retained; causal and local-competence gaps under review

**Merged PRs #1353/#1354, principal qualification:** one artifact fits source-separated ordinary text and authored grounded Generate/Copy/Stop cases. The 32/32 temporal cases were fitted; the 2/2 class cases change both selected-source fingerprint and emitted-token feedback; three exposed compositions were used in oracle revision. Specific learned post-copy feedback use is **UNVERIFIED**. Main-runner panels used the in-memory quantized model; separate-process artifact replay checked only 36 fitted grounded cases. Five mixed phases are independent host-authored rollouts, not a persistent model-owned session. [Principal review](ordinary-lexical-principal-review-2026-09-23.md) · [original result](ordinary-lexical-result-2026-09-23.md) · [next DeepSeek brief](deepseek-ordinary-lexical-successor-2026-09-23.md).

**Ordinary text:** on 5,376 targets from 24 open development documents, full-action loss is 7.3722 bits/target, better than fit-only unigram 9.0827 and worse than donor E 6.8701 and tuned exact two-token count 5.1217. The count reference is a strong fit/tune-separated control, but its token-only denominator differs from the model's Generate/Stop denominator. Greedy continuations collapse. This gap does not identify longer context or geometry as the remedy.

**Cost and limits:** 466,711 B artifact, 93.9 MiB externally measured peak RSS, 673.6 µs/token rollout-only latency, 611,814 packed-coefficient inspections per generated step despite 56,457 nonzero coefficients; whole-path time and energy remain UNAVAILABLE. Bias-envelope artifact validation is corrected in the principal review. The live local ledger reads 380,915,640 / 390,700,000 ms after a separately disclosed 3,000,000-ms principal audit charge; the 3,500,000-ms increment reported for DeepSeek's runs was recorded once without a reconstructed timing breakdown. Only inactive compiler output was reclaimed, restoring physical space above the reserve.

**Next:** prove the loaded artifact's individual panel behavior and isolate post-copy feedback from the pre-copy evidence fingerprint. Diagnose the exact local two-token deficit and curriculum before trying a matched bounded local-ID channel or a better evidenced repair. Test signed H4/shared geometric transport only for a witnessed role/order or distant-interference need against an information- and cost-matched ordinary control. Preserve Hopf/S7/E8/harmonic variants as conditional tools. Continue toward useful prose, memory, reasoning and coding in the same native path.
...
**This task, executed (`state-lexical-v3-10`, reused artifact `state-lexical-v3-8`):** the retained state-conditioned decoder now consumes a **position-resolved, tail-carrying, unknown-explicit** content observation, the **actual emitted token identity** and an **exact 15-coordinate typed causal block** (requested history; derived; prior_differs; committed; key_changed; exact equality / common prefix / exact divergence / lengths / unknown counts) in its recurrence, and **continues after the copied span** in the authored changed-derived-key case. Executed on seven declared authored worlds: learned **31/31** (fitting cases 23/23, held-out cases **8/8** from three other documents), teacher-forced **791/791** through the exported integer artifact, retained legacy copy **31/31**, retained comparator **20/31**, recurrence ablation **0/31**, context ablation **6/31**, paired computed-request difference **4/4** (operation and key_changed also differ), ten in-process restart boundaries plus a separate-process child agree, and the E prior generates and S policy loads/validates in the submitted probe; the principal replay executes S separately. The version-2 artifact and its recorded results are preserved. [Result](transferable-lexical-result-2026-09-22.md) and [evidence](../evidence/transferable-lexical-2026-09-22.json) own the exact scope; the principal review that set this task is [here](state-conditioned-lexical-review-2026-09-22.md).

**Still open (the remainder of this milestone):** ordinary-text learning is **NOT_RUN**. The scoped contract supervises an exactly owned span, not open prose continuation, and the fitted content index covers only ~2% of a source-separated prose population (measured in the evidence receipt). The response sentence family is authored over a small declared state world; no geometric advantage is claimed and energy remains UNAVAILABLE.

**Next:** learn vocabulary and continuation from source-separated ordinary text through the retained session and a shared grounded Generate/Copy/Stop path. Assess E/S donor adaptation at the compatible emission boundary where it earns quality or cost. Broader conversation and executed Rust follow through the same candidate, then whole-model laptop cost and product delivery.
...
## Previous active — recurrent lexical component reviewed; transferable language remains open
...
## Previous active — reviewed combined observation/computation lifecycle

PR #1348 merged as `535b3ecc`. The combined learned observation/intent artifact preserves 38/38 prior outcomes (34 language +4 API), 37/37 ingests, and the narrow-support ablation reproduces 17/38. Primary observed ingestion is 72/72; the 32-case panel was first evaluated in attempt2 and replayed thereafter. Principal loaded interactions retain 42 parent +42 child restores across relevant corrections and 479 old control rows. The [review](observed-computation-lifecycle-review-2026-09-22.md) owns these exact boundaries. General prose and unique geometric advantage remain unqualified.
...
**Executed:** development **16/16**, exposed regression **11/11** and fresh final **11/11** complete answers with correct depths; binder **84/84** exact spans+roles and cue-role matches; intent **48/48** statement and **36/36** question; nonasserting inputs write nothing. Controls (NoRead 0/38, UpdateDisabled 1/38, Unscoped 33/38, ParseScoreAuthority 19/38, reduced capacity typed) each change the measured answers. A pinned read is not spliced by a mid-answer correction while the unpinned control is; an owned capture survives replacement with origin liveness false; physical storage order is irrelevant; a real disk reload and a **separate process** reproduce every answer. [Evidence](../evidence/scoped-correction-memory-2026-09-22.json).

**Localized obstruction resolved:** the first fresh final scored 9/11 because one unseen three-token entity name was cut by one token in its clause-initial occurrence, so its exact lexical key differed from the same name's internal occurrence. The cause was the observation boundary, not identity, admission or version eligibility. Widened supervision (one-to-four-token names, five values) and copula-free correction/nonassertion cues fixed it; that informed panel is retained as exposed and the fresh final was drawn afterwards.

**Next complete milestone:** consume one Q8-derived computation result through this same owned session so a changed computation changes a subsequent read or complete answer, with source/update-disabled controls and no fixed host schedule. Then retained E/S language prediction and executed Rust through the same artifact. Structural banks, Hopf fiber, signed Spin-H4 and conditional paired-H4/icosian E8, S7 and harmonic functions remain tools for a witnessed need; no geometric component participates in this milestone's serving path and none is claimed.

**Geometric contribution measured in this step: none.** The retained H4 artifact is unchanged and was not re-swept: its exact-hit code search is saturated, so another sweep is uninformative.

## Previous active: correction-aware scoped conversation memory
...
**Corrected candidate and accepted runtime are distinct.** PR #1344 remains a draft while its semantic fitting, tagged identity, supervision/model/restore checks and coherent interventions are repaired and exercised. [Checks](../evidence/structured-argument-binding-principal-checks-2026-09-22.json) and [corrected replay](../evidence/structured-argument-binding-corrected-replay-audit-2026-09-22.json) own exact outcomes. Accepted main runtime remains PR #1343 until the model draft is actually merged; a documentation merge does not promote it. No ordinary-form/fresh-final/geometric-superiority/whole-path efficiency claim.

**Next complete milestone:** [finish learned structured argument binding](deepseek-structured-binding-completion-step-2026-09-22.md) across readable interrogatives, reordered arguments and trailing nonanswer text, exact surface/lexical identity separation, same causal session. Use a coherent scored objective, adequate cue semantics, an order-aware categorical comparison and a coherently fitted H4 hybrid on equal support. Then durable scopes/corrections and consumed Q8 results, E/S prose and executed Rust. Structural banks/Hopf/Spin and conditional E8/S7/scalar fields remain tools for witnessed needs.

**Research leadership and autonomy:** Codex owns holistic mathematics/ML/systems review and roadmap revision; DeepSeek owns substantive implementation/diagnostic choices and complete lifecycle delivery. Necessary local extensions remain prospectively authorized, with cumulative debit and storage reserve preserved. No arbitrary short timer or retry quota replaces judgment. Failed causal instruments retain their data and failure status.

## Historical PR #1343 submitted account (corrected above)
...
**Verified measurements:** corrected primary and matched categorical/cyclic128/128; observed-type fit64/128→128/128; same-request diagnostic3/3 versus fixed-depth2/3; actual128checkpoint restore parity. All896 arm/request rows and2944 transitions independently audited. These are exposed typed-component and three-case development results.

**Next:** [observed-text memory answers in one reusable session](../integration/deepseek-observed-text-session-step-2026-09-21.md). Learn request/source-role observations and goal-preserving continuation from actual text, vary required depth with the same request, and emit complete selected phrases with stop/EOS. Reuse the corrected runtime and historical role/span/scheduler interfaces with explicit tokenizer adaptation. Broader language, durable corrections and executed Rust follow through that same engine. Signed H4/Spin/Hopf and structural scopes remain active tools; finite fields or paired-H4/E8/S7 follow a concrete representation or cost need.

**Research leadership and autonomy:** Codex owns holistic mathematics/ML/systems architecture, evidence interpretation and roadmap revision. DeepSeek owns substantive implementation/diagnostic choices and complete lifecycle delivery, with prospectively accounted necessary local extensions. No arbitrary short timer or retry quota replaces scientific judgment, preservation or physical limits.

### Previous active (superseded by the section above)

## Former active: reviewed grounded computation; learned relational control next
...
## Former active: shared learned transitions and complete result continuation executed
...
Complete model/build charge137542/240000ms; shared123375513/132950000ms; parent3624940/3730000ms after pre-recorded180000ms extension. Parent storage cap increased by64MiB before use; current step128MiB and 128 MiB stop margin. No paid compute, cleanup or V3–V7 replay. Final delivery/storage accounting appends locally.
...
cycle ceilings540s/1500s, one model process,4GiB child RSS target and 128 MiB storage
...
12/16 to 16/16 with no lost correct cases. Prior62/62 and 24/24 sets are restored,
