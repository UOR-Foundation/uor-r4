...
## Active: grounded factorization and an owned dependent session executed

**Executed the grounded dependent-session brief** on merged `85df1739` (PR #1339). New module `learner/grounded_session.rs` and mode `--mode=grounded-session`.

**Part A.** A **constructive factorization** of the observed transition graph: the declared development labels give one permutation of the typed outcomes per observed primitive (bijective, distinct, closed; order spectrum 1x1, 1x2, 6x4 -> Q8); an exhaustive isomorphism search into a witnessed quaternion subgroup of the served table gives `A[p] = phi(perm_p)` and `Z[outcome]`, with `E[payload] = inverse(A[p])*Z[first outcome]` checked across every primitive and the reference outcome fixing the gauge. **All 672 declared development transitions are reproduced exactly** and the factorization matches the competent shared finite-state control on every panel - **288/288 development, 124/128 unseen length four, 64/64 unseen reversal** - carrying **102 serialized bytes against the control's 1668**. The fitted recurrence of the previous milestone reaches 248/288 on the same supervision, confirming the review's diagnosis. This is a compression/sharing result on supplied intermediate supervision, not language capability.

**Part B.** `SessionFrame` carries an exact evidence lease (sequence, absolute position, payload), retained state/outcome, phase, emissions and a typed terminal status, with typed `read`/`apply_primitive`/`emit`/`stop` and `RLSF` v1 serialization. Paused frames resume identically (4/4 chains) and advancing one frame leaves an unrelated one unchanged; a lease from another sequence or an overwritten payload is explicitly stale.

**Part C.** A genuinely dependent read: the second hop's keys are the **typed outcome labels**, so the first computed result forms the second query. First hop 4/4, second selection 4/4, complete answer 4/4. Changing the first source changes the first outcome, the second query key, the selected second occurrence **and** the final answer together; an added irrelevant record preserves all three; **removing the required record** and disabling the reader each return `NoRead`. The answer is a label token, absent from every record payload.

**Corrected:** the reversal failure is computational/learning failure, not reader confounding (every source is correct); the development probe was value-index parity, not sequence-disjoint validation; the whole-program cache was not a competent baseline; the `remaining` stop table is explicit program completion, not learned continuation.

**Next:** learned continuation rather than supplied-program exhaustion, semantic query formation rather than exact-key retrieval, persistent source ownership across hops, and then source-separated prose and an executed Rust task on the same native path. [Result](grounded-session-result-2026-09-21.md); [evidence](../evidence/grounded-session-2026-09-21.json). Retained roots `grounded-session-{1,2,3,4}` (0 unlisted). Energy UNAVAILABLE; whole-path D0-b not claimed.
