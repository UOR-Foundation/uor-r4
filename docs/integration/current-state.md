...
## Active: the corrected policy experiment is a clean negative

**Corrected reader-policy experiment executed (2026-09-21).** PR #1328's policy was misindexed: fit events were built from the legacy parent's `[0,0,0]` thresholds, so every event landed in gap band zero (buckets 0-7) while serving addressed all 32. One immutable `PolicyConfig` now supplies the fit-time selector, the events, the export and serving, with an acyclic binding (config digest from declared semantics plus thresholds; the artifact carries the fit-input digest; the delivery manifest carries artifact byte hashes) and a verified loader contract called before prediction. [Result](reader-policy-contract-result-2026-09-21.md), [design](reader-policy-contract-design-2026-09-21.md), [receipt](../evidence/native_geometric_reader_policy_contract_2026-09-21.txt).

**The corrected fit is a clean negative, and the PR #1328 text finding does not reproduce.** On 8 **new** disjoint reader documents (1,952 positions) the corrected H4 policy is **+0.1193 bits/token [+0.0702,+0.1679]** and categorical +0.1533, where the frozen parent is +0.4532, while a **constant one-nat boost** on the same ungated source and contract gives **-0.0148**: the small held-out benefit came from a generic small boost, not the fitted table. On the previously inspected documents the corrected policy gives +0.1469 versus the misindexed -0.0195. The corrected fit is materially different: 16 buckets earn their own action and the opcode histogram is [0,0,25,7], not [0,0,31,1]. **All four declared categories are false** - harm containment, useful transfer, relational preservation (present-query emitted correct **56 vs the parent's 77**) and absence (reads **19 vs 5**, loss **0.2969 vs 0.1379** bits/absent query).

**Metrics repaired and verified.** The actual-action decomposition satisfies `ranking + gate + dose = actual - Lpool` with **0** actual-vs-emitted-logit mismatches and 0 served-source disagreements; admission regret now uses correct denominators and reproduces the principal review's independent reconstruction (**0.1946** candidate-only / **0.8316** complete-stream nats/position, empty pools with `Lpool = 0`); the four-condition intervention shows disabled invariance **140/140** and the original occurrence still admitted **140/140**; reload rejection of a drifted threshold config, a swapped opcode table and a mismatched input identity passes with `reload_failures = 0`; a serialized-structure preflight validated the object actually written before sealing.

**Diagnosis.** The unresolved distinction is an **objective tradeoff** between expected action cost (fitted) and emitted-argmax correctness (measured) on a mixture whose text half has low copy reliability - not a representation limit and not insufficient support. Per-stratum reads, NoRead, action counts, correct payload reads and emitted-correct counts are serialized, and per-bucket/per-action costs are additive with fixed sources.

**Next constructive task (single successor):** a prospectively declared **constrained/Pareto utility objective on the same features** - choose the per-bucket action minimising expected cost **subject to** emitted-correct preservation on a declared construction stratum - before any new observation or representation is introduced. This is a bounded check on already-serialized costs, not a sweep; no admission change, ring widening or S7 step is justified by this evidence. Reader incremental cost and physical energy remain UNAVAILABLE.

Delivered sealed root `.uor-models/realtext-prior-2026-09-20/reader-utility-4` (0 unlisted). `reader-utility-2` stays sealed and byte-identical and `reader-utility-1` stays unsealed. Superseded attempt `reader-utility-3` is retained and never resealed: identical measurements with one bookkeeping defect in the new rejection probe.
