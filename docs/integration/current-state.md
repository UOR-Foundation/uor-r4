...
## Active: learned relational access and content-dependent session control executed

**Executed the learned relational-control brief** on the reviewed head `5821c48d` of PR #1340. That PR was still queued when the work started, so protection was preserved and the reviewed head was read directly; PR #1340 has since merged as `d9d896e8` with contents identical to the reviewed head for every file this work touches, and this branch is rebased onto that merge commit. New module `learner/relational_session.rs` and mode `--mode=relational-session`.

**Task.** Supplied memory worlds of `(role, key, value)` triples, every entity carrying four plausible facts. The record role token encodes the relation while the request names it, so the correspondence must be learned. Candidate admission is an exact identity join on the key; ranking uses a **learned relational compatibility** (`inverse(A[relation])*Z[role]` is the identity, with matched categorical and cyclic controls); a learned gate makes a request with no compatible candidate **explicitly unresolved** rather than emitting an incompatible fact; and the continuation decision is learned over observed content (the retrieved value is a key of the world).

**Result on fresh final worlds:** the primary completes **128/128 development and 128/128 final** with **128/128 correct depths**, emitting a decoded answer token per request. Freezing the learned relation match, freezing continuation to always-continue, and disabling reads each drop complete answers to **0/128**. The matched categorical and cyclic controls also reach 128/128, so this establishes learned relational access and control, not geometric advantage. Removing exactly the required record gives an explicit unresolved silent stop in 16/16 worlds.

**Causal interventions:** a single-position source edit changes the first retained value, the follow-up relation query, the selected second occurrence and the final answer together and matches the independently derived expectation; changing only the request relation changes the selected fact and answer with all records fixed; an added plausible distractor preserves answer and selection; pause/resume is identical on 128/128; interleaved frames stay independent; an owned captured payload stays usable after its origin was overwritten while separate origin liveness reports false. The 140-byte artifact is exported and independently reloaded with full-predictor parity on 256 requests.

**Next:** broaden the same path to varied request renderings and source-separated prose, then an executed Rust task; the supplied layout, the small world and the single-hop joins are the current scope limits. [Result](relational-session-result-2026-09-21.md); [evidence](../evidence/relational-session-2026-09-21.json). Retained roots `relational-session-{1,2,3,4}` (0 unlisted). Energy UNAVAILABLE; whole-path D0-b not claimed.
