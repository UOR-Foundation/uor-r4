...
**Next action.** Measure the VSA **routing** role (the codebook also feeds `route_shortlist_q30_with_memory`, which `ablate-prose` does not exercise), then obtain a **finer representation-derived code** — which needs a new artifact section for the continuous embeddings plus a re-export, and must be paired with a richer per-head read (separate identity and similarity channels). Repair or remove the two D0-a violators (the JEPA `i64` projection and the `f64` sampler). Confirm the NEGLIGIBLE bands on a second disjoint slice and run the `jepa` × `s2_readout` factorial before any further removal.
