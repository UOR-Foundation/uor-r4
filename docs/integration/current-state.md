...
## Active: frozen separable-reader attribution — EXECUTED, decisive negative

The [result record](s-attribution-result-2026-09-20.md) and its [evidence receipt](../evidence/native_geometric_s_attribution_2026-09-20.txt) report one complete **evaluation-only** experiment from base `8bbdb63d`, in the isolated worktree `codex/separable-attribution`, sealed and verified at `.uor-models/realtext-prior-2026-09-20/s-attribution-3` (30 files, 4,390,694 bytes; two earlier roots preserved sealed and superseded on cost-protocol and binary-identity grounds with identical numbers). No optimizer update, no reset/write or capacity fit, no new corpus, no projection campaign and no decoder change: the harness constructs no trainer.

**Repairs delivered.** (1) All three legacy `CPX3` tokenizer fields were the all-zero placeholder. Production export now *requires* the real raw 32-byte identity (`ZERO_DIGEST` is refused by `from_parts` and `QueryTrainer::new`); the hash-pinned legacy files load only through `QueryHard::import_legacy`, which the run verifies is the only path that accepts them. Corrected descendants are the identical size with **exactly 32 changed bytes, all inside 73..105**, and full row/integer-logit parity. Q `341f0721…`→`5adba9712ec46888…`, S `9f8cda09…`→`ed9affd638ee9367…`, L `b0e50887…`→`167e2ea045ed3ac1…`. (2) `inference_rows`/`rows_from_older` separate inference row selection from training-chain construction — S folds the older prefix **62** times instead of 124, L **3** instead of 65 (instrumented compose counts) — with row equality against the training path at every position. (3) `validate_tokens`/`generate_checked` reject out-of-vocabulary tokens and empty prompts instead of clamping. (4) `bin/query-read-attribution.rs` is the evaluation-only entry point. 18 `query_read` module tests pass.

**Reproduction first.** All **16** retained old-panel vectors (E; own/donor/identity-query/reversed/disabled for Q, S, L) recompute to a **maximum absolute delta of 0.0**; E micro CE 7.170815717556; the 288-window/17,342-target panel and its donor map match the retained manifest record-for-record; S11 reproduces its retained six-prompt generation exactly.

**The finding.** On the frozen decomposition `Z_S = E + u(q) + u(b)`, with the integer identity `Z11+Z00=Z10+Z01` verified with **0 failures** on both panels:
...
| S00 = E+2u(e) | 7.501118 | 7.387922 |
| M01 fit-average history (offline) | 6.984779 (older-present) | 6.894588 (older-present) |

On matched older-present positions, paired 2,000-draw document bootstrap, seed `0x12345678`: **primary M01−S11 = −0.018695 [−0.022344,−0.015019]** (new) and **−0.023622 [−0.026746,−0.020352]** (old); **Qonly−S11 = −0.007348 [−0.012050,−0.002327]** (new) and **−0.009978 [−0.014586,−0.005420]** (old); Honly−S11 = +0.129926 / +0.143589. Every interval is entirely below or above zero, with 0 zero-support resamples.

**Reading: S's measured gain is a current-token query/emission calibration, not history.** `Honly` (the older row alone) is statistically indistinguishable from E. Dropping the older row (Qonly) *improves* CE on both panels, and a content-free length-conditioned fit average (M01) beats the individual state by ~0.02 bits on both panels. The identity-anchored constant `u(e)` is harmful, which is why S01 is a poor local comparator. The exact-tail donor map is silent (−0.002298 old, reproducing the retained value; +0.005278 new, interval spanning zero) because it holds the length-conditioned offset fixed — the only part of the history term with a measurable effect. The sealed `result.json` carries a conservative `"inconclusive"` label from a two-branch rule keyed on S01; the review records the corrected classification: **the prompt's second branch — preserve the local result and target state maintenance directly; do not advertise the 120-state group product as useful memory.**

**Controls and generation.** E/S01/S00/Qonly are *exactly* invariant to donor and reversal on both panels; identity and coincidence counts reported separately (old q=e/b=e/q=b 303/4501/290). Every condition still repeats on 6 of 6 prompts at the local-pair level and only the all-32 prompt reaches a full bounded-ring cycle; repetition is reported, not read as language quality.

**Cost.** Under one declared protocol (retained prompt, 64 greedy tokens, one discarded warm-up then five timed repeats, `black_box`, complete numerical path): E 0.530 ms, S11 seam 0.770 ms, S11 old path 0.800 ms, S01 0.780 ms, Qonly 0.650 ms (medians). Whole-run peak RSS **722,681,856 B**; wall 102.47 s. The run binds its own executable SHA256 and both source-file hashes. Parent 454,788 B; each corrected artifact 53,555 B. **Physical energy UNAVAILABLE.**

**Next action:** build the smallest mechanism that can make older content *addressable* — a bounded selective write/reset or exact occurrence/version access — and qualify it against fresh behavioural criteria, not another score comparison on this panel. `M01` is offline analysis only and is never a serving incumbent. CPQK production continuation remains required **before** any new fit and is not repaired here. #973/#820/#963/#964 stay open.

**Resources:** the standing-authorized +2400000 ms extension to 180500000 ms was recorded before use; this step charges 2400000 ms once. See [ledger](resource-ledger-2026-09-19.md).
