...
## Active: contextual utility controller works; source ranking is now the bottleneck

**Contextual utility: preregistered positive.** A bounded causal action-dependent interaction (including NoRead) now replaces the factorising `sb[a]`. On the repaired construction (2,969 fit / 1,813 fresh candidate-bearing positions), contextual minus frozen global strength is **-0.257644 bits/candidate position, sequence-cluster 95% interval [-0.327088,-0.182968]** (140 sequences, 2,000 draws). All five preregistered conditions hold, including that the arm keeps reading (0.532 vs 0.365) and that the natural-text tradeoff improves (+0.2189 vs +0.2443 bits/token). The controller reads *more* and loses less, so this is not an all-NoRead artefact. Result: [contextual utility result](contextual-utility-result-2026-09-20.md); receipt: [evidence](../evidence/native_geometric_contextual_utility_2026-09-20.txt); retained sealed root `.uor-models/realtext-prior-2026-09-20/contextual-utility-2`. `RLR2` moves to v3 carrying the table (v2 readable); the artifact grows 64 serialized bytes.

**Critical negatives, not to be smoothed over.** On the *repaired* construction the H4 relation term is **not** significantly better than exact recurrence: **-0.010151 bits [-0.104470,+0.083675]**. The old -0.179540 belonged to the defective construction. The repaired categorical comparator now genuinely learns (149 assignments moved, objective -4485.84 -> -4973.43) and is the **strongest** static arm at **10.3936** bits, ahead of the geometric arm (10.7185) and the contextual arm (10.4608); the relation-channel lesion (10.7003) also fails to hurt. Unique-H4 benefit is therefore still **not** established, and the useful component is the learned contextual utility/code, not the H4 relative element. Natural text is still harmed in absolute terms (+0.2189 bits/token, above the old 0.05 tolerance; the panel is inside the fit mixture, so development evidence only, and the old +1.2908 came from a different fit/construction). Generation stays degenerate, though adjacent-token repetition falls on several prompts.

**Diagnosis to act on.** The target-using opportunity bound splits the residual headroom: mean strength loss **0.0933** nats versus mean ranking/admission loss **1.1939** nats (~12.8x). Where the correct payload is admitted (646 positions), the relation channel ranks it top only **46.3 %** of the time and the arm selects it only **25.2 %**, while the best finite action belongs to that correct source at **100 %**. Strength is largely repaired; **ranking is not solved**. Candidate-level exact-feature interactions cannot fix this — competitors share the query key and differ only in the preceding role token, so the only separating signal is the partner relation, which an exact-feature bucket cannot express.

**Next constructive task (prospective, single successor):** a matched-cost comparison of the **source term at higher resolution** on a **fresh construction draw** — the shared scalar `rank[rel]` over the 120-root descriptor versus a learned directed compatibility indexed at the `(query role, source role)` pair — with the contextual utility controller frozen on both sides and acceptance criteria frozen before scoring. Judge on covered-position selection success (the measured 46.3 % top-rank / 25.2 % selected gap) as well as hard-action CE, and keep the categorical and exact-recurrence comparators. Do not widen the ring, sweep widths, re-run S attribution or start a scheduler.

**Evidence limits that remain.** Served active-candidate incremental cost stays **UNAVAILABLE**: the 32-token cost probe admitted zero candidates, so it timed the local path (~1.37-1.41 ms/prediction, release). Whole-path D0-b compliance is not claimed; only the served selector arithmetic is bounded to integer add/compare/table reads over signed 4-bit entries. Physical energy UNAVAILABLE. The superseded attempt `contextual-utility-1` is retained, never resealed; its only defect was a struct-equality self-check, and repairing it left the measurement bit-identical.
