...
## Active: the read-confidence interface is preserved and feasible; text is not improved

**PR #1332's obstruction is resolved at the interface, not in text.** The witnessed rule `D>0 -> ungated top at eight nats, else NoRead` reproduces the frozen scored parent's own action on **2,969 development positions with 0 mismatches**, so the retained reader's learned Read/NoRead decision is carried into the influence address (`utility_bucket*2 + 1[D>0]`, 64 opcodes). Unlike the coarse 32-address class, this class is **feasible and optimal** on development: H4 selects text **+0.0228 bits/token** with present 148/149 and absent reads 7/7; categorical +0.0036 with 158/159 and 5/5; the raw witnessed parent rule itself would fail the text screen at +0.5724. On the one declared **fresh** draw (new construction seed and the four remaining `Dev` documents, 976 positions), `h4_confidence` keeps present answers (71/117 vs parent 73) and text inside the screen (**+0.0379**) but reads 11 absent queries against the parent's 10, failing absence by one; `categorical_confidence` preserves both (79 present, 7 absent reads, text +0.0201). **No useful text gain was obtained**, so no positive influence claim is made. [Result](reader-confidence-result-2026-09-21.md); [design](reader-confidence-design-2026-09-21.md); [evidence](../evidence/reader-confidence-2026-09-21.json).

**Next:** end this bounded influence integration. The evidenced successor is to separate the **natural-text stream from the present-query stream at the address** — a prospectively declared query-type/role observation or an explicit per-position reliability signal — because the sign-of-D bit fixes the present/absent collision but the five-bit bucket still merges text with present positions, and that shared eight-nat dose is the residual text harm. Do not add persistent state or dimensions from this result. Interface repairs are delivered: source-hash completeness (three modules), the actual revision recorded, the verified one-nat predictor used, and neighbour accounting corrected (edited occurrence excluded; 140→20 changed, 16 removed, 0 lost admission). Whole-path D0-b and physical energy remain unqualified.

Delivered root `.uor-models/realtext-prior-2026-09-20/reader-confidence-3` (sealed, verified, 0 unlisted, manifest `d81bda9e3dbff98a69ba5020631527698fbad83f3359611c1a15c5f3d6b0255f`); superseded `reader-confidence-1`/`-2` retained. Counterfactual identity residual 1.42e-13 bits over 5,442 positions; `reload_failures = 0`. Ledger and charges: [resource ledger](resource-ledger-2026-09-19.md).
