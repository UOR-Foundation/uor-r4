...
## September 20 principal audit: useful selection, unresolved ranking and copy utility

[PR #1314](https://github.com/UOR-Foundation/uor-r4/pull/1314) is delivered as `fbf542aa4c1f7ed336cd028c6b7b3502cba4118c`. The [correcting audit](occurrence-reader-audit-2026-09-20.md) reviewed its unchanged source/results and verified all retained manifest hashes. Original [result](occurrence-reader-result-2026-09-20.md), [design](occurrence-reader-design-2026-09-20.md) and [raw receipt](../evidence/native_geometric_occurrence_reader_2026-09-20.txt) remain preserved with that correction. No model is promoted.

- Constructed fresh reader-relevant accuracy is **411/443**, versus latest occurrence **279/443**. The slot-equivalence subset is **99/120** versus **64/120**; it uses a supplied eight-class partition, not demonstrated directed geometric transport or semantic advantage.
- Raw text has **5 correct and 83 wrong reads among 88**. Five wrong selections are among ten covered positions; 78 are uncovered. The earlier 10/10 ranking claim counted any read. Synthetic/raw all-position CE worsens **+0.6713/+2.7891 bits/token**. Both ranking and safe use need improvement.
- Future-token intervention was tautological; ReadDisabled only checked residual identity; the geometry coefficient ablation changed exact-match weighting; paired support against latest was not calculated. Cost timed admission in both paths and divided 29 predictions by 32. These particular PASS/attribution/overhead claims are withdrawn, while the inspected causal data flow, exported parameter equality and real three-process continuation remain useful evidence.
- The retained copy residual is **16 nats**. Next learn ranking, NoRead and bounded copy strength against actual language loss, with the smallest contextual distinction needed. Preserve frozen E plus S-query-only local emission. Do not freeze the ranker/amplitude based on the old precision-only diagnosis.

**Completed handoff reconciled:** the newly supplied final narrative is the same PR #1314 run, already audited in PR #1317. Source and retained evidence are unchanged; no successor has run. One additional correction: construction fitting already used 737 read and 515 NoRead labels. Poor transfer is not proof that abstention is unlearnable from construction. The [audit's handoff reconciliation](occurrence-reader-audit-2026-09-20.md#completed-handoff-reconciliation) records the actual population and margin-derived amplitude. The next prompt now specifies causal gain features, loss-based finite actions and an always-NoRead comparison while retaining a required contextual benefit.

**Next, after owner clarification:** execute the [contextual geometric reader prompt](deepseek-relational-reader-step-2026-09-20.md). Learn causal query/source descriptors and directed H4 relations over exact memory; a repaired/calibrated PR #1314 selector is the comparator. Copy utility remains the integration objective. Current exact-key admission and two-neighbor equality cannot test nonidentical-cue relational access, so one declared causal support change is allowed when required, with identical pools for ranking comparisons and all scan costs included. Full S7/harmonic memory and persistent role banks remain conditional successors. This is an adopted hypothesis, NOT_RUN; the old calibration-only draft is redirected, not a second task.

**Retained resources:** `.uor-models/realtext-prior-2026-09-20/occurrence-reader-4`, 21 files/3,385,009 bytes; earlier roots preserved. Live model ledger at this review: **181238565/191300000 ms**, about 167.7 minutes remaining. Preserve the prior 2700000-ms charge once; refresh before execution. This review performed no build, model forward, fit, artifact rewrite, cleanup, paid compute or ledger mutation. Source/document validation is separate from model evidence. #820/#973/#963/#964 remain open; wider capability and energy claims remain unqualified.
