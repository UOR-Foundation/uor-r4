...
**Next:** the [comprehensive memory-utility prompt](deepseek-memory-utility-step-2026-09-20.md), combining focused instrument repairs with constructive learning. The old fresh/raw panels become regression/development evidence; choose genuinely new evaluation only after design selection. No whole historical replay or unused CPQK repair is required. Role/scope persistence, directed finite transport, dependent reads and conditional S7/harmonic comparisons follow the [canonical sequence](project-track.md#structural-memory-and-geometric-representation-follow-up), supported by the [current/retired mechanism synthesis](geometric-attention-mechanism-synthesis-2026-09-20.md).
