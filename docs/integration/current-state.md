...
The [result record](relational-reader-result-2026-09-20.md), [design note](relational-reader-design-2026-09-20.md) (written before scoring) and [receipt](../evidence/native_geometric_relational_reader_2026-09-20.txt) report one complete constructive task from base `647483ab`, sealed and verified at `.uor-models/realtext-prior-2026-09-20/relational-reader-1`. Four arms share one causal pool, one action set, one dose and one objective; only the relation index differs. The [correcting audit](occurrence-reader-audit-2026-09-20.md) and [mechanism synthesis](geometric-attention-mechanism-synthesis-2026-09-20.md) remain the interpretation layer for PR #1314.

**Mechanism.** `q = Q(x_{i-1})`, `k = Q(x_{j-1})` over a **learned** 120-valued descriptor initialised from the frozen S write slots, `r = inverse(q)*k` (common-left invariant), `s = bias + rank[r] + Σ w_e φ_e + sb[a]`, learned action set `{NoRead} ∪ {(candidate, strength)}` with strengths 0.0625/1/8 nats. The descriptor is learned by a bounded discrete coordinate search on the **actual one-step action loss** of the observed next token; only **8 of 120** rank entries are non-zero and **50 of 4,096** roots moved from the frozen initialisation. The shared pool admits a candidate sharing any exact context token (`cur`, `prev2`, `prev`), newest-first, bounded to 24 of a 128-token ring, and every arm receives the exact indicators.

**Matched fresh comparison** (940 positions, 233 covered, 490 geometric; held-out assignments, frozen before scoring):

| arm | covered precision | reads | no-read | expected action loss (nats) | real CE Δ (bits) |
| --- | ---: | ---: | ---: | ---: | ---: |
| local (always NoRead) | — | 0 | 1.000 | 0.000 | 0.000 |
| exact-recurrence (no relation) | 0.755 | 940 | 0.000 | −0.807 | −1.082 |
| categorical control (arbitrary code, same table size) | 0.709 | 858 | 0.087 | −0.722 | −0.995 |
| **relational (learned descriptor)** | **0.873** | 832 | 0.115 | **−1.024** | **−1.500** |

Paired by sequence: precision **+0.0987 [+0.0418, +0.1603]**; expected action loss **−0.2172 [−0.2976, −0.1342]** nats.

**Decision: promising near miss, not promoted.** The predeclared rule required a precision lead ≥0.15 *and* a loss improvement ≥0.01 nats; the loss criterion passes decisively, precision is above zero with an interval excluding zero but below its bar, so the label stays **not positive** unchanged. Two findings make the miss informative: the **nongeometric categorical control is worse than no relation at all** (0.709 vs 0.755), so the gain tracks the group structure rather than a larger contextual table; and abstention emerges at ~11% without being imposed. The successor should state its practical margin on the calibrated quantity.

**Instrument repairs:** payload-correct ranking counter; a real future-token intervention (444 earlier positions compared, 0 changed decisions); a full matched fit as the comparison arm instead of post-hoc weight zeroing; paired intervals against that comparator; a local cost arm with **no reader work at all** (1,263.9 µs vs 1,260.1 µs per prediction); the SHA256 of the executable bytes rather than a hex re-encoding; and complete export/reload parity (940/940 positions on action *and* emitted argmax, 0 mismatches). One control is **mis-specified and diagnosed, not hidden**: the independent S-query-only reconstruction evaluates `u(b) + u(e)` while `z_local` is `E + u(b)`, so it differs by exactly the identity-row constant at every position; the baseline definition is unaffected.

**Next action:** re-run the same frozen panels with the corrected parity control and a prospective margin stated on the calibrated expected action loss, then carry the learned relation into **one dependent two-hop read with a learned scheduler**. Do not widen the ring or candidate bounds, sweep widths, reopen the frozen S attribution, or start a reset-only campaign. `#973`/`#820`/`#963`/`#964` stay open; physical energy UNAVAILABLE.

**Resources:** no extension was required; the projection fits the existing 191300000 ms limit. This step charges 900000 ms once. See [ledger](resource-ledger-2026-09-19.md).
