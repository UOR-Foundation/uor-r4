...
**Crossed copied-token intervention (decisive):** holding the selected-source fingerprint, typed facts, prefix and the forced `Copy` action fixed and changing only the token handed to `transition` leaves the post-copy decision at `Generate(808)`; changing only the fingerprint moves it to `Generate(1361)`; the no-copy control matches; the identity-erased arms alias. Classification: `decision_tracks_fingerprint = true`, `decision_tracks_feedback = false`. **The learned post-copy decision depends on the evidence fingerprint `m`, not on the token emitted by `Copy`**, so the delivered 2/2 is a content result and the learned role of emitted-token feedback remains UNVERIFIED.

**Prose diagnosis (same 5,376 dev targets):** full-action NLL equals token-conditional NLL at 7.3722; mean Stop probability is 2.28e-4 and Stop is never chosen; hidden-state and fingerprint clamp saturation are both 0.0; loss is 8.49/8.41/7.74/7.05 by `(prev,cur)` context frequency and roughly flat by position; state digest changes 100 % under a penultimate-token swap while next-token top-1 changes 56 %. Checkpoint before the final grounded-only phase is **UNAVAILABLE** (no intermediate state is persisted).

**Matched bounded local-channel comparator (`olx-channel-1`):** one learner, one split, one schedule, one seed, one architecture. Recurrence-only **6.2039** bits/target; recurrence + direct two-token channel **6.6732** (worse by 0.469, CI `[-0.527, -0.410]` excludes zero); both behind the tuned count reference 5.1217. The direct last-two-token channel is **falsified as a sufficient remedy**, and the plain full-context recurrence-only formulation recovers about **1.17 bits/target** against the delivered 7.3722 without geometry. That arm is a prose-only diagnostic and is **not** a successor artifact.
