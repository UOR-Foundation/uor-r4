...
## Active — learned grounded lexical realization

**Delivered — learned grounded lexical realization:** [result](grounded-lexical-realization-result-2026-09-22.md). A compact finite context-indexed table fitted from declared development response text selects Copy / learned word / Stop, and changes an **uncopied** word from **older owned evidence** while the request and the generated prefix up to that decision stay fixed: `it is Harbor` becomes `it was Harbor` on the same request and the same copied span; a consumed computation gives `it is now Tarn`; a same-value reassertion keeps `it is`. The context-disabled arm collapses the pair, the source-disabled arm emits nothing, and the same artifact keeps the retained lifecycle (56/56, 24/24, 32/32 panels; **38/38** prior matched, 0 missing, 0 different; 392 rows). Legacy answers remain byte-exact. An [independent reconstruction](../evidence/grounded-lexical-realization-audit-2026-09-22.json) rebuilds every claim from the saved emission records, not from the runner's booleans.

**Limits of that delivery:** four learned words, response *fragments* (`it is Harbor`), a development-exposed population, the `E/S` donor deliberately unused, and a bounded geometric evidence class that is computed and reported but **not** a key field (an exact-key lookup cannot generalise to an unseen class). No geometric advantage, broad prose, whole-path D0-b or energy saving is claimed.

**Next:** widen the learned lexical interface before adding representation — a source-separated text fit with a real learned vocabulary distribution and a variable-length learned prelude/suffix, keeping the same causal key, owned-span invariants and versioned output contract. Do not restart arithmetic fitting or a scheduler redesign: the measured limit is support and vocabulary width, not representation. Then broaden conversation/prose and executed Rust, followed by complete laptop/API/WASM/Studio qualification. Hopf fiber, E8/S7 and finite harmonic/scalar mechanisms remain tools for a witnessed distinction or cost need.
