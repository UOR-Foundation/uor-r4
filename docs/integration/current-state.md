...
**Delivered input:** [PR #1312](https://github.com/UOR-Foundation/uor-r4/pull/1312), merge `0bca39b541e0ee56cc58bfff65a625a7854d5433`, head/merge tree `482a10a49d4046a817622b0c4396534e2f271c92`. The source and report were already committed. All three attribution roots verify; retained `s-attribution-3` has 30 files/4390694 bytes and result SHA256 `994a1a15080ecc0c524aaa6ae6d93428c01f6fe4c5fd4d18ee80394d32a92165`. Corrected CPX3 files differ only in the 32 tokenizer bytes; the retained executable still matches its recorded hash. No training or model/build execution ran in this principal review.

| Frozen condition | Old panel, all targets | New positions, all targets |
| --- | ---: | ---: |
| E | 7.170816 | 7.048239 |
| S11: E + older row + query row | 7.032228 | 6.919775 |
| S-query-only: E + query row (old label Qonly) | **7.022581** | **6.912660** |
| Honly: E + older row | 7.171048 | 7.045576 |

Units are bits/target. The old panel has 36 docs/288 windows/17342 targets, 16766 older-present; the new panel has 35 docs/277 windows/17451 targets, **16897 older-present**. The 16692 figure is a strata count. Both panels use the same open-development document set. Older-present Qonly−S11 is −0.009978 [−0.014586,−0.005420] old and −0.007348 [−0.012050,−0.002327] new; saved-vector reaggregation reproduces these intervals. All generations remain repetitive and no useful-language candidate is promoted.

**Corrections carried with the evidence:** M01 includes one terminal state without a prediction target per fit window: 4096 extras, only 34 affecting evaluated lengths. There is no pooled-mean comparison identifying length dependence. Automatic equivalence/old-harm rules have latent mistakes. New donor indices are not saved; the selection is deterministic and written before scoring but sealed only at final report completion. `utc_start` is actually a report-time timestamp, phase timing is incomplete, and the cache is not bounded in source. The legacy import is hash-restricted by this runner, not intrinsically by its public API. These limitations do not erase Qonly's independent gain or justify rerunning the entire experiment. [Detailed review](occurrence-reader-review-2026-09-20.md).

**Cost scope:** timed evaluation reuses a 16390×4096 i32 parent cache (268533760 payload bytes) and precomputed 120×4096 residual tables (1966080 bytes each), excluding their construction. The E timing also does unused S row work. Artifact bytes (454788 parent plus 53555 reader) are not this harness's resident footprint. The old 0.53–0.80 ms numbers remain cached harness observations; direct serving latency and physical energy are UNAVAILABLE at this boundary.

**Next:** execute the [comprehensive occurrence-reader prompt](deepseek-occurrence-reader-step-2026-09-20.md). Freeze E and S's local query contribution, reuse exact sequence/slot memory and candidate-relative geometric selection, fit one small reader/NoRead integration, and demonstrate counterfactual distant binding through the current BPE token path. Compare a competent fixed exact-prefix/latest-occurrence selector, ReadDisabled, changed-source and actual generated behavior, then a bounded raw-text probe with complete cost. A reset-only 120-state update changes forgetting but adds no payload addressability. DeepSeek may improve the implementation and practical targets prospectively with a documented rationale; correctness/causality and honest evidence remain requirements.
