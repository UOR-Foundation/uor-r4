...
**Remaining ceiling, now well identified.** At context 16 the alphabet is small enough that tokens repeat *within* the run, so a first-order `(prev → next)` address accumulates several successors and becomes ambiguous — inherent to bigram addressing. The fix is **higher-order addressing**: an address formed from an *ordered word* of group elements composed through the 2I table. This is exactly W33's ordered-operation point (`P²=2P+3I`, `L²=2L+3I`, `PLPL=LPLP`, `Ω=LP−PL`, `ker(Ω)` insensitive to order — see [`nemesis-w33-relevance.md`](nemesis-w33-relevance.md)): the same operation multiset can act differently, so an ordered composition carries information a set or a hash cannot. That is the next mechanism to build and falsify. Still not a solution, and still not a language model.
