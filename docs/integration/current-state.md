...
## Active next: recover the prefix negative and isolate output-head learning

The [review after PR #1304](prefix-pilot-review-2026-09-20.md) preserves the failed numerical prefix pilot and corrects artifact, generation, statistical-preservation and resource claims. The [complete next DeepSeek prompt](deepseek-readout-diagnostic-step-2026-09-20.md) recovers the saved candidates without prefix retraining, then runs one matched empirical-versus-smoothed conditional-target fit of the existing output head on frozen integer context features. A floating head is a conditional offline diagnostic only. The new experiment is **NOT_RUN**; this principal review ran no Rust build, model forward or training.

**Retained result.** On the same 36-document / 288-window / 17,342-target open development panel: parent **7.558429**, learned older-prefix **7.575604**, fixed-action **7.723496**, local-tail **7.576439** bits/target. The main screen failed. The final primary action map differs on **1,083 of 4,096 token IDs**. Correct causal indexing, gradient scales, once-only averaging, hard forward and the 64/192 warm-up/joint schedule are source-supported. Each arm used 512 windows / 32,046 distinct targets for four passes, **128,184 target presentations**. This establishes discrete learning without a selected net language gain.

**Independent verification scope.** All sealed member hashes/sets and unchanged parent identities verify. The saved arm document means and paired arm-versus-arm intervals reproduce: primary gain over fixed **+0.147892 [0.135512,0.161202]**; over local-tail **+0.000835 [−0.016240,0.020440]**. The conditional donor map and its 1,177 eligible records across 36 docs are verified. Reported parent gain **−0.017174 [−0.026095,−0.008483]** and eligible permutation penalty **−0.001532 [−0.015194,+0.012677]** remain source-consistent reported results; their parent/control loss vectors were not persisted, so those CIs require recovery forwards for independent reproduction. A near-zero intervention on about 7% of history records is no detected benefit, not equivalence or absence of older information.

**Qualification corrections.** The three `.cpl2.json` files are action/hash descriptors, not complete hard artifacts; scoring used live trainer cores. CPXS preserves final masters/moments but does not establish complete runner mid-pass continuation. The group table remains lazy with floating construction rather than an artifact-bound exact table. Local-tail CE uses the correct state, but its generation incorrectly uses older-prefix state. Count-reference generation scans only three candidates, invalidating its reported greedy collapse. Recover/export/reload the saved parameters, bind an exactly verified table/order, correct both generation paths and persist sufficient control losses in a new report. No repeat of the rejected fits is needed.

**Other reporting corrections.** The probe did two batch updates over 16 windows. The reported action norm is pre-Jacobian palette credit. Final cache is 21,251 entries / 348,176,384 bytes; 9,891 / 162 MB described its initial panel stage. Count-reference gain-field signs are opposite the arm convention. Both full pilot attempts produced byte-identical final checkpoints, so the second fit added no new learned outcome. The raw [receipt](../evidence/native_geometric_ordered_prefix_pilot_2026-09-20.txt) remains unchanged and must be read with this review. The corrected 38,152 document offsets and pair fixed-point rule remain valid.

**Next causal question.** Freeze both parent embeddings, integer feature normalization, bias and F; train only the existing V4096×128 ternary output head. Compare occurrence-weighted empirical conditional histograms (the token-CE population objective) against normalized smoothed/backoff targets from the same 4,096 conditional-count windows. Verify parent CPCK output-master parity, reset moments and give both arms the same 512-update schedule. This tests a training objective, not an established readout defect. Neither count matching nor perfect local prose is a prerequisite for future geometric work.

**Roadmap implication.** Count references reach 7.079256/5.938052/5.061160 bits from 512/4,096/full-fit conditional counts, with a shared full-fit marginal. They expose unrecovered local performance without separating optimization, quantization, feature loss or output rank. The tested prefix correction is also separable: `Z(local,state)=Z_parent(local)+r(state)`, so its relative-token effect cannot depend on the local query. A later conditional geometric read, selective update and exact occurrence memory remain distinct candidates. General language, 2I advantage and energy savings remain unestablished.

**Resources.** Recorded JSON/prose agree at **159438565 / 159800000 ms**, remaining **361435 ms (6.02 min)**. The last 5,400,000 ms charge equals the entire projection, but the surrounding PR merges span 33m48s; it is retained as an unverified conservative charge, not independently measured 90-minute wall time. This review changes no balance. The next prompt proposes 5,400,000 ms of complete local work and a corresponding pre-use extension to 165200000 ms under standing authorization. Charge actual nonoverlapping intervals, not the reservation; preserve 128 MiB storage margin and all artifacts. No deletion or paid compute. #973/#820/#963/#964 remain open.
...
Complete model/build charge237196/1800000ms; shared124730786/132950000ms; parent4980213/7710000ms. No extension or paid compute;6GiBRAM,512MiBnewstorage,128MiBmargin. Exact resource/delivery receipts and restart note under .uor-handoff/2026-09-12-codex-v7/shared-core-first-step/dependent-attention-1/. Active worktree /Users/casey.allard/uor-r4-worktrees/shared-geometric-core; branch codex/dependent-geometric-attention; base4cb74afe64d4d6fcc5cf6cb6d961d23d99bdf15e. #973 assigned/open; #964/#820 open. All next-action sections below are historical.
