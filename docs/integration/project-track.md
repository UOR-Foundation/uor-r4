...
**Retain 5ed24f4e at bounded formatted Add/Copy scope → #1140 changed operation order and dependency → #973 broad language learning.**
...
The [action-conditioned emission result](../native_geometric_action_emission_1140.md) combines shared geometric lexical selection with exact committed Add/Copy state. Next diagnose Copy→Add with a changed dependency on the copied record, then refine the shared selection/transition mechanism where actual generation fails. Preserve current formatting, equal-value record identities, stopping and following independent turns. These finite authored operation and formatting forms do not establish arbitrary sequence or language construction. Continue broader lexical and contextual learning under #973; do not substitute manually supplied intermediate text or serving templates for learned behavior.
