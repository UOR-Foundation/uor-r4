...
**Retain 91ede422 at bounded word/span shared-emission scope → #973 instruction/fact role binding and broader language learning.**

Recovery PR #1198 restored artifact-backed serving and withdrew unsupported qualification. The [current-query refinement](../native_geometric_current_query_1140.md) repaired new-literal versus prior-result selection. The [operator transition](../native_geometric_operation_transition_1140.md) now learns whether to stop or execute another shared operation using an actual committed result, with preserved responses, candidate-availability controls and post-selection numeric transfers. Its actual API/checkpoint path generates the two-step result. The [shared lexical emitter](../native_geometric_lexical_emission_1140.md) adds learned byte/EOS and exact operand-read selection for two authored sentence/Rust-equality forms, with changed operands, names/order, executed Rust checks and preserved controls. The [contextual instruction refinement](../native_geometric_instruction_binding_1140.md) then preserves upstream binding across tested ordinary request phrases, with exact ordered operands, prior outputs and controls retained. All parent and rejected candidates remain preserved. Those earlier bounded results alone did not complete general prose, reasoning/coding or #1140.

The [Copy→Add result](../native_geometric_copy_add_1140.md), together with retained Add→Add and Add→Copy, meets #1140's declared bounded acceptance: changed operands/identifiers/dependencies, intermediate intervention, complete emission, executed Rust semantic checks and preserved parent behavior. This does not qualify arbitrary sequence construction or prose. The [word/span shared-emission result](../native_geometric_word_emission_973.md) now connects completed exact reads to learned value-first endings and Base/defer choices, preserving earlier plain/Rust continuations. Next under #973 diagnose and repair instruction-versus-fact read/write role binding in the observed absent-owner and instruction-induced relation failures. Sustained prose and owner-first composition remain unfinished. Keep exact occurrence/version identity, numeric formatting, stopping and abstention. Do not substitute manually supplied intermediate text or serving templates for learned behavior.
