...
| 3 — bound admission and transport | Apply actual serving admission masks during training; compare partial-lane/probed access with full training attention and a matched exact/cache control. Test observable input-dependent group transport against an ordinary recurrent/Householder control, including transport quantization error and shared-state drift. | **Next; implementation NOT_RUN.** Enter from both retained learned-code artifacts. First place actual bounded admission in the shared training/session graph; count all admission work and preserve exact occurrence identity/NoRead. Then integrate observable quantized group transport and its accumulated state/output error. Keep full-context and matched ordinary/exact-cache controls. |
