...
| 2 — discretize the working student | Add declared straight-through/annealed quantization, commitment/usage terms where needed, matched codebooks and explicit gains. Train and score both relaxed and exported hard paths throughout; preserve language behavior and measure the gap. | Open retention gate: [original recipe](quantized-recurrent-result-2026-09-25.md) and [projected successor](projected-recurrent-result-2026-09-25.md), PRs #1390/#1391, both fail. The successor adds 512 fully quantized updates per arm; packed minus continuous is +0.059801/+0.083525 against +0.05. Its other four gates pass at limited scope. The [completed precision comparison](precision-factorial-result-2026-09-25.md) finds parameter-family cost dominant; removing interface quantization still misses the original gate. Next: one paired learning of neighboring integer-code choices on the same parents and fixed grids, with full recurrent language loss and one final hard decision on the original five gates; NOT_RUN. No automatic sweep follows failure. All negatives remain retained; F32 emulation does not qualify integer serving. |
| 3 — bound admission and transport | Apply actual serving admission masks during training; compare partial-lane/probed access with full training attention and a matched exact/cache control. Test observable input-dependent group transport against an ordinary recurrent/Householder control, including transport quantization error and shared-state drift. | Not run |
| 4 — integer export and useful execution | Export the same learned student to Rust bounded operators, exact memory and normalized output. Run complete source-edited/NoRead conversations and code tasks, arithmetic/serialization checks, per-token parameter access and M1 wall/RAM/physical-energy comparisons at matched quality. | Not run |
