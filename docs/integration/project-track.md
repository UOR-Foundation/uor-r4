...
| 4 — integer export and useful execution | Export the same learned student to Rust bounded operators, exact memory and normalized output. Run complete source-edited/NoRead conversations and code tasks, arithmetic/serialization checks, per-token parameter access and M1 wall/RAM/physical-energy comparisons at matched quality. | **Partial implementation:** integer numerical attention/state/output now runs; actual loaded source/generation and compiled numerical-value instructions are inspected. It is an allocating dense prototype in the training crate, with a floating host sampler and about28–29x slower measured steps than F32. Next integrate a standalone integer serving session and measure whole-generation cost while preserving full256 and exact retained computation. Useful language/code, terminal D5 sparsity and energy remain open. |
