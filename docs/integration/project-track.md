...
**Recovery: retain `79710468` through repaired serving interfaces → resolve shared-router provenance across repeated independent queries → #1140 learned transitions/compositional emission → #973 broad language learning.**

The last retained writer is `79710468` from PR #1176. Its bounded phrase/boundary results remain valid at their recorded artifact scope. September 8's programme-complete claim is withdrawn: self-passing release checks, toy-model substitution and disconnected deterministic helpers do not meet the ordered issues' acceptance. See [the recovery record](recovery-2026-09-08.md).

Actual retained-artifact API/browser behavior now has bounded executed checks. The shared role candidate `b2da9f9a` is not promoted despite 183 construction, 12 open, 12 post-selection fresh cases and prior-panel preservation: its third independent 13+4 query after generated 17 and 18 returns 35 where the retained parent abstains. Current-query literal occurrences must be distinguished from multiple historical derived results before extending within-response composition. Preserve the experimental positive and negative receipts in [the recovery record](recovery-2026-09-08.md#shared-role-experiment-and-retention-decision). Complete-output and provenance controls must precede promotion. Existing arithmetic source-refresh and Add/Sub/Mul machinery can be retained as candidate infrastructure, but changed operands/dependencies, intermediate interventions, generated lexical ordering/EOS and executed complete Rust responses require actual qualification. Do not restart completed phrase-binding work or claim that adding arithmetic helper functions is generalized reasoning.
