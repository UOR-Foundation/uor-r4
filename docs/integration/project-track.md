...
| 3 — preserve context and implement transport | Preserve full 256-token access while implementing quantized input-dependent group transport against the ordinary recurrent/Householder control, including state/output drift. Any later admission change must share the actual training/serving mask and preserve the declared horizon. | **Orthant64 retention rejected; pruning deferred under [D9](DECISIONS.md#d9--prevent-experiment-loops-and-preserve-the-context-contract).** The [paired result](bounded-admission-result-2026-09-25.md) remains negative; recent64 is diagnostic and its training follow-up is withdrawn. Proceed from accepted learned-code parents through quantized transport into the integer bridge in one integrated implementation. Full causal access within 256 is the baseline, not a scalable-sparsity claim. Reopen pruning only with a demonstrated need, new candidate-recall learning evidence and a decision-changing comparison. |
...
programme or diagnostic-policy promotion is the next milestone. Full-access
integer implementation is independent of further sparse-admission fitting.
