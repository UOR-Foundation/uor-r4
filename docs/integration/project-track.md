...
| 3 — bound admission and transport | Apply actual serving admission masks during training; compare partial-lane/probed access with full training attention and a matched exact/cache control. Test observable input-dependent group transport against an ordinary recurrent/Householder control, including transport quantization error and shared-state drift. | **Admission implemented; orthant64 retention rejected.** The [completed paired continuation](bounded-admission-result-2026-09-25.md) passes NLL/read/noncollapse/mechanical gates but loses11/8 prior first-noun responses. Indexed utility versus recent32 passes; recent64 retains more source answers at lower same-weights NLL. Next: one matched recent64 continuation from accepted learned-code parents using the existing graph and fixed recipe; no policy-override promotion or index sweep. Retain exact occurrence identity/NoRead and measured index work. Observable quantized group transport/state drift follows bounded retention; older geometric access requires explicit candidate-recall credit. |
