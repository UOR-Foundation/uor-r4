...
| 2 — discretize the working student | Add declared straight-through/annealed quantization, commitment/usage terms where needed, matched codebooks and explicit gains. Train and score both relaxed and exported hard paths throughout; preserve language behavior and measure the gap. | Next; NOT_RUN. Continue the selected quaternion and ordinary checkpoints. Freeze language/read/output retention and cost gates, quantizer/surrogate, scales, saturation and hardening schedule before fitting; inspect actual loaded hard generation early. |
