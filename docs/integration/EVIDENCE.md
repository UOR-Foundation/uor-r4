...
| 2026-09-19 | D2 action | **VSA codebook wiring repaired** — codes derived from the learned 120-root assignment instead of a token-id hash (`learner/vsa_codes.rs`, LSH of the canonical icosian roots) | discrete (full-vocab) | open development slice | **Negative for the repair.** `vsa` ablation stays NEGLIGIBLE and mildly negative: Δ −0.0009 (fixed) → **−0.0013** (root); baseline BPB 1.7989 → 1.7993; flip rate 0.7 % → 1.2 %. Three causes separated: wiring (**fixed**), 120-code aliasing (**binding**, needs finer codes from the continuous embedding, which the artifact does not store), and the single-scalar-distance design ceiling (**structural** — one number cannot express identity *and* graded similarity *and* role). Not a retirement: the mechanism needs a finer code **and** a richer per-head read. Routing effect **NOT_MEASURED** (this instrument scores the full vocabulary, not the 64-candidate shortlist) | [receipt](../evidence/native_geometric_vsa_codebook_repair_2026-09-19.txt) |
...
- **The VSA routing role.** The codebook also feeds `route_shortlist_q30_with_memory`, and
  `ablate-prose` scores the full vocabulary rather than the 64-candidate shortlist, so the
  effect of a codebook change on **routing** is unmeasured. Required before any conclusion
  about the routing role.
