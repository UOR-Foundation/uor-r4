...
| `B_c(g)`, `theta_x^r` | **Objective** | `ConnectionGaugeCovarianceV4` represents each predictive role by the same three local coefficients `theta` in a declared tangent frame `B_c(g)` and transports them by `B_c(d) transpose(B_c(s))`. | V3 rejected the mixed-gauge ambient-R4 projection combination, not the exact H4 group law. V4 remains `NOT_RUN`; it must prove gauge covariance and finite-difference gradient agreement before opening fresh labels. |
