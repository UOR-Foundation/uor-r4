...
| `q_t^ℓ, k_i^ℓ, v_i^ℓ` | **Definition** | Learned geometric query, causal prefix/memory key, and value contribution used by one R⁴ mixer layer. These are not R4G1 route codes or CIDs. | Experimental decoder reference/operator state under #950/#951. |
...
| `d_R4^ℓ(q,k)` | **Definition** | Declared Riemannian/angular/geodesic compatibility used by `M_R4^ℓ`. It may approximate teacher attention; its geometric coordinates/support must remain intervention-testable. | Frozen by #950 before fitting; qualified by #951. |
...
| `P_mem^ℓ` | **Definition** | Deterministic, tokenizer-CID-bound projection from an identity-scoped memory span into layer-`ℓ` geometric key/value state. | Defined by #950, trained/qualified by #951, wired to product by #953. |
