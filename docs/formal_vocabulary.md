...
| `d_R4`, `mu_R4` | **Objective** | After parity, the separately trained intrinsic arm may score `-d_R4(q_i,P k_j)^2/tau` and aggregate transported values with an artifact-bound geometric weighted centroid `mu_R4`. | This is the now-authorized UOR intrinsic successor, not a claim that pinned upstream HELM-D computes `arcosh` geodesic distance squared. It remains `NOT_IMPLEMENTED`, with evidence `NOT_RUN`; the parity gate has passed. |
