...
| `R4SoftmaxTraceStateStudentV1` | **Objective** | Proposed bounded source-free state transition and next-token readout over the captured query-gauge Q, transported K/V support, weighted aggregate, and decoded model-frame trace, compared with the established suffix student, an equal-budget non-geometric recurrent state, and a transport/state-permuted control. | `NOT_IMPLEMENTED`, `NOT_RUN`. Promotion requires lower held-out loss and higher top-1, destructive-control separation, exact causal replay, zero source calls, and decoded continuation without the established short cycle. Intrinsic/readout replacement, resonance, corpus scaling, correctness/reasoning claims, WASM promotion, and exact lowering remain parked. |
...
| `M_R4^ℓ` | **Definition** | Declared bounded causal geometric mixing operator at layer `ℓ`; its metric, chart, support rule, transport, and aggregation must be frozen by the owning issue. | ADR-0005 accepts ordinary R4/Spin softmax attention as the reference baseline. Source-backed generation/native HTTP and the first source-free suffix-distillation compiler now pass at their declared scopes, but the suffix student decoded a short repetition and uses no geometric trace state. `R4SoftmaxTraceStateStudentV1` is active; intrinsic attention replacement, resonance, scale, reasoning, WASM promotion, and recurrent/exact lowering remain parked. A coherent geometry-native deployed realization remains `NOT_YET_IMPLEMENTED`. |
