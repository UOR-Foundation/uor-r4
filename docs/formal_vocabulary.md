...
| `R4SoftmaxTeacherTraceV1`, `R4SoftmaxTraceCompilerV1` | **Objective** | Proposed construction-only trace schema and compiler. The trace binds causal layerwise token, Q/K/V, attention, value, and logit states from the exact qualified reference; the compiler maps those observations into a source-free student/attention-state artifact without granting the student future or target inputs at inference. | Both names are proposed, `NOT_IMPLEMENTED`, and `NOT_RUN`. Promotion requires decoded-token and next-token-loss comparison against the exact reference plus causal-input and matched-control audits. This rung precedes intrinsic/readout, resonance, softmax replacement, recurrence, and exact lowering. |
...
| `M_R4^ℓ` | **Definition** | Declared bounded causal geometric mixing operator at layer `ℓ`; its metric, chart, support rule, transport, and aggregation must be frozen by the owning issue. | G0/G1 and ADR-0003 are historical comparators. ADR-0005 accepts ordinary R4/Spin softmax attention as the reference baseline; provider-free autonomous `R4SoftmaxReferenceGeneratorV1` (`HELM-D-R4`) native CLI generation and its dedicated opt-in loopback-only HTTP endpoint now pass at their declared scopes. Dashboard wiring/static native-readiness and WASM-isolation checks pass; browser interaction/E2E is `NOT_RUN`. The proposed source-free trace/compiler rung is active. Intrinsic attention, resonance replacement, and recurrent lowering are parked. A source-free/table-native deployed realization remains `NOT_YET_IMPLEMENTED`. |
