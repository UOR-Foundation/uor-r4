...
| `d_R4`, `mu_R4` | **Objective** | After parity, a separately trained intrinsic arm may score transported manifold-valued Q/K pairs and aggregate transported values with an artifact-bound equivariant geometric centroid `mu_R4`. | V1 implemented `-acosh(...)^2`, a normalized Lorentz centroid, and a coefficient-only fit, but attempt 02 stopped `UNAVAILABLE` before D3 when covariance missed its frozen bound; intrinsic attention remains unestablished. The active construction successor copies the pinned HELM-D Lorentz-inner-product score, learned scale/bias, and learned manifold Q/K/V roles rather than claiming that upstream computes `arcosh` distance squared. |
