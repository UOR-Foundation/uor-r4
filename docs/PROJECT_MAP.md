...
The [takeover review](integration/takeover-review-2026-09-19.md) reconciles the three distinct model families: retained native memory/shared-language experiments, TinyStories prose `.rgm` training/serving, and the current geometric addressed-memory core. Their artifacts, scorers and qualification results are not interchangeable. The [DeepSeek older-prefix handoff](integration/deepseek-ordered-prefix-step-2026-09-20.md) owns the immediate experiment specification; [current state](integration/current-state.md) owns its outcome and the [canonical plan](integration/project-track.md) owns programme acceptance.

The [post-qualification review](integration/cold-context-review-2026-09-19.md) records PR #1292's cold-route finding. PR #1294 subsequently implements `learner/cold_prior.rs` and `bin/cold-prior-pilot.rs`; its negative pilot is corrected in the [prior-learning review](integration/prior-learning-review-2026-09-19.md). Loss units/clipping, training/export bias mismatch and sampling prevent a convergence diagnosis. The subsequent prior-only curve is described below; unchanged joint training was not resumed. Coverage and timing tools are `crates/uor-r4-core/src/bin/geometric-realtext-coverage.rs` and `geometric-realtext-train-probe.rs`; their evidence scope remains narrower than a qualified language model. PR #1298 repairs `learner/prior_learning.rs` and adds `bin/prior-learning-gate.rs`, whose corrected 16-context learned fit passes. PR #1300 then adds `bin/prior-learning-realtext.rs` and trains the prior. PR #1302 repairs and executes `bin/prior-frozen-evaluate.rs`; the [latest review](integration/ordered-prefix-review-2026-09-20.md) verifies its positive gates, scopes the count/loop conclusions and identifies derived-index metadata repairs. The next experiment learns one exact group-state channel over the older prefix while freezing the prior, with a small ternary reader and fitted fixed-action/local-tail controls. Its explicit hard-forward surrogate avoids pretending table indices have ordinary derivatives. The pilot is not yet run. Use the [roadmap dependency sequence](integration/project-track.md#research-dependencies-and-exit-conditions) to connect this baseline to learned geometric state and exact-memory integration.
