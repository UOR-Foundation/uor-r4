...
The [learned typed attention result](native_geometric_relational_attention_973.md) and [relational_attention source](../crates/uor-r4-core/src/native_geometric/relational_attention/mod.rs) now establish the first compatibility/byte-EOS learning boundary on supplied records. The [dependent query result](native_geometric_dependent_attention_973.md) and [dependent_attention source](../crates/uor-r4-core/src/native_geometric/dependent_attention/mod.rs) now learn a query update between two actual reads. The [adaptive loop result](native_geometric_adaptive_attention_973.md) and [adaptive_attention source](../crates/uor-r4-core/src/native_geometric/adaptive_attention/mod.rs) now learn content-controlled read/emit scheduling at the typed byte scope. Variable-length text and joint language learning remain unfinished; none of these results promotes a prose model.
