...
The [post-qualification review](integration/cold-context-review-2026-09-19.md) records PR #1292's cold-route finding. PR #1294 subsequently implements `learner/cold_prior.rs` and `bin/cold-prior-pilot.rs`; its negative pilot is corrected in the [prior-learning review](integration/prior-learning-review-2026-09-19.md). Loss units/clipping, training/export bias mismatch and sampling prevent a convergence diagnosis. The subsequent prior-only curve is described below; unchanged joint training was not resumed. Coverage and timing tools are `crates/uor-r4-core/src/bin/geometric-realtext-coverage.rs` and `geometric-realtext-train-probe.rs`; their evidence scope remains narrower than a qualified language model. PR #1298 repairs `learner/prior_learning.rs` and adds `bin/prior-learning-gate.rs`, whose corrected 16-context learned fit passes. PR #1300 then adds `bin/prior-learning-realtext.rs` and trains the prior. PR #1302 repairs and executes `bin/prior-frozen-evaluate.rs`; the [latest review](integration/ordered-prefix-review-2026-09-20.md) verifies its positive gates, scopes the count/loop conclusions and identifies derived-index metadata repairs. PR #1304 executed that prefix pilot with a failed numerical screen. The [review after PR #1304](integration/prefix-pilot-review-2026-09-20.md) scopes its incomplete artifact/reload and generation claims. Next is checkpoint-based recovery plus a fixed-feature empirical-versus-smoothed output-head diagnostic, not another prefix fit or a requirement for perfect count matching. Use the [roadmap dependency sequence](integration/project-track.md#research-dependencies-and-exit-conditions) to connect this baseline to learned geometric state and exact-memory integration.
