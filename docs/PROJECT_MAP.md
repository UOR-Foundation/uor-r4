...
The [takeover review](integration/takeover-review-2026-09-19.md) reconciles the three distinct model families: retained native memory/shared-language experiments, TinyStories prose `.rgm` training/serving, and the current geometric addressed-memory core. Their artifacts, scorers and qualification results are not interchangeable. The [DeepSeek frozen-prior handoff](integration/deepseek-frozen-prior-step-2026-09-20.md) owns the immediate experiment specification; [current state](integration/current-state.md) owns its outcome and the [canonical plan](integration/project-track.md) owns programme acceptance.

The [post-qualification review](integration/cold-context-review-2026-09-19.md) records PR #1292's cold-route finding. PR #1294 subsequently implements `learner/cold_prior.rs` and `bin/cold-prior-pilot.rs`; its negative pilot is corrected in the [prior-learning review](integration/prior-learning-review-2026-09-19.md). Loss units/clipping, training/export bias mismatch and sampling prevent a convergence diagnosis. The subsequent prior-only curve is described below; unchanged joint training was not resumed. Coverage and timing tools are `crates/uor-r4-core/src/bin/geometric-realtext-coverage.rs` and `geometric-realtext-train-probe.rs`; their evidence scope remains narrower than a qualified language model. PR #1298 repairs `learner/prior_learning.rs` and adds `bin/prior-learning-gate.rs`, whose corrected 16-context learned fit passes. PR #1300 then adds `bin/prior-learning-realtext.rs` and trains the prior. The [latest review](integration/frozen-prior-review-2026-09-20.md) preserves its positive loss observation while withdrawing the full gate: the permutation overwrites repeated occurrences and the bootstrap treats windows as documents. The next action is corrected evaluation of the final saved CPL2, reconstructed exposure, count references and actual loop diagnostics, without training. Use the [roadmap dependency sequence](integration/project-track.md#research-dependencies-and-exit-conditions) to connect this baseline to learned geometric state and exact-memory integration.
