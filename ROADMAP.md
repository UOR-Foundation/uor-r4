...
`HuggingFaceLlamaOracle` decoder path, with CLI/evidence first. D3
...
> global context choose output. The active autonomous reference may execute
...
> The active gate is provider-free autonomous `R4SoftmaxReferenceGeneratorV1`
> (`HELM-D-R4`) generation using the credited attention seam and UOR's pinned
> SmolLM2 `HuggingFaceLlamaOracle` decoder path, CLI/evidence first. This intermediate remains transformer-
...
softmax causal attention in coherent R4/Spin frames; **Active:** provider-free
autonomous `R4SoftmaxReferenceGeneratorV1` (`HELM-D-R4`) generation,
CLI/evidence first; **Parked:**
...
two-document preflight and rejected tangent readout. The active next action is provider-free
...
`HuggingFaceLlamaOracle` decoder path. Intrinsic/resonance/replacement work is
...
9. **GI-2 A1Q-H / #973 — higher-scope attention:** Gate 0, paragraph,
   ...
   R4/Spin frames as the accepted baseline and build provider-free autonomous
   `R4SoftmaxReferenceGeneratorV1` (`HELM-D-R4`) generation with the credited
   HELM attention seam and UOR's pinned SmolLM2 `HuggingFaceLlamaOracle`
   decoder path.
...
`R4SoftmaxReferenceGeneratorV1` (`HELM-D-R4`) generation → blocked #954 → #955 →
...
- [ ] **#973 accepted attention and autonomous reference generation** — retain the
  ...
  attention in coherent R4/Spin frames as the baseline and implement the
  smallest provider-free autonomous `R4SoftmaxReferenceGeneratorV1`
  (`HELM-D-R4`) path with the credited HELM attention seam and UOR's pinned
  SmolLM2 `HuggingFaceLlamaOracle` decoder path, CLI/evidence first. Intrinsic
...
- [~] Text-based AI — the repository has the retained #958 route/storage
  ...
  autonomous `R4SoftmaxReferenceGeneratorV1` (`HELM-D-R4`) generation is active;
