"""Artifact-only generation through fixed-size recurrent R4/H4 memory."""
...
SCHEMA = "uor-r4.fixed-recurrent-r4-language-generation/1"
STATUS = "GENERATED"


def _validate_state(model: R4FixedRecurrentCausalKVBindingV1, state: Any) -> None:
    state_values = (
    ...
    if (
        ...
        raise RuntimeError("generation left the fixed recurrent R4 state contract")


def generate_fixed_recurrent_r4_language_path(
    root: Path,
    *,
    geometry_path: Path,
    frame_path: Path,
    ...
    """Generate once with unchanged weights and bounded recurrent state."""
    ...
    if (
        ...
        raise ValueError("recurrent-R4 geometry differs from the validated sidecar")
    if (
        _record(resolved_frames, frame_payload)["cid"]
        != EXPECTED_H4_FRAME_FILE_CID
    ):
        raise ValueError("recurrent-R4 frame file differs from the validated sidecar")
    ...
    if (
        ...
        raise ValueError("recurrent-R4 geometry artifact identity differs")
    ...
    if (
        ...
        raise ValueError("recurrent-R4 frame artifact identity differs")

    model = R4FixedRecurrentCausalKVBindingV1.from_learned_artifact(
        artifact_payload,
    ...
    if (
        ...
        raise RuntimeError("fixed recurrent wrapper changed the bound artifact")
    ...
    if model.recurrent_state_value_count() != initial_state_values:
        raise RuntimeError("fixed recurrent state storage changed with sequence length")
    ...
    return {
        ...
        "compression": {
            ...
            "score_law": "unchanged-qk-softmax-one-summary-one-source",
        ...
        "execution": {
            "implementation": "fixed-recurrent-r4-kv-cache",
            ...
            "first_summary_read_position": (
                LIVE_WINDOW + 1 if audit.summary_slots_read else None
            ),
