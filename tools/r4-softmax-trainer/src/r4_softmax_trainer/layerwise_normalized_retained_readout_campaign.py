...
order, optimizer schedule, recurrence, and state budget while changing only the
placement of normalization in the already-retained state readout.  An
independently selected V3 prompt-swap population remains mode-000 until the
frozen predecessor and newly fitted successor artifact CIDs exist.
...
from .layerwise_normalized_retained_readout import (
    POLICY as MODEL_POLICY,
)
from .layerwise_normalized_retained_readout import (
    R4LayerwiseNormalizedRetainedReadoutLanguagePathV1,
)
from .prompt_conditioning_v3 import (
...
ISSUE = 973
POLICY = "R4LayerwiseNormalizedRetainedReadoutV1"
...
FRESH_HELDOUT_FIRST_CAPACITY_STORY = 762_819
FRESH_HELDOUT_FIRST_SOURCE_STORY = 847_141
FRESH_HELDOUT_LAST_CAPACITY_STORY = 764_049
FRESH_HELDOUT_LAST_SOURCE_STORY = 848_492
FRESH_HELDOUT_STORY_CIDS = 1_231
...
PREPARATION_RELATIVE_PATH = "layerwise-normalized-retained-readout-preparation.json"
HELDOUT_RELATIVE_PATH = "data/fresh-heldout.u16"
PROBE_RELATIVE_PATH = "preflight/layerwise-normalized-retained-readout-probe.json"
STARTED_RELATIVE_PATH = "run/layerwise-normalized-retained-readout-started.json"
RESULT_RELATIVE_PATH = "run/layerwise-normalized-retained-readout-result.json"
...
PREPARATION_SCHEMA = "uor-r4.layerwise-normalized-retained-readout-preparation/1"
PROBE_SCHEMA = "uor-r4.layerwise-normalized-retained-readout-probe/1"
STARTED_SCHEMA = "uor-r4.layerwise-normalized-retained-readout-started/1"
CHECKPOINT_SCHEMA = "uor-r4.layerwise-normalized-retained-readout-checkpoint/1"
RESULT_SCHEMA = "uor-r4.layerwise-normalized-retained-readout-result/1"

TERMINAL_PASS = "LAYERWISE_NORMALIZED_RETAINED_READOUT_PROMPT_CAPACITY_PASS"
TERMINAL_GENERAL_LANGUAGE_REGRESSION = (
    "LAYERWISE_NORMALIZED_RETAINED_READOUT_GENERAL_LANGUAGE_REGRESSION"
)
TERMINAL_ABSOLUTE_NO_CAPACITY_GAIN = (
    "LAYERWISE_NORMALIZED_RETAINED_READOUT_ABSOLUTE_NO_CAPACITY_GAIN"
)
TERMINAL_PARTIAL = "LAYERWISE_NORMALIZED_RETAINED_READOUT_PROMPT_CAPACITY_PARTIAL"
TERMINAL_FAIL = "LAYERWISE_NORMALIZED_RETAINED_READOUT_PROMPT_CAPACITY_FAIL"
TERMINAL_INVALID = "INVALID_LAYERWISE_NORMALIZED_RETAINED_READOUT_PROMPT_CAPACITY"
TERMINAL_UNAVAILABLE = (
    "UNAVAILABLE_LAYERWISE_NORMALIZED_RETAINED_READOUT_PROMPT_CAPACITY"
)
VERIFICATION_RELATIVE_PATH = (
    "run/layerwise-normalized-retained-readout-independent-verification.json"
)
VERIFICATION_SCHEMA = (
    "uor-r4.layerwise-normalized-retained-readout-independent-verification/1"
)
...
def _verify_fresh_heldout_index(
    path: Path,
    ...
    if len(story_cids) != len(records) or any(
        not value.startswith("blake3:") for value in story_cids
    ):
        raise ValueError("fresh-heldout index has invalid or duplicate story CIDs")
    story_cids_cid = cid_bytes(canonical_json_bytes(sorted(story_cids)))
    if (
        len(story_cids) != FRESH_HELDOUT_STORY_CIDS
        or story_cids_cid != FRESH_HELDOUT_STORY_CIDS_CID
    ):
...
def prepare_layerwise_normalized_retained_readout(
    *,
    root: Path,
    ...
    if root.exists() or root.is_symlink():
        raise FileExistsError(
            "layerwise-normalized-retained-readout root is create-once"
        )
    ...
    train_index, fresh_heldout_story_cids = _verify_fresh_heldout_index(
    ...
    if (
        not (
            PRIOR_REVEALED_PROMPT_LAST_SOURCE_STORY_ORDINAL
            < population.last_source_story_ordinal
            < FRESH_HELDOUT_FIRST_SOURCE_STORY
        )
        or prompt_story_cids & fresh_heldout_story_cids
    ):
        raise ValueError("V3 prompt population violates the frozen disjoint boundaries")
    ...
    try:
        ...
        body = {
            ...
            "training": {
                ...
                "changed_variable": "LAYERWISE_NORMALIZATION_PLACEMENT_ONLY",
                "formula": "E@[N(h)+(g/sqrt(2))*(N(a1)+N(a2))]",
                "candidate_gain": 1,
                "control_gain": 0,
            ...
            "prompt_population": {
                ...
                "prior_populations": {
                    "v1": {
                        "path": str(prior_v1_prompt_population_path.resolve()),
                        "cid": PRIOR_V1_PROMPT_POPULATION_CID,
                    },
                    "v2": {
                        "path": str(prior_v2_prompt_population_path.resolve()),
                        "cid": PRIOR_V2_PROMPT_POPULATION_CID,
                    },
                    "excluded_story_cids": PRIOR_PROMPT_STORY_CIDS,
                    "excluded_story_cids_cid": PRIOR_PROMPT_STORY_CIDS_CID,
                },
            ...
            "scope": {
                "trained_arms": ["layerwise_normalized_readout_candidate"],
                "predecessor_retrained": False,
                "new_learned_parameters": 0,
                "new_persistent_state_values": 0,
                "vocabulary_projections_per_token": 1,
                "forbidden_followups": [
                    "scalar gain tuning",
                    "g=2",
                    "third parameter-free normalization placement",
        ...
        if root.exists() or root.is_symlink():
            raise FileExistsError(
                "layerwise-normalized-readout root appeared during preparation"
            )
        staging.rename(root)
    except BaseException:
        _cleanup_staging(staging)
        raise
    return load_layerwise_normalized_retained_readout_preparation(root)
...
def load_layerwise_normalized_retained_readout_preparation(
    root: Path,
) -> CampaignPreparation:
    ...
    if root.is_symlink() or not root.is_dir():
        raise ValueError(
            "layerwise-normalized-retained-readout root must be a regular directory"
        )
    root = root.resolve()
    ...
    if (
        ...
        raise ValueError("layerwise-normalized-readout preparation envelope differs")
    ...
    if not all(
        ...
        raise ValueError(
            "layerwise-normalized-readout preparation records are malformed"
        )
    ...
    if (
        ...
        raise ValueError("layerwise-normalized-readout predecessor binding differs")
    ...
    if (
        ...
        or train_index_record.get("story_cids") != FRESH_HELDOUT_STORY_CIDS
        or train_index_record.get("story_cids_cid") != FRESH_HELDOUT_STORY_CIDS_CID
    ):
        raise ValueError("layerwise-normalized-readout fresh-heldout artifact differs")
    commitment = _load_commitment_in_any_state(root)
    population_cid = prompt_record.get("population_cid")
    prior_populations = prompt_record.get("prior_populations")
    if (
        prompt_record.get("commitment_cid") != commitment.get("commitment_cid")
        or population_cid != commitment.get("population", {}).get("cid")
        or prompt_record.get("policy") != "R4RetainedPromptSwapContrastV3"
        ...
        or int(prompt_record["last_source_story_ordinal"])
        >= FRESH_HELDOUT_FIRST_SOURCE_STORY
        or not isinstance(prior_populations, Mapping)
        or prior_populations.get("excluded_story_cids") != PRIOR_PROMPT_STORY_CIDS
        or prior_populations.get("excluded_story_cids_cid")
        != PRIOR_PROMPT_STORY_CIDS_CID
        or not isinstance(prior_populations.get("v1"), Mapping)
        or not isinstance(prior_populations.get("v2"), Mapping)
        or prior_populations["v1"].get("cid") != PRIOR_V1_PROMPT_POPULATION_CID
        or prior_populations["v2"].get("cid") != PRIOR_V2_PROMPT_POPULATION_CID
    ):
        raise ValueError("layerwise-normalized-readout prompt commitment differs")
    excluded_story_cids = load_required_prior_story_cids(
        Path(str(prior_populations["v1"]["path"])),
        Path(str(prior_populations["v2"]["path"])),
    )
    if len(excluded_story_cids) != PRIOR_PROMPT_STORY_CIDS:
        raise ValueError("layerwise-normalized-readout prior exclusions differ")
...
def _initialization_identity(geometry: Any) -> dict[str, Any]:
    ...
    if set(left) != set(right):
        raise RuntimeError(
            "layerwise-normalized-readout candidate changed learned parameter names"
        )
    digest = blake3()
    for name in sorted(left):
        if not torch.equal(left[name], right[name]):
            raise RuntimeError(
                f"layerwise-normalized-readout initialization differs at {name}"
            )
...
def _mechanical_admission(preparation: CampaignPreparation) -> dict[str, Any]:
    ...
    candidate = R4LayerwiseNormalizedRetainedReadoutLanguagePathV1(geometry)
    g0 = R4LayerwiseNormalizedRetainedReadoutLanguagePathV1.matched_v1_control(geometry)
    ...
    if (
        ...
        raise RuntimeError(
            "layerwise-normalized-readout candidate changed the qualified architecture ledger"
        )
    batch = _window_batch(
    ...
    if output.loss is None:
        raise RuntimeError(
            "layerwise-normalized-readout admission produced no training loss"
        )
    ...
    if inactive:
        raise RuntimeError(
            f"layerwise-normalized-readout admission has inactive gradients: {inactive}"
        )
    original = batch[:1, :-1]
    altered = original.clone()
    altered[:, 61:] = (altered[:, 61:] + 1) % VOCAB_SIZE
    qualified_artifact = preparation.predecessor_artifact_path.read_bytes()
    qualified_vanilla = R4RetainedLanguagePathV1(geometry)
    qualified_g0 = (
        R4LayerwiseNormalizedRetainedReadoutLanguagePathV1.matched_v1_control(geometry)
    )
    qualified_vanilla.load_learned_artifact(qualified_artifact)
    qualified_g0.load_learned_artifact(qualified_artifact)
    with torch.no_grad():
        ...
        qualified_vanilla_enabled = qualified_vanilla(original)
        qualified_g0_enabled = qualified_g0(original)
        qualified_vanilla_state_off = qualified_vanilla(original, attention_off=True)
        qualified_g0_state_off = qualified_g0(original, attention_off=True)
    ...
    if causal_delta != 0.0 or parity_logits > 2e-5 or parity_state > 2e-5:
        raise RuntimeError(
            "layerwise-normalized-readout causal or stationary/direct admission failed"
        )
    ...
    qualified_g0_logits_exact = torch.equal(
        qualified_g0_enabled.logits,
        qualified_vanilla_enabled.logits,
    )
    qualified_g0_state_delta = _maximum_state_delta(
        qualified_g0_enabled.final_state,
        qualified_vanilla_enabled.final_state,
    )
    qualified_g0_state_off_logits_exact = torch.equal(
        qualified_g0_state_off.logits,
        qualified_vanilla_state_off.logits,
    )
    qualified_g0_artifact_exact = (
        qualified_g0.export_learned_artifact()
        == qualified_vanilla.export_learned_artifact()
        == qualified_artifact
    )
    ...
    if (
        ...
        or not qualified_g0_logits_exact
        or qualified_g0_state_delta != 0.0
        or not qualified_g0_state_off_logits_exact
        or not qualified_g0_artifact_exact
        or not readout_work_signatures_equal
        or vanilla_artifact != g0_artifact
        or vanilla_artifact != candidate_artifact
    ):
        raise RuntimeError(
            "layerwise-normalized-readout changed more than the retained-state readout seam"
        )
    ...
    if replay_delta != 0.0 or forbidden_reads != 0:
        raise RuntimeError(
            "layerwise-normalized-readout artifact replay or forbidden-read admission failed"
        )
    return {
        "passed": True,
        ...
        "qualified_v1_g0_logits_byte_identical": qualified_g0_logits_exact,
        "qualified_v1_g0_maximum_state_delta": qualified_g0_state_delta,
        "qualified_v1_g0_state_off_logits_byte_identical": (
            qualified_g0_state_off_logits_exact
        ),
        "qualified_v1_g0_artifact_byte_identical": qualified_g0_artifact_exact,
...
def _execute_probe(preparation: CampaignPreparation) -> dict[str, Any]:
    ...
    if device.type != "cpu":
        raise RuntimeError(
            "layerwise-normalized-readout probe selected a non-CPU device"
        )
    mechanics = _mechanical_admission(preparation)
    geometry = _exact_geometry(preparation.predecessor)
    candidate = R4LayerwiseNormalizedRetainedReadoutLanguagePathV1(geometry).to(device)
    ...
    if evaluation.loss is None or int(evaluation.audit.forbidden_reads) != 0:
        raise RuntimeError(
            "layerwise-normalized-readout timing evaluation failed its causal audit"
        )
    ...
    replay = R4LayerwiseNormalizedRetainedReadoutLanguagePathV1(geometry).to(device)
    replay.load_learned_artifact(artifact)
    with torch.no_grad():
    ...
    if replay_delta != 0.0:
        raise RuntimeError("layerwise-normalized-readout probe replay differs")
...
def probe_layerwise_normalized_retained_readout(
    ...
    if path.exists():
        ...
        if result.get("implementation") != trainer_implementation_contract():
            raise ValueError(
                "layerwise-normalized-readout probe implementation differs"
            )
        return result
    if (resolved_root / REVEAL_RELATIVE_PATH).exists():
        raise ValueError(
            "layerwise-normalized-readout probe cannot begin after prompt reveal"
        )
    preparation = load_layerwise_normalized_retained_readout_preparation(root)
    ...
    body = {
        ...
        "execution": execution,
        "eligible": eligible,
        "verdict": "LAYERWISE_NORMALIZED_RETAINED_READOUT_EXECUTION_ADMITTED"
        if eligible
    ...
    if prompt_verdict == VERDICT_PASS:
        if language_passed:
            return {
                ...
                "action": "freeze and promote the candidate; permit only the predeclared disjoint autonomous subject/scene-retention smoke",
            }
        return {
            "verdict": TERMINAL_GENERAL_LANGUAGE_REGRESSION,
            "action": "end the parameter-free readout ladder and pivot to a freshly frozen learned associative binding/readout",
        }
...
def _save_checkpoint(
    ...
    *,
    ...
    sidecar = {
        "schema": "uor-r4.layerwise-normalized-retained-readout-checkpoint-cid/1",
        "step": step,
...
def _load_checkpoint_envelope(
    ...
    *,
    ...
    """Verify and deserialize a checkpoint without constructing an optimizer."""

    path = root / CHECKPOINT_RELATIVE_PATH
    sidecar = _read_canonical_json(root / CHECKPOINT_CID_RELATIVE_PATH)
    if (
        sidecar.get("schema")
        != "uor-r4.layerwise-normalized-retained-readout-checkpoint-cid/1"
        ...
        raise ValueError("layerwise-normalized-readout checkpoint CID sidecar differs")
    ...
    if (
        ...
        raise ValueError("layerwise-normalized-readout checkpoint envelope differs")
    ...
    if (
        ...
        raise ValueError("layerwise-normalized-readout checkpoint step differs")
    return checkpoint
...
def _load_checkpoint(
    ...
    *,
    ...
    checkpoint = _load_checkpoint_envelope(
        root,
        run_contract_cid=run_contract_cid,
    )
    ...
    if not isinstance(rng, Tensor):
        raise TypeError("layerwise-normalized-readout checkpoint omitted CPU RNG state")
    ...
    if any(
        ...
        raise ValueError(
            "layerwise-normalized-readout checkpoint learning rate differs"
        )
    return checkpoint


def _load_completed_checkpoint_for_finalization(
    root: Path,
    *,
    model: torch.nn.Module,
    run_contract_cid: str,
) -> dict[str, Any]:
    """Restore only a completed model for post-reveal evaluation recovery."""

    checkpoint = _load_checkpoint_envelope(
        root,
        run_contract_cid=run_contract_cid,
    )
    if checkpoint.get("step") != OPTIMIZER_STEPS:
        raise ValueError(
            "post-reveal finalization requires the completed frozen checkpoint"
        )
    if not isinstance(checkpoint.get("initial_heldout"), Mapping):
        raise TypeError("completed checkpoint omitted initial heldout evidence")
    last_loss = checkpoint.get("last_loss")
    if isinstance(last_loss, bool) or not isinstance(last_loss, (int, float)):
        raise TypeError("completed checkpoint omitted its final training loss")
    model.load_state_dict(checkpoint["model_state"], strict=True)
    return checkpoint
...
def _write_progress(
    ...
    *,
    ...
    progress = {
        "schema": "uor-r4.layerwise-normalized-retained-readout-progress/1",
        "issue": ISSUE,
        "policy": POLICY,
        "status": status,
        ...
        "resume": "run-layerwise-normalized-retained-readout --resume",
    }
...
def _resume_elapsed(root: Path, checkpoint: Mapping[str, Any]) -> float:
    ...
    if (
        progress.get("schema")
        != "uor-r4.layerwise-normalized-retained-readout-progress/1"
        ...
        raise ValueError(
            "layerwise-normalized-readout durable progress contradicts its checkpoint"
        )
...
def _candidate_artifact_replay(
    *,
    model: R4LayerwiseNormalizedRetainedReadoutLanguagePathV1,
    geometry: Any,
    artifact: bytes,
...
def _run_contract(
    ...
    return {
        ...
        "decision": {
            ...
            "if_prompt_pass_and_language_fail": (
                "end parameter-free readout variants; pivot to learned associative binding/readout"
            ),
            "if_prompt_absolute_without_capacity": (
                "end parameter-free readout variants; pivot to learned associative binding/readout"
            ),
            "if_prompt_partial": (
                "end parameter-free readout variants; pivot to learned associative binding/readout"
        ...
        "post_reveal_recovery": (
            "completed-checkpoint finalization only; zero optimizer construction or steps"
        ),
        "forbidden_followups": [
            "scalar gain tuning",
            "g=2",
...
def _factories(
    *,
    ...
    def predecessor_factory() -> R4LayerwiseNormalizedRetainedReadoutLanguagePathV1:
        model = R4LayerwiseNormalizedRetainedReadoutLanguagePathV1.matched_v1_control(
            geometry
        ).to(device)
        model.load_learned_artifact(predecessor_artifact)
        return model

    def candidate_factory() -> R4LayerwiseNormalizedRetainedReadoutLanguagePathV1:
        model = R4LayerwiseNormalizedRetainedReadoutLanguagePathV1(geometry).to(device)
...
def _load_terminal_result(root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError(
            "layerwise-normalized-readout terminal result envelope differs"
        )
    ...
    if isinstance(candidate, Mapping):
        ...
        if (
            ...
            raise ValueError(
                "layerwise-normalized-readout terminal candidate artifact differs"
            )
    ...
    if isinstance(reveal_binding, Mapping):
        ...
        if reveal_binding != expected:
            raise ValueError(
                "layerwise-normalized-readout terminal prompt reveal binding differs"
            )
        if (
            ...
            raise ValueError(
                "layerwise-normalized-readout terminal artifacts differ from prompt reveal"
            )
    return result


def _recover_revealed_prompt_access(root: Path) -> dict[str, Any]:
    """Repair only a valid marker-before-chmod interruption after artifact freeze."""

    reveal = _read_canonical_json(root / REVEAL_RELATIVE_PATH)
    _verify_self_cid(reveal, "reveal_cid")
    candidate_path = root / CANDIDATE_ARTIFACT_RELATIVE_PATH
    if (
        candidate_path.is_symlink()
        or not candidate_path.is_file()
        or reveal.get("baseline_artifact_cid") != PREDECESSOR_ARTIFACT_CID
        or reveal.get("candidate_artifact_cid") != cid_file(candidate_path)
    ):
        raise ValueError("post-reveal finalization artifact binding differs")
    reveal_prompt_conditioning_population(
        root,
        baseline_artifact_cid=PREDECESSOR_ARTIFACT_CID,
        candidate_artifact_cid=str(reveal["candidate_artifact_cid"]),
    )
    return reveal


def run_layerwise_normalized_retained_readout(
    ...
    reveal_path = resolved_root / REVEAL_RELATIVE_PATH
    finalization_only = reveal_path.exists() or reveal_path.is_symlink()
    if finalization_only and not resume:
        raise ValueError(
            "layerwise-normalized-readout fitting is permanently closed after prompt reveal; "
            "use --resume for finalization-only recovery"
        )
    if finalization_only:
        _recover_revealed_prompt_access(resolved_root)
    preparation = load_layerwise_normalized_retained_readout_preparation(root)
    implementation = _require_implementation(preparation.manifest)
    ...
    if (
        ...
        raise ValueError(
            "layerwise-normalized-readout run requires its eligible current probe"
        )
    ...
    if resume:
        ...
        if (
            ...
            raise ValueError("layerwise-normalized-readout resume run contract differs")
    else:
        if (
            started_path.exists()
            ...
            raise FileExistsError(
                "layerwise-normalized-readout run already started; resume is required"
            )
        started = _with_cid(
            {
    ...
    geometry = _exact_geometry(preparation.predecessor)
    candidate = R4LayerwiseNormalizedRetainedReadoutLanguagePathV1(geometry).to(device)
    optimizer = None if finalization_only else _optimizer(candidate)
    elapsed_before = 0.0
    step = 0
    last_loss: float | None = None
    if finalization_only:
        checkpoint = _load_completed_checkpoint_for_finalization(
            preparation.root,
            model=candidate,
            run_contract_cid=contract_cid,
        )
        ...
        last_loss = float(checkpoint["last_loss"])
        candidate_artifact_path = preparation.root / CANDIDATE_ARTIFACT_RELATIVE_PATH
        if cid_bytes(candidate.export_learned_artifact()) != cid_file(
            candidate_artifact_path
        ):
            raise ValueError(
                "post-reveal completed checkpoint and candidate artifact differ"
            )
    elif resume and (preparation.root / CHECKPOINT_RELATIVE_PATH).exists():
        ...
        if optimizer is None:
            raise RuntimeError("optimizer is unavailable before candidate fitting")
        if resume and (preparation.root / PROGRESS_RELATIVE_PATH).exists():
            raise ValueError(
                "layerwise-normalized-readout progress exists without a resume checkpoint"
            )
    ...
    for next_step in range(step + 1, OPTIMIZER_STEPS + 1):
        if optimizer is None:
            raise RuntimeError(
                "post-reveal finalization cannot execute optimizer steps"
            )
        ...
        if progress_due:
            ...
            print(
                f"layerwise_normalized_retained_readout step={step}/{OPTIMIZER_STEPS} "
    ...
    predecessor_model = (
        R4LayerwiseNormalizedRetainedReadoutLanguagePathV1.matched_v1_control(
            geometry
        ).to(device)
    )
    ...
    if candidate_artifact_path.exists():
        if cid_bytes(candidate_artifact) != cid_file(candidate_artifact_path):
            raise ValueError(
                "resumed layerwise-normalized-readout candidate artifact differs"
            )
    else:
    ...
    if not replay["passed"]:
        raise RuntimeError(
            "layerwise-normalized-readout candidate artifact replay differs"
        )
...
def verify_layerwise_normalized_retained_readout_result(root: Path) -> dict[str, Any]:
    ...
    preparation = load_layerwise_normalized_retained_readout_preparation(resolved_root)
    implementation = _require_implementation(preparation.manifest)
    device, backend = _configure_device(CPU_PLAN)
    if device.type != "cpu":
        raise RuntimeError(
            "layerwise-normalized-readout verifier selected a non-CPU device"
        )
    geometry = _exact_geometry(preparation.predecessor)
    predecessor_artifact = preparation.predecessor_artifact_path.read_bytes()
    ...
    if verification_path.exists():
        ...
        if (
            ...
            raise ValueError(
                "cached layerwise-normalized-readout verification binding differs"
            )
        return verification
    ...
    candidate_initial = _evaluate_language(
        R4LayerwiseNormalizedRetainedReadoutLanguagePathV1(geometry).to(device),
        preparation.fresh_heldout,
        device,
    )
    ...
    if not all(comparisons.values()):
        raise RuntimeError(
            "independent layerwise-normalized-readout re-score differs from terminal evidence"
        )
