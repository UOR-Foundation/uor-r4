"""Joint-query data copying, dual historical exclusion and access boundaries."""
...
from r4_softmax_trainer.zoology_joint_query import data
from r4_softmax_trainer.zoology_query_readout import data as previous
from safetensors.torch import load as load_safetensors


class JointQueryDataTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.source_root = Path(cls.temporary.name) / "source"
        cls.root = Path(cls.temporary.name) / "joint"
    ...
    def test_both_published_histories_are_excluded_with_balanced_valid_fresh_worlds(
        self,
    ) -> None:
        fresh = data.load_development(self.root)
        ...
        self.assertEqual(
            self.metadata["historical_development"]["1067"]["manifest_cid"],
        ...
        with self.assertRaisesRegex(ValueError, "#1067 development differs"):
