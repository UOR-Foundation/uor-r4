"""Focused synthetic contracts for the bounded direct-readout capacity campaign."""
...
import torch
from r4_softmax_trainer import direct_retained_readout_campaign as subject
...
class FrozenContractTests(unittest.TestCase):
    def test_population_training_and_fresh_heldout_arithmetic(self) -> None:
        self.assertEqual(subject.REVEALED_V1_PROMPT_LAST_SOURCE_STORY_ORDINAL, 153_977)
        ...
        self.assertEqual(subject.FRESH_HELDOUT_SOURCE_OFFSET_TOKENS, 155_532_141)
        self.assertEqual(
            subject.FRESH_HELDOUT_CID,
            "blake3:1d9df266c6e08c813c262dc671906d3909baf08024d77c5f2d9bfc0bcd4548c1",
        )
        self.assertEqual(subject.FRESH_HELDOUT_FIRST_CAPACITY_STORY, 761_588)
        self.assertEqual(subject.FRESH_HELDOUT_FIRST_SOURCE_STORY, 845_784)
        self.assertEqual(subject.FRESH_HELDOUT_LAST_CAPACITY_STORY, 762_818)
        self.assertEqual(subject.FRESH_HELDOUT_LAST_SOURCE_STORY, 847_140)
        self.assertLess(
            subject.REVEALED_V1_PROMPT_LAST_SOURCE_STORY_ORDINAL,
...
class RecoveryAndProbeTests(unittest.TestCase):
    def test_reveal_permanently_closes_fitting_even_on_resume(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch.object(
                subject, "load_direct_retained_readout_preparation"
            ) as loader:
                for resume in (False, True):
                    with self.assertRaisesRegex(ValueError, "permanently closed"):
                        subject.run_direct_retained_readout(root, resume=resume)
    ...
    def test_probe_is_one_candidate_cpu4_five_step_gate(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                ...
                result = subject.probe_direct_retained_readout(
                    root,
    ...
    def test_verifier_rejects_same_process_as_terminal_writer(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                ...
                subject.verify_direct_retained_readout_result(root)
    ...
    def test_verifier_recomputes_every_terminal_input_and_rejects_mismatch(
        ...
        def verify_once(
            ...
            with ExitStack() as stack:
                ...
                stack.enter_context(
                    mock.patch.object(
                        subject, "R4DirectRetainedReadoutLanguagePathV1", model_type
                    )
                )
                stack.enter_context(
                ...
                verification = subject.verify_direct_retained_readout_result(root)
