...
from r4_softmax_trainer import prompt_conditioning_v2 as subject
...
class PopulationSelectionTests(unittest.TestCase):
    def test_v2_contract_is_disjoint_from_revealed_v1(self) -> None:
        self.assertEqual(subject.POLICY, "R4RetainedPromptSwapContrastV2")
        self.assertEqual(subject.PRIOR_DEVELOPMENT_LAST_SOURCE_STORY_ORDINAL, 153_977)
        self.assertTrue(subject.POPULATION_SCHEMA.endswith("/2"))
        self.assertTrue(subject.COMMITMENT_SCHEMA.endswith("/2"))
        self.assertTrue(subject.REVEAL_SCHEMA.endswith("/2"))
        self.assertEqual(subject.WORK_RELATIVE_PATH, "prompt-conditioning-v2")
    ...
    def test_source_entrypoint_verifies_source_and_tokenizer_before_selection(
        self,
    ) -> None:
        boundary = subject.PRIOR_DEVELOPMENT_LAST_SOURCE_STORY_ORDINAL
        stories = [
...
class PairedScoringTests(unittest.TestCase):
    ...
    def test_state_off_or_replay_failure_invalidates_without_model_verdict(
        self,
    ) -> None:
