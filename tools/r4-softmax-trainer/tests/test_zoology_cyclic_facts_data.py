"""Cyclic-facts data identity, three-history exclusion and access boundaries."""
...
from r4_softmax_trainer.zoology_cyclic_facts import data
from r4_softmax_trainer.zoology_english_binding import data as source
from r4_softmax_trainer.zoology_joint_query import data as previous


class CyclicFactsDataTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.temporary = tempfile.TemporaryDirectory()
        cls.source_root = Path(cls.temporary.name) / "source"
        cls.root = Path(cls.temporary.name) / "cyclic"
    ...
    def test_three_histories_and_union_exclusion_preserve_valid_balanced_worlds(
        self,
    ) -> None:
        fresh = data.load_development(self.root)
        ...
        self.assertEqual(set(self.historical), {"1063", "1067", "1069"})
        ...
        self.assertEqual(audit_union["rows"], 3840)
        self.assertEqual(audit_union["populations"], 3)
        ...
        self.assertEqual(all_union["rows"], 14080)
        self.assertEqual(all_union["populations"], 4)
        ...
        self.assertEqual(
            self.metadata["historical_development"]["1069"]["manifest_cid"],
        ...
        altered["historical_development"]["1069"]["development"]["cid"] = "blake3:wrong"
        with self.assertRaisesRegex(ValueError, "#1069 development differs"):
