...
fn emit_term_for_binary(
    ...
    let operator = match expr.op {
        ...
        syn::BinOp::Le(_) if scope.in_route_body => return reject_psi_residual_op(expr, "<=", "Le"),
        syn::BinOp::Lt(_) if scope.in_route_body => return reject_psi_residual_op(expr, "<", "Lt"),
        syn::BinOp::Ge(_) if scope.in_route_body => return reject_psi_residual_op(expr, ">=", "Ge"),
...
fn clone_term_spec(spec: &TermSpec) -> TermSpec {
    match spec {
        ...
        TermSpec::AxisInvocation { axis_index, kernel_id, input_index } => {
        ...
        TermSpec::ProjectField { source_index, byte_offset, byte_length } => {
        ...
        TermSpec::Nerve { value_index } => TermSpec::Nerve { value_index: *value_index },
...
pub fn axis(input: TokenStream) -> TokenStream {
    ...
    let (body_arena_const, body_arena_expr): (
        ...
        if let Err(e) =
            emit_term_for_block(&body_block, &input_ident, &mut arena, &mut scope)
        {
            return e.to_compile_error().into();
        }
...
const RESOLVER_FIELD_TABLE: &[(&str, &str, &str, &str, &str)] = &[
    ...
    ("nerve", "Nerve", "NerveResolver", "HasNerveResolver", "nerve_resolver"),
    ("chain_complex", "ChainComplex", "ChainComplexResolver", "HasChainComplexResolver", "chain_complex_resolver"),
    ("homology_groups", "HomologyGroup", "HomologyGroupResolver", "HasHomologyGroupResolver", "homology_group_resolver"),
    ("cochain_complex", "CochainComplex", "CochainComplexResolver", "HasCochainComplexResolver", "cochain_complex_resolver"),
    ("cohomology_groups", "CohomologyGroup", "CohomologyGroupResolver", "HasCohomologyGroupResolver", "cohomology_group_resolver"),
    ("postnikov", "Postnikov", "PostnikovResolver", "HasPostnikovResolver", "postnikov_resolver"),
    ("homotopy_groups", "HomotopyGroup", "HomotopyGroupResolver", "HasHomotopyGroupResolver", "homotopy_group_resolver"),
    ("k_invariants", "KInvariant", "KInvariantResolver", "HasKInvariantResolver", "k_invariant_resolver"),
];
