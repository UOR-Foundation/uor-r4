...
Before proposing or launching model work, apply the execution policy's progress-control rules and the current-state active execution contract. Use one short work card in the existing issue; require a concrete model/implementation deliverable, new causal evidence for repeats, distinct outcome decisions, fixed context/access conditions, named necessary checks and complete costs. A passing test is not a substitute milestone. Do not add another campaign or validation layer when existing evidence already settles the decision; retain the accepted artifact and advance an independent implementation dependency. Do not reduce a protected memory horizon or confuse vector width with sequence length.
...
Use an isolated full worktree. Inspect the specific mechanism source and implement a concrete change tied to observed behavior. Rust training may use floats and matmul. Owner-adopted D0-b permits bounded <=4-bit additive/shift/lookup linear maps at serving, with no multiplier instruction in the declared numerical kernel and no floating point in served computation. Geometric routing remains preferred; no transformer backbone is adopted. Read DECISIONS.md and the latest takeover/current-state correction before applying older contract language. Deterministic geometry/addressing and shared typed operators are current; additional gates require evidence and cost justification.
