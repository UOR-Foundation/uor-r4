...
Start with the [September 19 takeover review](docs/integration/takeover-review-2026-09-19.md), [latest frozen-prior review](docs/integration/frozen-prior-review-2026-09-20.md), [current state](docs/integration/current-state.md), [canonical plan](docs/integration/project-track.md) and [next DeepSeek handoff](docs/integration/deepseek-frozen-prior-step-2026-09-20.md). These reconcile the earlier work and current mechanism evidence. Live [programme issue #820](https://github.com/UOR-Foundation/uor-r4/issues/820) owns delivery status; dated experiment records retain their original evidence scope.
...
PR #1300 records a real-text curve of **9.1316 → 8.2182 → 7.1425 bits/target** at steps 0/256/512. Regrouping the saved losses by the actual 32 evaluated documents retains a **1.9892-bit gain over the frozen quantized bias**, with a nominal document interval excluding zero. The full gate remains **pending corrected evaluation**: the old permutation changes target frequencies and its uncertainty calculation samples windows as documents. The three generation prefixes all belong to one document, and all recorded final outputs repeat. This is a positive local prediction result with important evaluation limits, not useful language or geometric advantage.

Next, **repair the evaluator and replay the saved step-512 artifact without retraining**. Recover the exact population and optimizer exposure, preserve target occurrences in the permutation, aggregate true document clusters, add count references, and diagnose greedy loops. The [principal review](docs/integration/frozen-prior-review-2026-09-20.md) gives source findings and the [complete DeepSeek prompt](docs/integration/deepseek-frozen-prior-step-2026-09-20.md) specifies the bounded run. Refitting on a smaller corpus would change the experiment rather than validate this result.

The roadmap then tests one learned older-prefix state/read channel with matched capacity/cost controls, keeping exact occurrence memory distinct. Equal local tails force equal predictions from this two-token prior; deterministic finite-state generation eventually cycles if it never stops, but that does not explain the observed short loops. Use the corrected comparisons to select the next mechanism, without making perfect prior fluency or unchanged retraining a prerequisite. The [canonical dependency sequence](docs/integration/project-track.md#research-dependencies-and-exit-conditions) connects this increment to the full goal.
