...
**Latest result: S is numerically stronger, but its history benefit is unresolved.** The [review after PR #1310](docs/integration/query-read-review-2026-09-20.md) independently reproduces E/Q/S/L scores of **7.170816 /7.141506 /7.032228 /7.101049 bits/target** on the reused panel. Q fails its registered practical and matched-control screens. S's conditional older-prefix penalty is **−0.002298 [−0.014072,+0.008581]**, while its current-token query channel matters. The evidence therefore supports a better numerical candidate, not demonstrated useful older memory. All generations remain repetitive.

The review also identifies zero tokenizer fields in CPX3, incomplete CPQK continuation, missing executable provenance and redundant inference state folds. These scope artifact/cost claims without overturning the saved CE comparison. The earlier projection-loop and singularity-report defects are repaired; the full prescribed projection remains unrun. Read the [raw query result](docs/integration/query-read-result-2026-09-20.md) together with the review.

**Next: execute the [frozen separable-reader attribution prompt](docs/integration/deepseek-separable-attribution-step-2026-09-20.md)**, currently NOT_RUN. Preserve all weights; distinguish individual history, current query and learned offsets, repair metadata and inference/evaluation plumbing, then replay the old panel and unused positions in the same corpus. This is an evaluation task. Selective state reset, exact occurrence/version memory and shared composition follow from the resulting diagnosis; useful language/coding and full laptop cost remain open.
