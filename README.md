...
The [contextual instruction refinement](docs/native_geometric_instruction_binding_1140.md) retains `ccbe7f2a` over preserved parent `0f0f5fe4`. Existing learned signed-H4 routers now retain the correct operands under the tested ordinary sentence/Rust request phrases. Construction, open, changed-name/order and post-selection numeric checks pass; all 663 retained answers, earlier panels and 72 prior lexical outputs remain preserved. Actual native API passes 38 checks, and 30 generated Rust equalities compile and evaluate. These finite authored forms do not qualify general prose or Rust program synthesis. The next step is to combine retained operator transitions with formatted output and exact intermediate dependencies.
