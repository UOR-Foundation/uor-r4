...
The [mixed operator refinement](docs/native_geometric_mixed_operators_1140.md) retains 9ab64902 over preserved parent 866cb92d. The shared learned signed-H4 selectors now compose Add with Copy of the latest result or original total, preserving exact operand identities and learned stopping. Construction, open, changed-context and post-selection numeric/name checks pass, as do all previously correct retained outputs and controls. Actual native API passes 65 checks; 54 previously supported generated Rust equalities still compile and execute. These finite forms do not qualify general prose or Rust program synthesis. Next diagnose mixed-operation formatting and, if the observed seam supports it, learn action-conditioned shared lexical emission.
