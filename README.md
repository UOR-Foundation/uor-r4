...
The [shared lexical emitter](docs/native_geometric_lexical_emission_1140.md) retains `0f0f5fe4` over preserved parent `6f20982c`. A learned signed-H4 selector chooses bytes and exact operand reads, producing the tested sentence `17 is 4 plus 13` or Rust equality `17 == 4 + 13`. Construction, open, changed-name/order and post-selection numeric checks pass; 18 generated Rust equalities compile and evaluate. All 663 retained answers and earlier operator/query behavior remain preserved, and the actual API passes 20 checks. These are two authored output forms, not general prose or Rust program synthesis. The next step is shared contextual instruction binding across selection and emission, then combined operator and formatted output.
