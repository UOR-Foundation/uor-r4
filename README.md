...
Start with the [September 19 takeover review](docs/integration/takeover-review-2026-09-19.md), [latest real-text prior review](docs/integration/realtext-prior-review-2026-09-20.md), [current state](docs/integration/current-state.md), [canonical plan](docs/integration/project-track.md) and [next DeepSeek handoff](docs/integration/deepseek-realtext-prior-step-2026-09-20.md). These reconcile the earlier work and current mechanism evidence. Live [programme issue #820](https://github.com/UOR-Foundation/uor-r4/issues/820) owns delivery status; dated experiment records retain their original evidence scope.
...
PR #1298 establishes a corrected small contextual fitting result: **16/16, 0.04974 bits/target**, versus **2 bits** for the constant and **4.78358 bits** after context-target permutation. The retained file hashes and integer-score records independently reproduce the curve. This is an authored fitting result, not language or geometric advantage.

Next, **finish mid-pass checkpoint continuation and the integer-only shared trace inside one thin real-text runner, then execute the prior-only curve at steps 0/256/512**. Keep memory disabled, freeze a fit-only unigram bias and compare exported predictions with exact/quantized unigram, corrected count references and within-document context permutation. The [principal review](docs/integration/realtext-prior-review-2026-09-20.md) scopes remaining defects; the [complete DeepSeek prompt](docs/integration/deepseek-realtext-prior-step-2026-09-20.md) supplies the experiment and resources. Another synthetic fitting campaign or floating comparator is not indicated.

The roadmap then tests an older-prefix-sensitive geometric state/read above the local baseline, with exact occurrence memory kept distinct. Equal local tails force equal predictions from a two-token prior; metadata alone is not geometric prediction. The [canonical dependency sequence](docs/integration/project-track.md#research-dependencies-and-exit-conditions) connects each increment to the full model goal.
