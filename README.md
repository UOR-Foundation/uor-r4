...
The [ternary head projection](docs/evidence/native_geometric_head_projection_2026-09-20.txt) is now **executed**. With the parent's embeddings, integer features, bias and `F` frozen and only the output head changed, **Q0** (the existing quantizer applied once to the retained floating head) scores **7.416178** bits/target and **QG** (one activation-aware dyadic ternary calibration over the fit-feature Gram) scores **7.190872**, against the incumbent gradient-trained ternary head **E** at **7.170816** and the raw floating head **F** at **6.874144** on the same 36-document / 288-window / 17,342-target panel.

The predeclared screens are decisive and split. **Both conversions fail the practical screen against E** (QG −0.0201 bits with an interval entirely negative; Q0 −0.2454), while the **method-attribution screen passes strongly**: activation-aware calibration is **+0.2253 bits/target** better than the existing quantizer [0.200621, 0.250299] and reconstructs F's scores 12.7× better than E does. Lower squared-score reconstruction error therefore does **not** imply lower cross-entropy: E is furthest from F in fit-score error and still predicts best. **E remains the stronger qualified numerical baseline**; these two bounded conversions are rejected, which does not establish ternary infeasibility, feature-rank saturation or a need to widen the head. The earlier reading that feature rank/capacity was the measured dominant bottleneck is withdrawn. Generation stays degenerate (parent/E/Q0 period-1 pair cycles, QG period-3), so no language claim follows.

The [canonical dependency sequence](docs/integration/project-track.md#research-dependencies-and-exit-conditions) therefore returns the architectural priority to a **query-conditioned geometric read/update** on this strengthened local baseline, with a separable-prefix residual and a matched local-only control. Exact occurrence/version memory, learned write/reset, shared composition, conversation and executable Rust qualification remain the larger programme.
