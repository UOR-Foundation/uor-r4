...
Two source corrections matter: QG omitted the optimized nearest-code seed at its original scale, so the complete prescribed search is **NOT_RUN**; and reported Gram singularity was never measured. These do not invalidate the saved losses. E/S tokenizer metadata is now corrected without changing numerical bytes. E remains the numerical incumbent, and all six generations remain degenerate. Preserve the [raw receipt](docs/evidence/native_geometric_head_projection_2026-09-20.txt) with this scope correction.

**Executed: the [matched geometric query read](docs/integration/query-read-result-2026-09-20.md) is a valid negative.** Frozen corrected E reproduced exactly at **7.170815718** bits/target, and three equally parameterized arms were fitted for 512 batch-8 updates each, exported as new `CPX3` artifacts, reloaded and evaluated on the same 36-document / 288-window / 17,342-target panel. Q (joint older-state/current-query transport) beats E by **+0.029310** bits but fails the predeclared 0.10-bit practical screen, is **0.109278 bits worse than the separable arm S** and 0.040456 worse than the local-only arm L, and shows no older-content or query-route evidence. **S is the strongest arm so far at +0.138588 bits**, and its query channel is demonstrably used (+0.190660). All instrument checks pass; no candidate is promoted and every arm's generation remains degenerate. [Raw receipt](docs/evidence/native_geometric_query_read_2026-09-20.txt).

The two source corrections above are delivered with focused tests in the same change: `project_row` now sweeps the nearest-code seed at every admissible shift including `s0` (regression on a correlated Gram, and the defect was reproduced by temporarily restoring the guard), and the head-projection runner reports `singularity: NOT_MEASURED` with `singular_inputs_supported: true`. The complete originally specified projection remains **NOT_RUN**; no QG campaign was rerun and no old report byte changed.

**Next: continue from the separable read S** and test selective write/reset, exact occurrence/version access or a history-capacity question on it, against a new independent behaviour panel. Do not rebuild the multiplicative read, start a projection/width/precision sweep, or add a fresh corpus. Writing remains the existing ordered group product; selective writing/reset and exact occurrence/version memory are later dependencies. The [canonical plan](docs/integration/project-track.md#research-dependencies-and-exit-conditions) connects this test to useful shared-model conversation and coding.
