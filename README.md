...
The learned older-prefix group-state/read pilot has been **executed and is numerically negative**, and its three saved candidates have now been **recovered as executable artifacts without retraining**: an exact `2I` table bound through an explicit bijection to the historical enumeration (all 14,400 products verified twice, **0 mismatches** against the floating table), packed reader/output coefficients, nibble-packed actions and the raw parent digest in one `CPX2` file. Reloaded parity is exact (0 state and 0 logit mismatches on all 17,342 panel records), the recorded losses reproduce exactly (parent 7.558429; primary 7.575604; fixed-action 7.723496; local-tail 7.576439), and arm-aware generation plus a full-vocabulary count-reference argmax are corrected. [Receipt](docs/evidence/native_geometric_prefix_recovery_readout_2026-09-20.txt).

The paired **output-head diagnostic** is now executed. Freezing the parent's embeddings, integer features, bias and `F` and training only `w_o` on the recovered 4,096-window dose, the **empirical** conditional-target arm reaches **7.170816** bits/target (gain **+0.387614** [0.360308, 0.415469] over the parent) and the **smoothed** count-family arm **7.188585** (+0.369845 [0.337491, 0.400210]). The practical screen **passes** for both, but **smoothed-target training does not beat equally dosed empirical refitting** — it is slightly worse on true-label development CE — so no benefit is attributed to the smoothed objective. The conditional floating relaxation ran (teacher-fit gap 4.5024 bits/target) on the same frozen features and soft targets: soft objective **6.877289** and dev CE **6.874144** (gain +0.684285) versus the smoothed hard arm's 7.196477/7.188585. On the same sealed panel the 512-window smoothed count reference is 7.079256 and the full-fit reference 5.061160.

The measured bottleneck is therefore the **frozen local feature map**, not the output objective: refitting the head recovers most of the same-dose gap, smoothed targets change nothing, and even a convex floating head on the same features stays ~1.8 bits above the full-fit reference with a teacher KL of 3.2-4.5 bits. The next bounded step is a local **representation** increase under the same hard integer contract, measured against the same panel and references; count matching is not a prerequisite, and no geometric-superiority, general-language or energy claim follows.
