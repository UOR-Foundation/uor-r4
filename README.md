...
Start with the [September 19 takeover review](docs/integration/takeover-review-2026-09-19.md), [latest prior-learning review](docs/integration/prior-learning-review-2026-09-19.md), [current state](docs/integration/current-state.md), [canonical plan](docs/integration/project-track.md) and [next DeepSeek handoff](docs/integration/deepseek-prior-learning-step-2026-09-19.md). These reconcile the earlier work and current mechanism evidence. Live [programme issue #820](https://github.com/UOR-Foundation/uor-r4/issues/820) owns delivery status; dated experiment records retain their original evidence scope.
...
PR #1294 built the exact-token prior plus causal memory. Its recorded losses are **clipped nats mislabeled bits**: the prior-only 6.7011 corresponds to approximately 9.6676 clipped bits/target, versus unigram 6.4019 nats (approximately 9.2360 bits). These are arithmetic conversions of reported values, not new evaluations. The negative direction is retained; the report does not establish undertraining or modulo aliasing as the cause.

Next, repair stable loss units, target/reference alignment, training/serving quantization parity and evidence preservation. Then obtain **one prior-only learning curve above a frozen quantized fit-unigram bias**, using representative deterministic sampling, context controls and a real optimizer checkpoint. A cheap balanced contextual fixture must establish correct exported learning before the corpus run. Memory stays disabled during fitting; the retained joint artifact supports diagnostic work. The [principal review](docs/integration/prior-learning-review-2026-09-19.md) explains the choice and research, and the [complete DeepSeek prompt](docs/integration/deepseek-prior-learning-step-2026-09-19.md) specifies the conditional implementation/run and delivery. An unchanged multi-epoch run is not the selected next action.
...
| Component | Source |
| --- | --- |
| Current exact-token prior, geometric memory and low-bit substrate | [cold_prior.rs](crates/uor-r4-core/src/native_geometric/learner/cold_prior.rs), [geometric_attention.rs](crates/uor-r4-core/src/native_geometric/learner/geometric_attention.rs), [lowbit.rs](crates/uor-r4-core/src/native_geometric/learner/lowbit.rs), [group_table.rs](crates/uor-r4-core/src/native_geometric/learner/group_table.rs) |
