...
The [shared nonnumeric emission result](docs/native_geometric_word_emission_973.md) connects exact words/spans to the existing signed-H4 lexical selector. The [contextual writer correction](docs/native_geometric_writer_lexical_973.md) distinguishes the tested instruction/fact collision, and [owner/value field composition](docs/native_geometric_field_composition_973.md) lets that same selector place an exact stored owner before its value under explicit owner-first requests. General prose and program synthesis remain unqualified. The [owner/revision repair](docs/native_geometric_owner_revision_973.md) now includes [writer-role transfer across value primes](docs/native_geometric_writer_role_transfer_973.md), repairing the tested newest-fact binding while preserving complete revision chains. The earlier [current-version source candidate](docs/native_geometric_current_version_read_973.md) remains rejected. The [reader/writer integration](docs/native_geometric_current_source_integration_973.md) improves the tested current-version selection while preserving that writer state. The [query-owner refinement](docs/native_geometric_query_owner_selection_973.md) repairs the measured competing-chain selection tie using the same geometric code vocabulary. The [historical-read result](docs/native_geometric_historical_selection_973.md) is now integrated with [reverse phrase-start transfer](docs/native_geometric_reverse_start_transfer_973.md): the retained combined model recovers complete stored values and passes the bounded fresh/preservation gates. [Historical owner/value composition](docs/native_geometric_historical_field_973.md) now learns past-tense emission from the exact selected old version while preserving current-record safeguards and retained behavior. [Contextual historical selection](docs/native_geometric_historical_query_context_973.md) now repairs the tested longer historical owner-first forms and competing-owner selection while preserving current-intent deferral. The [contextual current-record handoff](docs/native_geometric_current_query_handoff_973.md) now repairs the tested longer current requests while preserving genuine dependency and factual-word roles through exact source-occurrence context. The [learned version intent](docs/native_geometric_historical_version_973.md) now selects the exact initial version through validated ancestor chains with owner-role masking, and its [truncated-history abstention](docs/native_geometric_truncated_history_abstention_973.md) answers `Unknown.` when later facts have evicted the requested root, preserving previous/current answers. The [reassertion-versus-truncation repair](docs/native_geometric_reassertion_chain_repair_973.md) answers an independent review: a resident same-value reassertion is a validated link, a chain is truncated only when its predecessor is proven evicted, and report directories are created exclusively. The [head contract](docs/native_geometric_head_reassertion_973.md) then makes a repeated current fact keep the retained history readable: the first hop below a same-value reassertion head is valid, previous is the head's immediate previous record and initial is the root. The [minimal repeated-head repair](docs/native_geometric_minimal_head_973.md) then learns previous-record selection on the two-record identical-assertion chain through that same contract and records the owner-first current-request formatting failures without crediting them. The [current-request form repair](docs/native_geometric_current_form_repair_973.md) answers the fourth independent review: the frozen persistent reader now scans the request words after the latest committed fact, so a current request whose instructions follow the question reaches its committed live head with the complete value, present tense, declared form and termination, and the earlier record carries an appended qualification correction. The [reader turn-scope and capacity repair](docs/native_geometric_reader_scope_repair_973.md) answers the fifth independent review: the owner scan now stops at the current turn's input boundary as well as the committed-fact cutoff, so an earlier question or generated answer never names the owner for the next turn, the feature buffer is sized from its true maximum, and every changed row is classified against the three preceding artifacts. Next under #973 learn intermediate-version intent with exact depth labels. [Current state](docs/integration/current-state.md) owns artifact identities, measured results and resource receipts.
