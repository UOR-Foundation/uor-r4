...
The [principal review after PR #1306](docs/integration/readout-result-review-2026-09-20.md) verifies the saved-data gains and corrects the teacher, exposure, artifact-identity and capacity interpretations. A finite floating fit does not prove a feature-rank bottleneck; E/S teacher KLs are not comparable across their different targets. The **next bounded experiment** projects the retained floating head into the existing ternary serving format, comparing ordinary quantization with fit-activation-aware calibration. Features and serving format stay fixed. [Complete DeepSeek prompt](docs/integration/deepseek-head-projection-step-2026-09-20.md). This is NOT_RUN; it precedes a return to query-conditioned geometric state research without imposing count matching. No general-language, geometric-superiority or energy claim follows.
