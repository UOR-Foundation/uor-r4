#!/usr/bin/env python3
"""Probe fixed canonical error function E*(x) = pi(x) - Li(x).

This is a math-only diagnostic for lower-envelope behavior on sampled windows.
"""
...
def parse_beta_list(text: str) -> List[float]:
    ...
    for raw in text.split(","):
        ...
        out.append(float(v))
    if not out:
        raise ValueError("beta list is empty")
    return out
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Fixed-error pi(x)-Li(x) oscillation probe")
    ...
    ap.add_argument(
        "--out-prefix",
        type=str,
        default=f"research/output/fixed_error_pi_li_probe_{date.today().isoformat()}",
    )
    args = ap.parse_args()
    ...
    e_vals = np.empty(len(xs), dtype=np.float64)
    for i, x in enumerate(xs):
        pix = float(sp.primepi(int(x)))
        e_vals[i] = pix - float(li(int(x)))
    ...
    for beta in betas:
        denom = np.power(xs.astype(np.float64), beta)
        ratio = abs_e / denom
        denom_tail = np.power(xs_tail.astype(np.float64), beta)
        ratio_tail = abs_e_tail / denom_tail
        beta_stats.append(
            BetaStats(
                beta=beta,
    ...
    lines: List[str] = []
    lines.append(f"# Fixed Error pi(x)-Li(x) Probe ({date.today().isoformat()})")
    lines.append("")
    lines.append(f"- Window: `[xmin, xmax] = [{xmin}, {xmax}]`")
    lines.append(f"- Samples: `{len(xs)}` (log-spaced unique integers)")
    ...
    lines.append("")
    lines.append("Interpretation: this is empirical finite-window behavior only; it is not a theorem-grade Ω result.")
    md_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
