#!/usr/bin/env python3
"""Check O5 endpoint inequality on grid using conservative theorem-budget constants."""
...
def main() -> None:
    ap = argparse.ArgumentParser(description="O5 endpoint check with theorem-budget constants")
    ...
    ap.add_argument("--output", default="research/output/o5_theorem_budget_endpoint_check_2026-02-17.json")
    args = ap.parse_args()
    ...
    c0_ref = float(c["C0_ref_from_o5"])
    c_h_budget = float(c["C_H_theorem_budget"])
    a_h = float(c["A_H"])
    ...
    for row in a4.get("per_n", []):
        ...
        rhs = c0_ref + b_ref + a_ref * c_h_budget * (lx ** a_h)
    ...
    payload: Dict[str, Any] = {
        ...
        "interpretation": (
            "Conservative theorem-budget constants still dominate tested endpoint rows; "
            "remaining work is asymptotic proof of assumptions, not finite-grid failure."
        ),
    }
    ...
    lines = [
        "# O5 Theorem-Budget Endpoint Check",
        "",
        ...
        f"- `|b_ref| = {b_ref}`",
        f"- `C0_ref = {c0_ref}`",
        f"- `C_H_theorem_budget = {c_h_budget}`",
        f"- `A_H = {a_h}`",
        "",
        "## Grid Check",
        "",
