#!/usr/bin/env python3
"""A3 density-transfer majorant via |H| = B * sqrt(k/x)."""
...
def main() -> None:
    ap = argparse.ArgumentParser(description="A3 density-transfer majorant")
    ...
    ap.add_argument("--safety-factor-b", type=float, default=1.1)
    ...
    ap.add_argument("--cache-dir", type=str, default="research/cache/a3_density_transfer_majorant")
    ap.add_argument("--no-cache", action="store_true")
    ap.add_argument("--output", type=str, default="research/output/a3_density_transfer_majorant.json")
    args = ap.parse_args()
    ...
    b_model = fit_log_envelope(
        ...
        a_max=args.a_max,
        a_step=args.a_step,
        safety_factor=args.safety_factor_b,
    )
    a_h = float(b_model["A"])
    c_b = float(b_model["C_uplifted"])
    ...
    report = {
        ...
        "config": {
            ...
            "a_max": args.a_max,
            "a_step": args.a_step,
            "safety_factor_b": max(1.0, args.safety_factor_b),
        ...
        "b_envelope": {
            "A_B": a_h,
            "C_B_train_max": float(b_model["C_train_max"]),
            "C_B_uplifted": c_b,
            "ratio_max_train": float(b_model["ratio_max_train"]),
            "ratio_max_valid": float(b_model["ratio_max_valid"]),
        },
        "h_transfer_envelope": {
            "A_H": a_h,
            "C_H_from_density_transfer": c_h,
            "transfer_identity": "|H| = B*sqrt(k/x), B <= C_B(log x)^A_B, k/x <= rho_guard",
        },
    ...
    with open(md, "w", encoding="utf-8") as f:
        f.write("# A3 Density-Transfer Majorant\n\n")
        f.write(f"Generated: {report['timestamp_utc']}\n\n")
        dg = report["density_guardrail"]
        be = report["b_envelope"]
        he = report["h_transfer_envelope"]
        f.write(f"- rho_sup_sampled: {dg['rho_sup_sampled']:.12g}\n")
        f.write(f"- rho_guard: {dg['rho_guard']:.12g}\n")
        f.write(f"- sqrt_rho_guard: {dg['sqrt_rho_guard']:.12g}\n")
        if dg.get("analytic_per_base"):
            f.write("- analytic density guard (per base):\n")
            for r in dg["analytic_per_base"]:
                f.write(
                    f"  - W={r['base']}: phi/W={r['phi_over_W']:.12g}, "
                    f"rho_upper_x_ge_x0={r['rho_upper_x_ge_x0']:.12g}\n"
                )
        f.write(f"- A_B: {be['A_B']:.6f}\n")
        f.write(f"- C_B_uplifted: {be['C_B_uplifted']:.12e}\n")
