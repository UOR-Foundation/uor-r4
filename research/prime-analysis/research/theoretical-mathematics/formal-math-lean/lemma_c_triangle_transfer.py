#!/usr/bin/env python3
"""Lemma C triangle-transfer constants around M_ref."""
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Lemma C triangle-transfer probe")
    ...
    ap.add_argument("--cache-dir", type=str, default="research/cache/lemma_c_triangle_transfer")
    ap.add_argument("--no-cache", action="store_true")
    ap.add_argument("--output", type=str, default="research/output/lemma_c_triangle_transfer_none_z128_ref512.json")
    args = ap.parse_args()
    ...
    def base_rows(b: int):
        ...
        for n in n_values:
            ...
            out.append({"n_max": n, "base": b, "x": xs, "e_scaled": e_scaled, "h_m": hm[:k], "h_ref": hr[:k], "s_ref": s_ref, "a_se": a_se, "b_se": b_se})
        return out
    ...
    # Freeze affine map on reference process.
    a_ref, b_ref, _ = hx.fit_linear(pooled_h_ref, pooled_e)

    # C0 := sup |E - (a_ref H_ref + b_ref)|
    c0 = 0.0
    c_smooth = 0.0
    c_link = 0.0
    for row in rows:
        ...
        a_se = row["a_se"]
        b_se = row["b_se"]
        for ev, hv, sv in zip(e, h_ref, s_ref):
            c0 = max(c0, abs(ev - (a_ref * hv + b_ref)))
            c_smooth = max(c_smooth, abs(ev - (a_se * sv + b_se)))
            c_link = max(c_link, abs((a_ref * hv + b_ref) - (a_se * sv + b_se)))

    # Check triangle bound for M process:
    # |E-(a_ref H_M + b_ref)| <= C0 + |a_ref|*|H_M-H_ref|
    per_row = []
    violations = 0
    max_gap = -1e18
    for row in rows:
        ...
        rhs = [c0 + abs(a_ref) * abs(hm - hr) for hm, hr in zip(h_m, h_ref)]
        gaps = [r - b for r, b in zip(resid, rhs)]
        vcount = sum(1 for g in gaps if g > 1e-12)
        violations += vcount
        max_gap = max(max_gap, max(gaps) if gaps else 0.0)
        per_row.append(
            {
                "n_max": row["n_max"],
                "base": row["base"],
                ...
                "max_gap_res_minus_rhs": max(gaps) if gaps else 0.0,
    ...
    report = {
        ...
        "constants": {
            ...
            "b_ref": b_ref,
            "C0_ref_residual": c0,
            "C_smooth_ref": c_smooth,
            "C_link_ref": c_link,
            "triangle_split_upper_bound_C0": c_smooth + c_link,
        },
        "inequality": {
            "form": "|E-(a_ref*H_M+b_ref)| <= C0_ref + |a_ref|*|H_M-H_ref|",
            "holds_on_test_grid": violations == 0 and max_gap <= 1e-12,
    ...
    with open(md, "w", encoding="utf-8") as f:
        c = report["constants"]
        i = report["inequality"]
        f.write("# Lemma C Triangle Transfer\n\n")
        f.write(f"Generated: {report['timestamp_utc']}\n\n")
        f.write(f"- a_ref: {c['a_ref']:.9f}\n")
        f.write(f"- b_ref: {c['b_ref']:.9f}\n")
        f.write(f"- C0_ref_residual: {c['C0_ref_residual']:.9f}\n")
        f.write(f"- C_smooth_ref: {c['C_smooth_ref']:.9f}\n")
        f.write(f"- C_link_ref: {c['C_link_ref']:.9f}\n")
        f.write(f"- C_smooth_ref + C_link_ref: {c['triangle_split_upper_bound_C0']:.9f}\n")
        f.write(f"- holds on test grid: {i['holds_on_test_grid']}\n")
        f.write(f"- total violations: {i['total_violations']}\n")
        f.write(f"- max gap (res-rhs): {i['global_max_gap_res_minus_rhs']:.3e}\n\n")
        for r in sorted(per_row, key=lambda z: (z["n_max"], z["base"])):
            f.write(
                f"- n_max={r['n_max']} base={r['base']} "
                f"max_res={r['max_residual']:.6f} max_rhs={r['max_rhs']:.6f} "
