#!/usr/bin/env python3
"""A3 mean-square majorant using m2 = (1/k) * sum g_i^2."""
...
def main() -> None:
    ap = argparse.ArgumentParser(description="A3 mean-square majorant")
    ...
    ap.add_argument("--safety-factor-upper", type=float, default=1.1)
    ...
    ap.add_argument("--cache-dir", type=str, default="research/cache/a3_mean_square_majorant")
    ap.add_argument("--no-cache", action="store_true")
    ap.add_argument("--output", type=str, default="research/output/a3_mean_square_majorant.json")
    args = ap.parse_args()
    ...
    for b in bases:
        ...
        for n in all_n:
            for x in x_map[n]:
                ...
                m2 = e2 / max(1.0, float(k))
                h_abs = abs(s) / math.sqrt(max(1.0, float(x)))
                h_upper_m2 = math.sqrt(max(0.0, e2 * dens))
                rows.append(
                    {
                        "base": b,
                        "n_max": n,
                        ...
                        "m2": m2,
                        "h_abs": h_abs,
                        "h_upper_m2": h_upper_m2,
                        "det_gap_h_minus_h_upper_m2": h_abs - h_upper_m2,
                    }
                )
                rho_values.append(dens)

    upper_model = fit_log_envelope(
        rows=rows,
        train_n=train_n,
        valid_n=valid_n,
        target_key="h_upper_m2",
        a_min=args.a_min,
        a_max=args.a_max,
        a_step=args.a_step,
        safety_factor=args.safety_factor_upper,
    )
    a_h = float(upper_model["A"])
    c_h = float(upper_model["C_uplifted"])
    ...
    # Deterministic upper target is fitted directly:
    # |H| <= sqrt((sum g_i^2) * k/x) <= C_H (log x)^A_H
    ...
    def check_split(ns: set):
        ...
        for r in rows:
            if int(r["n_max"]) not in ns:
                continue
            det_gap = max(det_gap, float(r["det_gap_h_minus_h_upper_m2"]))
            if float(r["det_gap_h_minus_h_upper_m2"]) > 1e-12:
                det_v += 1
        ...
        return {
            ...
            "deterministic_h_m2_holds": det_v == 0 and det_gap <= 1e-12,
            "deterministic_h_m2_violations": int(det_v),
            "deterministic_h_m2_max_gap": float(det_gap),
        }
    ...
    report = {
        ...
        "config": {
            ...
            "a_max": args.a_max,
            "a_step": args.a_step,
            "safety_factor_upper": max(1.0, args.safety_factor_upper),
        ...
        "upper_envelope": {
            "A_upper": a_h,
            "C_upper_train_max": float(upper_model["C_train_max"]),
            "C_upper_uplifted": c_h,
            "ratio_max_train": float(upper_model["ratio_max_train"]),
            "ratio_max_valid": float(upper_model["ratio_max_valid"]),
        },
        "h_transfer_envelope": {
            "A_H": a_h,
    ...
    with open(md, "w", encoding="utf-8") as f:
        f.write("# A3 Mean-Square Majorant\n\n")
        f.write(f"Generated: {report['timestamp_utc']}\n\n")
        me = report["upper_envelope"]
        he = report["h_transfer_envelope"]
        f.write(f"- A_upper: {me['A_upper']:.6f}\n")
        f.write(f"- C_upper_uplifted: {me['C_upper_uplifted']:.12e}\n")
        f.write(f"- A_H: {he['A_H']:.6f}\n")
        f.write(f"- C_H: {he['C_H_from_density_transfer']:.12e}\n")
        f.write(f"- train holds: {report['checks']['train']['holds']}\n")
        f.write(f"- valid holds: {report['checks']['valid']['holds']}\n")
        f.write(f"- deterministic H<=sqrt(m2*k/x) holds(train): {report['checks']['train']['deterministic_h_m2_holds']}\n")
        f.write(f"- deterministic H<=sqrt(m2*k/x) holds(valid): {report['checks']['valid']['deterministic_h_m2_holds']}\n")
