...
- Add schema contract doc under `docs/contracts/`
...
- Add timing hooks for stage-level runtime accounting in summary JSON
...
- write gate notes under `docs/governance/gates/`

## PR-4: Hypothesis-targeted experiments only
- Add decision gates so we stop exploring dead branches.
- include new route candidates (`phase4d`, `complex2`) only through the route matrix.
