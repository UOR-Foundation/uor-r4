//! CS-E6 typed-input carrier — wraps the canonicalize discipline's
...
/// CS-E6's typed-input carrier.
#[derive(Clone, Copy, Debug)]
pub struct E6Carrier<'a>(&'a [u8]);
...
impl ConstrainedTypeShape for E6Carrier<'_> {
    const IRI: &'static str = "https://uor.foundation/addr/composition/E6FiltrationCarrier";
    const SITE_COUNT: usize = 1;
    const CONSTRAINTS: &'static [ConstraintRef] = &[];
    const CYCLE_SIZE: u64 = u64::MAX;
}

impl prism::uor_foundation::pipeline::__sdk_seal::Sealed for E6Carrier<'_> {}

impl<'a> IntoBindingValue<'a> for E6Carrier<'a> {
    fn as_binding_value<const INLINE_BYTES: usize>(&self) -> TermValue<'a, INLINE_BYTES> {
        TermValue::borrowed(self.0)
    }
}

impl PartitionProductFields for E6Carrier<'_> {
