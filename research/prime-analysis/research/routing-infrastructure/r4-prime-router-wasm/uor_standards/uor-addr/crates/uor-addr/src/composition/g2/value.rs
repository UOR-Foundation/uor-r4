//! CS-G2 typed-input carrier — wraps the canonicalize discipline's
...
/// CS-G2's typed-input carrier.
#[derive(Clone, Copy, Debug)]
pub struct G2Carrier<'a>(&'a [u8]);
...
impl ConstrainedTypeShape for G2Carrier<'_> {
    const IRI: &'static str = "https://uor.foundation/addr/composition/G2ProductCarrier";
    const SITE_COUNT: usize = 1;
    const CONSTRAINTS: &'static [ConstraintRef] = &[];
    const CYCLE_SIZE: u64 = u64::MAX;
}

impl prism::uor_foundation::pipeline::__sdk_seal::Sealed for G2Carrier<'_> {}

impl<'a> IntoBindingValue<'a> for G2Carrier<'a> {
    fn as_binding_value<const INLINE_BYTES: usize>(&self) -> TermValue<'a, INLINE_BYTES> {
        TermValue::borrowed(self.0)
    }
}

impl PartitionProductFields for G2Carrier<'_> {
