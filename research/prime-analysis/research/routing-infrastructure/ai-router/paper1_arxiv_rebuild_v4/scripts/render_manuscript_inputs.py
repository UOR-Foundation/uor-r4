...
DRAFT = ROOT / "paper1_arxiv_rebuild_v4"
...
def render_method_figure() -> None:
    ...
    pair1 = abs(corr[0, 1])
    pair2 = abs(corr[2, 3])
    fig = plt.figure(figsize=(8.6, 4.15))
    gs = gridspec.GridSpec(1, 3, width_ratios=[1.00, 1.06, 1.18], wspace=0.36)

    ax = fig.add_subplot(gs[0, 0])
    ax.set_title("A. Fixed 4D chart", loc="left", fontweight="bold", fontsize=11.2)
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.axis("off")
    ...
    ax.text(0.23, 0.13, r"$x \in \mathbb{R}^{100}$", ha="center", fontsize=10.2)
    ax.add_patch(FancyArrowPatch((0.45, 0.58), (0.60, 0.58), arrowstyle="->", mutation_scale=11, linewidth=1.2, color=SLATE))
    for idx, xpos in zip([3, 65, 2, 21], [0.67, 0.77, 0.87, 0.97]):
        ...
        ax.add_patch(rect)
        ax.text(xpos, 0.57, f"{idx}", ha="center", va="center", fontsize=10.8, fontweight="bold")
    ax.text(0.82, 0.79, "fixed chart", ha="center", fontsize=10.2)
    ax.text(0.82, 0.72, f"$|\\rho| \\approx {pair1:.3f}, {pair2:.3f}$", ha="center", fontsize=9.6)
    ax.text(0.82, 0.13, r"$(3,65,2,21)$", ha="center", fontsize=10.0)

    ax = fig.add_subplot(gs[0, 1])
    ax.set_title("B. Torus phases and Hopf split", loc="left", fontweight="bold", fontsize=11.2)
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.axis("off")
    ax.text(0.04, 0.89, r"$q_1=a+ib,\ \ q_2=c+id$", ha="left", fontsize=11.2)
    ax.text(0.04, 0.80, r"phases $(\theta_1,\theta_2)$", ha="left", fontsize=9.8)
    ...
    ax.text(0.24, 0.78, r"$\theta_1$", color=BLUE, ha="center", fontsize=10.0)
    ax.add_patch(FancyArrowPatch((0.44, 0.61), (0.44, 0.41), arrowstyle="->", mutation_scale=11, linewidth=1.3, color=TEAL))
    ax.text(0.49, 0.51, r"$\theta_2$", color=TEAL, va="center", fontsize=10.0)
    ax.add_patch(FancyArrowPatch((0.51, 0.54), (0.66, 0.54), arrowstyle="->", mutation_scale=12, linewidth=1.2, color=SLATE))
    ...
    ax.text(0.05, 0.20, r"base $(\eta,\delta)$", fontsize=10.0)
    ax.text(0.05, 0.10, r"fiber $\alpha$", fontsize=10.0)

    ax = fig.add_subplot(gs[0, 2])
    ax.set_title("C. Transport and discretization", loc="left", fontweight="bold", fontsize=11.2)
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.axis("off")
    ax.text(0.04, 0.89, r"$\tilde{\alpha} = \alpha + \frac{1}{2}\lambda\cos(2\eta)\delta$", fontsize=10.4)
    ax.text(0.04, 0.80, r"transport-corrected phase", fontsize=9.8)
    ...
    for y, label, n, color in zip(yrows, labels, counts, colors):
        ax.text(0.05, y, label, fontsize=10.8, color=color, va="center")
        x0, width = 0.16, 0.31
    ...
    ax.add_patch(FancyArrowPatch((0.54, 0.52), (0.66, 0.52), arrowstyle="->", mutation_scale=12, linewidth=1.2, color=SLATE))
    left, bottom, width, height = 0.65, 0.24, 0.30, 0.44
    ...
    ax.text(0.80, 0.14, r"route index $r(x)$", ha="center", fontsize=10.0)
...
def render_scaling_figure() -> None:
    ...
    ax.plot(kline, hopf_fit, color=BLUE, linewidth=1.8, label=r"Angular")
    ax.plot(kline, perm_fit, color=ORANGE, linewidth=1.8, label=r"Permuted")
    ...
    ax.plot(k_vals, ratio, color=SLATE, linewidth=1.8, marker="o", markersize=4, label="Gap")
    ax2 = ax.twinx()
    ax2.plot(k_vals, active_frac_orig, color=BLUE, linewidth=1.4, linestyle="-", label=r"Struct. frac.")
    ax2.plot(k_vals, active_frac_perm, color=ORANGE, linewidth=1.4, linestyle="-", label=r"Perm. frac.")
    ax.set_xscale("log")
    ax.set_xlabel(r"Routing budget $K$")
    ...
    ax.legend(lines1 + lines2, labels1 + labels2, frameon=False, loc="upper left", handlelength=1.7)
