#!/usr/bin/env python3
"""Event-triggered bridge switching on the stronger reduced-alignment setting."""
...
def extract_geometry_features_v15(
    inputs: np.ndarray,
    ...
    *,
    ...
    for row in range(size):
        ...
        current_baseline = initial_scores[current_idx]
        switch_at: int | None = None
        switched_variant = current_variant
        ...
        for start in range(min_trigger_start, max_trigger_start + 1):
            ...
            if current_score >= current_baseline - trigger_drop:
                continue
            ...
            best_idx = int(np.argmax(np.array(window_scores)))
            if variants[best_idx] != current_variant and window_scores[best_idx] >= current_score + trigger_margin:
                switch_at = start
                switched_variant = variants[best_idx]
                break
        ...
        if switch_at is None:
            ...
            switched_features, _, _ = variant_cache[switched_variant]
            combined = np.concatenate([current_features[:switch_at], switched_features[switch_at:]], axis=0)
            features.extend(combined)

    return np.stack(features, axis=0).reshape(size, seq_len, -1)


def evaluate_geometry_v15(
    model: GeometryNativeSequenceModelV3,
    features: np.ndarray,
    targets: np.ndarray,
...
def run_bounded_sequence_comparison_v15(config: TaskConfigV12, *, seed: int = 0) -> list[SequenceMetricsV15]:
    ...
    test_features = extract_geometry_features_v15(test_inputs, geometry_model)
    geometry_test_loss, geometry_test_acc, geometry_transfer_query_acc, geometry_eval_seconds = evaluate_geometry_v15(
        geometry_model,
        test_features,
        test_targets,
        test_query_mask,
    ...
    return [
        SequenceMetricsV15(
            model="geometry_native_sequence_model_v15",
        ...
        SequenceMetricsV15(
            model="tiny_transformer_baseline_v15",
