#!/usr/bin/env python3
"""Bounded multiscale chart field on the stronger shifted family."""
...
COARSE_BLOCKS_V32 = 2
MAX_REGIONS_V32 = 4
TRANSITION_PENALTY_V32 = 0.30


@dataclass
class SequenceMetricsV32:
    model: str
    test_loss: float
    test_accuracy: float
...
class MultiscaleChartField(torch.nn.Module):
    def __init__(self, feature_dim: int, label_count: int, *, seed: int) -> None:
        super().__init__()
        torch.manual_seed(seed)
        self.local_proj = torch.nn.Linear(feature_dim, 20)
        self.local_out = torch.nn.Linear(20, label_count)
        self.coarse_proj = torch.nn.Linear(feature_dim, 12)
        self.coarse_out = torch.nn.Linear(12, label_count)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: [batch, blocks, feature_dim]
        local_hidden = torch.tanh(self.local_proj(x))
        local_logits = self.local_out(local_hidden)

        batch, block_count, feature_dim = x.shape
        coarse_width = block_count // COARSE_BLOCKS_V32
        coarse_inputs = x.reshape(batch, COARSE_BLOCKS_V32, coarse_width, feature_dim).mean(dim=2)
        coarse_hidden = torch.tanh(self.coarse_proj(coarse_inputs))
        coarse_logits = self.coarse_out(coarse_hidden)
        coarse_logits = coarse_logits.repeat_interleave(coarse_width, dim=1)
        return local_logits + coarse_logits
...
def _block_details_v32(
    ...
    *,
    ...
    bounds = _block_bounds(len(raw_snapshots), block_count=BLOCK_COUNT_V32)
...
def _build_training_set_v32(
    ...
    *,
    ...
    for row in range(calibration_inputs.shape[0]):
        ...
        block_features, block_caches, bounds = _block_details_v32(
            raw_snapshots,
            model,
...
def _fit_multiscale_chart_field(
    train_x: np.ndarray,
    train_y: np.ndarray,
    *,
    seed: int,
    epochs: int = 220,
    lr: float = 0.02,
) -> MultiscaleChartField:
    field = MultiscaleChartField(train_x.shape[-1], len(REGION_VARIANTS_V22), seed=seed)
...
def _decode_multiscale_path_v32(
    logits: np.ndarray,
    *,
    transition_penalty: float = TRANSITION_PENALTY_V32,
    max_regions: int = MAX_REGIONS_V32,
) -> list[int]:
...
def extract_geometry_features_v32(
    inputs: np.ndarray,
    model: GeometryNativeSequenceModelV3,
    *,
    support_window: int = 10,
    calibration_size: int = 256,
    calibration_seed: int = 293,
    field_seed: int = 0,
    ...
    train_x, train_y = _build_training_set_v32(
        calibration_inputs,
        calibration_targets,
    ...
    chart_field = _fit_multiscale_chart_field(train_x, train_y, seed=field_seed)

    size, seq_len = inputs.shape
    features: list[np.ndarray] = []
    for row in range(size):
        raw_snapshots = _collect_raw_snapshots_v27(inputs[row])
        block_features, _block_caches, bounds = _block_details_v32(
            raw_snapshots,
            model,
            support_window=support_window,
        )
        ...
        labels = _decode_multiscale_path_v32(logits)
        regions = _collapse_regions_v32(labels, bounds)
...
def run_bounded_sequence_comparison_v32(config: TaskConfigV27, *, seed: int = 0) -> list[SequenceMetricsV32]:
    ...
    test_features, field_param_count = extract_geometry_features_v32(
        test_inputs,
        geometry_model,
        calibration_seed=seed + 97,
        field_seed=seed + 101,
    )
    geometry_test_loss, geometry_test_acc, geometry_transfer_query_acc, geometry_eval_seconds = evaluate_geometry_v32(
        geometry_model,
        test_features,
        test_targets,
        test_query_mask,
    ...
    total_state_size = geometry_model.effective_state_size + BLOCK_COUNT_V32 + COARSE_BLOCKS_V32

    return [
        SequenceMetricsV32(
            model="geometry_native_sequence_model_v32",
        ...
        SequenceMetricsV32(
            model="tiny_transformer_baseline_v32",
