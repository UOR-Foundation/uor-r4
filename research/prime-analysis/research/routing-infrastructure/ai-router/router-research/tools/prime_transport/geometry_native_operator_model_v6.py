#!/usr/bin/env python3
"""Learned weighting over lawful H_v5 operator entries only."""
...
from geometry_native_operator_model_v5 import fiber_phase_lift_component_v5
...
OUTPUT_PATH_OPERATOR_WEIGHTING_V1 = Path(
...
def _generator_apply_v6(state: OperatorStateV4, generator_index: int) -> OperatorStateV4:
    ...
    if generator_index == 5:
        return fiber_phase_lift_component_v5(state)
    raise ValueError(f"unknown generator index {generator_index}")


def _update_discourse_v6(
    token_id: int,
    *,
    operator_state: OperatorStateV4,
    discourse_state: RuntimeDiscourseStateV6,
) -> RuntimeDiscourseStateV6:
    ...
    return RuntimeDiscourseStateV6(
        focus=focus,
        speaker=speaker,
        topic=topic,
        style=style,
...
class StructuredLawfulOperatorSelectorV6:
    """Structured additive selector over the six lawful H_v5 generators."""
    ...
    def param_count(self) -> int:
        return (
            ...
            + self.phi.size
            + self.twist.size
            + self.query_semiprime.size
            + self.binding_semiprime.size
            + self.style.size
            + self.gap.size
            + self.focus.size
            + self.speaker.size
    ...
    def scores(self, context: tuple[int, ...]) -> np.ndarray:
        token_id, b_idx, phi_idx, twist_idx, query_idx, binding_idx, style_idx, gap_idx, focus_idx, speaker_idx, topic_idx, query_flag_idx, tag_flag_idx = context
        return (
            self.base
            + self.token[token_id]
            + self.b[b_idx]
        ...
        for _ in range(epochs):
            for row in range(train_inputs.shape[0]):
                ...
                for col in range(train_inputs.shape[1]):
                    ...
                    context = _context_indices_v6(token_id, operator_state=operator_state, discourse_state=discourse_state)
                    ...
                    for generator_index in range(6):
                        next_operator = _generator_apply_v6(operator_state, generator_index)
                        next_discourse = _update_discourse_v6(
                            token_id,
                            operator_state=next_operator,
                            discourse_state=discourse_state,
                        )
                        pred_class = _predicted_class_v6(
                            token_id,
                            operator_state=next_operator,
...
def _class_probabilities_v6(scores: np.ndarray, predicted_classes: list[int]) -> np.ndarray:
...
def _evaluate_policy_v6(
    inputs: np.ndarray,
    targets: np.ndarray,
    query_mask: np.ndarray,
    *,
    policy_name: str,
    selector: StructuredLawfulOperatorSelectorV6 | None,
    ...
    for row in range(inputs.shape[0]):
        ...
        for col in range(inputs.shape[1]):
            ...
            if selector is not None:
                ...
                probs = _class_probabilities_v6(scores, predicted_classes)
            else:
    ...
    return SequenceMetricsV6(
        ...
        usage_T_z=usage[5] / max(total_usage, 1.0),
...
def _read_reference_metric_v6(csv_path: Path, model_name: str) -> SequenceMetricsV6:
...
def run_operator_weighting_v1(config: TaskConfigV6, *, seed: int = 241) -> list[SequenceMetricsV6]:
    ...
    selector = StructuredLawfulOperatorSelectorV6()
    train_seconds = selector.fit(train_inputs, train_targets)

    v5_fixed = _evaluate_policy_v6(
        test_inputs,
        test_targets,
        test_query_mask,
        policy_name="geometry_native_operator_model_v5_fixed_policy",
        selector=None,
        fixed_policy="token_cycle",
        param_count=0,
        train_seconds=0.0,
    )
    v6_learned = _evaluate_policy_v6(
        test_inputs,
        test_targets,
        test_query_mask,
        policy_name="geometry_native_operator_model_v6",
        selector=selector,
        fixed_policy=None,
    ...
    v3_ref = _read_reference_metric_v6(R5_CSV, "geometry_native_sequence_model_v3_reference")
    r5_ref = _read_reference_metric_v6(R5_CSV, "geometry_native_sequence_model_r5")
    transformer_ref = _read_reference_metric_v6(R5_CSV, "tiny_transformer_baseline_r5")
    transformer_ref.model = "tiny_transformer_baseline_v6_reference"
    return [v3_ref, r5_ref, v5_fixed, v6_learned, transformer_ref]


def write_operator_weighting_v1(rows: list[SequenceMetricsV6], output_path: Path = OUTPUT_PATH_OPERATOR_WEIGHTING_V1) -> None:
...
if __name__ == "__main__":
    metrics = run_operator_weighting_v1(TaskConfigV6(), seed=241)
    write_operator_weighting_v1(metrics)
