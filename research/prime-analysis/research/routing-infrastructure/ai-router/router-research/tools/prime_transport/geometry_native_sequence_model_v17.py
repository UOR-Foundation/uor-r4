#!/usr/bin/env python3
"""Conflict-triggered factor-level bridge revision on the stronger reduced-alignment setting."""
...
def extract_geometry_features_v17(
    inputs: np.ndarray,
    ...
    *,
    ...
    for row in range(size):
        ...
        max_trigger_start = max(min_trigger_start, seq_len - conflict_window)
        ...
        if trigger_start is None or trigger_alt is None:
            ...
            revised_suffix = _conflict_revised_suffix(
...
def run_bounded_sequence_comparison_v17(config: TaskConfigV12, *, seed: int = 0) -> list[SequenceMetricsV17]:
    ...
    test_features = extract_geometry_features_v17(test_inputs, geometry_model)
    geometry_test_loss, geometry_test_acc, geometry_transfer_query_acc, geometry_eval_seconds = evaluate_geometry_v17(
        geometry_model,
        test_features,
        test_targets,
        test_query_mask,
    ...
    return [
        SequenceMetricsV17(
            model="geometry_native_sequence_model_v17",
        ...
        SequenceMetricsV17(
            model="tiny_transformer_baseline_v17",
