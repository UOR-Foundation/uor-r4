#!/usr/bin/env python3
"""Fuller bounded extended transport-side state for spin_H."""
...
from geometry_native_operator_model_v10 import _to_v4_view as _to_v4_view_v10
...
from geometry_native_spinH_candidate_v3 import (
    NativeTauV3,
    PrimaryChartStateSpinHCandidateV3,
    TransportStateSpinHCandidateV3,
    active_transport_lift_v3,
)


OUTPUT_PATH_SPINH_EXTENDED_V1 = Path(
...
class AngularIdentityV1:
    base_angle: int
    fiber_phase: int


@dataclass(frozen=True)
class RadialUnfoldingIdentityV1:
    radial_class: int
    unfolding_load: int
...
@dataclass(frozen=True)
class PredictiveShellV1:
    current_spin: tuple[int, ...]
    fiber_spin: tuple[int, ...]
    radial_spin: tuple[int, ...]


@dataclass(frozen=True)
class ClosureShellV1:
    current_tau: tuple[int, int, int, int]
    fiber_tau: tuple[int, int, int, int]
    radial_tau: tuple[int, int, int, int]


@dataclass(frozen=True)
class SpinHExtendedV1:
...
@dataclass(frozen=True)
class TransportStateSpinHExtendedV1:
    spin_H_extended: SpinHExtendedV1
...
def primary_chart_of_extended_v1(state: object) -> PrimaryChartStateSpinHCandidateV3:
    return PrimaryChartStateSpinHCandidateV3(b=int(state.b), phi=int(state.phi), r=int(state.r))


def active_transport_lift_extended_v1(state: object) -> TransportStateSpinHExtendedV1:
    ...
    return TransportStateSpinHExtendedV1(spin_H_extended=extended)


def _summary_metrics_from_v3(path: Path = V3_PATH) -> dict[str, float]:
...
def summarize_spinH_extended_v1(depth: int = 8) -> list[dict[str, object]]:
    states, transitions = bounded_operator_surface_v8(depth=depth)

    primary_to_transport: dict[PrimaryChartStateSpinHCandidateV3, set[TransportStateSpinHExtendedV1]] = defaultdict(set)
    transport_to_primary: dict[TransportStateSpinHExtendedV1, set[PrimaryChartStateSpinHCandidateV3]] = defaultdict(set)
    transport_counter: Counter[TransportStateSpinHExtendedV1] = Counter()
    transport_transition_map: dict[TransportStateSpinHExtendedV1, dict[str, set[TransportStateSpinHExtendedV1]]] = defaultdict(lambda: defaultdict(set))
    spin_repr_counter: Counter[PredictiveShellV1] = Counter()
    ...
    for state in states:
        primary = primary_chart_of_extended_v1(state)
        transport = active_transport_lift_extended_v1(state)
        primary_to_transport[primary].add(transport)
        transport_to_primary[transport].add(primary)
        transport_counter[transport] += 1
        spin_repr_counter[transport.spin_H_extended.predictive_shell] += 1
    ...
    for transition in transitions:
        source_transport = active_transport_lift_extended_v1(transition.source)
        target_transport = active_transport_lift_extended_v1(transition.target)
    ...
    rows.append({"scope": "summary", "metric": "distinct_transport_identities_reached", "count": distinct_transport_count, "total": primary_count, "fraction": distinct_transport_count / max(primary_count, 1), "note": "distinct coherent transport identities under spin_H_extended"})
    rows.append({"scope": "summary", "metric": "collision_count", "count": collision_count, "total": primary_count, "fraction": collision_fraction, "note": "many-to-one collisions from primary chart states into coherent extended transport identities"})
    ...
    rows.append({"scope": "summary", "metric": "distinct_spin_classes_represented", "count": len(spin_repr_counter), "total": len(spin_repr_counter), "fraction": 1.0, "note": "number of distinct predictive shells represented"})
    ...
    rows.append({"scope": "comparison_vs_v3", "metric": "collision_change_vs_v3", "count": collision_count - int(v3_metrics["collision_count__count"]), "total": int(v3_metrics["collision_count__count"]), "fraction": collision_fraction - v3_metrics["collision_count__fraction"], "note": "change in collisions relative to spin_H_candidate_v3"})
    rows.append({"scope": "comparison_vs_v3", "metric": "ambiguity_change_vs_v3", "count": ambiguity_count - int(v3_metrics["ambiguity_count__count"]), "total": int(v3_metrics["ambiguity_count__count"]), "fraction": ambiguity_fraction - v3_metrics["ambiguity_count__fraction"], "note": "change in ambiguity relative to spin_H_candidate_v3"})
    rows.append({"scope": "comparison_vs_v3", "metric": "recursive_consistency_change_vs_v3", "count": canonical_transport_count - int(v3_metrics["canonical_states_under_iteration__count"]), "total": distinct_transport_count, "fraction": recursive_consistency_rate - v3_metrics["recursive_consistency_rate__fraction"], "note": "change in recursive consistency relative to spin_H_candidate_v3"})
...
def write_spinH_extended_v1(
    rows: list[dict[str, object]],
    output_path: Path = OUTPUT_PATH_SPINH_EXTENDED_V1,
) -> None:
...
if __name__ == "__main__":
    summary_rows = summarize_spinH_extended_v1(depth=8)
    write_spinH_extended_v1(summary_rows)
