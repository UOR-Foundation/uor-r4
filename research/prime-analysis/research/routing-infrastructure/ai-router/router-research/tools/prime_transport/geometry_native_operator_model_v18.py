#!/usr/bin/env python3
"""Fourteenth operator formulation rebuilding T_r* directly from spin_H_core_v4."""
...
from geometry_native_operator_model_v16 import coupled_torus_kick_component_v16
...
OUTPUT_PATH_OPERATOR_FROM_CORE_V4_RADIAL = Path(
    "/Users/adminamn/AI-Research/ai-router/router-research/results/"
    "prime_transport_recursive_system/prime_transport_operator_from_spinH_core_v4_radial.csv"
)


def _binary_spin_value_v18(word: tuple[int, ...]) -> int:
    return int("".join(str(int(bit)) for bit in word), 2)


def radial_transport_component_v18(state: OperatorStateV10) -> OperatorStateV10:
    core = active_transport_lift_core_v4(state)
    sigma_successor = sigma_update_v4(core.sigma, "radial_transport_unfolding")
    ...
    target_tau = NativeTauV3(
        ...
        lift_phase=(
            ...
            + core.rho.unfolding_load
            + target_phi
            + (_binary_spin_value_v18(sigma_successor.current_mode) % 7)
            + (_binary_spin_value_v18(sigma_successor.radial_mode) % 5)
        )
        % 12,
    )
...
class SparseLawfulOperatorV18:
    """Explicit sparse operator with T_c, T_z', and T_r* rebuilt from spin_H_core_v4."""
    ...
    def transitions(self, state: OperatorStateV10) -> tuple[OperatorTransitionV1, ...]:
        ...
        radial_target = radial_transport_component_v18(state)
        return (
    ...
    def _is_lawful(source: OperatorStateV10, target: OperatorStateV10, component: str) -> bool:
        ...
        if component == "fiber_phase_lift_spin_transport":
            return target == fiber_phase_lift_component_v17(source)
        if component == "radial_transport_unfolding":
            expected = radial_transport_component_v18(source)
            return (
                target == expected
...
def bounded_operator_surface_v18(depth: int = 8) -> tuple[tuple[OperatorStateV10, ...], tuple[OperatorTransitionV1, ...]]:
    operator = SparseLawfulOperatorV18()
...
def _summary_metrics_from_v15() -> dict[str, float]:
    rows = list(csv.DictReader(OUTPUT_PATH_OPERATOR_FROM_CORE_V3_RADIAL.open("r", encoding="utf-8")))
    out: dict[str, float] = {}
    for row in rows:
...
def summarize_operator_from_spinH_core_v4_radial(depth: int = 8) -> list[dict[str, object]]:
    old = _summary_metrics_from_v15()
    states, transitions = bounded_operator_surface_v18(depth=depth)
    ...
    rows.append({"scope": "operator", "metric": "state_count", "count": len(states), "total": len(states), "fraction": 1.0, "note": "reachable states with T_r* rebuilt from spin_H_core_v4"})
    ...
    rows.append({"scope": "core_dependency", "metric": "rebuilt_component_depends_on_spin_H_core_v4", "count": 1, "total": 1, "fraction": 1.0, "note": "T_r* calls active_transport_lift_core_v4 and uses sigma_direct with sigma_update_v4"})
    rows.append({"scope": "core_dependency", "metric": "sigma_direct_materially_used", "count": 1, "total": 1, "fraction": 1.0, "note": "T_r* uses sigma_update_v4 on sigma_direct and reads current_mode, fiber_mode, radial_mode, and regressive_phase"})
    ...
    rows.append({"scope": "comparison_vs_v15", "metric": "state_count_change", "count": len(states) - int(old["state_count__count"]), "total": int(old["state_count__count"]), "fraction": (len(states) / max(old["state_count__count"], 1.0)) - 1.0, "note": "reachable state change vs prior T_r* rebuild from spin_H_core_v3"})
    rows.append({"scope": "comparison_vs_v15", "metric": "transition_count_change", "count": len(transitions) - int(old["transition_count__count"]), "total": int(old["transition_count__count"]), "fraction": (len(transitions) / max(old["transition_count__count"], 1.0)) - 1.0, "note": "transition count change vs prior T_r* rebuild from spin_H_core_v3"})
    rows.append({"scope": "comparison_vs_v15", "metric": "class_diversity_change", "count": len(target_class_counter) - int(old["distinct_class_identities_reached__count"]), "total": int(old["distinct_class_identities_reached__count"]), "fraction": (len(target_class_counter) / max(old["distinct_class_identities_reached__count"], 1.0)) - 1.0, "note": "class diversity change vs prior T_r* rebuild from spin_H_core_v3"})
    rows.append({"scope": "comparison_vs_v15", "metric": "spin_diversity_change", "count": len(target_spin_counter) - int(old["distinct_spin_classes_reached__count"]), "total": int(old["distinct_spin_classes_reached__count"]), "fraction": (len(target_spin_counter) / max(old["distinct_spin_classes_reached__count"], 1.0)) - 1.0, "note": "spin diversity change vs prior T_r* rebuild from spin_H_core_v3"})
    rows.append({"scope": "comparison_vs_v15", "metric": "radial_diversity_change", "count": len(radial_counter) - int(old["distinct_radial_classes_reached__count"]), "total": int(old["distinct_radial_classes_reached__count"]), "fraction": (len(radial_counter) / max(old["distinct_radial_classes_reached__count"], 1.0)) - 1.0, "note": "radial diversity change vs prior T_r* rebuild from spin_H_core_v3"})
...
def write_operator_from_spinH_core_v4_radial(
    rows: list[dict[str, object]],
    output_path: Path = OUTPUT_PATH_OPERATOR_FROM_CORE_V4_RADIAL,
) -> None:
...
__all__ = [
    ...
    "SparseLawfulOperatorV18",
    "bounded_operator_surface_v18",
