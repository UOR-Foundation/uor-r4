#!/usr/bin/env python3
"""INC-0163: Matched-progress compute efficiency probe.

Extends the INC-0160 training efficiency probe mechanism to track:
- Per-step routing cost (active buckets, effective buckets)
- Per-step training progress (cosine similarity, NOT MSE)
- Periodic eval checkpoints (cosine on held-out data)
- Cumulative routing cost curves

The goal: compare ORIG vs PERM cumulative routing cost at matched
learning progress levels.

NO MSE is used for success criteria or primary comparisons.
Cosine similarity is the progress metric.
"""
import argparse
import json
import os
...
def eval_cosine(buckets, keys, z, y, d, dy):
    """Mean cosine similarity on a dataset (eval progress metric)."""
    n = z.shape[0]
...
def main():
    ap = argparse.ArgumentParser(
        description="INC-0163: matched-progress compute efficiency probe")
    ap.add_argument("--config", required=True)
    ap.add_argument("--seed", type=int, default=0)
    ap.add_argument("--routes", type=str, default="")
    ap.add_argument("--window", type=int, default=500,
                    help="Sliding window for per-window cosine")
    ...
    for route_id in route_ids:
        ...
        # Set up routing state (same as INC-0160 probe)
        ...
        v_ev = state["v_ev"]
        y_ev = state["y_ev"]
        z_ev = state.get("z_ev", None)
        if z_ev is None:
            z_ev = state.get("z_tr_eval", None)
        keys_ev = [
        ...
        step_cosines = []  # per-step cosine similarity

        # Curve samples
        progress_curve = []    # (step, window_cosine, cumul_unique, cumul_eff)
        eval_curve = []        # (step, eval_cosine, eval_eff_buckets)
        ...
        for _epoch in range(epochs):
            ...
            for jj in order:
                ...
                # Predict BEFORE update (for progress measurement)
    ...
    payload = {
        "experiment": "inc0163_matched_progress",
        "config": args.config,
        "seed": int(args.seed),
        "results": results,
    }
