# V1 Model Block Permutation

- **Conversation ID:** `699b1cf2-8224-832e-94ed-03fd10484ef6`
- **Date:** 2026-02-22

---

### User (2026-02-22 10:12:58)
...
### Assistant (2026-02-22 10:13:52)

import numpy as np, json, math, os, time
data_dir="/mnt/data"
cheb=np.load(os.path.join(data_dir,"chebyshev_psi.npz"))
pr=np.load(os.path.join(data_dir,"primes.npz"))
x_grid=cheb["x_grid"]
psi_minus_x=cheb["psi_minus_x"]
primes=pr["primes"]
len(x_grid), len(primes), primes[:5], primes[-1]

### Tool (2026-02-22 10:13:56)

(100000, 664579, array([ 2,  3,  5,  7, 11], dtype=int32), 9999991)

### Assistant (2026-02-22 10:13:57)
...
# interp psi_minus_x onto t_uni
f_interp=interp1d(t_raw, psi_minus_x.astype(np.float64), kind="linear", fill_value="extrapolate", assume_sorted=True)
u_full=f_interp(t_uni)
x_uni=np.exp(t_uni)
# restrict kept
kept_mask=(x_uni>=x_min) & (x_uni<=x_max)
u_raw=u_full[kept_mask]
t_kept=t_uni[kept_mask]
x_kept=x_uni[kept_mask]
N_kept=len(u_raw)
N_kept, dt, (x_kept[0], x_kept[-1]), (t_kept[0], t_kept[-1])

### Tool (2026-02-22 10:14:05)
...
### Assistant (2026-02-22 10:14:06)

def causal_moving_average(x, W):
    # past-only including current; for i<W-1 use average over 0..i
...
# vectorize causal MA faster
def causal_moving_average_fast(x, W):
    x=np.asarray(x, dtype=np.float64)
    n=len(x)
...
# split indices for train/test on kept (time order)
train_end=int(np.floor(train_frac*N_kept))
train_end

### Tool (2026-02-22 10:14:14)

12807

### Assistant (2026-02-22 10:14:15)

# standardize using TRAIN only
mu=float(u_detr[:train_end].mean())
sig=float(u_detr[:train_end].std(ddof=0))
u_std=(u_detr-mu)/(sig+1e-12)

# bandpass via FFT mask on full kept standardized signal
n=N_kept
U=rfft(u_std)
...
omega=(dtheta + np.pi)%(2*np.pi)-np.pi  # wrap to (-pi,pi]
# align features
theta_f=theta[:-1]
logA_f=logA[:-1]
N_feat=len(theta_f)
assert N_feat==N_kept-1
N_feat

### Tool (2026-02-22 10:14:24)

18295

### Assistant (2026-02-22 10:14:25)

# Build R on x_kept (length N_kept), then drop last to N_feat
expDelta=np.exp(Delta)
# prime counts using searchsorted
p=primes.astype(np.int64)
left_idx=np.searchsorted(p, x_kept, side="left")
right_idx=np.searchsorted(p, x_kept*expDelta, side="left")  # primes < upper
P=(right_idx-left_idx).astype(np.float64)
E=(x_kept*(expDelta-1.0)/np.log(x_kept)).astype(np.float64)
R=(P-E)/np.sqrt(E+1e-12)
R_f=R[:-1]  # align
len(R_f), N_feat

### Tool (2026-02-22 10:14:29)

(18295, 18295)

### Assistant (2026-02-22 10:14:31)

# lag features require i>=2
R0=R_f
R1=np.roll(R0,1); R2=np.roll(R0,2)
# mark invalid first 2
R1[:1]=np.nan
R2[:2]=np.nan

# compute train/test for feature-aligned indices
train_end_feat=int(np.floor(train_frac*N_feat))
test_start=train_end_feat
train_end_feat, test_start, N_feat

### Tool (2026-02-22 10:14:34)

(12806, 12806, 18295)

### Assistant (2026-02-22 10:14:35)

# Standardize non-angle scalars using TRAIN stats (on aligned indices, excluding NaNs)
def mean_std_train(arr):
    a=arr[:train_end_feat]
    a=a[~np.isnan(a)]
    m=float(a.mean())
    s=float(a.std(ddof=0))
    return m, s if s>0 else 1.0

mR0,sR0=mean_std_train(R0)
mR1,sR1=mean_std_train(R1)
mR2,sR2=mean_std_train(R2)
mLogA,sLogA=mean_std_train(logA_f)

omega_raw=omega  # length N_feat
mOm,sOm=mean_std_train(omega_raw)
# interaction raw based on theta_f
sin1=np.sin(theta_f); cos1=np.cos(theta_f)
int_s_raw=sin1*omega_raw
int_c_raw=cos1*omega_raw
mIs,sIs=mean_std_train(int_s_raw)
mIc,sIc=mean_std_train(int_c_raw)

(mR0,sR0,mOm,sOm)

### Tool (2026-02-22 10:14:41)
...
### Assistant (2026-02-22 10:14:43)

# prepare standardized arrays
R0z=(R0-mR0)/sR0
R1z=(R1-mR1)/sR1
R2z=(R2-mR2)/sR2
logAz=(logA_f-mLogA)/sLogA
omz=(omega_raw-mOm)/sOm
isz=(int_s_raw-mIs)/sIs
icz=(int_c_raw-mIc)/sIc

# Fourier features K=3
K=3
P0=np.column_stack([np.sin((k)*theta_f) for k in range(1,K+1)] + [np.cos((k)*theta_f) for k in range(1,K+1)])
# omega-related
Ofeat=np.column_stack([omz, isz, icz])

# B1 features
B1=np.column_stack([R0z,R1z,R2z,logAz])
# V1
V1=np.hstack([B1,P0,Ofeat])
V1.shape

### Tool (2026-02-22 10:14:48)

(18295, 13)

### Assistant (2026-02-22 10:14:49)
...
s=np.array([0.2,0.5,0.1,0.6,0.4])
auc_rank(y,s)

### Tool (2026-02-22 10:14:57)

1.0

### Assistant (2026-02-22 10:14:59)
...
for h in H:
    # valid indices
    # i>=2, i+h < N_feat
    valid_all=np.arange(2, N_feat-h, dtype=np.int64)
    train_valid=valid_all[valid_all+h < train_end_feat]  # both in train since i<h train end
    test_valid=valid_all[(valid_all>=test_start) & (valid_all+h < N_feat) & (valid_all+h>=test_start)]
    # truncate test_valid to drop remainder for blocks
    m=len(test_valid)
    nb=m//block_size
    m_trunc=nb*block_size
    test_valid_trunc=test_valid[:m_trunc]
    # targets
    y_train=(R0[train_valid+h]-R0[train_valid] > 0).astype(np.int8)
    y_test=(R0[test_valid_trunc+h]-R0[test_valid_trunc] > 0).astype(np.int8)
    # features
    X_train=V1[train_valid]
    X_test=V1[test_valid_trunc]
    # remove any rows with nan (due to lags early); should be none due to i>=2
    nan_train=np.isnan(X_train).any(axis=1)
    if nan_train.any():
        X_train=X_train[~nan_train]; y_train=y_train[~nan_train]
    nan_test=np.isnan(X_test).any(axis=1)
    if nan_test.any():
        X_test=X_test[~nan_test]; y_test=y_test[~nan_test]
        # also adjust test_valid_trunc correspondingly (for permutation contiguous assumption); but should not happen.
    # fit logistic
    clf=LogisticRegression(C=1.0, penalty="l2", solver="lbfgs", max_iter=1000)
    clf.fit(X_train, y_train)
    p_true=clf.predict_proba(X_test)[:,1]
    auc_true=auc_rank(y_test, p_true)
    brier_true=brier(y_test, p_true)
    # prepare arrays for permutation: theta and omega raw at test_valid_trunc
    theta_test=theta_f[test_valid_trunc]
    omega_test_raw=omega_raw[test_valid_trunc]
    # blocks
    theta_blocks=theta_test.reshape(nb, block_size)
    omega_blocks=omega_test_raw.reshape(nb, block_size)
    auc_perm=[]
    for r in range(1, Nperm+1):
        rng=np.random.default_rng(seed + 100000*h + r)
        perm=rng.permutation(nb)
        th_perm=theta_blocks[perm].reshape(-1)
        om_perm_raw=omega_blocks[perm].reshape(-1)
        # recompute P0 and omega terms
        P0_perm=np.column_stack([np.sin(k*th_perm) for k in range(1,K+1)] + [np.cos(k*th_perm) for k in range(1,K+1)])
        sinp=np.sin(th_perm); cosp=np.cos(th_perm)
        int_s=sinp*om_perm_raw
        int_c=cosp*om_perm_raw
        # standardize using TRAIN stats
        om_p=(om_perm_raw-mOm)/sOm
        is_p=(int_s-mIs)/sIs
        ic_p=(int_c-mIc)/sIc
        O_perm=np.column_stack([om_p,is_p,ic_p])
        # X_perm = B1_fixed + P0_perm + O_perm
        B1_fixed=B1[test_valid_trunc]  # standardized already
        Xp=np.hstack([B1_fixed, P0_perm, O_perm])
        # predict probs quickly
        logits=Xp @ clf.coef_.ravel() + clf.intercept_[0]
        pp=sigmoid(logits)
        auc_perm.append(auc_rank(y_test, pp))
    auc_perm=np.array(auc_perm, dtype=np.float64)
    # stats
    auc_mean=float(np.nanmean(auc_perm))
    auc_std=float(np.nanstd(auc_perm, ddof=0))
    p_value=float((1.0 + np.sum(auc_perm >= auc_true))/ (1.0+Nperm))
    z=float((auc_true-auc_mean)/(auc_std+1e-12))
    results[str(h)] = {
        "V1": {"AUC_true": float(auc_true), "Brier_true": float(brier_true),
               "n_train": int(len(y_train)), "n_test": int(len(y_test)),
               "pos_rate_test": float(y_test.mean())},
        "perm": {"auc_mean": auc_mean, "auc_std": auc_std, "p_value": p_value, "z": z,
                 "auc_perm_min": float(np.min(auc_perm)), "auc_perm_max": float(np.max(auc_perm)),
                 "n_blocks": int(nb), "m_trunc": int(m_trunc)}
    }
results

### Tool (2026-02-22 10:15:22)

{'25': {'V1': {'AUC_true': 0.8498096502877379,
   'Brier_true': 0.15826098360227306,
   'n_train': 12779,
   'n_test': 5120,
   'pos_rate_test': 0.509765625},
  'perm': {'auc_mean': 0.7686929695013051,
   ...
   'n_blocks': 10,
   'm_trunc': 5120}},
 '100': {'V1': {'AUC_true': 0.840580165737223,
   'Brier_true': 0.16360420020703548,
   'n_train': 12704,
   'n_test': 5120,
   'pos_rate_test': 0.5046875},
  'perm': {'auc_mean': 0.8344500340606109,
