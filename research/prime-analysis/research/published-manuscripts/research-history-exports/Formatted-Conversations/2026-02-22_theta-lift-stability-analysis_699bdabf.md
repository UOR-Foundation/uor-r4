# Theta Lift Stability Analysis

- **Conversation ID:** `699bdabf-c0a0-8329-8772-12a254a3eddb`
- **Date:** 2026-02-22

---

### User (2026-02-22 23:42:44)
...
### Assistant (2026-02-22 23:42:48)

import numpy as np, os, hashlib, json, math
from pathlib import Path
path="/mnt/data/A4_FEATURECACHE_h25_AR80_AR40_theta_omega_masks_v1.npz"
Path(path).exists(), os.path.getsize(path)

### Tool (2026-02-22 23:43:04)

(True, 1422464)

### Assistant (2026-02-22 23:43:06)
...
### Tool (2026-02-22 23:43:09)

'340f3ce5b561fe00e872ef98991390185f600f96'

### Assistant (2026-02-22 23:43:11)

npz=np.load(path, allow_pickle=True)
npz.files[:50], len(npz.files)

### Tool (2026-02-22 23:43:13)
...
### Assistant (2026-02-22 23:43:14)

theta=npz['theta']; R=npz['R']
theta.shape, R.shape, npz['N_kept'], npz['train_end'], npz['dt']

### Tool (2026-02-22 23:43:16)

((36592,), (36592,), array(36592), array(25614), array(0.00012297))

### Assistant (2026-02-22 23:43:18)

def sha1_array(a):
    # ensure contiguous bytes in little endian float64
...
### Tool (2026-02-22 23:43:20)

('8b33c57df436d37179b13b1899bf86a723f0e98a',
 '5753ee7d59c7d155c4a1b9b5fba8912d557598f2')

### Assistant (2026-02-22 23:43:22)

S=npz['S']; X80=npz['X80']; X40=npz['X40']; omega=npz['omega']
S.shape, X80.shape, X40.shape, omega.shape

### Tool (2026-02-22 23:43:26)

((36592, 2), (36592, 80), (36592, 40), (36591,))

### Assistant (2026-02-22 23:43:30)

npz['source_theta_key'], npz['source_R_key']

### Tool (2026-02-22 23:43:34)

(array('theta_mod', dtype='<U9'), array('R', dtype='<U1'))

### Assistant (2026-02-22 23:43:36)

h=25
N=len(R)
y=np.zeros(N, dtype=int)
valid_future=np.arange(N-h)
y[valid_future]=(R[valid_future+h]-R[valid_future] > 0).astype(int)
# last h remain 0 but will be masked out presumably
mask_train=npz['mask_train_lag79'].astype(bool)
mask_test=npz['mask_test_lag79'].astype(bool)
mask_train.sum(), mask_test.sum(), N

### Tool (2026-02-22 23:43:44)

(25510, 10953, 36592)

### Assistant (2026-02-22 23:43:45)

# sanity: ensure masks exclude last h and first 79
np.where(mask_train)[0].min(), np.where(mask_train)[0].max(), np.where(mask_test)[0].min(), np.where(mask_test)[0].max()

### Tool (2026-02-22 23:43:47)

(79, 25588, 25614, 36566)

### Assistant (2026-02-22 23:43:50)

from sklearn.linear_model import LogisticRegression
from sklearn.metrics import roc_auc_score
# prepare train data
Xb_train=X80[mask_train]
Xs_train=np.hstack([X80[mask_train], S[mask_train]])
y_train=y[mask_train]
Xb_test=X80[mask_test]
Xs_test=np.hstack([X80[mask_test], S[mask_test]])
y_test=y[mask_test]
y_train.mean(), y_test.mean()

### Tool (2026-02-22 23:43:54)

(0.493375147001176, 0.4972153747831644)

### Assistant (2026-02-22 23:43:57)

# Fit models with L2, C=1.0, solver lbfgs, max_iter
clf_base=LogisticRegression(penalty='l2', C=1.0, solver='lbfgs', max_iter=2000)
clf_full=LogisticRegression(penalty='l2', C=1.0, solver='lbfgs', max_iter=2000)
clf_base.fit(Xb_train, y_train)
clf_full.fit(Xs_train, y_train)
# predict probs on test
p_base=clf_base.predict_proba(Xb_test)[:,1]
p_full=clf_full.predict_proba(Xs_test)[:,1]
roc_auc_score(y_test, p_base), roc_auc_score(y_test, p_full), roc_auc_score(y_test, p_full)-roc_auc_score(y_test, p_base)

### Tool (2026-02-22 23:44:02)

(0.7849798016893133, 0.8053005819522191, 0.020320780262905846)

### Assistant (2026-02-22 23:44:17)
...
### Tool (2026-02-22 23:44:21)

([2191, 2191, 2191, 2190, 2190], 10953)

### Assistant (2026-02-22 23:44:24)

block_results=[]
start=0
for b,sz in enumerate(sizes):
    sl=idx_test[start:start+sz]
    # map to positions within Xb_test/p arrays: since Xb_test corresponds to idx_test order
    # start..start+sz in that order
    yb=y_test[start:start+sz]
    pb=p_base[start:start+sz]
    pf=p_full[start:start+sz]
    aucb=roc_auc_score(yb, pb)
    aucf=roc_auc_score(yb, pf)
    block_results.append((b, sz, aucb, aucf, aucf-aucb))
    start+=sz
block_results

### Tool (2026-02-22 23:44:31)

[(0, 2191, 0.7991312700998203, 0.8216430689554358, 0.022511798855615495),
 (1, 2191, 0.7693418673067147, 0.7961347101032578, 0.026792842796543037),
 (2, 2191, 0.7802240958952996, 0.8042662935626211, 0.02404219766732152),
 (3, 2190, 0.809597097097097, 0.8234718051384717, 0.013874708041374695),
 (4, 2190, 0.7749640643905914, 0.7944566761630887, 0.01949261177249728)]

### Assistant (2026-02-22 23:44:33)

deltas=np.array([r[4] for r in block_results])
mean_delta=float(deltas.mean())
std_delta=float(deltas.std(ddof=0))
# linear regression deltas ~ b
x=np.arange(K, dtype=float)
# slope, intercept
slope, intercept=np.polyfit(x, deltas, 1)
pred=intercept + slope*x
ss_res=float(np.sum((deltas-pred)**2))
ss_tot=float(np.sum((deltas-deltas.mean())**2))
R2=0.0 if ss_tot==0 else 1 - ss_res/ss_tot
mean_delta, std_delta, float(slope), float(R2)

### Tool (2026-02-22 23:44:38)

(0.021342831826670405,
 0.004417054823058703,
 -0.001895650892140481,
 0.3683673549237967)

### Assistant (2026-02-22 23:44:40)
