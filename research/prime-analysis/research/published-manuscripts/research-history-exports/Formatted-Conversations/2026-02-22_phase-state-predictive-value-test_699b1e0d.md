# Phase-state Predictive Value Test

- **Conversation ID:** `699b1e0d-6260-8327-87ed-3a22757b1f81`
- **Date:** 2026-02-22

---

### User (2026-02-22 10:17:38)
...
base_dir="/mnt/data"
cheb_path=os.path.join(base_dir,"chebyshev_psi.npz")
primes_path=os.path.join(base_dir,"primes.npz")
...
def fft_bandpass_real(sig, f_lo, f_hi):
    ...
    F_filt=F*mask
    out=np.fft.irfft(F_filt, n=N)
    return out

FAST=(0.002,0.01)
SLOW=(0.0002,0.002)

u_fast=fft_bandpass_real(u_std, FAST[0], FAST[1])
u_slow=fft_bandpass_real(u_std, SLOW[0], SLOW[1])

z_fast=hilbert(u_fast)
theta_fast=np.mod(np.angle(z_fast), 2*np.pi)
logA_fast=np.log(np.abs(z_fast)+1e-12)

z_slow=hilbert(u_slow)
theta_slow=np.mod(np.angle(z_slow), 2*np.pi)
# omegas
def wrap_to_pi(x):
    return (x+np.pi)%(2*np.pi)-np.pi

omega_fast=wrap_to_pi(np.diff(theta_fast))
omega_slow=wrap_to_pi(np.diff(theta_slow))

# align to N_feat = N_kept-1 by dropping last of theta/logA and last R later
N_feat=N_kept-1
theta_fast=theta_fast[:-1]
logA_fast=logA_fast[:-1]
theta_slow=theta_slow[:-1]
# omega arrays already length N_feat
assert len(omega_fast)==N_feat and len(theta_fast)==N_feat
...
R=R[:-1]  # align
assert len(R)==N_feat
train_end_feat=int(train_frac*N_feat)
...
def wf_feats(theta, omega):
    return np.column_stack([omega, np.sin(theta)*omega, np.cos(theta)*omega])

def build_X(idx, include_fast=False, include_slow=False):
    # idx array of indices into [0..N_feat-1]
    idx=np.asarray(idx, dtype=int)
    r0=R[idx]
    r1=R[idx-1]
    r2=R[idx-2]
    b1=np.column_stack([r0,r1,r2,logA_fast[idx]])
    cols=[b1]
    if include_fast:
        cols.append(fourier_feats(theta_fast[idx], K))
        cols.append(wf_feats(theta_fast[idx], omega_fast[idx]))
    if include_slow:
        cols.append(fourier_feats(theta_slow[idx], K))
        cols.append(wf_feats(theta_slow[idx], omega_slow[idx]))
    X=np.column_stack(cols)
    return X

# standardize columns except pure sin/cos features (Fourier) -> these are in [-1,1].
# We'll standardize: in each model, columns corresponding to B1 and omega columns and interactions, logA etc. 
# We'll detect "bounded" columns by checking if all abs<=1+eps on TRAIN and TEST; but interaction terms can exceed.
# Use structural indices: for each model, we know which columns are Fourier (sin/cos) vs others.
def standardize_X(X_train, X_test, nonstd_mask):
    # nonstd_mask: boolean array len p, True if should standardize
    mu=X_train[:,nonstd_mask].mean(axis=0)
    sd=X_train[:,nonstd_mask].std(axis=0, ddof=0)+1e-12
    Xtr=X_train.copy()
    Xte=X_test.copy()
    Xtr[:,nonstd_mask]=(Xtr[:,nonstd_mask]-mu)/sd
    Xte[:,nonstd_mask]=(Xte[:,nonstd_mask]-mu)/sd
    return Xtr,Xte,mu,sd

def fit_eval_model(idx_train, idx_test, model_kind):
    # model_kind: "base","fast","both"
    inc_fast = model_kind in ("fast","both")
    inc_slow = model_kind=="both"
    Xtr=build_X(idx_train, include_fast=inc_fast, include_slow=inc_slow)
    Xte=build_X(idx_test, include_fast=inc_fast, include_slow=inc_slow)
    # non-angle scalars standardization:
    p=Xtr.shape[1]
    nonstd=np.zeros(p, dtype=bool)
    # B1 always first 4
    nonstd[:4]=True
    col=4
    if inc_fast:
        col += 2*K  # Fourier fast no standardize
        # WF fast 3 columns standardize (omega and interactions)
        nonstd[col:col+3]=True
        col += 3
    if inc_slow:
        col += 2*K  # Fourier slow
        nonstd[col:col+3]=True
        col += 3
    assert col==p
    Xtr_s,Xte_s,mu,sd=standardize_X(Xtr,Xte,nonstd)
    # labels
    # y depends on horizon handled outside; pass y arrays?

### Assistant (2026-02-22 10:19:06)

from sklearn.metrics import brier_score_loss, accuracy_score

def train_test_indices_for_h(h):
    # valid where i>=2 for lags, and i+h within same split
    train_valid = np.arange(2, train_end_feat-h, dtype=int)  # i+h <= train_end_feat-1
    test_start=train_end_feat
    test_end=N_feat-h
    test_valid = np.arange(max(2,test_start), test_end, dtype=int)
    return train_valid, test_valid

def labels_for_h(idx, h):
    return (R[idx+h]-R[idx] > 0).astype(int)

def fit_model(idx_train, y_train, model_kind):
    inc_fast = model_kind in ("fast","both")
    inc_slow = model_kind=="both"
    Xtr=build_X(idx_train, include_fast=inc_fast, include_slow=inc_slow)
    # standardize
    ...
    if inc_slow:
        ...
        col+=3
    Xtr_s=Xtr.copy()
    mu=Xtr[:,nonstd].mean(axis=0)
    sd=Xtr[:,nonstd].std(axis=0, ddof=0)+1e-12
    Xtr_s[:,nonstd]=(Xtr_s[:,nonstd]-mu)/sd
    clf=LogisticRegression(penalty="l2", C=1.0, solver="lbfgs", max_iter=2000)
    clf.fit(Xtr_s, y_train)
    return clf, nonstd, mu, sd

def predict_model(clf, nonstd, mu, sd, idx, model_kind, theta_fast_override=None, omega_fast_override=None, theta_slow_override=None, omega_slow_override=None):
    inc_fast = model_kind in ("fast","both")
    inc_slow = model_kind=="both"
    # build features with overrides for the rows in idx only
    idx=np.asarray(idx, dtype=int)
    r0=R[idx]; r1=R[idx-1]; r2=R[idx-2]
    b1=np.column_stack([r0,r1,r2,logA_fast[idx]])
    cols=[b1]
    if inc_fast:
        th = theta_fast[idx] if theta_fast_override is None else theta_fast_override
        om = omega_fast[idx] if omega_fast_override is None else omega_fast_override
        cols.append(fourier_feats(th, K))
        cols.append(wf_feats(th, om))
    if inc_slow:
        ths = theta_slow[idx] if theta_slow_override is None else theta_slow_override
        oms = omega_slow[idx] if omega_slow_override is None else omega_slow_override
        cols.append(fourier_feats(ths, K))
        cols.append(wf_feats(ths, oms))
    X=np.column_stack(cols)
    Xs=X.copy()
    Xs[:,nonstd]=(Xs[:,nonstd]-mu)/sd
    p=clf.predict_proba(Xs)[:,1]
    return p

def eval_metrics(y, p):
    # AUC can fail if only one class
    out={}
    ...
    out["Accuracy@0.5"]=float(np.mean((p>=0.5)==y))
    out["pos_rate_test"]=float(y.mean())
    return out

def block_permute(arrs, block_size, rng):
    # arrs list of arrays (n, ...) aligned first dimension n
    n=arrs[0].shape[0]
    blocks=[]
    start=0
    while start<n:
    ...
    for b in order:
        s,e=blocks[b]
        idx_perm.extend(range(s,e))
    idx_perm=np.array(idx_perm, dtype=int)
    return [a[idx_perm] for a in arrs]

def perm_test_auc(clf, nonstd, mu, sd, idx_test, y_test, model_kind, which="fast", block_size=512, Nperm=80, seed=12345):
    rng=np.random.default_rng(seed)
    aucs=[]
    # extract original phase state arrays for those idx
    if which=="fast":
        th=theta_fast[idx_test]
        om=omega_fast[idx_test]
        for r in range(Nperm):
            th_p, om_p = block_permute([th, om], block_size, rng)
            p=predict_model(clf, nonstd, mu, sd, idx_test, model_kind, theta_fast_override=th_p, omega_fast_override=om_p)
        ...
        th=theta_slow[idx_test]
        om=omega_slow[idx_test]
        for r in range(Nperm):
            th_p, om_p = block_permute([th, om], block_size, rng)
            p=predict_model(clf, nonstd, mu, sd, idx_test, model_kind, theta_slow_override=th_p, omega_slow_override=om_p)
            if len(np.unique(y_test))<2:
                auc=float("nan")
            else:
                auc=float(roc_auc_score(y_test,p))
...
# Run per horizon
H=[25,100]
block_size=512; Nperm=80; seed=12345
results={}
for h in H:
    idx_tr, idx_te = train_test_indices_for_h(h)
    y_tr=labels_for_h(idx_tr, h)
    y_te=labels_for_h(idx_te, h)
    # fit models
    per={}
    models={}
    for kind in ["base","fast","both"]:
        clf, nonstd, mu, sd = fit_model(idx_tr, y_tr, kind)
        p_te = predict_model(clf, nonstd, mu, sd, idx_te, kind)
        models[kind]=(clf,nonstd,mu,sd,p_te)
        per[kind]=eval_metrics(y_te, p_te)
    # permutation tests on test-valid rows
    # A) fast joint: use M_fast model, permute fast state
    clf,nonstd,mu,sd,_ = models["fast"]
    auc_true = per["fast"]["AUC"]
    aucs = perm_test_auc(clf,nonstd,mu,sd,idx_te,y_te,"fast",which="fast",block_size=block_size,Nperm=Nperm,seed=seed+17*h)
    auc_mean=float(np.nanmean(aucs)); auc_std=float(np.nanstd(aucs, ddof=0))
    # p-value one-sided: perm auc >= true? If true better, p = (1+count>=)/(1+N)
    if np.isnan(auc_true):
        pval=float("nan"); z=float("nan")
    else:
        pval=float((1.0+np.sum(aucs>=auc_true))/(1.0+len(aucs)))
        z=float((auc_true-auc_mean)/(auc_std+1e-12))
    fast_joint={"auc_true":auc_true,"auc_mean":auc_mean,"auc_std":auc_std,"p":pval,"z":z}
    # B) slow given fast: use M_both, permute slow state only
    clf,nonstd,mu,sd,_ = models["both"]
    auc_true = per["both"]["AUC"]
    aucs = perm_test_auc(clf,nonstd,mu,sd,idx_te,y_te,"both",which="slow",block_size=block_size,Nperm=Nperm,seed=seed+31*h)
    auc_mean=float(np.nanmean(aucs)); auc_std=float(np.nanstd(aucs, ddof=0))
    if np.isnan(auc_true):
        pval=float("nan"); z=float("nan")
    else:
        pval=float((1.0+np.sum(aucs>=auc_true))/(1.0+len(aucs)))
        z=float((auc_true-auc_mean)/(auc_std+1e-12))
    slow_given_fast={"auc_true":auc_true,"auc_mean":auc_mean,"auc_std":auc_std,"p":pval,"z":z}
    results[h]={"metrics":{"M_base":per["base"],"M_fast":per["fast"],"M_both":per["both"]},
                "perm_tests":{"fast_joint":fast_joint,"slow_given_fast":slow_given_fast}}
results, dt, N_kept, N_feat, train_end_feat

### Tool (2026-02-22 10:19:44)

({25: {'metrics': {'M_base': {'AUC': 0.7942130334385347,
     'Brier': 0.18475418147233527,
     'Accuracy@0.5': 0.7104685212298683,
     'pos_rate_test': 0.5065885797950219},
    'M_fast': {'AUC': 0.8490863953877292,
     'Brier': 0.15870605322971718,
     'Accuracy@0.5': 0.7736090775988287,
     'pos_rate_test': 0.5065885797950219},
    'M_both': {'AUC': 0.8558294186634877,
     'Brier': 0.1549923155643019,
     'Accuracy@0.5': 0.7726939970717424,
     'pos_rate_test': 0.5065885797950219}},
   'perm_tests': {'fast_joint': {'auc_true': 0.8490863953877292,
     'auc_mean': 0.7629762779449537,
     'auc_std': 0.013108362666243536,
     'p': 0.012345679012345678,
     'z': 6.569097882678814},
    'slow_given_fast': {'auc_true': 0.8558294186634877,
     'auc_mean': 0.8324021162801667,
     'auc_std': 0.006905038808501981,
     'p': 0.012345679012345678,
     'z': 3.392783593204843}}},
  100: {'metrics': {'M_base': {'AUC': 0.8363084096316442,
     'Brier': 0.1656282517204896,
     'Accuracy@0.5': 0.7487474485062163,
     'pos_rate_test': 0.5006494711449249},
    'M_fast': {'AUC': 0.8427630855838545,
     'Brier': 0.16234260670206013,
     'Accuracy@0.5': 0.7580256077194285,
     'pos_rate_test': 0.5006494711449249},
    'M_both': {'AUC': 0.8483642176554801,
     'Brier': 0.15968329056507805,
     'Accuracy@0.5': 0.762293560957506,
     'pos_rate_test': 0.5006494711449249}},
   'perm_tests': {'fast_joint': {'auc_true': 0.8427630855838545,
     'auc_mean': 0.8337202309871277,
     'auc_std': 0.003921128508513847,
     'p': 0.012345679012345678,
     'z': 2.306186745674393},
    'slow_given_fast': {'auc_true': 0.8483642176554801,
