...
- **Conversation ID:** `699bd7e2-0b34-832b-a12b-d20f73f9474a`
- **Date:** 2026-02-22

---

### User (2026-02-22 23:30:30)
...
### Assistant (2026-02-22 23:30:33)

import hashlib, numpy as np, os, json, math, statistics
path="/mnt/data/A4_FEATURECACHE_h25_AR80_AR40_theta_omega_masks_v1.npz"
os.path.exists(path), os.path.getsize(path)

### Tool (2026-02-22 23:30:55)

(True, 1422464)

### Assistant (2026-02-22 23:30:56)
...
### Tool (2026-02-22 23:30:58)

'340f3ce5b561fe00e872ef98991390185f600f96'

### Assistant (2026-02-22 23:30:59)

npz=np.load(path, allow_pickle=False)
list(npz.keys())[:50], len(npz.keys())

### Tool (2026-02-22 23:31:02)
...
R_sha1=hashlib.sha1(R.tobytes()).hexdigest()
dt, N_kept, train_end, theta.shape, R.shape, theta_sha1, R_sha1

### Tool (2026-02-22 23:31:07)
...
### Assistant (2026-02-22 23:31:10)

X80=npz['X80']
mask_tr=npz['mask_train_lag79'].astype(bool)
mask_te=npz['mask_test_lag79'].astype(bool)
X80.shape, mask_tr.shape, mask_te.sum(), mask_tr.sum()

### Tool (2026-02-22 23:31:13)

((36592, 80), (36592,), 10953, 25510)

### Assistant (2026-02-22 23:31:14)

h=25
idx=np.arange(N_kept)
valid_future = idx + h < N_kept
y = (R[idx+h] - R[idx] > 0).astype(np.int8)
# but y undefined for last h; set 0 but mask should exclude
y_full = np.zeros(N_kept, dtype=np.int8)
y_full[valid_future]=y
# apply train/test masks
train_mask = mask_tr & valid_future
test_mask = mask_te & valid_future
n_train=int(train_mask.sum()); n_test=int(test_mask.sum())
pos_rate=float(y_full[train_mask].mean())
n_train, n_test, pos_rate

### Tool (2026-02-22 23:31:21)

---------------------------------------------------------------------------
IndexError                                Traceback (most recent call last)
Cell In[6], line 4
      2 idx=np.arange(N_kept)
      3 valid_future = idx + h < N_kept
----> 4 y = (R[idx+h] - R[idx] > 0).astype(np.int8)
      5 # but y undefined for last h; set 0 but mask should exclude
      6 y_full = np.zeros(N_kept, dtype=np.int8)

IndexError: index 36592 is out of bounds for axis 0 with size 36592

### Assistant (2026-02-22 23:31:22)

h=25
N=N_kept
y_full=np.zeros(N, dtype=np.int8)
y_full[:N-h] = (R[h:] - R[:-h] > 0).astype(np.int8)
valid_future = np.zeros(N, dtype=bool)
valid_future[:N-h]=True
train_mask = mask_tr & valid_future
test_mask = mask_te & valid_future
n_train=int(train_mask.sum()); n_test=int(test_mask.sum())
pos_rate=float(y_full[train_mask].mean())
...
### Tool (2026-02-22 23:31:25)

(25510, 10953, 0.493375147001176)

### Assistant (2026-02-22 23:31:26)

mu=npz['mu_X80_train']; sd=npz['sd_X80_train']
mu.shape, sd.shape, float(sd.min())

### Tool (2026-02-22 23:31:30)

((80,), (80,), 0.49281391501426697)

### Assistant (2026-02-22 23:31:31)

Xb = (X80 - mu)/sd
# add intercept
Xb_i = np.concatenate([np.ones((N,1)), Xb], axis=1)
Xb_tr = Xb_i[train_mask]
y_tr = y_full[train_mask]
Xb_te = Xb_i[test_mask]
y_te = y_full[test_mask]
Xb_tr.shape, y_tr.shape

### Tool (2026-02-22 23:31:34)

((25510, 81), (25510,))

### Assistant (2026-02-22 23:31:36)

import numpy.linalg as la, numpy as np
def fit_logit_irls(X, y, ridge=1e-6, max_iter=50, tol=1e-8):
    n,p=X.shape
    beta=np.zeros(p)
    for it in range(max_iter):
        ...
        W = p_hat*(1-p_hat)
        # gradient
        g = X.T@(y - p_hat) - ridge*beta
        # Hessian (negative): X^T W X + ridge I
        # build weighted X
        Xw = X * np.sqrt(W)[:,None]
        H = Xw.T@Xw + ridge*np.eye(p)
        step = la.solve(H, g)
        beta_new = beta + step
        if la.norm(step) < tol*(1+la.norm(beta)):
            beta=beta_new
            break
        beta=beta_new
    return beta, it+1

beta, n_iter = fit_logit_irls(Xb_tr, y_tr, ridge=1e-6, max_iter=100, tol=1e-9)
n_iter, beta[:5], float(la.norm(beta))

### Tool (2026-02-22 23:31:47)

(6,
 array([-0.03678931, -0.95938835,  0.0411499 ,  0.07534609,  0.0584735 ]),
 1.359418127366346)

### Assistant (2026-02-22 23:31:53)

from sklearn.metrics import roc_auc_score
def predict_prob(X,beta):
    z=X@beta
    return 1/(1+np.exp(-np.clip(z,-35,35)))
p_tr = predict_prob(Xb_tr, beta)
p_te = predict_prob(Xb_te, beta)
auc_tr=float(roc_auc_score(y_tr, p_tr))
auc_te=float(roc_auc_score(y_te, p_te))
auc_tr, auc_te

### Tool (2026-02-22 23:31:55)

(0.7752511068017929, 0.7849366889308109)

### Assistant (2026-02-22 23:31:58)

theta_full=theta
S_full=np.column_stack([np.sin(theta_full), np.cos(theta_full)])  # N x 2
Xn_tr = S_full[train_mask]
# baseline matrices
p = p_tr
W = p*(1-p)
r = y_tr - p
...
# compute A etc
ridge=1e-6
Xb_w = Xb_mat * np.sqrt(W)[:,None]
A = Xb_w.T@Xb_w + ridge*np.eye(Xb_mat.shape[1])
# Precompute solves
A_inv = la.inv(A)
# components
B = (Xb_mat*W[:,None]).T @ Xn_tr
C = (Xn_tr*W[:,None]).T @ Xn_tr
xbtr = Xb_mat.T @ r
u = Xn_tr.T @ r - B.T @ (A_inv @ xbtr)
J = C - B.T @ (A_inv @ B)
# ridge J
J = J + ridge*np.eye(2)
T = float(u.T @ la.solve(J, u))
T

### Tool (2026-02-22 23:32:07)

873.8105617565056

### Assistant (2026-02-22 23:32:09)

from scipy.stats import chi2
pval=float(chi2.sf(T, df=2))
pval

### Tool (2026-02-22 23:32:11)

1.796583472798499e-190

### Assistant (2026-02-22 23:32:12)
...
def sample_shift():
    while True:
        ...
        # compute min(|s|, N-|s|)
        d=min(s, N-s)
        if d>=min_shift and s!=0:
            return s
shifts=[sample_shift() for _ in range(replicates)]
# compute T for each
T_null=np.empty(replicates, dtype=float)
for k,s in enumerate(shifts):
    theta_shift=np.roll(theta_full, s)
    Xn_shift = np.column_stack([np.sin(theta_shift), np.cos(theta_shift)])[train_mask]
    Bk = (Xb_mat*W[:,None]).T @ Xn_shift
    Ck = (Xn_shift*W[:,None]).T @ Xn_shift
    uk = Xn_shift.T @ r - Bk.T @ (A_inv @ xbtr)
    Jk = Ck - Bk.T @ (A_inv @ Bk)
    Jk = Jk + ridge*np.eye(2)
    T_null[k]= float(uk.T @ la.solve(Jk, uk))
T_null.mean(), T_null.std(), np.quantile(T_null,[0.95,0.99])[:]

### Tool (2026-02-22 23:32:22)

---------------------------------------------------------------------------
KeyboardInterrupt                         Traceback (most recent call last)
Cell In[14], line 23
     21     uk = Xn_shift.T @ r - Bk.T @ (A_inv @ xbtr)
     22     Jk = Ck - Bk.T @ (A_inv @ Bk)
---> 23     Jk = Jk + ridge*np.eye(2)
     24     T_null[k]= float(uk.T @ la.solve(Jk, uk))
     25 T_null.mean(), T_null.std(), np.quantile(T_null,[0.95,0.99])[:]
...
### Assistant (2026-02-22 23:33:30)

# Recompute necessary objects quickly (some may still exist but ensure)
S_full = np.column_stack([np.sin(theta_full), np.cos(theta_full)])
S_train = S_full[train_mask]
sqrtW = np.sqrt(W)
XbW_T = (Xb_mat*W[:,None]).T   # 81 x n_train
A_inv = A_inv  # from before
xbtr = xbtr
ridge=1e-6

def score_T_for_Xn(Xn):
    Bk = XbW_T @ Xn                      # 81x2
    Ck = (Xn*sqrtW[:,None]).T @ (Xn*sqrtW[:,None])  # 2x2
    uk = Xn.T @ r - Bk.T @ (A_inv @ xbtr)           # 2
    Jk = Ck - Bk.T @ (A_inv @ Bk)
    Jk = Jk + ridge*np.eye(2)
    return float(uk.T @ la.solve(Jk, uk))

# compute real again
T_real = score_T_for_Xn(S_train)
T_real

### Tool (2026-02-22 23:33:39)

873.8105617565056

### Assistant (2026-02-22 23:33:40)

replicates=500
min_shift=50
rng=npr.default_rng(12345)
N=N_kept
# sample shifts vectorized by rejection
shifts=[]
while len(shifts)<replicates:
    cand = rng.integers(0,N,size=replicates*2)
    for s in cand:
        s=int(s)
        if s==0: 
            continue
...
S2 = S_full  # N x2
for k,s in enumerate(shifts):
    Xn_shift = np.roll(S2, s, axis=0)[train_mask]
    T_null[k]=score_T_for_Xn(Xn_shift)

float(T_null.mean()), float(T_null.std()), [float(x) for x in np.quantile(T_null,[0.95,0.99])]

### Tool (2026-02-22 23:33:45)

(47.975891291237595,
 208.29116006797025,
 [117.53196486480446, 282.750792287296])

### Assistant (2026-02-22 23:34:38)

emp_p = float((np.sum(T_null >= T_real)+1)/(replicates+1))
emp_p, int(np.sum(T_null>=T_real))

### Tool (2026-02-22 23:34:41)

(0.003992015968063872, 1)

### Assistant (2026-02-22 23:34:42)
