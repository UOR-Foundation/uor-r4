# A4 Canonical Cache Creation

- **Conversation ID:** `699baac8-4108-8326-9695-e854bf754f34`
- **Date:** 2026-02-22

---

### User (2026-02-22 20:18:08)
...
### Assistant (2026-02-22 20:18:10)

import numpy as np, os, hashlib, math, json, textwrap, datetime, pathlib, sys, importlib
base="/mnt/data"
cheb_path=os.path.join(base,"chebyshev_psi.npz")
primes_path=os.path.join(base,"primes.npz")
(os.path.exists(cheb_path), os.path.exists(primes_path), os.listdir(base)[:5])

### Tool (2026-02-22 20:18:42)
...
### Assistant (2026-02-22 20:18:44)

import numpy as np, hashlib, os, math, datetime, json
def sha1_float64(arr: np.ndarray)->str:
...
t=np.linspace(tmin,tmax,L, dtype=np.float64)
dt=float(t[1]-t[0])
u_interp=np.interp(t, t_raw, psi_minus_x.astype(np.float64))
x_t=np.exp(t)
kept=(x_t>=x_min) & (x_t<=x_max)
x_kept=x_t[kept].astype(np.float64)
u_raw=u_interp[kept].astype(np.float64)
N_kept=int(x_kept.size)
train_end=int(math.floor(train_frac*N_kept))
u_cma=causal_moving_average(u_raw, W)
u=u_raw-u_cma
mu=float(u[:train_end].mean())
sig=float(np.sqrt(np.mean((u[:train_end]-mu)**2)))
u_z=(u-mu)/sig
# bandpass via FFT
U=np.fft.fft(u_z)
freq=np.fft.fftfreq(N_kept, d=1.0)
mask=(np.abs(freq)>=bp_lo) & (np.abs(freq)<=bp_hi)
U_f=U*mask
u_bp=np.fft.ifft(U_f).real.astype(np.float64)

hash_x=sha1_float64(x_kept)
hash_u=sha1_float64(u_bp)
dt, N_kept, train_end, hash_x, hash_u

### Tool (2026-02-22 20:18:55)
...
### Assistant (2026-02-22 20:18:56)

expected_u="b16b32cb5d4e63be1c61b88fcf4711d0656da14e"
def build_u_bp(cma_mode):
    if cma_mode==1:
        u_cma=causal_moving_average(u_raw,W)
    elif cma_mode==2:
        # denom fixed W, missing prehistory treated as 0
        x=u_raw
        cs=np.cumsum(x,dtype=np.float64)
        out=np.empty_like(x)
        ...
        # denom fixed W, missing prehistory padded with x[0]
        x=u_raw
        cs=np.cumsum(x,dtype=np.float64)
        out=np.empty_like(x)
        x0=float(x[0])
        ...
        # using np.convolve with ones/W and causal (full) then take first n with padding zeros
        x=u_raw
        kernel=np.ones(W,dtype=np.float64)/W
        y=np.convolve(x,kernel,mode='full')[:x.size]
        u_cma=y
    else:
        raise ValueError
    u=u_raw-u_cma
    ...
    u_z=(u-mu)/sig
    U=np.fft.fft(u_z)
    freq=np.fft.fftfreq(u_z.size,d=1.0)
    mask=(np.abs(freq)>=bp_lo) & (np.abs(freq)<=bp_hi)
    u_bp=np.fft.ifft(U*mask).real.astype(np.float64)
    return sha1_float64(u_bp), (mu,sig,float(u_bp[:train_end].mean()), float(u_bp[:train_end].std(ddof=0)))
for m in [1,2,3,4]:
    h,stats=build_u_bp(m)
    print(m,h,h==expected_u,stats[:2])

### Tool (2026-02-22 20:19:18)

1 d93b2b1abdfdcff256f1b93b3a4070f309c76a6b False (7.832216551321668, 185.710181714036)
2 03bb234de0140c64ee83897db947047524522ca1 False (8.497411519222634, 185.71427188797827)
3 f0a7b30c3dcb4a7316958308cca936d4d33ba317 False (6.630953832344691, 186.05621413750475)
4 2985c2dfe7cfc68034e79c1ab86afe46946cf850 False (8.49741151922263, 185.71427188797827)

### Assistant (2026-02-22 20:19:22)

def build_u_bp_detrend_full():
    u_full=u_interp.astype(np.float64)
    # causal moving average on full length L
    cs=np.cumsum(u_full,dtype=np.float64)
    out=np.empty_like(u_full)
    for i in range(u_full.size):
        j0=i-W+1
        if j0<=0:
            s=cs[i]; denom=i+1
        else:
            s=cs[i]-cs[j0-1]; denom=W
        out[i]=s/denom
    u_full_d=u_full-out
    # subset kept
    u=u_full_d[kept]
    ...
    mask=(np.abs(freq)>=bp_lo)&(np.abs(freq)<=bp_hi)
    u_bp=np.fft.ifft(U*mask).real.astype(np.float64)
    return sha1_float64(u_bp), u_bp
h,_=build_u_bp_detrend_full()
h, h==expected_u

### Tool (2026-02-22 20:19:38)

('1867946872cb6ff4c6cff7bb5b0359f0ad720d1f', False)

### Assistant (2026-02-22 20:19:39)

import glob, os
glob.glob("/mnt/data/*A4_CANONICAL_CACHE*")

### Tool (2026-02-22 20:19:43)

[]

### Assistant (2026-02-22 20:19:44)
...
def build_u_bp_rfft():
    ...
    u_z=(u-mu)/sig
    Ur=np.fft.rfft(u_z)
    fr=np.fft.rfftfreq(u_z.size,d=1.0)
    mask=(fr>=bp_lo)&(fr<=bp_hi)
    Ur_f=Ur*mask
    u_bp=np.fft.irfft(Ur_f, n=u_z.size).astype(np.float64)
    return sha1_float64(u_bp), u_bp[:5]
h,head=build_u_bp_rfft()
h, h==expected_u, head

### Tool (2026-02-22 20:19:57)

('b16b32cb5d4e63be1c61b88fcf4711d0656da14e',
 True,
 array([-0.39578496, -0.42567722, -0.45479376, -0.48307517, -0.51046521]))

### Assistant (2026-02-22 20:19:58)

# rebuild u_bp with rfft method (canonical for matching)
u_cma=causal_moving_average(u_raw,W)
u=u_raw-u_cma
mu=float(u[:train_end].mean())
sig=float(np.sqrt(np.mean((u[:train_end]-mu)**2)))
u_z=(u-mu)/sig
Ur=np.fft.rfft(u_z)
fr=np.fft.rfftfreq(N_kept,d=1.0)
mask=(fr>=bp_lo)&(fr<=bp_hi)
Ur_f=Ur*mask
u_bp=np.fft.irfft(Ur_f, n=N_kept).astype(np.float64)
hash_u=sha1_float64(u_bp)
hash_u

### Tool (2026-02-22 20:20:04)

'b16b32cb5d4e63be1c61b88fcf4711d0656da14e'

### Assistant (2026-02-22 20:20:06)

expected_theta="8b33c57df436d37179b13b1899bf86a723f0e98a"
def hilbert_fft(x):
    x=np.asarray(x,dtype=np.float64)
    n=x.size
    X=np.fft.fft(x)
...
theta_mod_A=np.mod(theta, 2*np.pi).astype(np.float64)
variant_hashes["A"]=sha1_float64(theta_mod_A)
variant_hashes["A"], variant_hashes["A"]==expected_theta

### Tool (2026-02-22 20:20:11)

('f535f692ba9acd1df86681903e521450b39e0c52', False)

### Assistant (2026-02-22 20:20:13)

import numpy as np, math
theta_mod_candidates={}
theta_mod_candidates["A"]=theta_mod_A
# B unwrap then mod
theta_unw=np.unwrap(theta)
theta_mod_candidates["B"]=np.mod(theta_unw,2*np.pi).astype(np.float64)
# C atan2
theta_at=np.arctan2(z.imag, z.real)
theta_mod_candidates["C"]=np.mod(theta_at,2*np.pi).astype(np.float64)
# D scipy.signal.hilbert if available
try:
    import scipy.signal
    z2=scipy.signal.hilbert(u_bp.astype(np.float64))
    theta2=np.angle(z2)
    theta_mod_candidates["D"]=np.mod(theta2,2*np.pi).astype(np.float64)
except Exception as e:
    theta_mod_candidates["D"]=None
# E sign flip
theta_mod_candidates["E"]=np.mod(-theta,2*np.pi).astype(np.float64)

hashes={}
for k,v in theta_mod_candidates.items():
    if v is None: 
        hashes[k]=None
    else:
        hashes[k]=sha1_float64(v)
hashes

### Tool (2026-02-22 20:20:19)

{'A': 'f535f692ba9acd1df86681903e521450b39e0c52',
 'B': '7cc1d91e3b922feb4b46a55c8e635a5071c0089e',
 'C': 'f535f692ba9acd1df86681903e521450b39e0c52',
 'D': '8b33c57df436d37179b13b1899bf86a723f0e98a',
 'E': 'c402891cc197513fc3564f0c6b97372eaaf5f92c'}

### Assistant (2026-02-22 20:20:22)

expected_R="5753ee7d59c7d155c4a1b9b5fba8912d557598f2"
...
lo=np.searchsorted(pr, xv, side='left')
hi=np.searchsorted(pr, ends, side='left')
P=(hi-lo).astype(np.float64)
E = xv*(expDelta-1.0)/np.log(xv)
R = (P - E)/np.sqrt(E)
hash_R=sha1_float64(R)
hash_R, hash_R==expected_R, R[:3]

### Tool (2026-02-22 20:20:28)

('5753ee7d59c7d155c4a1b9b5fba8912d557598f2',
 True,
 array([-0.6745253 , -0.56851728, -0.56953462]))

### Assistant (2026-02-22 20:20:30)
...
theta_mod=theta_mod_candidates["D"]
# save npz with metadata as small arrays
np.savez_compressed(
    cache_path,
    ...
    bandpass_lo=np.array([bp_lo],dtype=np.float64),
    bandpass_hi=np.array([bp_hi],dtype=np.float64),
)

history=textwrap.dedent("""\
# A4 Canonical Cache + Fingerprint Lock (2026-02-22)

This note exists to stop future humans (and their obedient compute workers) from silently drifting the preprocessing pipeline.

## What happened (timeline bullets)

- Reconfirmed canonical substrate: ψ(x)−x on step-100 x-grid, interpolated to uniform log grid, restricted to x∈[1e5,9e6], L=131072, causal detrend W=2000, TRAIN-only standardization, FFT bandpass [0.002,0.01], then Hilbert phase.
- Rotation/transport scans: repeated evidence that phase transport matters; naive stationary phase maps drift across scale (do transport before declaring “structure”).
- Two-curve gating failing: 2D phase-only dynamic models (θ, ω) do not forward-simulate θ reliably; θ is predictive but not Markovian in (θ,ω) alone.
- MR interaction lift: interactions (phase × pressure-like terms / residual context) show lift over single-axis effects, but remain sensitive to label stickiness.
- Transport lift at h=25 only: transport alignment improves short-horizon behavior most; longer horizons often dominated by persistence unless using de-sticky targets.
- τ/125 commensurability negative: the “125-sample implied period” does not by itself explain the optimal predictive shift; commensurability tests did not validate a simple fraction-of-period law.
- Fingerprint drift incident: u_bp hash mismatch was traced to FFT masking implementation details. Matching required rFFT/irFFT masking on nonnegative frequencies (not full FFT with |f| mask).
- Theta drift incident: Hilbert phase hash mismatch was traced to analytic-signal implementation. Matching required scipy.signal.hilbert (double precision), then angle→mod 2π.

## Leader rules going forward

- **Always load the cache file** for A4 experiments.
- **Always hash-gate** x_kept, u_bp, theta_mod, R before any modeling.
- **One experiment per cycle** unless it is a strict falsification of an established pillar.
- If hashes mismatch: stop immediately, do not “patch” by intuition. Identify the exact numerical source (interp, detrend, standardization, FFT mask, Hilbert) and fix that only.
...
(os.path.exists(cache_path), os.path.getsize(cache_path), os.path.exists(history_path), os.path.getsize(history_path))

### Tool (2026-02-22 20:20:52)

(True, 1097790, True, 2219)

### Assistant (2026-02-22 20:20:54)

out = {
 "status":"SUCCESS",
 "shape":{"dt":dt,"N_kept":N_kept,"train_end":train_end},
 "hashes":{"x_kept":hash_x,"u_bp":hash_u,"theta_mod":hashes["D"],"R":hash_R},
 "theta_variant_used":"D",
 "cache_file":cache_path,
 "history_file":history_path,
 "notes":[
   "Step1 fingerprints passed: x_kept and u_bp match expected.",
   "u_bp required rFFT/irFFT masking (nonnegative frequencies) to match expected hash.",
   "theta_mod matched only via scipy.signal.hilbert (variant D).",
   "R fingerprint passed via searchsorted prime counts in multiplicative window."
 ]
}
json.dumps(out, indent=2)

### Tool (2026-02-22 20:20:59)
