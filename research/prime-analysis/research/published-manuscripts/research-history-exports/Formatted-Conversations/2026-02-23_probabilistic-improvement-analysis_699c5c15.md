# Probabilistic Improvement Analysis

- **Conversation ID:** `699c5c15-91b4-8333-bda5-f8680a039966`
- **Date:** 2026-02-23

---

### User (2026-02-23 08:54:33)
...
### Assistant (2026-02-23 08:54:35)

import numpy as np, hashlib, math, os, json, sys
from pathlib import Path

base = Path("/mnt/data")
feature_path = base/"A4_FEATURECACHE_h25_AR80_AR40_theta_omega_masks_v1.npz"
cheb_path = base/"chebyshev_psi.npz"
canon_path = base/"A4_CANONICAL_CACHE_L131072_x1e5_9e6_W2000_bp002_001_D001_seed12345.npz"
feature_path.exists(), cheb_path.exists(), canon_path.exists()

### Tool (2026-02-23 08:55:17)

(True, True, True)

### Assistant (2026-02-23 08:55:20)

canon = np.load(canon_path, allow_pickle=True)
canon.files[:20], {k:canon[k] for k in ['dt','train_end'] if k in canon.files}

### Tool (2026-02-23 08:55:24)

(['x_kept', 'u_bp', 'theta_mod', 'R', 'dt', 'train_end', 'params', 'hashes'],
 {'dt': array([0.00012297]), 'train_end': array([25614])})

### Assistant (2026-02-23 08:55:26)

dt_canon = float(canon['dt'].ravel()[0])
train_end_canon = int(canon['train_end'].ravel()[0])
x_kept_canon = canon['x_kept']
N_kept_canon = len(x_kept_canon)
dt_canon, train_end_canon, N_kept_canon, x_kept_canon[:3], x_kept_canon[-3:]

### Tool (2026-02-23 08:55:37)
...
### Assistant (2026-02-23 08:55:39)

# load feature cache
feat = np.load(feature_path, allow_pickle=True)
feat.files[:30]

### Tool (2026-02-23 08:55:41)
...
### Assistant (2026-02-23 08:55:44)

dt_feat = float(feat['dt'])
N_kept_feat = int(feat['N_kept'])
train_end_feat = int(feat['train_end'])
dt_feat, N_kept_feat, train_end_feat

### Tool (2026-02-23 08:55:47)

(0.00012297217348543395, 36592, 25614)

### Assistant (2026-02-23 08:55:50)

canon_hashes = canon['hashes'].item()
canon_hashes.keys(), canon_hashes.get('x_kept')

### Tool (2026-02-23 08:55:58)

---------------------------------------------------------------------------
AttributeError                            Traceback (most recent call last)
Cell In[6], line 2
      1 canon_hashes = canon['hashes'].item()
----> 2 canon_hashes.keys(), canon_hashes.get('x_kept')

AttributeError: 'str' object has no attribute 'keys'

### Assistant (2026-02-23 08:56:00)

canon_hashes = canon['hashes']
type(canon_hashes), canon_hashes.shape, canon_hashes[:10]

### Tool (2026-02-23 08:56:01)
...
canon=np.load(canon_path, allow_pickle=True)
feat=np.load(feature_path, allow_pickle=True)
dt_canon=float(canon['dt'].ravel()[0]); train_end_canon=int(canon['train_end'].ravel()[0]); x_kept_canon=canon['x_kept']; N_kept_canon=len(x_kept_canon)
dt_feat=float(feat['dt']); N_kept_feat=int(feat['N_kept']); train_end_feat=int(feat['train_end'])
dt_canon, dt_feat, N_kept_canon, N_kept_feat, train_end_canon, train_end_feat

### Tool (2026-02-23 08:56:18)

(0.00012297217348543395, 0.00012297217348543395, 36592, 36592, 25614, 25614)

### Assistant (2026-02-23 08:56:20)

canon_hashes = canon['hashes']
canon_hashes, type(canon_hashes)

### Tool (2026-02-23 08:56:21)
...
### Assistant (2026-02-23 08:56:23)

canon_hashes_dict=json.loads(str(canon_hashes.ravel()[0]))
canon_hashes_dict

### Tool (2026-02-23 08:56:25)
...
### Assistant (2026-02-23 08:56:26)

import scipy, scipy.signal, scipy.interpolate
scipy.__version__

### Tool (2026-02-23 08:56:32)

'1.14.1'

### Assistant (2026-02-23 08:56:35)

cheb=np.load(cheb_path, allow_pickle=True)
x_grid=cheb['x_grid']; psi_minus_x=cheb['psi_minus_x']
len(x_grid), x_grid[:3], x_grid[-3:], psi_minus_x[:3]

### Tool (2026-02-23 08:56:37)
...
### Assistant (2026-02-23 08:56:39)

# Step: uniform t grid length 131072 over [min(log(x_grid)), max(log(x_grid))]
L=131072
...
interp = scipy.interpolate.interp1d(t_raw, psi_minus_x.astype(np.float64), kind='linear', bounds_error=False, fill_value="extrapolate")
u_full = interp(t_uni)
x_full = np.exp(t_uni)
# restrict to x in [1e5, 9e6]
x_min=1e5; x_max=9e6
kept = np.where((x_full>=x_min) & (x_full<=x_max))[0]
x_kept = x_full[kept]
u_raw = u_full[kept]
N_kept=len(kept)
dt, N_kept, x_kept[:3], x_kept[-3:]

### Tool (2026-02-23 08:56:49)
...
### Assistant (2026-02-23 08:56:51)

def sha1_of_float_array(arr: np.ndarray) -> str:
    # stable byte representation: little-endian float64
    a = np.asarray(arr, dtype=np.float64)
    return hashlib.sha1(a.tobytes()).hexdigest()

sha1_x = sha1_of_float_array(x_kept)
sha1_x, canon_hashes_dict['x_kept']

### Tool (2026-02-23 08:56:56)

('c455fedeff29bb2d0b2c81864970c09638720baf',
 'c455fedeff29bb2d0b2c81864970c09638720baf')

### Assistant (2026-02-23 08:56:57)
...
U = np.fft.rfft(u_z)
freq = np.fft.rfftfreq(len(u_z), d=1.0)  # cycles per sample with sample spacing 1
mask = (freq>=bp_lo) & (freq<=bp_hi)
U_bp = U*mask
u_bp = np.fft.irfft(U_bp, n=len(u_z))
theta_new = np.angle(scipy.signal.hilbert(u_bp)) % (2*np.pi)
theta_sha1 = sha1_of_float_array(theta_new)
theta_sha1[:10], len(theta_new), theta_new[:3]

### Tool (2026-02-23 08:57:13)

('c888f75abc', 36592, array([1.32477148, 1.3581499 , 1.39142863]))

### Assistant (2026-02-23 08:57:15)

h=50
idx=np.arange(N_kept)
train_valid = (idx>=79) & (idx<train_end) & (idx+h<train_end)
test_valid = (idx>=79) & (idx>=train_end) & (idx+h<N_kept)
train_valid.sum(), test_valid.sum(), N_kept, train_end

### Tool (2026-02-23 08:57:20)

(25485, 10928, 36592, 25614)

### Assistant (2026-02-23 08:57:22)
...
### Tool (2026-02-23 08:57:27)

((36592,), (36592, 80), (80,), (80,))

### Assistant (2026-02-23 08:57:28)

# verify R hash matches canonical
sha1_R = sha1_of_float_array(R)
sha1_R, canon_hashes_dict['R']

### Tool (2026-02-23 08:57:31)

('5753ee7d59c7d155c4a1b9b5fba8912d557598f2',
 '5753ee7d59c7d155c4a1b9b5fba8912d557598f2')

### Assistant (2026-02-23 08:57:34)

sd_safe = np.where(sd_X80==0, 1.0, sd_X80)
X80z = (X80 - mu_X80)/sd_safe
X80z_train = X80z[train_valid]
X80z_test = X80z[test_valid]
y = (R[idx+h] - R[idx] > 0).astype(np.int8)
y_train = y[train_valid]
y_test = y[test_valid]
y_train.mean(), y_test.mean()

### Tool (2026-02-23 08:57:39)

---------------------------------------------------------------------------
IndexError                                Traceback (most recent call last)
Cell In[12], line 5
      3 X80z_train = X80z[train_valid]
      4 X80z_test = X80z[test_valid]
----> 5 y = (R[idx+h] - R[idx] > 0).astype(np.int8)
      6 y_train = y[train_valid]
      7 y_test = y[test_valid]

IndexError: index 36592 is out of bounds for axis 0 with size 36592

### Assistant (2026-02-23 08:57:40)

idx=np.arange(N_kept)
y_full = np.zeros(N_kept, dtype=np.int8)
valid_for_y = idx + h < N_kept
y_full[valid_for_y] = (R[idx[valid_for_y]+h] - R[idx[valid_for_y]] > 0).astype(np.int8)
y_train = y_full[train_valid]
y_test = y_full[test_valid]
y_train.mean(), y_test.mean(), y_train.size, y_test.size

### Tool (2026-02-23 08:57:46)

(0.49668432411222285, 0.5058565153733529, 25485, 10928)

### Assistant (2026-02-23 08:57:48)

def logistic_irls(X, y, ridge=1e-6, max_iter=50, tol=1e-8):
    X = np.asarray(X, dtype=np.float64)
    ...
    for it in range(max_iter):
        z = X @ w
        # stable sigmoid
        p_hat = 1/(1+np.exp(-np.clip(z, -50, 50)))
        W = p_hat*(1-p_hat)
        # avoid zero weights
        W = np.maximum(W, 1e-9)
        # working response
        ...
        grad = X.T @ r - ridge*w
        # Hessian: -X^T diag(W) X - ridge I
        # Solve (X^T diag(W) X + ridge I) delta = grad
        Xw = X * W[:,None]**0.5
        H = Xw.T @ Xw + ridge*np.eye(p)
        try:
...
# baseline design matrix with intercept
X0_train = np.hstack([np.ones((X80z_train.shape[0],1)), X80z_train])
X0_test = np.hstack([np.ones((X80z_test.shape[0],1)), X80z_test])

w0 = logistic_irls(X0_train, y_train, ridge=1e-6, max_iter=100)
p0 = sigmoid(X0_test @ w0)

# full with phase sin/cos
theta_train = theta_new[train_valid]
theta_test = theta_new[test_valid]
phase_train = np.column_stack([np.sin(theta_train), np.cos(theta_train)])
phase_test = np.column_stack([np.sin(theta_test), np.cos(theta_test)])
X1_train = np.hstack([np.ones((X80z_train.shape[0],1)), X80z_train, phase_train])
X1_test = np.hstack([np.ones((X80z_test.shape[0],1)), X80z_test, phase_test])
w1 = logistic_irls(X1_train, y_train, ridge=1e-6, max_iter=100)
p1 = sigmoid(X1_test @ w1)

p0.min(), p0.max(), p1.min(), p1.max()

### Tool (2026-02-23 08:58:11)

(0.004674778291350091,
 0.9964828061578636,
 0.004377392802749498,
 0.9959850684371022)

### Assistant (2026-02-23 08:58:33)
...
overall = {
    "logloss_p0": logloss(y_test,p0),
    "logloss_p1": logloss(y_test,p1),
    "brier_p0": brier(y_test,p0),
    "brier_p1": brier(y_test,p1)
}
overall["delta_logloss"] = overall["logloss_p1"]-overall["logloss_p0"]
overall["delta_brier"] = overall["brier_p1"]-overall["brier_p0"]
overall

### Tool (2026-02-23 08:58:41)
...
### Assistant (2026-02-23 08:58:42)

c = np.abs(p0-0.5)
thresh = float(np.median(c))
unc = c<=thresh
con = c>thresh
def metrics_for(mask):
    yt=y_test[mask]; p0m=p0[mask]; p1m=p1[mask]
    return {
        "n": int(mask.sum()),
        "logloss_p0": logloss(yt,p0m),
        "logloss_p1": logloss(yt,p1m),
        "delta_logloss": logloss(yt,p1m)-logloss(yt,p0m),
        "brier_p0": brier(yt,p0m),
        "brier_p1": brier(yt,p1m),
        "delta_brier": brier(yt,p1m)-brier(yt,p0m),
    }
strata={"uncertain": metrics_for(unc), "confident": metrics_for(con)}
thresh, strata

### Tool (2026-02-23 08:58:54)
...
### Assistant (2026-02-23 08:59:00)

out = {
  "identity": {"status":"PASS", "theta_sha1": theta_sha1},
  "threshold": {"median_confidence_test": thresh},
  "overall": overall,
  "strata": strata,
  "notes": "Fingerprint lock satisfied: dt/N_kept/train_end match canonical A4; x_kept SHA1 matches canonical. theta_new recomputed from chebyshev_psi with canonical preprocessing (L=131072, causal W=2000, TRAIN-only zscore) but bandpass [0.003,0.009] via rFFT mask, Hilbert phase modulo 2π. Logistic fits via IRLS with tiny ridge=1e-6 for numerical stability. Negative deltas indicate improvement (lower is better). Lift is concentrated in low-confidence region."
}
json.dumps(out, indent=2)[:500]

### Tool (2026-02-23 08:59:07)

'{\n  "identity": {\n    "status": "PASS",\n    "theta_sha1": "c888f75abc219749cde8f870c119a6dcd98ec2b4"\n  },\n  "threshold": {\n    "median_confidence_test": 0.24314474212232848\n  },\n  "overall": {\n    "logloss_p0": 0.5045922583854335,\n    "logloss_p1": 0.47361248339121403,\n    "brier_p0": 0.16874014571169382,\n    "brier_p1": 0.15638036379042922,\n    "delta_logloss": -0.0309797749942195,\n    "delta_brier": -0.012359781921264607\n  },\n  "strata": {\n    "uncertain": {\n      "n": 5464,\n      "logloss_p0"'

### Assistant (2026-02-23 08:59:09)
