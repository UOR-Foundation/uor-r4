# Script Timeout Solutions

- **Conversation ID:** `699e942a-bf70-832b-a367-a1953a250dcf`
- **Date:** 2026-02-25

---

### User (2026-02-25 01:18:25)
...
### Assistant (2026-02-25 01:18:28)

import gzip, math, json, os, sys, time
import numpy as np
from sklearn.linear_model import LogisticRegression, Lasso
from sklearn.preprocessing import StandardScaler
from sklearn.metrics import roc_auc_score, log_loss, mean_squared_error
...
def zeta_select_lasso(h, L=80, alphas=(3e-4, 1e-3, 3e-3, 1e-2)):
    ...
    return {
        ...
        "train_end": train_end,
        "N_kept": int(N),
        "gamma_window": [float(gmin), float(gmax)],
        "K_inband": int(K),
        "alpha_best": float(alpha_best),
        "alpha_val_mse": float(mse_best),
        "nz_features": int(len(sel)),
        "nz_unique_gammas": int(len(sel_g)),
        "selected_gammas_head": sel_g[:40],
        "selected_gammas_count": int(len(sel_g)),
        ...
        "auc_ar80_theta_sel": auc2,
        "ll_ar80_theta_sel": ll2,
        "delta_auc": float(auc2 - auc_ar),
        "delta_ll": float(ll2 - ll_ar),
        "score_stat": float(stat0),
        "p_emp_shift": float(p_emp),
        "null_q": qs,
        "nshifts": int(nsh),
...
out["meta"]["elapsed_sec"] = float(time.time() - t0)
print(json.dumps(out, indent=2))

### Tool (2026-02-25 01:18:58)

WARNING: Automatic interrupt triggered after 60.0 seconds.

### Assistant (2026-02-25 01:20:21)

import gzip, math, json, time
import numpy as np
from sklearn.linear_model import LogisticRegression, Lasso
from sklearn.preprocessing import StandardScaler
from sklearn.metrics import roc_auc_score, log_loss, mean_squared_error
...
# Load in-band gammas, then cap for runtime
gammas = []
with gzip.open(ZEROS, "rt") as f:
    for line in f:
        s = line.strip()
...
K_cap = 256  # cap to keep runtime under tool limits
if K_full > K_cap:
    # evenly subsample across the in-band list
    idx = np.linspace(0, K_full - 1, K_cap).round().astype(int)
    gammas = gammas[idx]
K = len(gammas)
...
def make_dataset(h, L=80):
    ...
    valid = (idx >= L) & (idx + h < N)
    i_valid = idx[valid]
    X = np.stack([R[i_valid - j] for j in range(1, L + 1)], axis=1).astype(np.float32)
    y = (R[i_valid + h] - R[i_valid] > 0).astype(int)
    train_mask = (i_valid < train_end)
    test_mask = ~train_mask
    return i_valid, X, y, train_mask, test_mask
...
def build_Z(i_valid):
    tt = t[i_valid].astype(np.float64)  # n
    phase = np.outer(tt, gammas)  # n x K
    Z = np.concatenate([np.cos(phase), np.sin(phase)], axis=1)
...
def fit_ar_plus_phase(i_valid, X, y, train_mask, test_mask, theta):
    S = np.stack([np.sin(theta[i_valid]), np.cos(theta[i_valid])], axis=1).astype(np.float32)
    scX = StandardScaler()
    scS = StandardScaler()
    Xtr = scX.fit_transform(X[train_mask]); Xte = scX.transform(X[test_mask])
    Str = scS.fit_transform(S[train_mask]); Ste = scS.transform(S[test_mask])
...
def exact_shift_p(theta, i_valid, X, y, train_mask, test_mask):
    clf, sc = ar80_fit(X, y, train_mask)
    p = ar80_pred(clf, sc, X, test_mask)
    r = (y[test_mask] - p).astype(np.float64)
    w = (p * (1 - p)).astype(np.float64)

    idx_test = np.where(test_mask)[0]
    th = theta[i_valid][idx_test].astype(np.float64)
    s = np.sin(th)
    c = np.cos(th)
    ...
    p_emp = float((stat >= stat0).mean())
    qs = np.quantile(stat, [0.95, 0.99, 0.995]).tolist()
    return stat0, p_emp, qs, int(len(th))

def zeta_select_lasso(h, L=80, alphas=(1e-3, 3e-3)):
    i_valid, X, y, train_mask, test_mask = make_dataset(h, L)
    Z = build_Z(i_valid)
    ...
    p_tr = ar80_pred(clf, sc, X, train_mask)
    p_te = ar80_pred(clf, sc, X, test_mask)
    r_tr = (y[train_mask] - p_tr).astype(np.float32)
    w_tr = (p_tr * (1 - p_tr)).astype(np.float32)
    ...
    best = None
    for a_ in alphas:
        m = Lasso(alpha=a_, max_iter=15000, tol=1e-3)
        m.fit(Xsub, ysub)
        pred = m.predict(Xval)
        mse = mean_squared_error(yval, pred)
        nz  = int((np.abs(m.coef_) > 1e-6).sum())
        score = mse
        if best is None or score < best[0]:
            best = (score, mse, a_, nz)
    ...
    Xtr = Ztr * np.sqrt(w_tr)[:, None]
    ytr = r_tr * np.sqrt(w_tr)
    m = Lasso(alpha=alpha_best, max_iter=20000, tol=1e-3)
    m.fit(Xtr, ytr)
    ...
    s = (Z @ beta_raw).astype(np.float64)
...
out = {
    "meta": {
        ...
        "K_inband_used": int(K),
        "elapsed_sec": None,
        "note": "Runtime-capped version: subsampled in-band zeros to K_inband_used=256, reduced solver iters/tols."
    },
    "res25": zeta_select_lasso(25),
    "res50": zeta_select_lasso(50),
}
...
### Tool (2026-02-25 01:20:57)

WARNING: Automatic interrupt triggered after 60.0 seconds.

### Assistant (2026-02-25 01:22:05)
...
def build_Z(i_valid):
    tt = t[i_valid].astype(np.float64)
...
def zeta_select_lasso_fast(h, L=80, alpha=3e-3):
    i_valid, X, y, train_mask, test_mask = make_dataset(h, L)
    Z = build_Z(i_valid)
    ...
    # standardize Z on TRAIN only
    scZ = StandardScaler().fit(Z[train_mask])
    Ztr = scZ.transform(Z[train_mask])
    Zall = scZ.transform(Z)

    # weighted lasso on TRAIN
    Xtr = Ztr * np.sqrt(w_tr)[:, None]
    ytr = r_tr * np.sqrt(w_tr)
    m = Lasso(alpha=alpha, max_iter=6000, tol=1e-3)
    m.fit(Xtr, ytr)

    coef = m.coef_
    sel = np.where(np.abs(coef) > 1e-6)[0]

    # synth oscillator + Hilbert phase
    beta_raw = coef / scZ.scale_
    s = (Zall @ beta_raw).astype(np.float64)
    s_full = np.zeros(N, dtype=np.float64)
    s_full[i_valid] = s
    theta_sel = np.angle(hilbert(s_full))
...
out = {
    "meta": {
        "note": "Ultra-fast run to fit tool time limit: only h=25, subsample zeros to K=128, single alpha=0.003, no shift-null.",
        "elapsed_sec": None
    },
    "res25": zeta_select_lasso_fast(25, alpha=3e-3),
}
out["meta"]["elapsed_sec"] = float(time.time() - t0)
print(json.dumps(out, indent=2))

### Tool (2026-02-25 01:22:45)

WARNING: Automatic interrupt triggered after 60.0 seconds.

### Assistant (2026-02-25 01:23:52)
...
with gzip.open(ZEROS,"rt") as f:
    for line in f:
        ...
        if v>gmax: break
        gammas.append(v)
gammas=np.array(gammas,dtype=np.float64); K_full=len(gammas)
K_cap=256
if K_full>K_cap:
    idx=np.linspace(0,K_full-1,K_cap).round().astype(int)
    gammas=gammas[idx]
...
def zeta_select_lasso(h,L=80,alphas=(3e-4,1e-3,3e-3,1e-2)):
    ...
    Z=build_Z(i_valid)
    clf,sc=ar80_fit(X,y,train_mask)
    p_tr=ar80_pred(clf,sc,X,train_mask); p_te=ar80_pred(clf,sc,X,test_mask)
    r_tr=y[train_mask]-p_tr; w_tr=p_tr*(1-p_tr)
    tr_rows=np.where(train_mask)[0]
    split=int(len(tr_rows)*0.8)
    ...
    theta_sel=np.angle(hilbert(s_full))
    auc_ar=float(roc_auc_score(y[test_mask],p_te)); ll_ar=float(log_loss(y[test_mask],p_te))
    auc2,ll2=fit_ar_plus_phase(i_valid,X,y,train_mask,test_mask,theta_sel)
    sel_cos=[j for j in sel if j<K]
    sel_sin=[j-K for j in sel if j>=K]
    sel_g=sorted(set(gammas[sel_cos].tolist()+gammas[sel_sin].tolist()))
...
out={"meta":{"dt":dt,"train_end":train_end,"N_kept":int(N),"gamma_window":[float(gmin),float(gmax)],"K_inband_full":int(K_full),"K_inband_used":int(K)},
     "res25":zeta_select_lasso(25),
     "res50":zeta_select_lasso(50),
     "elapsed_sec":float(time.time()-t0)}
out

### Tool (2026-02-25 01:24:18)

WARNING: Automatic interrupt triggered after 60.0 seconds.

### Assistant (2026-02-25 01:25:26)

import numpy as np, math, time, gzip, json
...
A4="/mnt/data/A4_CANONICAL_CACHE_L131072_x1e5_9e6_W2000_bp002_001_D001_seed12345.npz"
D=np.load(A4)
R=D["R"]; train_end=int(D["train_end"]); N=len(R)
...
def ar80_metrics(h):
    X,y,train_mask,test_mask=make_dataset(h,80)
    sc=StandardScaler()
    Xtr=sc.fit_transform(X[train_mask]); Xte=sc.transform(X[test_mask])
    clf=LogisticRegression(max_iter=2000,solver="lbfgs")
    clf.fit(Xtr,y[train_mask])
    p=clf.predict_proba(Xte)[:,1]
    return float(roc_auc_score(y[test_mask],p)), float(log_loss(y[test_mask],p)), int(test_mask.sum()), float(y[test_mask].mean())

t0=time.time()
out={"h25":{}, "h50":{}, "elapsed_sec":None}
out["h25"]["auc"], out["h25"]["logloss"], out["h25"]["n_test"], out["h25"]["pos_rate"]=ar80_metrics(25)
out["h50"]["auc"], out["h50"]["logloss"], out["h50"]["n_test"], out["h50"]["pos_rate"]=ar80_metrics(50)
out["elapsed_sec"]=float(time.time()-t0)
out

### Tool (2026-02-25 01:25:41)

WARNING: Automatic interrupt triggered after 60.0 seconds.

### Assistant (2026-02-25 01:26:49)
...
t0=time.time()
auc,ll,n_test,pos=ar80_metrics(25)
{"h":25,"auc_ar80":auc,"logloss_ar80":ll,"n_test":n_test,"pos_rate_test":pos,"elapsed_sec":float(time.time()-t0)}

### Tool (2026-02-25 01:26:59)

WARNING: Automatic interrupt triggered after 60.0 seconds.

### Assistant (2026-02-25 01:28:09)

Tried. The runtime sandbox hard-kills Python after **~60 seconds**, so your script (two AR80 fits + building a dense `Z` + cross-validated Lasso + Hilbert + shift-null) gets guillotined before it can print anything. It’s not “slow”, it’s “not allowed to finish.”  

If you actually want *results*, you’ll need to run this **outside** this sandbox (your machine / your server), or you need to rewrite it to fit the 60s ceiling.

Here’s what I can do *usefully* right now:

## Why it times out (the real reasons)
Your script is expensive in three places:
1. **Building `Z`:** `np.outer(tt, gammas)` creates an `n×K` matrix, then you double it to `2K` with sin/cos.
2. **Lasso CV-ish loop:** multiple Lasso fits on a big weighted design.
3. **AR80 twice + AR80+phase twice** (h=25 and h=50) adds more wall time.

Even with aggressive caps (I tried), it still overruns the 60s limit.

## Minimal changes that make it runnable (without changing the idea)
If you run locally, you’ll get your output basically as-is. If you *must* run under tight time, you need to do some combination of:

- **Compute only one horizon** first (`h=25` *or* `h=50`).
- **Subsample gammas hard** (e.g., `K=128` or even `K=64`).
- **Drop the internal TRAIN 80/20 split** (pick one alpha, refit once).
- **Use `SGDRegressor(loss="squared_error", penalty="l1")`** as a proxy for Lasso (it’s much faster on big sparse-ish problems).
- **Skip `exact_shift_p`** until you’ve verified there’s *any* lift.

If you want, I can rewrite your script into a “fast path” that preserves the same statistical question, just not the same amount of compute. (No, I’m not asking you questions. I’m stating the obvious.)

---

# One discriminating next experiment (at the current frontier)
Per the project history, we already have a locked canonical pipeline and we’re trying to determine whether “global winding-phase / zeta-structured coordinates” add anything real beyond transport + AR baselines. That’s explicitly a live frontier (H1c). fileciteturn0file0

### Experiment A5.01 — “Zeta-Lasso phase vs random-gamma Lasso phase (matched K), held-out”
**Question:** Does a phase synthesized from *in-band true zeros* selected by residualized L1 regression beat a **density-matched random-gamma** control when both are trained the same way?

**Inputs (existing artifacts):**
- Canonical cache: `/mnt/data/A4_CANONICAL_CACHE_L131072_x1e5_9e6_W2000_bp002_001_D001_seed12345.npz`
- Zero list: `/mnt/data/zeros1.gz`
- Reference discipline: `HISTORY_2026-02-22_A4_cache_and_fingerprint_lock.md` (fingerprint lock rules)

**Method (tight, discriminating):**
1. Fix horizon **h=25** (desticky target already defined in Protocol; avoids sticky-label nonsense).
2. Build AR80 baseline on TRAIN only.
3. Residualize TRAIN: `r = y - p_AR80`, weights `w=p(1-p)`.
4. Fit **one** L1 model on `Z=[cos(tγ), sin(tγ)]` to predict `r` (weighted least squares), producing coefficients `β`.
5. Construct `s(t)=Zβ`, take Hilbert phase `θ_sel`.
6. Evaluate on TEST only: AR80 vs AR80+sin/cos(θ_sel) AUC/logloss.
7. **Control:** repeat steps 4–6 with a random gamma set sampled **uniformly in the same [gmin,gmax]**, same K, same alpha. (Do 50 reps. Cheap compared to your full shift-null.)
8. Report: ΔAUC distribution vs control, and where the selected γ’s cluster (do they concentrate near the known dominant low-frequency mode).

**Decision rule:**
- If ΔAUC_true is indistinguishable from the random-gamma distribution, that’s strong evidence your “zeta selection” is just picking a convenient Fourier-ish basis, not something zeta-specific.
- If ΔAUC_true is reliably above random-gamma, that’s a clean win for “global phase coordinate matters” (H1c stays live with teeth).

This is exactly the kind of experiment that *stops us from re-running old anchors* and actually discriminates between “zeta matters” vs “any oscillatory basis works.” fileciteturn0file0

---

## What you should run locally (practical)
- Put your script in a file, run with:
  - `OMP_NUM_THREADS=1` (prevents BLAS thread storms)
  - `MKL_NUM_THREADS=1`
- Start with **only `res25`**, no shift-null, and K capped to 256.
- Once you see lift, *then* add the nulls and `res50`.

---

If you want the blunt truth: the script is conceptually fine, but it’s built like a “desktop science fair” version. It needs a “does this effect exist at all?” fast path before it earns the right to do expensive nulls.
