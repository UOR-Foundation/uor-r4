# Phase Modulation Mean Reversion

- **Conversation ID:** `699b93b0-cf14-832b-a22f-05478f26ee74`
- **Date:** 2026-02-22

---

### User (2026-02-22 18:39:34)
...
# Load files
base="/mnt/data"
psi=np.load(os.path.join(base,"chebyshev_psi.npz"))
...
# Uniform t grid over min/max of t_raw
t_raw=np.log(x_grid)
...
# FFT bandpass using rfft mask on full kept
U=np.fft.rfft(u_z)
...
theta=np.mod(np.angle(analytic), 2*np.pi).astype(np.float64)

# Build R
expDelta=math.exp(Delta)
# prime counts using searchsorted
lo = np.searchsorted(primes, x_kept, side="left")
hi = np.searchsorted(primes, x_kept*expDelta, side="left")
...
def sha1_arr(a):
    ...
    return hashlib.sha1(a64.tobytes()).hexdigest()
...
for h in h_list:
    h=int(h)
    valid = np.arange(0, N_kept-h, dtype=np.int64)
    ...
    th = theta[valid]
    # MR features
    X_mr = np.column_stack([np.ones(valid.size, dtype=np.float64), Ri.astype(np.float64)])
    # full features
    cols=[np.ones(valid.size, dtype=np.float64), Ri.astype(np.float64)]
    for k in range(1, K+1):
        cols.append((Ri*np.cos(k*th)).astype(np.float64))
        cols.append((Ri*np.sin(k*th)).astype(np.float64))
    X_full=np.column_stack(cols)
    # split
    Xmr_tr=X_mr[train_idx]; Xmr_te=X_mr[test_idx]
    Xfu_tr=X_full[train_idx]; Xfu_te=X_full[test_idx]
    # fit ridge
    mr_model=Ridge(alpha=ridge_alpha, fit_intercept=False, random_state=seed)
    fu_model=Ridge(alpha=ridge_alpha, fit_intercept=False, random_state=seed)
    mr_model.fit(Xmr_tr, dR_train)
    fu_model.fit(Xfu_tr, dR_train)
    # predict
    mr_hat=mr_model.predict(Xmr_te)
    fu_hat=fu_model.predict(Xfu_te)
    # metrics
    mse_mr=float(mean_squared_error(dR_test, mr_hat))
    mse_fu=float(mean_squared_error(dR_test, fu_hat))
    r2_mr=float(r2_score(dR_test, mr_hat))
    r2_fu=float(r2_score(dR_test, fu_hat))
    # auc
    def safe_auc(y_true, scores):
        if y_true.min()==y_true.max():
            return None
        return float(roc_auc_score(y_true, scores))
    auc_mr=safe_auc(y_test, mr_hat)
    auc_fu=safe_auc(y_test, fu_hat)
    by_h[str(h)] = {
        "counts": {
            "n_train": int(train_idx.size),
            "n_test": int(test_idx.size),
            "train_sign_pos_rate": float(y_train.mean()) if y_train.size else None,
            "test_sign_pos_rate": float(y_test.mean()) if y_test.size else None
        },
        "MR_model": {
            "mse_test": mse_mr,
            "r2_test": r2_mr,
            "sign_auc_test": auc_mr,
            "coef": mr_model.coef_.astype(np.float64).tolist()
        },
        "FULL_interaction_model": {
            "mse_test": mse_fu,
            "r2_test": r2_fu,
            "sign_auc_test": auc_fu,
            "coef": fu_model.coef_.astype(np.float64).tolist()
        },
        "incremental": {
            "delta_mse": float(mse_fu - mse_mr),
            "delta_r2": float(r2_fu - r2_mr),
            "delta_sign_auc": (None if (auc_mr is None or auc_fu is None) else float(auc_fu - auc_mr))
        }
    }
...
out={
  "meta":{
    ...
    "params":{
      ...
      "bandpass_cycles_per_sample":[f_lo,f_hi],"h_list":h_list,"K":K,"ridge_alpha":ridge_alpha
    }
  },
  "shape":{
    "dt":dt,
    "N_kept":N_kept,
    "train_end":train_end,
    "x_min_kept":float(x_kept.min()),
    "x_max_kept":float(x_kept.max())
  },
  "hashes_sha1":hashes,
  "by_h":by_h,
  "notes":{
    "interpretation_hint":"FULL model tests whether mean-reversion slope (dR vs R_i) depends on phase via interaction terms R_i*cos(kθ_i), R_i*sin(kθ_i). Negative delta_mse / positive delta_r2 / higher sign_auc indicate phase-modulated MR."
  }
}
out

### Tool (2026-02-22 18:40:32)
...
{'meta': {'role': 'stateless_compute_worker',
  ...
  'params': {'L': 131072,
   ...
   'K': 3,
   'ridge_alpha': 1.0}},
 'shape': {'dt': 0.00012297217348543395,
  'N_kept': 36592,
  'train_end': 25614,
  'x_min_kept': 100009.83380914277,
  'x_max_kept': 8999671.172393441},
 'hashes_sha1': {'x_kept': 'c455fedeff29bb2d0b2c81864970c09638720baf',
  ...
  '100': {'counts': {'n_train': 25514,
   ...
   'FULL_interaction_model': {'mse_test': 0.21615503945656075,
    ...
    'coef': [-0.024177597620033017,
     ...
     -0.0416222198820406,
     -0.02904138274566225]},
   'incremental': {'delta_mse': -0.004400426035244015,
    'delta_r2': 0.008295508819255115,
    'delta_sign_auc': 0.000453373009668967}}},
 'notes': {'interpretation_hint': 'FULL model tests whether mean-reversion slope (dR vs R_i) depends on phase via interaction terms R_i*cos(kθ_i), R_i*sin(kθ_i). Negative delta_mse / positive delta_r2 / higher sign_auc indicate phase-modulated MR.'}}

### Assistant (2026-02-22 18:40:36)
