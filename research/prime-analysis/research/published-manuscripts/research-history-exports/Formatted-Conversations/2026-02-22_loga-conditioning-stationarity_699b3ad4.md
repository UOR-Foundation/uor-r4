# LogA Conditioning Stationarity

- **Conversation ID:** `699b3ad4-0ab8-8327-be80-fc84201a71b8`
- **Date:** 2026-02-22

---

### User (2026-02-22 12:20:23)
...
# FFT bandpass on full kept, using N_kept
U=rfft(u_std)
freqs=rfftfreq(N_kept, d=1.0)  # cycles/sample
mask=(freqs>=FAST[0]) & (freqs<=FAST[1])
U_bp=np.where(mask, U, 0.0)
...
# features aligned to omega indices
...
# Prime residual R on original kept indices
# Prime counts in [x, x*exp(Delta))
expD=math.exp(Delta)
hi = xk*expD
# searchsorted expects sorted primes
lo_idx=np.searchsorted(primes, xk, side="left")
hi_idx=np.searchsorted(primes, hi, side="left")
P=(hi_idx-lo_idx).astype(float)
E=(xk*(expD-1.0)/np.log(xk)).astype(float)
R=(P-E)/np.sqrt(E)

def build_y(h):
    # y for i where i+h valid in R (length N_kept)
    y=np.full(N_feat, np.nan)
    max_i = N_kept - h - 1
    if max_i < 0:
        return y
    i = np.arange(0, min(N_feat, max_i+1))
    y[i] = (R[i+h] - R[i] > 0).astype(float)
    return y

# bin edges from TRAIN
theta_edges = np.linspace(0.0, 2*np.pi, B_theta_f+1)
omega_train = omega[:train_end_feat]
# quantile edges
q=np.linspace(0,1,B_omega_f+1)
omega_edges=np.quantile(omega_train, q)
# make monotone, and extend to cover [-pi,pi]
omega_edges[0]=-np.pi
omega_edges[-1]=np.pi
# ensure strict increasing by tiny jitter if needed
for k in range(1,len(omega_edges)):
    if omega_edges[k] <= omega_edges[k-1]:
        omega_edges[k]=omega_edges[k-1]+1e-9

logA_train = logA_f[:train_end_feat]
qA=np.linspace(0,1,R_logA+1)
logA_edges=np.quantile(logA_train, qA)
# extend
logA_edges[0]=-np.inf
logA_edges[-1]=np.inf
for k in range(1,len(logA_edges)-1):
    if logA_edges[k] <= logA_edges[k-1]:
        logA_edges[k]=logA_edges[k-1]+1e-9

def assign_bins(theta_vals, omega_vals):
    b_theta = np.digitize(theta_vals, theta_edges, right=False) - 1
    b_theta = np.clip(b_theta, 0, B_theta_f-1)
    b_omega = np.digitize(omega_vals, omega_edges, right=False) - 1
    b_omega = np.clip(b_omega, 0, B_omega_f-1)
    fast_bin = b_theta*B_omega_f + b_omega
    return fast_bin.astype(int)

fast_bin = assign_bins(theta_f, omega)

def assign_regime(logA_vals):
    r = np.digitize(logA_vals, logA_edges, right=False) - 1
    r = np.clip(r, 0, R_logA-1)
    return r.astype(int)

regime = assign_regime(logA_f)

B_fast = B_theta_f*B_omega_f

def bin_stats(indices, y, fast_bin_arr):
    # indices are integer positions in feature arrays
    fb = fast_bin_arr[indices]
    yy = y[indices]
    # remove nan
    ok = ~np.isnan(yy)
    fb=fb[ok]; yy=yy[ok]
    n=np.bincount(fb, minlength=B_fast).astype(float)
    s=np.bincount(fb, weights=yy, minlength=B_fast).astype(float)
    p=(s+eps)/(n+2*eps)
    return p,n,s

def stationarity_metrics(p_tr, n_tr, p_te, n_te, min_count):
    use = (n_tr>=min_count) & (n_te>=min_count)
    idx=np.where(use)[0]
    if idx.size < 2:
        return {"bins_used": int(idx.size), "corr": None, "wrmse": None, "frac_abs_z_gt_2": None}
    pt=p_tr[idx]; ps=p_te[idx]; w=n_te[idx]
    # corr
    c=float(np.corrcoef(pt, ps)[0,1])
    # weighted rmse
    wrmse=float(np.sqrt(np.sum(w*(ps-pt)**2)/np.sum(w)))
    # z using test variance under p_train
    var = pt*(1-pt)/np.maximum(w,1.0)
    z=(ps-pt)/np.sqrt(np.maximum(var,1e-12))
    fracz=float(np.mean(np.abs(z)>2.0))
    return {"bins_used": int(idx.size), "corr": c, "wrmse": wrmse, "frac_abs_z_gt_2": fracz}

out={"params":{
    "L":L,"x_min":x_min,"x_max":x_max,"W":W,"Delta":Delta,
    "train_frac":train_frac,"seed":seed,"FAST":list(FAST),"H":H,
    "B_theta_f":B_theta_f,"B_omega_f":B_omega_f,"R_logA":R_logA,
    "min_count_bin":min_count_bin,"min_count_bin_baseline":min_count_bin_baseline,"eps":eps
},"derived":{"dt":dt,"N_feat":N_feat,"train_end_feat":train_end_feat},"per_h":{}}
...
for h in H:
    y=build_y(h)
    # valid indices
    all_i=np.arange(N_feat)
    train_valid=all_i[(all_i < train_end) & (all_i + h < train_end)]
    test_valid=all_i[(all_i >= train_end) & (all_i + h < N_kept) & (all_i + h >= train_end)]
    # baseline
    p_tr,n_tr,_=bin_stats(train_valid, y, fast_bin)
    p_te,n_te,_=bin_stats(test_valid, y, fast_bin)
    base=stationarity_metrics(p_tr,n_tr,p_te,n_te,min_count_bin_baseline)
    base_details={"bins_used":base["bins_used"],"corr":base["corr"],"wrmse":base["wrmse"],"frac_abs_z_gt_2":base["frac_abs_z_gt_2"],
                  "n_train_total": int(len(train_valid)), "n_test_total": int(len(test_valid))}
    # regimes
    regimes=[]
    weights=[]
    corr_list=[]
    wrmse_list=[]
    fracz_list=[]
    for r in range(R_logA):
        tr_idx=train_valid[regime[train_valid]==r]
        te_idx=test_valid[regime[test_valid]==r]
        p_tr_r,n_tr_r,_=bin_stats(tr_idx,y,fast_bin)
        p_te_r,n_te_r,_=bin_stats(te_idx,y,fast_bin)
        met=stationarity_metrics(p_tr_r,n_tr_r,p_te_r,n_te_r,min_count_bin)
        # weight: total n_test used bins
        use=(n_tr_r>=min_count_bin)&(n_te_r>=min_count_bin)
        w=float(n_te_r[use].sum())
        weights.append(w)
        corr_list.append(met["corr"])
        wrmse_list.append(met["wrmse"])
        fracz_list.append(met["frac_abs_z_gt_2"])
        regimes.append({"r":r,"n_train":int(len(tr_idx)),"n_test":int(len(te_idx)),
                        "bins_used":met["bins_used"],"corr":met["corr"],"wrmse":met["wrmse"],
                        "frac_abs_z_gt_2":met["frac_abs_z_gt_2"],"n_test_used_weight":w})
    # aggregate with weights over regimes that have metrics
    wsum=sum(w for w,c in zip(weights,corr_list) if (c is not None) and (w>0))
    if wsum>0:
        weighted_corr=float(sum(w*c for w,c in zip(weights,corr_list) if (c is not None) and (w>0))/wsum)
    else:
        weighted_corr=None
    wsum2=sum(w for w,m in zip(weights,wrmse_list) if (m is not None) and (w>0))
    weighted_wrmse=float(sum(w*m for w,m in zip(weights,wrmse_list) if (m is not None) and (w>0))/wsum2) if wsum2>0 else None
    wsum3=sum(w for w,m in zip(weights,fracz_list) if (m is not None) and (w>0))
    overall_fracz=float(sum(w*m for w,m in zip(weights,fracz_list) if (m is not None) and (w>0))/wsum3) if wsum3>0 else None
    
    deltas={"delta_corr": (weighted_corr-base["corr"]) if (weighted_corr is not None and base["corr"] is not None) else None,
            "delta_wrmse": (weighted_wrmse-base["wrmse"]) if (weighted_wrmse is not None and base["wrmse"] is not None) else None,
            "delta_fracz": (overall_fracz-base["frac_abs_z_gt_2"]) if (overall_fracz is not None and base["frac_abs_z_gt_2"] is not None) else None}
    # regime defs edges list finite
    regime_defs={"R":R_logA,"logA_edges":[float(e) if np.isfinite(e) else (None) for e in logA_edges.tolist()]}
    agg={"weighted_corr":weighted_corr,"weighted_wrmse":weighted_wrmse,"overall_frac_abs_z_gt_2":overall_fracz,
         "total_n_test_used_weight": float(sum(weights))}
    per_h={"baseline_unconditioned":base_details,
           "by_logA_regime":{"regime_defs":regime_defs,"regimes":regimes,"aggregate":agg,"deltas_vs_baseline":deltas}}
    out["per_h"][str(h)]=per_h
    
    # decide
    flag="no"
    if deltas["delta_corr"] is not None and deltas["delta_wrmse"] is not None and deltas["delta_fracz"] is not None:
        if (deltas["delta_corr"]>0.05) and (deltas["delta_wrmse"]<0) and (deltas["delta_fracz"]<0):
            flag="yes"
        elif (deltas["delta_corr"]>0.02) and ((deltas["delta_wrmse"]<0) or (deltas["delta_fracz"]<0)):
            flag="partial"
    summary_flags[str(h)]=flag

out["summary"]={"does_logA_conditioning_restore_stationarity":summary_flags,
 ...
 'per_h': {'25': {'baseline_unconditioned': {'bins_used': 91,
  ...
  '100': {'baseline_unconditioned': {'bins_used': 89,
   ...
   'by_logA_regime': {'regime_defs': {'R': 4,
    ...
    'aggregate': {'weighted_corr': 0.6775649911633166,
     'weighted_wrmse': 0.22400271778402078,
     'overall_frac_abs_z_gt_2': 0.6944187247035823,
     'total_n_test_used_weight': 3007.0},
    'deltas_vs_baseline': {'delta_corr': -0.0384554176559776,
     'delta_wrmse': 0.04637883416195962,
     'delta_fracz': 0.12138501683841385}}}},
 'summary': {'does_logA_conditioning_restore_stationarity': {'25': 'no',
   '100': 'no'},
  'notes': 'Interpretation heuristic: improvement means corr increases and wrmse/fracz decrease vs unconditioned baseline on bins that meet occupancy thresholds.'}}

### Assistant (2026-02-22 12:22:35)
