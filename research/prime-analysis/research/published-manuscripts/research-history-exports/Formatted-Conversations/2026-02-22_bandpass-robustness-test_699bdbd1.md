# Bandpass Robustness Test

- **Conversation ID:** `699bdbd1-7864-8333-9154-d906c42a71b8`
- **Date:** 2026-02-22

---

### User (2026-02-22 23:47:17)
...
### Assistant (2026-02-22 23:47:20)

import numpy as np, math, hashlib, json, os, sys, statistics
...
# Uniform log grid over full range of x_grid
t_raw = np.log(x_grid.astype(np.float64))
t_u = np.linspace(t_raw.min(), t_raw.max(), L)
dt = t_u[1]-t_u[0]
# Interpolate psi_minus_x onto uniform t
u_interp = np.interp(t_u, t_raw, psi_minus_x.astype(np.float64))
x_u = np.exp(t_u)
# Restrict kept
mask = (x_u>=x_min) & (x_u<=x_max)
u_raw = u_interp[mask]
x_kept = x_u[mask]
N_kept = len(x_kept)
train_end = int(math.floor(train_frac * N_kept))

# Causal moving average detrend (past-only incl current)
...
# Bandpass via rFFT/irFFT nonnegative freqs
U = rfft(u_z)
freqs = rfftfreq(N_kept, d=1.0)  # cycles/sample
bp_mask = (freqs>=bp_lo) & (freqs<=bp_hi)
U_bp = U * bp_mask
u_bp = irfft(U_bp, n=N_kept)

# Hilbert analytic and theta in [0,2pi)
analytic = hilbert(u_bp)
theta = np.angle(analytic)
theta_mod = np.mod(theta, 2*np.pi)
theta_bytes = theta_mod.astype(np.float64).tobytes()
theta_sha1 = hashlib.sha1(theta_bytes).hexdigest()

# Compute prime residual R on kept
...
hi_idx = np.searchsorted(primes, x2, side='left')
P = (hi_idx - lo_idx).astype(np.int32)
E = x_kept*(expD-1.0)/np.log(x_kept)
R = (P - E)/np.sqrt(E)

# Target y_i = 1{R[i+h]-R[i]>0}
dR = R[h:] - R[:-h]
y_all = (dR > 0).astype(np.int8)
# sample indices i correspond to original i in 0..N_kept-h-1
i_all = np.arange(N_kept-h, dtype=np.int32)

# Build AR80 design matrix with lags 0..79
p_lags=80
min_i = p_lags-1
valid_mask = i_all >= min_i
i_v = i_all[valid_mask]
y_v = y_all[valid_mask]
# features: R[i], R[i-1],...,R[i-79]
X = np.stack([R[i_v - k] for k in range(p_lags)], axis=1)

# Train/test split with no leakage: for train, i+h < train_end and i < train_end
train_mask = (i_v < train_end) & (i_v + h < train_end)
test_mask  = (i_v >= train_end) & (i_v + h < (N_kept))  # additionally ensure i+h in test? requested? We'll do strict: i_v + h >= train_end
test_mask = (i_v >= train_end) & (i_v + h >= train_end) & (i_v + h < N_kept)
X_train, y_train, i_train = X[train_mask], y_v[train_mask], i_v[train_mask]
X_test, y_test, i_test = X[test_mask], y_v[test_mask], i_v[test_mask]

# Fit baseline AR80 logistic
...
# Efficient score test for adding sin/cos(theta)
# Use train data; baseline linear predictor from clf
# For score test we need p_i under null on train samples (already p_train)
# W_i = p(1-p)
Wv = p_train*(1.0-p_train)
# Baseline design X1 includes intercept + AR80 features
X1 = np.concatenate([np.ones((X_train.shape[0],1)), X_train], axis=1)
# Theta features for indices i_train
th = theta_mod[i_train]
X2 = np.column_stack([np.sin(th), np.cos(th)])

# Compute components for efficient info
# A = X1^T W X1
# B = X1^T W X2
# C = X2^T W X2
# U2 = X2^T (y-p)
...
B = WX1.T @ WX2
C = WX2.T @ WX2
U2 = X2.T @ (y_train - p_train)

# Solve for A^{-1}B
# use robust solve with np.linalg.solve
A_inv_B = np.linalg.solve(A, B)
I_eff = C - B.T @ A_inv_B
U_eff = U2  # already efficient because nuisance-adjustment accounted in I_eff formula
# Actually the efficient score is U2 - B^T A^{-1} U1, but U1 = X1^T(y-p)=0 for MLE? With regularization, not exactly.
# We'll approximate with U1=0; but to be safer compute U1 and adjust.
U1 = X1.T @ (y_train - p_train)
U_eff = U2 - B.T @ np.linalg.solve(A, U1)
T_real = float(U_eff.T @ np.linalg.solve(I_eff, U_eff))

# Chi2 p-value
import scipy.stats as st
p_chi2 = float(1.0 - st.chi2.cdf(T_real, df=2))

# Circular-shift null: shift theta_mod only, recompute T using same Wv, X1, y, p_train
reps=300
min_shift=50
T_null=[]
n_theta = len(theta_mod)
# for sample indices i_train, use shifted theta[(i_train + k) % n_theta]
for _ in range(reps):
    k = int(rng.integers(min_shift, n_theta - min_shift))
    th_s = theta_mod[(i_train + k) % n_theta]
    X2s = np.column_stack([np.sin(th_s), np.cos(th_s)])
    WX2s = X2s * sqrtW[:,None]
    B_s = WX1.T @ WX2s
    C_s = WX2s.T @ WX2s
    U2_s = X2s.T @ (y_train - p_train)
    A_inv_Bs = np.linalg.solve(A, B_s)
    Ieff_s = C_s - B_s.T @ A_inv_Bs
    Ueff_s = U2_s - B_s.T @ np.linalg.solve(A, U1)
    Ts = float(Ueff_s.T @ np.linalg.solve(Ieff_s, Ueff_s))
    T_null.append(Ts)
T_null = np.array(T_null, dtype=np.float64)
null_mean = float(T_null.mean())
null_std = float(T_null.std(ddof=0))
p95 = float(np.quantile(T_null, 0.95))
p99 = float(np.quantile(T_null, 0.99))
emp_p = float((np.sum(T_null >= T_real) + 1.0)/(reps + 1.0))

(T_real, p_chi2, auc_train, auc_test, theta_sha1, N_kept, train_end, X_train.shape, X_test.shape, y_train.mean(), y_test.mean(), null_mean, p95)

### Tool (2026-02-22 23:48:20)

---------------------------------------------------------------------------
KeyboardInterrupt                         Traceback (most recent call last)
Cell In[1], line 164
    162 k = int(rng.integers(min_shift, n_theta - min_shift))
    163 th_s = theta_mod[(i_train + k) % n_theta]
--> 164 X2s = np.column_stack([np.sin(th_s), np.cos(th_s)])
    165 WX2s = X2s * sqrtW[:,None]
    166 B_s = WX1.T @ WX2s
...
def causal_ma(a, W):
    ...
    # sum over [j0,i] = c[i] - c[j0-1]
    c_shift = np.concatenate(([0.0], c[:-1]))
    s = c - c_shift[j0]
    denom = (idx - j0 + 1).astype(np.float64)
    return s/denom
...
bp_mask = (freqs>=bp_lo) & (freqs<=bp_hi)
u_bp = irfft(U*bp_mask, n=N_kept)

theta_mod = np.mod(np.angle(hilbert(u_bp)), 2*np.pi).astype(np.float64)
theta_sha1 = hashlib.sha1(theta_mod.tobytes()).hexdigest()
...
i_v = i_all[valid]
y_v = y_all[valid]
# Build X lags efficiently using stride? Use loop over lags but small.
X = np.stack([R[i_v - k] for k in range(p_lags)], axis=1).astype(np.float64)
...
reps=300
min_shift=50
n_theta = theta_mod.size
T_null = np.empty(reps, dtype=np.float64)
for r in range(reps):
    k = int(rng.integers(min_shift, n_theta - min_shift))
    th_s = theta_mod[(i_train + k) % n_theta]
    X2s = np.column_stack([np.sin(th_s), np.cos(th_s)])
    ...
    Ueff_s = U2_s - B_s.T @ (A_inv @ U1)
    T_null[r] = float(Ueff_s.T @ np.linalg.solve(Ieff_s, Ueff_s))
...
dict(theta_sha1=theta_sha1, auc_train=auc_train, auc_test=auc_test, T_real=T_real, p_chi2=p_chi2,
     null_mean=null_mean, null_std=null_std, p95=p95, p99=p99, emp_p=emp_p,
     N_kept=N_kept, train_end=train_end, n_train=int(X_train.shape[0]), n_test=int(X_test.shape[0]),
     y_train_rate=float(y_train.mean()), y_test_rate=float(y_test.mean()), dt=dt)

### Tool (2026-02-22 23:50:00)

{'theta_sha1': '1aa28e1b1b3c1563921dc01ff475468b59514aad',
 'auc_train': 0.8356702524612365,
 'auc_test': 0.8374178294540402,
 'T_real': 1473.9427249255941,
 'p_chi2': 0.0,
 'null_mean': 38.62732332232523,
 'null_std': 111.348897185584,
 'p95': 85.77527122197168,
 'p99': 487.3649598585998,
 'emp_p': 0.0033222591362126247,
 'N_kept': 18296,
 'train_end': 12807,
 'n_train': 12703,
 'n_test': 5464,
 'y_train_rate': 0.49720538455482954,
 'y_test_rate': 0.5065885797950219,
 'dt': 0.00024594622340595577}

### Assistant (2026-02-22 23:50:23)
