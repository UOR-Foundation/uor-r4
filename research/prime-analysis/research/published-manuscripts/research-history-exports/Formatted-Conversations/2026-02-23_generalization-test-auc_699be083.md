# Generalization Test AUC

- **Conversation ID:** `699be083-56cc-832c-9ee9-079de6b4b6bb`
- **Date:** 2026-02-23

---

### User (2026-02-23 00:07:18)
...
### Assistant (2026-02-23 00:07:21)

import numpy as np, os, hashlib, math, json, sys
...
# paths
base = "/mnt/data"
featurecache_path = os.path.join(base, "A4_FEATURECACHE_h25_AR80_AR40_theta_omega_masks_v1.npz")
canonical_path = os.path.join(base, "A4_CANONICAL_CACHE_L131072_x1e5_9e6_W2000_bp002_001_D001_seed12345.npz")
cheb_path = os.path.join(base, "chebyshev_psi.npz")

for p in [featurecache_path, canonical_path, cheb_path]:
    assert os.path.exists(p), p

feat = np.load(featurecache_path, allow_pickle=True)
canon = np.load(canonical_path, allow_pickle=True)

feat.files[:20], canon.files[:20], len(feat.files), len(canon.files)

### Tool (2026-02-23 00:08:06)
...
N_kept_can=int(canon["R"].shape[0])
(dt_feat, N_kept_feat, train_end_feat, dt_can, N_kept_can, train_end_can)

### Tool (2026-02-23 00:08:13)

(0.00012297217348543395, 36592, 25614, 0.00012297217348543395, 36592, 25614)

### Assistant (2026-02-23 00:08:21)

cheb = np.load(cheb_path, allow_pickle=True)
cheb.files

### Tool (2026-02-23 00:08:23)

['N', 'step', 'x_grid', 'psi_grid', 'psi_minus_x']

### Assistant (2026-02-23 00:08:25)
...
### Tool (2026-02-23 00:08:27)

0.00012297217348543397

### Assistant (2026-02-23 00:08:29)

dt_lock = dt_can
t0 = float(t_raw.min())
t_uniform2 = t0 + dt_lock*np.arange(L, dtype=np.float64)
# compare last value to t_raw.max()
float(t_uniform2[-1]-t_raw.max()), float(t_uniform2[1]-t_uniform2[0])

### Tool (2026-02-23 00:08:43)

(-3.552713678800501e-15, 0.00012297217348543395)

### Assistant (2026-02-23 00:08:45)

u_interp = np.interp(t_uniform2, t_raw, psi_minus_x)
x_uniform2 = np.exp(t_uniform2)
x_min=1e5; x_max=9e6
kept = np.where((x_uniform2>=x_min) & (x_uniform2<=x_max))[0]
N_kept_new = kept.size
N_kept_new

### Tool (2026-02-23 00:08:48)

36592

### Assistant (2026-02-23 00:08:51)

W=2000
u_raw = u_interp[kept]
ma = causal_moving_average(u_raw, W)
u_det = u_raw - ma
train_end = train_end_feat
u_train = u_det[:train_end]
mu = float(u_train.mean())
sd = float(u_train.std(ddof=0))
u_z = (u_det - mu) / (sd if sd>0 else 1.0)

# bandpass [0.003,0.009]
f_lo, f_hi = 0.003, 0.009
spec = rfft(u_z)
freqs = rfftfreq(u_z.size, d=1.0)  # cycles/sample
mask = (freqs>=f_lo) & (freqs<=f_hi)
spec_bp = spec * mask
u_bp_new = irfft(spec_bp, n=u_z.size)
# hilbert
analytic = scipy.signal.hilbert(u_bp_new)
theta_new = np.mod(np.angle(analytic), 2*np.pi).astype(np.float64)
theta_sha1 = sha1_arr(theta_new)
theta_sha1[:10], theta_new.shape

### Tool (2026-02-23 00:08:57)

('26888d653c', (36592,))

### Assistant (2026-02-23 00:08:59)

h=50
R = feat["R"].astype(np.float64)
X80 = feat["X80"].astype(np.float64)  # likely shape (N_kept,80)
mu_X80 = feat["mu_X80_train"].astype(np.float64)
sd_X80 = feat["sd_X80_train"].astype(np.float64)
# standardize X80
X80_z = (X80 - mu_X80) / sd_X80

idx = np.arange(N_kept_feat)
mask_train = (idx>=79) & (idx<train_end) & ((idx+h)<train_end)
mask_test  = (idx>=79) & (idx>=train_end) & ((idx+h)<N_kept_feat)
n_train = int(mask_train.sum())
n_test = int(mask_test.sum())
n_train, n_test

### Tool (2026-02-23 00:09:04)

(25485, 10928)

### Assistant (2026-02-23 00:09:07)

dR = R[idx+h] - R[idx]
y = (dR>0).astype(np.int8)
pos_rate_test = float(y[mask_test].mean())
pos_rate_test

### Tool (2026-02-23 00:09:11)

---------------------------------------------------------------------------
IndexError                                Traceback (most recent call last)
Cell In[9], line 1
----> 1 dR = R[idx+h] - R[idx]
      2 y = (dR>0).astype(np.int8)
      3 pos_rate_test = float(y[mask_test].mean())

IndexError: index 36592 is out of bounds for axis 0 with size 36592

### Assistant (2026-02-23 00:09:14)

N=N_kept_feat
dR = np.full(N, np.nan, dtype=np.float64)
dR[:-h] = R[h:] - R[:-h]
y = np.zeros(N, dtype=np.int8)
valid = ~np.isnan(dR)
y[valid] = (dR[valid]>0).astype(np.int8)
pos_rate_test = float(y[mask_test].mean())
pos_rate_test

### Tool (2026-02-23 00:09:18)

0.5058565153733529

### Assistant (2026-02-23 00:09:22)
...
Xb_train = X80_z[train_idx]
Xb_test = X80_z[test_idx]
y_train = y[train_idx]
y_test = y[test_idx]

# baseline logistic
clf_base = LogisticRegression(penalty='l2', C=1e6, solver='lbfgs', max_iter=2000)
clf_base.fit(Xb_train, y_train)
p_base_test = clf_base.predict_proba(Xb_test)[:,1]
auc_base = float(roc_auc_score(y_test, p_base_test))

# full: add sin/cos(theta_new)
sinth = np.sin(theta_new)
costh = np.cos(theta_new)
Z = np.column_stack([sinth, costh])
# standardize on TRAIN samples
Z_mu = Z[train_idx].mean(axis=0)
Z_sd = Z[train_idx].std(axis=0, ddof=0)
Z_z = (Z - Z_mu) / Z_sd

Xf_train = np.hstack([X80_z[train_idx], Z_z[train_idx]])
Xf_test = np.hstack([X80_z[test_idx], Z_z[test_idx]])

clf_full = LogisticRegression(penalty='l2', C=1e6, solver='lbfgs', max_iter=2000)
clf_full.fit(Xf_train, y_train)
p_full_test = clf_full.predict_proba(Xf_test)[:,1]
auc_full = float(roc_auc_score(y_test, p_full_test))
delta_auc = float(auc_full - auc_base)
auc_base, auc_full, delta_auc

### Tool (2026-02-23 00:09:33)

(0.8293325896446375, 0.853260237444391, 0.023927647799753426)

### Assistant (2026-02-23 00:10:02)

p_train = clf_base.predict_proba(Xb_train)[:,1]
W = p_train*(1-p_train)
Zt = Z_z[train_idx]  # n_train x2
res = y_train - p_train
U = Zt.T @ res
I = (Zt.T * W) @ Zt  # 2x2
# invert
...
T = float(U.T @ I_inv @ U)
p_chi2 = float(1-chi2.cdf(T, df=2))
T, p_chi2

### Tool (2026-02-23 00:10:06)

(568.7127770444116, 0.0)

### Assistant (2026-02-23 00:10:08)

p_chi2 = float(chi2.sf(T, df=2))
p_chi2

### Tool (2026-02-23 00:10:10)

3.2032406774294856e-124

### Assistant (2026-02-23 00:10:11)

null_reps=300
min_shift=50
N = N_kept_feat
shifts = rng.integers(min_shift, N-min_shift, size=null_reps, endpoint=False)
T_null = np.empty(null_reps, dtype=np.float64)

for k,s in enumerate(shifts):
    ths = np.roll(theta_new, int(s))
    Zs = np.column_stack([np.sin(ths), np.cos(ths)])
    mu_s = Zs[train_idx].mean(axis=0)
    sd_s = Zs[train_idx].std(axis=0, ddof=0)
    Zs_z = (Zs - mu_s)/sd_s
    Zst = Zs_z[train_idx]
    U_s = Zst.T @ res
    I_s = (Zst.T * W) @ Zst
    T_null[k] = float(U_s.T @ np.linalg.pinv(I_s) @ U_s)

T_p95 = float(np.quantile(T_null, 0.95))
T_p99 = float(np.quantile(T_null, 0.99))
emp_p = float((np.sum(T_null >= T) + 1) / (null_reps + 1))
T_p95, T_p99, emp_p, T_null.min(), T_null.max()

### Tool (2026-02-23 00:10:19)
...
out = {
  ...
  "auc":{
    "auc_baseline_test": auc_base,
    "auc_full_test": auc_full,
    "delta_auc": delta_auc
  },
  "score_test_real":{
    "T_chi2": T,
  ...
  "null":{
    "replicates": null_reps,
    "T_null_p95": T_p95,
    "T_null_p99": T_p99,
    "empirical_p_right_tail": emp_p
  },
