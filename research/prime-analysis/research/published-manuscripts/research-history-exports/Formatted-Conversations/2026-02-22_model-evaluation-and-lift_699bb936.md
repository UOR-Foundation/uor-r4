# Model Evaluation and Lift

- **Conversation ID:** `699bb936-dd4c-832e-8ea8-7f8ca46d625e`
- **Date:** 2026-02-22

---

### User (2026-02-22 21:19:38)
...
### Tool (2026-02-22 21:19:58)

(['x_kept', 'u_bp', 'theta_mod', 'R', 'dt', 'train_end', 'params', 'hashes'],
 8)

### Assistant (2026-02-22 21:20:04)
...
h=25
N_kept, train_end, theta.shape, R.shape

### Tool (2026-02-22 21:20:08)

(36592, 25614, (36592,), (36592,))

### Assistant (2026-02-22 21:20:10)

def sha1_arr(a: np.ndarray)->str:
    # ensure deterministic bytes: use C-order contiguous and view as bytes
    b=np.ascontiguousarray(a)
    return hashlib.sha1(b.view(np.uint8)).hexdigest()

sha_theta=sha1_arr(theta.astype(np.float64, copy=False))
sha_R=sha1_arr(R.astype(np.float64, copy=False))
sha_theta, sha_R

### Tool (2026-02-22 21:20:13)

('8b33c57df436d37179b13b1899bf86a723f0e98a',
 '5753ee7d59c7d155c4a1b9b5fba8912d557598f2')

### Assistant (2026-02-22 21:20:15)
...
valid_mask=(i_all>=2) & (i_all+h < N_kept)
idx= i_all[valid_mask]
n_total=len(idx)
train_mask = idx < train_end
test_mask = idx >= train_end
idx_train=idx[train_mask]
idx_test=idx[test_mask]
n_total, len(idx_train), len(idx_test), idx_train[:3], idx_test[:3]

### Tool (2026-02-22 21:20:23)

(36565, 25612, 10953, array([2, 3, 4]), array([25614, 25615, 25616]))

### Assistant (2026-02-22 21:20:25)

# Build target y for all valid rows
dR = R[idx + h] - R[idx]
y = (dR > 0).astype(int)
...
# Base features
X_base = np.column_stack([R[idx], R[idx-1], R[idx-2]])

# gate threshold q on train valid
absR_train = np.abs(R[idx_train])
q = np.quantile(absR_train, 0.20)
G = (np.abs(R[idx]) <= q).astype(int)

# sanity
q, G.mean(), G[train_mask].mean(), G[test_mask].mean(), y.mean(), y[train_mask].mean(), y[test_mask].mean()

### Tool (2026-02-22 21:20:33)
...
### Assistant (2026-02-22 21:20:36)

from sklearn.exceptions import ConvergenceWarning
import warnings
warnings.filterwarnings("ignore", category=ConvergenceWarning)

def fit_eval(X, y, train_mask_bool, test_mask_bool):
    X_train=X[train_mask_bool]
    y_train=y[train_mask_bool]
    X_test=X[test_mask_bool]
    y_test=y[test_mask_bool]
    ...
    return clf, scaler, p_test, auc, ll

# model matrices
X0 = X_base
X1 = np.column_stack([X_base, T])
X3 = np.column_stack([X_base, T, G, G*T])

# masks aligned to idx array
train_bool=train_mask
test_bool=test_mask

m0, sc0, p0, auc0, ll0 = fit_eval(X0, y, train_bool, test_bool)
m1, sc1, p1, auc1, ll1 = fit_eval(X1, y, train_bool, test_bool)
m3, sc3, p3, auc3, ll3 = fit_eval(X3, y, train_bool, test_bool)

auc0, auc1, auc3, ll0, ll1, ll3

### Tool (2026-02-22 21:20:47)
...
### Assistant (2026-02-22 21:20:50)

# overall deltas
delta_auc_10=auc1-auc0
delta_ll_10=ll1-ll0
delta_auc_31=auc3-auc1
delta_ll_31=ll3-ll1
delta_auc_30=auc3-auc0
delta_ll_30=ll3-ll0

# subset eval
y_test = y[test_bool]
G_test = G[test_bool]
def subset_auc(p_test, mask):
    y_s=y_test[mask]
    p_s=p_test[mask]
    if len(y_s)==0 or len(np.unique(y_s))<2:
        return None, len(y_s), float(y_s.mean()) if len(y_s)>0 else None
    return float(roc_auc_score(y_s, p_s)), len(y_s), float(y_s.mean())

low_mask = (G_test==1)
high_mask = (G_test==0)

m0_low_auc, n_low, pos_low = subset_auc(p0, low_mask)
m1_low_auc, _, _ = subset_auc(p1, low_mask)
m3_low_auc, _, _ = subset_auc(p3, low_mask)

m0_high_auc, n_high, pos_high = subset_auc(p0, high_mask)
m1_high_auc, _, _ = subset_auc(p1, high_mask)
m3_high_auc, _, _ = subset_auc(p3, high_mask)

delta_low_31 = (m3_low_auc - m1_low_auc) if (m3_low_auc is not None and m1_low_auc is not None) else None
delta_high_31 = (m3_high_auc - m1_high_auc) if (m3_high_auc is not None and m1_high_auc is not None) else None

(n_low, n_high, n_low+n_high, len(y_test), m0_low_auc, m0_high_auc, delta_low_31, delta_high_31)

### Tool (2026-02-22 21:20:59)
...
(2355,
 ...
 0.5450326988746718,
 0.7387104140961535,
 -0.015267446646915928,
 0.00017062796285460813)

### Assistant (2026-02-22 21:21:04)

rng=np.random.default_rng(12345)
T_train=T[train_bool]
G_train=G[train_bool]
Xbase_train=X_base[train_bool]
y_train=y[train_bool]

T_test=T[test_bool]
G_test=G[test_bool]
Xbase_test=X_base[test_bool]
y_test=y[test_bool]

# Precompute M1 auc to use
auc_M1=auc1

def fit_auc_perm(interaction_train):
    # build train/test feature matrices for perm M3
    Xtr=np.column_stack([Xbase_train, T_train, G_train, interaction_train])
    Xte=np.column_stack([Xbase_test, T_test, G_test, G_test*T_test])  # unshifted interaction on test
    scaler=StandardScaler()
...
for _ in range(n_perm):
    ...
    inter=G_train*T_shift
    aucp=fit_auc_perm(inter)
    delta_perm.append(aucp-auc_M1)
delta_perm=np.array(delta_perm)
perm_mean=float(delta_perm.mean())
perm_std=float(delta_perm.std(ddof=1))
delta_obs=float(delta_auc_31)
p_emp=float((np.sum(delta_perm>=delta_obs)+1)/(n_perm+1))
perm_mean, perm_std, delta_obs, p_emp, delta_perm[:5]

### Tool (2026-02-22 21:21:25)

(-0.0003139020607498248,
 0.0006572551917172078,
 -0.00036660849167280407,
 0.7024793388429752,
 array([-2.23066013e-05, -6.15182053e-04,  1.89656126e-04,  3.76344706e-04,
        -1.61381091e-05]))

### Assistant (2026-02-22 21:21:59)
