# Phase-transport Model Comparison

- **Conversation ID:** `699e9eda-a288-832d-8658-07fd1540641d`
- **Date:** 2026-02-25

---

### User (2026-02-25 02:03:58)
...
dt=float(canon['dt'])
assert len(x_kept)==N_kept
# interp u_raw onto x_kept using numpy.interp
x_grid=psi['x_grid']
u_raw=psi['psi_minus_x']
u_interp=np.interp(x_kept, x_grid, u_raw)
W=2000
ma=causal_moving_average(u_interp, W)
u_detrended=u_interp-ma
mu_train=u_detrended[:train_end].mean()
sd_train=u_detrended[:train_end].std(ddof=0)
z=(u_detrended-mu_train)/sd_train
# bandpass rfft
freqs=rfftfreq(N_kept, d=1.0)
lo,hi=0.003,0.009
Z=rfft(z)
mask=(freqs>=lo) & (freqs<=hi)
Z_filt=np.where(mask, Z, 0)
u_bp=irfft(Z_filt, n=N_kept)
a=hilbert(u_bp)
theta_mod=np.angle(a)
# map to [0,2pi)
theta_mod = np.mod(theta_mod, 2*np.pi)
theta_u=np.unwrap(theta_mod)
theta_mod[:5], theta_u[:5], mu_train, sd_train, dt, N_kept, train_end
...
### Assistant (2026-02-25 02:04:40)

R_fc=fc['R']
R_canon=canon['R']
float(np.max(np.abs(R_fc-R_canon))), R_fc.shape

### Tool (2026-02-25 02:04:43)

(0.0, (36592,))

### Assistant (2026-02-25 02:04:49)

h=50; ell=25
idx=np.arange(N_kept)
train_valid=(idx>=79)&(idx<train_end)&(idx+h<train_end)&(idx-ell>=0)
test_valid=(idx>=79)&(idx>=train_end)&(idx+h<N_kept)&(idx-ell>=0)
train_idx=idx[train_valid]
test_idx=idx[test_valid]
n_train=len(train_idx); n_test=len(test_idx)
# y_i
y = (R_fc[idx+h]-R_fc[idx] > 0).astype(int)  # for all idx where idx+h valid
y_train=y[train_idx]
y_test=y[test_idx]
pos_rate_test=float(y_test.mean())
# omega, concentration
D = theta_u[train_idx+h]-theta_u[train_idx]
m = np.mean(np.exp(1j*D))
omega = np.angle(m)/h
concentration_R = abs(np.mean(np.exp(1j*(D - h*omega))))
# delta and eps
def wrap_pi(x):
    return (x + np.pi)%(2*np.pi) - np.pi
delta = wrap_pi(theta_u[train_idx]-theta_u[train_idx-ell])
eps = wrap_pi(theta_u[train_idx+h]-theta_u[train_idx]-h*omega)
R_before = abs(np.mean(np.exp(1j*eps)))
R_before, omega, concentration_R, n_train, n_test, pos_rate_test

### Tool (2026-02-25 02:04:56)

---------------------------------------------------------------------------
IndexError                                Traceback (most recent call last)
Cell In[4], line 9
      7 n_train=len(train_idx); n_test=len(test_idx)
      8 # y_i
----> 9 y = (R_fc[idx+h]-R_fc[idx] > 0).astype(int)  # for all idx where idx+h valid
     10 y_train=y[train_idx]
     11 y_test=y[test_idx]

IndexError: index 36592 is out of bounds for axis 0 with size 36592

### Assistant (2026-02-25 02:04:57)
...
# theta_new
x_grid=psi['x_grid']; u_raw=psi['psi_minus_x']
u_interp=np.interp(x_kept, x_grid, u_raw)
W=2000
u_detrended=u_interp-causal_moving_average(u_interp,W)
mu_train=u_detrended[:train_end].mean()
sd_train=u_detrended[:train_end].std(ddof=0)
z=(u_detrended-mu_train)/sd_train
freqs=rfftfreq(N_kept,d=1.0)
lo,hi=0.003,0.009
Z=rfft(z); mask=(freqs>=lo)&(freqs<=hi)
u_bp=irfft(np.where(mask,Z,0), n=N_kept)
theta_mod=np.mod(np.angle(hilbert(u_bp)), 2*np.pi)
theta_u=np.unwrap(theta_mod)
...
D=theta_u[train_idx+h]-theta_u[train_idx]
omega=np.angle(np.mean(np.exp(1j*D)))/h
concentration_R=float(abs(np.mean(np.exp(1j*(D-h*omega)))))
delta=wrap_pi(theta_u[train_idx]-theta_u[train_idx-ell])
eps=wrap_pi(theta_u[train_idx+h]-theta_u[train_idx]-h*omega)
R_before=float(abs(np.mean(np.exp(1j*eps))))
n_train,n_test,pos_rate_test,omega,concentration_R,R_before

### Tool (2026-02-25 02:05:10)
...
### Assistant (2026-02-25 02:05:12)

def concentration_for_gamma(g):
    return abs(np.mean(np.exp(1j*(eps - g*delta))))
# golden section maximize on [-2,2]
def golden_max(f, a, b, tol=1e-6, max_iter=200):
    gr=(math.sqrt(5)+1)/2
    c=b-(b-a)/gr
    d=a+(b-a)/gr
    fc=f(c); fd=f(d)
...
gamma_hat, R_after, iters = golden_max(concentration_for_gamma, -2.0, 2.0, tol=1e-7, max_iter=400)
gamma_hat, R_after, iters

### Tool (2026-02-25 02:05:19)

(1.2495979916498405, 0.8243351117220649, 37)

### Assistant (2026-02-25 02:05:25)

X80=fc['X80']
mu_X80=fc['mu_X80_train']
sd_X80=fc['sd_X80_train']
X80z=(X80-mu_X80)/sd_X80

def add_phase_cols(X_base, theta_vals, train_rows, test_rows):
    s=np.sin(theta_vals); c=np.cos(theta_vals)
    P=np.column_stack([s,c])
    mu=P[train_rows].mean(axis=0)
    sd=P[train_rows].std(axis=0, ddof=0)
    Pz=(P-mu)/sd
    Xtr=np.hstack([X_base[train_rows], Pz[train_rows]])
    Xte=np.hstack([X_base[test_rows], Pz[test_rows]])
    return Xtr, Xte

def fit_eval(Xtr, ytr, Xte, yte):
    clf=LogisticRegression(penalty='l2', solver='lbfgs', max_iter=2000, n_jobs=None)
    clf.fit(Xtr, ytr)
    p=clf.predict_proba(Xte)[:,1]
    auc=float(roc_auc_score(yte, p))
    ll=float(log_loss(yte, p, labels=[0,1]))
    return auc, ll

train_rows=train_idx
test_rows=test_idx
# M0
auc0,ll0 = fit_eval(X80z[train_rows], y_train, X80z[test_rows], y_test)

# phase arrays on full idx
theta_i_full=theta_mod
theta_hat_full=wrap2(theta_i_full + h*omega)
theta_hat_corr_full=wrap2(theta_i_full + h*omega + gamma_hat*wrap_pi(theta_u - np.roll(theta_u, ell)))  # but needs theta_u[i-ell]; roll works but for i<ell wrong
# fix: compute delta_full = wrap_pi(theta_u - theta_u_shift)
theta_u_shift=np.empty_like(theta_u)
theta_u_shift[:ell]=theta_u[:ell]  # placeholder
theta_u_shift[ell:]=theta_u[:-ell]
delta_full=wrap_pi(theta_u - theta_u_shift)
theta_hat_corr_full=wrap2(theta_i_full + h*omega + gamma_hat*delta_full)
theta_oracle_full=np.empty_like(theta_i_full)
theta_oracle_full[:-h]=theta_i_full[h:]
theta_oracle_full[-h:]=np.nan

# M1
Xtr,Xte = add_phase_cols(X80z, theta_i_full, train_rows, test_rows)
auc1,ll1 = fit_eval(Xtr, y_train, Xte, y_test)
# M2
Xtr,Xte = add_phase_cols(X80z, theta_hat_full, train_rows, test_rows)
auc2,ll2 = fit_eval(Xtr, y_train, Xte, y_test)
# M3
Xtr,Xte = add_phase_cols(X80z, theta_hat_corr_full, train_rows, test_rows)
auc3,ll3 = fit_eval(Xtr, y_train, Xte, y_test)
# M4 (oracle uses theta_mod[i+h])
# Need theta_oracle for train/test indices valid; those already ensure i+h valid
theta_oracle_vals = theta_mod  # use shifted indexing inside add_phase? simplest create array with oracle at i = theta_mod[i+h]
theta_oracle_arr = np.empty_like(theta_mod)
theta_oracle_arr[:-h]=theta_mod[h:]
theta_oracle_arr[-h:]=theta_mod[-h:]  # unused
Xtr,Xte = add_phase_cols(X80z, theta_oracle_arr, train_rows, test_rows)
auc4,ll4 = fit_eval(Xtr, y_train, Xte, y_test)

auc0,ll0,auc1,auc2,auc3,auc4

### Tool (2026-02-25 02:05:40)
...
### Assistant (2026-02-25 02:06:11)

(ll0,ll1,ll2,ll3,ll4)

### Tool (2026-02-25 02:06:12)
...
out={
 ...
 "params":{
   "omega": float(omega),
   "concentration_R": float(concentration_R),
   "gamma": float(gamma_hat),
   "R_before": float(R_before),
   "R_after": float(R_after)
 },
 "metrics_test":{
   "M0_AR80":{"auc": float(auc0), "logloss": float(ll0)},
   "M1_AR80_theta_i":{"auc": float(auc1), "logloss": float(ll1)},
   "M2_AR80_theta_hat":{"auc": float(auc2), "logloss": float(ll2)},
   "M3_AR80_theta_hat_corr":{"auc": float(auc3), "logloss": float(ll3)},
   "M4_AR80_theta_oracle":{"auc": float(auc4), "logloss": float(ll4)}
 },
 "comparisons":{
   "auc_corr_minus_hat": float(auc3-auc2),
   "auc_oracle_minus_corr": float(auc4-auc3),
   "logloss_corr_minus_hat": float(ll3-ll2),
   "logloss_oracle_minus_corr": float(ll4-ll3)
 },
