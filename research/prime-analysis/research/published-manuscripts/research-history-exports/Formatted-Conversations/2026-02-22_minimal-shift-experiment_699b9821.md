# Minimal Shift Experiment

- **Conversation ID:** `699b9821-a8b8-8327-8f26-a26e81a45f67`
- **Date:** 2026-02-22

---

### User (2026-02-22 18:58:29)
...
# interpolate
u_interp = np.interp(t_uniform, t_raw, psi_minus_x_grid.astype(np.float64))
...
### Tool (2026-02-22 18:59:20)

(0.00012297217348543395, 36592, 25614)

### Assistant (2026-02-22 18:59:23)
...
u_std = (u_detr - mu) / (sig if sig>0 else 1.0)
...
start_idx = np.searchsorted(primes, x_kept, side='left')
end_idx = np.searchsorted(primes, end_x, side='left')
P = (end_idx - start_idx).astype(np.float64)
...
len(train_idx), len(test_idx), train_idx[-1], test_idx[0]

### Tool (2026-02-22 18:59:38)

(25514, 10878, 25513, 25614)

### Assistant (2026-02-22 18:59:39)

dR = R[valid + h] - R[valid]
y_sign = (dR > 0).astype(int)

def build_features(Ri, th):
    # base: Ri, cos/sin k=1..K
    cols = [Ri.reshape(-1,1)]
    for k in range(1, K+1):
        cols.append(np.cos(k*th).reshape(-1,1))
        cols.append(np.sin(k*th).reshape(-1,1))
    X_base = np.hstack(cols)
    # interactions
    cols_int = []
    for k in range(1, K+1):
        cols_int.append((Ri*np.cos(k*th)).reshape(-1,1))
        cols_int.append((Ri*np.sin(k*th)).reshape(-1,1))
    X_full = np.hstack([X_base, np.hstack(cols_int)])
    return X_base, X_full

def fit_eval(theta_used):
    Ri = R[valid]
    th = theta_used[valid]
    Xb, Xf = build_features(Ri, th)
    # fit
    rb = Ridge(alpha=1.0, fit_intercept=True, random_state=12345)
    rf = Ridge(alpha=1.0, fit_intercept=True, random_state=12345)
    rb.fit(Xb[train_idx], dR[train_idx])
    rf.fit(Xf[train_idx], dR[train_idx])
    # preds
    pb = rb.predict(Xb[test_idx])
    pf = rf.predict(Xf[test_idx])
    # metrics
    mse_b = mean_squared_error(dR[test_idx], pb)
    mse_f = mean_squared_error(dR[test_idx], pf)
    r2_b = r2_score(dR[test_idx], pb)
    r2_f = r2_score(dR[test_idx], pf)
    # sign auc
    y_test = y_sign[test_idx]
    # handle degenerate
    def auc_safe(y, s):
        if len(np.unique(y))<2:
            return None
        return float(roc_auc_score(y, s))
    auc_b = auc_safe(y_test, pb)
    auc_f = auc_safe(y_test, pf)
    # interaction norms
    inter_coef = rf.coef_[-2*K:]
    inter_norm = float(np.linalg.norm(inter_coef))
    # deltas
    deltas = {
        "mse": float(mse_f - mse_b),
        "r2": float(r2_f - r2_b),
        "sign_auc": None if (auc_b is None or auc_f is None) else float(auc_f - auc_b)
    }
    return {
        "BASE": {"mse_test": float(mse_b), "r2_test": float(r2_b), "sign_auc_test": auc_b},
        "FULL": {"mse_test": float(mse_f), "r2_test": float(r2_f), "sign_auc_test": auc_f},
        "incremental_lift_FULL_minus_BASE": deltas,
        "interaction_block_coef_l2": inter_norm
    }

shift_angle = 2*np.pi*(shift_bins/B)
theta0 = theta.copy()
theta16 = np.mod(theta + shift_angle, 2*np.pi)

res0 = fit_eval(theta0)
res16 = fit_eval(theta16)

pos_rates = {
    "train_pos_rate": float(y_sign[train_idx].mean()),
    "test_pos_rate": float(y_sign[test_idx].mean())
}
res0, res16, pos_rates

### Tool (2026-02-22 18:59:53)

({'BASE': {'mse_test': 0.20911503573042478,
   'r2_test': 0.6057846196602606,
   'sign_auc_test': 0.8855667864044212},
  'FULL': {'mse_test': 0.2094167422948782,
   'r2_test': 0.6052158543983962,
   'sign_auc_test': 0.8854914383251972},
  'incremental_lift_FULL_minus_BASE': {'mse': 0.00030170656445341337,
 ...
 {'BASE': {'mse_test': 0.20911503573042478,
  ...
  'incremental_lift_FULL_minus_BASE': {'mse': 0.00030170656445341337,
   ...
   'sign_auc': -7.534807922404685e-05},
  'interaction_block_coef_l2': 0.05110553539968009},
 {'train_pos_rate': 0.49431684565336675, 'test_pos_rate': 0.49963228534657106})

### Assistant (2026-02-22 18:59:56)
