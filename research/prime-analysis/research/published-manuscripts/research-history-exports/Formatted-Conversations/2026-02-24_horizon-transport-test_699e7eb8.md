# Horizon transport test

- **Conversation ID:** `699e7eb8-df30-832f-83fa-685c19f5a733`
- **Date:** 2026-02-24

---

### User (2026-02-24 23:46:53)
...
### Assistant (2026-02-24 23:46:57)

import numpy as np, hashlib, json, math, os, textwrap, sys
from pathlib import Path
import numpy as np
import scipy
from scipy import signal, interpolate

### Assistant (2026-02-24 23:47:17)
...
list(fc.files)[:30], list(canon.files)[:30], (fc['dt'] if 'dt' in fc.files else None)

### Tool (2026-02-24 23:47:26)
...
### Assistant (2026-02-24 23:47:28)

dt_fc=float(fc['dt']); N_kept=int(fc['N_kept']); train_end=int(fc['train_end'])
dt_c=float(canon['dt']); N_kept_c=len(canon['R']); train_end_c=int(canon['train_end'])
dt_fc, N_kept, train_end, dt_c, N_kept_c, train_end_c

### Tool (2026-02-24 23:47:30)

(0.00012297217348543395, 36592, 25614, 0.00012297217348543395, 36592, 25614)

### Assistant (2026-02-24 23:47:32)
...
def compute_theta_new(psi_path, L=131072, x_min=1e5, x_max=9e6, W=2000, bandpass=(0.003,0.009), train_end=25614):
    d=np.load(psi_path, allow_pickle=False)
    x_grid=d['x_grid']; psi_minus_x=d['psi_minus_x']
    t_raw=np.log(x_grid.astype(float))
    t0=float(t_raw.min()); t1=float(t_raw.max())
    t_uniform=np.linspace(t0, t1, L)
    # interpolate
    f_interp=interpolate.interp1d(t_raw, psi_minus_x.astype(float), kind='linear', bounds_error=False, fill_value="extrapolate")
    u_full=f_interp(t_uniform)
    x_full=np.exp(t_uniform)
    keep=(x_full>=x_min) & (x_full<=x_max)
    x_kept=x_full[keep]
    u_raw=u_full[keep]
    ...
    dt=(t_uniform[1]-t_uniform[0])  # uniform dt in t-space before keep
    # dt between kept indices? should be same as uniform dt
    return dict(theta_mod=theta, u_bp=u_bp, x_kept=x_kept, dt=dt, N_kept=N, train_end=train_end,
                mu_u_train=float(mu), sd_u_train=float(sd), t0=t0, t1=t1, L=L)

theta_new_obj=compute_theta_new(psi_path, bandpass=(0.003,0.009), train_end=train_end)
theta_new_obj['dt'], theta_new_obj['N_kept'], theta_new_obj['train_end'], sha1_arr(theta_new_obj['x_kept'])[:10], sha1_arr(canon['x_kept'])[:10]

### Tool (2026-02-24 23:47:59)

(0.00012297217348543395, 36592, 25614, 'c455fedeff', 'c455fedeff')

### Assistant (2026-02-24 23:48:01)
...
# avoid division by 0
sd_X80_safe=np.where(sd_X80==0, 1.0, sd_X80)
X80z=(X80-mu_X80)/sd_X80_safe
R=fc['R'].astype(float)
theta_new=theta_new_obj['theta_mod']

def phase_map_for_h(h, B=12):
    N=len(R)
    idx_train=np.where((np.arange(N)>=79) & (np.arange(N)<train_end) & (np.arange(N)+h<train_end))[0]
    idx_test=np.where((np.arange(N)>=79) & (np.arange(N)>=train_end) & (np.arange(N)+h<N))[0]
    y_train=(R[idx_train+h]-R[idx_train] > 0).astype(int)
    y_test=(R[idx_test+h]-R[idx_test] > R[idx_test]).astype(int)
    # Fit logistic regression baseline on TRAIN-valid(h)
    model=LogisticRegression(solver='lbfgs', max_iter=2000)
    model.fit(X80z[idx_train], y_train)
    p_test=model.predict_proba(X80z[idx_test])[:,1]
    r=y_test - p_test
    # bin theta
    theta=theta_new[idx_test]
    bins=(np.floor(theta/(2*np.pi)*B)).astype(int)
    bins=np.clip(bins,0,B-1)
    counts=np.bincount(bins, minlength=B).astype(int)
    means=np.zeros(B, dtype=float)
    for b in range(B):
        if counts[b]>0:
            means[b]=r[bins==b].mean()
        else:
            means[b]=0.0
    return dict(counts=counts.tolist(), means=means.tolist(),
                n_train=int(len(idx_train)), n_test=int(len(idx_test)),
                y_train_balance=float(y_train.mean()) if len(y_train)>0 else None,
                y_test_balance=float(y_test.mean()) if len(y_test)>0 else None)

maps={}
for h in [25,50,100]:
    maps[h]=phase_map_for_h(h,B=12)
maps[25]['n_train'], maps[25]['n_test'], maps[50]['n_train'], maps[100]['n_test']

### Tool (2026-02-24 23:48:20)

(25510, 10953, 25485, 10878)

### Assistant (2026-02-24 23:48:44)

def corr_and_l2(v1,v2):
...
def best_shift(v1,v2):
    v1=np.asarray(v1,float); v2=np.asarray(v2,float)
    best=None
    for k in range(len(v1)):
        v2s=np.roll(v2, k)
        c,l2=corr_and_l2(v1,v2s)
        # treat nan corr as -inf
        c_cmp=-1e9 if np.isnan(c) else c
        if best is None or c_cmp>best[0] or (abs(c_cmp-best[0])<1e-12 and l2<best[2]-1e-12):
            best=(c_cmp, k, l2, c, l2)
    # best stores c_cmp, k, l2
    _,k,_,c,l2=best
    # also raw
    c_raw,l2_raw=corr_and_l2(v1,v2)
    return dict(corr_raw=c_raw, L2_raw=l2_raw, k_star=int(k), corr_best=c, L2_best=l2)

comparisons={}
comparisons['25_50']=best_shift(maps[25]['means'], maps[50]['means'])
comparisons['50_100']=best_shift(maps[50]['means'], maps[100]['means'])
comparisons['25_100']=best_shift(maps[25]['means'], maps[100]['means'])
comparisons

### Tool (2026-02-24 23:48:53)

{'25_50': {'corr_raw': 0.8745857018419727,
  'L2_raw': 0.1543803091962963,
  'k_star': 11,
  'corr_best': 0.8855043339605517,
  'L2_best': 0.14761373451241033},
 '50_100': {'corr_raw': 0.8136807404535853,
  'L2_raw': 0.21633674157783903,
  'k_star': 1,
  'corr_best': 0.958506871680074,
  'L2_best': 0.1146564768582043},
 '25_100': {'corr_raw': 0.9453482987057229,
  'L2_raw': 0.13274086294437448,
  'k_star': 0,
  'corr_best': 0.9453482987057229,
  'L2_best': 0.13274086294437448}}

### Assistant (2026-02-24 23:48:54)

theta_sha1=sha1_arr(theta_new)
theta_sha1

### Tool (2026-02-24 23:48:56)

'c888f75abc219749cde8f870c119a6dcd98ec2b4'

### Assistant (2026-02-24 23:48:57)

[min(maps[h]['counts']) for h in [25,50,100]], [maps[h]['counts'] for h in [25,50,100]][0]

### Tool (2026-02-24 23:48:59)

([872, 872, 872], [923, 921, 889, 885, 912, 872, 964, 899, 931, 946, 886, 925])

### Assistant (2026-02-24 23:49:00)
