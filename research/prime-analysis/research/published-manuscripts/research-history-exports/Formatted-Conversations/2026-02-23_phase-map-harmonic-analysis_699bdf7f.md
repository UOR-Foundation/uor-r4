# Phase-map Harmonic Analysis

- **Conversation ID:** `699bdf7f-9b74-8328-8313-227a13f91064`
- **Date:** 2026-02-23

---

### User (2026-02-23 00:02:59)
...
### Assistant (2026-02-23 00:03:16)

canon=np.load(canon_path, allow_pickle=True)
canon.files

### Tool (2026-02-23 00:03:17)

['x_kept', 'u_bp', 'theta_mod', 'R', 'dt', 'train_end', 'params', 'hashes']

### Assistant (2026-02-23 00:03:19)

dt_c=float(canon['dt']); N_kept=int(canon['x_kept'].shape[0]); train_end=int(canon['train_end'])
dt_f=float(feat['dt']); N_kept_f=int(feat['N_kept']); train_end_f=int(feat['train_end'])
dt_c, N_kept, train_end, dt_f, N_kept_f, train_end_f

### Tool (2026-02-23 00:03:21)

(0.00012297217348543395, 36592, 25614, 0.00012297217348543395, 36592, 25614)

### Assistant (2026-02-23 00:03:22)

params=canon['params'].item()
params

### Tool (2026-02-23 00:03:25)
...
Nkept=len(u_raw)
dt, Nkept, x_kept[0], x_kept[-1]

### Tool (2026-02-23 00:03:47)

(0.00012297217348543395, 36592, 100009.83380914277, 8999671.172393441)

### Assistant (2026-02-23 00:03:48)

int(0.7*Nkept)

### Tool (2026-02-23 00:03:50)

25614

### Assistant (2026-02-23 00:03:52)
...
U=fft.rfft(u_z)
freq=fft.rfftfreq(Nkept, d=1.0)  # cycles per sample
mask=(freq>=f_lo) & (freq<=f_hi)
U_f=np.where(mask, U, 0)
u_bp=fft.irfft(U_f, n=Nkept)
theta_new=np.mod(np.angle(signal.hilbert(u_bp)), 2*np.pi)
sha_theta_new=sha1_arr(theta_new)
sha_theta_new, theta_new[:3]

### Tool (2026-02-23 00:03:58)

('c888f75abc219749cde8f870c119a6dcd98ec2b4',
 array([1.32477148, 1.3581499 , 1.39142863]))

### Assistant (2026-02-23 00:04:00)

R=feat['R'].astype(np.float64)
sha_R=sha1_arr(R)
sha_R, canon['hashes'].item()

### Tool (2026-02-23 00:04:02)
...
### Assistant (2026-02-23 00:04:03)

theta_c=feat['theta'].astype(np.float64)
sha_theta_c=sha1_arr(theta_c)
sha_theta_c

### Tool (2026-02-23 00:04:05)

'8b33c57df436d37179b13b1899bf86a723f0e98a'

### Assistant (2026-02-23 00:04:06)

h=25
y_full = (R[h:] - R[:-h] > 0).astype(np.int8)
y_full.shape, feat['mask_train_lag79'].shape, feat['X80'].shape

### Tool (2026-02-23 00:04:10)

((36567,), (36592,), (36592, 80))

### Assistant (2026-02-23 00:04:12)

mask_train=feat['mask_train_lag79'].astype(bool)
mask_test=feat['mask_test_lag79'].astype(bool)
mask_train.sum(), mask_test.sum(), mask_train[-30:].sum(), mask_test[-30:].sum()

### Tool (2026-02-23 00:04:17)

(25510, 10953, 0, 5)

### Assistant (2026-02-23 00:04:18)

mask_test[-30:], mask_test[-25:].sum()

### Tool (2026-02-23 00:04:21)
...
### Assistant (2026-02-23 00:04:22)

y=np.zeros(Nkept, dtype=np.int8)
y[:Nkept-h]=y_full
# verify masks do not include invalid region
assert (mask_train[Nkept-h:]==False).all()
assert (mask_test[Nkept-h:]==False).all()
# standardize X80 using cached mu/sd
X80=feat['X80'].astype(np.float64)
muX=feat['mu_X80_train'].astype(np.float64)
sdX=feat['sd_X80_train'].astype(np.float64)
X80z=(X80-muX)/sdX
# build design matrices
def fit_auc(theta_source):
    sin1=np.sin(theta_source); cos1=np.cos(theta_source)
    sin2=np.sin(2*theta_source); cos2=np.cos(2*theta_source)
    # Model1
    X1=np.column_stack([X80z, sin1, cos1])
    X2=np.column_stack([X80z, sin1, cos1, sin2, cos2])
    # train/test
    X1_tr,X1_te=X1[mask_train], X1[mask_test]
    X2_tr,X2_te=X2[mask_train], X2[mask_test]
    y_tr,y_te=y[mask_train], y[mask_test]
    # logistic
    clf1=LogisticRegression(penalty='l2', C=1.0, solver='lbfgs', max_iter=2000, n_jobs=None)
    clf2=LogisticRegression(penalty='l2', C=1.0, solver='lbfgs', max_iter=2000, n_jobs=None)
    clf1.fit(X1_tr, y_tr)
    clf2.fit(X2_tr, y_tr)
    p1=clf1.predict_proba(X1_te)[:,1]
    p2=clf2.predict_proba(X2_te)[:,1]
    auc1=roc_auc_score(y_te, p1)
    auc2=roc_auc_score(y_te, p2)
    return float(auc1), float(auc2), float(auc2-auc1)

auc1_c, auc2_c, d_c = fit_auc(theta_c)
auc1_n, auc2_n, d_n = fit_auc(theta_new)
auc1_c, auc2_c, d_c, auc1_n, auc2_n, d_n

### Tool (2026-02-23 00:04:33)

(0.80526957277557,
 0.8027091817371821,
 -0.002560391038387899,
 0.8121783173033673,
 0.812129569543947,
 -4.874775942032006e-05)

### Assistant (2026-02-23 00:05:08)
