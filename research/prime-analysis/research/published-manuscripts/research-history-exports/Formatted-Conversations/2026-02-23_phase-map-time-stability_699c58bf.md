# Phase-map time stability

- **Conversation ID:** `699c58bf-8274-832e-8f49-a7a84253dcf9`
- **Date:** 2026-02-23

---

### User (2026-02-23 08:40:44)
...
### Assistant (2026-02-23 08:40:47)

import numpy as np, json, hashlib, math, os
...
def sha1_arr(a: np.ndarray)->str:
    # use C-contiguous bytes
    return sha1_bytes(np.ascontiguousarray(a).view(np.uint8).tobytes())
...
list(fc.files)[:20], list(canon.files)[:20], list(psi.files)[:10]

### Tool (2026-02-23 08:41:42)
...
### Assistant (2026-02-23 08:41:45)

# extract
R_fc=fc['R']
dt_fc=float(fc['dt'])
N_kept_fc=int(fc['N_kept'])
train_end_fc=int(fc['train_end'])

x_kept=canon['x_kept']
R_canon=canon['R']
dt_canon=float(canon['dt'])
train_end_canon=int(canon['train_end'])
N_kept_canon=len(x_kept)

# hashes from history file
expected = {
 "x_kept":"c455fedeff29bb2d0b2c81864970c09638720baf",
 "theta_mod":"8b33c57df436d37179b13b1899bf86a723f0e98a",
 "R":"5753ee7d59c7d155c4a1b9b5fba8912d557598f2",
...
hash_x=sha1_arr(x_kept)
hash_R=sha1_arr(R_canon)
hash_theta=sha1_arr(canon['theta_mod'])
hash_u=sha1_arr(canon['u_bp'])
(dt_canon,N_kept_canon,train_end_canon,hash_x,hash_R,hash_theta,hash_u)

### Tool (2026-02-23 08:42:00)
...
### Assistant (2026-02-23 08:42:02)

hash_R_fc=sha1_arr(R_fc)
(dt_fc,N_kept_fc,train_end_fc,hash_R_fc, np.allclose(R_fc,R_canon), np.max(np.abs(R_fc-R_canon)))

### Tool (2026-02-23 08:42:04)
...
# interpolate
f_interp=interp1d(t_raw, psi_minus_x.astype(float), kind='linear', fill_value="extrapolate", assume_sorted=True)
u_full = f_interp(t_uniform)
x_full = np.exp(t_uniform)
# keep range
x_min=1e5; x_max=9e6
kept = np.where((x_full>=x_min) & (x_full<=x_max))[0]
x_kept_new = x_full[kept]
u_raw = u_full[kept]
N_kept=len(kept)
...
W=2000
# compute via cumulative sum for efficiency
cs=np.cumsum(np.insert(u_raw,0,0.0))
# window lengths vary at start
...
mu=float(u_train.mean()); sd=float(u_train.std(ddof=0))
u_z = (u_detr - mu)/sd

# bandpass via rfft/irfft with nonnegative freqs
U=rfft(u_z)
freq = np.fft.rfftfreq(N_kept, d=1.0)  # cycles per sample
f_lo, f_hi = 0.003, 0.009
mask=(freq>=f_lo) & (freq<=f_hi)
U_bp = U*mask
u_bp_new = irfft(U_bp, n=N_kept)

theta_new = np.mod(np.angle(hilbert(u_bp_new)), 2*np.pi)
theta_sha1=sha1_arr(theta_new.astype(np.float64))
(dt, N_kept, train_end, sha1_arr(x_kept_new), theta_sha1)[:4], theta_sha1

### Tool (2026-02-23 08:42:27)
...
### Assistant (2026-02-23 08:42:29)

X80=fc['X80']
X80.shape

### Tool (2026-02-23 08:42:34)

(36592, 80)

### Assistant (2026-02-23 08:42:36)

muX=fc['mu_X80_train']; sdX=fc['sd_X80_train']
# compute train stats from X80 itself on mask_train_lag79?
mask_train_lag79=fc['mask_train_lag79'].astype(bool)
train_X=X80[mask_train_lag79]
(train_X.mean(), train_X.std(), np.max(np.abs(train_X.mean(axis=0)-muX)), np.max(np.abs(train_X.std(axis=0)-sdX)))

### Tool (2026-02-23 08:42:42)

(-0.023354748, 0.4933867, 0.0, 0.0)

### Assistant (2026-02-23 08:42:43)

h=50
i = np.arange(N_kept)
train_valid = (i>=79) & (i<train_end) & (i+h<train_end)
test_valid  = (i>=79) & (i>=train_end) & (i+h<N_kept)

y = (R_fc[i+h] - R_fc[i] > 0).astype(np.int8)
# y defined for i up to N_kept-h-1; but indexing i+h uses full, for i beyond N_kept-h => wraps error. so restrict
# We'll make y_full with length N_kept and ignore invalid
y_full = np.zeros(N_kept, dtype=np.int8)
valid_for_y = (i+h<N_kept)
y_full[valid_for_y] = (R_fc[i[valid_for_y]+h] - R_fc[i[valid_for_y]] > 0).astype(np.int8)

y_train = y_full[train_valid]
X_train = X80[train_valid]

# Fit logistic
clf = LogisticRegression(penalty='l2', C=1.0, solver='lbfgs', max_iter=2000, random_state=12345)
clf.fit(X_train, y_train)

p_test = clf.predict_proba(X80[test_valid])[:,1]
y_test = y_full[test_valid].astype(np.float64)
r_test = y_test - p_test

len(p_test), y_test.mean(), p_test.mean()

### Tool (2026-02-23 08:42:59)

---------------------------------------------------------------------------
IndexError                                Traceback (most recent call last)
Cell In[7], line 6
      3 train_valid = (i>=79) & (i<train_end) & (i+h<train_end)
      4 test_valid  = (i>=79) & (i>=train_end) & (i+h<N_kept)
----> 6 y = (R_fc[i+h] - R_fc[i] > 0).astype(np.int8)
      7 # y defined for i up to N_kept-h-1; but indexing i+h uses full, for i beyond N_kept-h => wraps error. so restrict
      8 # We'll make y_full with length N_kept and ignore invalid
      9 y_full = np.zeros(N_kept, dtype=np.int8)

IndexError: index 36592 is out of bounds for axis 0 with size 36592

### Assistant (2026-02-23 08:43:00)
...
train_valid = (i>=79) & (i<train_end_canon) & (i+h<train_end_canon)
test_valid  = (i>=79) & (i>=train_end_canon) & (i+h<N_kept)

y_full = np.full(N_kept, -1, dtype=np.int8)
valid_for_y = (i+h<N_kept)
y_full[valid_for_y] = (R[i[valid_for_y]+h] - R[i[valid_for_y]] > 0).astype(np.int8)

# sanity: y_full should be valid on train_valid/test_valid
assert np.all(y_full[train_valid] >= 0)
assert np.all(y_full[test_valid] >= 0)

X_train = X80[train_valid]
y_train = y_full[train_valid]

clf = LogisticRegression(penalty='l2', C=1.0, solver='lbfgs', max_iter=5000, random_state=12345)
clf.fit(X_train, y_train)
...
(len(p_test), float(y_test.mean()), float(p_test.mean()), float(np.mean(r_test)))

### Tool (2026-02-23 08:43:13)

(10928, 0.5058565153733529, 0.5042616073559959, 0.0015949080173569465)

### Assistant (2026-02-23 08:43:21)

import numpy as np, math

B=12
theta=theta_new
bins = np.floor(theta/(2*np.pi)*B).astype(int)
bins = np.clip(bins,0,B-1)

test_idx = np.where(test_valid)[0]
assert len(test_idx)==len(r_test)
# full map
def phase_map(indices, residuals):
    ...
    for idx,res in zip(indices, residuals):
        ...
        counts[b]+=1
        sums[b]+=float(res)
    means=np.zeros(B, dtype=float)
    means[:] = np.nan
    nz=counts>0
    means[nz]=sums[nz]/counts[nz]
    return counts, means

full_counts, full_means = phase_map(test_idx, r_test)

# blocks
blocks_idx = np.array_split(test_idx, 5)
blocks=[]
for bi, idx_block in enumerate(blocks_idx):
    # residual slice corresponding:
    # r_test aligned with test_idx order. We'll map by positions.
    # find positions in test_idx:
    pos_start = np.searchsorted(test_idx, idx_block[0])
    pos_end = pos_start + len(idx_block)
    res_block = r_test[pos_start:pos_end]
    c,m = phase_map(idx_block, res_block)
    # metrics: corr between m and full_means
    # handle nan
    mask = np.isfinite(m) & np.isfinite(full_means)
    if mask.sum()>=2 and np.std(m[mask])>0 and np.std(full_means[mask])>0:
        corr = float(np.corrcoef(m[mask], full_means[mask])[0,1])
    else:
        corr = float('nan')
    diff = m - full_means
    diff_mask = np.isfinite(diff)
    L2 = float(np.sqrt(np.sum((diff[diff_mask])**2)))
    # best circular shift
    best_k=0
    best_L2=math.inf
    for k in range(B):
        m_shift = np.roll(m, k)
        d = m_shift - full_means
        dm = np.isfinite(d)
        L2k = float(np.sqrt(np.sum((d[dm])**2)))
        if L2k < best_L2:
            best_L2=L2k; best_k=k
    amp_std = float(np.nanstd(m))
    blocks.append((bi,len(idx_block),c,m,corr,L2,best_k,best_L2,amp_std))

full_counts.sum(), len(test_idx), full_counts, full_means

### Tool (2026-02-23 08:43:52)

(10928,
 10928,
 array([911, 921, 889, 885, 912, 872, 964, 899, 931, 946, 886, 912]),
 array([ 0.02566099,  0.04294804,  0.12625394,  0.20336136,  0.19155086,
         0.08743568, -0.00126088, -0.04210099, -0.13704467, -0.21388453,
        -0.20224803, -0.04438225]))

### Assistant (2026-02-23 08:43:54)

corrs=[b[4] for b in blocks]
corrs_clean=[c for c in corrs if np.isfinite(c)]
mean_corr=float(np.mean(corrs_clean)) if corrs_clean else float('nan')
min_corr=float(np.min(corrs_clean)) if corrs_clean else float('nan')
max_corr=float(np.max(corrs_clean)) if corrs_clean else float('nan')
mean_best_L2=float(np.mean([b[7] for b in blocks]))
amp_stds=[b[8] for b in blocks]
amp_range=[float(np.min(amp_stds)), float(np.max(amp_stds))]
mean_best_L2, mean_corr, amp_range, corrs_clean

### Tool (2026-02-23 08:44:03)

(0.22678539889247432,
 0.9084772701799937,
 [0.11922424049274674, 0.17994019632377553],
 [0.933085636621226,
  0.9172583659368323,
  0.867121714640261,
  0.9135537390063309,
...
### Assistant (2026-02-23 08:44:05)

def map_to_list(counts, means):
    out=[]
    for b in range(B):
        out.append({"bin": int(b), "count": int(counts[b]), "mean_residual": float(means[b])})
    return out

full_map = map_to_list(full_counts, full_means)

blocks_out=[]
for bi,n,c,m,corr,L2,bk,bl2,amp in blocks:
    blocks_out.append({
        "block_index": int(bi),
        "n": int(n),
        "map": map_to_list(c,m),
        "corr_to_full": float(corr),
        "L2_to_full": float(L2),
        "best_shift": {"k_star": int(bk), "L2_best": float(bl2)},
        "amplitude_std": float(amp)
    })
...
out = {
    ...
    "summary":{
        ...
        "min_corr": min_corr,
        "max_corr": max_corr,
        "mean_best_shift_L2": mean_best_L2,
        "amplitude_std_range": amp_range
    },
    "notes": f"Fingerprint-locked dt/N_kept/train_end to canonical A4 (dt={dt_canon}, N_kept={N_kept_canon}, train_end={train_end_canon}); x_kept SHA1 matched canonical. theta recomputed with bandpass [0.003,0.009] via rFFT mask + scipy.hilbert; desticky target y=1{{R[i+50]-R[i]>0}}; AR80 logistic fit once on TRAIN-valid (i>=79,i<train_end,i+50<train_end) and applied to all TEST-valid. L2 is Euclidean norm over 12-bin mean residual vector; best_shift uses np.roll by k in [0..11]."
}
print(json.dumps(out, indent=2))

### Tool (2026-02-23 08:44:22)
