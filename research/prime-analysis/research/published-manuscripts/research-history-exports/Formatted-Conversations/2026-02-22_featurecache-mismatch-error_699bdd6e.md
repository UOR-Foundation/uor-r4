# Featurecache Mismatch Error

- **Conversation ID:** `699bdd6e-a8e4-8326-b91c-7dd0cde6143d`
- **Date:** 2026-02-22

---

### User (2026-02-22 23:54:12)
...
### Assistant (2026-02-22 23:54:15)

import os, hashlib, numpy as np, math, json, textwrap, sys
from pathlib import Path
fc_path = Path("/mnt/data/A4_FEATURECACHE_h25_AR80_AR40_theta_omega_masks_v1.npz")
psi_path = Path("/mnt/data/chebyshev_psi.npz")
(fc_path.exists(), psi_path.exists(), fc_path.stat().st_size if fc_path.exists() else None)

### Tool (2026-02-22 23:54:51)

(True, True, 1422464)

### Assistant (2026-02-22 23:54:52)
...
def sha1_file(path):
    ...
    with open(path,'rb') as f:
        for chunk in iter(lambda: f.read(1<<20), b''):
            h.update(chunk)
    return h.hexdigest()
sha1_fc = sha1_file(fc_path)
sha1_fc

### Tool (2026-02-22 23:54:54)

'340f3ce5b561fe00e872ef98991390185f600f96'

### Assistant (2026-02-22 23:54:56)

fc = np.load(fc_path, allow_pickle=True)
sorted(fc.files)[:50], len(fc.files)

### Tool (2026-02-22 23:54:58)
...
### Assistant (2026-02-22 23:54:59)

train_end = int(fc['train_end'])
N_kept = int(fc['N_kept'])
dt = float(fc['dt'])
source_theta_key = str(fc['source_theta_key'])
source_R_key = str(fc['source_R_key'])
(train_end, N_kept, dt, source_theta_key, source_R_key)

### Tool (2026-02-22 23:55:02)

(25614, 36592, 0.00012297217348543395, 'theta_mod', 'R')

### Assistant (2026-02-22 23:55:04)

[k for k in fc.files if 'sha' in k.lower() or 'hash' in k.lower()]

### Tool (2026-02-22 23:55:08)

[]

### Assistant (2026-02-22 23:55:09)

def sha1_array(arr: np.ndarray):
    # stable: use C-contiguous float64 bytes; preserve dtype and shape
    a = np.ascontiguousarray(arr)
    return hashlib.sha1(a.view(np.uint8)).hexdigest()
theta = fc['theta'].astype(np.float64)
R = fc['R'].astype(np.float64)
sha1_theta = sha1_array(theta)
sha1_R = sha1_array(R)
sha1_theta, sha1_R, theta.shape, R.shape

### Tool (2026-02-22 23:55:15)
...
### Assistant (2026-02-22 23:55:18)

mask_train = fc['mask_train_lag79'].astype(bool)
mask_test = fc['mask_test_lag79'].astype(bool)
mask_train.shape, mask_test.shape, mask_train.sum(), mask_test.sum()

### Tool (2026-02-22 23:55:22)

((36592,), (36592,), 25510, 10953)

### Assistant (2026-02-22 23:55:27)

h=25
y = np.zeros(N_kept, dtype=np.int8)
valid = np.arange(N_kept-h)
y[:N_kept-h] = (R[h:] - R[:-h] > 0).astype(np.int8)
# Mark invalid tail as -1
y[N_kept-h:] = -1
# check masks do not include invalid
(mask_train[N_kept-h:].any(), mask_test[N_kept-h:].any(), (mask_train & (y==-1)).sum(), (mask_test & (y==-1)).sum())

### Tool (2026-02-22 23:55:33)

(False, False, 0, 0)

### Assistant (2026-02-22 23:55:35)

X80 = fc['X80'].astype(np.float64)
X80.shape

### Tool (2026-02-22 23:55:37)

(36592, 80)

### Assistant (2026-02-22 23:55:40)

mu_X80 = fc['mu_X80_train'].astype(np.float64)
sd_X80 = fc['sd_X80_train'].astype(np.float64)
mu_S = fc['mu_S_train'].astype(np.float64)
sd_S = fc['sd_S_train'].astype(np.float64)
mu_X80.shape, sd_X80.shape, mu_S.shape, sd_S.shape

### Tool (2026-02-22 23:55:42)

((80,), (80,), (2,), (2,))

### Assistant (2026-02-22 23:55:43)

S = fc['S'].astype(np.float64)
S.shape, np.max(np.abs(S[:,0]-np.sin(theta))), np.max(np.abs(S[:,1]-np.cos(theta)))

### Tool (2026-02-22 23:55:46)

((36592, 2), 2.9802137091472503e-08, 2.9801990097944042e-08)

### Assistant (2026-02-22 23:55:47)
...
Xte = X[mask_test]
yte = y[mask_test]
# Fit with lbfgs, no regularization if possible
try:
...
a_hat = float(coef[-2])  # sin standardized
b_hat = float(coef[-1])  # cos standardized
A_amp = float(math.hypot(a_hat, b_hat))
theta_star = float(math.atan2(a_hat, b_hat) % (2*math.pi))
auc_test = float(roc_auc_score(yte, clf.predict_proba(Xte)[:,1]))
# early/late split
test_idx = np.flatnonzero(mask_test)
mid = len(test_idx)//2
early_mask = np.zeros_like(mask_test)
late_mask = np.zeros_like(mask_test)
early_mask[test_idx[:mid]] = True
late_mask[test_idx[mid:]] = True
auc_early = float(roc_auc_score(y[early_mask], clf.predict_proba(X[early_mask])[:,1]))
auc_late = float(roc_auc_score(y[late_mask], clf.predict_proba(X[late_mask])[:,1]))
(a_hat,b_hat,A_amp,theta_star,auc_test,auc_early,auc_late, intercept)

### Tool (2026-02-22 23:55:57)
...
t_raw = np.log(x_grid)
L_new=65536
t_uni = np.linspace(t_raw.min(), t_raw.max(), L_new)
# interpolate
...
### Tool (2026-02-22 23:56:22)

(18296, 0.00024594622340595577)

### Assistant (2026-02-22 23:56:26)
