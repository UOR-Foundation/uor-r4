# Leakage-Audited Test Setup

- **Conversation ID:** `699a7198-f97c-8330-bb93-bccfabf83dfb`
- **Date:** 2026-02-21

---

### User (2026-02-21 22:01:48)
...
### Assistant (2026-02-21 22:01:49)

import numpy as np, math, json, os, statistics
from numpy.fft import rfft, irfft
import scipy.signal as signal, scipy.interpolate as interp
...
primes = pr['primes']
x_grid[:5], psi_minus_x[:5], primes[:5], primes[-1], len(primes)

### Tool (2026-02-21 22:02:01)
...
# interpolate
f_interp = interp.interp1d(t_grid, psi_minus_x.astype(np.float64), kind='linear', fill_value='extrapolate', assume_sorted=True)
psi_u = f_interp(t_uniform)
x_u = np.exp(t_uniform)
mask = (x_u>=1e5) & (x_u<=9e6)
idx = np.nonzero(mask)[0]
len(idx), idx[0], idx[-1], float(x_u[idx[0]]), float(x_u[idx[-1]])

### Tool (2026-02-21 22:02:09)

(18296, 46811, 65106, 100006.32008830582, 8998557.287555724)

### Assistant (2026-02-21 22:02:11)
...
# primes array sorted int32
p = primes.astype(np.int64)
lo = x_kept
hi = x_kept*expD
# searchsorted
lo_idx = np.searchsorted(p, lo, side='left')
hi_idx = np.searchsorted(p, hi, side='left')  # half-open
P = (hi_idx - lo_idx).astype(np.int64)
E = x_kept*(expD-1.0)/np.log(x_kept)
R = (P - E)/np.sqrt(E)
R[:5], P[:5], E[:5]

### Tool (2026-02-21 22:02:16)
...
### Assistant (2026-02-21 22:02:46)

def stickiness_stats(y):
    y = np.asarray(y, dtype=np.int8)
    n=len(y)
    if n==0:
        return {"n":0,"positive_rate":None,"flip_rate":None,"mean_run_length":None}
    pos = float(y.mean())
    flips = float(np.mean(y[1:]!=y[:-1])) if n>1 else 0.0
    # run lengths
    runs=[]
    run=1
    for i in range(1,n):
    ...
    return {"n":n,"positive_rate":pos,"flip_rate":flips,"mean_run_length":mrl}
...
y_sticky = (R > median_R_train).astype(np.int8)

H = [1,5,10,25,50,100]
# build y_h arrays full length valid n-h
y_h = {}
for h in H:
    dy = R[h:] - R[:-h]
    y_h[h] = (dy>0).astype(np.int8)

# stickiness diagnostics per h on train/test segments requiring within segment
stickiness = {}
for h in H:
    # valid i for segment: i from 0..train_end-h-1
    y_tr = y_h[h][:max(0, train_end - h)]
    # test: i from train_end .. n-h-1; slice in y_h[h] corresponds to i
    if n-h > train_end:
        y_te = y_h[h][train_end:(n-h)]
    else:
        y_te = np.array([],dtype=np.int8)
    stickiness[h] = {"train": stickiness_stats(y_tr), "test": stickiness_stats(y_te)}
# sticky reference diagnostics h=1 on train/test within segments
sticky_ref = {
    "median_R_train": median_R_train,
    "train": stickiness_stats(y_sticky[:train_end]),
    "test": stickiness_stats(y_sticky[train_end:]),
}
sticky_ref

### Tool (2026-02-21 22:02:56)
...
### Assistant (2026-02-21 22:02:58)

W=2000
u_raw_kept = u_raw.astype(np.float64)
cs = np.cumsum(u_raw_kept)
u_ma = np.empty_like(u_raw_kept)
for i in range(n):
    j0 = max(0, i-W+1)
    s = cs[i] - (cs[j0-1] if j0>0 else 0.0)
    u_ma[i] = s / (i-j0+1)
u_det = u_raw_kept - u_ma
mean_train = float(u_det[:train_end].mean())
std_train = float(u_det[:train_end].std(ddof=0))
u = (u_det - mean_train)/std_train
mean_train, std_train, float(u[:train_end].mean()), float(u[:train_end].std())

### Tool (2026-02-21 22:03:02)

(7.296664503769434, 176.3404688541781, 3.9946183317503876e-17, 1.0)

### Assistant (2026-02-21 22:03:04)

lo_f, hi_f = 0.002, 0.01
U = rfft(u)
freqs = np.fft.rfftfreq(n, d=1.0)  # cycles/sample
mask_bp = (freqs>=lo_f) & (freqs<=hi_f)
U_bp = U * mask_bp
u_bp = irfft(U_bp, n=n)
# analytic signal
...
def eval_metrics(y_true, prob):
    ...
    out["accuracy@0.5"]=float(accuracy_score(y_true, prob>=0.5))
    out["n"]=int(len(y_true))
    out["positive_rate"]=float(y_true.mean()) if len(y_true)>0 else None
    return out

def fit_logistic(X, y):
    # solver lbfgs, no penalty? use l2 with large C
    model = LogisticRegression(max_iter=5000, solver='lbfgs')
    model.fit(X, y)
    return model

# AR(1) fit on train
Rt = R_train[:-1]; Rt1 = R_train[1:]
# fit Rt1 = a Rt + b
A_mat = np.column_stack([Rt, np.ones_like(Rt)])
a,b = np.linalg.lstsq(A_mat, Rt1, rcond=None)[0]
a=float(a); b=float(b)

def ar1_predict_R(Ri, h):
    if abs(a-1.0)>1e-12:
        ah = a**h
        return ah*Ri + b*(1-ah)/(1-a)
    else:
        return Ri + h*b
...
results={}
for h in H:
    # define valid train/test i indices
    train_i0 = np.arange(0, max(0, train_end - h))
    test_i0 = np.arange(train_end, n - h) if (n-h)>train_end else np.array([],dtype=int)
    y_train = y_h[h][train_i0]
    y_test = y_h[h][test_i0]
    
    res_h={}
    # Baseline A persistence proxy
    # need y1 available at those i; for h maybe y1 defined only up to n-2; but our ranges ensure i < n-h <= n-1; ok
    prob_persist = y1[test_i0].astype(float) if len(test_i0)>0 else np.array([],dtype=float)
    res_h["baseline_persistence_proxy"] = eval_metrics(y_test, prob_persist)
    
    # Baseline B AR(1) on R
    if len(test_i0)>0:
        Ri = R[test_i0]
        R_pred = np.array([ar1_predict_R(r, h) for r in Ri])
        dR_pred = R_pred - Ri
        prob_ar = (dR_pred>0).astype(float)
    else:
        prob_ar = np.array([],dtype=float)
    res_h["baseline_AR1_on_R"] = eval_metrics(y_test, prob_ar)
    res_h["baseline_AR1_on_R"]["a"]=a; res_h["baseline_AR1_on_R"]["b"]=b
    
    # Baseline C logistic on lagged R
    # build features with lags 0,1,2; require i>=2
    train_i = np.arange(2, max(2, train_end - h))
    test_i = np.arange(max(train_end,2), n - h) if (n-h)>max(train_end,2) else np.array([],dtype=int)
    def lag_feat(i_arr):
        return np.column_stack([R[i_arr], R[i_arr-1], R[i_arr-2]])
    X_tr = lag_feat(train_i) if len(train_i)>0 else np.empty((0,3))
    y_tr = y_h[h][train_i] if len(train_i)>0 else np.array([],dtype=np.int8)
    X_te = lag_feat(test_i) if len(test_i)>0 else np.empty((0,3))
    y_te = y_h[h][test_i] if len(test_i)>0 else np.array([],dtype=np.int8)
    if len(train_i)>0 and len(np.unique(y_tr))==2 and len(test_i)>0:
        lr = fit_logistic(X_tr, y_tr)
        prob = lr.predict_proba(X_te)[:,1]
        res_h["baseline_logit_lagged_R"] = eval_metrics(y_te, prob)
        res_h["baseline_logit_lagged_R"]["coef"]=lr.coef_.ravel().tolist()
        res_h["baseline_logit_lagged_R"]["intercept"]=lr.intercept_.tolist()
    else:
        res_h["baseline_logit_lagged_R"] = eval_metrics(y_te, np.full_like(y_te, y_tr.mean() if len(y_tr)>0 else 0.5, dtype=float))
    
    # Phase/Amp models: use indices i where i+h exists; for train require i<train_end-h; test i>=train_end
    # features at i use theta_mod[i], logA[i]
    def phase_features(i_arr, K=3):
        th = theta_mod[i_arr]
    ...
    def amp_features(i_arr):
        return logA[i_arr].reshape(-1,1)  # no intercept included; logistic will add intercept
    # For amp-only, include intercept via sklearn; ok.
    
    train_i_pa = train_i0  # no lag requirement
    test_i_pa = test_i0
    
    y_tr_pa = y_h[h][train_i_pa]
    y_te_pa = y_h[h][test_i_pa]
    
    # Phase-only
    Xtr = phase_features(train_i_pa,3)
    Xte = phase_features(test_i_pa,3) if len(test_i_pa)>0 else np.empty((0,7))
    if len(train_i_pa)>0 and len(np.unique(y_tr_pa))==2 and len(test_i_pa)>0:
        lr = fit_logistic(Xtr, y_tr_pa)
        prob = lr.predict_proba(Xte)[:,1]
        res_h["phase_only_FourierK3"] = eval_metrics(y_te_pa, prob)
        res_h["phase_only_FourierK3"]["coef"]=lr.coef_.ravel().tolist()
        res_h["phase_only_FourierK3"]["intercept"]=lr.intercept_.tolist()
    else:
        res_h["phase_only_FourierK3"] = eval_metrics(y_te_pa, np.full_like(y_te_pa, y_tr_pa.mean() if len(y_tr_pa)>0 else 0.5, dtype=float))
    # Amp-only
    Xtr = amp_features(train_i_pa)
    Xte = amp_features(test_i_pa) if len(test_i_pa)>0 else np.empty((0,1))
    if len(train_i_pa)>0 and len(np.unique(y_tr_pa))==2 and len(test_i_pa)>0:
        lr = fit_logistic(Xtr, y_tr_pa)
        prob = lr.predict_proba(Xte)[:,1]
        res_h["amp_only_logA"] = eval_metrics(y_te_pa, prob)
        res_h["amp_only_logA"]["coef"]=lr.coef_.ravel().tolist()
        res_h["amp_only_logA"]["intercept"]=lr.intercept_.tolist()
    else:
        res_h["amp_only_logA"] = eval_metrics(y_te_pa, np.full_like(y_te_pa, y_tr_pa.mean() if len(y_tr_pa)>0 else 0.5, dtype=float))
    # Phase+Amp
    Xtr = np.column_stack([phase_features(train_i_pa,3), logA[train_i_pa]])
    Xte = np.column_stack([phase_features(test_i_pa,3), logA[test_i_pa]]) if len(test_i_pa)>0 else np.empty((0,8))
    if len(train_i_pa)>0 and len(np.unique(y_tr_pa))==2 and len(test_i_pa)>0:
        lr = fit_logistic(Xtr, y_tr_pa)
        prob = lr.predict_proba(Xte)[:,1]
        res_h["phase_plus_amp"] = eval_metrics(y_te_pa, prob)
        res_h["phase_plus_amp"]["coef"]=lr.coef_.ravel().tolist()
        res_h["phase_plus_amp"]["intercept"]=lr.intercept_.tolist()
    else:
        res_h["phase_plus_amp"] = eval_metrics(y_te_pa, np.full_like(y_te_pa, y_tr_pa.mean() if len(y_tr_pa)>0 else 0.5, dtype=float))
    
    results[h]=res_h

results[1]["phase_plus_amp"]

### Tool (2026-02-21 22:03:49)
...
{'AUC': 0.5172675521317607,
 ...
 'coef': [-0.08965302326450461,
  ...
  0.05491505422473574],
 'intercept': [-0.08977405494843811]}

### Assistant (2026-02-21 22:04:10)
...
out = {
  "params":{
    ...
    "test_n": int(n-train_end),
    "Delta": Delta,
    "detrend_W_causal": W,
    "bandpass_cycles_per_sample":[lo_f, hi_f],
...
### Tool (2026-02-21 22:04:15)

17699

### Assistant (2026-02-21 22:04:17)
