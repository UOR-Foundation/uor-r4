# Two-field counter-rotation test

- **Conversation ID:** `699b91dd-e9c4-8332-9164-b001073dbca6`
- **Date:** 2026-02-22

---

### User (2026-02-22 18:31:44)
...
assert psi_path.exists() and primes_path.exists()
...
# interpolate psi_minus_x to uniform t grid
u_interp=np.interp(t_uni, t_raw, psi_minus_x.astype(np.float64))
x_uni=np.exp(t_uni)
...
W=int(params["detrend_W"])
# causal moving average (past-only), using cumulative sum
...
# FFT bandpass on full kept segment
f_low,f_high=params["bandpass_cycles_per_sample"]
U=np.fft.rfft(u_std)
freq=np.fft.rfftfreq(N_kept, d=1.0)  # cycles/sample
...
# theta bins
B=int(params["B_theta"])
bin_theta=np.floor(B*theta_mod/(2*np.pi)).astype(int)
bin_theta=np.clip(bin_theta, 0, B-1)
...
x_lo=x_kept
x_hi=x_kept*expD
idx_lo=np.searchsorted(primes, x_lo, side='left')
idx_hi=np.searchsorted(primes, x_hi, side='left')
P=(idx_hi-idx_lo).astype(np.int64)
E=x_kept*(expD-1.0)/np.log(x_kept)
R=(P - E)/np.sqrt(E)

# 2) absR bucket edges from TRAIN indices valid at h=100
h_edge=100
valid_train_edge=np.where((np.arange(N_kept)+h_edge < N_kept) & (np.arange(N_kept) < train_end) & (np.arange(N_kept)+h_edge < train_end))[0]
absR_train=np.abs(R[valid_train_edge])
q=np.linspace(0,1,params["n_buckets_absR"]+1)
edges=np.quantile(absR_train, q)
...
bucket_absR=bucketize(np.abs(R), edges)
low_buckets=set(params["use_buckets_lowR"])
low_mask=np.isin(bucket_absR, list(low_buckets))

# helper: fit laplace-smoothed probs
alpha=float(params["laplace_alpha"])
def fit_probs(bin_arr, y_arr, idx, B, alpha):
    # idx: indices to use
    b=bin_arr[idx]
    y=y_arr[idx].astype(int)
    n1=np.bincount(b, weights=y, minlength=B)
    n=np.bincount(b, minlength=B)
    p=(n1 + alpha)/(n + 2*alpha)
    return p, n.astype(int), n1.astype(int)
...
def best_shift_single(bin_arr, y_arr, train_in_idx, val_idx, B, alpha):
    p, n, n1=fit_probs(bin_arr, y_arr, train_in_idx, B, alpha)
    best=None
    best_auc=-1e9
    for s in range(B):
        p_rot=np.roll(p, s)
        scores=p_rot[bin_arr[val_idx]]
        auc=auc_safe(y_arr[val_idx], scores)
        if auc is None:
            continue
        if auc>best_auc:
            best_auc=auc
    ...
    if best is None:
        return p, 0, None
    return p, int(best), float(best_auc)

def eval_single(bin_arr, y_arr, test_idx, p, shift, B):
    p_rot=np.roll(p, shift)
    scores=p_rot[bin_arr[test_idx]]
    return auc_safe(y_arr[test_idx], scores)

def best_shifts_two_gate(bin_arr, y_arr, R_arr, train_in_idx, val_idx, B, alpha):
    # fit separate probs on train_in for pos/neg
    train_pos=train_in_idx[R_arr[train_in_idx]>=0]
    train_neg=train_in_idx[R_arr[train_in_idx]<0]
    # if one side empty, fall back: use overall probs for missing side
    p_all,_,_=fit_probs(bin_arr, y_arr, train_in_idx, B, alpha)
    if len(train_pos)>0:
        p_pos,_,_=fit_probs(bin_arr, y_arr, train_pos, B, alpha)
    else:
        p_pos=p_all
    if len(train_neg)>0:
        p_neg,_,_=fit_probs(bin_arr, y_arr, train_neg, B, alpha)
    else:
        p_neg=p_all
    best_auc=-1e9
    best_sp=0
    best_sn=0
    # precompute bins and gate for val
    bval=bin_arr[val_idx]
    gate=(R_arr[val_idx]>=0)
    yval=y_arr[val_idx]
    for sp in range(B):
        ppos=np.roll(p_pos, sp)
        score_pos=ppos[bval]
        for sn in range(B):
            pneg=np.roll(p_neg, sn)
            score_neg=pneg[bval]
            scores=np.where(gate, score_pos, score_neg)
            auc=auc_safe(yval, scores)
            if auc is None:
                continue
            if auc>best_auc:
                best_auc=auc
                ...
                best_sn=sn
    if best_auc<-1e8:
        return p_pos, p_neg, 0, 0, None
    return p_pos, p_neg, int(best_sp), int(best_sn), float(best_auc)

def eval_two_gate(bin_arr, y_arr, R_arr, test_idx, p_pos, p_neg, sp, sn):
    b=bin_arr[test_idx]
    gate=(R_arr[test_idx]>=0)
    scores=np.where(gate, np.roll(p_pos, sp)[b], np.roll(p_neg, sn)[b])
    return auc_safe(y_arr[test_idx], scores)

out={
    "meta":{
        "experiment_id":"A4.07_two_field_counter_rotation_gate_by_signR",
        "goal":params["goal"] if "goal" in params else "two-field gated by sign(R_i) with independent circular shifts",
        "files_used":["chebyshev_psi.npz","primes.npz"],
        "params":params
    },
    "data_shape":{"dt":dt,"N_kept":N_kept,"train_end":train_end},
    "absR_bucket_edges":[float(e) for e in edges],
    "by_h":{},
...
for h in params["h_list"]:
    h=int(h)
    i_all=np.arange(N_kept-h, dtype=int)
    y=(R[i_all+h]-R[i_all] > 0).astype(int)
    # train/test split indices among i_all
    train_mask=(i_all < train_end) & (i_all + h < train_end)
    test_mask=(i_all >= train_end) & (i_all + h < N_kept)
    train_idx=i_all[train_mask]
    test_idx=i_all[test_mask]
    n_train=len(train_idx); n_test=len(test_idx)
    train_pos_rate=float(y[train_mask].mean()) if n_train>0 else None
    test_pos_rate=float(y[test_mask].mean()) if n_test>0 else None

    # train/val split
    n_val=int(math.floor(params["train_val_frac_of_train"]*n_train))
    n_in=n_train - n_val
    train_in_idx=train_idx[:n_in]
    val_idx=train_idx[n_in:]  # may be empty
    # baseline single curve
    if len(val_idx)>0:
        p, best_s, val_auc=best_shift_single(bin_theta, y, train_in_idx, val_idx, B, alpha)
    else:
        p, best_s, val_auc=fit_probs(bin_theta, y, train_in_idx, B, alpha)[0], 0, None
    test_auc_best=eval_single(bin_theta, y, test_idx, p, best_s, B) if n_test>0 else None
    test_auc_shift0=eval_single(bin_theta, y, test_idx, p, 0, B) if n_test>0 else None

    # two-curve gate
    if len(val_idx)>0:
        p_pos, p_neg, sp, sn, val_auc2=best_shifts_two_gate(bin_theta, y, R, train_in_idx, val_idx, B, alpha)
    else:
        # no val: default shifts 0
        p_all,_,_=fit_probs(bin_theta, y, train_in_idx, B, alpha)
        p_pos=p_all; p_neg=p_all; sp=0; sn=0; val_auc2=None
    test_auc2=eval_two_gate(bin_theta, y, R, test_idx, p_pos, p_neg, sp, sn) if n_test>0 else None

    # lowR subset
    low_train_mask=train_mask & low_mask[i_all]
    low_test_mask=test_mask & low_mask[i_all]
    low_train_idx=i_all[low_train_mask]
    low_test_idx=i_all[low_test_mask]
    n_train_low=len(low_train_idx); n_test_low=len(low_test_idx)
    train_low_pos_rate=float(y[low_train_mask].mean()) if n_train_low>0 else None
    test_low_pos_rate=float(y[low_test_mask].mean()) if n_test_low>0 else None

    n_val_low=int(math.floor(params["train_val_frac_of_train"]*n_train_low))
    n_in_low=n_train_low - n_val_low
    train_in_low=low_train_idx[:n_in_low]
    val_low=low_train_idx[n_in_low:]
    if len(val_low)>0:
        pL, best_sL, val_aucL=best_shift_single(bin_theta, y, train_in_low, val_low, B, alpha)
    else:
        pL, best_sL, val_aucL=fit_probs(bin_theta, y, train_in_low, B, alpha)[0], 0, None
    test_auc_best_L=eval_single(bin_theta, y, low_test_idx, pL, best_sL, B) if n_test_low>0 else None
    test_auc_shift0_L=eval_single(bin_theta, y, low_test_idx, pL, 0, B) if n_test_low>0 else None

    if len(val_low)>0:
        p_posL, p_negL, spL, snL, val_auc2L=best_shifts_two_gate(bin_theta, y, R, train_in_low, val_low, B, alpha)
    else:
        p_allL,_,_=fit_probs(bin_theta, y, train_in_low, B, alpha)
        p_posL=p_allL; p_negL=p_allL; spL=0; snL=0; val_auc2L=None
    test_auc2L=eval_two_gate(bin_theta, y, R, low_test_idx, p_posL, p_negL, spL, snL) if n_test_low>0 else None

    out["by_h"][str(h)]={
        "overall":{
            "counts":{"n_train":int(n_train),"n_test":int(n_test),
                      "train_pos_rate":train_pos_rate,"test_pos_rate":test_pos_rate},
            "single_curve":{"best_shift":int(best_s),
                           "val_auc_best":val_auc,
                           "test_auc_best":test_auc_best,
                           "test_auc_shift0":test_auc_shift0},
            "two_curve_gate_signR":{"best_shift_pos":int(sp),
                                    "best_shift_neg":int(sn),
                                    "val_auc_best":val_auc2,
                                    "test_auc_best":test_auc2}
        },
        "lowR":{
            "counts":{"n_train_low":int(n_train_low),"n_test_low":int(n_test_low),
                      "train_low_pos_rate":train_low_pos_rate,"test_low_pos_rate":test_low_pos_rate},
            "single_curve":{"best_shift":int(best_sL),
                           "val_auc_best":val_aucL,
                           "test_auc_best":test_auc_best_L,
                           "test_auc_shift0":test_auc_shift0_L},
            "two_curve_gate_signR":{"best_shift_pos":int(spL),
                                    "best_shift_neg":int(snL),
                                    "val_auc_best":val_auc2L,
                                    "test_auc_best":test_auc2L}
        }
    }
...
summ=[]
for h in params["h_list"]:
    d_over=delta(out["by_h"][str(h)]["overall"]["two_curve_gate_signR"]["test_auc_best"],
                out["by_h"][str(h)]["overall"]["single_curve"]["test_auc_best"])
    d_low=delta(out["by_h"][str(h)]["lowR"]["two_curve_gate_signR"]["test_auc_best"],
               out["by_h"][str(h)]["lowR"]["single_curve"]["test_auc_best"])
    summ.append((h,d_over,d_low))
out["summary"]["interpretation_hint"]=(
    "Compare two-curve vs single-curve: positive delta means sign(R)-gated counter-rotation adds lift; "
    f"deltas (h,overall,lowR)={summ}"
)
json_str=json.dumps(out, indent=2, sort_keys=False)
len(json_str), json_str[:500]

### Tool (2026-02-22 18:33:32)

(3809,
 '{\n  "meta": {\n    "experiment_id": "A4.07_two_field_counter_rotation_gate_by_signR",\n    "goal": "two-field gated by sign(R_i) with independent circular shifts",\n    "files_used": [\n      "chebyshev_psi.npz",\n      "primes.npz"\n    ],\n    "params": {\n      "L": 131072,\n      "x_range": [\n        100000.0,\n        9000000.0\n      ],\n      "Delta": 0.01,\n      "train_frac": 0.7,\n      "detrend_W": 2000,\n      "bandpass_cycles_per_sample": [\n        0.002,\n        0.01\n      ],\n      "seed": 12345,')

### Assistant (2026-02-22 18:33:50)
