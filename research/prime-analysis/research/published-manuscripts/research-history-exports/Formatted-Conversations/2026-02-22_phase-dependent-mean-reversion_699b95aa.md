...
- **Conversation ID:** `699b95aa-3784-832a-9859-8572bcd0ce46`
- **Date:** 2026-02-22

---

### User (2026-02-22 18:47:58)
...
primes = pr['primes']
x_grid[:5], psi_minus_x[:5], primes[:5], primes.dtype, x_grid.shape, psi_minus_x.shape, primes.shape

### Tool (2026-02-22 18:48:11)
...
### Assistant (2026-02-22 18:48:14)

def sha1_float64(arr):
...
def fft_bandpass(sig, f_lo, f_hi):
    ...
    # keep symmetry: rfft already one-sided
    Xf = X * mask
    y = irfft(Xf, n=n)
    return y
...
u_raw_kept = u_interp[kept].astype(np.float64)
N_kept = int(x_kept.size)
train_end = int(math.floor(train_frac*N_kept))

u_ma = causal_moving_average(u_raw_kept, W)
u = u_raw_kept - u_ma
mu = u[:train_end].mean()
...
theta = np.mod(np.angle(hilbert(u_bp)), 2*np.pi).astype(np.float64)

# R
expD = math.exp(Delta)
factor = expD - 1.0
# primes array int32 sorted
p = primes.astype(np.int64)
# vectorized searchsorted
lo = np.searchsorted(p, x_kept, side='left')
hi = np.searchsorted(p, x_kept*expD, side='left')
P_counts = (hi-lo).astype(np.float64)
E = x_kept*factor/np.log(x_kept)
R = (P_counts - E)/np.sqrt(E)

dt, N_kept, train_end, x_kept[0], x_kept[-1], (mu,sd), np.isnan(R).sum()

### Tool (2026-02-22 18:48:42)
...
### Assistant (2026-02-22 18:48:44)

hashes = {
    "x_kept": sha1_float64(x_kept),
    "u_bp": sha1_float64(u_bp),
    "theta": sha1_float64(theta),
    "R": sha1_float64(R),
}
hashes

### Tool (2026-02-22 18:48:46)
...
def ols_with_intercept(x, y):
    ...
    if dof>0:
        ...
        se_b = float(math.sqrt(s2*XtX_inv[1,1]))
    else:
        se_b = np.nan
    return a,b,r2,se_b

def per_bin_fits(R, dR, theta, idx, B, min_n):
    # idx array of indices to use
    bins = np.floor(B*theta[idx]/(2*np.pi)).astype(int)
    ...
    for b in range(B):
        ...
        if n < min_n:
            ...
            continue
        i_sel = idx[mask]
        a,beta,r2,se_b = ols_with_intercept(R[i_sel], dR[i_sel])
        out.append({"n":n, "a":a, "beta":beta, "r2":r2, "se_beta":se_b})
    return out

def stability(train_bins, test_bins):
    # compute corr and MAD over bins with both not None
    bt=[]; bs=[]
    for b,(tr,te) in enumerate(zip(train_bins,test_bins)):
        if tr is None or te is None: 
            continue
        bt.append(tr["beta"]); bs.append(te["beta"])
    bt=np.array(bt); bs=np.array(bs)
    if bt.size<2:
        return {"n_common": int(bt.size), "corr_beta_train_test": np.nan, "mad_beta_train_test": np.nan}
    corr = float(np.corrcoef(bt,bs)[0,1])
    mad = float(np.mean(np.abs(bt-bs)))
    return {"n_common": int(bt.size), "corr_beta_train_test": corr, "mad_beta_train_test": mad}

def fit_fourier(beta_train_by_bin, B, K):
    # collect bins with train beta not None
    angles=[]
    y=[]
    bins=[]
    for b,rec in enumerate(beta_train_by_bin):
        if rec is None: continue
        ang = 2*np.pi*(b+0.5)/B
        angles.append(ang); y.append(rec["beta"]); bins.append(b)
    angles=np.array(angles); y=np.array(y)
    n=y.size
    # design matrix: [1, cos(k ang), sin(k ang) ...]
    cols=[np.ones(n)]
    ...
    coef, *_ = np.linalg.lstsq(X,y,rcond=None)
    return coef, np.array(bins), angles, y

def eval_fourier_on_bins(coef, bins, B, K, beta_test_by_bin):
    # evaluate on bins that have test beta too
    y_true=[]; y_pred=[]; bins_common=[]
    for b in bins:
        te = beta_test_by_bin[b]
        tr_ok = True
        if te is None: 
            continue
        ang = 2*np.pi*(b+0.5)/B
        ...
        feat=[1.0]
        for k in range(1,K+1):
            feat.append(math.cos(k*ang))
            feat.append(math.sin(k*ang))
        yhat=float(np.dot(coef, feat))
        y_true.append(te["beta"]); y_pred.append(yhat); bins_common.append(int(b))
    y_true=np.array(y_true); y_pred=np.array(y_pred)
    if y_true.size==0:
        return {"n_bins_test":0, "mse_test":np.nan, "r2_test":np.nan}
    mse=float(np.mean((y_true-y_pred)**2))
    ss_tot=float(np.sum((y_true-y_true.mean())**2))
    ss_res=float(np.sum((y_true-y_pred)**2))
    r2=1.0-ss_res/ss_tot if ss_tot>0 else np.nan
    return {"n_bins_test":int(y_true.size), "mse_test":mse, "r2_test":r2}

B=36; min_n=200; K=3
by_h={}
for h in [25,100]:
    valid = np.arange(0, N_kept-h, dtype=int)
    train_idx = valid[(valid < train_end) & (valid+h < train_end)]
    test_idx  = valid[(valid >= train_end) & (valid+h < N_kept)]
    dR = R[h:] - R[:-h]  # aligned with i from 0..N_kept-h-1
    # We'll index dR by i (same as R[i])
    train_bins = per_bin_fits(R, dR, theta, train_idx, B, min_n)
    test_bins  = per_bin_fits(R, dR, theta, test_idx, B, min_n)
    stab = stability(train_bins, test_bins)
    coef, bins_fit, angles_fit, y_fit = fit_fourier(train_bins, B, K)
    fourier_eval = eval_fourier_on_bins(coef, bins_fit, B, K, test_bins)
    by_h[h]={
        "counts": {
        ...
        "per_bin_train": train_bins,
        "per_bin_test": test_bins,
        "stability": stab,
        "fourier_fit": {
            "K": K,
            "coeffs": coef.tolist(),
            "n_bins_train_used": int(len(bins_fit))
        },
        "fourier_generalization": fourier_eval
    }
by_h.keys(), by_h[25]["stability"], by_h[100]["stability"], by_h[100]["fourier_generalization"]

### Tool (2026-02-22 18:49:21)
...
### Assistant (2026-02-22 18:49:22)

by_h[25]["fourier_generalization"]

### Tool (2026-02-22 18:49:26)

{'n_bins_test': 36,
 'mse_test': 0.029992158712910444,
 'r2_test': -0.01750473738242353}

### Assistant (2026-02-22 18:49:28)

def summarize_betas(binrecs):
    betas=[r["beta"] for r in binrecs if r is not None]
    betas=np.array(betas)
    return {
        "n_bins": int(betas.size),
        "mean_beta": float(betas.mean()),
        "median_beta": float(np.median(betas)),
        "std_beta": float(betas.std(ddof=0)),
        "min_beta": float(betas.min()),
        "max_beta": float(betas.max()),
        "frac_negative": float((betas<0).mean())
    }
for h in [25,100]:
    by_h[h]["beta_summary_train"]=summarize_betas(by_h[h]["per_bin_train"])
    by_h[h]["beta_summary_test"]=summarize_betas(by_h[h]["per_bin_test"])
by_h[25]["beta_summary_train"], by_h[25]["beta_summary_test"], by_h[100]["beta_summary_train"]

### Tool (2026-02-22 18:49:32)
