# Rotation-rate Invariance Experiment

- **Conversation ID:** `699b9eca-5014-832f-ab6f-42b6294e0a78`
- **Date:** 2026-02-22

---

### User (2026-02-22 19:26:48)
...
### Assistant (2026-02-22 19:40:26)

import numpy as np, hashlib, math, json, os, scipy.signal as signal
from numpy.fft import rfft, irfft, rfftfreq
# Load data
cheb_path="/mnt/data/chebyshev_psi.npz"
data=np.load(cheb_path)
x_grid=data["x_grid"]
...
x_uniform=np.exp(t)
x_lo,x_hi=1e5,9e6
kept_mask=(x_uniform>=x_lo)&(x_uniform<=x_hi)
...
W=2000
# causal moving average past-only (including current). Use cumsum.
...
for i in range(N_kept):
    ...
    ma[i]=s/(i-j0+1)
u=u_raw - ma
mean_train=float(u[:train_end].mean())
std_train=float(u[:train_end].std(ddof=0))
u_z=(u-mean_train)/(std_train if std_train>0 else 1.0)
# bandpass by FFT mask in cycles/sample [0.002,0.01]
...
theta=np.angle(analytic).astype(np.float64)
tau=2*np.pi
theta_mod=np.mod(theta, tau)
theta_unwrap=np.unwrap(theta).astype(np.float64)
dtheta=np.diff(theta_unwrap)
omega=dtheta/dt
cycles_per_t=omega/tau
cycles_per_sample=dtheta/tau
A=np.abs(analytic)[1:]  # align with cycles_per_t length
# hashes
...
hashes={
    ...
    "u_bp": sha1_arr(u_bp.astype(np.float64)),
    "theta_mod": sha1_arr(theta_mod.astype(np.float64))
}
hashes

### Tool (2026-02-22 19:40:47)
...
def robust_stats(x):
    ...
    rstd=float(1.4826*mad)
    return {"mean":mean,"std":std,"median":med,"robust_std":rstd}
...
def pearson(x,y):
    ...
    sx=np.sqrt(np.sum(x0*x0)); sy=np.sqrt(np.sum(y0*y0))
    if sx==0 or sy==0:
        return float("nan")
    return float(np.sum(x0*y0)/(sx*sy))

N_omega=cycles_per_t.size # N_kept-1
global_stats={"cycles_per_t": robust_stats(cycles_per_t),
              "cycles_per_sample": robust_stats(cycles_per_sample)}
train_stats={"cycles_per_t": robust_stats(cycles_per_t[:train_end-1]),
             "cycles_per_sample": robust_stats(cycles_per_sample[:train_end-1])}
test_stats={"cycles_per_t": robust_stats(cycles_per_t[train_end-1:]),
            "cycles_per_sample": robust_stats(cycles_per_sample[train_end-1:])}

K=6
edges=np.linspace(0, N_omega, K+1, dtype=int)
slabs=[]
slab_medians=[]
trend_slopes=[]
for k in range(K):
    s=int(edges[k]); e=int(edges[k+1])  # [s,e)
    if e<=s:
        continue
    cpt=cycles_per_t[s:e]
    cps=cycles_per_sample[s:e]
    amp=A[s:e]
    # x range approx from x_kept indices aligned to dtheta index i corresponds to between i and i+1; use x_kept[i]..x_kept[i+1] but approximating by x_kept[s]..x_kept[e]
    x_start=float(x_kept[s])
    x_end=float(x_kept[min(e, N_kept-1)])
    slab={
        "slab_index": k,
        "idx_range":[s,e-1],
        "x_range_approx":[x_start,x_end],
        "cycles_per_t": robust_stats(cpt),
        "cycles_per_sample": robust_stats(cps),
        "trend_slope_cycles_per_t_per_index": slope_ols(cpt),
        "corr_cycles_per_t_with_amp": pearson(cpt, amp)
    }
    slabs.append(slab)
    slab_medians.append(slab["cycles_per_t"]["median"])
    trend_slopes.append(abs(slab["trend_slope_cycles_per_t_per_index"]) if np.isfinite(slab["trend_slope_cycles_per_t_per_index"]) else np.nan)

slab_medians=np.array(slab_medians, dtype=np.float64)
mean_med=float(np.mean(slab_medians))
std_med=float(np.std(slab_medians, ddof=0))
cv=float(std_med/mean_med) if mean_med!=0 else float("nan")
stability={
    "cv_slabs_cycles_per_t": cv,
    "max_minus_min_median_cycles_per_t": float(np.max(slab_medians)-np.min(slab_medians)),
    "abs_train_test_diff_median_cycles_per_t": float(abs(train_stats["cycles_per_t"]["median"]-test_stats["cycles_per_t"]["median"])),
    "mean_abs_trend_slope": float(np.nanmean(trend_slopes)) if len(trend_slopes)>0 else float("nan")
}
stability, global_stats["cycles_per_t"]["median"], train_stats["cycles_per_t"]["median"], test_stats["cycles_per_t"]["median"]

### Tool (2026-02-22 19:41:10)
...
out={
 ...
 "shape":{"dt":dt,"N_kept":int(N_kept),"train_end":int(train_end)},
 "hashes_sha1":hashes,
 "global":global_stats,
 "train":train_stats,
 "test":test_stats,
...
# ensure JSON serializable with nan -> null? spec didn't mention. Keep nan as null for JSON compliance.
def clean(obj):
    ...
    return obj
clean_out=clean(out)
print(json.dumps(clean_out, indent=2, sort_keys=False))

### Tool (2026-02-22 19:41:21)
