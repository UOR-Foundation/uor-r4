# Phase Correction AUC Logloss

- **Conversation ID:** `699c5fcf-0ecc-8332-955f-62b31197aa5f`
- **Date:** 2026-02-23

---

### User (2026-02-23 09:10:27)
...
(fc_path.exists(), psi_path.exists(), canon_path.exists())

### Tool (2026-02-23 09:11:40)

(True, True, True)

### Assistant (2026-02-23 09:11:43)

fc=np.load(fc_path, allow_pickle=True)
fc.files[:30], len(fc.files)

### Tool (2026-02-23 09:11:46)
...
dt_can=float(canon["dt"])
N_can=int(len(canon["x_kept"]))
train_end_can=int(canon["train_end"])
hash_x=sha1_arr(canon["x_kept"])
hash_theta_can=sha1_arr(canon["theta_mod"])
hash_R_can=sha1_arr(canon["R"])
dt_fc, N_fc, train_end_fc, dt_can, N_can, train_end_can, hash_x, hash_theta_can, hash_R_can

### Tool (2026-02-23 09:12:03)
...
### Assistant (2026-02-23 09:12:06)

# verify fc R matches canonical R hash
hash_R_fc=sha1_arr(fc["R"])
hash_theta_fc=sha1_arr(fc["theta"])
hash_R_fc, hash_theta_fc

### Tool (2026-02-23 09:12:09)

('5753ee7d59c7d155c4a1b9b5fba8912d557598f2',
 '8b33c57df436d37179b13b1899bf86a723f0e98a')

### Assistant (2026-02-23 09:12:10)
...
u_raw=u_interp[kept]
N_kept=len(x_kept)
dt, N_kept, sha1_arr(x_kept), hash_x

### Tool (2026-02-23 09:12:22)
...
### Assistant (2026-02-23 09:12:23)

# causal moving average past-only including current
W=2000
# use cumulative sum
c=np.cumsum(np.insert(u_raw,0,0.0))
# moving average with variable window for first W-1 points
ma=np.empty_like(u_raw)
for i in range(N_kept):
    start=max(0,i-W+1)
    ma[i]=(c[i+1]-c[start])/(i-start+1)
u_detr=u_raw - ma
# standardize using TRAIN only first train_end indices
...
### Tool (2026-02-23 09:12:35)

'c888f75abc219749cde8f870c119a6dcd98ec2b4'

### Assistant (2026-02-23 09:12:37)
...
R.shape, X80.shape, mu_X80.shape

### Tool (2026-02-23 09:12:42)

((36592,), (36592, 80), (80,))

### Assistant (2026-02-23 09:12:43)

mu_X80[:5], sd_X80[:5], X80[:3,:5].mean()

### Tool (2026-02-23 09:12:48)

(array([-0.02447307, -0.02443667, -0.02440876, -0.02437092, -0.02433225]),
 array([0.49355462, 0.49357596, 0.49358726, 0.49360111, 0.49361372]),
 nan)

### Assistant (2026-02-23 09:12:49)
...
### Tool (2026-02-23 09:12:54)

(25485, 10928)

### Assistant (2026-02-23 09:12:55)

y=(R[idx+h]-R[idx]>0).astype(np.int8)
# standardize X80 using provided mu/sd (train stats)
...
y_test=y[test_valid]
# check for nans
np.isnan(X_train).any(), np.isnan(X_test).any(), np.isnan(y_train).any()

### Tool (2026-02-23 09:12:59)

---------------------------------------------------------------------------
IndexError                                Traceback (most recent call last)
Cell In[11], line 1
----> 1 y=(R[idx+h]-R[idx]>0).astype(np.int8)
      2 # standardize X80 using provided mu/sd (train stats)
      3 X80_z=(X80 - mu_X80)/sd_X80

IndexError: index 36592 is out of bounds for axis 0 with size 36592

### Assistant (2026-02-23 09:13:01)

idx=np.arange(N_kept)
y_full=np.zeros(N_kept, dtype=np.int8)
y_full[:N_kept-h]=(R[h:]-R[:-h]>0).astype(np.int8)
train_valid=(idx>=79)&(idx<train_end)&(idx+h<train_end)
test_valid=(idx>=79)&(idx>=train_end)&(idx+h<N_kept)
X80_z=(X80 - mu_X80)/sd_X80
X_train=X80_z[train_valid]
...
np.isnan(X_train).any(), np.isnan(X_test).any(), y_train.mean(), y_test.mean()

### Tool (2026-02-23 09:13:10)

(False, False, 0.49668432411222285, 0.5058565153733529)

### Assistant (2026-02-23 09:13:14)
...
# baseline AR80
m0=fit_logistic(X_train, y_train)
p0=predict_proba(m0, X_test)

# full model with sin/cos standardized using TRAIN-valid
sin_t=np.sin(theta_new)
cos_t=np.cos(theta_new)
# standardize sin/cos based on train_valid rows
sin_mu, sin_sd = sin_t[train_valid].mean(), sin_t[train_valid].std(ddof=0)
cos_mu, cos_sd = cos_t[train_valid].mean(), cos_t[train_valid].std(ddof=0)
sin_z=(sin_t - sin_mu)/sin_sd
cos_z=(cos_t - cos_mu)/cos_sd
X_full_train=np.column_stack([X_train, sin_z[train_valid], cos_z[train_valid]])
X_full_test=np.column_stack([X_test, sin_z[test_valid], cos_z[test_valid]])
m1=fit_logistic(X_full_train, y_train)
p1=predict_proba(m1, X_full_test)
coef=m1.coef_.ravel()
a_sin=float(coef[-2])
b_cos=float(coef[-1])
A_amp=float((a_sin*a_sin + b_cos*b_cos)**0.5)
theta_star=float(math.atan2(a_sin, b_cos))  # maximizer
a_sin, b_cos, A_amp, theta_star

### Tool (2026-02-23 09:13:28)
...
ll0=logloss(y_test, p0)
ll1=logloss(y_test, p1)
auc0, auc1, auc1-auc0, ll0, ll1, ll1-ll0

### Tool (2026-02-23 09:13:45)
...
### Assistant (2026-02-23 09:13:46)

test_indices=np.where(test_valid)[0]
n=len(test_indices)
# split into 5 contiguous blocks
K=5
sizes=[n//K + (1 if i < n%K else 0) for i in range(K)]
blocks=[]
start=0
for k,sz in enumerate(sizes):
    inds=test_indices[start:start+sz]
    start+=sz
    # map to positions in p arrays which correspond to test_indices order
# easier: p0 and p1 are aligned with X_test which is test_valid order, which is same as test_indices
# thus slice p arrays accordingly
start=0
for k,sz in enumerate(sizes):
    yb=y_test[start:start+sz]
    p0b=p0[start:start+sz]
    p1b=p1[start:start+sz]
    blocks.append({
        "block_index": k,
        "n": int(sz),
        "auc_p0": float(roc_auc_score(yb,p0b)),
        "auc_p1": float(roc_auc_score(yb,p1b)),
        "delta_auc": float(roc_auc_score(yb,p1b)-roc_auc_score(yb,p0b)),
        "logloss_p0": logloss(yb,p0b),
        "logloss_p1": logloss(yb,p1b),
        "delta_logloss": logloss(yb,p1b)-logloss(yb,p0b)
    })
    start+=sz

dauc=[b["delta_auc"] for b in blocks]
dll=[b["delta_logloss"] for b in blocks]
summary={
    "mean_delta_auc": float(np.mean(dauc)),
    "min_delta_auc": float(np.min(dauc)),
    "max_delta_auc": float(np.max(dauc)),
    "mean_delta_logloss": float(np.mean(dll)),
    "min_delta_logloss": float(np.min(dll)),
    "max_delta_logloss": float(np.max(dll)),
}
blocks, summary

### Tool (2026-02-23 09:14:01)
...
out={
 ...
 "blocks":blocks,
 "summary":summary,
 "notes": "Fingerprint lock PASS: dt=0.00012297217348543395 N_kept=36592 train_end=25614; R/theta in featurecache match canonical hashes. theta_new recomputed from chebyshev_psi with canonical preprocessing and bp=[0.003,0.009] (rFFT mask), then Hilbert phase; sin/cos standardized on TRAIN-valid."
}
print(json.dumps(out, indent=2))

### Tool (2026-02-23 09:14:20)
