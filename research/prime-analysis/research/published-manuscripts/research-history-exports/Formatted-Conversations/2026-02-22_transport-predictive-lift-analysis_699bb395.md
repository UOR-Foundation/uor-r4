# Transport predictive lift analysis

- **Conversation ID:** `699bb395-e6a4-8325-9f06-fb74b5cc28cc`
- **Date:** 2026-02-22

---

### User (2026-02-22 20:55:40)
...
### Assistant (2026-02-22 20:55:42)

import numpy as np, hashlib, os, json, math, warnings
path="/mnt/data/A4_CANONICAL_CACHE_L131072_x1e5_9e6_W2000_bp002_001_D001_seed12345.npz"
data=np.load(path, allow_pickle=True)
list(data.keys())[:20], len(data.keys())

### Tool (2026-02-22 20:55:58)

(['x_kept', 'u_bp', 'theta_mod', 'R', 'dt', 'train_end', 'params', 'hashes'],
 8)

### Assistant (2026-02-22 20:56:00)
...
def sha1_bytes(arr):
    ...
    return h.hexdigest()
sha_theta=sha1_bytes(theta)
sha_R=sha1_bytes(R)
sha_theta, sha_R, N, train_end, dt

### Tool (2026-02-22 20:56:07)
...
### Assistant (2026-02-22 20:56:09)

from sklearn.linear_model import LogisticRegression
from sklearn.metrics import roc_auc_score, log_loss

### Assistant (2026-02-22 20:56:15)
...
def wrap_to_pi(x):
    ...
    if isinstance(y, np.ndarray):
        y = np.where(y<=-math.pi+1e-15, y+2*math.pi, y)
    else:
        if y <= -math.pi + 1e-15:
            y += 2*math.pi
    return y
...
def fit_eval_logreg(Xtr, ytr, Xte, yte):
    ...
    coef = np.concatenate([clf.intercept_.ravel(), clf.coef_.ravel()]).astype(float).tolist()
    return clf, auc, ll, coef, proba

def run_for_h(h, do_perm=False, n_perm=40, seed=12345):
    rng=np.random.default_rng(seed)
    valid = np.arange(2, N-h, dtype=np.int64)  # i+h < N => i <= N-h-1, so stop N-h
    # split by i < train_end
    is_train = valid < train_end
    train_idx = valid[is_train]
    test_idx = valid[~is_train]
    # targets
    y = (R[valid+h]-R[valid] > 0).astype(np.int32)
    y_train = y[is_train]
    y_test = y[~is_train]
    # features
    base = np.column_stack([R[valid], R[valid-1], R[valid-2]])
    dtheta = wrap_to_pi(theta[valid+h]-theta[valid])
    T = dtheta / TWOPI
    add = np.column_stack([base, T])
    # train/test matrices
    Xb_tr = base[is_train]; Xb_te = base[~is_train]
    Xa_tr = add[is_train];  Xa_te = add[~is_train]
    # standardize per model
    Xb_trz, Xb_tez, mu_b, sd_b = standardize_train_apply(Xb_tr, Xb_te)
    Xa_trz, Xa_tez, mu_a, sd_a = standardize_train_apply(Xa_tr, Xa_te)
    # fit models
    mb, auc_b, ll_b, coef_b, prob_b = fit_eval_logreg(Xb_trz, y_train, Xb_tez, y_test)
    ma, auc_a, ll_a, coef_a, prob_a = fit_eval_logreg(Xa_trz, y_train, Xa_tez, y_test)
    # lift
    lift = {"delta_auc": float(auc_a-auc_b), "delta_logloss": float(ll_a-ll_b)}
    # slabs on test
    slabs = np.array_split(np.arange(len(test_idx)), 3)
    slab_list=[]
    for s, inds in enumerate(slabs):
        if len(inds)==0:
            slab_list.append({"slab": s, "n": 0, "delta_auc": None})
            continue
        y_s = y_test[inds]
        ...
        if len(np.unique(y_s))<2:
            ...
            auc_bs=float(roc_auc_score(y_s, prob_b[inds]))
            auc_as=float(roc_auc_score(y_s, prob_a[inds]))
            delta=float(auc_as-auc_bs)
        slab_list.append({"slab": s, "n": int(len(inds)), "delta_auc": delta})
    perm_out={"skipped": True}
    if do_perm:
        # base_auc fixed
        base_auc_test = auc_b
        T_train = T[is_train].copy()
        T_test = T[~is_train].copy()
        delta_aucs=[]
        for j in range(n_perm):
            perm = rng.permutation(len(T_train))
            T_tr_shuf = T_train[perm]
            Xa_tr_perm = np.column_stack([Xb_tr, T_tr_shuf])
            Xa_te_perm = np.column_stack([Xb_te, T_test])
            Xa_trz_p, Xa_tez_p, _, _ = standardize_train_apply(Xa_tr_perm, Xa_te_perm)
            clf_p=LogisticRegression(penalty="l2", C=1.0, solver="liblinear", max_iter=200, fit_intercept=True)
            clf_p.fit(Xa_trz_p, y_train)
            proba_p=clf_p.predict_proba(Xa_tez_p)[:,1]
            auc_p=float(roc_auc_score(y_test, proba_p))
            delta_aucs.append(auc_p - base_auc_test)
        delta_aucs=np.array(delta_aucs, dtype=float)
        delta_obs = auc_a - base_auc_test
        perm_mean=float(delta_aucs.mean())
        perm_std=float(delta_aucs.std(ddof=0))
        p_emp=float((np.sum(delta_aucs >= delta_obs)+1)/(n_perm+1))
        perm_out={
            "n_perm": int(n_perm),
    ...
    out={
        "counts":{
            ...
            "pos_rate_train": float(y_train.mean()) if len(y_train) else None,
            "pos_rate_test": float(y_test.mean()) if len(y_test) else None
        },
        "metrics":{
            "BASE":{"auc_test": float(auc_b), "logloss_test": float(ll_b), "coef": coef_b},
            "ADD_T":{"auc_test": float(auc_a), "logloss_test": float(ll_a), "coef": coef_a}
        },
        "lift": lift,
        "slabs_test": slab_list,
        "permutation": perm_out
    }
    return out

res25 = run_for_h(25, do_perm=True, n_perm=40, seed=12345)
res100 = run_for_h(100, do_perm=False)
res25["lift"], res100["lift"], res25["permutation"]

### Tool (2026-02-22 20:56:51)

({'delta_auc': 0.0012082242204877192, 'delta_logloss': -0.0007147131235846205},
 {'delta_auc': -0.0006789101943178721, 'delta_logloss': 0.0009115966051833069},
 {'n_perm': 40,
...
### Assistant (2026-02-22 20:57:05)

res25["slabs_test"], res100["slabs_test"]

### Tool (2026-02-22 20:57:06)

([{'slab': 0, 'n': 3651, 'delta_auc': -4.775838940695998e-05},
  {'slab': 1, 'n': 3651, 'delta_auc': 0.0005129440284366016},
  {'slab': 2, 'n': 3651, 'delta_auc': 0.0033606609049618985}],
 [{'slab': 0, 'n': 3626, 'delta_auc': -0.0015718537943736344},
  {'slab': 1, 'n': 3626, 'delta_auc': -0.00011295181872272853},
  {'slab': 2, 'n': 3626, 'delta_auc': -0.0005714033611783353}])

### Assistant (2026-02-22 20:57:08)

res25["counts"], res100["counts"]

### Tool (2026-02-22 20:57:11)
