# Commensurability Test Results

- **Conversation ID:** `699ba2ed-6168-832f-89b8-76c24f37dd8b`
- **Date:** 2026-02-22

---

### User (2026-02-22 19:44:32)
...
train_frac = 0.7
W = 2000
bp_lo, bp_hi = 0.002, 0.01
seed = 12345

t_raw = np.log(x_grid.astype(np.float64))
y_raw = psi_minus_x.astype(np.float64)
...
u_interp = np.interp(t, t_raw, y_raw).astype(np.float64)

x_uniform = np.exp(t)
kept_mask = (x_uniform >= x_min) & (x_uniform <= x_max)
kept_idx = np.nonzero(kept_mask)[0]
x_kept = x_uniform[kept_idx].astype(np.float64)
u_raw_kept = u_interp[kept_idx].astype(np.float64)

N_kept = int(len(kept_idx))
train_end = int(math.floor(train_frac * N_kept))

# causal moving average detrend (past-only)
# y[t] - mean(y[max(0,t-W+1):t+1])
u = u_raw_kept
# cumulative sum for O(n)
cs = np.cumsum(u, dtype=np.float64)
# window sums
start = np.arange(N_kept) - (W-1)
start_clip = np.clip(start, 0, None)
window_len = np.arange(N_kept) - start_clip + 1
window_sum = cs - np.concatenate(([0.0], cs[:-1]))  # placeholder wrong

### Assistant (2026-02-22 19:45:08)
...
start_clip = np.clip(start, 0, None)
# sums
prev_idx = start_clip - 1
prev_vals = np.where(prev_idx >= 0, cs[prev_idx], 0.0)
window_sum = cs - prev_vals
window_len = np.arange(N_kept) - start_clip + 1
...
mu = float(u_detr[train_slice].mean())
sigma = float(u_detr[train_slice].std(ddof=0))
if sigma == 0.0:
    sigma = 1.0
u_std = (u_detr - mu) / sigma

# bandpass via FFT mask [bp_lo,bp_hi] cycles/sample on full kept
n = N_kept
U = rfft(u_std)
...
hashes = {
    "x_kept": sha1_array(x_kept),
    "u_bp": sha1_array(u_bp),
    "theta_mod": sha1_array(theta_mod)
}

# rotation rate
dtheta = np.diff(theta_unwrap)
s = (dtheta/dt) / (2*np.pi)  # cycles per unit log-time
N_s = len(s)  # N_kept-1
train_end_s = train_end - 1  # split on i < train_end-1
# robust filter
m = float(np.median(s))
mad = float(np.median(np.abs(s - m)))
rs = float(1.4826 * mad)
if rs == 0.0:
    rs = float(s.std(ddof=0))  # fallback
    if rs == 0.0:
        rs = 1.0
valid_mask = np.abs(s - m) <= 6.0*rs
valid_idx = np.nonzero(valid_mask)[0]
kept_frac = float(valid_mask.mean())

s_valid = s[valid_mask].astype(np.float64)
# split masks on original i
train_mask_valid = valid_mask.copy()
train_mask_valid[train_end_s:] = False
test_mask_valid = valid_mask.copy()
test_mask_valid[:train_end_s] = False
s_train = s[train_mask_valid].astype(np.float64)
s_test = s[test_mask_valid].astype(np.float64)

def frac(x):
    return x - np.floor(x)

def mrl_from_s(s_arr, C):
    r = frac(s_arr * C)
    return float(np.abs(np.mean(np.exp(2j*np.pi*r))))

def shuffle_stats(s_arr, C, rng, n_shuffle=200):
    obs = mrl_from_s(s_arr, C)
    mrls = np.empty(n_shuffle, dtype=np.float64)
    for k in range(n_shuffle):
        perm = rng.permutation(s_arr)
        mrls[k] = mrl_from_s(perm, C)
    null_mean = float(mrls.mean())
    null_std = float(mrls.std(ddof=0))
    if null_std == 0.0:
        z = float("inf") if obs > null_mean else float("-inf") if obs < null_mean else 0.0
    else:
        z = float((obs - null_mean)/null_std)
    p_emp = float(np.mean(mrls >= obs))
    return obs, null_mean, null_std, z, p_emp
...
def pack_split(s_arr, C, rng):
    obs, nm, ns, z, p = shuffle_stats(s_arr, C, rng, n_shuffle=200)
    return {"MRL": obs, "null_mean": nm, "null_std": ns, "z": z, "p_empirical": p}

# For reproducibility, use independent RNG streams per constant by jumping
# We'll just draw sequentially; deterministic.
comm = {
    "tau_2pi": {
        "global": pack_split(s_valid, C_tau, rng),
        "train": pack_split(s_train, C_tau, rng),
        "test": pack_split(s_test, C_tau, rng)
    },
    "C_125": {
        "global": pack_split(s_valid, C_125, rng),
        "train": pack_split(s_train, C_125, rng),
        "test": pack_split(s_test, C_125, rng)
    }
}

median_s_valid = float(np.median(s_valid)) if len(s_valid)>0 else float("nan")
T_cycle = float(1.0/median_s_valid) if median_s_valid!=0 and not math.isnan(median_s_valid) else float("nan")
scale_factor = float(np.exp(T_cycle)) if not math.isnan(T_cycle) else float("nan")
...
out = {
    ...
    "hashes_sha1": hashes,
    "rotation_rate": {
        "median_s": median_s_valid,
        "robust_std_s": rs,
        "kept_frac_after_outlier_filter": kept_frac
    },
    "implied_scaling": {
        "T_cycle_log": T_cycle,
        "scale_factor_exp_T_cycle": scale_factor
    },
    "commensurability": comm,
    "notes": {
        "interpretation_hint": "If z>>0 and p_empirical small for tau or 125, that supports phase-lock/commensurability; otherwise tau/125 is not a privileged constant for the instantaneous rotation rate."
    }
}

# ensure JSON-serializable (handle inf)
def clean(obj):
...
out_clean = clean(out)
print(json.dumps(out_clean, indent=2, sort_keys=False))

### Tool (2026-02-22 19:45:45)
