...
- **Conversation ID:** `699b2f13-b24c-832f-8fd7-5902409daa21`
- **Date:** 2026-02-22

---

### User (2026-02-22 11:30:15)
...
### Assistant (2026-02-22 11:30:17)

import numpy as np, math, json, os, textwrap, warnings
from numpy.fft import rfft, irfft, rfftfreq

### Assistant (2026-02-22 11:30:54)

import numpy as np, math, json, os, warnings
from numpy.fft import rfft, irfft, rfftfreq
from scipy.signal import hilbert
from sklearn.metrics import roc_auc_score, log_loss

### Assistant (2026-02-22 11:30:58)

import numpy as np, os, math, json, warnings
data_psi = np.load('/mnt/data/chebyshev_psi.npz')
data_pr = np.load('/mnt/data/primes.npz')
list(data_psi.files), list(data_pr.files)[:10], {k:data_pr[k].shape for k in data_pr.files if hasattr(data_pr[k],'shape')}

### Tool (2026-02-22 11:31:01)

(['N', 'step', 'x_grid', 'psi_grid', 'psi_minus_x'],
 ['N', 'primes', 'is_prime'],
 {'N': (), 'primes': (664579,), 'is_prime': (10000001,)})

### Assistant (2026-02-22 11:31:03)

x_grid = data_psi['x_grid'].astype(float)
psi_minus_x = data_psi['psi_minus_x'].astype(float)
primes = data_pr['primes'].astype(int)
len(x_grid), x_grid[0], x_grid[-1], psi_minus_x[:3]

### Tool (2026-02-22 11:31:04)

(100000, 1.0, 9999901.0, array([-1.        , -2.33956825,  5.14585683]))

### Assistant (2026-02-22 11:31:06)
...
# keep indices in x range
keep = (x_uniform>=x_min) & (x_uniform<=x_max)
t = t_uniform[keep]
x = x_uniform[keep]
u_raw = u_uniform[keep]
N = len(x)
dt, N, (x[0], x[-1]), (t[0], t[-1])

### Tool (2026-02-22 11:31:13)
...
### Assistant (2026-02-22 11:31:15)

# causal moving average detrend W
W=int(W)
cumsum = np.cumsum(np.insert(u_raw,0,0.0))
# moving average for index i uses past W points including i: start=max(0,i-W+1)
# compute efficiently with convolution-like using cumulative sums
idx = np.arange(N)
start = np.maximum(0, idx-W+1)
# sum over [start, i]
sum_window = cumsum[idx+1]-cumsum[start]
...
mu = float(u_det[train_slice].mean())
sig = float(u_det[train_slice].std(ddof=0))
u_std = (u_det - mu) / (sig if sig>0 else 1.0)
mu, sig, n_train

### Tool (2026-02-22 11:31:20)

(7.296664503769434, 176.3404688541781, 12807)

### Assistant (2026-02-22 11:31:22)

def bandpass_fft(signal, f_lo, f_hi):
    n=len(signal)
    X = rfft(signal)
    freqs = rfftfreq(n, d=1.0)  # cycles/sample (since sample spacing 1)
...
theta_fast_unwrap = np.unwrap(np.angle(z_fast))  # unwrapped principal angle
omega_fast = np.diff(theta_fast_unwrap)  # length N-1

# Optional slow
u_slow = bandpass_fft(u_std, SLOW[0], SLOW[1])
z_slow = hilbert(u_slow)
theta_slow = np.mod(np.angle(z_slow), 2*np.pi)
theta_slow_unwrap = np.unwrap(np.angle(z_slow))
omega_slow = np.diff(theta_slow_unwrap)
len(theta_fast), len(omega_fast)

### Tool (2026-02-22 11:31:29)

(18296, 18295)

### Assistant (2026-02-22 11:31:30)

# Build R_i using primes counts in [x_i, x_i*exp(Delta))
expD = math.exp(Delta)
# Use primes array for counting via searchsorted
primes_sorted = primes
def count_primes_in_interval(a,b):
    # inclusive a <= p < b
    lo = np.searchsorted(primes_sorted, a, side='left')
    hi = np.searchsorted(primes_sorted, b, side='left')
    return hi-lo

# Vectorize counts with searchsorted on arrays
a = x
b = x*expD
lo = np.searchsorted(primes_sorted, a, side='left')
hi = np.searchsorted(primes_sorted, b, side='left')
...
### Assistant (2026-02-22 11:31:38)

# Define feature arrays aligned to omega (drop last)
theta_f_feat = theta_fast[:-1]
omega_f_feat = omega_fast
theta_s_feat = theta_slow[:-1]
omega_s_feat = omega_slow
R_feat = R[:-1]
N_feat = N-1

# Binning functions
theta_edges = np.linspace(0, 2*np.pi, B_theta+1)
def bin_uniform(vals, edges):
    # return bin index 0..B-1, with rightmost inclusive
    bins = np.searchsorted(edges, vals, side='right') - 1
    bins = np.clip(bins, 0, len(edges)-2)
    return bins

theta_bin = bin_uniform(theta_f_feat, theta_edges)

# omega quantile edges from TRAIN portion of omega_feat
omega_train = omega_f_feat[:n_train-1]  # since omega defined for indices < n_train-1 corresponds to i within train for theta_feat
qs = np.linspace(0,1,B_omega+1)
omega_edges = np.quantile(omega_train, qs)
# ensure monotone strictly increasing by jitter if ties
# if many ties, compress bins
unique_edges = np.unique(omega_edges)
len(unique_edges), omega_edges[:3], omega_edges[-3:]

### Tool (2026-02-22 11:31:45)

(11,
 array([-1.50970451,  0.00780703,  0.0148836 ]),
 array([0.03238952, 0.03783121, 1.23415628]))

### Assistant (2026-02-22 11:31:47)

def bin_by_edges(vals, edges):
    # edges length B+1
    bins = np.searchsorted(edges, vals, side='right') - 1
    bins = np.clip(bins, 0, len(edges)-2)
    return bins

omega_bin = bin_by_edges(omega_f_feat, omega_edges)

# Optional theta_s bins
theta_s_edges = np.linspace(0, 2*np.pi, B_theta_s+1)
theta_s_bin = bin_uniform(theta_s_feat, theta_s_edges)

# Build combined bin indices
B2 = B_theta*B_omega
bin2 = theta_bin*B_omega + omega_bin  # 0..B2-1

B3 = B_theta*B_omega*B_theta_s
bin3 = (theta_bin*B_omega + omega_bin)*B_theta_s + theta_s_bin
bin2.min(), bin2.max(), B2, bin3.max(), B3

### Tool (2026-02-22 11:31:53)

(0, 119, 120, 959, 960)

### Assistant (2026-02-22 11:31:55)

def compute_surface(bin_idx, y, valid_idx, n_bins):
    # returns p,n arrays length n_bins; bins with n=0 -> p=nan
    n = np.bincount(bin_idx[valid_idx], minlength=n_bins).astype(int)
    s = np.bincount(bin_idx[valid_idx], weights=y[valid_idx].astype(float), minlength=n_bins)
    p = np.full(n_bins, np.nan, dtype=float)
    nz = n>0
    p[nz] = s[nz]/n[nz]
    return p, n

def weighted_rmse(p_train, p_test, n_test):
    mask = np.isfinite(p_train) & np.isfinite(p_test) & (n_test>0)
    if not np.any(mask):
        return float('nan')
    w = n_test[mask].astype(float)
    return float(np.sqrt(np.sum(w*(p_train[mask]-p_test[mask])**2)/np.sum(w)))

def corr_bins(p_train, p_test, n_test, min_count=20):
    mask = np.isfinite(p_train) & np.isfinite(p_test) & (n_test>=min_count)
    if mask.sum()<2:
        return float('nan')
    a=p_train[mask]; b=p_test[mask]
    if np.std(a)==0 or np.std(b)==0:
        return float('nan')
    return float(np.corrcoef(a,b)[0,1])

def kl_occupancy(n_train, n_test, smoothing=1.0):
    P = (n_train + smoothing).astype(float)
    Q = (n_test + smoothing).astype(float)
    P /= P.sum()
    Q /= Q.sum()
    return float(np.sum(Q*np.log(Q/P)))

def transfer_metrics(p_train, bin_idx, y, test_valid_idx, eps=1e-6):
    # predictions on test: p_hat from p_train, fallback to global mean of train p over occupied bins
    # But in our surfaces, some bins may be nan; handle by replacing with overall mean of train y across train_valid
    p_hat = p_train[bin_idx[test_valid_idx]]
    # fill nan
    if np.any(~np.isfinite(p_hat)):
        # overall mean from finite p_train weighted by n? cannot compute here. We'll set to mean of y on test_valid? no leakage. Use mean of finite p_train.
        fallback = float(np.nanmean(p_train))
        p_hat = np.where(np.isfinite(p_hat), p_hat, fallback)
    y_true = y[test_valid_idx].astype(int)
    # AUC
    auc = float('nan')
    ...
    return {"auc": auc, "brier": brier, "logloss": ll}

### Assistant (2026-02-22 11:32:16)

# Build y_h on feature indices: y_h(i)=1{R_{i+h}-R_i>0}
def y_for_h(h):
    y = np.zeros(N_feat, dtype=int)
    # for i where i+h < N, compare R[i+h] - R[i]
    diff = R[h: h+N_feat] - R[:N_feat]  # R[h:] length N-h; but we slice to N_feat
    # Need ensure h+N_feat <= N
    # N_feat=N-1 so h+N_feat <= N+h-1 may exceed. We'll handle properly:

### Assistant (2026-02-22 11:32:20)

def y_for_h(h):
    y = np.full(N_feat, -1, dtype=int)  # -1 invalid
    max_i = N-1-h  # i <= this for i+h <= N-1
    if max_i < 0:
        return y
    idx_valid = np.arange(0, max_i+1)
    diff = R[idx_valid + h] - R[idx_valid]
    y[idx_valid] = (diff > 0).astype(int)
    return y

ys = {h: y_for_h(h) for h in H}
{h: (ys[h]==-1).sum() for h in H}, N, N_feat

### Tool (2026-02-22 11:32:25)

({25: 24, 100: 99}, 18296, 18295)

### Assistant (2026-02-22 11:32:27)

def valid_indices_for_split(h, split='train'):
    y = ys[h]
    valid = (y!=-1)
    if split=='train':
        # i and i+h in train region (indices < n_train)
        valid &= (np.arange(N_feat) < n_train) & (np.arange(N_feat)+h < n_train)
    else:
        valid &= (np.arange(N_feat) >= n_train) & (np.arange(N_feat)+h < N) & (np.arange(N_feat)+h >= n_train)
    return np.where(valid)[0]

# compute results per h
per_h={}
for h in H:
    train_idx = valid_indices_for_split(h,'train')
    test_idx = valid_indices_for_split(h,'test')
    y = ys[h]
    # fast2d surfaces
    p_tr, n_tr = compute_surface(bin2, y, train_idx, B2)
    p_te, n_te = compute_surface(bin2, y, test_idx, B2)
    stat = {
        "wrmse": weighted_rmse(p_tr, p_te, n_te),
        "corr": corr_bins(p_tr, p_te, n_te, min_count=20),
        "kl_occupancy": kl_occupancy(n_tr, n_te, smoothing=1.0)
    }
    transfer = transfer_metrics(p_tr, bin2, y, test_idx, eps=1e-6)
    fast2d = {
        "bin_defs": {"theta_bins": B_theta, "omega_bins": B_omega, "omega_edges": omega_edges.tolist()},
        "train": {"p": p_tr.tolist(), "n": n_tr.tolist()},
        "test": {"p": p_te.tolist(), "n": n_te.tolist()},
        "stationarity": stat,
        "transfer_perf": transfer,
        "splits": {"n_train_valid": int(len(train_idx)), "n_test_valid": int(len(test_idx))}
    }
    # optional 3d
    p3_tr, n3_tr = compute_surface(bin3, y, train_idx, B3)
    p3_te, n3_te = compute_surface(bin3, y, test_idx, B3)
    stat3 = {
        "wrmse": weighted_rmse(p3_tr, p3_te, n3_te),
        "corr": corr_bins(p3_tr, p3_te, n3_te, min_count=10), # relax due to sparsity
        "kl_occupancy": kl_occupancy(n3_tr, n3_te, smoothing=1.0)
    }
    transfer3 = transfer_metrics(p3_tr, bin3, y, test_idx, eps=1e-6)
    fast_slow3d = {
        "bin_defs": {"theta_f_bins": B_theta, "omega_f_bins": B_omega, "theta_s_bins": B_theta_s,
                     "omega_edges": omega_edges.tolist()},
        "train": {"p": p3_tr.tolist(), "n": n3_tr.tolist()},
        "test": {"p": p3_te.tolist(), "n": n3_te.tolist()},
        "stationarity": stat3,
        "transfer_perf": transfer3,
        "splits": {"n_train_valid": int(len(train_idx)), "n_test_valid": int(len(test_idx))}
    }
    per_h[str(h)] = {"fast2d": fast2d, "optional_fast_slow3d": fast_slow3d}

# stationarity decision heuristic
summary={"is_gating_surface_stationary":{}, "notes":""}
notes=[]
for h in H:
    s=per_h[str(h)]["fast2d"]["stationarity"]
    # heuristic: corr>0.6 and wrmse<0.1 and kl<0.05
    stationary = (np.isfinite(s["corr"]) and s["corr"]>0.6 and np.isfinite(s["wrmse"]) and s["wrmse"]<0.1 and s["kl_occupancy"]<0.05)
    summary["is_gating_surface_stationary"][str(h)] = bool(stationary)
    notes.append(f"h={h}: wrmse={s['wrmse']:.3f}, corr={s['corr']:.3f}, kl={s['kl_occupancy']:.3f}")
...
out = {
 "params":{
     ...
     "dt":dt,"n_kept":N,"n_feat":N_feat,"train_end_index":n_train
 },
 "per_h": per_h,
 "summary": summary
}
...
### Tool (2026-02-22 11:32:49)

61359

### Assistant (2026-02-22 11:32:52)
...
# quick check for "NaN"
s = json.dumps(out_s)
("NaN" in s, "Infinity" in s, len(s))

### Tool (2026-02-22 11:32:56)
