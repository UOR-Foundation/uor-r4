# Log-spectrum stability audit

- **Conversation ID:** `699a9b7a-9eb4-8330-82b0-f541319313b1`
- **Date:** 2026-02-22

---

### User (2026-02-22 01:00:30)
...
### Assistant (2026-02-22 01:00:32)

import numpy as np, json, math, os
from scipy import signal, interpolate
path="/mnt/data/chebyshev_psi.npz"
data=np.load(path)
data.files[:10], {k:data[k].shape for k in data.files}

### Tool (2026-02-22 01:00:51)
...
### Assistant (2026-02-22 01:00:54)

from scipy.signal import welch, find_peaks
def pipeline(Nmax, L=65536, nperseg=4096):
    x=data['x_grid']
    f=data['psi_minus_x']
    mask=x<=Nmax
    x=x[mask]; f=f[mask]
    ...
    # interpolation
    u=np.interp(t_uniform, t, f.astype(np.float64))
    # standardize
    u=(u - u.mean())/u.std(ddof=0)
    # welch PSD (cycles/sample)
    freqs, Pxx = welch(u, fs=1.0, window='hann', nperseg=nperseg, noverlap=nperseg//2, detrend='constant', scaling='density')
    # band selection
    band=(freqs>=0.002) & (freqs<=0.02)
    fb=freqs[band]; Pb=Pxx[band]
    # find peaks
    peaks, props = find_peaks(Pb, distance=2)  # minimal separation 2 bins
    if len(peaks)==0:
        return dt, [], []
    peak_freqs=fb[peaks]
    peak_powers=Pb[peaks]
    # take top 20 by power
    order=np.argsort(peak_powers)[::-1]
    top=order[:20]
    top_freqs=peak_freqs[top]
    top_pows=peak_powers[top]
    # sort by frequency for easier matching
    sort_idx=np.argsort(top_freqs)
    top_freqs=top_freqs[sort_idx]
    top_pows=top_pows[sort_idx]
    gamma=2*np.pi*top_freqs/dt
    return dt, top_freqs.tolist(), gamma.tolist()

results={}
for Nmax in [1_000_000, 3_000_000, 10_000_000]:
    dt, pf, gamma = pipeline(Nmax)
    results[str(Nmax)]={"dt":dt,"peak_frequencies":pf,"gamma_est":gamma}
len(results["10000000"]["peak_frequencies"]), results["10000000"]["dt"]

### Tool (2026-02-22 01:01:04)

(18, 0.00024594622340595577)

### Assistant (2026-02-22 01:01:07)

{k:len(v["peak_frequencies"]) for k,v in results.items()}, results["1000000"]["dt"], results["3000000"]["dt"]

### Tool (2026-02-22 01:01:09)

({'1000000': 17, '3000000': 19, '10000000': 18},
 0.00021080966740006792,
 0.0002275744235307526)

### Assistant (2026-02-22 01:01:11)

import numpy as np
ref=np.array(results["10000000"]["peak_frequencies"])
def mean_shift(other):
    other=np.array(other)
    shifts=[]
    for f0 in ref:
        j=np.argmin(np.abs(other-f0))
        shifts.append(abs(other[j]-f0))
    return np.array(shifts)
sh1=mean_shift(results["1000000"]["peak_frequencies"])
sh3=mean_shift(results["3000000"]["peak_frequencies"])
all_shifts=np.concatenate([sh1,sh3])
mean_peak_shift=float(all_shifts.mean())
std_peak_shift=float(all_shifts.std(ddof=0))
mean_peak_shift, std_peak_shift, sh1.mean(), sh3.mean()

### Tool (2026-02-22 01:01:17)

(0.00030517578125,
 0.0002466706472367786,
 0.00031195746527777775,
 0.00029839409722222225)

### Assistant (2026-02-22 01:01:20)
