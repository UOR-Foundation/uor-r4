# Phase-drift Test Results

- **Conversation ID:** `699c4fb2-9e30-8333-81e6-35454eed1884`
- **Date:** 2026-02-23

---

### User (2026-02-23 08:01:41)
...
# pull fingerprints expected
dt=float(canonical["dt"])
N_kept=int(canonical["N_kept"]) if "N_kept" in canonical.files else len(canonical["R"])
train_end=int(canonical["train_end"])
theta_canon=canonical["theta_mod"]
R=canonical["R"]
# verify expected
(dt, N_kept, train_end, theta_canon.shape, R.shape)

### Tool (2026-02-23 08:02:02)

(0.00012297217348543395, 36592, 25614, (36592,), (36592,))

### Assistant (2026-02-23 08:02:05)

canonical.files[:20], ("x_kept" in canonical.files)

### Tool (2026-02-23 08:02:16)

(['x_kept', 'u_bp', 'theta_mod', 'R', 'dt', 'train_end', 'params', 'hashes'],
 True)

### Assistant (2026-02-23 08:02:17)

x_kept=canonical["x_kept"]
hashes=canonical["hashes"].item()
hashes

### Tool (2026-02-23 08:02:19)

'{"x_kept": "c455fedeff29bb2d0b2c81864970c09638720baf", "u_bp": "b16b32cb5d4e63be1c61b88fcf4711d0656da14e", "theta_mod": "8b33c57df436d37179b13b1899bf86a723f0e98a", "R": "5753ee7d59c7d155c4a1b9b5fba8912d557598f2"}'

### Assistant (2026-02-23 08:02:20)
...
u_raw=np.interp(t_uniform, t_raw, psi_minus_x.astype(np.float64))
x_uniform=np.exp(t_uniform)
# restrict x range
x_min, x_max=1e5, 9e6
keep=(x_uniform>=x_min) & (x_uniform<=x_max)
x_kept_new=x_uniform[keep]
# dt
dt_new=float(t_uniform[1]-t_uniform[0])
N_kept_new=int(keep.sum())
train_end_new=int(round(0.7*N_kept_new))
dt_new, N_kept_new, train_end_new, x_kept_new[:3], x_kept_new[-3:]

### Tool (2026-02-23 08:02:30)
...
### Assistant (2026-02-23 08:02:31)

def sha1_arr(a: np.ndarray)->str:
    # ensure contiguous bytes, little-endian float64
    return hashlib.sha1(np.ascontiguousarray(a).view(np.uint8)).hexdigest()
sha1_x=sha1_arr(x_kept_new)
sha1_x, json.loads(hashes)["x_kept"]

### Tool (2026-02-23 08:02:36)

('c455fedeff29bb2d0b2c81864970c09638720baf',
 'c455fedeff29bb2d0b2c81864970c09638720baf')

### Assistant (2026-02-23 08:02:38)

u_raw_kept=u_raw[keep]
W=2000
# causal moving average including current sample: for i, mean of u_raw_kept[max(0,i-W+1):i+1]
# efficient via cumsum
cs=np.cumsum(np.insert(u_raw_kept,0,0.0))
idx=np.arange(len(u_raw_kept))
start=np.maximum(0, idx-W+1)
count=idx-start+1
ma=(cs[idx+1]-cs[start])/count
u_detr=u_raw_kept - ma
# zscore using train only
mu=u_detr[:train_end_new].mean()
sd=u_detr[:train_end_new].std(ddof=0)
u_z=(u_detr-mu)/sd
# bandpass function with rfft mask
def bandpass_rfft(u, f_lo, f_hi):
    ...
    U_bp=U*mask
    return np.fft.irfft(U_bp, n=n)
u_bp_repro=bandpass_rfft(u_z, 0.002,0.01)
sha1_u_bp=sha1_arr(u_bp_repro)
sha1_u_bp, json.loads(hashes)["u_bp"]

### Tool (2026-02-23 08:02:54)

('b16b32cb5d4e63be1c61b88fcf4711d0656da14e',
 'b16b32cb5d4e63be1c61b88fcf4711d0656da14e')

### Assistant (2026-02-23 08:02:55)

u_bp_new=bandpass_rfft(u_z, 0.003,0.009)
analytic=hilbert(u_bp_new)
theta_new=np.angle(analytic) % (2*np.pi)
theta_sha1=sha1_arr(theta_new)
theta_sha1

### Tool (2026-02-23 08:03:00)

'c888f75abc219749cde8f870c119a6dcd98ec2b4'

### Assistant (2026-02-23 08:03:01)

fc=np.load(featurecache_path, allow_pickle=True)
fc.files

### Tool (2026-02-23 08:03:04)
...
### Assistant (2026-02-23 08:03:05)

dt_fc=float(fc["dt"]); N_fc=int(fc["N_kept"]); train_end_fc=int(fc["train_end"])
(dt_fc, N_fc, train_end_fc, sha1_arr(fc["R"]), sha1_arr(fc["theta"]))

### Tool (2026-02-23 08:03:13)
...
N=N_kept
i_all=np.arange(N)
valid = (i_all>=79) & (i_all>=k) & (i_all+h < N)
train_mask = valid & (i_all < train_end) & (i_all + h < train_end)
test_mask  = valid & (i_all >= train_end) & (i_all + h < N)
i_train=i_all[train_mask]
i_test=i_all[test_mask]
len(i_train), len(i_test)

### Tool (2026-02-23 08:03:47)

(25485, 10928)

### Assistant (2026-02-23 08:03:49)
...
### Tool (2026-02-23 08:03:51)

---------------------------------------------------------------------------
IndexError                                Traceback (most recent call last)
Cell In[11], line 1
----> 1 y = (R[i_all+h] - R[i_all] > 0).astype(int)
      2 y_train=y[i_train]; y_test=y[i_test]
      3 pos_rate_test=float(y_test.mean())

IndexError: index 36592 is out of bounds for axis 0 with size 36592

### Assistant (2026-02-23 08:03:53)

y_full=np.zeros(N, dtype=int)
idx=np.arange(N-h)
y_full[idx]= (R[idx+h]-R[idx] > 0).astype(int)
y_train=y_full[i_train]; y_test=y_full[i_test]
pos_rate_test=float(y_test.mean())
pos_rate_test

### Tool (2026-02-23 08:03:56)

0.5058565153733529

### Assistant (2026-02-23 08:03:58)

X80=fc["X80"]
X80.shape, X80.dtype

### Tool (2026-02-23 08:04:01)

((36592, 80), dtype('float32'))

### Assistant (2026-02-23 08:04:03)

mu_X80=fc["mu_X80_train"].astype(np.float64)
sd_X80=fc["sd_X80_train"].astype(np.float64)
# avoid zero
sd_X80[sd_X80==0]=1.0
X80_std=((X80.astype(np.float64)-mu_X80)/sd_X80)
# theta_block from theta_new
S=np.column_stack([np.sin(theta_new), np.cos(theta_new)])
# dtheta features
dtheta=wrap_to_pi(theta_new - np.roll(theta_new, k))
# but for i<k, roll uses end; those indices excluded by mask i>=k so ok
D=np.column_stack([np.sin(dtheta), np.cos(dtheta)])
# standardize S and D using train indices only
def standardize_block(M, idx_train):
    mu=M[idx_train].mean(axis=0)
    sd=M[idx_train].std(axis=0, ddof=0)
    sd[sd==0]=1.0
    return (M-mu)/sd, mu, sd
S_std, mu_S, sd_S = standardize_block(S, i_train)
D_std, mu_D, sd_D = standardize_block(D, i_train)

# Build design matrices for M1 and M2
X1=np.hstack([X80_std, S_std])
X2=np.hstack([X80_std, S_std, D_std])

X1_train=X1[i_train]; X1_test=X1[i_test]
X2_train=X2[i_train]; X2_test=X2[i_test]

### Assistant (2026-02-23 08:04:15)

def fit_lr(X, y):
    clf=LogisticRegression(penalty="l2", C=1.0, solver="liblinear", max_iter=300, fit_intercept=True)
    clf.fit(X, y)
    return clf
clf1=fit_lr(X1_train, y_train)
clf2=fit_lr(X2_train, y_train)
p1=clf1.predict_proba(X1_test)[:,1]
p2=clf2.predict_proba(X2_test)[:,1]
auc1=float(roc_auc_score(y_test, p1))
auc2=float(roc_auc_score(y_test, p2))
auc1, auc2, auc2-auc1

### Tool (2026-02-23 08:04:20)

(0.8532432867020421, 0.8536642413035321, 0.00042095460149005337)

### Assistant (2026-02-23 08:04:22)

p0=clf1.predict_proba(X1_train)[:,1]
res=y_train - p0
Z=D_std[i_train]  # standardized
W=p0*(1-p0)
U=Z.T @ res
I=Z.T @ (Z*W[:,None])
# invert
T=float(U.T @ np.linalg.inv(I) @ U)
p_chi=float(1-chi2.cdf(T, df=2))
T, p_chi

### Tool (2026-02-23 08:04:30)

(13.162015831641128, 0.0013864511642923683)

### Assistant (2026-02-23 08:04:31)

rng=npr.default_rng(12345)
min_shift=50
reps=300
T_null=np.empty(reps)
for r in range(reps):
    s=int(rng.integers(min_shift, N-min_shift))
    th_shift=np.roll(theta_new, s)
    dth_shift=wrap_to_pi(th_shift - np.roll(th_shift, k))
    D_shift=np.column_stack([np.sin(dth_shift), np.cos(dth_shift)])
    Zs=(D_shift - mu_D)/sd_D
    Zs_train=Zs[i_train]
    U_s=Zs_train.T @ res
    I_s=Zs_train.T @ (Zs_train*W[:,None])
    T_null[r]=float(U_s.T @ np.linalg.inv(I_s) @ U_s)
T_p95=float(np.quantile(T_null,0.95))
T_p99=float(np.quantile(T_null,0.99))
emp_p=float((1+np.sum(T_null>=T))/(1+reps))
T_p95, T_p99, emp_p, T_null.mean()

### Tool (2026-02-23 08:04:43)
...
### Assistant (2026-02-23 08:05:16)

Xn = np.hstack([np.ones((len(i_train),1)), X1_train])  # nuisance with intercept
# Weighted matrices
WXn = Xn * W[:,None]
A = Xn.T @ WXn  # (p x p)
B = Xn.T @ (Z*W[:,None])  # (p x 2)
C = Z.T @ (Z*W[:,None])   # (2 x 2)
# efficient information for Z
I_eff = C - B.T @ np.linalg.inv(A) @ B
# efficient score
U_raw = Z.T @ res
U_eff = U_raw - B.T @ np.linalg.inv(A) @ (Xn.T @ res)  # but Xn.T@res should be ~0 at MLE
T_eff=float(U_eff.T @ np.linalg.inv(I_eff) @ U_eff)
p_eff=float(1-chi2.cdf(T_eff, df=2))
T_eff, p_eff, float(np.linalg.norm(Xn.T@res))

### Tool (2026-02-23 08:05:43)

(13.217403986054546, 0.001348581482010336, 1.9318303328242792)

### Assistant (2026-02-23 08:05:49)

A_inv=np.linalg.inv(A)
XnTres = Xn.T @ res
T_null_eff=np.empty(reps)
for r in range(reps):
    s=int(rng.integers(min_shift, N-min_shift))
    th_shift=np.roll(theta_new, s)
    dth_shift=wrap_to_pi(th_shift - np.roll(th_shift, k))
    D_shift=np.column_stack([np.sin(dth_shift), np.cos(dth_shift)])
    Zs=((D_shift - mu_D)/sd_D)[i_train]
    # compute B_s, C_s
    B_s = Xn.T @ (Zs*W[:,None])
    C_s = Zs.T @ (Zs*W[:,None])
    Ieff_s = C_s - B_s.T @ A_inv @ B_s
    Uraw_s = Zs.T @ res
    Ueff_s = Uraw_s - B_s.T @ A_inv @ XnTres
    T_null_eff[r]=float(Ueff_s.T @ np.linalg.inv(Ieff_s) @ Ueff_s)
T_p95=float(np.quantile(T_null_eff,0.95))
T_p99=float(np.quantile(T_null_eff,0.99))
emp_p=float((1+np.sum(T_null_eff>=T_eff))/(1+reps))
T_p95, T_p99, emp_p, T_null_eff.mean(), T_eff

### Tool (2026-02-23 08:06:03)
