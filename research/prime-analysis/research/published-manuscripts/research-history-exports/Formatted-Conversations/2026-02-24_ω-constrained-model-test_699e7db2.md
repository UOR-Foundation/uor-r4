# ω-constrained model test

- **Conversation ID:** `699e7db2-84e4-832c-a3eb-9328d2d4d70f`
- **Date:** 2026-02-24

---

### User (2026-02-24 23:42:28)
...
# Paths
featurecache_path="/mnt/data/A4_FEATURECACHE_h25_AR80_AR40_theta_omega_masks_v1.npz"
psi_path="/mnt/data/chebyshev_psi.npz"
canonical_path="/mnt/data/A4_CANONICAL_CACHE_L131072_x1e5_9e6_W2000_bp002_001_D001_seed12345.npz"

fc=np.load(featurecache_path, allow_pickle=True)
canon=np.load(canonical_path, allow_pickle=True)
psi=np.load(psi_path, allow_pickle=True)

# Extract canonical fingerprints and core params
dt=float(canon["dt"])
train_end=int(canon["train_end"])
R_canon=canon["R"]
theta_canon=canon["theta_mod"]
x_kept=canon["x_kept"]
N_kept=int(len(R_canon))

# Check dt/N_kept/train_end vs expected from project history
expected_dt=0.00012297217348543395
expected_N=36592
expected_train_end=25614
identity_ok = (abs(dt-expected_dt) < 1e-15) and (N_kept==expected_N) and (train_end==expected_train_end)

# Also verify featurecache dt/N_kept/train_end if present
fc_dt = float(fc["dt"]) if "dt" in fc.files else None
fc_train_end = int(fc["train_end"]) if "train_end" in fc.files else None
fc_N = int(fc["N_kept"]) if "N_kept" in fc.files else (fc["R"].shape[0] if "R" in fc.files else None)
identity_ok = identity_ok and (fc_dt is None or abs(fc_dt-dt)<1e-15) and (fc_train_end is None or fc_train_end==train_end) and (fc_N is None or fc_N==N_kept)

if not identity_ok:
    out={"identity":{"status":"FAIL","theta_sha1":""},"notes":"Fingerprint-lock failure (dt/N_kept/train_end mismatch)."}
    print(json.dumps(out, indent=2))
    raise SystemExit

# Recompute theta_new with new bandpass [0.003,0.009] using canonical pipeline L=131072, x_range [1e5,9e6], W=2000, train-only z
L=int(canon["L"]) if "L" in canon.files else 131072
x_min=float(canon["x_min"]) if "x_min" in canon.files else 100000.0
x_max=float(canon["x_max"]) if "x_max" in canon.files else 9000000.0
W=int(canon["detrend_W"]) if "detrend_W" in canon.files else 2000
...
t_uniform=np.linspace(t_min, t_max, L, endpoint=True)
# interpolate psi_minus_x onto t_uniform using x as exp(t_uniform)
# need u_raw on t_uniform: interpolate psi_minus_x as function of t_raw
u_raw=np.interp(t_uniform, t_raw, psi_minus_x.astype(float))

# compute x_uniform, keep indices within [x_min,x_max]
x_uniform=np.exp(t_uniform)
keep_mask=(x_uniform>=x_min) & (x_uniform<=x_max)
x_kept_new=x_uniform[keep_mask]
# ensure x_kept_new matches canonical x_kept
if x_kept_new.shape[0]!=x_kept.shape[0] or np.max(np.abs(x_kept_new - x_kept))>1e-9:
    out={"identity":{"status":"FAIL","theta_sha1":""},"notes":"x_kept mismatch vs canonical (pipeline drift)."}
    print(json.dumps(out, indent=2))
    raise SystemExit

u_raw_kept=u_raw[keep_mask]

# causal moving average past-only including current, window W
# efficient using cumulative sum
u=u_raw_kept
cs=np.cumsum(np.insert(u,0,0.0))
# for i, start=max(0,i-W+1), len=i-start+1
idx=np.arange(len(u))
...
# standardize using TRAIN-only (first train_end samples)
mu=u_detr[:train_end].mean()
sd=u_detr[:train_end].std(ddof=0)
u_z=(u_detr - mu) / (sd if sd>0 else 1.0)

# bandpass rfft mask for cycles/sample in [0.003,0.009]
bp_lo=0.003; bp_hi=0.009
U=rfft(u_z)
...
theta_new=np.angle(analytic)  # in [-pi,pi]
theta_new_mod=np.mod(theta_new, 2*np.pi)

# sha1 of theta_new_mod float64 bytes
theta_sha1=hashlib.sha1(theta_new_mod.astype(np.float64).tobytes()).hexdigest()

# Load X80 from featurecache (assume already standardized using TRAIN for h=25 masks; but we'll use as-is)
# Try keys
X80=None
for k in ["X80", "X_AR80", "X80_std"]:
    if k in fc.files:
        X80=fc[k]
        break
if X80 is None:
    # reconstruct X80 from R_canon
    p=80
    X80=np.column_stack([R_canon[np.arange(p-1, N_kept-1)-j] for j in range(p)])
# Ensure X80 rows align with indices i that are valid for lag79 (i>=79). Typically X80 corresponds to i indices starting at 79.
# Determine if X80 has N_kept rows or N_kept-79
if X80.shape[0]==N_kept:
    # assume rows correspond to all i, with first 79 maybe garbage/zeros; we'll slice by i indices directly
    pass
elif X80.shape[0]==N_kept-79:
    # rows correspond to i=79..N_kept-1
    pass
else:
    # attempt to infer
    pass

# Helper to get X row for index i
def get_X_rows(indices):
    if X80.shape[0]==N_kept:
        return X80[indices]
    elif X80.shape[0]==N_kept-79:
        return X80[indices-79]
    else:
        raise ValueError("Unexpected X80 shape")

# Build masks for h=100
h=100
i_all=np.arange(N_kept)
train_valid = (i_all>=79) & (i_all<train_end) & (i_all+h<train_end)
test_valid  = (i_all>=79) & (i_all>=train_end) & (i_all+h<N_kept)

train_idx=i_all[train_valid]
test_idx=i_all[test_valid]

# Labels
y = (R_canon[i_all+h] - R_canon[i_all] > 0).astype(int)  # this works for i up to N_kept-h-1; we'll index only valid
y_train=y[train_idx]
y_test=y[test_idx]

# Baseline AR80 logistic
X_train=get_X_rows(train_idx)
X_test=get_X_rows(test_idx)

# Standardize X based on TRAIN-valid subset (avoid using featurecache mu/sd which maybe for different mask)
muX=X_train.mean(axis=0)
sdX=X_train.std(axis=0, ddof=0)
sdX[sdX==0]=1.0
X_train_z=(X_train-muX)/sdX
X_test_z=(X_test-muX)/sdX

def fit_logit(Xtr, ytr):
    clf=LogisticRegression(penalty="l2", C=1.0, solver="lbfgs", max_iter=2000, random_state=12345)
    clf.fit(Xtr, ytr)
    return clf

base=fit_logit(X_train_z, y_train)
p0=base.predict_proba(X_test_z)[:,1]
auc0=roc_auc_score(y_test, p0)
ll0=log_loss(y_test, p0, labels=[0,1])

# Unconstrained AR80 + sin/cos(theta_new_mod at i)
sint=np.sin(theta_new_mod)
cost=np.cos(theta_new_mod)
X_train_u=np.column_stack([X_train_z, sint[train_idx], cost[train_idx]])
X_test_u=np.column_stack([X_test_z, sint[test_idx], cost[test_idx]])

mod_u=fit_logit(X_train_u, y_train)
p1u=mod_u.predict_proba(X_test_u)[:,1]
auc_u=roc_auc_score(y_test, p1u)
ll_u=log_loss(y_test, p1u, labels=[0,1])

coef_u=mod_u.coef_[0]
a_u=float(coef_u[-2]); b_u=float(coef_u[-1])
A_u=float(math.hypot(a_u,b_u))
theta_star_u=float(math.atan2(a_u,b_u))
# wrap to [-pi,pi]
def wrap_to_pi(x):
    return ((x+math.pi)%(2*math.pi))-math.pi
theta_star_u=wrap_to_pi(theta_star_u)

# omega_eff from known theta stars
theta25=1.798790823975004
theta50=1.2740265612162895
omega_eff=wrap_to_pi(theta50-theta25)/25.0

# rotated phi = wrap(theta - omega*h) mapped to [0,2pi)
phi=theta_new_mod - omega_eff*h
phi_wrapped=np.mod(phi, 2*np.pi)
sinp=np.sin(phi_wrapped); cosp=np.cos(phi_wrapped)

X_train_r=np.column_stack([X_train_z, sinp[train_idx], cosp[train_idx]])
X_test_r=np.column_stack([X_test_z, sinp[test_idx], cosp[test_idx]])

mod_r=fit_logit(X_train_r, y_train)
p1r=mod_r.predict_proba(X_test_r)[:,1]
auc_r=roc_auc_score(y_test, p1r)
ll_r=log_loss(y_test, p1r, labels=[0,1])

coef_r=mod_r.coef_[0]
a_r=float(coef_r[-2]); b_r=float(coef_r[-1])
A_r=float(math.hypot(a_r,b_r))
theta_star_r=float(wrap_to_pi(math.atan2(a_r,b_r)))

# comparison
delta_auc_u=auc_u-auc0
delta_ll_u=ll_u-ll0
delta_auc_r=auc_r-auc0
delta_ll_r=ll_r-ll0

# sanity predicted theta*_100 from h50
theta_pred_100=wrap_to_pi(theta50 + omega_eff*(100-50))
theta_star_uncon_minus_pred=wrap_to_pi(theta_star_u - theta_pred_100)

out={
  "identity":{"status":"PASS","theta_sha1":theta_sha1},
  "omega":{"omega_eff_rad_per_sample":omega_eff},
  "unconstrained":{
    "coef":{"a_sin":a_u,"b_cos":b_u,"A_amp":A_u,"theta_star_rad":theta_star_u},
    "test":{"auc":float(auc_u),"logloss":float(ll_u),"delta_auc_vs_p0":float(delta_auc_u),"delta_logloss_vs_p0":float(delta_ll_u)}
  },
  "rotated":{
    "coef":{"a_sin":a_r,"b_cos":b_r,"A_amp":A_r,"theta_star_rot_rad":theta_star_r},
    "test":{"auc":float(auc_r),"logloss":float(ll_r),"delta_auc_vs_p0":float(delta_auc_r),"delta_logloss_vs_p0":float(delta_ll_r)}
  },
  "comparison":{
    "delta_auc_rot_minus_uncon":float(auc_r-auc_u),
    "delta_logloss_rot_minus_uncon":float(ll_r-ll_u)
  },
  "sanity":{
    "theta_pred_h100_from_h50":theta_pred_100,
    "theta_star_uncon_minus_pred_wrapped":theta_star_uncon_minus_pred
  },
  "notes":"Rotating sin/cos by constant omega*h is a linear reparameterization; any difference vs unconstrained is due to finite-sample fitting/regularization/numerics, not new information. Theta recomputed with canonical pipeline except bandpass [0.003,0.009]."
}
print(json.dumps(out, indent=2))

### Tool (2026-02-24 23:43:46)

---------------------------------------------------------------------------
IndexError                                Traceback (most recent call last)
Cell In[1], line 142
    139 test_idx=i_all[test_valid]
    141 # Labels
--> 142 y = (R_canon[i_all+h] - R_canon[i_all] > 0).astype(int)  # this works for i up to N_kept-h-1; we'll index only valid
    143 y_train=y[train_idx]
    144 y_test=y[test_idx]

IndexError: index 36592 is out of bounds for axis 0 with size 36592

### Assistant (2026-02-24 23:43:49)
...
dt=float(canon["dt"]); train_end=int(canon["train_end"])
R_canon=canon["R"]; x_kept=canon["x_kept"]
N_kept=len(R_canon)
...
if not identity_ok:
    print(json.dumps({"identity":{"status":"FAIL","theta_sha1":""},"notes":"Fingerprint-lock failure (dt/N_kept/train_end mismatch)."}, indent=2))
    raise SystemExit
...
t_raw=np.log(x_grid.astype(float))
t_uniform=np.linspace(t_raw.min(), t_raw.max(), L, endpoint=True)
u_raw=np.interp(t_uniform, t_raw, psi_minus_x.astype(float))
x_uniform=np.exp(t_uniform)
keep_mask=(x_uniform>=x_min)&(x_uniform<=x_max)
x_kept_new=x_uniform[keep_mask]
...
if x_kept_new.shape[0]!=x_kept.shape[0] or np.max(np.abs(x_kept_new-x_kept))>1e-9:
    print(json.dumps({"identity":{"status":"FAIL","theta_sha1":""},"notes":"x_kept mismatch vs canonical (pipeline drift)."}, indent=2))
    raise SystemExit
...
ma=(cs[idx+1]-cs[start])/count
u_detr=u-ma
mu=u_detr[:train_end].mean(); sd=u_detr[:train_end].std(ddof=0); sd=sd if sd>0 else 1.0
u_z=(u_detr-mu)/sd

bp_lo=0.003; bp_hi=0.009
U=rfft(u_z)
freq=rfftfreq(len(u_z), d=1.0)
mask=(freq>=bp_lo)&(freq<=bp_hi)
u_bp=irfft(U*mask, n=len(u_z))
theta_new_mod=np.mod(np.angle(hilbert(u_bp)), 2*np.pi)
theta_sha1=hashlib.sha1(theta_new_mod.astype(np.float64).tobytes()).hexdigest()

# X80 from featurecache
X80=None
for k in ["X80","X_AR80","X80_std","X80z","X80_z"]:
    if k in fc.files:
        X80=fc[k]
        break
if X80 is None:
...
def get_X_rows(indices):
    if X80.shape[0]==N_kept:
        ...
        raise ValueError(f"Unexpected X80 shape {X80.shape}")

h=100
i_all=np.arange(N_kept)
train_valid=(i_all>=79)&(i_all<train_end)&(i_all+h<train_end)
test_valid=(i_all>=79)&(i_all>=train_end)&(i_all+h<N_kept)
train_idx=i_all[train_valid]; test_idx=i_all[test_valid]

y=np.empty(N_kept, dtype=int)
y[:] = 0
valid_y = i_all < (N_kept-h)
y[valid_y]=(R_canon[i_all[valid_y]+h]-R_canon[i_all[valid_y]]>0).astype(int)

y_train=y[train_idx]; y_test=y[test_idx]
X_train=get_X_rows(train_idx); X_test=get_X_rows(test_idx)

muX=X_train.mean(axis=0); sdX=X_train.std(axis=0, ddof=0); sdX[sdX==0]=1.0
X_train_z=(X_train-muX)/sdX
X_test_z=(X_test-muX)/sdX

def fit_logit(Xtr, ytr):
    clf=LogisticRegression(penalty="l2", C=1.0, solver="lbfgs", max_iter=3000, random_state=12345)
    clf.fit(Xtr, ytr)
    return clf

base=fit_logit(X_train_z, y_train)
p0=base.predict_proba(X_test_z)[:,1]
auc0=float(roc_auc_score(y_test,p0))
ll0=float(log_loss(y_test,p0,labels=[0,1]))

sint=np.sin(theta_new_mod); cost=np.cos(theta_new_mod)
Xu_tr=np.column_stack([X_train_z, sint[train_idx], cost[train_idx]])
Xu_te=np.column_stack([X_test_z, sint[test_idx], cost[test_idx]])
mod_u=fit_logit(Xu_tr, y_train)
p1u=mod_u.predict_proba(Xu_te)[:,1]
auc_u=float(roc_auc_score(y_test,p1u))
ll_u=float(log_loss(y_test,p1u,labels=[0,1]))
a_u=float(mod_u.coef_[0][-2]); b_u=float(mod_u.coef_[0][-1])
A_u=float(math.hypot(a_u,b_u))
theta_star_u=float(wrap_to_pi(math.atan2(a_u,b_u)))

theta25=1.798790823975004; theta50=1.2740265612162895
omega_eff=float(wrap_to_pi(theta50-theta25)/25.0)

phi=np.mod(theta_new_mod - omega_eff*h, 2*np.pi)
sinp=np.sin(phi); cosp=np.cos(phi)
Xr_tr=np.column_stack([X_train_z, sinp[train_idx], cosp[train_idx]])
Xr_te=np.column_stack([X_test_z, sinp[test_idx], cosp[test_idx]])
mod_r=fit_logit(Xr_tr, y_train)
p1r=mod_r.predict_proba(Xr_te)[:,1]
auc_r=float(roc_auc_score(y_test,p1r))
ll_r=float(log_loss(y_test,p1r,labels=[0,1]))
a_r=float(mod_r.coef_[0][-2]); b_r=float(mod_r.coef_[0][-1])
A_r=float(math.hypot(a_r,b_r))
theta_star_r=float(wrap_to_pi(math.atan2(a_r,b_r)))

theta_pred_100=float(wrap_to_pi(theta50 + omega_eff*(100-50)))
theta_star_uncon_minus_pred=float(wrap_to_pi(theta_star_u - theta_pred_100))
...
out={
 ...
 "unconstrained":{
   "coef":{"a_sin":a_u,"b_cos":b_u,"A_amp":A_u,"theta_star_rad":theta_star_u},
   "test":{"auc":auc_u,"logloss":ll_u,"delta_auc_vs_p0":auc_u-auc0,"delta_logloss_vs_p0":ll_u-ll0}
 },
 "rotated":{
   "coef":{"a_sin":a_r,"b_cos":b_r,"A_amp":A_r,"theta_star_rot_rad":theta_star_r},
   "test":{"auc":auc_r,"logloss":ll_r,"delta_auc_vs_p0":auc_r-auc0,"delta_logloss_vs_p0":ll_r-ll0}
 },
 "comparison":{
   "delta_auc_rot_minus_uncon":auc_r-auc_u,
   "delta_logloss_rot_minus_uncon":ll_r-ll_u
 },
 "sanity":{
   "theta_pred_h100_from_h50":theta_pred_100,
   "theta_star_uncon_minus_pred_wrapped":theta_star_uncon_minus_pred
