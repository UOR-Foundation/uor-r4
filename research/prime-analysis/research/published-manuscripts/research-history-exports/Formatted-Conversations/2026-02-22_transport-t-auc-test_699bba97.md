# Transport T AUC Test

- **Conversation ID:** `699bba97-3668-832b-8302-890e8447254e`
- **Date:** 2026-02-22

---

### User (2026-02-22 21:25:31)
...
R=data["R"]
N_kept=int(data["N_kept"]) if "N_kept" in data.files else len(R)
train_end=int(data["train_end"])
h=25
p=20
# hashes sha1 of raw float arrays bytes
def sha1_arr(a):
    a=np.ascontiguousarray(a)
    return hashlib.sha1(a.view(np.uint8)).hexdigest()
hash_theta=sha1_arr(theta)
hash_R=sha1_arr(R)
...
# transport T(i)
dtheta=wrap_to_pi(theta[h:]-theta[:-h])  # length N_kept-h
T_full=dtheta/(2*np.pi)  # in (-0.5,0.5]
# For indices i where defined: i in [0, N_kept-h-1]
# Build valid rows i>=p and i+h < N_kept => i <= N_kept-h-1
valid_i=np.arange(p, N_kept-h, dtype=int)
# split
train_mask=valid_i < train_end
test_mask=~train_mask
train_i=valid_i[train_mask]
test_i=valid_i[test_mask]
n_train=len(train_i); n_test=len(test_i)

# target y(i)
dR=R[h:]-R[:-h]  # length N_kept-h aligned with i
y_full=(dR>0).astype(int)
y_train=y_full[train_i]
y_test=y_full[test_i]

pos_rate_train=float(y_train.mean())
pos_rate_test=float(y_test.mean())

# build X_AR for given indices i: [R[i], R[i-1],...,R[i-p]] length p+1
def build_X_AR(indices):
    # create matrix (len, p+1)
    X=np.empty((len(indices), p+1), dtype=float)
    for j,lag in enumerate(range(0,p+1)):
        X[:,j]=R[indices - lag]
    return X
X_ar_train=build_X_AR(train_i)
X_ar_test=build_X_AR(test_i)
...
Xar_tr_z, Xar_te_z, mu_ar, sd_ar = standardize(X_ar_train, X_ar_test)

# Fit M_AR
clf_ar=LogisticRegression(penalty="l2", C=1.0, solver="liblinear", max_iter=200, fit_intercept=True)
clf_ar.fit(Xar_tr_z, y_train)
proba_ar=clf_ar.predict_proba(Xar_te_z)[:,1]
auc_ar=float(roc_auc_score(y_test, proba_ar))
ll_ar=float(log_loss(y_test, proba_ar, labels=[0,1]))
l2_norm_ar=float(np.linalg.norm(clf_ar.coef_.ravel(), ord=2))

# Build T for indices i: T_full[i] (since T_full aligned to i)
T_train=T_full[train_i]
T_test=T_full[test_i]

# Augmented
X_aug_train=np.column_stack([X_ar_train, T_train])
X_aug_test=np.column_stack([X_ar_test, T_test])
Xaug_tr_z, Xaug_te_z, mu_aug, sd_aug = standardize(X_aug_train, X_aug_test)

clf_aug=LogisticRegression(penalty="l2", C=1.0, solver="liblinear", max_iter=200, fit_intercept=True)
clf_aug.fit(Xaug_tr_z, y_train)
proba_aug=clf_aug.predict_proba(Xaug_te_z)[:,1]
auc_aug=float(roc_auc_score(y_test, proba_aug))
ll_aug=float(log_loss(y_test, proba_aug, labels=[0,1]))
coef_T=float(clf_aug.coef_.ravel()[-1])  # last column is T

delta_auc_obs=auc_aug-auc_ar
delta_ll_obs=ll_aug-ll_ar

# permutation: circular shift T within TRAIN valid rows
rng=np.random.default_rng(12345)
delta_auc_perm=[]
for k in range(120):
    s=int(rng.integers(1, n_train))  # 1..n_train-1 inclusive
    T_shift=np.roll(T_train, s)
    Xp_tr=np.column_stack([X_ar_train, T_shift])
    Xp_te=np.column_stack([X_ar_test, T_test])  # TEST uses real T
    Xp_tr_z, Xp_te_z, _, _ = standardize(Xp_tr, Xp_te)
    clf_p=LogisticRegression(penalty="l2", C=1.0, solver="liblinear", max_iter=200, fit_intercept=True)
    clf_p.fit(Xp_tr_z, y_train)
    proba_p=clf_p.predict_proba(Xp_te_z)[:,1]
    auc_p=float(roc_auc_score(y_test, proba_p))
    delta_auc_perm.append(auc_p-auc_ar)
delta_auc_perm=np.array(delta_auc_perm)
perm_mean=float(delta_auc_perm.mean())
perm_std=float(delta_auc_perm.std(ddof=0))
p_emp=float((np.sum(delta_auc_perm>=delta_auc_obs)+1)/(len(delta_auc_perm)+1))

out={
 "status":"SUCCESS",
 "file_used":os.path.basename(path),
 "hashes_sha1":{"theta":hash_theta,"R":hash_R},
 "shape":{"N_kept":int(N_kept),"train_end":int(train_end),"h":25,"p":20,"n_train":int(n_train),"n_test":int(n_test)},
 "counts":{"pos_rate_train":pos_rate_train,"pos_rate_test":pos_rate_test},
 "metrics":{
   "M_AR":{"auc":auc_ar,"logloss":ll_ar,"l2_coef_norm":l2_norm_ar},
   "M_AR_T":{"auc":auc_aug,"logloss":ll_aug,"coef_T":coef_T},
   "lift":{"delta_auc":delta_auc_obs,"delta_logloss":delta_ll_obs}
 },
 "perm_circshift_T":{
   "n_perm":120,
...
{'status': 'SUCCESS',
 ...
 'shape': {'N_kept': 36592,
  ...
  'n_train': 25594,
  'n_test': 10953},
 'counts': {'pos_rate_train': 0.4944908963038212,
  'pos_rate_test': 0.4972153747831644},
 'metrics': {'M_AR': {'auc': 0.7158942903169812,
   'logloss': 0.6165557908977202,
   'l2_coef_norm': 0.9454803272187284},
  'M_AR_T': {'auc': 0.7169373990076131,
   'logloss': 0.6159495507136934,
   'coef_T': 0.03957934692497069},
  'lift': {'delta_auc': 0.0010431086906319553,
   'delta_logloss': -0.0006062401840267739}},
 'perm_circshift_T': {'n_perm': 120,
  'delta_auc_obs': 0.0010431086906319553,
  'perm_mean': -0.0005933208611979795,
  'perm_std': 0.00145419704635523,
  'p_empirical': 0.09090909090909091},
 'notes': ['TEST uses real T; TRAIN shifts preserve autocorrelation structure',
  'Standardization uses TRAIN only']}

### Assistant (2026-02-22 21:28:09)
