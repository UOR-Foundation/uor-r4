...
- **Conversation ID:** `699bbd6e-4284-8326-a244-34c508ed007c`
- **Date:** 2026-02-22

---

### User (2026-02-22 21:37:38)
...
### Assistant (2026-02-22 21:37:40)

import numpy as np, hashlib, math, json, os, random
...
### Tool (2026-02-22 21:37:50)

(['x_kept', 'u_bp', 'theta_mod', 'R', 'dt', 'train_end', 'params', 'hashes'],
 8)

### Assistant (2026-02-22 21:37:53)
...
N_kept=len(R)
def sha1_arr(a):
    # hash raw bytes with dtype+shape? We'll hash bytes of contiguous array in C order.
...
mask_valid=(i_all>=p) & (i_all+h < N_kept)
i_valid=i_all[mask_valid]
n_rows=len(i_valid)

# build y and T
dR = R[i_valid + h] - R[i_valid]
y = (dR > 0).astype(int)

dtheta = theta[i_valid + h] - theta[i_valid]
# vectorized wrap_to_pi
dtheta_wrapped = (dtheta + math.pi) % (2*math.pi) - math.pi
T = dtheta_wrapped / (2*math.pi)  # in (-0.5,0.5]

# build X_AR
# X_AR rows length p+1 : R[i], R[i-1],...,R[i-p]
lags = np.arange(0,p+1)
X_AR = np.stack([R[i_valid - lag] for lag in lags], axis=1)

# X_FULL
X_FULL = np.concatenate([X_AR, T[:,None]], axis=1)

# split train/test by i index
train_mask = i_valid < train_end
test_mask = ~train_mask
n_train = int(train_mask.sum())
n_test = int(test_mask.sum())

XAR_tr, XAR_te = X_AR[train_mask], X_AR[test_mask]
XFL_tr, XFL_te = X_FULL[train_mask], X_FULL[test_mask]
y_tr, y_te = y[train_mask], y[test_mask]
T_tr, T_te = T[train_mask], T[test_mask]

pos_rate_train=float(y_tr.mean()) if n_train>0 else float('nan')
pos_rate_test=float(y_te.mean()) if n_test>0 else float('nan')

def standardize_train_apply(Xtr, Xte):
    mu = Xtr.mean(axis=0)
    sd = Xtr.std(axis=0, ddof=0)
    sd_safe = np.where(sd==0, 1.0, sd)
    return (Xtr-mu)/sd_safe, (Xte-mu)/sd_safe, mu, sd_safe

XAR_tr_z, XAR_te_z, mu_ar, sd_ar = standardize_train_apply(XAR_tr, XAR_te)
XFL_tr_z, XFL_te_z, mu_fl, sd_fl = standardize_train_apply(XFL_tr, XFL_te)

model_params=dict(penalty="l2", C=1.0, solver="liblinear", max_iter=300, fit_intercept=True)

M_AR = LogisticRegression(**model_params)
M_AR.fit(XAR_tr_z, y_tr)
proba_ar = M_AR.predict_proba(XAR_te_z)[:,1]
auc_ar = float(roc_auc_score(y_te, proba_ar))
ll_ar = float(log_loss(y_te, proba_ar, labels=[0,1]))

M_FULL = LogisticRegression(**model_params)
M_FULL.fit(XFL_tr_z, y_tr)
proba_full = M_FULL.predict_proba(XFL_te_z)[:,1]
auc_full = float(roc_auc_score(y_te, proba_full))
ll_full = float(log_loss(y_te, proba_full, labels=[0,1]))

delta_auc_obs = auc_full - auc_ar
delta_ll_obs = ll_full - ll_ar
coef_T_obs = float(M_FULL.coef_[0,-1])  # last coef in standardized space

# permutation loop
delta_auc_perm=[]
for k in range(n_perm):
    # random shift s in [1, n_train-1]
    s = int(rng.integers(1, n_train))
    T_tr_perm = np.roll(T_tr, s)
    # Build permuted full train matrix (AR lags same)
    XFL_tr_perm = np.concatenate([XAR_tr, T_tr_perm[:,None]], axis=1)
    # Standardize full using permuted TRAIN stats
    mu_p = XFL_tr_perm.mean(axis=0)
    sd_p = XFL_tr_perm.std(axis=0, ddof=0)
    sd_p = np.where(sd_p==0, 1.0, sd_p)
    XFL_tr_perm_z = (XFL_tr_perm - mu_p)/sd_p
    
    # TEST uses real T_te (unmodified) and same AR
    XFL_te_real = np.concatenate([XAR_te, T_te[:,None]], axis=1)
    XFL_te_real_z = (XFL_te_real - mu_p)/sd_p
    
    M_perm = LogisticRegression(**model_params)
    M_perm.fit(XFL_tr_perm_z, y_tr)
    proba_perm = M_perm.predict_proba(XFL_te_real_z)[:,1]
    auc_perm = float(roc_auc_score(y_te, proba_perm))
    delta_auc_perm.append(auc_perm - auc_ar)

delta_auc_perm = np.array(delta_auc_perm)
perm_mean=float(delta_auc_perm.mean())
perm_std=float(delta_auc_perm.std(ddof=0))
z = float((delta_auc_obs - perm_mean)/perm_std) if perm_std>0 else float('inf')
p_emp = float((1 + int((delta_auc_perm >= delta_auc_obs).sum())) / (1 + n_perm))

# slabs: split TEST rows into 3 contiguous slabs equal length
test_indices = np.nonzero(test_mask)[0]  # indices into row arrays
m = len(test_indices)
edges=[0, m//3, 2*(m//3), m]
slabs=[]
for sidx in range(3):
    a,b=edges[sidx], edges[sidx+1]
    idx = test_indices[a:b]
    y_s=y[idx]
    # need corresponding standardized features for observed models
    # AR: use XAR_te_z corresponding positions in test_mask
    # Map idx to positions within XAR_te_z: idx are row indices overall; we need selection in test set order
    # Create mapping row->test position

### Tool (2026-02-22 21:38:34)

---------------------------------------------------------------------------
KeyboardInterrupt                         Traceback (most recent call last)
Cell In[3], line 93
     90 XFL_tr_perm_z = (XFL_tr_perm - mu_p)/sd_p
     92 # TEST uses real T_te (unmodified) and same AR
---> 93 XFL_te_real = np.concatenate([XAR_te, T_te[:,None]], axis=1)
     94 XFL_te_real_z = (XFL_te_real - mu_p)/sd_p
     96 M_perm = LogisticRegression(**model_params)
...
theta=d["theta_mod"]; R=d["R"]; dt=float(d["dt"]); train_end=int(d["train_end"])
N_kept=len(R)
...
h=25; p=40; n_perm=200
two_pi=2*math.pi

i_all=np.arange(N_kept)
valid=(i_all>=p) & (i_all+h < N_kept)
i_valid=i_all[valid]
dR = R[i_valid+h]-R[i_valid]
...
dtheta = theta[i_valid+h]-theta[i_valid]
dtheta_wrapped = (dtheta + math.pi) % (2*math.pi) - math.pi
T = dtheta_wrapped / (2*math.pi)

# AR lags
lags=np.arange(0,p+1)
X_AR=np.empty((len(i_valid), p+1), dtype=np.float64)
for j,lag in enumerate(lags):
    X_AR[:,j]=R[i_valid-lag]

train_mask = i_valid < train_end
test_mask = ~train_mask

XAR_tr=X_AR[train_mask]; XAR_te=X_AR[test_mask]
T_tr=T[train_mask]; T_te=T[test_mask]
...
XAR_tr_z, XAR_te_z, mu_ar, sd_ar = standardize(XAR_tr, XAR_te)

XFL_tr=np.concatenate([XAR_tr, T_tr[:,None]], axis=1)
XFL_te=np.concatenate([XAR_te, T_te[:,None]], axis=1)
XFL_tr_z, XFL_te_z, mu_fl, sd_fl = standardize(XFL_tr, XFL_te)

params=dict(penalty="l2", C=1.0, solver="liblinear", max_iter=300, fit_intercept=True)

M_AR=LogisticRegression(**params)
M_AR.fit(XAR_tr_z, y_tr)
proba_ar=M_AR.predict_proba(XAR_te_z)[:,1]
auc_ar=float(roc_auc_score(y_te, proba_ar))
ll_ar=float(log_loss(y_te, proba_ar, labels=[0,1]))

M_FULL=LogisticRegression(**params)
M_FULL.fit(XFL_tr_z, y_tr)
proba_full=M_FULL.predict_proba(XFL_te_z)[:,1]
auc_full=float(roc_auc_score(y_te, proba_full))
ll_full=float(log_loss(y_te, proba_full, labels=[0,1]))
...
delta_auc_obs=auc_full-auc_ar
delta_ll_obs=ll_full-ll_ar
coef_T_obs=float(M_FULL.coef_[0,-1])

# Precompute test full standardized with observed mu/sd (used for all perms since sd/mu invariant under circular shift)
# Note: mu/sd of T unchanged by circular shift; AR unchanged, so mu_fl/sd_fl same for all perms.
XFL_te_real_z = XFL_te_z

rng=np.random.default_rng(12345)
delta_auc_perm=np.empty(n_perm, dtype=np.float64)

# For speed, compute train standardized AR part once and then assemble permuted last column standardized
XAR_tr_z_part = XAR_tr_z
# For FULL standardization: AR cols standardized with mu_fl/sd_fl; note those differ from mu_ar/sd_ar generally.
XAR_tr_fl_z = (XAR_tr - mu_fl[:-1])/sd_fl[:-1]
# last col z for observed train:
T_tr_z = (T_tr - mu_fl[-1])/sd_fl[-1]

for k in range(n_perm):
    s=int(rng.integers(1, n_train))
    T_perm_z = (np.roll(T_tr, s) - mu_fl[-1]) / sd_fl[-1]
    X_tr_perm_z = np.concatenate([XAR_tr_fl_z, T_perm_z[:,None]], axis=1)
    M_perm=LogisticRegression(**params)
    M_perm.fit(X_tr_perm_z, y_tr)
    proba_perm = M_perm.predict_proba(XFL_te_real_z)[:,1]
    auc_perm=float(roc_auc_score(y_te, proba_perm))
    delta_auc_perm[k]=auc_perm-auc_ar
...
# slabs
# map test_mask selection positions
...
for sidx in range(3):
    a,b=edges[sidx], edges[sidx+1]
    pos_slice=np.arange(a,b)
    # subset in test-set order
    y_s=y_te[pos_slice]
    # if slab has only one class, AUC undefined; handle
    proba_ar_s=proba_ar[pos_slice]
    proba_full_s=proba_full[pos_slice]
    if len(np.unique(y_s))<2:
        delta=float("nan")
    else:
        delta=float(roc_auc_score(y_s, proba_full_s)-roc_auc_score(y_s, proba_ar_s))
    slabs.append({"slab":sidx,"n":int(len(y_s)),"delta_auc":delta})

out={
 "status":"SUCCESS",
 "file_used":path,
 "hashes_sha1":{"theta":sha_theta,"R":sha_R},
 "shape":{"N_kept":int(N_kept),"train_end":int(train_end),"dt":dt,"h":h,"p":p,"n_perm":n_perm},
 "counts":{"n_train":int(n_train),"n_test":int(n_test),"pos_rate_train":float(y_tr.mean()),"pos_rate_test":float(y_te.mean())},
 "observed":{
     "auc_AR":auc_ar,"auc_FULL":auc_full,"delta_auc":delta_auc_obs,
     "logloss_AR":ll_ar,"logloss_FULL":ll_full,"delta_logloss":delta_ll_obs,
     "coef_T":coef_T_obs
 },
 "permutation":{
     "n_perm":n_perm,
     "delta_auc_obs":delta_auc_obs,
     "perm_mean":perm_mean,
     "perm_std":perm_std,
     "z":z,
     "p_empirical":p_emp
...
### Tool (2026-02-22 21:40:28)

---------------------------------------------------------------------------
KeyboardInterrupt                         Traceback (most recent call last)
Cell In[4], line 92
     90 X_tr_perm_z = np.concatenate([XAR_tr_fl_z, T_perm_z[:,None]], axis=1)
     91 M_perm=LogisticRegression(**params)
---> 92 M_perm.fit(X_tr_perm_z, y_tr)
     93 proba_perm = M_perm.predict_proba(XFL_te_real_z)[:,1]
     94 auc_perm=float(roc_auc_score(y_te, proba_perm))
...
File /opt/pyvenv/lib/python3.11/site-packages/sklearn/linear_model/_logistic.py:1221, in LogisticRegression.fit(self, X, y, sample_weight)
   1215     if effective_n_jobs(self.n_jobs) != 1:
   1216         warnings.warn(
   1217             "'n_jobs' > 1 does not have any effect when"
   1218             " 'solver' is set to 'liblinear'. Got 'n_jobs'"
   1219             " = {}.".format(effective_n_jobs(self.n_jobs))
   1220         )
-> 1221     self.coef_, self.intercept_, self.n_iter_ = _fit_liblinear(
   1222         X,
   1223         y,
   1224         self.C,
   1225         self.fit_intercept,
   1226         self.intercept_scaling,
   1227         self.class_weight,
   1228         self.penalty,
   1229         self.dual,
   1230         self.verbose,
   1231         self.max_iter,
   1232         self.tol,
   1233         self.random_state,
   1234         sample_weight=sample_weight,
   1235     )
   1236     return self
   1238 if solver in ["sag", "saga"]:

File /opt/pyvenv/lib/python3.11/site-packages/sklearn/svm/_base.py:1217, in _fit_liblinear(X, y, C, fit_intercept, intercept_scaling, class_weight, penalty, dual, verbose, max_iter, tol, random_state, multi_class, loss, epsilon, sample_weight)
   1214 sample_weight = _check_sample_weight(sample_weight, X, dtype=np.float64)
   1216 solver_type = _get_liblinear_solver_type(multi_class, penalty, loss, dual)
-> 1217 raw_coef_, n_iter_ = liblinear.train_wrap(
   1218     X,
   1219     y_ind,
   1220     sp.issparse(X),
   1221     solver_type,
   ...
   1225     class_weight_,
   1226     max_iter,
   1227     rnd.randint(np.iinfo("i").max),
   1228     epsilon,
   1229     sample_weight,
   1230 )
   1231 # Regarding rnd.randint(..) in the above signature:
   1232 # seed for srand in range [0..INT_MAX); due to limitations in Numpy
   1233 # on 32-bit platforms, we can't get to the UINT_MAX limit that
   1234 # srand supports
   1235 n_iter_max = max(n_iter_)

KeyboardInterrupt: 

WARNING: Automatic interrupt triggered after 60.0 seconds.

### Assistant (2026-02-22 21:41:30)
...
base_params=dict(penalty="l2", C=1.0, solver="liblinear", max_iter=300, fit_intercept=True)
M_AR=LogisticRegression(**base_params)
M_AR.fit(XAR_tr_z, y_tr)
proba_ar=M_AR.predict_proba(XAR_te_z)[:,1]
auc_ar=float(roc_auc_score(y_te, proba_ar))
ll_ar=float(log_loss(y_te, proba_ar, labels=[0,1]))

M_FULL=LogisticRegression(**base_params)
M_FULL.fit(XFL_tr_z, y_tr)
proba_full=M_FULL.predict_proba(XFL_te_z)[:,1]
auc_full=float(roc_auc_score(y_te, proba_full))
ll_full=float(log_loss(y_te, proba_full, labels=[0,1]))
...
# permutation using warm_start reuse
perm_params=dict(**base_params)
perm_params["warm_start"]=True
M_perm=LogisticRegression(**perm_params)

# precompute AR cols standardized for FULL stats (same as XFL_tr_z without last col)
XAR_tr_fl_z = (XAR_tr - mu_fl[:-1])/sd_fl[:-1]
XAR_te_fl_z = (XAR_te - mu_fl[:-1])/sd_fl[:-1]
T_te_z = (T_te - mu_fl[-1])/sd_fl[-1]
X_te_real_z = np.concatenate([XAR_te_fl_z, T_te_z[:,None]], axis=1)

X_tr_perm_z = np.empty((n_train, p+2), dtype=np.float64)
X_tr_perm_z[:,:-1]=XAR_tr_fl_z
...
for k in range(n_perm):
    s=int(rng.integers(1,n_train))
    T_perm_z = (np.roll(T_tr, s) - mu_fl[-1]) / sd_fl[-1]
    X_tr_perm_z[:,-1]=T_perm_z
    M_perm.fit(X_tr_perm_z, y_tr)
    proba_perm=M_perm.predict_proba(X_te_real_z)[:,1]
    delta_auc_perm[k]=float(roc_auc_score(y_te, proba_perm))-auc_ar
...
for sidx in range(3):
    ...
    if len(np.unique(y_s))<2:
        ...
        delta=float(roc_auc_score(y_s, proba_full[a:b]) - roc_auc_score(y_s, proba_ar[a:b]))
    slabs.append({"slab":sidx,"n":int(b-a),"delta_auc":delta})
...
### Tool (2026-02-22 21:41:57)

---------------------------------------------------------------------------
KeyboardInterrupt                         Traceback (most recent call last)
Cell In[5], line 89
     87 T_perm_z = (np.roll(T_tr, s) - mu_fl[-1]) / sd_fl[-1]
     88 X_tr_perm_z[:,-1]=T_perm_z
---> 89 M_perm.fit(X_tr_perm_z, y_tr)
     90 proba_perm=M_perm.predict_proba(X_te_real_z)[:,1]
     91 delta_auc_perm[k]=float(roc_auc_score(y_te, proba_perm))-auc_ar
...
### Tool (2026-02-22 21:43:27)

'{"status": "SUCCESS", "file_used": "/mnt/data/A4_CANONICAL_CACHE_L131072_x1e5_9e6_W2000_bp002_001_D001_seed12345.npz", "hashes_sha1": {"theta": "8b33c57df436d37179b13b1899bf86a723f0e98a", "R": "5753ee'

### Assistant (2026-02-22 21:44:23)
