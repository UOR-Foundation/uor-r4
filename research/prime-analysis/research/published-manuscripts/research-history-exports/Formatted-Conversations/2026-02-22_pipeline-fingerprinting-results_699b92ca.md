# Pipeline fingerprinting results

- **Conversation ID:** `699b92ca-9afc-832c-9ac0-200ce629ec78`
- **Date:** 2026-02-22

---

### User (2026-02-22 18:35:41)
...
prm = np.load('/mnt/data/primes.npz')
...
# Step 1: uniform t grid on [min(log(x_grid)), max(log(x_grid))]
t_raw = np.log(x_grid)
t_min, t_max = float(t_raw.min()), float(t_raw.max())
t_uni = np.linspace(t_min, t_max, L, dtype=np.float64)
dt = float(t_uni[1] - t_uni[0])

# interpolate psi_minus_x onto uniform t
# psi_minus_x provided on x_grid; ensure t_raw is increasing (should be)
u_uni = np.interp(t_uni, t_raw, psi_minus_x).astype(np.float64)

# Step 2: x=exp(t) and kept mask
x_uni = np.exp(t_uni)
kept = (x_uni >= x_min) & (x_uni <= x_max)
idx = np.nonzero(kept)[0]
x_kept = x_uni[kept].astype(np.float64)
u_raw = u_uni[kept].astype(np.float64)
...
# Step 4: causal detrend using moving average W (past-only, includes current)
def causal_ma(y, W):
    y = np.asarray(y, dtype=np.float64)
    n = y.size
    cs = np.cumsum(y, dtype=np.float64)
...
# Step 5: standardize using TRAIN only
u_train = u[:train_end]
mean_train = float(u_train.mean())
std_train = float(u_train.std(ddof=0))
u_z = (u - mean_train) / (std_train if std_train != 0 else 1.0)

# Step 6: FFT bandpass on u_z (length N_kept)
U = np.fft.rfft(u_z)
freqs = np.fft.rfftfreq(N_kept, d=1.0)  # cycles/sample
mask = (freqs >= f_lo) & (freqs <= f_hi)
U_bp = U * mask
u_bp = np.fft.irfft(U_bp, n=N_kept).astype(np.float64)

# Step 7: Hilbert analytic => theta
analytic = signal.hilbert(u_bp)
theta = np.mod(np.angle(analytic), 2*np.pi).astype(np.float64)

# Step 8: R_i from primes in [x_i, x_i*exp(Delta))
expD = math.exp(Delta)
right = x_kept * expD
# count primes in [x, right)
# primes are int; x_kept float; use searchsorted on primes
left_idx = np.searchsorted(primes, x_kept, side='left')
right_idx = np.searchsorted(primes, right, side='left')
...
def sha1_f64(arr):
    ...
    b = a.astype('<f8', copy=False).tobytes(order='C')
    return hashlib.sha1(b).hexdigest()

hashes = {
    "x_kept": sha1_f64(x_kept),
    "u_bp": sha1_f64(u_bp),
    "theta": sha1_f64(theta),
    "R": sha1_f64(R)
}

# Step 10: theta circular stats MRL
def mean_resultant_length(th):
    z = np.exp(1j*th)
    m = z.mean()
    return float(np.abs(m))

MRL_train = mean_resultant_length(theta[:train_end])
MRL_test = mean_resultant_length(theta[train_end:])

# Step 11: correlations on TRAIN
def corr(a,b):
    ...
    if np.isnan(c) or not np.isfinite(c):
        return None
    return float(c)
...
corr_sin = corr(u_bp[:train_end], sin_th[:train_end])
corr_cos = corr(u_bp[:train_end], cos_th[:train_end])

# Stats as requested
stats = {
    "u_raw_train_mean": float(u_raw[:train_end].mean()),
    "u_raw_train_std": float(u_raw[:train_end].std(ddof=0)),
    "u_z_train_mean": float(u_z[:train_end].mean()),
    "u_z_train_std": float(u_z[:train_end].std(ddof=0)),
    "u_bp_train_mean": float(u_bp[:train_end].mean()),
    "u_bp_train_std": float(u_bp[:train_end].std(ddof=0)),
    "R_train_mean": float(R[:train_end].mean()),
    "R_train_std": float(R[:train_end].std(ddof=0))
}
...
out = {
    ...
    "shape": {
        ...
        "N_kept": N_kept,
        "train_end": train_end,
        "x_min_kept": x_min_kept,
        "x_max_kept": x_max_kept
    },
    "stats": stats,
    "theta_circular": {
        "MRL_train": MRL_train,
    ...
    "sanity_corr_train": {
        "corr_u_bp_sin_theta": corr_sin,
        "corr_u_bp_cos_theta": corr_cos
    },
    "hashes_sha1": hashes
}
