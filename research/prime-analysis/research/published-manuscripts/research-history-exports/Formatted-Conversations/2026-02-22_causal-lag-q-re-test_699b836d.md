# Causal lag-q re-test

- **Conversation ID:** `699b836d-e4dc-8329-954b-0b9c886a6dda`
- **Date:** 2026-02-22

---

### User (2026-02-22 17:30:05)
...
### Assistant (2026-02-22 17:30:08)

import numpy as np, math, json, os, sys, statistics
from numpy.fft import rfft, irfft

### Assistant (2026-02-22 17:30:55)

import importlib, pkgutil, sklearn

### Assistant (2026-02-22 17:30:59)

from sklearn.linear_model import LogisticRegression
from sklearn.metrics import roc_auc_score

### Assistant (2026-02-22 17:31:13)

import scipy, scipy.signal

### Assistant (2026-02-22 17:31:18)

import numpy as np, scipy.signal as signal, math, json, os, pandas as pd
psi_path="/mnt/data/chebyshev_psi.npz"
pr_path="/mnt/data/primes.npz"
psi=np.load(psi_path)
pr=np.load(pr_path)
psi.files, pr.files[:5], pr['primes'][:5], pr['primes'][-1], pr['N']

### Tool (2026-02-22 17:31:22)
...
def fft_bandpass_real(x, f_lo, f_hi):
    # x real 1d, return bandpassed via rfft mask
    n=len(x)
    X=rfft(x)
...
def build_fourier_features(theta, K):
    # theta array shape (m,)
    m=len(theta)
    cols=[np.ones(m)]
...
def compute_R(x_vals, primes, Delta):
    # x_vals float array positive
    hi = x_vals * math.exp(Delta)
...
### Assistant (2026-02-22 17:31:39)

x_grid=psi['x_grid'].astype(float)
t_raw=np.log(x_grid)
u_raw_grid=psi['psi_minus_x'].astype(float)
...
def run_for_L(L):
    ...
    dt=float(t[1]-t[0])
    u_interp=np.interp(t, t_raw, u_raw_grid)
    x=np.exp(t)
    kept=(x>=params["x_range"][0]) & (x<=params["x_range"][1])
    xk=x[kept]
    uk_raw=u_interp[kept]
    N_kept=len(xk)
    train_end=int(math.floor(params["train_frac"]*N_kept))
    # detrend causal
    ma=causal_moving_average(uk_raw, params["detrend_W"])
    u=uk_raw - ma
    ...
    if sd==0 or not np.isfinite(sd):
        sd=1.0
    u_z=(u-mu)/sd
    # bandpass FFT mask on full kept
    f_lo,f_hi=params["bandpass_cycles_per_sample"]
    u_bp=fft_bandpass_real(u_z, f_lo, f_hi)
    theta=hilbert_phase(u_bp)
    # compute R
    R=compute_R(xk, primes, params["Delta"])
    h=params["horizon"]
    valid_y = np.arange(0, N_kept-h, dtype=int)
    y = (R[valid_y+h]-R[valid_y] > 0).astype(int)
    # baseline mean-reversion on test_valid (q=0 set)
    test_valid0 = valid_y[valid_y>=train_end]
    auc_mr=None
    if len(test_valid0)>0:
        y0=y[test_valid0]
        if len(np.unique(y0))==2:
            scores=-R[test_valid0]
            auc_mr=float(roc_auc_score(y0, scores))
    by_q={}
    K=params["fourier_logistic_K"]
    rng=np.random.RandomState(params["seed"])
    for q in params["q_list"]:
        # build valid i indices with shift and horizon
        valid_i = valid_y[valid_y-q>=0]  # also ensures i+h<N_kept already
        train_i = valid_i[(valid_i < train_end) & (valid_i+h < train_end)]
        test_i = valid_i[(valid_i >= train_end)]
        # counts
        n_train=len(train_i); n_test=len(test_i)
        entry={"n_train_valid":int(n_train),
               "n_test_valid":int(n_test),
               "train_pos_rate":None,
               "test_pos_rate":None,
               "auc_phase_model":None,
               "fit_status":"ok"}
        if n_train==0:
            entry["fit_status"]="no_train_samples"
            by_q[str(q)]=entry
            continue
        ytr=y[train_i]
        if len(np.unique(ytr))<2:
            entry["fit_status"]="train_single_class"
            entry["train_pos_rate"]=float(np.mean(ytr)) if len(ytr)>0 else None
            by_q[str(q)]=entry
            continue
        entry["train_pos_rate"]=float(np.mean(ytr))
        # features
        th_tr=theta[train_i - q]
        Xtr=build_fourier_features(th_tr, K)
        model=LogisticRegression(solver="lbfgs", max_iter=2000, random_state=params["seed"])
        try:
            model.fit(Xtr, ytr)
        except Exception as e:
            entry["fit_status"]="fit_error:"+str(e.__class__.__name__)
            by_q[str(q)]=entry
            continue
        if n_test==0:
            entry["fit_status"]="no_test_samples"
            by_q[str(q)]=entry
            continue
        yte=y[test_i]
        entry["test_pos_rate"]=float(np.mean(yte))
        if len(np.unique(yte))<2:
            entry["fit_status"]="test_single_class"
            by_q[str(q)]=entry
            continue
        th_te=theta[test_i - q]
        Xte=build_fourier_features(th_te, K)
        probs=model.predict_proba(Xte)[:,1]
        entry["auc_phase_model"]=float(roc_auc_score(yte, probs))
        by_q[str(q)]=entry
    # determine q_star by max auc_phase_model among ok entries
    best_q=None; best_auc=None
    for q in params["q_list"]:
        e=by_q[str(q)]
        auc=e["auc_phase_model"]
        if auc is None: 
            continue
        if (best_auc is None) or (auc>best_auc):
            best_auc=auc; best_q=q
    auc_q0=by_q["0"]["auc_phase_model"]
    out={
        "dt":dt,
        "N_kept":int(N_kept),
        "train_end":int(train_end),
        "h":int(h),
        "auc_mr_baseline":auc_mr,
        "by_q":by_q,
        "q_star":best_q,
        "auc_at_q_star":best_auc,
        "auc_at_q0":auc_q0
    }
    return out
...
results.keys(), results["65536"]["q_star"], results["131072"]["q_star"]

### Tool (2026-02-22 17:32:12)

(dict_keys(['65536', '131072']), 0, 0)

### Assistant (2026-02-22 17:32:30)

for L in ["65536","131072"]:
    print("L",L,"auc_q0",results[L]["auc_at_q0"],"q_star",results[L]["q_star"],"auc_star",results[L]["auc_at_q_star"],"auc_mr",results[L]["auc_mr_baseline"])
    # show q aucs
    aucs={int(q):results[L]["by_q"][q]["auc_phase_model"] for q in results[L]["by_q"]}
    # sort
    for q,auc in sorted(aucs.items()):
        print(q,auc)
    print()

### Tool (2026-02-22 17:32:35)

L 65536 auc_q0 0.7043987293129068 q_star 0 auc_star 0.7043987293129068 auc_mr 0.83624518685392
0 0.7043987293129068
5 0.6878476773212876
10 0.6720313032861893
15 0.6556329992986539
20 0.639349153013006
25 0.622426079056527
30 0.6056826114096285
35 0.590482876521764
40 0.5765355650794648
50 0.5581851443544393
60 0.5453819967263168

L 131072 auc_q0 0.7726604446753601 q_star 0 auc_star 0.7726604446753601 auc_mr 0.8785036054005204
0 0.7726604446753601
5 0.762499102093605
10 0.7516486068464666
15 0.7402474858198397
20 0.7272473223797487
25 0.7138888414700414
30 0.701329678945857
35 0.6885199646212204
40 0.6752136087622818
50 0.6481959645002038
60 0.61892977670568

### Assistant (2026-02-22 17:32:42)
...
out={
 "meta":{
   ...
   "detrend_W":params["detrend_W"],
   "bandpass_cycles_per_sample":params["bandpass_cycles_per_sample"],
   "horizon":params["horizon"],
   "q_list":params["q_list"],
   "fourier_logistic_K":params["fourier_logistic_K"],
 ...
 "by_L": {L:results[L] for L in results}
}
# ensure schema: by_L values are objects with keys by_q etc already
json_str=json.dumps(out, indent=2, sort_keys=False)
len(json_str.splitlines()), json_str[:300]

### Tool (2026-02-22 17:32:54)

(244,
 '{\n  "meta": {\n    "experiment_id": "A4.01c_causal_lag_sweep_boundary_fix",\n    "goal": "Re-test lag-q improvement with causal TEST evaluation allowing pre-boundary history in theta_{i-q}.",\n    "files_used": [\n      "chebyshev_psi.npz",\n      "primes.npz"\n    ],\n    "seed": 12345,\n    "x_range": [\n ')

### Assistant (2026-02-22 17:32:58)
