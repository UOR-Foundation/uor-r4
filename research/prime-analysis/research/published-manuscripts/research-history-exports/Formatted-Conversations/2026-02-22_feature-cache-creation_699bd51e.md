...
- **Conversation ID:** `699bd51e-f4cc-832c-b0cc-a7b417e0f014`
- **Date:** 2026-02-22

---

### User (2026-02-22 23:18:42)
...
### Assistant (2026-02-22 23:18:44)

import numpy as np, os, hashlib, json, math, pathlib, sys, textwrap
from numpy.lib import format as npformat

### Assistant (2026-02-22 23:19:00)

import numpy as np, hashlib, os, json, math, pathlib, sys
def sha1_bytes(b: bytes)->str:
    return hashlib.sha1(b).hexdigest()

canonical_path = "/mnt/data/A4_CANONICAL_CACHE_L131072_x1e5_9e6_W2000_bp002_001_D001_seed12345.npz"
npz = np.load(canonical_path, allow_pickle=True)
keys = list(npz.keys())
keys[:20], len(keys)

### Tool (2026-02-22 23:19:03)

(['x_kept', 'u_bp', 'theta_mod', 'R', 'dt', 'train_end', 'params', 'hashes'],
 8)

### Assistant (2026-02-22 23:19:07)
...
N = int(theta.shape[0])
sha_theta = sha1_bytes(theta.tobytes(order="C"))
sha_R = sha1_bytes(R.tobytes(order="C"))
mismatch=[]
if sha_theta!=expected["theta_sha1"]: mismatch.append("theta_sha1")
if sha_R!=expected["R_sha1"]: mismatch.append("R_sha1")
if abs(dt-expected["dt"])>0: 
    # float exact? use tolerance
    if not np.isclose(dt, expected["dt"], rtol=0, atol=0): mismatch.append("dt")
if N!=expected["N_kept"]: mismatch.append("N_kept")
if train_end!=expected["train_end"]: mismatch.append("train_end")
sha_theta, sha_R, dt, N, train_end, mismatch[:]

### Tool (2026-02-22 23:19:13)
...
S[:,0] = np.sin(theta64).astype(np.float32)
S[:,1] = np.cos(theta64).astype(np.float32)

omega = wrap_to_pi(theta64[1:] - theta64[:-1]).astype(np.float32)
...
for lag in range(40):
    X40[lag:, lag] = R32[lag:]
# Wait: want [R_i, R_{i-1},...,R_{i-39}] columns 0..39
# Above fills column lag with R_{i-lag}? Actually X40[i,lag]=R_{i}? if we fill like: X40[lag:,lag]=R32[:-lag]? Let's correct.

### Assistant (2026-02-22 23:19:32)
...
# masks
h=25
train_lag39 = (np.arange(N) >= 39) & (np.arange(N) < train_end) & (np.arange(N)+h < train_end)
test_lag39  = (np.arange(N) >= 39) & (np.arange(N) >= train_end) & (np.arange(N)+h < N)
train_lag79 = (np.arange(N) >= 79) & (np.arange(N) < train_end) & (np.arange(N)+h < train_end)
test_lag79  = (np.arange(N) >= 79) & (np.arange(N) >= train_end) & (np.arange(N)+h < N)

# standardization stats functions
def masked_mean_std(X, mask_rows):
    # X: 2D float32 with NaNs in initial rows; mask_rows should exclude those but still safe.
    Xm = X[mask_rows]
    mu = np.nanmean(Xm, axis=0)
    sd = np.nanstd(Xm, axis=0, ddof=0)
    sd = np.maximum(sd, 1e-8)
    return mu.astype(np.float64), sd.astype(np.float64)

mu_X40, sd_X40 = masked_mean_std(X40, train_lag39)
mu_X80, sd_X80 = masked_mean_std(X80, train_lag79)

# S stats on train_lag79 rows
Sm = S[train_lag79]
mu_S = Sm.mean(axis=0).astype(np.float64)
sd_S = Sm.std(axis=0, ddof=0).astype(np.float64)
sd_S = np.maximum(sd_S, 1e-8)

# omega stats: indices i where train_lag79 true AND i < N-1
omega_train_mask = train_lag79[:-1]  # since omega aligned to i, length N-1 corresponds to i=0..N-2
om = omega[omega_train_mask]
mu_omega = float(om.mean())
sd_omega = float(om.std(ddof=0))
sd_omega = max(sd_omega, 1e-8)

# integrity: NaNs in S/omega
assert not np.isnan(S).any()
assert not np.isnan(omega).any()
# X40/X80 NaNs only prefix
nan_prefix_40 = int(np.isnan(X40).all(axis=1).sum())  # but this counts rows fully NaN; should be 39 and 79
nan_prefix_40, nan_prefix_40==39

### Tool (2026-02-22 23:19:48)

(0, False)

### Assistant (2026-02-22 23:19:58)

X40 = np.full((N,40), np.nan, dtype=np.float32)
for k in range(40):
    start = 39  # enforce full-NaN for i<39
    X40[start:,k] = R32[start-k:N-k]
X80 = np.full((N,80), np.nan, dtype=np.float32)
for k in range(80):
    start = 79
    X80[start:,k] = R32[start-k:N-k]

# verify prefix rows all NaN
assert np.isnan(X40[:39]).all()
assert np.isnan(X80[:79]).all()
# verify one row example i=39 col0=R39, col39=R0
i=39
float(X40[i,0]), float(R32[i]), float(X40[i,39]), float(R32[0])

### Tool (2026-02-22 23:20:03)
...
### Assistant (2026-02-22 23:20:07)

mu_X40, sd_X40 = masked_mean_std(X40, train_lag39)
mu_X80, sd_X80 = masked_mean_std(X80, train_lag79)
# masks should already exclude prefix by i>= lag, but fine.

# compute sha1s
def sha1_arr(a):
    return sha1_bytes(a.tobytes(order="C"))
def sha1_mask(mask):
    return sha1_bytes(mask.astype(np.uint8, copy=False).tobytes(order="C"))

sha_S = sha1_arr(S)
sha_omega = sha1_arr(omega)
sha_X40 = sha1_arr(X40)
sha_X80 = sha1_arr(X80)
sha_m_tr39 = sha1_mask(train_lag39)
sha_m_te39 = sha1_mask(test_lag39)
sha_m_tr79 = sha1_mask(train_lag79)
sha_m_te79 = sha1_mask(test_lag79)

sha_muX40 = sha1_arr(mu_X40.astype(np.float64))
sha_sdX40 = sha1_arr(sd_X40.astype(np.float64))
sha_muX80 = sha1_arr(mu_X80.astype(np.float64))
sha_sdX80 = sha1_arr(sd_X80.astype(np.float64))
sha_muS = sha1_arr(mu_S.astype(np.float64))
sha_sdS = sha1_arr(sd_S.astype(np.float64))
sha_muO = sha1_arr(np.array([mu_omega], dtype=np.float64))
sha_sdO = sha1_arr(np.array([sd_omega], dtype=np.float64))

counts = (int(train_lag39.sum()), int(test_lag39.sum()), int(train_lag79.sum()), int(test_lag79.sum()))
counts

### Tool (2026-02-22 23:20:15)

(25550, 10953, 25510, 10953)

### Assistant (2026-02-22 23:20:21)

out_path = "/mnt/data/A4_FEATURECACHE_h25_AR80_AR40_theta_omega_masks_v1.npz"
# prepare dict
save_dict = {
    "theta": theta64,  # float64
    "R": R64,          # float64
    "S": S.astype(np.float32, copy=False),
    "omega": omega.astype(np.float32, copy=False),
    "X40": X40.astype(np.float32, copy=False),
    "X80": X80.astype(np.float32, copy=False),
    "mask_train_lag39": train_lag39.astype(np.uint8),
    "mask_test_lag39": test_lag39.astype(np.uint8),
    "mask_train_lag79": train_lag79.astype(np.uint8),
    "mask_test_lag79": test_lag79.astype(np.uint8),
    "mu_X40_train": mu_X40.astype(np.float64),
    "sd_X40_train": sd_X40.astype(np.float64),
    "mu_X80_train": mu_X80.astype(np.float64),
    "sd_X80_train": sd_X80.astype(np.float64),
    "mu_S_train": mu_S.astype(np.float64),
    "sd_S_train": sd_S.astype(np.float64),
    ...
    "source_canonical": np.array([os.path.basename(canonical_path)], dtype=object),
    "theta_sha1": np.array([sha_theta], dtype=object),
    "R_sha1": np.array([sha_R], dtype=object),
}
# write
...
### Tool (2026-02-22 23:20:30)

('07af2ac1f59e65fb9921fc8d07ce706e371aa013', 1423351)

### Assistant (2026-02-22 23:20:37)
...
meta = {
 ...
 "saved_arrays":{
   "theta":{"shape":shape_str(theta64),"dtype":dtype_str(theta64),"sha1":sha_theta},
   "R":{"shape":shape_str(R64),"dtype":dtype_str(R64),"sha1":sha_R},
   "S":{"shape":

### Tool (2026-02-22 23:20:48)
