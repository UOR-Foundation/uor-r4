# Hilbert Phase Incremental Test

- **Conversation ID:** `699b849c-9140-8325-8d64-ff8038bde250`
- **Date:** 2026-02-22

---

### User (2026-02-22 17:35:12)
...
### Assistant (2026-02-22 17:35:30)

import numpy as np, math, json, os, sys, statistics
from numpy.fft import rfft, irfft, rfftfreq

### Assistant (2026-02-22 17:35:46)
...
cheb.files, pr.files[:5]

### Tool (2026-02-22 17:35:48)

(['N', 'step', 'x_grid', 'psi_grid', 'psi_minus_x'],
 ['N', 'primes', 'is_prime'])

### Assistant (2026-02-22 17:35:51)
...
### Tool (2026-02-22 17:36:18)

(0.00012297217348543395, 36592, 25614, 100009.83380914277, 8999671.172393441)

### Assistant (2026-02-22 17:36:22)

W=params["detrend_W"]
u = u_raw - ma_causal(u_raw, W)
mu = float(u[:train_end].mean())
sigma = float(u[:train_end].std(ddof=0))
u_z = (u - mu) / (sigma if sigma>0 else 1.0)
...
theta.shape, float(theta.min()), float(theta.max())

### Tool (2026-02-22 17:36:25)

((36592,), 0.0002090886821039302, 6.283009641243227)

### Assistant (2026-02-22 17:36:30)
...
lo_idx = np.searchsorted(primes, xL, side='left')
hi_idx = np.searchsorted(primes, xU, side='left')
P = (hi_idx - lo_idx).astype(float)

E = xL*(mult-1.0)/np.log(xL)
R = (P - E)/np.sqrt(E)
R[:5], float(R.mean()), float(R.std())

### Tool (2026-02-22 17:36:36)

(array([-0.6745253 , -0.56851728, -0.56953462, -0.57055196, -0.67857064]),
 -0.025847250185465763,
 0.4877311808737089)

### Assistant (2026-02-22 17:36:41)

h=params["h"]
R_lags=params["R_lags"]
valid = np.arange(R_lags, N_kept-h, dtype=int)  # ensures i+h < N_kept
y = (R[valid + h] - R[valid] > 0).astype(int)

# define train/test rows
train_mask = (valid < train_end) & (valid + h < train_end)
test_mask = (valid >= train_end) & (valid + h < N_kept)

train_idx = np.nonzero(train_mask)[0]
test_idx = np.nonzero(test_mask)[0]
n_train = int(len(train_idx))
n_test = int(len(test_idx))
train_pos = float(y[train_idx].mean()) if n_train>0 else float('nan')
test_pos = float(y[test_idx].mean()) if n_test>0 else float('nan')
n_train, n_test, train_pos, test_pos

### Tool (2026-02-22 17:36:45)

(25512, 10878, 0.4942772028849169, 0.49963228534657106)

### Assistant (2026-02-22 17:36:49)

K=params["K"]

def phase_feats(theta_vals, K):
    feats=[]
...
# base arrays for all valid rows
i_vals = valid
R_i = R[i_vals]
AR_cols = [R_i] + [R[i_vals - lag] for lag in range(1, R_lags+1)]
X_AR = np.column_stack(AR_cols)
X_MR = R_i.reshape(-1,1)
X_PHASE = phase_feats(theta[i_vals], K)
X_ARP = np.column_stack([X_AR, X_PHASE])

X_MR.shape, X_AR.shape, X_PHASE.shape, X_ARP.shape

### Tool (2026-02-22 17:36:56)

((36490, 1), (36490, 3), (36490, 6), (36490, 9))

### Assistant (2026-02-22 17:37:01)
...
def fit_eval(X, y, train_idx, test_idx):
    ...
    try:
        ...
        if len(np.unique(ytr))<2 or len(np.unique(yte))<2:
            out["error"]=f"single_class: train_classes={np.unique(ytr).tolist()} test_classes={np.unique(yte).tolist()}"
            return out
        ...
        out["auc"]=float(roc_auc_score(yte, proba))
        out["logloss"]=float(log_loss(yte, proba, labels=[0,1]))
        return out
...
res = {}
res["MR"]=fit_eval(X_MR, y, train_idx, test_idx)
res["AR"]=fit_eval(X_AR, y, train_idx, test_idx)
res["PHASE"]=fit_eval(X_PHASE, y, train_idx, test_idx)
res["AR_PLUS_PHASE"]=fit_eval(X_ARP, y, train_idx, test_idx)
res

### Tool (2026-02-22 17:37:12)

{'MR': {'auc': 0.8785036054005204, 'logloss': 0.4399906500551298},
 'AR': {'auc': 0.8782778653946622, 'logloss': 0.4402525673106962},
 'PHASE': {'auc': 0.7726485458310861, 'logloss': 0.5906354854660015},
 'AR_PLUS_PHASE': {'auc': 0.8816867490650364, 'logloss': 0.43466799182571203}}

### Assistant (2026-02-22 17:37:17)

delta_auc = None
delta_ll = None
if res["AR"].get("auc") is not None and res["AR_PLUS_PHASE"].get("auc") is not None:
    delta_auc = float(res["AR_PLUS_PHASE"]["auc"] - res["AR"]["auc"])
if res["AR"].get("logloss") is not None and res["AR_PLUS_PHASE"].get("logloss") is not None:
    delta_ll = float(res["AR_PLUS_PHASE"]["logloss"] - res["AR"]["logloss"])
delta_auc, delta_ll

### Tool (2026-02-22 17:37:21)

(0.0034088836703741743, -0.0055845754849841445)

### Assistant (2026-02-22 17:37:23)
...
out = {
  "meta": {
    ...
    "params": params,
    "files_used": {
      "chebyshev_psi": os.path.basename(cheb_path),
      "primes": os.path.basename(primes_path)
    }
  },
  "data_shape": {
    "dt": dt,
  ...
  "models": {
    ...
    "AR": res["AR"],
    "PHASE": res["PHASE"],
    "AR_PLUS_PHASE": res["AR_PLUS_PHASE"]
  },
  "incremental": {
    "delta_auc": delta_auc,
