...
- **Conversation ID:** `699efd67-4da0-832f-bc68-be3b2a89afbf`
- **Date:** 2026-02-25

---

### User (2026-02-25 08:47:22)
...
# paths
feat_path = Path("/mnt/data/A4_FEATURECACHE_h25_AR80_AR40_theta_omega_masks_v1.npz")
psi_path = Path("/mnt/data/chebyshev_psi.npz")
canon_path = Path("/mnt/data/A4_CANONICAL_CACHE_L131072_x1e5_9e6_W2000_bp002_001_D001_seed12345.npz")
...
# load needed from featurecache
X80 = feat["X80"]  # likely standardized? We'll apply mu/sd
mu_X = feat["mu_X80_train"]
sd_X = feat["sd_X80_train"]
R = feat["R"]

# canon values
x_kept_c = canon["x_kept"]
dt_c = float(canon["dt"])
train_end_c = int(canon["train_end"])
N_kept_c = int(canon["N_kept"]) if "N_kept" in canon.files else len(x_kept_c)

dt_c, train_end_c, N_kept_c, len(x_kept_c)

### Tool (2026-02-25 08:48:10)

(0.00012297217348543395, 25614, 36592, 36592)

### Assistant (2026-02-25 08:48:13)
...
x_kept = x_uniform[keep_mask]
u_raw = u_interp[keep_mask]
N_kept = len(x_kept)

sha1_x = sha1_arr(x_kept.astype(np.float64))
sha1_x, sha1_arr(x_kept_c.astype(np.float64)), N_kept

### Tool (2026-02-25 08:48:21)

('c455fedeff29bb2d0b2c81864970c09638720baf',
 'c455fedeff29bb2d0b2c81864970c09638720baf',
 36592)

### Assistant (2026-02-25 08:48:23)

W=2000
u = u_raw
cumsum = np.cumsum(np.insert(u,0,0.0))
# for each i, window start = max(0, i-W+1), end=i inclusive => length = i-start+1
idx = np.arange(N_kept)
start = np.maximum(0, idx - W + 1)
length = idx - start + 1
ma = (cumsum[idx+1] - cumsum[start]) / length
u_detr = u - ma

# train-only zscore ddof=0 using indices < train_end
train_end = 25614
u_train = u_detr[:train_end]
mu_u = u_train.mean()
sd_u = u_train.std(ddof=0)
u_z = (u_detr - mu_u) / (sd_u if sd_u>0 else 1.0)

# bandpass via rFFT mask
lo, hi = 0.003, 0.009
U = fft.rfft(u_z)
freqs = fft.rfftfreq(N_kept, d=1.0)  # cycles/sample
mask = (freqs >= lo) & (freqs <= hi)
U_bp = U * mask
u_bp = fft.irfft(U_bp, n=N_kept)

analytic = hilbert(u_bp)
amp = np.abs(analytic)
logA = np.log(amp + 1e-12)
theta_mod = np.mod(np.angle(analytic), 2*np.pi)
theta_u = np.unwrap(np.angle(analytic))
# sanity
float(theta_mod.min()), float(theta_mod.max()), float(logA.mean())

### Tool (2026-02-25 08:48:33)

(3.676980435929987e-05, 6.28269245117255, -0.9272992792915535)

### Assistant (2026-02-25 08:48:34)

h=50
assert len(R)==N_kept
y = (R[h:] - R[:-h] > 0).astype(np.int8)  # y_i defined for i where i+h valid; we'll index with i
# We'll use full length N_kept, but y valid only up to N_kept-h-1; for convenience create y_full with nan
y_full = np.full(N_kept, -1, dtype=np.int8)
y_full[:N_kept-h] = y

# build valid masks per definitions
ell=25
i = np.arange(N_kept)
...
### Tool (2026-02-25 08:48:45)

(25485, 10928, array([79, 80, 81]), array([25614, 25615, 25616]), 36541)

### Assistant (2026-02-25 08:48:47)

dtheta_h = theta_u[train_idx + h] - theta_u[train_idx]
omega = np.angle(np.mean(np.exp(1j*dtheta_h))) / h  # rad per sample
# delta and eps
delta = (theta_u[train_idx] - theta_u[train_idx - ell])
# wrap to (-pi,pi]
def wrappi(a):
    return (a + np.pi) % (2*np.pi) - np.pi
delta_w = wrappi(delta)
eps = theta_u[train_idx + h] - theta_u[train_idx] - h*omega
eps_w = wrappi(eps)

# objective for gamma
z = np.exp(1j*eps_w)
d = delta_w.astype(np.float64)
def obj(g):
    return -abs(np.mean(z * np.exp(-1j*g*d)))

# golden section search on [-2,2]
def golden_section(f, a, b, tol=1e-6, max_iter=200):
    gr = (math.sqrt(5) + 1) / 2
    c = b - (b-a)/gr
    d_ = a + (b-a)/gr
    fc = f(c); fd = f(d_)
...
gamma_hat, fval, iters = golden_section(obj, -2.0, 2.0, tol=1e-6)
omega, gamma_hat, iters, fval

### Tool (2026-02-25 08:48:57)

(0.03302979547646936, 1.2495974982546985, 32, -0.824334756255289)

### Assistant (2026-02-25 08:48:59)

def wrap2(a):
    return np.mod(a, 2*np.pi)

theta_oracle = theta_mod[test_idx + h]
theta_hat_corr = wrap2(theta_mod[test_idx] + h*omega + gamma_hat*wrappi(theta_u[test_idx] - theta_u[test_idx-ell]))
err = wrappi(theta_oracle - theta_hat_corr)

# strata by median logA on test_valid only
logA_test = logA[test_idx]
median_logA = float(np.median(logA_test))
low_mask = logA_test <= median_logA
high_mask = ~low_mask
low_idx = np.where(low_mask)[0]
high_idx = np.where(high_mask)[0]
len(low_idx), len(high_idx), median_logA

### Tool (2026-02-25 08:49:04)

(5464, 5464, -0.00897013140708288)

### Assistant (2026-02-25 08:49:05)

sd_X_safe = np.where(sd_X==0, 1.0, sd_X)
X80_std = (X80 - mu_X) / sd_X_safe

y_train = y_full[train_idx].astype(int)
y_test = y_full[test_idx].astype(int)
# ensure valid labels 0/1
assert set(np.unique(y_train)).issubset({0,1})
assert set(np.unique(y_test)).issubset({0,1})
y_train.mean(), y_test.mean()

### Tool (2026-02-25 08:49:11)

(0.49668432411222285, 0.5058565153733529)

### Assistant (2026-02-25 08:49:12)

def fit_logit(X, y):
    # deterministic solver
    clf = LogisticRegression(solver="lbfgs", max_iter=2000, n_jobs=None)
    clf.fit(X, y)
    return clf

Xtr = X80_std[train_idx]
Xte = X80_std[test_idx]

m_ar80 = fit_logit(Xtr, y_train)
p_ar80 = m_ar80.predict_proba(Xte)[:,1]

def metrics(y_true, p):
    return float(roc_auc_score(y_true, p)), float(log_loss(y_true, p, labels=[0,1]))

auc_ar80, ll_ar80 = metrics(y_test, p_ar80)
auc_ar80, ll_ar80

### Tool (2026-02-25 08:49:19)

(0.8293776464597737, 0.504581912196593)

### Assistant (2026-02-25 08:49:26)

S_hat_train = np.column_stack([np.sin(wrap2(theta_mod[train_idx] + h*omega + gamma_hat*wrappi(theta_u[train_idx]-theta_u[train_idx-ell]))),
                               np.cos(wrap2(theta_mod[train_idx] + h*omega + gamma_hat*wrappi(theta_u[train_idx]-theta_u[train_idx-ell])))])
# oracle uses theta_mod[i+h]
S_orc_train = np.column_stack([np.sin(theta_mod[train_idx + h]), np.cos(theta_mod[train_idx + h])])

mu_hat = S_hat_train.mean(axis=0)
sd_hat = S_hat_train.std(axis=0, ddof=0)
sd_hat_safe = np.where(sd_hat==0, 1.0, sd_hat)

S_hat_train_std = (S_hat_train - mu_hat)/sd_hat_safe
S_orc_train_std = (S_orc_train - mu_hat)/sd_hat_safe  # locked scaling

# on test
S_hat_test = np.column_stack([np.sin(theta_hat_corr), np.cos(theta_hat_corr)])
S_orc_test = np.column_stack([np.sin(theta_oracle), np.cos(theta_oracle)])
S_hat_test_std = (S_hat_test - mu_hat)/sd_hat_safe
S_orc_test_std = (S_orc_test - mu_hat)/sd_hat_safe

Xtr_fore = np.hstack([Xtr, S_hat_train_std])
Xte_fore = np.hstack([Xte, S_hat_test_std])
Xtr_orc = np.hstack([Xtr, S_orc_train_std])
Xte_orc = np.hstack([Xte, S_orc_test_std])

m_fore = fit_logit(Xtr_fore, y_train)
m_orc = fit_logit(Xtr_orc, y_train)

p_fore = m_fore.predict_proba(Xte_fore)[:,1]
p_orc = m_orc.predict_proba(Xte_orc)[:,1]

auc_fore, ll_fore = metrics(y_test, p_fore)
auc_orc, ll_orc = metrics(y_test, p_orc)
auc_fore, ll_fore, auc_orc, ll_orc

### Tool (2026-02-25 08:49:44)
...
### Assistant (2026-02-25 08:49:59)

def theta_match_stats(err_subset):
    return {
        "mean_cos": float(np.mean(np.cos(err_subset))),
        "mean_abs_err": float(np.mean(np.abs(err_subset))),
        "rmse": float(np.sqrt(np.mean(err_subset**2))),
    }

def strat_metrics(mask_idx):
    # mask_idx are positions within test_idx
    y_s = y_test[mask_idx]
    # if only one class, auc undefined; handle by nan
    def auc_safe(y,p):
    ...
    out = {
        "AR80": {"auc": auc_safe(y_s, p_ar80[mask_idx]), "logloss": ll_safe(y_s, p_ar80[mask_idx])},
        "forecast": {"auc": auc_safe(y_s, p_fore[mask_idx]), "logloss": ll_safe(y_s, p_fore[mask_idx])},
        "oracle": {"auc": auc_safe(y_s, p_orc[mask_idx]), "logloss": ll_safe(y_s, p_orc[mask_idx])},
    }
    return out

# positions in test arrays
low_pos = np.where(low_mask)[0]
high_pos = np.where(high_mask)[0]

metrics_low = strat_metrics(low_pos)
metrics_high = strat_metrics(high_pos)

theta_low = theta_match_stats(err[low_pos])
theta_high = theta_match_stats(err[high_pos])

def derived(m):
    auc_ar = m["AR80"]["auc"]; auc_f = m["forecast"]["auc"]; auc_o = m["oracle"]["auc"]
    denom = (auc_o - auc_ar)
    rec = (auc_f - auc_ar)/denom if denom!=0 and not np.isnan(denom) else float("nan")
    return {"recovery_fraction": float(rec), "auc_oracle_minus_forecast": float(auc_o - auc_f)}

derived_low = derived(metrics_low)
derived_high = derived(metrics_high)

metrics_low, theta_low, derived_low

### Tool (2026-02-25 08:50:17)

({'AR80': {'auc': 0.7865721215164758, 'logloss': 0.5567142427718819},
  'forecast': {'auc': 0.8258966476730458, 'logloss': 0.5109704709024316},
  'oracle': {'auc': 0.8682215558279669, 'logloss': 0.45204766420141357}},
...
### Assistant (2026-02-25 08:50:19)

metrics_high, theta_high, derived_high

### Tool (2026-02-25 08:50:21)

({'AR80': {'auc': 0.8711896670318466, 'logloss': 0.4524495816213041},
  'forecast': {'auc': 0.9075062236818688, 'logloss': 0.3927361127306984},
  'oracle': {'auc': 0.9200530965926959, 'logloss': 0.36351113295347354}},
...
### Assistant (2026-02-25 08:50:26)

result = {
  "counts": {"n_train": int(len(train_idx)), "n_test": int(len(test_idx))},
  "params_train": {"omega": float(omega), "gamma": float(gamma_hat)},
  "thresholds": {"median_logA_test": float(median_logA)},
  "strata": {
    "low": {
      "n": int(len(low_pos)),
      "theta_match": theta_low,
      "metrics": metrics_low,
      "derived": derived_low
    },
    "high": {
      "n": int(len(high_pos)),
      "theta_match": theta_high,
      "metrics": metrics_high,
      "derived": derived_high
    }
  },
  "notes": (
    "Fingerprint checks: x_kept_sha1="+sha1_x+
    "; R_sha1="+sha1_arr(R.astype(np.float64))+
    "; canonical_theta_sha1="+sha1_arr(canon['theta_mod'].astype(np.float64))+
    ". Recomputed (bp0.003_0.009) theta_mod_sha1="+sha1_arr(theta_mod.astype(np.float64))+
    "; u_bp_sha1="+sha1_arr(u_bp.astype(np.float64))+
    ". Models: LogisticRegression(lbfgs, default L2). "
    "Locked phase scaling uses TRAIN-valid S_hat mean/std; oracle standardized with same. "
    "filecite: fileciteturn0file0"
  )
}
print(json.dumps(result, indent=2))

### Tool (2026-02-25 08:50:34)
