# MAD-gate k optimization

- **Conversation ID:** `699f7399-1dc8-8327-bd23-80052240f5c1`
- **Date:** 2026-02-25

---

### User (2026-02-25 17:11:40)
...
### Assistant (2026-02-25 17:11:43)

import numpy as np, os, json, math, hashlib
from pathlib import Path
base="/mnt/data"
featurecache_path=Path(base)/"A4_FEATURECACHE_h25_AR80_AR40_theta_omega_masks_v1.npz"
canonical_path=Path(base)/"A4_CANONICAL_CACHE_L131072_x1e5_9e6_W2000_bp002_001_D001_seed12345.npz"
psi_path=Path(base)/"chebyshev_psi.npz"
featurecache_path.exists(), canonical_path.exists(), psi_path.exists()

### Tool (2026-02-25 17:12:10)

(True, True, True)

### Assistant (2026-02-25 17:12:13)

import numpy as np
fc=np.load(featurecache_path, allow_pickle=True)
fc.files[:20], len(fc.files)

### Tool (2026-02-25 17:12:14)
...
### Tool (2026-02-25 17:12:20)

(0.00012297217348543395, 36592, 25614)

### Assistant (2026-02-25 17:12:22)

cc=np.load(canonical_path, allow_pickle=True)
cc.files[:20], len(cc.files)

### Tool (2026-02-25 17:12:25)

(['x_kept', 'u_bp', 'theta_mod', 'R', 'dt', 'train_end', 'params', 'hashes'],
 8)

### Assistant (2026-02-25 17:12:27)

import scipy, scipy.signal, scipy.fft
scipy.__version__

### Tool (2026-02-25 17:12:36)

'1.14.1'

### Assistant (2026-02-25 17:12:41)
...
mu=fc["mu_X80_train"].astype(np.float64)
sd=fc["sd_X80_train"].astype(np.float64)
R=fc["R"].astype(np.float64)

# standardize X80 using train stats
X80s=(X80-mu)/sd
...
y[:] = 0
y_valid = (idx+h < N_kept)
y[y_valid] = (R[idx[y_valid]+h] - R[idx[y_valid]] > 0).astype(np.int8)

# recompute theta_mod and logA from chebyshev_psi
psi=np.load(psi_path, allow_pickle=True)
x_grid=psi["x_grid"].astype(np.float64)
psi_minus_x=psi["psi_minus_x"].astype(np.float64) if "psi_minus_x" in psi.files else psi["psi_minus_x_grid"].astype(np.float64)
# canonical x_kept
x_kept=cc["x_kept"].astype(np.float64)

# interpolate linear in x
u_raw=np.interp(x_kept, x_grid, psi_minus_x)

W=2000
cumsum=np.cumsum(u_raw)
...
# train-only zscore (first train_end)
m=u_detr[:train_end].mean()
s=u_detr[:train_end].std(ddof=0)
u_z=(u_detr - m)/(s if s>0 else 1.0)
...
Xtr=X80s[train_valid]
ytr=y[train_valid]
Xte=X80s[test_valid]
yte=y[test_valid]

m0=fit_lr(Xtr, ytr)
p0_tr=m0.predict_proba(Xtr)[:,1]
p0_te=m0.predict_proba(Xte)[:,1]
auc0=roc_auc_score(yte, p0_te)

# MAD stats from TRAIN-valid logA
logA_tr=logA[train_valid]
med=np.median(logA_tr)
MAD=np.median(np.abs(logA_tr - med))

def build_phase_cols(k, gated=True):
    if gated:
        denom = (k*MAD + 1e-12)
        w=np.clip((logA - med)/denom, 0.0, 1.0)
    else:
        w=np.ones_like(logA)
    s = w*np.sin(theta_mod)
    c = w*np.cos(theta_mod)
    # standardize on TRAIN-valid
    s_tr=s[train_valid]; c_tr=c[train_valid]
    ms=s_tr.mean(); ss=s_tr.std(ddof=0)
    mc=c_tr.mean(); sc=c_tr.std(ddof=0)
    ss = ss if ss>0 else 1.0
    sc = sc if sc>0 else 1.0
    s_z=(s-ms)/ss
    c_z=(c-mc)/sc
    return np.column_stack([s_z, c_z])

def auc_train_for_k(k):
    cols=build_phase_cols(k, gated=True)
    X=np.hstack([X80s, cols])
    Xtr=X[train_valid]; ytr=y[train_valid]
    lr=fit_lr(Xtr, ytr)
    p=lr.predict_proba(Xtr)[:,1]
    return roc_auc_score(ytr, p)

def fit_eval_model(cols):
    X=np.hstack([X80s, cols])
    lr=fit_lr(X[train_valid], y[train_valid])
    p_te=lr.predict_proba(X[test_valid])[:,1]
    return roc_auc_score(y[test_valid], p_te)

# plain phase (no gating)
auc_plain = fit_eval_model(build_phase_cols(k=1.0, gated=False))

# fixed k=3
auc_k3 = fit_eval_model(build_phase_cols(3.0, gated=True))
auc_train_k3 = auc_train_for_k(3.0)

# golden-section maximize train AUC over k in [0.5,10]
def golden_max(f, a, b, tol=1e-3, max_iter=80):
    ...
    k_star=(a+b)/2
    auc_star=f(k_star)
    return k_star, auc_star, it

k_star, auc_train_star, iters = golden_max(auc_train_for_k, 0.5, 10.0)

auc_kstar = fit_eval_model(build_phase_cols(k_star, gated=True))

out={
  "counts":{"n_train": int(train_valid.sum()), "n_test": int(test_valid.sum())},
  "train_mad":{"median_logA_train": float(med), "MAD_logA_train": float(MAD)},
  "k_optimization":{
      "k_star": float(k_star),
      "auc_train_at_k_star": float(auc_train_star),
      "auc_train_at_k_eq_3": float(auc_train_k3)
  },
  "test_auc":{
      "M0_AR80": float(auc0),
      "M_plain_phase": float(auc_plain),
      "M_k_eq_3": float(auc_k3),
      "M_k_star": float(auc_kstar),
      "delta_k_star_minus_k3": float(auc_kstar-auc_k3)
  },
  "notes": f"Golden-section iters={iters}. TRAIN-valid=i>=79,i<train_end,i+h<train_end; TEST-valid=i>=79,i>=train_end,i+h<N_kept. Phase extracted per closed spec with rFFT mask [0.003,0.009]. Featurecache X80 standardized using stored mu/sd; R used for y."
}
out

### Tool (2026-02-25 17:13:33)

WARNING: Automatic interrupt triggered after 60.0 seconds.

### Assistant (2026-02-25 17:14:36)

import numpy as np, math, scipy.signal
from sklearn.linear_model import LogisticRegression
from sklearn.metrics import roc_auc_score

# reload quickly
fc=np.load("/mnt/data/A4_FEATURECACHE_h25_AR80_AR40_theta_omega_masks_v1.npz", allow_pickle=True)
cc=np.load("/mnt/data/A4_CANONICAL_CACHE_L131072_x1e5_9e6_W2000_bp002_001_D001_seed12345.npz", allow_pickle=True)
psi=np.load("/mnt/data/chebyshev_psi.npz", allow_pickle=True)

dt=float(fc["dt"]); N_kept=int(fc["N_kept"]); train_end=int(fc["train_end"])
X80=fc["X80"].astype(np.float64)
mu=fc["mu_X80_train"].astype(np.float64); sd=fc["sd_X80_train"].astype(np.float64)
R=fc["R"].astype(np.float64)
X80s=(X80-mu)/sd
...
y=np.zeros(N_kept, dtype=np.int8)
valid = (idx+h < N_kept)
y[valid]=(R[idx[valid]+h]-R[idx[valid]]>0).astype(np.int8)
...
ma=np.empty_like(u_raw)
# for i < W
i_small=np.arange(min(W, N_kept))
ma[i_small]=cs[i_small]/(i_small+1)
if N_kept>W:
    i_big=np.arange(W, N_kept)
    ma[i_big]=(cs[i_big]-cs[i_big-W])/W
u_detr=u_raw-ma

m=u_detr[:train_end].mean()
s=u_detr[:train_end].std(ddof=0); s= s if s>0 else 1.0
u_z=(u_detr-m)/s

U=np.fft.rfft(u_z)
freq=np.fft.rfftfreq(N_kept, d=1.0)
mask=(freq>=0.003)&(freq<=0.009)
u_bp=np.fft.irfft(U*mask, n=N_kept)
...
Xte=X80s[test_valid]; yte=y[test_valid]

lr0=LogisticRegression(max_iter=400, solver="lbfgs")
lr0.fit(Xtr, ytr)
auc0=float(roc_auc_score(yte, lr0.predict_proba(Xte)[:,1]))

logA_tr=logA[train_valid]
med=float(np.median(logA_tr))
MAD=float(np.median(np.abs(logA_tr-med)))

sinth=np.sin(theta_mod); costh=np.cos(theta_mod)

def phase_cols_from_w(w):
    s=w*sinth; c=w*costh
    s_tr=s[train_valid]; c_tr=c[train_valid]
    ms=s_tr.mean(); ss=s_tr.std(ddof=0); ss=ss if ss>0 else 1.0
    mc=c_tr.mean(); sc=c_tr.std(ddof=0); sc=sc if sc>0 else 1.0
    return np.column_stack([(s-ms)/ss,(c-mc)/sc])

def gate_w(k):
    denom = (k*MAD + 1e-12)
    return np.clip((logA - med)/denom, 0.0, 1.0)

# reusable LR with warm start
lr=LogisticRegression(max_iter=300, solver="lbfgs", warm_start=True)

def train_auc_for_k(k):
    w=gate_w(k)
    cols=phase_cols_from_w(w)
    X=np.hstack([X80s, cols])
    lr.fit(X[train_valid], y[train_valid])
    p=lr.predict_proba(X[train_valid])[:,1]
    return float(roc_auc_score(y[train_valid], p))

def test_auc_for_cols(cols):
    X=np.hstack([X80s, cols])
    lr.fit(X[train_valid], y[train_valid])
    p=lr.predict_proba(X[test_valid])[:,1]
    return float(roc_auc_score(y[test_valid], p))

# plain phase
cols_plain=phase_cols_from_w(np.ones_like(logA))
auc_plain=test_auc_for_cols(cols_plain)

# k=3
auc_train_k3=train_auc_for_k(3.0)
auc_k3=test_auc_for_cols(phase_cols_from_w(gate_w(3.0)))

# golden section with fewer iterations/tol
def golden_max(f, a, b, tol=1e-2, max_iter=40):
    gr=(math.sqrt(5)-1)/2
    c=b - gr*(b-a)
    d=a + gr*(b-a)
    fc=f(c); fd=f(d)
    ...
    k_star=(a+b)/2
    auc_star=f(k_star)
    return float(k_star), float(auc_star), int(it)

k_star, auc_train_star, iters = golden_max(train_auc_for_k, 0.5, 10.0)
auc_kstar=test_auc_for_cols(phase_cols_from_w(gate_w(k_star)))

out={
  "counts":{"n_train": int(train_valid.sum()), "n_test": int(test_valid.sum())},
  "train_mad":{"median_logA_train": med, "MAD_logA_train": MAD},
  "k_optimization":{
      "k_star": k_star,
      "auc_train_at_k_star": auc_train_star,
      "auc_train_at_k_eq_3": auc_train_k3
  },
  "test_auc":{
      "M0_AR80": auc0,
      "M_plain_phase": auc_plain,
      "M_k_eq_3": auc_k3,
      "M_k_star": auc_kstar,
      "delta_k_star_minus_k3": auc_kstar-auc_k3
  },
  "notes": f"Golden-section iters={iters}, tol=1e-2. TRAIN-valid=i>=79,i<train_end,i+h<train_end; TEST-valid=i>=79,i>=train_end,i+h<N_kept. Phase extracted per closed spec with rFFT mask [0.003,0.009]. X80 standardized using featurecache mu/sd; y from R[i+h]-R[i]."
}
out

### Tool (2026-02-25 17:15:21)

WARNING: Automatic interrupt triggered after 60.0 seconds.

### Assistant (2026-02-25 17:16:29)
...
mu=fc["mu_X80_train"].astype(np.float32); sd=fc["sd_X80_train"].astype(np.float32)
...
cs=np.cumsum(u_raw)
ma=np.empty_like(u_raw)
m_small=min(W, N_kept)
i_small=np.arange(m_small)
ma[i_small]=cs[i_small]/(i_small+1)
if N_kept>W:
    i_big=np.arange(W, N_kept)
    ma[i_big]=(cs[i_big]-cs[i_big-W])/W
...
analytic=scipy.signal.hilbert(u_bp)
theta_mod=np.mod(np.angle(analytic), 2*np.pi)
logA=np.log(np.abs(analytic)+1e-12).astype(np.float32)
sinth=np.sin(theta_mod).astype(np.float32)
costh=np.cos(theta_mod).astype(np.float32)
...
def phase_cols(w):
    s=w*sinth; c=w*costh
    s_tr=s[train_valid]; c_tr=c[train_valid]
    ms=float(s_tr.mean()); ss=float(s_tr.std(ddof=0)); ss=ss if ss>0 else 1.0
    mc=float(c_tr.mean()); sc=float(c_tr.std(ddof=0)); sc=sc if sc>0 else 1.0
    return np.column_stack([(s-ms)/ss,(c-mc)/sc]).astype(np.float32)

def gate_w(k):
    denom = k*MAD + 1e-12
    return np.clip((logA - med)/denom, 0.0, 1.0).astype(np.float32)

Xtr=X80s[train_valid]; ytr=y[train_valid]
Xte=X80s[test_valid]; yte=y[test_valid]

# use liblinear for speed
lr0=LogisticRegression(max_iter=200, solver="liblinear")
lr0.fit(Xtr, ytr)
auc0=float(roc_auc_score(yte, lr0.decision_function(Xte)))

lr=LogisticRegression(max_iter=150, solver="liblinear", warm_start=True)

def train_auc_k(k):
    cols=phase_cols(gate_w(k))
    X=np.hstack([X80s, cols])
    lr.fit(X[train_valid], ytr)
    return float(roc_auc_score(ytr, lr.decision_function(X[train_valid])))

def test_auc_cols(cols):
    X=np.hstack([X80s, cols])
    lr.fit(X[train_valid], ytr)
    return float(roc_auc_score(yte, lr.decision_function(X[test_valid])))

auc_plain=test_auc_cols(phase_cols(np.ones_like(logA, dtype=np.float32)))
auc_train_k3=train_auc_k(3.0)
auc_k3=test_auc_cols(phase_cols(gate_w(3.0)))

def golden_max(f, a, b, tol=5e-2, max_iter=20):
    gr=(math.sqrt(5)-1)/2
    ...
    while (b-a)>tol and it<max_iter:
        ...
        it+=1
    k_star=(a+b)/2
    return float(k_star), float(f(k_star)), int(it)

k_star, auc_train_star, iters=golden_max(train_auc_k, 0.5, 10.0)
auc_kstar=test_auc_cols(phase_cols(gate_w(k_star)))
...
out={
 ...
 "notes": f"Optimization via golden-section on TRAIN AUC over k∈[0.5,10], tol=5e-2, iters={iters}. Phase/logA per closed spec (interp ψ−x onto x_kept; causal MA W=2000; TRAIN-only zscore; rFFT mask [0.003,0.009]; Hilbert). Logistic solver=liblinear for determinism/speed."
}
out

### Tool (2026-02-25 17:17:05)

WARNING: Automatic interrupt triggered after 60.0 seconds.

### Assistant (2026-02-25 17:18:14)
...
# slice train/test once
X80_tr=X80s[train_valid]; X80_te=X80s[test_valid]
y_tr=y[train_valid]; y_te=y[test_valid]
sin_tr=sinth[train_valid]; cos_tr=costh[train_valid]; logA_tr=logA[train_valid]
sin_te=sinth[test_valid]; cos_te=costh[test_valid]; logA_te=logA[test_valid]

n_train=int(train_valid.sum()); n_test=int(test_valid.sum())
n_train, n_test

### Tool (2026-02-25 17:18:39)

WARNING: Automatic interrupt triggered after 60.0 seconds.

### Assistant (2026-02-25 17:19:46)

import time, numpy as np, scipy.signal, math
from sklearn.linear_model import LogisticRegression
from sklearn.metrics import roc_auc_score
t0=time.time()
fc=np.load("/mnt/data/A4_FEATURECACHE_h25_AR80_AR40_theta_omega_masks_v1.npz", allow_pickle=True)
...
### Tool (2026-02-25 17:19:57)

WARNING: Automatic interrupt triggered after 60.0 seconds.

### Assistant (2026-02-25 17:21:05)

import numpy as np, time
t0=time.time()
fc=np.load("/mnt/data/A4_FEATURECACHE_h25_AR80_AR40_theta_omega_masks_v1.npz", allow_pickle=True)
time.time()-t0, fc.files[:5], len(fc.files)

### Tool (2026-02-25 17:21:17)
...
bash -lc python - <<'PY'
import numpy as np, time
p='/mnt/data/A4_FEATURECACHE_h25_AR80_AR40_theta_omega_masks_v1.npz'
t=time.time(); fc=np.load(p, allow_pickle=True); print('opened',time.time()-t)
# force load small arrays
for k in ['dt','N_kept','train_end']:
    a=fc[k]
print('loaded small',time.time()-t)
PY

### Assistant (2026-02-25 17:22:37)

bash -lc ls -lh /mnt/data/A4_FEATURECACHE_h25_AR80_AR40_theta_omega_masks_v1.npz && python -c "import numpy as np, time; t=time.time(); fc=np.load('/mnt/data/A4_FEATURECACHE_h25_AR80_AR40_theta_omega_masks_v1.npz', allow_pickle=True); print('open',time.time()-t); t2=time.time(); dt=float(fc['dt']); print('dt',dt,'load',time.time()-t2)"

### Tool (2026-02-25 17:22:45)

-rw-r--r-- 1 root root 1.4M Feb 25 22:22 /mnt/data/A4_FEATURECACHE_h25_AR80_AR40_theta_omega_masks_v1.npz

### Assistant (2026-02-25 17:22:45)

bash -lc python -u - <<'PY'
import numpy as np, time, sys
p='/mnt/data/A4_FEATURECACHE_h25_AR80_AR40_theta_omega_masks_v1.npz'
print('start'); sys.stdout.flush()
t=time.time(); fc=np.load(p, allow_pickle=True); print('opened in',time.time()-t); sys.stdout.flush()
print('keys',len(fc.files)); sys.stdout.flush()
print('dt',float(fc['dt'])); sys.stdout.flush()
PY

### Assistant (2026-02-25 17:22:52)

bash -lc python -c "print('hi')"

### Tool (2026-02-25 17:22:58)

hi

### Assistant (2026-02-25 17:22:58)

bash -lc python - <<'PY'
import zipfile
z=zipfile.ZipFile('/mnt/data/A4_FEATURECACHE_h25_AR80_AR40_theta_omega_masks_v1.npz')
print('names',len(z.namelist()))
print(z.namelist()[:5])
PY

### Tool (2026-02-25 17:23:05)

names 23
['theta.npy', 'R.npy', 'dt.npy', 'N_kept.npy', 'train_end.npy']

### Assistant (2026-02-25 17:23:05)

bash -lc python -u - <<'PY'
import numpy as np, time
p='/mnt/data/A4_FEATURECACHE_h25_AR80_AR40_theta_omega_masks_v1.npz'
t=time.time(); fc=np.load(p, allow_pickle=False); print('opened',time.time()-t)
print('dt',float(fc['dt']))
PY

### Assistant (2026-02-25 17:23:12)

bash -lc python - <<'PY'
import zipfile, io, numpy as np
z=zipfile.ZipFile('/mnt/data/A4_FEATURECACHE_h25_AR80_AR40_theta_omega_masks_v1.npz')
with z.open('dt.npy') as f:
    data=f.read()
arr=np.load(io.BytesIO(data))
print('dt',float(arr))
PY

### Tool (2026-02-25 17:23:19)

dt 0.00012297217348543395

### Assistant (2026-02-25 17:23:19)

import zipfile, io, numpy as np, time, scipy.signal, math
from sklearn.linear_model import LogisticRegression
from sklearn.metrics import roc_auc_score

def load_npz_via_zip(path, keys):
    out={}
    with zipfile.ZipFile(path) as z:
        for k in keys:
            ...
            with z.open(name) as f:
                data=f.read()
            out[k]=np.load(io.BytesIO(data), allow_pickle=False)
    return out

t0=time.time()
fc_needed=load_npz_via_zip("/mnt/data/A4_FEATURECACHE_h25_AR80_AR40_theta_omega_masks_v1.npz",
                          ["X80","mu_X80_train","sd_X80_train","R","dt","N_kept","train_end"])
time.time()-t0, [fc_needed[k].shape for k in ["X80","R"]]

### Tool (2026-02-25 17:23:27)

(0.12581682205200195, [(36592, 80), (36592,)])

### Assistant (2026-02-25 17:23:31)

cc_needed=load_npz_via_zip("/mnt/data/A4_CANONICAL_CACHE_L131072_x1e5_9e6_W2000_bp002_001_D001_seed12345.npz",
                          ["x_kept"])
cc_needed["x_kept"].shape

### Tool (2026-02-25 17:23:33)

(36592,)

### Assistant (2026-02-25 17:23:34)

psi_needed=load_npz_via_zip("/mnt/data/chebyshev_psi.npz", ["x_grid","psi_minus_x"])
psi_needed["x_grid"].shape, psi_needed["psi_minus_x"].shape

### Tool (2026-02-25 17:23:37)

((100000,), (100000,))

### Assistant (2026-02-25 17:23:39)

import numpy as np, math, scipy.signal, time
from sklearn.linear_model import LogisticRegression
from sklearn.metrics import roc_auc_score

X80=fc_needed["X80"].astype(np.float32)
mu=fc_needed["mu_X80_train"].astype(np.float32)
sd=fc_needed["sd_X80_train"].astype(np.float32)
R=fc_needed["R"].astype(np.float32)
dt=float(fc_needed["dt"])
N_kept=int(fc_needed["N_kept"])
train_end=int(fc_needed["train_end"])
...
# theta/logA recompute
x_grid=psi_needed["x_grid"].astype(np.float64)
psi_minus_x=psi_needed["psi_minus_x"].astype(np.float64)
x_kept=cc_needed["x_kept"].astype(np.float64)
...
def make_cols(sin_vec, cos_vec, logA_vec, k=None):
    if k is None:
        w=np.ones_like(logA_vec, dtype=np.float32)
    else:
        denom=k*MAD + 1e-12
        w=np.clip((logA_vec-med)/denom, 0.0, 1.0).astype(np.float32)
    s=w*sin_vec; c=w*cos_vec
    return s, c

def standardize_cols(s_tr, c_tr, s_te, c_te):
    ms=float(s_tr.mean()); ss=float(s_tr.std(ddof=0)); ss=ss if ss>0 else 1.0
    mc=float(c_tr.mean()); sc=float(c_tr.std(ddof=0)); sc=sc if sc>0 else 1.0
    return ((s_tr-ms)/ss, (c_tr-mc)/sc, (s_te-ms)/ss, (c_te-mc)/sc)

lr0=LogisticRegression(max_iter=200, solver="liblinear")
lr0.fit(X80_tr, y_tr)
auc0=float(roc_auc_score(y_te, lr0.decision_function(X80_te)))

lr=LogisticRegression(max_iter=150, solver="liblinear", warm_start=True)

def fit_auc_train(k):
    s_tr, c_tr = make_cols(sin_tr, cos_tr, logA_tr, k=k)
    s_te, c_te = make_cols(sin_te, cos_te, logA_te, k=k)
    zs_tr, zc_tr, zs_te, zc_te = standardize_cols(s_tr, c_tr, s_te, c_te)
    Xtr=np.hstack([X80_tr, zs_tr[:,None], zc_tr[:,None]])
    lr.fit(Xtr, y_tr)
    return float(roc_auc_score(y_tr, lr.decision_function(Xtr)))
...
def fit_auc_test(k):
    ...
    zs_tr, zc_tr, zs_te, zc_te = standardize_cols(s_tr, c_tr, s_te, c_te)
    Xtr=np.hstack([X80_tr, zs_tr[:,None], zc_tr[:,None]])
    Xte=np.hstack([X80_te, zs_te[:,None], zc_te[:,None]])
    lr.fit(Xtr, y_tr)
    return float(roc_auc_score(y_te, lr.decision_function(Xte)))

# plain
auc_plain=fit_auc_test(k=None)

# k=3
auc_train_k3=fit_auc_train(3.0)
auc_k3=fit_auc_test(3.0)

def golden_max(f, a, b, tol=5e-2, max_iter=18):
    gr=(math.sqrt(5)-1)/2
    c=b - gr*(b-a); d=a + gr*(b-a)
    fc=f(c); fd=f(d)
    it=0
...
k_star, auc_train_star, iters = golden_max(fit_auc_train, 0.5, 10.0)
auc_kstar=fit_auc_test(k_star)

out={
 "counts":{"n_train": int(y_tr.size), "n_test": int(y_te.size)},
 "train_mad":{"median_logA_train": med, "MAD_logA_train": MAD},
 "k_optimization":{
    "k_star": k_star,
    "auc_train_at_k_star": auc_train_star,
 ...
 "notes": f"Preproc_time_sec={prep_time:.3f}. Golden-section tol=5e-2 iters={iters}. Solver=liblinear. Phase/logA recomputed per closed spec with rFFT mask [0.003,0.009]."
}
out

### Tool (2026-02-25 17:24:08)

{'counts': {'n_train': 25485, 'n_test': 10928},
 'train_mad': {'median_logA_train': -1.2598497867584229,
  'MAD_logA_train': 0.5187720060348511},
 'k_optimization': {'k_star': 8.946800936109817,
  'auc_train_at_k_star': 0.8369341483883828,
  'auc_train_at_k_eq_3': 0.8369636499718786},
 'test_auc': {'M0_AR80': 0.8293647826553037,
 ...
 'notes': 'Preproc_time_sec=0.087. Golden-section tol=5e-2 iters=11. Solver=liblinear. Phase/logA recomputed per closed spec with rFFT mask [0.003,0.009].'}

### Assistant (2026-02-25 17:24:46)
