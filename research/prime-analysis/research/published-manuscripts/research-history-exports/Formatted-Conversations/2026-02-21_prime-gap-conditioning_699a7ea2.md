# Prime Gap Conditioning

- **Conversation ID:** `699a7ea2-4454-8328-8f25-da257ad5b9f1`
- **Date:** 2026-02-21

---

### User (2026-02-21 22:57:28)
...
### Assistant (2026-02-21 22:57:50)

import numpy as np, math, json, os, sys
from numpy.fft import rfft, irfft

### Assistant (2026-02-21 22:57:54)

data_dir="/mnt/data"
cheb=np.load(os.path.join(data_dir,"chebyshev_psi.npz"))
pr=np.load(os.path.join(data_dir,"primes.npz"))
gaps=np.load(os.path.join(data_dir,"gaps.npz"))
cheb.files, pr.files, gaps.files[:10]

### Tool (2026-02-21 22:57:56)

(['N', 'step', 'x_grid', 'psi_grid', 'psi_minus_x'],
 ['N', 'primes', 'is_prime'],
 ['N', 'gaps'])

### Assistant (2026-02-21 22:57:58)

x_grid=cheb["x_grid"].astype(np.float64)
psi_minus_x=cheb["psi_minus_x"].astype(np.float64)
primes=pr["primes"].astype(np.int64)
gaps_arr=gaps["gaps"].astype(np.int64)
len(primes), len(gaps_arr), primes[:5], gaps_arr[:5]

### Tool (2026-02-21 22:58:00)

(664579, 664578, array([ 2,  3,  5,  7, 11]), array([1, 2, 2, 4, 2]))

### Assistant (2026-02-21 22:58:02)

# Step 1: uniform t grid L=65536 by interpolating psi_minus_x from x_grid onto t=log(x)
L=65536
t_grid=np.log(x_grid)
t_min=float(t_grid.min())
t_max=float(t_grid.max())
t_uniform=np.linspace(t_min, t_max, L)
# interpolate psi_minus_x as function of t; x_grid increasing, so t_grid increasing
u_raw_full=np.interp(t_uniform, t_grid, psi_minus_x)
dt=float(t_uniform[1]-t_uniform[0])
dt

### Tool (2026-02-21 22:58:05)

0.00024594622340595577

### Assistant (2026-02-21 22:58:07)

# Step 2: restrict to x in [1e5, 9e6]
x_uniform=np.exp(t_uniform)
mask=(x_uniform>=1e5) & (x_uniform<=9e6)
idx_kept=np.where(mask)[0]
t_kept=t_uniform[idx_kept]
x_kept=x_uniform[idx_kept]
u_raw=u_raw_full[idx_kept]
n_kept=len(idx_kept)
n_kept, x_kept[0], x_kept[-1]

### Tool (2026-02-21 22:58:11)

(18296, 100006.32008830582, 8998557.287555724)

### Assistant (2026-02-21 22:58:12)

# Step 3: split time ordered 70/30 on kept indices
train_end=int(0.7*n_kept)
train_idx=np.arange(train_end)
test_idx=np.arange(train_end, n_kept)
train_end, len(test_idx)

### Tool (2026-02-21 22:58:14)

(12807, 5489)

### Assistant (2026-02-21 22:58:17)

# Step 4: preprocess u(t) leakage-safe:
W=2000
# causal moving average detrend: for each i, avg over last W samples incl i.
...
sigma=float(u_detr[train_idx].std(ddof=0))
u_std=(u_detr - mu)/(sigma if sigma>0 else 1.0)

mu, sigma

### Tool (2026-02-21 22:58:20)

(7.296664503769434, 176.3404688541781)

### Assistant (2026-02-21 22:58:22)

# bandpass [0.002,0.01] cycles/sample via FFT on full kept sequence
fmin=0.002
fmax=0.01
N=n_kept
U=rfft(u_std)
...
# Hilbert transform analytic signal for real u_bp.
# implement via FFT method: analytic signal keeps positive freqs doubled, zeros negative.
...
theta_mod.min(), theta_mod.max()

### Tool (2026-02-21 22:58:29)

(0.00023981725979008343, 6.282344256781272)

### Assistant (2026-02-21 22:58:30)

# Phase binning B=12
B=12
bin_edges=np.linspace(0, 2*np.pi, B+1)
# bin index 0..B-1, using right-exclusive except last
bin_idx=np.digitize(theta_mod, bin_edges, right=False)-1
bin_idx[bin_idx==B]=B-1
bin_idx.min(), bin_idx.max(), np.bincount(bin_idx, minlength=B)[:5], bin_edges[:3]

### Tool (2026-02-21 22:58:32)

(0,
 11,
 array([1577, 1535, 1521, 1617, 1707]),
 array([0.        , 0.52359878, 1.04719755]))

### Assistant (2026-02-21 22:58:34)

# Map primes p in [1e5,9e6) to nearest t-index using monotone x_kept
pmin=100000
pmax=9000000  # exclusive 9e6? spec says [1e5,9e6)
mask_p=(primes>=pmin) & (primes<pmax)
pr_range=primes[mask_p]
len(pr_range), pr_range[0], pr_range[-1]

### Tool (2026-02-21 22:58:36)

(592897, 100003, 8999993)

### Assistant (2026-02-21 22:58:38)

# efficient nearest index: use searchsorted
def nearest_index_monotone(arr, vals):
    # arr increasing float, vals int
    arr=np.asarray(arr)
    vals=np.asarray(vals)
    pos=np.searchsorted(arr, vals, side='left')
    pos=np.clip(pos, 0, len(arr)-1)
    # compare with previous
    prev=np.clip(pos-1, 0, len(arr)-1)
    choose_prev=np.abs(arr[prev]-vals) <= np.abs(arr[pos]-vals)
    return np.where(choose_prev, prev, pos)

idx_map=nearest_index_monotone(x_kept, pr_range)
# compute split membership for primes: based on idx_map in train or test
is_train_prime=idx_map < train_end
num_train_primes=int(is_train_prime.sum())
num_test_primes=int((~is_train_prime).sum())
num_train_primes, num_test_primes

### Tool (2026-02-21 22:58:41)

(162139, 430758)

### Assistant (2026-02-21 22:58:43)

# Gap events: compute gaps for primes in range where both ends in [1e5,9e6)
p_all=primes
g_all=np.diff(p_all)
# indices n where p_n in range and p_{n+1} in range
mask_gap=(p_all[:-1]>=pmin) & (p_all[1:]<pmax)
p_n=p_all[:-1][mask_gap]
g_n=g_all[mask_gap]
# map p_n to indices
idx_n=nearest_index_monotone(x_kept, p_n)
bin_n=bin_idx[idx_n]
is_train_gap=idx_n < train_end
num_gaps_train=int(is_train_gap.sum())
num_gaps_test=int((~is_train_gap).sum())
len(p_n), num_gaps_train, num_gaps_test, g_n[:5].tolist()

### Tool (2026-02-21 22:58:47)

(592896, 162139, 430757, [16, 24, 6, 8, 12])

### Assistant (2026-02-21 22:58:49)

K=[10,20,50,100,200,500]
def per_bin_stats(bin_assign, gaps, split_mask):
    out=[]
    for b in range(B):
        sel=(bin_assign==b) & split_mask
        n=int(sel.sum())
        frac={}
        if n==0:
            for k in K: frac[str(k)]=None
        else:
            gg=gaps[sel]
            for k in K:
                frac[str(k)]=float((gg>=k).mean())
        out.append({"bin":b,"n":n,"frac_ge_k":frac})
    return out

train_stats=per_bin_stats(bin_n, g_n, is_train_gap)
test_stats=per_bin_stats(bin_n, g_n, ~is_train_gap)

# variance across bins of frac_ge_k on test
var_across={}
for k in K:
    vals=[d["frac_ge_k"][str(k)] for d in test_stats if d["n"]>0]
    var_across[str(k)]=float(np.var(vals, ddof=0)) if len(vals)>0 else None

# MI estimate on TEST
# gap_cat: 0 if g<20,1 if 20<=g<50,2 if 50<=g<100,3 if g>=100
test_sel=~is_train_gap
b_test=bin_n[test_sel]
g_test=g_n[test_sel]
gap_cat=np.zeros_like(g_test)
gap_cat[(g_test>=20)&(g_test<50)]=1
gap_cat[(g_test>=50)&(g_test<100)]=2
gap_cat[(g_test>=100)]=3
# plugin MI in nats
def mutual_info_discrete(x, y, nx, ny):
    # x 0..nx-1, y 0..ny-1
    n=len(x)
    ...
    for i in range(nx):
        for j in range(ny):
            pxy=joint[i,j]
            if pxy>0 and px[i,0]>0 and py[0,j]>0:
                mi += pxy*math.log(pxy/(px[i,0]*py[0,j]))
    return mi, joint, px.squeeze(), py.squeeze()

mi_nats, joint, px, py = mutual_info_discrete(b_test, gap_cat, B, 4)
mi_nats

### Tool (2026-02-21 22:58:59)

5.926486310472349e-05

### Assistant (2026-02-21 22:59:00)

result={
  "params":{"L":65536,"W":W,"bandpass":[0.002,0.01],"B":B,"K":K,"seed":12345},
  "counts":{
      "num_primes_used_train":num_train_primes,
      "num_primes_used_test":num_test_primes,
      "num_gaps_train":num_gaps_train,
      "num_gaps_test":num_gaps_test
  },
  "by_split":{
      "train":{"per_bin":train_stats},
      "test":{
          "per_bin":test_stats,
          "frac_ge_k_var_across_bins":var_across,
          "mi_bin_gapcat":float(mi_nats)
      }
  }
}
# ensure JSON serializable and reasonably small
json_text=json.dumps(result, separators=(',',':'))
len(json_text)

### Tool (2026-02-21 22:59:05)

4214

### Assistant (2026-02-21 22:59:06)
