# Γ Sweep Stability Analysis

- **Conversation ID:** `699b5733-e3f4-832f-859f-b254fbb5961c`
- **Date:** 2026-02-22

---

### User (2026-02-22 14:21:30)
...
### Assistant (2026-02-22 14:21:33)

import numpy as np, json, math, gzip, os, sys
...
horizons=[25,100]
seed=12345
Gamma_list=[80,100,120,150,180,200,250,300,350,400,500,650,800]
K=3
...
# uniform t grid length L on full available range
...
def fit_phase_logistic(theta, y, idx_train, idx_test):
    X_train=fourier_features(theta[idx_train], K)
    y_train=y[idx_train].astype(int)
    X_test=fourier_features(theta[idx_test], K)
    y_test=y[idx_test].astype(int)
    # if train has single class, return nan
    if len(np.unique(y_train))<2 or len(np.unique(y_test))<2:
        return float("nan"), None, y_test, None
    clf=LogisticRegression(max_iter=2000, solver="lbfgs")
    clf.fit(X_train, y_train)
    p=clf.predict_proba(X_test)[:,1]
    auc=float(roc_auc_score(y_test, p))
    return auc, p, y_test, clf
...
# Build REAL pipeline
z_real=detrend_standardize(y_real)
s_real_bp=fft_bandpass(z_real, bandpass[0], bandpass[1])
theta_real=hilbert_phase(s_real_bp)

# Targets using primes
pr=np.load(primes_path)["primes"].astype(np.int64)

expDelta=math.exp(Delta)
# prime counts for each x_i: [x_i, x_i*expDelta)
x_hi=x*expDelta
lo_idx=np.searchsorted(pr, x, side="left")
hi_idx=np.searchsorted(pr, x_hi, side="left")
P=(hi_idx-lo_idx).astype(np.float64)
E=x*(expDelta-1.0)/np.log(x)
R=(P-E)/np.sqrt(E)

# build desticky labels for each horizon
def build_desticky_labels(R, h):
    n=R.size
    y=np.zeros(n-h, dtype=np.int8)
    y[:] = (R[h:]-R[:n-h] > 0).astype(np.int8)
    return y  # aligned to i=0..n-h-1

AUC_real={}
preds_real={}
ytests_real={}
for h in horizons:
    y_h=build_desticky_labels(R,h)
    n_valid=y_h.size
    # indices in original i space are 0..n_valid-1. test split uses train_end on i, but need i+h within bounds; by construction.
    idx_train=np.arange(0, max(0, train_end-h))
    idx_test=np.arange(train_end, n_valid)
    auc, p_test, y_test, clf = fit_phase_logistic(theta_real[:n_valid], y_h, idx_train, idx_test)
    AUC_real[str(h)]=auc
    preds_real[str(h)]=p_test
    ytests_real[str(h)]=y_test
...
with gzip.open(zeros_path,'rt') as f:
    for line in f:
        ...
        if not line: 
            continue
        # try parse first token float
        try:
            g=float(line.split()[0])
        except:
            continue
        gammas.append(g)
        if len(gammas)>=100000:
            break
gammas=np.array(gammas, dtype=np.float64)
gammas=gammas[np.isfinite(gammas)]
gammas.sort()
maxG=max(Gamma_list)
gammas_max=gammas[gammas<=maxG]
m=gammas_max.size

# Precompute trig matrices efficiently
t_vec=t.astype(np.float64)
exp_half=np.exp(t_vec/2.0)
a=0.5
g=gammas_max
den=(a*a + g*g)
# We'll compute cos and sin in blocks to limit memory if needed
# but 18296x600 ok
cos_mat=np.cos(np.outer(t_vec, g))
sin_mat=np.sin(np.outer(t_vec, g))
# contribution per gamma: c_j(t) = 2*exp_half*(a cos + g sin)/den
# y_zero = -sum c_j(t)
contrib = (a*cos_mat + sin_mat*g) / den  # N x m
contrib *= (2.0*exp_half)[:,None]
cum = np.cumsum(contrib, axis=1)  # N x m

# map Gamma to index in g
Gamma_to_idx={}
for Gval in Gamma_list:
    Gamma_to_idx[Gval]=int(np.searchsorted(g, Gval, side="right")-1)

def build_zero_signal(Gval):
    j=Gamma_to_idx[Gval]
    if j<0:
...
AUC_zero_cache={}
for Gval in Gamma_list:
    y0=build_zero_signal(Gval)
    z0=detrend_standardize(y0)
    s0=fft_bandpass(z0, bandpass[0], bandpass[1])
    th0=hilbert_phase(s0)
    # similarity on TEST segment (full kept indices from train_end..)
    corr_bp_test=corr(s_real_bp[train_end:], s0[train_end:])
    R_theta_test=phase_locking_value(theta_real[train_end:], th0[train_end:])
    # AUCs for horizons
    aucs={}
    for h in horizons:
        y_h=build_desticky_labels(R,h)
        ...
        idx_train=np.arange(0, max(0, train_end-h))
        idx_test=np.arange(train_end, n_valid)
        auc, _, _, _ = fit_phase_logistic(th0[:n_valid], y_h, idx_train, idx_test)
        aucs[str(h)]=auc
    per_G[str(Gval)]={"K":K,"corr_bp_test":corr_bp_test,"R_theta_test":R_theta_test,"AUC_zero":aucs,"n_zeros_used": int(Gamma_to_idx[Gval]+1 if Gamma_to_idx[Gval]>=0 else 0)}
    # cache selected for bootstrap
    if Gval in (200,400,800):
        theta_zero_cache[Gval]=th0
        s_zero_bp_cache[Gval]=s0

# Bootstrap
rng=np.random.default_rng(seed)
def block_bootstrap_auc(y_true, y_score, block_len=512, B=200):
    y_true=np.asarray(y_true); y_score=np.asarray(y_score)
    n=y_true.size
    if n==0: 
        return {"mean": float("nan"), "std": float("nan"), "n_valid":0, "n_nan":B}
    ...
    if len(aucs)==0:
        return {"mean": float("nan"), "std": float("nan"), "n_valid":0, "n_nan":n_nan}
    return {"mean": float(np.mean(aucs)), "std": float(np.std(aucs, ddof=0)), "n_valid": len(aucs), "n_nan": n_nan}

stability={"bootstrap":{}}
# prepare predictions for REAL and selected Gammas
def get_test_arrays(theta, h):
    y_h=build_desticky_labels(R,h)
    n_valid=y_h.size
    idx_train=np.arange(0, max(0, train_end-h))
    idx_test=np.arange(train_end, n_valid)
    auc, p_test, y_test, clf = fit_phase_logistic(theta[:n_valid], y_h, idx_train, idx_test)
    return auc, p_test, y_test

# REAL
for h in horizons:
    auc, p_test, y_test = get_test_arrays(theta_real, h)
    stability["bootstrap"].setdefault("REAL",{})[str(h)]=block_bootstrap_auc(y_test, p_test, 512, 200)

for Gval in (200,400,800):
    for h in horizons:
        auc, p_test, y_test = get_test_arrays(theta_zero_cache[Gval], h)
        stability["bootstrap"].setdefault(f"Gamma_{Gval}",{})[str(h)]=block_bootstrap_auc(y_test, p_test, 512, 200)

# Conclusions
minG_R=None
for Gval in Gamma_list:
    if per_G[str(Gval)]["R_theta_test"]>=0.99:
        minG_R=Gval
        break
...
for Gval in Gamma_list:
    ...
    for h in horizons:
        ar=AUC_real[str(h)]
        az=per_G[str(Gval)]["AUC_zero"][str(h)]
        if not (np.isfinite(ar) and np.isfinite(az) and abs(az-ar)<=eps):
            ok=False; break
    if ok:
        minG_A=Gval
        break
...
notes=[]
notes.append("R_theta_test uses phase-locking value |mean(exp(i*(theta_real-theta_zero)))| on TEST.")
notes.append("corr_bp_test is Pearson correlation between bandpassed standardized signals on TEST.")
notes.append("Desticky target y=1{R_{i+h}-R_i>0} avoids median-threshold drift but remains autocorrelated.")
if m==0:
    notes.append("Zero list parsing produced zero gammas <= max(Gamma_list). Check zeros1.gz format.")
else:
    notes.append(f"Used {m} zeros up to maxΓ={maxG} from zeros1.gz.")
...
out={
    "meta":{
        ...
        "max_zeros_loaded": int(len(gammas)),
        "zeros_used_maxGamma": int(m),
        "params":{"L":L,"x_range":[x_min,x_max],"W":W,"train_frac":train_frac,"bandpass":list(bandpass),"Delta":Delta,"horizons":horizons,"seed":seed,"K":K}
    },
    "real":{
        "AUC_real":AUC_real,
        "class_balance_test": {str(h): float(ytests_real[str(h)].mean()) if ytests_real[str(h)] is not None else None for h in horizons}
    },
    "per_G":per_G,
    "stability":stability,
    "conclusion":{
        "min_Gamma_for_R_theta_ge_0.99": minG_R,
        "min_Gamma_for_AUC_within_eps_of_real": {"eps":eps, "Gamma": minG_A},
        "notes":notes
    }
}
print(json.dumps(out, indent=2))

### Tool (2026-02-22 14:23:27)
