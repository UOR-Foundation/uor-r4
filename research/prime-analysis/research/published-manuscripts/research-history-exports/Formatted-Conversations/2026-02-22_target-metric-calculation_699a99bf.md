...
- **Conversation ID:** `699a99bf-4028-832b-ae0e-0bbd491c236a`
- **Date:** 2026-02-22

---

### User (2026-02-22 00:53:08)
...
# Bandpass via FFT on full signal length L
freqs = rfftfreq(L, d=1.0)  # cycles/sample
U = rfft(u_det)
mask = (freqs >= band_lo) & (freqs <= band_hi)
# Keep DC? no, detrended already; mask excludes 0
U_bp = U * mask
u_bp = irfft(U_bp, n=L)

# Hilbert analytic signal for phase
...
theta_k = theta[kept_idx]
phase_bin = np.floor(theta_k * (B/(2*np.pi))).astype(int)
phase_bin = np.clip(phase_bin, 0, B-1)

# Compute P,E,z for each kept index
expD = math.exp(Delta)
x_k = x_uniform[kept_idx]
# counts using searchsorted
lo = np.searchsorted(primes, x_k, side="left")
hi = np.searchsorted(primes, x_k*expD, side="left")
...
# g(theta) score and standardize by TRAIN mean/std
th_train = theta_k[train_idx]
def g_raw(th):
    return (np.cos(th) + np.sin(th) 
            + 0.5*np.cos(2*th) + 0.5*np.sin(2*th)
            + 0.25*np.cos(3*th) + 0.25*np.sin(3*th))
g_all = g_raw(theta_k)
g_mean = float(np.mean(g_all[train_idx]))
g_std = float(np.std(g_all[train_idx], ddof=0))
g_std = g_std if g_std>0 else 1.0
g_z = (g_all - g_mean)/g_std

# rate_bin on TEST: mean(P/E) for each bin using all TEST indices
rate_bin = []
pe_ratio = P / E
for j in range(B):
    m = (phase_bin[test_idx] == j)
    rate_bin.append(float(np.mean(pe_ratio[test_idx][m])) if np.any(m) else float("nan"))

# helper MI in bits
ln2 = math.log(2.0)
def mi_bits(x_cat, y_bin):
    # mutual_info_score assumes labels 1D arrays
    mi_nats = mutual_info_score(x_cat, y_bin)
    return float(mi_nats/ln2)

def contingency_12x2(x_cat, y_bin, B):
    tab = np.zeros((B,2), dtype=np.int64)
    for xc, yv in zip(x_cat, y_bin):
        tab[int(xc), int(yv)] += 1
    return tab
...
targets = {}
for h in H:
    # TRAIN-valid indices for thresholds: i in TRAIN and i+h in TRAIN => i <= n_train-1-h
    train_valid = np.arange(0, max(0, n_train - h), dtype=int)
    # TEST valid indices: i in TEST and i+h < n
    test_valid = test_idx[test_idx + h < n]
    # precompute arrays for thresholding
    z_train_future = z[train_valid + h] if len(train_valid)>0 else np.array([], dtype=float)
    z_train_diff = (z[train_valid + h] - z[train_valid]) if len(train_valid)>0 else np.array([], dtype=float)

    med_thr = float(np.median(z_train_future)) if z_train_future.size else float("nan")
    q90_thr = float(np.quantile(z_train_future, 0.9)) if z_train_future.size else float("nan")
    q75_inc = float(np.quantile(z_train_diff, 0.75)) if z_train_diff.size else float("nan")

    # define function to compute metrics for given y on test_valid
    def compute_metrics(y):
        y = y.astype(int)
        xcat = phase_bin[test_valid]
        # pos rate
        pos_rate = float(np.mean(y)) if y.size else float("nan")
        # MI bits
        MI = mi_bits(xcat, y) if y.size else float("nan")
        # chi2 p
        if y.size:
            tab = contingency_12x2(xcat, y, B)
            # chi2_contingency requires all expected >0? it handles but if a column all zero, may error
            try:
        ...
        if y.size and (np.unique(y).size==2):
            auc = float(roc_auc_score(y, g_z[test_valid]))
        else:
            auc = float("nan")
        # permutation MI p
        if y.size:
            ...
            for _ in range(200):
                ...
                mi_perm.append(mi_bits(xcat, yp))
            mi_perm = np.array(mi_perm, dtype=float)
            p_perm = float(np.mean(mi_perm >= mi_real))
        else:
            p_perm = float("nan")
        return {
            "pos_rate": pos_rate,
    ...
    # targets definitions
    # primary median
    y_primary = (z[test_valid + h] > med_thr).astype(int) if test_valid.size else np.array([],dtype=int)
    # desticky sign
    y_sign = ((z[test_valid + h] - z[test_valid]) > 0).astype(int) if test_valid.size else np.array([],dtype=int)
    # extreme q90
    y_q90 = (z[test_valid + h] > q90_thr).astype(int) if test_valid.size else np.array([],dtype=int)
    # increment q75
    y_inc = ((z[test_valid + h] - z[test_valid]) > q75_inc).astype(int) if test_valid.size else np.array([],dtype=int)

    for name, yarr in [
        ("T_primary_median", y_primary),
        ("T_desticky_sign", y_sign),
        ("T_extreme_q90", y_q90),
        ("T_increment_q75", y_inc),
    ]:
        targets.setdefault(name, {})[str(h)] = compute_metrics(yarr)
...
out = {
    ...
    "real_test": {
        "per_target": targets,
        "rate_bin_real": rate_bin
    }
}

# Ensure JSON serializable with nan as null
def clean(obj):
...
clean_out = clean(out)
print(json.dumps(clean_out, separators=(",",":")))

### Tool (2026-02-22 00:54:16)
