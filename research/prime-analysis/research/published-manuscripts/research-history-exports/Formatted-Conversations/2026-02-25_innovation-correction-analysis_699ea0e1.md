# Innovation Correction Analysis

- **Conversation ID:** `699ea0e1-58a8-8330-b1f2-c5d9c5562803`
- **Date:** 2026-02-25

---

### User (2026-02-25 02:12:38)
...
x_kept=cache["x_kept"]
dt=float(cache["dt"]) if "dt" in cache else None
N_kept=int(cache["N_kept"]) if "N_kept" in cache.files else len(x_kept)
train_end=int(cache["train_end"])
N_kept, train_end, dt, x_kept[:3], x_kept[-3:]

### Tool (2026-02-25 02:12:56)
...
### Assistant (2026-02-25 02:12:59)

psi=np.load(psi_path)
psi.files, psi["x_grid"][:5], psi["x_grid"][-5:], psi["psi_minus_x"][:5]

### Tool (2026-02-25 02:13:01)
...
def causal_moving_average(x, W):
    ...
    for i in range(n):
        start=max(0, i-W+1)
        s=c[i] - (c[start-1] if start>0 else 0.0)
        out[i]=s/(i-start+1)
    return out

def wrappi(a):
    twopi=2*np.pi
    y=((a+np.pi) % twopi) - np.pi  # in [-pi, pi)
    # map -pi to +pi to get (-pi,pi]
    y=np.where(np.isclose(y, -np.pi), np.pi, y)
    return y

# 1) recompute theta_mod and unwrap per spec
x_grid=psi["x_grid"].astype(float)
u_native=psi["psi_minus_x"].astype(float)

u_interp=np.interp(x_kept, x_grid, u_native)  # linear interp
...
theta_mod=np.angle(analytic) % (2*np.pi)
theta_u=np.unwrap(theta_mod)

# 2) valid index sets
h=50
ell=25
train_valid = np.arange(ell, train_end - h, dtype=int)  # i+h < train_end
test_valid = np.arange(train_end, N_kept - h, dtype=int)

len(train_valid), len(test_valid), train_valid[:3], train_valid[-3:], test_valid[:3], test_valid[-3:]

### Tool (2026-02-25 02:13:14)

(25539,
 10928,
 array([25, 26, 27]),
 array([25561, 25562, 25563]),
 array([25614, 25615, 25616]),
 array([36539, 36540, 36541]))

### Assistant (2026-02-25 02:13:16)

twopi=2*np.pi

# 3) TRAIN transport
D = theta_u[train_valid + h] - theta_u[train_valid]
m = np.mean(np.exp(1j*D))
omega = np.angle(m)/h
R_transport_train = np.abs(np.mean(np.exp(1j*(D - h*omega))))

# 4) eps, delta on TRAIN
delta = wrappi(theta_u[train_valid] - theta_u[train_valid - ell])
eps = wrappi(theta_u[train_valid + h] - theta_u[train_valid] - h*omega)

R_before_train = np.abs(np.mean(np.exp(1j*eps)))

def objective_gamma(g, eps_arr, delta_arr):
    return np.abs(np.mean(np.exp(1j*(eps_arr - g*delta_arr))))

# 5) golden section maximize on [-2,2]
...
f_train=lambda g: objective_gamma(g, eps, delta)
gamma_hat, obj_hat = golden_section_max(f_train, -2.0, 2.0)

R_after_train = objective_gamma(gamma_hat, eps, delta)

# 7-8) TEST concentrations with TRAIN omega and gamma
delta_t = wrappi(theta_u[test_valid] - theta_u[test_valid - ell])
eps_t = wrappi(theta_u[test_valid + h] - theta_u[test_valid] - h*omega)

R_before_test = np.abs(np.mean(np.exp(1j*eps_t)))
R_after_test = objective_gamma(gamma_hat, eps_t, delta_t)
delta_R_test = R_after_test - R_before_test

# 9) optional gamma on TEST
f_test=lambda g: objective_gamma(g, eps_t, delta_t)
gamma_test_star, obj_test_star = golden_section_max(f_test, -2.0, 2.0)
R_after_test_star = obj_test_star

omega, gamma_hat, R_transport_train, R_before_train, R_after_train, R_before_test, R_after_test, gamma_test_star, R_after_test_star
...
out={
  ...
  "concentration":{
    "R_transport_train": float(R_transport_train),
    "R_before_train": float(R_before_train),
    "R_after_train": float(R_after_train),
    "R_before_test": float(R_before_test),
    "R_after_test": float(R_after_test),
    "delta_R_test": float(delta_R_test)
  },
  "diagnostic_optional":{
    "gamma_test_star": float(gamma_test_star),
    "R_after_test_star": float(R_after_test_star)
  },
  "notes": "theta_new recomputed per closed spec on x_kept; causal detrend W=2000; TRAIN-only standardization (train_end=25614); rFFT bandpass [0.003,0.009] cycles/sample with d=1.0; Hilbert phase unwrapped. omega estimated from TRAIN via arg(mean exp(i*D))/h. Note: R_transport_train equals R_before_train because exp(i*wrappi(x))=exp(i*x)."
}
print(json.dumps(out, indent=2))

### Tool (2026-02-25 02:13:36)
