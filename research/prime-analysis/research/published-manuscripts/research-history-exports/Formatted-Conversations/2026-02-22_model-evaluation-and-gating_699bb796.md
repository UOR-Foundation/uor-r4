...
- **Conversation ID:** `699bb796-d268-8328-a96d-6f5965a0b53e`
- **Date:** 2026-02-22

---

### User (2026-02-22 21:12:42)
...
### Assistant (2026-02-22 21:12:44)

import numpy as np, hashlib, json, math, os, sklearn
...
### Tool (2026-02-22 21:12:52)

(['x_kept', 'u_bp', 'theta_mod', 'R', 'dt', 'train_end', 'params', 'hashes'],
 8)

### Assistant (2026-02-22 21:12:55)
...
hashes=data["hashes"].item() if data["hashes"].dtype==object else data["hashes"]
type(hashes), hashes

### Tool (2026-02-22 21:12:59)
...
### Assistant (2026-02-22 21:13:01)

hashes=json.loads(hashes)
def sha1_arr(a:np.ndarray):
    a=np.ascontiguousarray(a)
    return hashlib.sha1(a.view(np.uint8)).hexdigest()
sha_theta=sha1_arr(theta.astype(np.float64))
sha_R=sha1_arr(R.astype(np.float64))
sha_x=sha1_arr(x_kept.astype(np.float64))
sha_theta, sha_R, sha_x, hashes

### Tool (2026-02-22 21:13:06)
...
def wrap_to_pi(x):
    ...
    # adjust -pi to +pi
    y = np.where(y <= -np.pi + 1e-15, y + 2*np.pi, y)
    return y

h=25
N=len(R)
i_valid=np.arange(2, N-h, dtype=np.int64)
# split masks
train_mask=i_valid < train_end
test_mask=~train_mask
i_train=i_valid[train_mask]
i_test=i_valid[test_mask]
...
# transport
T = wrap_to_pi(theta[i_valid + h] - theta[i_valid])/(2*np.pi)  # (-0.5,0.5]
T_train=T[train_mask]; T_test=T[test_mask]

# base features
def build_base(idx):
    return np.vstack([R[idx], R[idx-1], R[idx-2]]).T

X0_train=build_base(i_train)
X0_test=build_base(i_test)

# gate threshold q from TRAIN valid rows
absR_train=np.abs(R[i_train])
q=float(np.quantile(absR_train, 0.20))
G_train=(absR_train <= q).astype(float)
G_test=(np.abs(R[i_test]) <= q).astype(float)

# model builder with standardization on TRAIN
def fit_eval(Xtr, ytr, Xte, yte):
    ...
    Xtr_z=(Xtr-mu)/sigma
    Xte_z=(Xte-mu)/sigma
    clf=LogisticRegression(penalty="l2", C=1.0, solver="liblinear", max_iter=200, fit_intercept=True)
    clf.fit(Xtr_z, ytr)
    p=clf.predict_proba(Xte_z)[:,1]
    auc=roc_auc_score(yte, p) if len(np.unique(yte))==2 else float("nan")
    ll=log_loss(yte, p, labels=[0,1])
    return clf, p, auc, ll, mu, sigma

# M0
clf0, p0, auc0, ll0, mu0, sig0 = fit_eval(X0_train, y_train, X0_test, y_test)

# M1: add T
X1_train=np.column_stack([X0_train, T_train])
X1_test=np.column_stack([X0_test, T_test])
clf1, p1, auc1, ll1, *_ = fit_eval(X1_train, y_train, X1_test, y_test)

# M2: gated T, features BASE + [G, G*T]
X2_train=np.column_stack([X0_train, G_train, G_train*T_train])
X2_test=np.column_stack([X0_test, G_test, G_test*T_test])
clf2, p2, auc2, ll2, *_ = fit_eval(X2_train, y_train, X2_test, y_test)

auc0, auc1, auc2, ll0, ll1, ll2, q, len(i_train), len(i_test), y_test.mean()

### Tool (2026-02-22 21:13:33)
...
### Assistant (2026-02-22 21:13:36)

def subset_metrics(mask, p0, p1, p2):
    yt=y_test[mask]
    res={}
    res["n"]=int(mask.sum())
    res["pos_rate"]=float(yt.mean()) if res["n"]>0 else float("nan")
    def auc_safe(p):
        return float(roc_auc_score(yt, p[mask])) if res["n"]>0 and len(np.unique(yt))==2 else float("nan")
    a0=auc_safe(p0); a1=auc_safe(p1); a2=auc_safe(p2)
    res["M0_auc"]=a0
    res["M1_delta"]=float(a1-a0) if np.isfinite(a1) and np.isfinite(a0) else float("nan")
    res["M2_delta"]=float(a2-a0) if np.isfinite(a2) and np.isfinite(a0) else float("nan")
    return res, a0, a1, a2

mask_low = G_test==1.0
mask_high = G_test==0.0
low_res, low_a0, low_a1, low_a2 = subset_metrics(mask_low, p0,p1,p2)
high_res, high_a0, high_a1, high_a2 = subset_metrics(mask_high, p0,p1,p2)
low_res, high_res, mask_low.sum(), mask_high.sum()

### Tool (2026-02-22 21:13:42)
...
for _ in range(n_perm):
    s=int(rng.integers(1, len(T_train)))  # [1,n_train-1] inclusive? high excluded so ok
    T_shift=np.roll(T_train, s)
    X2p_train=np.column_stack([X0_train, G_train, G_train*T_shift])
    # fit on train, eval on test with true features
    _, _, aucp, _, *_ = fit_eval(X2p_train, y_train, X2_test, y_test)
    deltas.append(aucp-auc0)
deltas=np.array(deltas, dtype=float)
perm_mean=float(np.mean(deltas))
perm_std=float(np.std(deltas, ddof=0))
p_emp=float((np.sum(deltas >= delta_obs) + 1)/ (n_perm+1))
delta_obs, perm_mean, perm_std, p_emp, deltas[:5]

### Tool (2026-02-22 21:13:50)

(-0.0003392337238998788,
 -0.00036951226766366446,
 0.000881678807549033,
 0.6049382716049383,
 array([ 5.66834412e-07, -7.82298175e-04,  3.58472751e-04,  7.01741002e-04,
         2.24399741e-05]))

### Assistant (2026-02-22 21:14:07)

out={
 "status":"SUCCESS",
 "file_used":os.path.basename(path),
 "hashes_sha1":{"theta":sha_theta,"R":sha_R},
 "shape":{"N_kept":int(N),"train_end":int(train_end),"h":25,"q_gate":q},
 "overall_test":{
   "M0":{"auc":float(auc0),"logloss":float(ll0)},
   "M1":{"auc":float(auc1),"logloss":float(ll1),
         "delta_auc_vs_M0":float(auc1-auc0),
         "delta_logloss_vs_M0":float(ll1-ll0)},
   "M2":{"auc":float(auc2),"logloss":float(ll2),
         "delta_auc_vs_M0":float(auc2-auc0),
         "delta_logloss_vs_M0":float(ll2-ll0)},
 },
 "subsets_test":{
   "low":low_res,
   "high":high_res
