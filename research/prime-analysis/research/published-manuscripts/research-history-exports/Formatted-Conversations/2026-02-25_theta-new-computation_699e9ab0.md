# Theta_new Computation

- **Conversation ID:** `699e9ab0-eb98-832e-b2c1-f7e631a01979`
- **Date:** 2026-02-25

---

### User (2026-02-25 01:46:12)
...
### Assistant (2026-02-25 01:46:15)

import numpy as np, hashlib, json, math
from pathlib import Path
import scipy.signal

cheb_path = Path("/mnt/data/chebyshev_psi.npz")
canon_path = Path("/mnt/data/A4_CANONICAL_CACHE_L131072_x1e5_9e6_W2000_bp002_001_D001_seed12345.npz")

cheb = np.load(cheb_path, allow_pickle=False)
canon = np.load(canon_path, allow_pickle=False)

cheb.files, canon.files[:10], len(canon.files)

### Tool (2026-02-25 01:46:24)

(['N', 'step', 'x_grid', 'psi_grid', 'psi_minus_x'],
 ['x_kept', 'u_bp', 'theta_mod', 'R', 'dt', 'train_end', 'params', 'hashes'],
 8)

### Assistant (2026-02-25 01:46:26)

def sha1_bytes(b: bytes)->str:
    return hashlib.sha1(b).hexdigest()

def float64_hash(arr)->str:
    a = np.asarray(arr, dtype=np.float64, order='C')
    return sha1_bytes(a.tobytes(order='C'))

def uint8_hash(arr)->str:
    a = np.asarray(arr, dtype=np.uint8, order='C')
    return sha1_bytes(a.tobytes(order='C'))

dt = float(canon["dt"])
train_end = int(canon["train_end"])
x_kept = np.asarray(canon["x_kept"], dtype=np.float64)
N_kept = int(x_kept.shape[0])
x_kept_sha1 = float64_hash(x_kept)
dt, train_end, N_kept, x_kept_sha1, canon["hashes"].item() if canon["hashes"].dtype==object else canon["hashes"]

### Tool (2026-02-25 01:46:31)

---------------------------------------------------------------------------
ValueError                                Traceback (most recent call last)
Cell In[2], line 17
     15 N_kept = int(x_kept.shape[0])
     16 x_kept_sha1 = float64_hash(x_kept)
---> 17 dt, train_end, N_kept, x_kept_sha1, canon["hashes"].item() if canon["hashes"].dtype==object else canon["hashes"]
...
N_kept = int(x_kept.shape[0])
x_kept_sha1 = float64_hash(x_kept)
dt, train_end, N_kept, x_kept_sha1, list(canon["params"].item().keys())[:5], canon["hashes"].item().keys()

### Tool (2026-02-25 01:46:35)

---------------------------------------------------------------------------
AttributeError                            Traceback (most recent call last)
Cell In[3], line 7
      5 N_kept = int(x_kept.shape[0])
      6 x_kept_sha1 = float64_hash(x_kept)
----> 7 dt, train_end, N_kept, x_kept_sha1, list(canon["params"].item().keys())[:5], canon["hashes"].item().keys()

AttributeError: 'str' object has no attribute 'keys'

### Assistant (2026-02-25 01:46:36)

canon = np.load(canon_path, allow_pickle=True)
print(type(canon["params"]), canon["params"].dtype, canon["params"].shape)
print(type(canon["hashes"]), canon["hashes"].dtype, canon["hashes"].shape)
print("params:", canon["params"])
print("hashes:", canon["hashes"])

### Tool (2026-02-25 01:46:38)

<class 'numpy.ndarray'> object (1,)
<class 'numpy.ndarray'> object (1,)
params: ['{"L": 131072, "x_range": [100000.0, 9000000.0], "Delta": 0.01, "train_frac": 0.7, "detrend_W": 2000, "bandpass_cycles_per_sample": [0.002, 0.01], "seed": 12345, "bandpass_impl": "rFFT_nonnegative_mask", "hilbert_impl": "scipy.signal.hilbert", "R_impl": "searchsorted_primes_multiplicative_window"}']
hashes: ['{"x_kept": "c455fedeff29bb2d0b2c81864970c09638720baf", "u_bp": "b16b32cb5d4e63be1c61b88fcf4711d0656da14e", "theta_mod": "8b33c57df436d37179b13b1899bf86a723f0e98a", "R": "5753ee7d59c7d155c4a1b9b5fba8912d557598f2"}']

### Assistant (2026-02-25 01:46:39)

import json as _json
canon_hashes = _json.loads(canon["hashes"][0])
x_kept_sha1, canon_hashes["x_kept"], dt, train_end, N_kept

### Tool (2026-02-25 01:46:42)
...
### Assistant (2026-02-25 01:46:43)

cheb = np.load(cheb_path, allow_pickle=False)
x_grid = np.asarray(cheb["x_grid"], dtype=np.float64)
psi_minus_x = np.asarray(cheb["psi_minus_x"], dtype=np.float64)

# u_raw on native grid is psi_minus_x already.
u_interp = np.interp(x_kept, x_grid, psi_minus_x).astype(np.float64, copy=False)

W = 2000
# causal moving average with variable window at start
c = np.cumsum(u_interp, dtype=np.float64)
ma = np.empty_like(u_interp, dtype=np.float64)

# for i < W: mean of 0..i inclusive
idx = np.arange(N_kept, dtype=np.int64)
start = idx - (W - 1)
start_clipped = np.maximum(start, 0)
# sum over [start_clipped, i] inclusive
sum_window = c - np.where(start_clipped>0, c[start_clipped-1], 0.0)
den = (idx - start_clipped + 1).astype(np.float64)
ma = sum_window / den

u_detrended = (u_interp - ma).astype(np.float64, copy=False)

mu_train = float(u_detrended[:train_end].mean(dtype=np.float64))
sd_train = float(u_detrended[:train_end].std(dtype=np.float64, ddof=0))
z = ((u_detrended - mu_train) / sd_train).astype(np.float64, copy=False)

Z = np.fft.rfft(z, n=N_kept)
freqs = np.fft.rfftfreq(N_kept, d=1.0).astype(np.float64, copy=False)

bp_lo, bp_hi = 0.003, 0.009
bp_mask_bool = (freqs >= bp_lo) & (freqs <= bp_hi)
bp_mask = bp_mask_bool.astype(np.uint8)

Z_masked = Z * bp_mask  # uint8 will broadcast; ok
u_bp = np.fft.irfft(Z_masked, n=N_kept).astype(np.float64, copy=False)

analytic = scipy.signal.hilbert(u_bp.astype(np.float64, copy=False))
theta = np.angle(analytic).astype(np.float64, copy=False)
two_pi = 2.0*np.pi
theta_mod = np.mod(theta, two_pi).astype(np.float64, copy=False)

# hashes
sha = {
    "u_interp": float64_hash(u_interp),
    "ma": float64_hash(ma),
    "u_detrended": float64_hash(u_detrended),
    "z": float64_hash(z),
    "freqs": float64_hash(freqs),
    "bp_mask": uint8_hash(bp_mask),
    "u_bp": float64_hash(u_bp),
    "analytic_real": float64_hash(np.real(analytic)),
    "analytic_imag": float64_hash(np.imag(analytic)),
    "theta_mod": float64_hash(theta_mod),
}
bandpass_info = {
    "bp_lo": bp_lo,
    "bp_hi": bp_hi,
    "n_freqs": int(freqs.shape[0]),
    "n_masked_in": int(bp_mask_bool.sum())
}
identity = {
    "status": "PASS",
    "dt": dt,
    "N_kept": N_kept,
    "train_end": train_end,
    "x_kept_sha1": x_kept_sha1
}
scalars = {"mu_train": mu_train, "sd_train": sd_train}
# sanity: ensure float64 everywhere
all_float64 = all(arr.dtype==np.float64 for arr in [u_interp, ma, u_detrended, z, freqs, u_bp, np.real(analytic), np.imag(analytic), theta_mod])
all_float64

### Tool (2026-02-25 01:47:02)

True

### Assistant (2026-02-25 01:47:03)

out = {
    "identity": identity,
    "scalars": scalars,
    "bandpass": bandpass_info,
    "sha1": sha,
