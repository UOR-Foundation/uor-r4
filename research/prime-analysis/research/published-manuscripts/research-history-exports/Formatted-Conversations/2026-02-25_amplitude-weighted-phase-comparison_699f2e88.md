# Amplitude-Weighted Phase Comparison

- **Conversation ID:** `699f2e88-5204-8329-b7f3-d3ef03435906`
- **Date:** 2026-02-25

---

### User (2026-02-25 12:16:58)
...
fc.files[:20], canon.files[:20], psi.files[:10]

### Tool (2026-02-25 12:17:29)
...
### Assistant (2026-02-25 12:17:32)

x_kept=canon["x_kept"]
N_kept=len(x_kept)
train_end=int(canon["train_end"])
assert N_kept==int(canon["params"].item().get("N_kept",N_kept)) if isinstance(canon["params"].item(), dict) else True
x_grid=psi["x_grid"]
psi_minus_x=psi["psi_minus_x"]

u_interp=np.interp(x_kept, x_grid, psi_minus_x)

W=2000
# causal moving average including current sample
# use cumulative sum
cs=np.cumsum(u_interp, dtype=float)
ma=np.empty_like(u_interp, dtype=float)
for i in range(N_kept):
    j0=max(0, i-W+1)
    count=i-j0+1
    ma[i]=(cs[i] - (cs[j0-1] if j0>0 else 0.0))/count
u_det=u_interp - ma
...
# rfft bandpass with d=1.0 (as per spec)
freqs=rfftfreq(N_kept, d=1.0)
Z=rfft(z)
mask=(freqs>=0.003) & (freqs<=0.009)
Z_bp=Z*mask
u_bp=irfft(Z_bp, n=N_kept)

analytic=signal.hilbert(u_bp)
theta_mod=np.mod(np.angle(analytic), 2*np.pi)
logA=np.log(np.abs(analytic)+1e-12)

theta_mod[:5], logA[:5], u_bp.shape

### Tool (2026-02-25 12:17:42)

(array([1.32477116, 1.35814957, 1.3914283 , 1.42460533, 1.45767855]),
 array([0.13901057, 0.13155029, 0.12393461, 0.11616258, 0.1082332 ]),
 (36592,))

### Assistant (2026-02-25 12:17:45)

R=fc["R"]
# compute y for all i where i+h < N_kept
h=50
dR = R[h:] - R[:-h]
y = (dR>0).astype(int)  # length N_kept-h
# indices i correspond to y_i for i=0..N_kept-h-1
...
n_train, n_test, train_end, N_kept

### Tool (2026-02-25 12:17:51)

(25485, 10928, 25614, 36592)

### Assistant (2026-02-25 12:17:54)

X80=fc["X80"]
mu_X80=fc["mu_X80_train"]
sd_X80=fc["sd_X80_train"]
X80s=(X80 - mu_X80)/sd_X80

# plain phase features using recomputed theta_mod at same indices i
S=np.column_stack([np.sin(theta_mod), np.cos(theta_mod)])
# standardize S using train_valid only
S_mu=S[train_valid].mean(axis=0)
S_sd=S[train_valid].std(axis=0, ddof=0)
S_std=(S - S_mu)/S_sd

# weights from logA train_valid
logA_train=logA[train_valid]
m=np.median(logA_train)
mad=np.median(np.abs(logA_train - m))
sigma=1.4826*mad
sigma=max(sigma,1e-6)
w_raw=(logA - m)/(2*sigma)
w_clip=np.clip(w_raw, -1, 1)
w=0.5*(w_clip+1.0)

Sw=np.column_stack([w*np.sin(theta_mod), w*np.cos(theta_mod)])
Sw_mu=Sw[train_valid].mean(axis=0)
Sw_sd=Sw[train_valid].std(axis=0, ddof=0)
Sw_std=(Sw - Sw_mu)/Sw_sd

# train/test datasets
X0_train=X80s[train_valid]
X0_test=X80s[test_valid]
...
auc0, p0=fit_predict_auc(X0_train,y_train,X0_test,y_test)

X1_train=np.hstack([X80s[train_valid], S_std[train_valid]])
X1_test=np.hstack([X80s[test_valid], S_std[test_valid]])
auc1, p1=fit_predict_auc(X1_train,y_train,X1_test,y_test)

X2_train=np.hstack([X80s[train_valid], Sw_std[train_valid]])
X2_test=np.hstack([X80s[test_valid], Sw_std[test_valid]])
auc2, p2=fit_predict_auc(X2_train,y_train,X2_test,y_test)

auc0, auc1, auc2, auc2-auc1

### Tool (2026-02-25 12:18:07)

---------------------------------------------------------------------------
IndexError                                Traceback (most recent call last)
Cell In[4], line 9
      7 S=np.column_stack([np.sin(theta_mod), np.cos(theta_mod)])
      8 # standardize S using train_valid only
----> 9 S_mu=S[train_valid].mean(axis=0)
     10 S_sd=S[train_valid].std(axis=0, ddof=0)
     11 S_std=(S - S_mu)/S_sd
...
### Assistant (2026-02-25 12:18:09)

# Rebuild aligned arrays length N_kept-h
theta_i=theta_mod[i_all]
logA_i=logA[i_all]

S_i=np.column_stack([np.sin(theta_i), np.cos(theta_i)])
S_mu=S_i[train_valid].mean(axis=0)
S_sd=S_i[train_valid].std(axis=0, ddof=0)
S_std=(S_i - S_mu)/S_sd

logA_train=logA_i[train_valid]
m=np.median(logA_train)
mad=np.median(np.abs(logA_train - m))
sigma=max(1.4826*mad, 1e-6)
w_raw=(logA_i - m)/(2*sigma)
w=0.5*(np.clip(w_raw, -1, 1)+1.0)

Sw_i=np.column_stack([w*np.sin(theta_i), w*np.cos(theta_i)])
Sw_mu=Sw_i[train_valid].mean(axis=0)
Sw_sd=Sw_i[train_valid].std(axis=0, ddof=0)
Sw_std=(Sw_i - Sw_mu)/Sw_sd

X80s=(X80 - mu_X80)/sd_X80
X0_train=X80s[i_all][train_valid]
X0_test=X80s[i_all][test_valid]
y_train=y[train_valid]; y_test=y[test_valid]

from sklearn.metrics import roc_auc_score
from sklearn.linear_model import LogisticRegression

def auc_model(Xtr,ytr,Xte,yte):
    clf=LogisticRegression(max_iter=2000, solver="lbfgs")
    clf.fit(Xtr,ytr)
    p=clf.predict_proba(Xte)[:,1]
    return float(roc_auc_score(yte,p)), p

auc0,p0=auc_model(X0_train,y_train,X0_test,y_test)
X1_train=np.hstack([X0_train, S_std[train_valid]])
X1_test=np.hstack([X0_test, S_std[test_valid]])
auc1,p1=auc_model(X1_train,y_train,X1_test,y_test)
X2_train=np.hstack([X0_train, Sw_std[train_valid]])
X2_test=np.hstack([X0_test, Sw_std[test_valid]])
auc2,p2=auc_model(X2_train,y_train,X2_test,y_test)

auc0,auc1,auc2,auc2-auc1

### Tool (2026-02-25 12:18:19)
...
### Assistant (2026-02-25 12:18:41)

logA_test=logA_i[test_valid]
med_test=float(np.median(logA_test))
low_mask = logA_test <= med_test
high_mask = ~low_mask
n_low=int(low_mask.sum()); n_high=int(high_mask.sum())

def auc_from_preds(yte, p, mask):
    return float(roc_auc_score(yte[mask], p[mask]))

auc0_low=auc_from_preds(y_test,p0,low_mask)
auc1_low=auc_from_preds(y_test,p1,low_mask)
auc2_low=auc_from_preds(y_test,p2,low_mask)

auc0_high=auc_from_preds(y_test,p0,high_mask)
auc1_high=auc_from_preds(y_test,p1,high_mask)
auc2_high=auc_from_preds(y_test,p2,high_mask)

( n_low,n_high, med_test, auc2_low-auc1_low, auc2_high-auc1_high )

### Tool (2026-02-25 12:18:46)

(5464,
 5464,
 -0.008970108204465492,
 0.0050948745361185654,
 0.012066627158038079)

### Assistant (2026-02-25 12:18:48)

out={
  "counts":{"n_train":n_train,"n_test":n_test},
  "weight_stats_train":{
    "median_logA_train":float(m),
    "mad_logA_train":float(mad),
    "sigma_robust":float(sigma)
  },
  "overall_test":{
    "auc_M0":float(auc0),
    "auc_M1_plain":float(auc1),
    "auc_M2_weighted":float(auc2),
    "delta_weighted_minus_plain":float(auc2-auc1)
  },
  "strata_test":{
    "median_logA_test":med_test,
    "low":{
      "n":n_low,
      "auc_M0":float(auc0_low),
      "auc_M1_plain":float(auc1_low),
      "auc_M2_weighted":float(auc2_low),
      "delta_weighted_minus_plain":float(auc2_low-auc1_low)
    },
    "high":{
      "n":n_high,
      "auc_M0":float(auc0_high),
      "auc_M1_plain":float(auc1_high),
      "auc_M2_weighted":float(auc2_high),
      "delta_weighted_minus_plain":float(auc2_high-auc1_high)
    }
  },
