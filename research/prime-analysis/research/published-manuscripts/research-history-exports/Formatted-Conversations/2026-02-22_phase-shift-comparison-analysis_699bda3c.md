# Phase-shift comparison analysis

- **Conversation ID:** `699bda3c-bfdc-8333-ab37-723782802a27`
- **Date:** 2026-02-22

---

### User (2026-02-22 23:40:31)
...
### Assistant (2026-02-22 23:40:34)

import numpy as np, math, statistics
e=np.array([-0.028221906130996476,0.0025751087232632345,-0.011154086610841637,0.07383129761888059,0.20459812697496876,0.12335354214393685,0.05261890314031656,0.19277574144812107,-0.057184937141269425,-0.14128765777228558,-0.12158205808314303,-0.17394598006943915],dtype=float)
l=np.array([-0.09011408416208197,0.002469029347660561,0.052439948905376316,0.044701702520726104,0.10992063747356307,0.20337862925691685,-0.020290818960534078,-0.07541249753113623,-0.08979076938342652,-0.06109763508414045,-0.03873969169546709,-0.17219625628519947],dtype=float)
B=12
L2=[]
corr=[]
for k in range(B):
    l_shift=np.roll(l,-k)  # Wait: definition l_shift[k][j]=l[(j+k) mod 12]. That equals roll left by k? let's verify.
    # For j index, take l[j+k]; that's l shifted left by k (elements move earlier). np.roll(l,-k) gives l[(j+k)%n]. good.
    diff=e-l_shift
    L2.append(float(np.sqrt(np.sum(diff*diff))))
    # corrcoef
    c=np.corrcoef(e,l_shift)[0,1]
    corr.append(float(c))
L2,corr

### Tool (2026-02-22 23:40:45)
...
### Assistant (2026-02-22 23:40:47)

L2_arr=np.array(L2)
sorted_idx=np.argsort(L2_arr)
rank0=int(np.where(sorted_idx==0)[0][0]+1)
sorted_idx, L2_arr[sorted_idx], rank0

### Tool (2026-02-22 23:40:54)

(array([10,  0,  1, 11,  9,  2,  3,  8,  7,  4,  5,  6]),
 array([0.30686021, 0.34013451, 0.38058217, 0.38569356, 0.44739277,
        0.49794159, 0.58700687, 0.59934769, 0.66209699, 0.66621423,
        0.67957301, 0.69011873]),
 2)

### Assistant (2026-02-22 23:40:56)
