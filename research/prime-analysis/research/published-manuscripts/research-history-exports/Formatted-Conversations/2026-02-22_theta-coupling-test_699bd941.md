...
- **Conversation ID:** `699bd941-1420-8327-8e9a-5438c931e15d`
- **Date:** 2026-02-22

---

### User (2026-02-22 23:36:21)
...
### Tool (2026-02-22 23:36:38)

(True, 1422464)

### Assistant (2026-02-22 23:36:40)
...
### Tool (2026-02-22 23:36:42)

'340f3ce5b561fe00e872ef98991390185f600f96'

### Assistant (2026-02-22 23:36:44)

data=np.load(path, allow_pickle=True)
data.files[:50], len(data.files)

### Tool (2026-02-22 23:36:47)
...
### Assistant (2026-02-22 23:36:49)

theta=data['theta']; R=data['R']
dt=float(data['dt']); N_kept=int(data['N_kept']); train_end=int(data['train_end'])
theta.shape, R.shape, dt, N_kept, train_end

### Tool (2026-02-22 23:36:50)

((36592,), (36592,), 0.00012297217348543395, 36592, 25614)

### Assistant (2026-02-22 23:36:53)
...
### Tool (2026-02-22 23:36:56)

('8b33c57df436d37179b13b1899bf86a723f0e98a',
 '5753ee7d59c7d155c4a1b9b5fba8912d557598f2')

### Assistant (2026-02-22 23:36:58)

X80=data['X80']; S=data['S']
X80.shape, S.shape

### Tool (2026-02-22 23:37:04)

((36592, 80), (36592, 2))

### Assistant (2026-02-22 23:37:08)

mask_train=data['mask_train_lag79'].astype(bool)
mask_test=data['mask_test_lag79'].astype(bool)
mask_train.shape, mask_test.shape, mask_train.sum(), mask_test.sum()

### Tool (2026-02-22 23:37:12)

((36592,), (36592,), 25510, 10953)

### Assistant (2026-02-22 23:37:15)

h=25
(mask_train[-h:].any(), mask_test[-h:].any(), mask_train[:79].any(), mask_test[:79].any())

### Tool (2026-02-22 23:37:21)

(False, False, False, False)

### Assistant (2026-02-22 23:37:24)

y=np.zeros(N_kept, dtype=np.int8)
y[:N_kept-h]= (R[h:]-R[:-h] > 0).astype(np.int8)
# For last h, set to -1 invalid
y[N_kept-h:]= -1
y_train=y[mask_train]
y_test=y[mask_test]
(y_train.min(), y_train.max(), (y_train==-1).sum(), (y_test==-1).sum(), y_test.mean())

### Tool (2026-02-22 23:37:29)

(0, 1, 0, 0, 0.4972153747831644)

### Assistant (2026-02-22 23:37:31)

from sklearn.linear_model import LogisticRegression
from sklearn.metrics import roc_auc_score
mu_X80=data['mu_X80_train']; sd_X80=data['sd_X80_train']
mu_S=data['mu_S_train']; sd_S=data['sd_S_train']
# standardize
X80z=(X80 - mu_X80)/sd_X80
Sz=(S - mu_S)/sd_S
Xb_train=X80z[mask_train]
Xb_test=X80z[mask_test]
Xs_train=np.hstack([Xb_train, Sz[mask_train]])
Xs_test=np.hstack([Xb_test, Sz[mask_test]])
yt_train=y_train
yt_test=y_test
# fit models
base=LogisticRegression(max_iter=2000, solver='lbfgs')
full=LogisticRegression(max_iter=2000, solver='lbfgs')
base.fit(Xb_train, yt_train)
full.fit(Xs_train, yt_train)
pb=base.predict_proba(Xb_test)[:,1]
pf=full.predict_proba(Xs_test)[:,1]
auc_base=roc_auc_score(yt_test, pb)
auc_full=roc_auc_score(yt_test, pf)
auc_base, auc_full, auc_full-auc_base

### Tool (2026-02-22 23:37:46)

(0.7849765673988456, 0.8052693060299645, 0.020292738631118912)

### Assistant (2026-02-22 23:38:05)

test_idx=np.flatnonzero(mask_test)
n_test=len(test_idx)
mid=n_test//2
early_idx=test_idx[:mid]
late_idx=test_idx[mid:]
# compute auc using corresponding positions in pb/pf arrays which aligned to mask_test order
# map mask_test order positions
# pb corresponds to mask_test True rows in order; early positions 0:mid etc
pb_early=pb[:mid]; pf_early=pf[:mid]; y_early=yt_test[:mid]
pb_late=pb[mid:]; pf_late=pf[mid:]; y_late=yt_test[mid:]
auc_be=roc_auc_score(y_early, pb_early)
auc_fe=roc_auc_score(y_early, pf_early)
auc_bl=roc_auc_score(y_late, pb_late)
auc_fl=roc_auc_score(y_late, pf_late)
(auc_be, auc_fe, auc_fe-auc_be, auc_bl, auc_fl, auc_fl-auc_bl, (auc_fl-auc_bl)-(auc_fe-auc_be),
 y_early.mean(), y_late.mean(), len(y_early), len(y_late))

### Tool (2026-02-22 23:38:10)
...
### Assistant (2026-02-22 23:38:13)

B=12
two_pi=2*math.pi
theta_test=theta[mask_test]
# bins
bins=np.floor(theta_test/two_pi*B).astype(int)
bins=np.clip(bins,0,B-1)
r=yt_test - pb
def curve_for(mask_slice):
    # mask_slice boolean over test rows in order
    out=[]
    for b in range(B):
        sel=(bins==b) & mask_slice
        cnt=int(sel.sum())
        mean=float(r[sel].mean()) if cnt>0 else float('nan')
        out.append({"bin":b,"count":cnt,"mean_residual":mean})
    return out
mask_early=np.zeros(n_test,dtype=bool); mask_early[:mid]=True
mask_late=~mask_early
curve_early=curve_for(mask_early)
curve_late=curve_for(mask_late)
curve_early, curve_late[:2]

### Tool (2026-02-22 23:38:22)
...
### Assistant (2026-02-22 23:38:25)

# compute corr and L2 between mean_residual arrays
import numpy as np
mE=np.array([d["mean_residual"] for d in curve_early], dtype=float)
mL=np.array([d["mean_residual"] for d in curve_late], dtype=float)
# pearson
corr=float(np.corrcoef(mE, mL)[0,1])
L2=float(np.linalg.norm(mE-mL))
corr, L2

### Tool (2026-02-22 23:38:28)

(0.6241829543382801, 0.34013451389765376)

### Assistant (2026-02-22 23:38:30)

curve_late

### Tool (2026-02-22 23:38:33)
