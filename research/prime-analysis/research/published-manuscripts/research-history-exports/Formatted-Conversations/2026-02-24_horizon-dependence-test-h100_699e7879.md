# Horizon-dependence test h100

- **Conversation ID:** `699e7879-f3ac-8330-9346-8a255d70dac5`
- **Date:** 2026-02-24

---

### User (2026-02-24 23:20:14)
...
### Assistant (2026-02-24 23:20:19)

import numpy as np, hashlib, json, math, os, pandas as pd
from numpy.fft import rfft, irfft
from scipy.signal import hilbert
from sklearn.linear_model import LogisticRegression
from sklearn.metrics import roc_auc_score, log_loss

### Assistant (2026-02-24 23:20:35)

import sys, numpy as np, hashlib, math, os, json
print("python", sys.version)

### Assistant (2026-02-24 23:20:39)

1+1

### Tool (2026-02-24 23:20:41)

2

### Assistant (2026-02-24 23:20:44)

import numpy as np, hashlib, math, os, json, sys
print("ok numpy", np.__version__)

### Tool (2026-02-24 23:20:46)

ok numpy 1.24.0

### Assistant (2026-02-24 23:20:49)

import scipy, scipy.signal
print("scipy", scipy.__version__)

### Tool (2026-02-24 23:20:51)

scipy 1.14.1

### Assistant (2026-02-24 23:20:53)

import sklearn
print("sklearn", sklearn.__version__)

### Tool (2026-02-24 23:20:54)

sklearn 1.4.2

### Assistant (2026-02-24 23:20:57)

import pandas as pd
print(pd.__version__)

### Tool (2026-02-24 23:20:59)

2.2.3

### Assistant (2026-02-24 23:21:01)

import numpy as np, os, hashlib, math, json
base="/mnt/data"
paths={
"featurecache": os.path.join(base,"A4_FEATURECACHE_h25_AR80_AR40_theta_omega_masks_v1.npz"),
"cheb": os.path.join(base,"chebyshev_psi.npz"),
"canonical": os.path.join(base,"A4_CANONICAL_CACHE_L131072_x1e5_9e6_W2000_bp002_001_D001_seed12345.npz"),
}
{k:os.path.exists(v) for k,v in paths.items()}

### Tool (2026-02-24 23:21:04)

{'featurecache': True, 'cheb': True, 'canonical': True}

### Assistant (2026-02-24 23:21:06)

canon=np.load(paths["canonical"], allow_pickle=True)
canon.files[:20], {k:canon[k].shape if hasattr(canon[k],'shape') else type(canon[k]) for k in canon.files if k in ["x_kept","u_bp","theta_mod","R","dt","train_end","N_kept"]}

### Tool (2026-02-24 23:21:09)
...
### Assistant (2026-02-24 23:21:11)

dt=float(canon["dt"])
train_end=int(canon["train_end"])
N_kept=canon["x_kept"].shape[0]
hashes=canon["hashes"].item()
dt, train_end, N_kept, hashes

### Tool (2026-02-24 23:21:14)
...
### Assistant (2026-02-24 23:21:16)

hashes=json.loads(hashes)
hashes

### Tool (2026-02-24 23:21:17)
...
cheb=np.load(paths["cheb"], allow_pickle=True)
...
x_kept=x_uni[keep]
# compare x_kept sha1
sha_x=sha1_arr(x_kept)
sha_x, hashes["x_kept"], len(x_kept)

### Tool (2026-02-24 23:21:36)

('c455fedeff29bb2d0b2c81864970c09638720baf',
 'c455fedeff29bb2d0b2c81864970c09638720baf',
 36592)

### Assistant (2026-02-24 23:21:39)

W=2000
u=u_raw[keep]
ma=causal_moving_average(u, W)
u_det=u - ma
...
def bandpass_rfft(u_z, f_lo, f_hi):
    N=len(u_z)
    U=rfft(u_z)
    freqs=rfftfreq(N, d=1.0)  # cycles per sample
    mask=(freqs>=f_lo) & (freqs<=f_hi)
    # remove DC automatically by mask
    U_f=np.where(mask, U, 0)
    return irfft(U_f, n=N)

u_bp_new=bandpass_rfft(u_z, 0.003, 0.009)
theta_new=(np.angle(hilbert(u_bp_new))%(2*math.pi)).astype(np.float64)

sha_theta=sha1_arr(theta_new)
sha_u_bp=sha1_arr(u_bp_new.astype(np.float64))
sha_theta, sha_u_bp

### Tool (2026-02-24 23:21:44)

('c888f75abc219749cde8f870c119a6dcd98ec2b4',
 '0bc49e3072d667568223a4384b64bd96e5cae503')

### Assistant (2026-02-24 23:21:46)

fc=np.load(paths["featurecache"], allow_pickle=True)
fc.files

### Tool (2026-02-24 23:21:49)
...
### Tool (2026-02-24 23:22:10)

('5753ee7d59c7d155c4a1b9b5fba8912d557598f2',
 '5753ee7d59c7d155c4a1b9b5fba8912d557598f2')

### Assistant (2026-02-24 23:22:13)
...
train_n=train_valid.sum(); test_n=test_valid.sum()
train_n, test_n

### Tool (2026-02-24 23:22:19)

(25435, 10878)

### Assistant (2026-02-24 23:22:21)

y=(R[idx+h]-R[idx] > 0).astype(int)
y_train=y[train_valid]; y_test=y[test_valid]
y_train.mean(), y_test.mean(), y_test.min(), y_test.max()

### Tool (2026-02-24 23:22:25)

---------------------------------------------------------------------------
IndexError                                Traceback (most recent call last)
Cell In[16], line 1
----> 1 y=(R[idx+h]-R[idx] > 0).astype(int)
      2 y_train=y[train_valid]; y_test=y[test_valid]
      3 y_train.mean(), y_test.mean(), y_test.min(), y_test.max()

IndexError: index 36592 is out of bounds for axis 0 with size 36592

### Assistant (2026-02-24 23:22:26)

y=np.full(N_kept, -1, dtype=int)
valid_y = idx + h < N_kept
y[valid_y]= (R[idx[valid_y]+h]-R[idx[valid_y]] > 0).astype(int)
y_train=y[train_valid]; y_test=y[test_valid]
y_train.mean(), y_test.mean(), (y_test.min(), y_test.max())

### Tool (2026-02-24 23:22:30)

(0.4936897975230981, 0.49963228534657106, (0, 1))

### Assistant (2026-02-24 23:22:33)
...
X0_train=X80_z[train_valid]
X0_test=X80_z[test_valid]
m0=fit_logit(X0_train, y_train)
p0=m0.predict_proba(X0_test)[:,1]
auc_p0=roc_auc_score(y_test, p0)
ll_p0=log_loss(y_test, p0, labels=[0,1])
auc_p0, ll_p0

### Tool (2026-02-24 23:22:37)

(0.8749628541406202, 0.4436031692028677)

### Assistant (2026-02-24 23:22:46)

sin1=np.sin(theta_new); cos1=np.cos(theta_new)
mu_sin=sin1[train_valid].mean(); sd_sin=sin1[train_valid].std(ddof=0)
mu_cos=cos1[train_valid].mean(); sd_cos=cos1[train_valid].std(ddof=0)
sin_z=(sin1-mu_sin)/sd_sin
cos_z=(cos1-mu_cos)/sd_cos
X1_train=np.column_stack([X80_z[train_valid], sin_z[train_valid], cos_z[train_valid]])
X1_test=np.column_stack([X80_z[test_valid], sin_z[test_valid], cos_z[test_valid]])
m1=fit_logit(X1_train, y_train)
p1=m1.predict_proba(X1_test)[:,1]
auc_p1=roc_auc_score(y_test, p1)
ll_p1=log_loss(y_test, p1, labels=[0,1])
auc_p1, ll_p1, auc_p1-auc_p0, ll_p1-ll_p0

### Tool (2026-02-24 23:22:50)

(0.8890106229298504,
 0.4213263470382152,
 0.014047768789230197,
 -0.022276822164652488)

### Assistant (2026-02-24 23:22:59)

coef=m1.coef_[0]
a_sin=float(coef[-2]); b_cos=float(coef[-1])
A_amp=math.sqrt(a_sin*a_sin + b_cos*b_cos)
theta_star=math.atan2(a_sin, b_cos)  # [-pi,pi]
theta_star_mod=theta_star%(2*math.pi)
a_sin,b_cos,A_amp,theta_star,theta_star_mod

### Tool (2026-02-24 23:23:09)
...
omega_eff=wrap_to_pi(theta50-theta25)/(50-25)
theta_pred=wrap_to_pi(theta50 + omega_eff*50)
theta_pred_mod=theta_pred%(2*math.pi)
delta=wrap_to_pi(theta_star_mod - theta_pred_mod)
omega_eff, theta_pred_mod, delta

### Tool (2026-02-24 23:23:18)

(-0.020990570510348583, 0.2244980356988604, 0.013375313796679666)

### Assistant (2026-02-24 23:23:20)
