# Forensic pipeline diff

- **Conversation ID:** `699e90a1-c998-832b-8af5-0515a0f99696`
- **Date:** 2026-02-25

---

### User (2026-02-25 01:03:20)
...
N_kept_c = int(x_kept_c.shape[0])

# canonical expected
dt_expected = 0.00012297217348543395
N_kept_expected = 36592
train_end_expected = 25614
(dt_c, N_kept_c, train_end_c, dt_expected, N_kept_expected, train_end_expected)

### Tool (2026-02-25 01:03:52)

(0.00012297217348543395, 36592, 25614, 0.00012297217348543395, 36592, 25614)

### Assistant (2026-02-25 01:03:55)

list(cheb.files)[:20], {k: cheb[k].shape for k in cheb.files}

### Tool (2026-02-25 01:03:58)
...
x_raw = np.asarray(cheb["x_grid"], dtype=np.float64)
psi_minus_x_raw = np.asarray(cheb["psi_minus_x"], dtype=np.float64)
t_raw = np.log(x_raw)

t_uniform = np.linspace(t_raw.min(), t_raw.max(), L, dtype=np.float64)
x_grid_full = np.exp(t_uniform)

kept_mask = (x_grid_full >= x_min) & (x_grid_full <= x_max)
kept_idx = np.nonzero(kept_mask)[0]
x_kept = x_grid_full[kept_idx]
N_kept = x_kept.size
train_end = int(np.floor(train_frac * N_kept))

# interpolate psi_minus_x onto t_uniform
psi_interp_full = np.interp(t_uniform, t_raw, psi_minus_x_raw).astype(np.float64)
psi_interp_kept = psi_interp_full[kept_idx].astype(np.float64)

# causal moving average detrend
u_raw = psi_interp_kept
n = u_raw.size
cumsum = np.cumsum(u_raw, dtype=np.float64)
# for i, start = max(0, i-W+1); sum = cumsum[i]-cumsum[start-1]
idx = np.arange(n)
start = idx - (W-1)
start[start < 0] = 0
sum_window = cumsum - np.where(start>0, cumsum[start-1], 0.0)
len_window = idx - start + 1
ma = sum_window / len_window
u_detrended = (u_raw - ma).astype(np.float64)

train_mean = float(u_detrended[:train_end].mean())
train_std = float(u_detrended[:train_end].std(ddof=0))
u_z = ((u_detrended - train_mean)/train_std).astype(np.float64)

freqs = rfftfreq(n, d=1.0).astype(np.float64)
bp_mask = ((freqs >= bp_lo) & (freqs <= bp_hi)).astype(np.uint8)
U = rfft(u_z)
U_filt = U * bp_mask  # broadcasting ok
u_bp = irfft(U_filt, n=n).astype(np.float64)

analytic = signal.hilbert(u_bp)  # complex128
analytic_real = np.asarray(np.real(analytic), dtype=np.float64)
analytic_imag = np.asarray(np.imag(analytic), dtype=np.float64)

theta = np.angle(analytic)  # in [-pi, pi]
theta_unwrapped = np.unwrap(theta).astype(np.float64)
theta_mod = np.mod(theta_unwrapped, 2*np.pi).astype(np.float64)

# sha1s
hashes = {
    "x_grid_full": sha1_float64(x_grid_full),
    "x_kept": sha1_float64(x_kept),
    "psi_interp": sha1_float64(psi_interp_kept),
    "u_detrended": sha1_float64(u_detrended),
    "u_z": sha1_float64(u_z),
    "freqs": sha1_float64(freqs),
    "bp_mask": sha1_uint8(bp_mask),
    "u_bp": sha1_float64(u_bp),
    "analytic_real": sha1_float64(analytic_real),
    "analytic_imag": sha1_float64(analytic_imag),
    "theta_unwrapped": sha1_float64(theta_unwrapped),
    "theta_mod": sha1_float64(theta_mod),
}

N_kept, train_end, dt_c

### Tool (2026-02-25 01:04:25)

(36592, 25614, 0.00012297217348543395)

### Assistant (2026-02-25 01:04:27)

dt = float(t_uniform[1]-t_uniform[0])
dt

### Tool (2026-02-25 01:04:30)

0.00012297217348543395

### Assistant (2026-02-25 01:04:33)

canon_x_hash = hashlib.sha1(np.asarray(x_kept_c, dtype=np.float64, order='C').tobytes(order='C')).hexdigest()
hashes["x_kept"], canon_x_hash

### Tool (2026-02-25 01:04:35)

('c455fedeff29bb2d0b2c81864970c09638720baf',
 'c455fedeff29bb2d0b2c81864970c09638720baf')

### Assistant (2026-02-25 01:04:38)

out = {
  "identity": {"status":"PASS","dt":dt,"N_kept":int(N_kept),"train_end":int(train_end)},
  "sha1": {
      "theta_mod": hashes["theta_mod"],
      "x_kept": hashes["x_kept"],
      "psi_interp": hashes["psi_interp"],
      "u_detrended": hashes["u_detrended"],
      "u_z": hashes["u_z"],
      "freqs": hashes["freqs"],
      "bp_mask": hashes["bp_mask"],
      "u_bp": hashes["u_bp"],
      "analytic_real": hashes["analytic_real"],
      "analytic_imag": hashes["analytic_imag"],
      "theta_unwrapped": hashes["theta_unwrapped"],
  },
  "scalars":{"train_mean": train_mean, "train_std": train_std},
  "bandpass":{"bp_lo":bp_lo,"bp_hi":bp_hi,"n_freqs": int(freqs.size), "n_kept_freqs_in_mask": int(bp_mask.sum())},
  "notes": f"x_grid_full_sha1={hashes['x_grid_full']}; x_kept_matches_canonical={'YES' if hashes['x_kept']==canon_x_hash else 'NO'}; canonical_x_kept_sha1={canon_x_hash}"
}
json.dumps(out)[:200]

### Tool (2026-02-25 01:04:45)

'{"identity": {"status": "PASS", "dt": 0.00012297217348543395, "N_kept": 36592, "train_end": 25614}, "sha1": {"theta_mod": "c2064ba4ada59124606f1256a2c127a9d17b59ee", "x_kept": "c455fedeff29bb2d0b2c818'

### Assistant (2026-02-25 01:04:48)
