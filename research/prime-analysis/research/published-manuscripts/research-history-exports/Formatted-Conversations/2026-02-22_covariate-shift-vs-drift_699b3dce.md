# Covariate Shift vs Drift

- **Conversation ID:** `699b3dce-81f8-8328-b11c-a77b8d49c2b9`
- **Date:** 2026-02-22

---

### User (2026-02-22 12:33:06)
...
### Tool (2026-02-22 12:33:52)

(['N', 'step', 'x_grid', 'psi_grid', 'psi_minus_x'],
 ['N', 'primes', 'is_prime'])

### Assistant (2026-02-22 12:33:54)
...
# uniform t grid length L spanning [min,max] of t_raw
...
def causal_moving_average(a, W):
    # past-only including current, with smaller window at start
...
mu=float(u[train_idx].mean())
sd=float(u[train_idx].std(ddof=0))
u_std=(u-mu)/(sd if sd>0 else 1.0)

# FFT bandpass mask on full kept sequence
n=N
U=np.fft.rfft(u_std)
...
A=np.abs(analytic)
logA=np.log(A+1e-12)
omega=np.angle(np.exp(1j*(theta[1:]-theta[:-1])))  # wrap_to_pi
# align to omega indices i=0..N-2
theta_f=theta[:-1]
logA_f=logA[:-1]
R_full=None
...
# compute prime counts in [x, x*exp(Delta))
x2=x_kept*np.exp(Delta)
# searchsorted uses primes sorted
left=np.searchsorted(primes, x_kept, side="left")
right=np.searchsorted(primes, x2, side="left")
P=(right-left).astype(float)
E=x_kept*(np.exp(Delta)-1.0)/np.log(x_kept)
R=(P-E)/np.sqrt(E)
# R aligned to indices 0..N-1; for features we use R_i at i
# We'll compute standardized scalars using TRAIN stats later per h

def build_design(indices, stats):
    # indices are i values (aligned to omega, so i<=N-2). require i>=2
    ...
    Rim2=R[idx-2]
    logAi=logA_f[idx]
    # standardize scalars with stats
    def z(x, m, s): 
        s= s if s>0 else 1.0
        return (x-m)/s
    Ri_z=z(Ri, stats["R_mu"], stats["R_sd"])
    Rim1_z=z(Rim1, stats["R_mu"], stats["R_sd"])
    Rim2_z=z(Rim2, stats["R_mu"], stats["R_sd"])
    logA_z=z(logAi, stats["logA_mu"], stats["logA_sd"])
    # Fourier K=3
    th=theta_f[idx]
    four=[]
    for k in (1,2,3):
        four.append(np.sin(k*th))
        four.append(np.cos(k*th))
    four=np.vstack(four).T
    # omega terms standardized omega
    om=omega[idx]
    om_z=z(om, stats["omega_mu"], stats["omega_sd"])
    omega_terms=np.vstack([om_z,
                           np.sin(th)*om_z,
                           np.cos(th)*om_z]).T
    X=np.column_stack([Ri_z,Rim1_z,Rim2_z,logA_z,four,omega_terms])
    return X

def model_and_weight_for_h(h):
    # valid i sets
    max_i=N-2  # since omega defined
    # also need i>=2 for lagged R terms and i+h < N for y
    i_all=np.arange(2, max_i+1)  # inclusive
    # train-valid: i and i+h in train
    train_valid=i_all[(i_all<train_end) & (i_all+h<train_end)]
    test_valid=i_all[(i_all>=train_end) & (i_all+h<N)]  # ensures i+h in test automatically
    # labels y = 1{R_{i+h}-R_i>0}
    y_train=(R[train_valid+h]-R[train_valid] > 0).astype(int)
    y_test=(R[test_valid+h]-R[test_valid] > 0).astype(int)
    # stats for standardization computed on TRAIN samples for scalars & omega/logA
    stats={
        "R_mu": float(R[train_idx].mean()),
        "R_sd": float(R[train_idx].std(ddof=0)),
        "logA_mu": float(logA_f[train_idx[:-1]].mean()),  # train indices for features exclude last
        "logA_sd": float(logA_f[train_idx[:-1]].std(ddof=0)),
        "omega_mu": float(omega[train_idx[:-2]].mean()) if len(train_idx)>2 else float(omega[train_idx[:-1]].mean()),
        "omega_sd": float(omega[train_idx[:-2]].std(ddof=0)) if len(train_idx)>2 else float(omega[train_idx[:-1]].std(ddof=0)),
    }
    X_train=build_design(train_valid, stats)
    X_test=build_design(test_valid, stats)
    # fit logistic
    clf=LogisticRegression(C=1.0, penalty="l2", solver="lbfgs", max_iter=500, random_state=seed)
    ...
    # density ratio estimation on z features (no R terms)
    def z_features(indices):
        idx=np.asarray(indices,dtype=int)
        th=theta_f[idx]
        om=omega[idx]
        la=logA_f[idx]
        # standardize omega/logA with TRAIN stats for z
        om_z=(om-stats["omega_mu"])/(stats["omega_sd"] if stats["omega_sd"]>0 else 1.0)
        la_z=(la-stats["logA_mu"])/(stats["logA_sd"] if stats["logA_sd"]>0 else 1.0)
        Z=np.column_stack([np.sin(th), np.cos(th), om_z, la_z])
        return Z
    Z_train=z_features(train_valid)
    Z_test=z_features(test_valid)
    Z=np.vstack([Z_train,Z_test])
    d=np.concatenate([np.zeros(len(Z_train),dtype=int), np.ones(len(Z_test),dtype=int)])
    disc=LogisticRegression(C=1.0, penalty="l2", solver="lbfgs", max_iter=500, random_state=seed)
    # cross-validated discriminator AUC
    skf=StratifiedKFold(n_splits=5, shuffle=True, random_state=seed)
    qhat=cross_val_predict(disc, Z, d, cv=skf, method="predict_proba")[:,1]
    disc_auc=float(roc_auc_score(d,qhat))
    # fit on full for weights
    disc.fit(Z,d)
    q_test=disc.predict_proba(Z_test)[:,1]
    eps=1e-9
    q_test=np.clip(q_test, eps, 1-eps)
    n_tr=len(Z_train); n_te=len(Z_test)
    w=q_test/(1-q_test) * (n_tr/n_te)
    w=np.clip(w, 0, w_clip)
    # normalize mean=1
    w=w/float(w.mean())
    w_mean=float(w.mean()); w_std=float(w.std(ddof=0)); w_max=float(w.max())
    ESS=float((w.sum()**2)/(np.sum(w**2)+1e-12))
    # weighted metrics
    brier_w=float(np.sum(w*(p_test-y_test)**2)/np.sum(w))
    ll_w=float(np.sum(w* (-(y_test*np.log(np.clip(p_test,1e-15,1-1e-15)) + (1-y_test)*np.log(np.clip(1-p_test,1e-15,1-1e-15)))))/np.sum(w))
    out={
        "counts":{
            "n_train": int(len(train_valid)),
            "n_test": int(len(test_valid)),
            "y_test_pos_rate": float(y_test.mean()) if len(y_test)>0 else float("nan")
        },
        "unweighted":{"AUC":auc,"Brier":brier,"LogLoss":ll},
        "density_ratio":{
            "disc_auc":disc_auc,
            "w_mean":w_mean,"w_std":w_std,"w_max":w_max,
            "ESS":ESS,
            "ESS_frac": float(ESS/len(Z_test)) if len(Z_test)>0 else float("nan")
        },
        "weighted":{"Brier_w":brier_w,"LogLoss_w":ll_w}
    }
    # heuristic decision
    rel_brier=(brier - brier_w)/brier if brier>0 else 0.0
    rel_ll=(ll - ll_w)/ll if ll>0 else 0.0
    ess_frac=out["density_ratio"]["ESS_frac"]
    if (rel_brier>0.01 and rel_ll>0.01 and ess_frac>0.5):
        verdict="yes"
    elif ((rel_brier>0.005 or rel_ll>0.005) and ess_frac>0.3):
        verdict="partial"
    else:
        verdict="no"
    return out, verdict, {"rel_brier_improve":float(rel_brier), "rel_logloss_improve":float(rel_ll)}
    
per_h={}
verdicts={}
improves={}
for h in H:
    res, verdict, imp = model_and_weight_for_h(h)
    per_h[str(h)]=res
    verdicts[str(h)]=verdict
    improves[str(h)]=imp

summary={
    "does_covariate_shift_explain_drift":verdicts,
    "improvement_fractions":improves,
    "rule":"If weighting materially improves Brier/logloss and calibration while ESS is reasonable, covariate shift is dominant; if not, concept drift dominates."
}

params={
    "L":L,"x_min":x_min,"x_max":x_max,"W":W,"Delta":Delta,"train_frac":train_frac,"seed":seed,
    "FAST":list(FAST),"H":H,"w_clip":w_clip,
    "dt":dt,"n_kept":N,"train_end":train_end
}

out={"params":params,"per_h":per_h,"summary":summary}
out.keys(), out["per_h"]["25"]["unweighted"]

### Tool (2026-02-22 12:34:50)

(dict_keys(['params', 'per_h', 'summary']),
 {'AUC': 0.849141738563661,
  'Brier': 0.1586867529669626,
  'LogLoss': 0.49194811994415616})

### Assistant (2026-02-22 12:35:14)
...
out_s=sanitize(out)
json_str=json.dumps(out_s, indent=2, sort_keys=True)
json_str[:500]

### Tool (2026-02-22 12:35:19)

'{\n  "params": {\n    "Delta": 0.01,\n    "FAST": [\n      0.002,\n      0.01\n    ],\n    "H": [\n      25,\n      100\n    ],\n    "L": 65536,\n    "W": 2000,\n    "dt": 0.00024594622340595577,\n    "n_kept": 18296,\n    "seed": 12345,\n    "train_end": 12807,\n    "train_frac": 0.7,\n    "w_clip": 20.0,\n    "x_max": 9000000.0,\n    "x_min": 100000.0\n  },\n  "per_h": {\n    "100": {\n      "counts": {\n        "n_test": 5389,\n        "n_train": 12705,\n        "y_test_pos_rate": 0.5004639079606606\n      },\n      "den'

### Assistant (2026-02-22 12:35:20)
