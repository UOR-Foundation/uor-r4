# Forecast vs Oracle Metrics

- **Conversation ID:** `699ef846-ae60-832e-a4d6-3b496a9d32f6`
- **Date:** 2026-02-25

---

### User (2026-02-25 08:25:32)
...
# Load arrays
# attempt keys
fc.files[:20], canonical.files[:20], cheb.files[:10]

### Tool (2026-02-25 08:25:58)
...
### Assistant (2026-02-25 08:26:02)

dt=float(fc['dt'])
N_kept=int(fc['N_kept'])
train_end=int(fc['train_end'])
dt_c=float(canonical['dt'])
N_kept_c=int(len(canonical['x_kept']))
train_end_c=int(canonical['train_end'])
(dt,N_kept,train_end,dt_c,N_kept_c,train_end_c)

### Tool (2026-02-25 08:26:05)

(0.00012297217348543395, 36592, 25614, 0.00012297217348543395, 36592, 25614)

### Assistant (2026-02-25 08:26:07)

x_kept=canonical['x_kept'].astype(float)
...
cumsum=np.cumsum(u_interp)
ma=np.empty_like(u_interp)
for i in range(N_kept):
    start=max(0,i-W+1)
    # sum = cumsum[i]-cumsum[start-1]
    s = cumsum[i] - (cumsum[start-1] if start>0 else 0.0)
    ma[i]=s/(i-start+1)
u_detr=u_interp - ma

mu_train=u_detr[:train_end].mean()
sd_train=u_detr[:train_end].std(ddof=0)
z=(u_detr-mu_train)/sd_train
...
a=hilbert(u_bp)
theta_raw=np.angle(a)
two_pi=2*np.pi
theta_mod=np.mod(theta_raw, two_pi)
theta_u=np.unwrap(theta_mod)

theta_mod.min(), theta_mod.max(), theta_u[:5], theta_u[-5:]

### Tool (2026-02-25 08:26:17)

(2.9910716778050616e-05,
 6.282692110578014,
 array([1.32477116, 1.35814957, 1.3914283 , 1.42460533, 1.45767855]),
 array([1157.2625451 , 1157.29639459, 1157.33015325, 1157.36381945,
        1157.39739152]))

### Assistant (2026-02-25 08:26:19)
...
valid_y = np.arange(N_kept-h)
y[valid_y]=(R[valid_y+h]-R[valid_y]>0).astype(np.int8)
...
### Tool (2026-02-25 08:26:24)

(25485, 10928, 25614, 36592)

### Assistant (2026-02-25 08:26:26)

idx_train=np.where(train_valid)[0]
D=theta_u[idx_train+h]-theta_u[idx_train]
m=np.mean(np.exp(1j*D))
omega=(np.angle(m))/h
omega, abs(m)

### Tool (2026-02-25 08:26:29)

(0.03302979347475797, 0.7550040497126541)

### Assistant (2026-02-25 08:26:31)
...
eps=wrappi(theta_u[idx_train+h]-theta_u[idx_train]-h*omega)
...
gamma_hat, val, it, bracket = golden_max(Rgamma, -2.0, 2.0, tol=1e-7, max_iter=500)
gamma_hat, val, it, bracket

### Tool (2026-02-25 08:26:43)

(1.2495979916498405,
 0.8243351117220649,
 37,
 (1.2495979546255023, 1.249598028674179))

### Assistant (2026-02-25 08:26:45)

def wrap2(x):
    return np.mod(x, 2*np.pi)

all_valid=(i>=79)&(i+h<N_kept)&(i-ell>=0)
idx_all=np.where(all_valid)[0]

theta_i=theta_mod
theta_oracle=theta_mod.copy()  # placeholder
theta_oracle[:-h]=theta_mod[h:]
# for last h indices oracle undefined; but not used due to mask

delta_all=wrappi(theta_u - np.roll(theta_u, ell))
# but careful for i<ell; those invalid anyway
corr=wrap2(theta_i + h*omega + gamma_hat*delta_all)

idx_test=np.where(test_valid)[0]
err=wrappi(theta_oracle[idx_test]-corr[idx_test])
theta_match={
...
### Tool (2026-02-25 08:26:52)

{'mean_cos': 0.413793301363658,
 'mean_abs_err': 1.1215034790128102,
 'rmse': 1.1821088748307276}

### Assistant (2026-02-25 08:26:54)

X80=fc['X80'].astype(float)
mu_X=fc['mu_X80_train'].astype(float)
sd_X=fc['sd_X80_train'].astype(float)
sd_X_adj=np.where(sd_X==0, 1.0, sd_X)
X80z=(X80-mu_X)/sd_X_adj
...
clf0.fit(X80z[idx_train], y_train)
p0=clf0.predict_proba(X80z[idx_test])[:,1]
auc0=float(roc_auc_score(y_test, p0))
ll0=float(log_loss(y_test, p0, eps=1e-15))

# Phase features
S_hat=np.column_stack([np.sin(corr), np.cos(corr)])
S_orc=np.column_stack([np.sin(theta_oracle), np.cos(theta_oracle)])

mu_S_hat=S_hat[idx_train].mean(axis=0)
sd_S_hat=S_hat[idx_train].std(axis=0, ddof=0)
sd_S_hat=np.where(sd_S_hat==0, 1.0, sd_S_hat)

S_hat_z=(S_hat-mu_S_hat)/sd_S_hat
S_orc_z=(S_orc-mu_S_hat)/sd_S_hat  # locked scaling

X_fore=np.hstack([X80z, S_hat_z])
X_orc=np.hstack([X80z, S_orc_z])

clfF=LogisticRegression(max_iter=2000, solver='lbfgs')
clfF.fit(X_fore[idx_train], y_train)
pF=clfF.predict_proba(X_fore[idx_test])[:,1]
aucF=float(roc_auc_score(y_test, pF))
llF=float(log_loss(y_test, pF, eps=1e-15))

clfO=LogisticRegression(max_iter=2000, solver='lbfgs')
clfO.fit(X_orc[idx_train], y_train)
pO=clfO.predict_proba(X_orc[idx_test])[:,1]
aucO=float(roc_auc_score(y_test, pO))
llO=float(log_loss(y_test, pO, eps=1e-15))

auc0,aucF,aucO,ll0,llF,llO

### Tool (2026-02-25 08:27:07)
...
### Assistant (2026-02-25 08:27:28)

auc_gain_F=aucF-auc0
auc_gain_O=aucO-auc0
recovery = auc_gain_F/auc_gain_O if auc_gain_O!=0 else float('nan')
derived={
    "auc_gain_forecast": float(auc_gain_F),
    "auc_gain_oracle": float(auc_gain_O),
    "recovery_fraction": float(recovery),
    "auc_oracle_minus_forecast": float(aucO-aucF),
    "logloss_oracle_minus_forecast": float(llO-llF)
}
derived

### Tool (2026-02-25 08:27:32)
