# Fingerprint Gate Verification

- **Conversation ID:** `699b9b7f-7e28-8332-8333-77465455375a`
- **Date:** 2026-02-22

---

### User (2026-02-22 19:12:50)
...
### Assistant (2026-02-22 19:12:54)

import numpy as np, json, hashlib, math, os, sys, statistics
...
list(cheb.files), cheb["x_grid"][:5], cheb["psi_minus_x"][:5]

### Tool (2026-02-22 19:13:20)

(['N', 'step', 'x_grid', 'psi_grid', 'psi_minus_x'],
 array([  1, 101, 201, 301, 401], dtype=int32),
 array([-1.        , -2.33956825,  5.14585683, -1.76766456,  2.82480716]))

### Assistant (2026-02-22 19:13:21)

pr = np.load(primes_path)
list(pr.files), pr["primes"][:5], pr["N"]

### Tool (2026-02-22 19:13:22)

(['N', 'primes', 'is_prime'],
 array([ 2,  3,  5,  7, 11], dtype=int32),
 array(10000000, dtype=int32))

### Assistant (2026-02-22 19:13:24)

import scipy, scipy.signal, sklearn
scipy.__version__, sklearn.__version__

### Tool (2026-02-22 19:13:25)

('1.14.1', '1.4.2')

### Assistant (2026-02-22 19:13:27)
...
def causal_moving_average(x: np.ndarray, W: int) -> np.ndarray:
    # past-only including current; for i, avg over max(0,i-W+1)..i
...
def build_arrays(L=131072, x_range=(1e5,9e6), W=2000, band=(0.002,0.01), train_frac=0.7, Delta=0.01):
    x_grid = cheb["x_grid"].astype(np.float64)
    t_raw = np.log(x_grid)
    psi_minus_x = cheb["psi_minus_x"].astype(np.float64)
    # uniform t grid spanning full range
    ...
    U_f = U * mask
    u_bp = np.fft.irfft(U_f, n=N).astype(np.float64)
    # theta
    ...
    x0 = x_kept
    x1 = x0 * math.exp(Delta)
    # count primes in [x0, x1)
    lo = np.searchsorted(primes, x0, side="left")
    hi = np.searchsorted(primes, x1, side="left")
    P = (hi - lo).astype(np.float64)
    E = x0 * (math.exp(Delta)-1.0) / np.log(x0)
    R = (P - E) / np.sqrt(E)
    return dt, x_kept, u_bp, theta, R, train_end

dt, x_kept, u_bp, theta, R, train_end = build_arrays()
dt, len(x_kept), train_end, x_kept[:3], u_bp[:3], theta[:3], R[:3]

### Tool (2026-02-22 19:13:46)

(0.00012297217348543395,
 36592,
 25614,
 array([100009.83380914, 100022.13299199, 100034.43368739]),
 array([-0.39578496, -0.42567722, -0.45479376]),
 array([2.18949203, 2.23517012, 2.28014344]),
 array([-0.6745253 , -0.56851728, -0.56953462]))

### Assistant (2026-02-22 19:13:48)

hashes = {
    "x_kept": sha1_of_array(x_kept),
    "u_bp": sha1_of_array(u_bp),
    "theta": sha1_of_array(theta),
    "R": sha1_of_array(R),
}
hashes

### Tool (2026-02-22 19:13:50)
...
def fit_eval_logit(X_train, y_train, X_test, y_test, C=1.0, max_iter=2000):
    # handle case y has one class in train or test
...
def quantile_bucket_edges(x, nbuckets):
...
def bucketize(values, edges):
    # bucket index 0..len(edges)-2 with right-open except last
    return np.clip(np.searchsorted(edges, values, side="right")-1, 0, len(edges)-2)

def run_h(h):
    N = len(R)
    tau = 2*np.pi
    # valid i: i>=2 and i+h < N
    i_idx = np.arange(2, N-h, dtype=np.int64)
    dR = R[i_idx + h] - R[i_idx]
    y = (dR > 0).astype(np.int32)
    dtheta = wrap_to_pi(theta[i_idx + h] - theta[i_idx])
    dphi = dtheta / tau
    # features
    Ri = R[i_idx]
    Ri1 = R[i_idx-1]
    Ri2 = R[i_idx-2]
    X_base = np.column_stack([Ri, Ri1, Ri2])
    X_add = np.column_stack([Ri, Ri1, Ri2, dphi])
    X_int = np.column_stack([Ri, Ri1, Ri2, dphi, Ri*dphi])
    # split by i < train_end
    train_mask = i_idx < train_end
    test_mask = ~train_mask
    # counts and pos rates
    n_train = int(train_mask.sum()); n_test=int(test_mask.sum())
    train_pos = float(y[train_mask].mean()) if n_train>0 else float("nan")
    test_pos = float(y[test_mask].mean()) if n_test>0 else float("nan")
    # transport stats
    mean_train = float(dphi[train_mask].mean()); std_train=float(dphi[train_mask].std(ddof=0))
    mean_test = float(dphi[test_mask].mean()); std_test=float(dphi[test_mask].std(ddof=0))
    corr_train = float(np.corrcoef(Ri[train_mask], dphi[train_mask])[0,1])
    corr_test = float(np.corrcoef(Ri[test_mask], dphi[test_mask])[0,1])
    # standardize features using TRAIN
    def eval_model(X):
        Xtr, Xte, _, _ = standardize_train_test(X[train_mask], X[test_mask])
        return fit_eval_logit(Xtr, y[train_mask], Xte, y[test_mask])
    auc_b, ll_b = eval_model(X_base)
    auc_a, ll_a = eval_model(X_add)
    auc_i, ll_i = eval_model(X_int)
    # deltas as auc delta
    # lowR buckets
    absR_train = np.abs(Ri[train_mask])
    edges = quantile_bucket_edges(absR_train, 5)
    # lowR threshold is buckets 0,1
    b_test = bucketize(np.abs(Ri[test_mask]), edges)
    low_mask_test = (b_test==0) | (b_test==1)
    n_test_low = int(low_mask_test.sum())
    # for lowR evaluation, still train on all train rows? spec says evaluate restricted to TEST rows in lowR; models same fitted on full train with standardized features using TRAIN stats (already). We'll do fit on full train, eval on low test.
    def eval_low(X):
        Xtr, Xte, mu, sd = standardize_train_test(X[train_mask], X[test_mask])
        model = LogisticRegression(C=1.0, max_iter=2000, solver="lbfgs")
        model.fit(Xtr, y[train_mask])
        p = model.predict_proba(Xte[low_mask_test])[:,1]
        y_low = y[test_mask][low_mask_test]
        auc = roc_auc_score(y_low, p) if len(np.unique(y_low))==2 else float("nan")
        ll = log_loss(y_low, p, labels=[0,1])
        return auc, ll
    auc_b_l, ll_b_l = eval_low(X_base)
    auc_a_l, ll_a_l = eval_low(X_add)
    auc_i_l, ll_i_l = eval_low(X_int)
    return {
        "counts": {
            "n_train": n_train,
            "n_test": n_test,
        ...
        "transport_stats": {
            ...
            "mean_test": mean_test,
            "std_test": std_test,
            "corr_R_transport_train": corr_train,
            "corr_R_transport_test": corr_test
        },
        "overall": {
            "BASE_MR": {"auc_test": float(auc_b), "logloss_test": float(ll_b)},
            "ADD_TRANSPORT": {"auc_test": float(auc_a), "logloss_test": float(ll_a), "delta_vs_BASE": float(auc_a-auc_b)},
            "ADD_TRANSPORT_INTERACT": {"auc_test": float(auc_i), "logloss_test": float(ll_i), "delta_vs_BASE": float(auc_i-auc_b)}
        },
        "lowR": {
            "bucket_edges_absR_train": [float(x) for x in edges],
            "n_test_lowR": n_test_low,
            "BASE_MR": {"auc_test": float(auc_b_l), "logloss_test": float(ll_b_l)},
            "ADD_TRANSPORT": {"auc_test": float(auc_a_l), "logloss_test": float(ll_a_l), "delta_vs_BASE": float(auc_a_l-auc_b_l)},
            "ADD_TRANSPORT_INTERACT": {"auc_test": float(auc_i_l), "logloss_test": float(ll_i_l), "delta_vs_BASE": float(auc_i_l-auc_b_l)}
        }
    }

res25 = run_h(25)
res100 = run_h(100)
res25["overall"]["BASE_MR"], res25["overall"]["ADD_TRANSPORT"], res100["overall"]["ADD_TRANSPORT_INTERACT"]

### Tool (2026-02-22 19:14:20)

({'auc_test': 0.7118063472250222, 'logloss_test': 0.6198253804407922},
...
out = {
    ...
    "notes": [
        ...
        "theta=arg(hilbert(u_bp)) mod 2pi; Delta_phi=wrap_to_pi(theta[i+h]-theta[i])/(2pi)"
    ]
}
print(json.dumps(out, indent=2, sort_keys=True)[:2000])

### Tool (2026-02-22 19:14:52)
