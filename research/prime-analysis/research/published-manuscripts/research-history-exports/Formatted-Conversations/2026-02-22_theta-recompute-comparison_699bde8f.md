# Theta recompute comparison

- **Conversation ID:** `699bde8f-764c-8331-b943-6030c427224f`
- **Date:** 2026-02-22

---

### User (2026-02-22 23:58:59)
...
### Assistant (2026-02-22 23:59:03)

import numpy as np, hashlib, json, os, math
from pathlib import Path
base="/mnt/data"
paths={
"canon": f"{base}/A4_CANONICAL_CACHE_L131072_x1e5_9e6_W2000_bp002_001_D001_seed12345.npz",
"feat": f"{base}/A4_FEATURECACHE_h25_AR80_AR40_theta_omega_masks_v1.npz",
"psi": f"{base}/chebyshev_psi.npz"
}
for k,v in paths.items():
    print(k, os.path.exists(v), v)
canon=np.load(paths["canon"], allow_pickle=True)
canon.files[:20], len(canon.files)

### Tool (2026-02-22 23:59:23)

canon True /mnt/data/A4_CANONICAL_CACHE_L131072_x1e5_9e6_W2000_bp002_001_D001_seed12345.npz
feat True /mnt/data/A4_FEATURECACHE_h25_AR80_AR40_theta_omega_masks_v1.npz
psi True /mnt/data/chebyshev_psi.npz
(['x_kept', 'u_bp', 'theta_mod', 'R', 'dt', 'train_end', 'params', 'hashes'],
 8)

### Assistant (2026-02-22 23:59:26)

feat=np.load(paths["feat"], allow_pickle=True)
feat.files, {k:feat[k].shape for k in feat.files if hasattr(feat[k],'shape')}

### Tool (2026-02-22 23:59:39)
...
(['theta',
 ...
 {'theta': (36592,),
  ...
  'X40': (36592, 40),
  'X80': (36592, 80),
  'mask_train_lag39': (36592,),
  'mask_test_lag39': (36592,),
  'mask_train_lag79': (36592,),
  'mask_test_lag79': (36592,),
  'mu_X40_train': (40,),
  'sd_X40_train': (40,),
  'mu_X80_train': (80,),
  'sd_X80_train': (80,),
...
N_c=len(x_kept_c)
dt_c, N_c, train_end_c

### Tool (2026-02-22 23:59:58)

(0.00012297217348543395, 36592, 25614)

### Assistant (2026-02-23 00:00:01)

psi=np.load(paths["psi"], allow_pickle=True)
x_grid=psi["x_grid"]
...
x_kept_new=x_uniform[kept]
N_kept_new=int(kept.sum())
dt_new, N_kept_new, x_kept_new[0], x_kept_new[-1]

### Tool (2026-02-23 00:00:08)

(0.00012297217348543395, 36592, 100009.83380914277, 8999671.172393441)

### Assistant (2026-02-23 00:00:10)

W=2000
u_interp=np.interp(t_uniform, t_raw, psi_minus_x.astype(np.float64))
u_raw=u_interp[kept]

# causal moving average past-only including current, window up to W
u=u_raw
...
train_end_new=train_end_req
mu=float(u_detr[:train_end_new].mean())
sd=float(u_detr[:train_end_new].std(ddof=0))
u_z=(u_detr-mu)/sd
...
mask=(freqs>=0.003)&(freqs<=0.009)
U_bp=U*mask
u_bp=irfft(U_bp, n=N)

theta_new=np.mod(np.angle(hilbert(u_bp)), 2*np.pi)
theta_sha1=hashlib.sha1(theta_new.astype(np.float64).tobytes()).hexdigest()
theta_sha1, theta_new[:3], theta_new.mean()

### Tool (2026-02-23 00:00:25)

('c888f75abc219749cde8f870c119a6dcd98ec2b4',
 array([1.32477148, 1.3581499 , 1.39142863]),
 3.1560954214749524)

### Assistant (2026-02-23 00:00:27)
...
mask_train=feat["mask_train_lag79"].astype(bool)
mask_test=feat["mask_test_lag79"].astype(bool)
h=25
valid_future=np.zeros_like(mask_train)
valid_future[:-h]=True
y=((R[h:]-R[:-h])>0).astype(int)
y_full=np.full(R.shape, -1, dtype=int)
y_full[:-h]=y

def fit(theta):
    phase=np.column_stack([np.sin(theta), np.cos(theta), np.sin(2*theta), np.cos(2*theta)])
    X=np.hstack([X80_z, phase])
    train_idx=mask_train & valid_future
    test_idx=mask_test & valid_future
    Xtr=X[train_idx]
    ...
    phase_coefs=coefs[-4:]
    return auc, phase_coefs, clf.intercept_[0]

auc_can, coef_can, intercept_can = fit(feat["theta"].astype(np.float64))
auc_new, coef_new, intercept_new = fit(theta_new.astype(np.float64))
auc_can, coef_can, auc_new, coef_new

### Tool (2026-02-23 00:00:45)

---------------------------------------------------------------------------
InvalidParameterError                     Traceback (most recent call last)
Cell In[6], line 41
     38     phase_coefs=coefs[-4:]
     39     return auc, phase_coefs, clf.intercept_[0]
---> 41 auc_can, coef_can, intercept_can = fit(feat["theta"].astype(np.float64))
     42 auc_new, coef_new, intercept_new = fit(theta_new.astype(np.float64))
     43 auc_can, coef_can, auc_new, coef_new

Cell In[6], line 34, in fit(theta)
     32     except TypeError:
     33         clf=LogisticRegression(penalty=None, solver='lbfgs', max_iter=2000)
---> 34     clf.fit(Xtr, ytr)
     35 p=clf.predict_proba(Xte)[:,1]
     36 auc=float(roc_auc_score(yte, p))

File /opt/pyvenv/lib/python3.11/site-packages/sklearn/base.py:1467, in _fit_context.<locals>.decorator.<locals>.wrapper(estimator, *args, **kwargs)
...
File /opt/pyvenv/lib/python3.11/site-packages/sklearn/base.py:666, in BaseEstimator._validate_params(self)
...
File /opt/pyvenv/lib/python3.11/site-packages/sklearn/utils/_param_validation.py:95, in validate_parameter_constraints(parameter_constraints, params, caller_name)
     89 else:
     90     constraints_str = (
...
### Assistant (2026-02-23 00:00:47)

from sklearn.linear_model import LogisticRegression
from sklearn.metrics import roc_auc_score
from sklearn.exceptions import ConvergenceWarning
import warnings, numpy as np, math
...
def fit(theta):
    ...
    coefs=clf.coef_.ravel()
    phase_coefs=coefs[-4:]
    return auc, phase_coefs, float(clf.intercept_[0]), int(train_idx.sum()), int(test_idx.sum()), float(yte.mean())

auc_can, coef_can, b0_can, ntr, nte, pos = fit(feat["theta"].astype(np.float64))
auc_new, coef_new, b0_new, _, _, _ = fit(theta_new.astype(np.float64))
auc_can, coef_can, auc_new, coef_new, (ntr, nte, pos)

### Tool (2026-02-23 00:00:57)

(0.8027096151987911,
 array([ 0.41599779, -0.79507508,  0.17715191, -0.03105319]),
 0.8121254683302611,
 array([ 1.00840185, -0.23570815,  0.0307249 ,  0.01459527]),
 (25510, 10953, 0.4972153747831644))

### Assistant (2026-02-23 00:01:14)

delta_coef=float(np.linalg.norm(coef_new-coef_can))
delta_auc=float(auc_new-auc_can)
delta_coef, delta_auc

### Tool (2026-02-23 00:01:16)

(0.8290709082327131, 0.009415853131469976)

### Assistant (2026-02-23 00:01:18)

out={
"identity":{
"status":"PASS" if (abs(dt_new-dt_req)<1e-15 and N_kept_new==N_req and train_end_new==train_end_req) else "FAIL",
"dt_new":dt_new,
"N_kept_new":N_kept_new,
"train_end_new":train_end_new,
"mismatch_fields": None
},
"theta_new":{"sha1":theta_sha1},
"fits":{
"canonical_theta":{
"coef_phase":{"sin1":float(coef_can[0]),"cos1":float(coef_can[1]),"sin2":float(coef_can[2]),"cos2":float(coef_can[3])},
"auc_test":auc_can
},
"new_theta":{
"coef_phase":{"sin1":float(coef_new[0]),"cos1":float(coef_new[1]),"sin2":float(coef_new[2]),"cos2":float(coef_new[3])},
"auc_test":auc_new
}
},
"comparison":{
"delta_coef_L2":delta_coef,
"delta_auc_test":delta_auc
},
"notes":f"Fingerprint-lock satisfied (dt={dt_new}, N_kept={N_kept_new}, train_end={train_end_new}). Logistic regression: intercept + standardized AR80 + phase harmonics; TRAIN rows=25510, TEST rows=10953, TEST pos_rate≈{pos:.6f}. theta_new computed from chebyshev_psi via canonical A4 grid/detrend/standardize but with bandpass [0.003,0.009] cycles/sample."
}
json.dumps(out, indent=2)[:500]

### Tool (2026-02-23 00:01:27)
