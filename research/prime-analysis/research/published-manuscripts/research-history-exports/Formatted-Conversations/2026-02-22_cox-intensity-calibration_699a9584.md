# Cox-Intensity Calibration

- **Conversation ID:** `699a9584-d31c-8326-95b7-643b8cb758a5`
- **Date:** 2026-02-22

---

### User (2026-02-22 00:35:06)
...
x_min=1e5; x_max=9e6
W=2000
band_lo=0.002; band_hi=0.01
Delta=0.01
B=12
H=[1,10,50]
seed=12345

# Step 1: uniform t grid interpolation
...
u_interp = np.interp(t_uniform, t_grid, psi_minus_x.astype(np.float64))
...
# bandpass via FFT mask on full u_z (as requested)
# rfft frequencies in cycles/sample
U = rfft(u_z)
...
# g(theta) raw and normalize on TRAIN
g_raw = (np.cos(theta) + np.sin(theta) +
         0.5*np.cos(2*theta) + 0.5*np.sin(2*theta) +
         0.25*np.cos(3*theta) + 0.25*np.sin(3*theta))
g_mu = g_raw[train_idx].mean()
g_sd = g_raw[train_idx].std(ddof=0)
g = (g_raw - g_mu) / (g_sd if g_sd>0 else 1.0)

# Step 2: compute P_i and E_i
# counts primes in [x, x*exp(Delta))
expD = float(np.exp(Delta))
# precompute for efficient search
primes = primes.astype(np.int64)
def count_primes_interval(a, b):
    # [a,b)
    lo = np.searchsorted(primes, a, side='left')
    hi = np.searchsorted(primes, b, side='left')
    return hi-lo

# Vectorized via searchsorted on arrays
a = x_k
b = x_k*expD
lo = np.searchsorted(primes, a, side='left')
hi = np.searchsorted(primes, b, side='left')
P = (hi-lo).astype(np.int64)
E = (x_k*(expD-1.0)/np.log(x_k)).astype(np.float64)

# Step 3: Poisson regression with offset log(E)
# MLE for beta using Newton-Raphson on train
P_tr = P[train_idx].astype(np.float64)
E_tr = E[train_idx]
g_tr = g[train_idx]
# remove any E<=0
valid_tr = np.isfinite(E_tr) & (E_tr>0) & np.isfinite(g_tr)
P_tr = P_tr[valid_tr]; E_tr = E_tr[valid_tr]; g_tr = g_tr[valid_tr]

beta = 0.0
for _ in range(50):
    lam = E_tr * np.exp(beta*g_tr)
    grad = np.sum(g_tr*(P_tr - lam))
    hess = -np.sum((g_tr**2)*lam)
    step = grad / (hess if hess!=0 else -1e-12)
    beta_new = beta - step
...
beta_hat = float(beta); se_hat = float(se)

# Helpers: MI and AUC
def mutual_info_bins_y(bins, y, B):
    # bins in 0..B-1, y in {0,1}
    ...
    for bi, yi in zip(bins, y):
        if 0<=bi<B and (yi==0 or yi==1):
            joint[bi, yi] += 1.0
    joint /= n
...
def auc_score(y, s):
    ...
    m1 = np.sum(y==1); m0 = np.sum(y==0)
    if m1==0 or m0==0:
        return float('nan')
    # rank-based AUC with tie handling: use scipy? implement manually
    order = np.argsort(s)
    ...
    while i<len(s):
        ...
        r += (j-i+1)
        i = j+1
    sum_r1 = ranks[y==1].sum()
    auc = (sum_r1 - m1*(m1+1)/2.0) / (m1*m0)
    return float(auc)

# Step 4 real test
phase_bin = np.floor(B*theta/(2*np.pi)).astype(np.int64)
phase_bin = np.clip(phase_bin, 0, B-1)

ratio = P / E
rate_bin_real = []
for bidx in range(B):
    m = (test_idx[:,None] if False else None)
for bidx in range(B):
    maskb = (phase_bin[test_idx]==bidx)
    rate_bin_real.append(float(ratio[test_idx][maskb].mean()) if maskb.any() else float('nan'))

R = (P - E)/np.sqrt(E)
s_fixed = np.sin(theta) + np.cos(theta)

def compute_targets_and_metrics(R_arr, phase_bin_arr, score_arr, P_arr, E_arr, train_cut):
    out_MI = {'primary':{}, 'desticky':{}}
    out_AUC = {'primary':{}, 'desticky':{}}
    n_tot = len(R_arr)
    train_idx = np.arange(train_cut)
    test_idx = np.arange(train_cut, n_tot)
    for h in H:
        # primary thresholds from train valid i with i+h in train
        valid_train_i = train_idx[train_idx + h < train_cut]
        thr = np.median(R_arr[valid_train_i + h]) if len(valid_train_i)>0 else float('nan')
        # test valid i with i in test and i+h < n_tot
        valid_test_i = test_idx[test_idx + h < n_tot]
        pb = phase_bin_arr[valid_test_i]
        # primary y
        y_primary = (R_arr[valid_test_i + h] > thr).astype(np.int64)
        out_MI['primary'][str(h)] = mutual_info_bins_y(pb, y_primary, B)
        out_AUC['primary'][str(h)] = auc_score(y_primary, score_arr[valid_test_i])
        # desticky
        y_desticky = ((R_arr[valid_test_i + h] - R_arr[valid_test_i]) > 0).astype(np.int64)
        out_MI['desticky'][str(h)] = mutual_info_bins_y(pb, y_desticky, B)
        out_AUC['desticky'][str(h)] = auc_score(y_desticky, score_arr[valid_test_i])
    return out_MI, out_AUC

MI_real, AUC_real = compute_targets_and_metrics(R, phase_bin, s_fixed, P, E, n_train)

# Step 5 simulation
rng = np.random.default_rng(seed)
lambda_sim = E * np.exp(beta_hat * g)
P_sim = rng.poisson(lam=lambda_sim)
...
for bidx in range(B):
    maskb = (phase_bin[test_idx]==bidx)
    rate_bin_sim.append(float(ratio_sim[test_idx][maskb].mean()) if maskb.any() else float('nan'))

MI_sim, AUC_sim = compute_targets_and_metrics(R_sim, phase_bin, s_fixed, P_sim, E, n_train)

# distances
# dist_rate
rb_real = np.array(rate_bin_real, dtype=np.float64)
rb_sim = np.array(rate_bin_sim, dtype=np.float64)
mask = np.isfinite(rb_real) & np.isfinite(rb_sim)
dist_rate = float(np.mean((rb_sim[mask]-rb_real[mask])**2)) if mask.any() else float('nan')

# dist_MI over targets,h
mi_vals = []
for target in ['primary','desticky']:
    for h in H:
        r = MI_real[target][str(h)]
        s = MI_sim[target][str(h)]
        if np.isfinite(r) and np.isfinite(s):
            mi_vals.append((s-r)**2)
dist_MI = float(np.mean(mi_vals)) if len(mi_vals)>0 else float('nan')
...
out = {
 ...
 "beta_hat":{"beta":beta_hat,"se":se_hat},
 "real_test":{
   "rate_bin_real": rate_bin_real,
   "MI": MI_real,
   "AUC_fixed_phase_score": AUC_real
 },
 "sim_test":{
   "rate_bin_sim": rate_bin_sim,
   "MI": MI_sim,
   "AUC_fixed_phase_score": AUC_sim
 },
 "distances":{"dist_rate":dist_rate,"dist_MI":dist_MI}
}
out_json = json.dumps(out, separators=(',',':'))
out_json[:500], len(out_json)

### Tool (2026-02-22 00:36:10)

('{"params":{"L":65536,"x_min":100000.0,"x_max":9000000.0,"W":2000,"bandpass":[0.002,0.01],"Delta":0.01,"B":12,"H":[1,10,50],"seed":12345,"dt":0.00024594622340595577,"n_kept":18296,"n_train":12807,"n_test":5489},"beta_hat":{"beta":-0.008413021697745597,"se":0.00038700013720353643},"real_test":{"rate_bin_real":[0.9939529885168753,0.9930211180388189,0.9937697244504252,0.9956108387490735,0.9984233812786812,1.0006921927254269,1.006634847447225,1.009854288542299,1.0064578599293128,1.0036331766074016,1.',
 1626)

### Assistant (2026-02-22 00:36:15)

out_json

### Tool (2026-02-22 00:36:16)
