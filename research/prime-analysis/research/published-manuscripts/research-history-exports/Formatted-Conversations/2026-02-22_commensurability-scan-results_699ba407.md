# Commensurability Scan Results

- **Conversation ID:** `699ba407-8084-8332-87d0-4fecfab9342e`
- **Date:** 2026-02-22

---

### User (2026-02-22 19:49:13)
...
### Assistant (2026-02-22 19:49:16)

import numpy as np, hashlib, json, math, os, sys, statistics
...
### Tool (2026-02-22 19:49:54)

(0.00012297217348543395, 0.0, 16.118085750909312)

### Assistant (2026-02-22 19:49:56)

interp=np.interp(t_uniform, t_raw, psi_minus_x.astype(np.float64))
x_uniform=np.exp(t_uniform)
kept=(x_uniform>=1e5)&(x_uniform<=9e6)
x_kept=x_uniform[kept].astype(np.float64)
u_raw=interp[kept].astype(np.float64)
N_kept=len(u_raw)
train_end=int(np.floor(0.7*N_kept))
N_kept, train_end, x_kept[0], x_kept[-1]

### Tool (2026-02-22 19:49:59)

(36592, 25614, 100009.83380914277, 8999671.172393441)

### Assistant (2026-02-22 19:50:01)

W=2000
u=u_raw
cs=np.cumsum(u, dtype=np.float64)
# moving sum over past W including current:
# sum_{k=max(0,i-W+1)}^i u[k]
idx=np.arange(N_kept)
start=idx-(W-1)
start_clip=np.clip(start,0,None)
# sums: cs[i]-cs[start_clip-1] if start_clip>0 else cs[i]
prev = np.where(start_clip>0, cs[start_clip-1], 0.0)
movsum = cs - prev
winlen = idx - start_clip + 1
movavg = movsum / winlen
u_detr = u - movavg
...
U_filt=np.where(mask, U, 0)
u_bp=irfft(U_filt, n=N_kept).astype(np.float64)
# Hilbert
z=signal.hilbert(u_bp)
theta=np.angle(z)
theta_mod=np.mod(theta, 2*np.pi).astype(np.float64)
...
### Tool (2026-02-22 19:50:10)

{'x_kept': 'c455fedeff29bb2d0b2c81864970c09638720baf',
 'u_bp': 'b16b32cb5d4e63be1c61b88fcf4711d0656da14e',
 'theta_mod': '8b33c57df436d37179b13b1899bf86a723f0e98a'}

### Assistant (2026-02-22 19:50:12)

dtheta = theta_unwrap[1:] - theta_unwrap[:-1]
s_i = (dtheta/dt)/(2*np.pi)  # cycles per unit log-time
s_i = s_i.astype(np.float64)
m=np.median(s_i)
mad=np.median(np.abs(s_i - m))
rs=1.4826*mad
keep = np.abs(s_i - m) <= 6*rs if rs>0 else np.ones_like(s_i, dtype=bool)
s = s_i[keep]
kept_frac=float(keep.mean())
Nf=len(s)
median_s=float(np.median(s))
robust_std=float(rs)
kept_frac, Nf, median_s, robust_std

### Tool (2026-02-22 19:50:16)

(0.9781640294061381, 35792, 33.22660783530962, 13.17409686266538)

### Assistant (2026-02-22 19:50:17)

import numpy as np, math
two_pi=2*np.pi
def rayleigh_p_from_z(z,N):
    # z can be array
    z=np.asarray(z, dtype=np.float64)
    N=float(N)
    # series correction
    term1 = 1.0 + (2*z - z*z)/(4*N) - (24*z - 132*z*z + 76*z**3 - 9*z**4)/(288*(N**2))
    p = np.exp(-z) * term1
    # clamp
    p = np.clip(p, 0.0, 1.0)
    return p

def rayleigh_stats(Cs, s):
    Cs=np.asarray(Cs, dtype=np.float64)
    N=len(s)
    mrl=np.empty_like(Cs)
    for j,C in enumerate(Cs):
        ang = two_pi*C*s
        v = np.exp(1j*ang).mean()
        mrl[j]=np.abs(v)
    z = N * mrl**2
    p = rayleigh_p_from_z(z, N)
    return mrl, z, p

# targets
C_tau=two_pi
C_125=125.0
mrl_tau,z_tau,p_tau = rayleigh_stats([C_tau], s)
mrl_125,z_125,p_125 = rayleigh_stats([C_125], s)
mrl_tau[0],p_tau[0],mrl_125[0],p_125[0]

### Tool (2026-02-22 19:50:30)

(0.003630028795379955,
 0.6239843117012009,
 0.006372882822446653,
 0.2337185434868109)

### Assistant (2026-02-22 19:50:32)

Cs=np.linspace(1.0,200.0,1991, dtype=np.float64)
N=len(s)
chunk=50
mrl=np.empty(Cs.shape, dtype=np.float64)
for start in range(0,len(Cs),chunk):
    end=min(len(Cs),start+chunk)
    Cc=Cs[start:end]
    ang = two_pi * s[:,None] * Cc[None,:]
    # compute mean exp(i*ang)
    mean_cos = np.cos(ang).mean(axis=0)
    mean_sin = np.sin(ang).mean(axis=0)
    mrl[start:end]=np.sqrt(mean_cos**2 + mean_sin**2)
z = N * mrl**2
p = rayleigh_p_from_z(z, N)
p.min(), p.max(), float(mrl.max())

### Tool (2026-02-22 19:50:40)

(0.00020690548910672234, 0.9993192842835528, 0.015394949757934649)

### Assistant (2026-02-22 19:50:43)
...
# enforce monotone nondecreasing when going from largest to smallest: q_adj[i]=min_{j>=i} q_sorted[j]
q_adj=np.minimum.accumulate(q_sorted[::-1])[::-1]
q_adj=np.clip(q_adj,0,1)
q=np.empty_like(q_adj)
q[order]=q_adj
# top hits
topk=15
top_idx=order[:topk]
...
# ranks for tau and 125 nearest grid
def nearest_idx(Ct):
    return int(np.argmin(np.abs(Cs - Ct)))
idx_tau=nearest_idx(C_tau)
idx_125=nearest_idx(C_125)
# rank position by p (1=smallest)
rank_by_p=np.empty(m, dtype=int)
rank_by_p[order]=np.arange(1,m+1)
tau_rank={"C_grid": float(Cs[idx_tau]), "rank_by_p": int(rank_by_p[idx_tau]), "p": float(p[idx_tau]), "q_fdr": float(q[idx_tau])}
c125_rank={"C_grid": float(Cs[idx_125]), "rank_by_p": int(rank_by_p[idx_125]), "p": float(p[idx_125]), "q_fdr": float(q[idx_125])}
bonf_alpha=0.05/m
n_pass_bonf=int((p < bonf_alpha).sum())
n_pass_fdr=int((q <= 0.05).sum())
bonf_alpha, n_pass_bonf, n_pass_fdr, tau_rank, c125_rank, top_hits[0]

### Tool (2026-02-22 19:50:55)

(2.5113008538422903e-05,
 0,
 0,
...
"rotation_rate":{
    "median_s":median_s,
    "robust_std_s":robust_std,
    "kept_frac":kept_frac,
    "N_filtered":Nf
},
"targets":{
    "tau_2pi":{"C":C_tau,"MRL":float(mrl_tau[0]),"z":float(z_tau[0]),"p_rayleigh":float(p_tau[0])},
    "C_125":{"C":C_125,"MRL":float(mrl_125[0]),"z":float(z_125[0]),"p_rayleigh":float(p_125[0])}
},
"scan":{
    "C_min":1.0,"C_max":200.0,"n_grid":int(m),
    "top_hits":top_hits,
    "tau_rank_nearest_grid":tau_rank,
    "C125_rank_nearest_grid":c125_rank,
    "bonferroni_alpha":bonf_alpha,
    "n_pass_bonferroni":n_pass_bonf,
    "n_pass_fdr_q05":n_pass_fdr
},
"notes":{
"interpretation_hint":"If tau or 125 are truly privileged, they should rank near the top and survive correction; otherwise any apparent preference is noise or a broad, non-specific effect."
}
