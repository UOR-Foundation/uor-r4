# Stratified Mechanism Verification

- **Conversation ID:** `699c5536-5328-8328-b090-433c37893ce1`
- **Date:** 2026-02-23

---

### User (2026-02-23 08:25:12)
...
### Assistant (2026-02-23 08:26:20)

dt_fc=float(fc['dt'])
dt_c=float(canon['dt'])
N_fc=int(fc['N_kept'])
N_c=int(len(canon['x_kept']))
train_end_fc=int(fc['train_end'])
train_end_c=int(canon['train_end'])
(dt_fc, dt_c, N_fc, N_c, train_end_fc, train_end_c)

### Tool (2026-02-23 08:26:25)

(0.00012297217348543395, 0.00012297217348543395, 36592, 36592, 25614, 25614)

### Assistant (2026-02-23 08:26:29)

hashes=canon['hashes'].item()
hashes

### Tool (2026-02-23 08:26:33)
...
### Assistant (2026-02-23 08:26:34)

hashes=json.loads(hashes)
def sha1_arr(a: np.ndarray)->str:
    # use raw bytes of C-contiguous float64
    b=np.ascontiguousarray(a).view(np.uint8)
    return hashlib.sha1(b).hexdigest()

R_fc=fc['R'].astype(np.float64)
R_c=canon['R'].astype(np.float64)
sha1_R_fc=sha1_arr(R_fc)
sha1_R_c=sha1_arr(R_c)
sha1_R_fc, sha1_R_c, hashes['R'], np.allclose(R_fc,R_c)

### Tool (2026-02-23 08:26:38)
...
# interpolate
f_interp=interp1d(t_raw, psi_minus_x, kind='linear', fill_value="extrapolate", assume_sorted=True)
u_uniform=f_interp(t_uniform)
x_uniform=np.exp(t_uniform)
x_min=1e5; x_max=9e6
kept_mask=(x_uniform>=x_min) & (x_uniform<=x_max)
x_kept=x_uniform[kept_mask]
N_kept=len(x_kept)
dt, N_kept, sha1_arr(x_kept.astype(np.float64)), hashes['x_kept']

### Tool (2026-02-23 08:26:57)
...
### Assistant (2026-02-23 08:27:00)

W=2000
u_raw=u_uniform[kept_mask]
n=len(u_raw)
...
Uf_bp=Uf*mask
u_bp=irfft(Uf_bp, n=n)
# hilbert
analytic=hilbert(u_bp)
theta_mod=np.mod(np.angle(analytic), 2*np.pi)
theta_sha1=sha1_arr(theta_mod.astype(np.float64))
theta_sha1[:10], theta_sha1

### Tool (2026-02-23 08:27:24)

('c888f75abc', 'c888f75abc219749cde8f870c119a6dcd98ec2b4')

### Assistant (2026-02-23 08:27:29)
...
### Tool (2026-02-23 08:27:42)

(25485, 10928)

### Assistant (2026-02-23 08:27:46)

R=R_fc
absR_thresh=float(np.median(np.abs(R[train_valid])))
median_future=float(np.median(R[idx[train_valid]+h]))
absR_thresh, median_future

### Tool (2026-02-23 08:27:49)

(0.32995611380415996, -0.027134746076412856)

### Assistant (2026-02-23 08:27:57)

dR=R[idx+h]-R[idx]  # but idx+h out of range last h; need safe
# We'll create arrays length N_kept with nan
y_dest=np.full(N_kept, np.nan)
y_med=np.full(N_kept, np.nan)
valid_all=(idx+h<N_kept)
y_dest[valid_all]=(R[idx[valid_all]+h]-R[idx[valid_all]]>0).astype(int)
y_med[valid_all]=(R[idx[valid_all]+h]>median_future).astype(int)

# strata on test_valid based on |R_i|
low_mask=(np.abs(R)<=absR_thresh)
high_mask=~low_mask
# ensure only within test_valid
(test_valid & low_mask).sum(), (test_valid & high_mask).sum()

### Tool (2026-02-23 08:28:05)

---------------------------------------------------------------------------
IndexError                                Traceback (most recent call last)
Cell In[9], line 1
----> 1 dR=R[idx+h]-R[idx]  # but idx+h out of range last h; need safe
      2 # We'll create arrays length N_kept with nan
      3 y_dest=np.full(N_kept, np.nan)

IndexError: index 36592 is out of bounds for axis 0 with size 36592

### Assistant (2026-02-23 08:28:07)

idx=np.arange(N_kept)
valid_all = idx + h < N_kept
y_dest=np.full(N_kept, -1, dtype=int)
y_med=np.full(N_kept, -1, dtype=int)
y_dest[valid_all]=((R[idx[valid_all]+h]-R[idx[valid_all]])>0).astype(int)
y_med[valid_all]=(R[idx[valid_all]+h]>median_future).astype(int)

low_mask=(np.abs(R)<=absR_thresh)
high_mask=~low_mask
(test_valid & low_mask).sum(), (test_valid & high_mask).sum(), y_dest[test_valid].min(), y_med[test_valid].min()

### Tool (2026-02-23 08:28:13)

(5637, 5291, 0, 0)

### Assistant (2026-02-23 08:28:15)

X80=fc['X80'].astype(np.float64)
X80.shape

### Tool (2026-02-23 08:28:17)

(36592, 80)

### Assistant (2026-02-23 08:28:19)

mu_X80=fc['mu_X80_train'].astype(np.float64)
sd_X80=fc['sd_X80_train'].astype(np.float64)
mu_X80.shape, sd_X80.shape, np.isfinite(sd_X80).all(), sd_X80.min()

### Tool (2026-02-23 08:28:23)

((80,), (80,), True, 0.49281391501426697)

### Assistant (2026-02-23 08:28:38)

from math import isnan
def fit_and_eval(y_arr):
    # build train/test sets
    y_train=y_arr[train_valid]
    y_test=y_arr[test_valid]
    # baseline features
    Xb=(X80 - mu_X80)/sd_X80
    # phase feats
    s=np.sin(theta_mod)
    c=np.cos(theta_mod)
    s_mu=s[train_valid].mean(); s_sd=s[train_valid].std(ddof=0)
    c_mu=c[train_valid].mean(); c_sd=c[train_valid].std(ddof=0)
    s_z=(s-s_mu)/s_sd
    c_z=(c-c_mu)/c_sd
    
    X_train_b=Xb[train_valid]
    X_test_b=Xb[test_valid]
    X_train_f=np.column_stack([X_train_b, s_z[train_valid], c_z[train_valid]])
    X_test_f=np.column_stack([X_test_b, s_z[test_valid], c_z[test_valid]])
    
    # fit
    lr_b=LogisticRegression(max_iter=2000, solver='lbfgs', C=1e6)
    lr_f=LogisticRegression(max_iter=2000, solver='lbfgs', C=1e6)
    lr_b.fit(X_train_b, y_train)
    lr_f.fit(X_train_f, y_train)
    pb=lr_b.predict_proba(X_test_b)[:,1]
    pf=lr_f.predict_proba(X_test_f)[:,1]
    
    out={}
    for name,mask in [('low', low_mask),('high', high_mask)]:
        m=test_valid & mask
        yt=y_arr[m]
        pbt=pb[m[test_valid]]  # careful mapping: pb aligned to test_valid indices only
        pft=pf[m[test_valid]]
        n=int(len(yt))
        pos=float(yt.mean()) if n>0 else float('nan')
        # auc
        auc_b=float('nan'); auc_f=float('nan')
        if n>0 and len(np.unique(yt))==2:
            auc_b=float(roc_auc_score(yt, pbt))
            auc_f=float(roc_auc_score(yt, pft))
        delta=auc_f-auc_b if (not isnan(auc_f) and not isnan(auc_b)) else float('nan')
        out[name]={"n":n,"pos_rate":pos,"auc_baseline":auc_b,"auc_full":auc_f,"delta_auc":delta}
    return out

res_dest=fit_and_eval(y_dest)
res_med=fit_and_eval(y_med)
res_dest, res_med

### Tool (2026-02-23 08:29:00)
