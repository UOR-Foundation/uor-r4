...
- **Conversation ID:** `699a9fea-e368-832f-a04c-5f5157c4b883`
- **Date:** 2026-02-22

---

### User (2026-02-22 01:19:30)
...
### Assistant (2026-02-22 01:19:32)

import numpy as np, json, math, os, sys, statistics
from pathlib import Path
import numpy as np

### Assistant (2026-02-22 01:20:08)
...
psi_log_path = base/"psi_log_spectrum.json"
...
u_psi_raw = np.interp(t_uniform, t_grid, psi_minus_x.astype(np.float64))
# standardize
u_psi = (u_psi_raw - u_psi_raw.mean())/u_psi_raw.std(ddof=0)
dt, t_min, t_max, u_psi.mean(), u_psi.std()

### Tool (2026-02-22 01:20:13)

(0.00024594622340595577, 0.0, 16.118085750909312, 1.214306433183765e-17, 1.0)

### Assistant (2026-02-22 01:20:14)

# Build log-factorial baseline using loggamma
try:
...
### Tool (2026-02-22 01:20:16)

True

### Assistant (2026-02-22 01:20:18)

from scipy.special import gammaln
x_floor = np.floor(x_grid).astype(np.int64)
S = gammaln(x_floor + 1).astype(np.float64)  # log factorial
x = x_grid.astype(np.float64)
# f_log(x)=S(x)-x log x + x
f_log = S - x*np.log(x) + x
u_log_raw = np.interp(t_uniform, t_grid, f_log)
u_log = (u_log_raw - u_log_raw.mean())/u_log_raw.std(ddof=0)
u_log.mean(), u_log.std()

### Tool (2026-02-22 01:20:22)

(2.7755575615628914e-16, 0.9999999999999999)

### Assistant (2026-02-22 01:20:24)

# Cox null simulation via per-bin Poisson counts with mean 100/log(mid)
seed=12345
rng = np.random.default_rng(seed)
step=100
# bins correspond to x_grid values: 1,101,...,9,999,901; interval [x_i, x_i+100)
x_start = x_grid.astype(np.int64)
# midpoints, avoid log(1)
mid = x_start + step/2
mid = np.maximum(mid, 2)
lam = step / np.log(mid.astype(np.float64))
counts = rng.poisson(lam)
counts[:10], counts.mean(), counts.sum()

### Tool (2026-02-22 01:20:29)

(array([21, 24, 17, 18, 18, 15, 18, 18,  8, 13]), 6.65257, 665257)

### Assistant (2026-02-22 01:20:31)

# generate pseudo-prime points and accumulate weights per bin
# We'll generate per bin: uniform in [x, x+100)
# compute sum log points per bin
w = np.zeros_like(x_start, dtype=np.float64)
# generate in chunks to avoid huge overhead
for i, c in enumerate(counts):
    if c:
        pts = x_start[i] + rng.random(c)*step
        # avoid pts<2 for log
        pts = np.maximum(pts, 2.0)
        w[i] = np.log(pts).sum()
# cumulative psi_tilde at each x_grid point is sum weights up to bin i inclusive? 
# since x_grid is start of bin, psi(x_grid) should include events <= x_grid, i.e. prior bins only.
# But for consistency with chebyshev psi on x_grid points, we can define psi at x_grid[i] as cumulative weights of events with p <= x_grid[i].
# Our events in bin i are in [x_i, x_i+100) > x_i, so none are <= x_i except maybe exactly x_i. We'll ignore that.
# So psi_tilde_grid[i] = sum_{j < i} w[j]
psi_tilde = np.concatenate([[0.0], np.cumsum(w[:-1])])
f_cox = psi_tilde - x
u_cox_raw = np.interp(t_uniform, t_grid, f_cox)
u_cox = (u_cox_raw - u_cox_raw.mean())/u_cox_raw.std(ddof=0)
(u_cox.mean(), u_cox.std(), psi_tilde[-1], counts.sum())

### Tool (2026-02-22 01:20:41)

(-5.551115123125783e-17, 1.0, 10006269.21562201, 665257)

### Assistant (2026-02-22 01:20:45)

from scipy.signal import welch, get_window
def extract_peaks(u, f_lo, f_hi, M=60, nperseg=4096, noverlap=2048):
    f, Pxx = welch(u, fs=1.0, window='hann', nperseg=nperseg, noverlap=noverlap, detrend=False, scaling='density')
    # restrict range
    mask = (f >= f_lo) & (f <= f_hi)
    idx = np.where(mask)[0]
    # local maxima within mask excluding endpoints
    cand = []
    for k in idx:
        if k==0 or k==len(Pxx)-1: 
            continue
        if Pxx[k] > Pxx[k-1] and Pxx[k] > Pxx[k+1]:
            cand.append(k)
    cand = np.array(cand, dtype=np.int64)
    if cand.size==0:
        return [], {"f": f.tolist(), "Pxx": Pxx.tolist()}
    # sort by power
    order = cand[np.argsort(Pxx[cand])[::-1]]
    selected = []
    for k in order:
        if all(abs(k - s) >= 2 for s in selected):
            selected.append(int(k))
            if len(selected) >= M:
                break
    # compute relative power normalized by max among selected
    powers = Pxx[selected]
    rel = (powers / powers.max()) if powers.max()>0 else powers
    peaks=[]
    for k, rp, p in zip(selected, rel, powers):
        fk = float(f[k])
        peaks.append({
            "f": fk,
            "gamma": float(2*math.pi*fk/dt),
            "relative_power": float(rp)
        })
    return peaks
# test for psi rangeB
peaks_psi_B = extract_peaks(u_psi, 0.0, 0.05)
len(peaks_psi_B), peaks_psi_B[0]

### Tool (2026-02-22 01:20:52)

(54, {'f': 0.000732421875, 'gamma': 18.711173117145268, 'relative_power': 1.0})

### Assistant (2026-02-22 01:20:56)

def peaks_for_signal(u):
    return {
        "rangeA": extract_peaks(u, 0.0, 0.02),
        "rangeB": extract_peaks(u, 0.0, 0.05)
    }
peaks = {
    "psi": peaks_for_signal(u_psi),
    "logfactorial": peaks_for_signal(u_log),
    "cox": peaks_for_signal(u_cox)
}
{k:{r:len(peaks[k][r]) for r in peaks[k]} for k in peaks}

### Tool (2026-02-22 01:20:58)

{'psi': {'rangeA': 20, 'rangeB': 54},
 'logfactorial': {'rangeA': 1, 'rangeB': 48},
 'cox': {'rangeA': 20, 'rangeB': 54}}

### Assistant (2026-02-22 01:21:00)

hist = json.loads(Path(psi_log_path).read_text())
historical_gammas = [float(p["gamma_est"]) for p in hist["top_25_peaks"]]
historical_gammas[:5], len(historical_gammas)

### Tool (2026-02-22 01:21:02)
...
### Assistant (2026-02-22 01:21:04)

# new psi gamma list: use peaks_psi_B gammas
new_gammas = np.array([p["gamma"] for p in peaks["psi"]["rangeB"]], dtype=float)
# nearest distances for each historical
nearest_dist = []
for g in historical_gammas:
    nearest_dist.append(float(np.min(np.abs(new_gammas - g))))
nearest_dist = np.array(nearest_dist)
eps1 = float(np.mean(nearest_dist <= 1.0))
eps2 = float(np.mean(nearest_dist <= 2.0))
eps1, eps2, nearest_dist[:10]

### Tool (2026-02-22 01:21:07)

(0.08,
 0.12,
 array([4.67779328e+00, 1.83213570e+01, 2.33889664e+00, 6.23705771e+00,
        7.01668992e+00, 4.67779328e+00, 7.10542736e-15, 3.50834496e+00,
        5.84724160e+00, 1.05250349e+01]))

### Assistant (2026-02-22 01:21:11)

band_lo, band_hi = 0.0075, 0.0085
def band_info(peaks_list):
    band = [p for p in peaks_list if (p["f"]>=band_lo and p["f"]<=band_hi)]
    if not band:
        return {"has_peak": False}
    # strongest by relative_power? But relative_power normalized within selected peaks; compare original? We'll use relative_power.
    best = max(band, key=lambda d: d["relative_power"])
    return {"has_peak": True, "strongest": best}
band = {}
for sig in ["psi","logfactorial","cox"]:
    band[sig]={}
    for r in ["rangeA","rangeB"]:
        band[sig][r]=band_info(peaks[sig][r])
band

### Tool (2026-02-22 01:21:15)

{'psi': {'rangeA': {'has_peak': True,
   'strongest': {'f': 0.008056640625,
    'gamma': 205.82290428859793,
    'relative_power': 0.04239497749880892}},
  'rangeB': {'has_peak': True,
   'strongest': {'f': 0.008056640625,
    'gamma': 205.82290428859793,
    'relative_power': 0.04239497749880892}}},
 'logfactorial': {'rangeA': {'has_peak': False},
  'rangeB': {'has_peak': False}},
 'cox': {'rangeA': {'has_peak': True,
   'strongest': {'f': 0.008056640625,
    'gamma': 205.82290428859793,
...
out = {
  "params": {
      ...
      "spectrum_scan_ranges_cycles_per_sample": {"A":[0.0,0.02],"B":[0.0,0.05]},
      "welch": {"window":"hann","nperseg":4096,"noverlap":2048,"scaling":"density"},
      "peak_extract": {"M_target":60,"min_separation_bins":2},
      "cox_process": {
          "method":"per_step Poisson counts with mean ≈ step/log(midpoint); points uniform within each step interval",
          "intensity_lambda_x":"1/log(x)",
          "prime_powers_included": False
      },
      "logfactorial_baseline": {
          "S(x)":"log(floor(x)!) via gammaln(floor(x)+1)",
          "residual":"S(x) - x log x + x"
      }
  },
  "peaks": {
      "psi": peaks["psi"],
      "logfactorial": peaks["logfactorial"],
      "cox": peaks["cox"]
  },
  "psi_log_spectrum_check": {
      "historical_gammas": historical_gammas,
      "eps_1_match_rate": eps1,
      "eps_2_match_rate": eps2,
      "nearest_distances": nearest_dist.tolist()
  },
  "band_f0p008": band
}
len(json.dumps(out))

### Tool (2026-02-22 01:21:25)

20304

### Assistant (2026-02-22 01:21:27)

# Ensure floats are plain python types
print(json.dumps(out, ensure_ascii=False, separators=(',',':'))[:500])

### Tool (2026-02-22 01:21:29)
