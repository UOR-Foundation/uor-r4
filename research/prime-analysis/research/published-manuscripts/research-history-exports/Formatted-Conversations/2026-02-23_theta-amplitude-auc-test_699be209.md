# Theta Amplitude AUC Test

- **Conversation ID:** `699be209-a6ac-8327-b759-e6549db036bb`
- **Date:** 2026-02-23

---

### User (2026-02-23 00:13:50)
...
### Assistant (2026-02-23 00:13:53)

import numpy as np, os, hashlib, json, math
...
feat_path=os.path.join(base,"A4_FEATURECACHE_h25_AR80_AR40_theta_omega_masks_v1.npz")
psi_path=os.path.join(base,"chebyshev_psi.npz")
assert os.path.exists(canon_path) and os.path.exists(feat_path) and os.path.exists(psi_path)

canon=np.load(canon_path, allow_pickle=True)
feat=np.load(feat_path, allow_pickle=True)
list(canon.files)[:20], list(feat.files)[:20], canon['dt'], canon['train_end'], canon['x_kept'].shape, canon['R'].shape

### Tool (2026-02-23 00:14:25)
...
### Assistant (2026-02-23 00:14:30)

def sha1_bytes(x: np.ndarray):
    return hashlib.sha1(np.ascontiguousarray(x).view(np.uint8)).hexdigest()

dt=float(canon['dt'])
N_kept=int(canon['x_kept'].shape[0])
train_end=int(canon['train_end'])
dt_feat=float(feat['dt'])
N_feat=int(feat['N_kept'])
train_end_feat=int(feat['train_end'])
sha_R_canon=sha1_bytes(canon['R'])
sha_R_feat=sha1_bytes(feat['R'])
sha_theta_canon=sha1_bytes(canon['theta_mod'])
sha_theta_feat=sha1_bytes(feat['theta'])
dt, N_kept, train_end, dt_feat, N_feat, train_end_feat, sha_R_canon, sha_R_feat, sha_theta_canon, sha_theta_feat

### Tool (2026-02-23 00:14:34)
...
idx=np.where(kept)[0]
x_kept_new=x_uni[idx]
u_raw=u_interp[idx]
N_new=len(idx)
train_end_new=int(np.floor(0.7*N_new))
dt_new, N_new, train_end_new

### Tool (2026-02-23 00:14:44)

(0.00012297217348543395, 36592, 25614)

### Assistant (2026-02-23 00:14:50)
...
W=2000
u=u_raw - causal_moving_average(u_raw, W)
# standardize train only
mu=u[:train_end_new].mean()
sd=u[:train_end_new].std(ddof=0)
u_z=(u-mu)/sd
# bandpass rfft
bp_lo, bp_hi = 0.003, 0.009
U=np.fft.rfft(u_z)
freq=np.fft.rfftfreq(N_new, d=1.0)  # cycles/sample
mask=(freq>=bp_lo)&(freq<=bp_hi)
U_bp=U*mask
u_bp=np.fft.irfft(U_bp, n=N_new)
# analytic
z=hilbert(u_bp)
theta=np.mod(np.angle(z), 2*np.pi)
A=np.abs(z)
logA=np.log(A + 1e-12)
dlogA=np.empty_like(logA)
dlogA[0]=0.0
dlogA[1:]=logA[1:]-logA[:-1]
# standardize logA and dlogA train-only
def zscore_train(x):
    mu=x[:train_end_new].mean()
    sd=x[:train_end_new].std(ddof=0)
    return (x-mu)/sd, mu, sd
logA_z, logA_mu, logA_sd = zscore_train(logA)
dlogA_z, dlogA_mu, dlogA_sd = zscore_train(dlogA)

theta_sha1=sha1_bytes(theta)
theta_sha1, logA_mu, logA_sd

### Tool (2026-02-23 00:14:58)

('c888f75abc219749cde8f870c119a6dcd98ec2b4',
 -1.2789475404851443,
 0.7728016300756664)

### Assistant (2026-02-23 00:15:03)

R=feat['R'].astype(float)
h=50
min_i=79
N=N_new
train_mask=(np.arange(N)>=min_i) & (np.arange(N)<train_end_new) & (np.arange(N)+h<train_end_new)
test_mask=(np.arange(N)>=min_i) & (np.arange(N)>=train_end_new) & (np.arange(N)+h<N)
train_idx=np.where(train_mask)[0]
test_idx=np.where(test_mask)[0]
y=np.zeros(N, dtype=int)
valid=np.arange(N)+h<N
y[valid]= (R[np.arange(N)[valid]+h]-R[np.arange(N)[valid]] > 0).astype(int)
n_train=len(train_idx); n_test=len(test_idx)
pos_rate_test=float(y[test_idx].mean())
n_train, n_test, pos_rate_test

### Tool (2026-02-23 00:15:07)

(25485, 10928, 0.5058565153733529)

### Assistant (2026-02-23 00:15:14)

X80=feat['X80'].astype(float)
X80.shape, X80[train_idx].mean(), X80[train_idx].std()

### Tool (2026-02-23 00:15:17)

((36592, 80), -0.022754984645613353, 0.49318892196815867)

### Assistant (2026-02-23 00:15:24)

from sklearn.preprocessing import StandardScaler

def fit_logit_predict_auc(X_train, y_train, X_test, y_test):
    # use liblinear maybe stable for small features; for 80 use lbfgs
...
sinth=np.sin(theta)
costh=np.cos(theta)
phase=np.column_stack([sinth, costh])
amp=np.column_stack([logA_z, dlogA_z])

X0=X80
X1=np.hstack([X80, phase])
X2=np.hstack([X80, amp])
X3=np.hstack([X80, amp, phase])

y_train=y[train_idx]; y_test=y[test_idx]
auc0, m0, p0 = fit_logit_predict_auc(X0[train_idx], y_train, X0[test_idx], y_test)
auc1, m1, p1 = fit_logit_predict_auc(X1[train_idx], y_train, X1[test_idx], y_test)
auc2, m2, p2 = fit_logit_predict_auc(X2[train_idx], y_train, X2[test_idx], y_test)
auc3, m3, p3 = fit_logit_predict_auc(X3[train_idx], y_train, X3[test_idx], y_test)
auc0, auc1, auc2, auc3, auc1-auc0, auc3-auc2

### Tool (2026-02-23 00:15:31)

(0.8293632081792357,
 0.8532840555823552,
 0.8267006686498365,
 0.851029070858123,
 0.023920847403119505,
 0.02432840220828647)

### Assistant (2026-02-23 00:15:59)

def score_stat(y, p, X):
    # y, p shape (n,), X shape (n,k)
    r=(y-p)[:,None]
    U=(X*r).sum(axis=0)  # k
    w=(p*(1-p))
    I = (X.T * w) @ X
...
T_real, U_real, I_real = score_stat(y_test.astype(float), p2, phase[test_idx])
p_chi2=float(1-st.chi2.cdf(T_real, df=2))
T_real, p_chi2

### Tool (2026-02-23 00:16:06)

(1016.0236462366297, 0.0)

### Assistant (2026-02-23 00:16:10)

p_chi2_sf=float(st.chi2.sf(T_real, df=2))
p_chi2_sf

### Tool (2026-02-23 00:16:13)

2.361937917049155e-221

### Assistant (2026-02-23 00:16:19)

seed=12345
rng=np.random.default_rng(seed)
min_shift=50
reps=300
shifts=rng.integers(low=min_shift, high=N-min_shift, size=reps, endpoint=False)
T_null=[]
theta_arr=theta
for s in shifts:
    th_shift=np.roll(theta_arr, s)
    phase_shift=np.column_stack([np.sin(th_shift), np.cos(th_shift)])
    T,_U,_I=score_stat(y_test.astype(float), p2, phase_shift[test_idx])
    T_null.append(T)
T_null=np.array(T_null)
emp_p = float((1 + np.sum(T_null >= T_real)) / (1 + reps))
np.percentile(T_null,95), np.percentile(T_null,99), emp_p, T_null.max(), T_null.mean()

### Tool (2026-02-23 00:16:25)
...
out={
 ...
 "auc":{
   "M0_AR80":float(auc0),
   "M1_AR80_phase":float(auc1),
   "M2_AR80_amp":float(auc2),
   "M3_AR80_amp_phase":float(auc3),
   "delta_phase_over_AR80":float(auc1-auc0),
   "delta_phase_over_AR80_amp":float(auc3-auc2)
 },
 "score_test_phase_over_amp":{
   "T_chi2":float(T_real),
   "df":2,
   "p_value_chi2":float(p_chi2_sf)
 },
 "null":{
   "replicates":reps,
   "T_null_p95":float(np.percentile(T_null,95)),
   "T_null_p99":float(np.percentile(T_null,99)),
   "empirical_p_right_tail":emp_p
 },
 "notes":(
   "Identity cross-checks vs A4 canonical: dt/N_kept/train_end match; "
   f"R_sha1={sha_R_feat} matches canonical; featurecache theta_sha1={sha_theta_feat}. "
   "Bandpass for theta_new+amp was [0.003,0.009] cycles/sample; phase features are sin/cos(theta_new). "
   "Amp features (logA,dlogA) were TRAIN-zscored and not shifted in null; null shifts theta only (min_shift=50), baseline p fixed from M2."
 )
}
json.dumps(out, indent=2)[:400]

### Tool (2026-02-23 00:16:38)
