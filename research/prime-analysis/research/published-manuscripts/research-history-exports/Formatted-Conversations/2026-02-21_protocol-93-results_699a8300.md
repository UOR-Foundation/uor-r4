# Protocol 9.3 Results

- **Conversation ID:** `699a8300-bdf8-8325-b6a1-03a26febdd9c`
- **Date:** 2026-02-21

---

### User (2026-02-21 23:16:05)
...
### Assistant (2026-02-21 23:16:07)

import numpy as np, math, json, os, numpy.fft as fft
...
L=65536
# Step1: interpolate psi_minus_x onto uniform t grid length L
...
# Step2: restrict indices where x=exp(t) in [1e5,9e6]
...
# Step4 causal detrend W=2000: u_det = u_raw - causal moving average
W=2000
# compute causal moving average efficiently via cumsum
c = np.cumsum(np.insert(u_raw,0,0.0))
# for i, average over last min(W,i+1) samples
# compute start indices
idx = np.arange(n)
start = np.maximum(0, idx-W+1)
# sums over [start, i] inclusive => c[i+1]-c[start]
sums = c[idx+1]-c[start]
lens = idx-start+1
ma = sums/lens
u_det = u_raw - ma

# Step5 standardize using TRAIN mean/std
mu = u_det[train_slice].mean()
sd = u_det[train_slice].std(ddof=0)
u = (u_det - mu)/sd

sigma_list = [0.00,0.01,0.02,0.05]
seed=12345

def bandpass_fft(sig, f_lo, f_hi):
    # sig 1D real, returns real bandpassed by zeroing outside [f_lo,f_hi] and symmetric
    n = len(sig)
    X = fft.rfft(sig)
...
def hilbert_phase(sig):
    # analytic signal via FFT method: construct hilbert transform
    n = len(sig)
    X = fft.fft(sig)
    h = np.zeros(n)
    if n%2==0:
...
def omega_peak(test_theta):
    omega = np.diff(test_theta)
    m = len(omega)
    # FFT power spectrum
    O = fft.rfft(omega)
    P = (np.abs(O)**2)
    freqs = fft.rfftfreq(m, d=1.0)
    # search in [0.002,0.05]
    band = (freqs>=0.002) & (freqs<=0.05)
    if not np.any(band):
        return float('nan'), float('nan')
    P_band = P[band]
    f_band = freqs[band]
    k = int(np.argmax(P_band))
    peak_f = float(f_band[k])
    peak_pow = float(P_band[k])
    rel = float(peak_pow / (P_band.max() if P_band.max()>0 else 1.0))
    return peak_f, rel

# Baseline sigma=0
u0 = u.copy()
u0_bp = bandpass_fft(u0, 0.002, 0.01)
theta0 = hilbert_phase(u0_bp)

results=[]
for j,sig in enumerate(sigma_list):
    rng = np.random.default_rng(seed + 1000*j + 17)
    if sig==0.0:
        u_noisy = u0
    else:
        noise = rng.normal(0.0, sig, size=u.shape)  # since u has unit std on TRAIN, sigma*std_train(u)=sigma
        u_noisy = u + noise
    u_bp = bandpass_fft(u_noisy, 0.002, 0.01)
    theta = hilbert_phase(u_bp)
    # circular corr on TEST with theta0
    dtheta = theta[test_slice] - theta0[test_slice]
    corr = float(np.mean(np.cos(dtheta)))
    peak_f, rel = omega_peak(theta[test_slice])
    results.append({
        "sigma": float(sig),
        "circular_corr_test": corr,
        "omega_peak_freq_test": peak_f,
        "omega_peak_rel_power_test": rel
    })

out = {
    "params": {"L": L, "dt": float(dt), "W": 2000, "bandpass": [0.002,0.01], "sigma_list": sigma_list, "seed": seed},
    "results": results
}
print(json.dumps(out, separators=(',',':')))

### Tool (2026-02-21 23:16:37)
