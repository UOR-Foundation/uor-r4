...
- **Conversation ID:** `699b8288-2b40-8326-8cb9-777ff5bed566`
- **Date:** 2026-02-22

---

### User (2026-02-22 17:26:19)
...
cheb.files, pr.files[:10], pr.files

### Tool (2026-02-22 17:26:52)

(['N', 'step', 'x_grid', 'psi_grid', 'psi_minus_x'],
 ['N', 'primes', 'is_prime'],
 ['N', 'primes', 'is_prime'])

### Assistant (2026-02-22 17:26:55)
...
def causal_moving_average(x, W):
    # past-only inclusive window size W
    n=len(x)
    c=np.cumsum(np.insert(x,0,0.0))
    ma=np.empty(n, dtype=float)
    for i in range(n):
        j0=max(0, i-W+1)
        ma[i]=(c[i+1]-c[j0])/(i-j0+1)
    return ma
...
def compute_R(x, primes, Delta):
    # x array float
    # intervals [x_i, x_i*exp(Delta))
    hi = x * math.exp(Delta)
...
def run_one(L, x_range=(1e5,9e6), Delta=0.01, train_frac=0.7, detrend_W=2000, band=(0.002,0.01), seed=12345, horizons=(25,100), q_list=(0,30), K=3):
    tmin=float(np.min(np.log(x_grid)))
    tmax=float(np.max(np.log(x_grid)))
    t=np.linspace(tmin, tmax, L)
    dt=(tmax-tmin)/(L-1)
    x=np.exp(t)
    # interpolate psi_minus_x from x_grid
    u_raw=np.interp(x, x_grid, psi_minus_x)
    ...
    # detrend causal
    ma=causal_moving_average(uk_raw, detrend_W)
    u=uk_raw-ma
    ...
    u_z=(u-mu)/sd
    # bandpass
    u_bp=fft_bandpass(u_z, band[0], band[1])
    # hilbert phase
    z=scipy.signal.hilbert(u_bp)
    theta=np.mod(np.angle(z), 2*np.pi)
    # residual R
    R=compute_R(xk, primes, Delta)
    out={"dt": dt, "N_kept": int(N), "train_end": int(train_end), "by_h": {}}
    rng=np.random.default_rng(seed)
    for h in horizons:
        h=int(h)
        y=np.zeros(N, dtype=np.int8)
        valid_y = np.arange(N-h)
        y[:N-h] = (R[h:]-R[:N-h] > 0).astype(np.int8)
        # last h entries are meaningless; we'll exclude via i+h<N mask
        out_h={"by_q": {}}
        for q in q_list:
            q=int(q)
            # valid indices i
            i=np.arange(N)
            valid = (i-q>=0) & (i+h < N)
            # train-valid and test-valid per spec
            train_valid = valid & (i < train_end) & ((i-q) < train_end) & ((i+h) < train_end)
            test_valid  = valid & (i >= train_end) & ((i-q) >= train_end) & ((i+h) < N)
            y_train=y[train_valid]
            y_test=y[test_valid]
            res={
                "n_train_valid": int(y_train.size),
                "n_test_valid": int(y_test.size),
                "train_pos_rate": None if y_train.size==0 else float(np.mean(y_train)),
                "test_pos_rate": None if y_test.size==0 else float(np.mean(y_test)),
                "auc_phase_model": None,
                "auc_sanity_Ri": None,
                "fit_status": "ok"
            }
            # sanity auc with R_i on test-valid
            if y_test.size>0 and len(np.unique(y_test))>=2:
                res["auc_sanity_Ri"]=float(roc_auc_score(y_test, R[test_valid]))
            else:
                if y_test.size>0:
                    res["auc_sanity_Ri"]=None
            # phase model
            if y_train.size==0:
                res["fit_status"]="no_train_samples"
            elif len(np.unique(y_train))<2:
                res["fit_status"]="train_single_class"
            elif y_test.size==0:
                res["fit_status"]="no_test_samples"
            elif len(np.unique(y_test))<2:
                res["fit_status"]="test_single_class"
            else:
                try:
                    theta_lag_train=theta[train_valid - 0]  # placeholder
                except Exception as e:
                    pass
                try:
                    theta_lag_train=theta[(np.where(train_valid)[0]-q)]
                    X_train=build_features(theta_lag_train)
                    clf=LogisticRegression(solver="lbfgs", max_iter=2000, random_state=seed)
                    clf.fit(X_train, y_train)
                    theta_lag_test=theta[(np.where(test_valid)[0]-q)]
                    X_test=build_features(theta_lag_test)
                    p=clf.predict_proba(X_test)[:,1]
                    res["auc_phase_model"]=float(roc_auc_score(y_test, p))
                    res["fit_status"]="ok"
                except Exception as e:
                    res["fit_status"]=type(e).__name__
                    res["auc_phase_model"]=None
            out_h["by_q"][str(q)]=res
        out["by_h"][str(h)]=out_h
    return out

# quick run for L=32768 first to ensure works
res32768 = run_one(32768)
res32768["by_h"]["25"]["by_q"]["0"]

### Tool (2026-02-22 17:27:35)
...
### Assistant (2026-02-22 17:27:39)

res32768["by_h"]["25"]["by_q"]["30"], res32768["by_h"]["100"]["by_q"]["0"], res32768["by_h"]["100"]["by_q"]["30"]

### Tool (2026-02-22 17:27:43)
...
### Assistant (2026-02-22 17:27:45)

res131072 = run_one(131072)
res131072["by_h"]["25"]["by_q"]["0"], res131072["by_h"]["25"]["by_q"]["30"], res131072["by_h"]["100"]["by_q"]["0"], res131072["by_h"]["100"]["by_q"]["30"]

### Tool (2026-02-22 17:27:53)
...
### Tool (2026-02-22 17:28:19)

3849

### Assistant (2026-02-22 17:28:27)
