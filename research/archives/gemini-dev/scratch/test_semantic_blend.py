...
def retrieve_geometric_resonance_with_blend(prompt_text, routing_data, blend_glove=True, top_n=5):
    ...
    for r in routing_data["all_routes"]:
        ...
        if blend_glove:
            # Inject the GloVe components (dims 0..50) of the query vector S
    ...
    for win_idx, items in corpus_index.items():
        ...
        # Slice norm is computed over the full vector or slice?
        # Let's keep it as is, or use the blended representation
        ...
        for item in items:
            ...
            if blend_glove:
                # Reconstruct the GloVe part of the sentence words
...
print("==================================================")
print("WITHOUT GLOVE BLENDING IN RETRIEVAL:")
routing_data = route_query_to_manifold(query)
res_no_blend = retrieve_geometric_resonance_with_blend(query, routing_data, blend_glove=False)
for s, rel, win, sim, snorm in res_no_blend:
    print(f"  Score: {rel:.6f} | Sim: {sim:.4f} | Sentence: '{s}'")

print("==================================================")
print("WITH GLOVE BLENDING IN RETRIEVAL:")
