...
# Load server module or copy the relevant math from server.py
# Let's inspect the math by importing server functions or copy-pasting
# Let's just copy the relevant functions and load manifold_cache.json
# since we want to run this independently.
...
# Let's define the projection logic
M_MAX = 512
NUM_WINDOWS = 16
N_SAMPLES = 257
SPARSE_RADIUS = 0.3
...
for idx, x in enumerate(x_grid):
    ...
    # Orthonormal basis Q (we can generate it since it's deterministic)
    H = 4.0 * np.sqrt(x)
...
def retrieve_geometric_resonance(prompt_text, routing_data, top_n=5):
...
# Trace the query "explain how the r4 router works in this project"
query = "explain how the r4 router works in this project"
print(f"Tracing query: '{query}'")
words_in_query = [w.lower().strip(".,?!()\"';:-") for w in query.split() if w.strip()]
for w in words_in_query:
    print(f"Word in query: '{w}', in vocab_vectors: {w in vocab_vectors}")

routing_data = route_query_to_manifold(query)
print(f"Routed to window: {routing_data['routed']['window_index']}")

resonances = retrieve_geometric_resonance(query, routing_data)
print("\nTop 5 resonant sentences:")
for s, rel, win, sim, snorm in resonances:
    print(f"Score: {rel:.6f} | Sim: {sim:.4f} | SliceNorm: {snorm:.4f} | Win: {win} | Sentence: '{s}'")
