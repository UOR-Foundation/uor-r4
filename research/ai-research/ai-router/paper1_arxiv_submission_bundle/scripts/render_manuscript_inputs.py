...
def render_tables() -> None:
    ...
    setup_lines = [
        ...
        r"Primary routing metric & $e_{\mathrm{eff}} = \exp\!\left(-\sum_{r=1}^{K} p_r \log p_r\right)$ \\",
        r"\bottomrule",
        r"\end{tabular}",
    ]
...
def render_method_figure() -> None:
    ...
    label_fs = 10.8
    ax.text(0.23, 0.13, r"$x \in \mathbb{R}^{100}$", ha="center", fontsize=label_fs)
    ...
    for idx, xpos in zip([3, 65, 2, 21], [0.67, 0.77, 0.87, 0.97]):
        ...
        ax.add_patch(rect)
        ax.text(xpos, 0.57, f"{idx}", ha="center", va="center", fontsize=11.0, fontweight="bold")
    ax.text(0.82, 0.79, "fixed chart", ha="center", fontsize=label_fs)
    ax.text(0.82, 0.72, f"$|\\rho| \\approx {pair1:.3f}, {pair2:.3f}$", ha="center", fontsize=10.2)
    ax.text(0.82, 0.13, r"$(3,65,2,21)$", ha="center", fontsize=label_fs)
    ...
    ax.set_ylim(0, 1)
    ax.axis("off")
    ax.text(0.04, 0.89, r"$q_1=a+ib,\ \ q_2=c+id$", ha="left", fontsize=11.4)
    ax.text(0.04, 0.80, r"phases $(\theta_1,\theta_2)$", ha="left", fontsize=label_fs)
    ...
    ax.text(0.24, 0.78, r"$\theta_1$", color=BLUE, ha="center", fontsize=label_fs)
    ax.add_patch(FancyArrowPatch((0.44, 0.61), (0.44, 0.41), arrowstyle="->", mutation_scale=11, linewidth=1.3, color=TEAL))
    ax.text(0.49, 0.51, r"$\theta_2$", color=TEAL, va="center", fontsize=label_fs)
    ...
    ax.text(0.05, 0.20, r"base $(\eta,\delta)$", fontsize=label_fs)
    ax.text(0.05, 0.10, r"fiber $\alpha$", fontsize=label_fs)
    ...
    ax.set_ylim(0, 1)
    ax.axis("off")
    ax.text(0.04, 0.89, r"$\tilde{\alpha} = \alpha + \frac{1}{2}\lambda\cos(2\eta)\delta$", fontsize=10.8)
    ax.text(0.04, 0.80, r"transport-corrected phase", fontsize=label_fs)
    ...
    for y, label, n, color in zip(yrows, labels, counts, colors):
        ax.text(0.05, y, label, fontsize=label_fs, color=color, va="center")
        x0, width = 0.16, 0.31
    ...
    left, bottom, width, height = 0.62, 0.21, 0.36, 0.53
    ...
    ax.text(0.80, 0.11, r"route index $r(x)$", ha="center", fontsize=label_fs)
