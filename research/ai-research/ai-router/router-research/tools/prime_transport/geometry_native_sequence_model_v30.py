#!/usr/bin/env python3
"""Explicit structured chart/regime field over contiguous regions on the shifted family."""
...
BLOCK_COUNT_V30 = 6
MAX_REGIONS_V30 = 3
TRANSITION_PENALTY_V30 = 0.35


@dataclass
class SequenceMetricsV30:
    model: str
    test_loss: float
    test_accuracy: float
...
class StructuredChartField(torch.nn.Module):
    def __init__(self, feature_dim: int, label_count: int, *, seed: int) -> None:
        ...
        self.out = torch.nn.Conv1d(24, label_count, kernel_size=1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return self.out(contextual).transpose(1, 2)
...
def _block_bounds(seq_len: int, *, block_count: int = BLOCK_COUNT_V30) -> list[tuple[int, int]]:
    cut_points = np.linspace(0, seq_len, block_count + 1, dtype=np.int64)
    bounds: list[tuple[int, int]] = []
    for idx in range(block_count):
        start = int(cut_points[idx])
        end = int(cut_points[idx + 1])
        if end <= start:
            end = start + 1
        bounds.append((start, min(end, seq_len)))
    bounds[-1] = (bounds[-1][0], seq_len)
    return bounds


def _block_details_v30(
    ...
    *,
    ...
    bounds = _block_bounds(len(raw_snapshots))
...
def _build_training_set_v30(
    ...
    *,
    ...
    for row in range(calibration_inputs.shape[0]):
        ...
        block_features, block_caches, bounds = _block_details_v30(
            raw_snapshots,
            model,
...
def _fit_structured_chart_field(
    train_x: np.ndarray,
    train_y: np.ndarray,
    *,
    seed: int,
    epochs: int = 180,
    lr: float = 0.02,
) -> StructuredChartField:
    field = StructuredChartField(train_x.shape[-1], len(REGION_VARIANTS_V22), seed=seed)
...
def _decode_contiguous_chart_path_v30(
    logits: np.ndarray,
    *,
    transition_penalty: float = TRANSITION_PENALTY_V30,
    max_regions: int = MAX_REGIONS_V30,
) -> list[int]:
...
def extract_geometry_features_v30(
    inputs: np.ndarray,
    model: GeometryNativeSequenceModelV3,
    *,
    support_window: int = 10,
    calibration_size: int = 256,
    calibration_seed: int = 269,
    field_seed: int = 0,
    ...
    train_x, train_y = _build_training_set_v30(
        calibration_inputs,
        calibration_targets,
    ...
    chart_field = _fit_structured_chart_field(train_x, train_y, seed=field_seed)
    ...
    for row in range(size):
        ...
        labels = _decode_contiguous_chart_path_v30(logits)
        regions = _collapse_regions_v30(labels, bounds)
...
def run_bounded_sequence_comparison_v30(config: TaskConfigV27, *, seed: int = 0) -> list[SequenceMetricsV30]:
    ...
    test_features, field_param_count = extract_geometry_features_v30(
        test_inputs,
        geometry_model,
        calibration_seed=seed + 71,
        field_seed=seed + 79,
    )
    geometry_test_loss, geometry_test_acc, geometry_transfer_query_acc, geometry_eval_seconds = evaluate_geometry_v30(
        geometry_model,
        test_features,
        test_targets,
        test_query_mask,
    ...
    total_state_size = geometry_model.effective_state_size + BLOCK_COUNT_V30 + MAX_REGIONS_V30

    return [
        SequenceMetricsV30(
            model="geometry_native_sequence_model_v30",
        ...
        SequenceMetricsV30(
            model="tiny_transformer_baseline_v30",
