#!/usr/bin/env python3
"""Twelfth operator formulation rebuilding T_c directly from spin_H_core_v4."""
...
def coupled_torus_kick_component_v16(state: OperatorStateV10) -> OperatorStateV10:
    core = active_transport_lift_core_v4(state)
    sigma_successor = sigma_update_v4(core.sigma, "coupled_torus_kick")
...
class SparseLawfulOperatorV16:
    """Explicit sparse operator with T_c rebuilt from spin_H_core_v4."""
    ...
    def _is_lawful(source: OperatorStateV10, target: OperatorStateV10, component: str) -> bool:
        ...
        if component == "coupled_torus_kick":
            expected = coupled_torus_kick_component_v16(source)
            return (
                target == expected
...
def bounded_operator_surface_v16(depth: int = 8) -> tuple[tuple[OperatorStateV10, ...], tuple[OperatorTransitionV1, ...]]:
    operator = SparseLawfulOperatorV16()
...
def summarize_operator_from_spinH_core_v4(depth: int = 8) -> list[dict[str, object]]:
    old = _summary_metrics_from_v13()
    states, transitions = bounded_operator_surface_v16(depth=depth)
    ...
    rows.append({"scope": "operator", "metric": "state_count", "count": len(states), "total": len(states), "fraction": 1.0, "note": "reachable states with T_c rebuilt from spin_H_core_v4"})
    rows.append({"scope": "operator", "metric": "transition_count", "count": len(transitions), "total": len(transitions), "fraction": 1.0, "note": "nonzero operator entries with one core-v4-derived component"})
    ...
    rows.append({"scope": "core_dependency", "metric": "rebuilt_component_depends_on_spin_H_core_v4", "count": 1, "total": 1, "fraction": 1.0, "note": "T_c calls active_transport_lift_core_v4 and uses sigma_direct with sigma_update_v4"})
    rows.append({"scope": "core_dependency", "metric": "sigma_direct_materially_used", "count": 1, "total": 1, "fraction": 1.0, "note": "T_c uses sigma_update_v4 on sigma_direct and reads current_mode, fiber_mode, radial_mode, and regressive_phase"})
    ...
    rows.append({"scope": "comparison_vs_v13", "metric": "state_count_change", "count": len(states) - int(old["state_count__count"]), "total": int(old["state_count__count"]), "fraction": (len(states) / max(old["state_count__count"], 1.0)) - 1.0, "note": "reachable state change vs prior T_c rebuild from spin_H_core_v3"})
    rows.append({"scope": "comparison_vs_v13", "metric": "transition_count_change", "count": len(transitions) - int(old["transition_count__count"]), "total": int(old["transition_count__count"]), "fraction": (len(transitions) / max(old["transition_count__count"], 1.0)) - 1.0, "note": "transition count change vs prior T_c rebuild from spin_H_core_v3"})
    rows.append({"scope": "comparison_vs_v13", "metric": "class_diversity_change", "count": len(target_class_counter) - int(old["distinct_class_identities_reached__count"]), "total": int(old["distinct_class_identities_reached__count"]), "fraction": (len(target_class_counter) / max(old["distinct_class_identities_reached__count"], 1.0)) - 1.0, "note": "class diversity change vs prior T_c rebuild from spin_H_core_v3"})
    rows.append({"scope": "comparison_vs_v13", "metric": "spin_diversity_change", "count": len(target_spin_counter) - int(old["distinct_spin_classes_reached__count"]), "total": int(old["distinct_spin_classes_reached__count"]), "fraction": (len(target_spin_counter) / max(old["distinct_spin_classes_reached__count"], 1.0)) - 1.0, "note": "spin diversity change vs prior T_c rebuild from spin_H_core_v3"})
    rows.append({"scope": "comparison_vs_v13", "metric": "radial_diversity_change", "count": len(radial_counter) - int(old["distinct_radial_classes_reached__count"]), "total": int(old["distinct_radial_classes_reached__count"]), "fraction": (len(radial_counter) / max(old["distinct_radial_classes_reached__count"], 1.0)) - 1.0, "note": "radial diversity change vs prior T_c rebuild from spin_H_core_v3"})
...
def write_operator_from_spinH_core_v4(
    rows: list[dict[str, object]],
    output_path: Path = OUTPUT_PATH_OPERATOR_FROM_CORE_V4,
) -> None:
...
__all__ = [
    ...
    "SparseLawfulOperatorV16",
    "bounded_operator_surface_v16",
