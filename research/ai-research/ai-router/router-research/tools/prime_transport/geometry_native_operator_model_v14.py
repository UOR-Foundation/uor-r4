#!/usr/bin/env python3
"""Tenth operator formulation rebuilding T_z' directly from spin_H_core_v3."""
...
def _binary_spin_value_v14(word: tuple[int, ...]) -> int:
    return int("".join(str(int(bit)) for bit in word), 2)


def _projection_profile_map_v14(state: OperatorStateV10) -> dict[str, tuple[int, ...]]:
    core = active_transport_lift_core_v3(state)
    return dict(core.sigma.global_regressive_mode_index.generator_projection_profile)


def _orbit_profile_map_v14(state: OperatorStateV10) -> dict[str, tuple[tuple[int, ...], ...]]:
    core = active_transport_lift_core_v3(state)
    return dict(core.sigma.global_regressive_mode_index.generator_orbit_profile)


def fiber_phase_lift_component_v14(state: OperatorStateV10) -> OperatorStateV10:
    core = active_transport_lift_core_v3(state)
    profile_map = _projection_profile_map_v14(state)
    orbit_map = _orbit_profile_map_v14(state)

    source_tau = project_tau_v3(core)
    lift_projection = profile_map["fiber_phase_lift_spin_transport"]
    lift_orbit = orbit_map["fiber_phase_lift_spin_transport"]
    ...
    h_weight = sum(core.h.recursive_phase) + sum(core.h.fiber_recursive_phase) + core.h.holonomy_bit
    phi_step = 1 + ((theta_weight + rho_weight + sigma_weight + h_weight) % 2)

    target_phi = (state.phi + phi_step) % 3
    target_spin = NativeSpinHV1(horizon=state.spin_h.horizon, bits=tuple(int(bit) for bit in lift_projection))
    target_tau = NativeTauV3(
        swap_phase=source_tau.swap_phase,
        coupled_phase=(source_tau.coupled_phase + core.rho.radial_target + len(lift_orbit)) % 5,
        twist_phase=(source_tau.twist_phase + core.h.holonomy_bit + (core.theta.base_angle % 2)) % 2,
        lift_phase=(source_tau.lift_phase + core.rho.unfolding_load + _binary_spin_value_v14(lift_projection) + core.theta.fiber_phase) % 12,
    )
...
class SparseLawfulOperatorV14:
    """Explicit sparse operator with T_c and T_z' rebuilt from spin_H_core_v3."""
...
def bounded_operator_surface_v14(depth: int = 8) -> tuple[tuple[OperatorStateV10, ...], tuple[OperatorTransitionV1, ...]]:
    operator = SparseLawfulOperatorV14()
...
def summarize_operator_from_spinH_core_v3_lift(depth: int = 8) -> list[dict[str, object]]:
    old = _summary_metrics_from_v13()
    states, transitions = bounded_operator_surface_v14(depth=depth)
    ...
    rows.append({"scope": "operator", "metric": "state_count", "count": len(states), "total": len(states), "fraction": 1.0, "note": "reachable states with T_z' rebuilt from spin_H_core_v3"})
    ...
    rows.append({"scope": "core_dependency", "metric": "rebuilt_component_depends_on_spin_H_core_v3", "count": 1, "total": 1, "fraction": 1.0, "note": "T_z' calls active_transport_lift_core_v3 and reads sigma** global_regressive_mode_index"})
    rows.append({"scope": "core_dependency", "metric": "sigma_parent_mode_identity_materially_used", "count": 1, "total": 1, "fraction": 1.0, "note": "T_z' uses sigma** generator_projection_profile, generator_orbit_profile, and derived mode orbit"})
...
def write_operator_from_spinH_core_v3_lift(
    rows: list[dict[str, object]],
    output_path: Path = OUTPUT_PATH_OPERATOR_FROM_CORE_V3_LIFT,
) -> None:
...
__all__ = [
    ...
    "SparseLawfulOperatorV14",
    "bounded_operator_surface_v14",
