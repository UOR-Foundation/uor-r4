#!/usr/bin/env python3
"""Meso-window contradiction-centered factor repair on the stronger reduced-alignment setting."""
...
def extract_geometry_features_v19(
    inputs: np.ndarray,
    ...
    *,
    ...
    for row in range(size):
        ...
        if repair_center is None or repair_alt is None:
            ...
            half = meso_window // 2
            repair_start = max(0, repair_center - half)
            repair_end = min(seq_len, repair_start + meso_window)
            repair_start = max(0, repair_end - meso_window)
            repaired = _mesorepair_features(
                raw_snapshots,
...
def run_bounded_sequence_comparison_v19(config: TaskConfigV12, *, seed: int = 0) -> list[SequenceMetricsV19]:
    ...
    test_features = extract_geometry_features_v19(test_inputs, geometry_model)
    geometry_test_loss, geometry_test_acc, geometry_transfer_query_acc, geometry_eval_seconds = evaluate_geometry_v19(
        geometry_model,
        test_features,
        test_targets,
        test_query_mask,
    ...
    return [
        SequenceMetricsV19(
            model="geometry_native_sequence_model_v19",
        ...
        SequenceMetricsV19(
            model="tiny_transformer_baseline_v19",
