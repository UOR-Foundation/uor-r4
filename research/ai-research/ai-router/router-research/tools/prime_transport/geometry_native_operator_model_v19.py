#!/usr/bin/env python3
"""Operator formulation rebuilding T_c directly from spin_H_core_v5."""
...
OUTPUT_PATH_OPERATOR_FROM_CORE_V4 = Path(
    "/Users/adminamn/AI-Research/ai-router/router-research/results/"
    "prime_transport_recursive_system/prime_transport_operator_from_spinH_core_v4.csv"
)


def _binary_spin_value_v16(word: tuple[int, ...]) -> int:
    return int("".join(str(int(bit)) for bit in word), 2)


def coupled_torus_kick_component_v19(state: OperatorStateV10) -> OperatorStateV10:
    core = active_transport_lift_core_v5(state)
    sigma_successor = sigma_update_v5(core.sigma, "coupled_torus_kick")
    source_tau = project_tau_v5(core)
    ...
    target_tau = NativeTauV3(
        ...
        lift_phase=(
            ...
            + core.rho.unfolding_load
            + (_binary_spin_value_v16(sigma_successor.current_mode) % 5)
            + (_binary_spin_value_v16(sigma_successor.radial_mode) % 7)
            + sigma_successor.family_holonomy_class
        )
        % 12,
    )
...
class SparseLawfulOperatorV19:
    """Explicit sparse operator with T_c rebuilt from spin_H_core_v5."""
    ...
    def transitions(self, state: OperatorStateV10) -> tuple[OperatorTransitionV1, ...]:
        ...
        coupled_target = coupled_torus_kick_component_v19(state)
    ...
    def _is_lawful(source: OperatorStateV10, target: OperatorStateV10, component: str) -> bool:
        ...
        if component == "coupled_torus_kick":
            expected = coupled_torus_kick_component_v19(source)
            return (
                target == expected
...
def bounded_operator_surface_v19(depth: int = 8) -> tuple[tuple[OperatorStateV10, ...], tuple[OperatorTransitionV1, ...]]:
    operator = SparseLawfulOperatorV19()
...
def _summary_metrics_from_v13() -> dict[str, float]:
    rows = list(csv.DictReader(OUTPUT_PATH_OPERATOR_FROM_CORE_V3.open("r", encoding="utf-8")))
    out: dict[str, float] = {}
    for row in rows:
...
def summarize_operator_from_spinH_core_v5(depth: int = 8) -> list[dict[str, object]]:
    old = list(csv.DictReader(OUTPUT_PATH_OPERATOR_FROM_CORE_V4.open("r", encoding="utf-8")))
    old_metrics: dict[str, float] = {}
    for row in old:
        if row["scope"] in {"operator", "class_escape", "spin_escape", "radial_escape"}:
            old_metrics[f"{row['metric']}__count"] = float(row["count"])
            old_metrics[f"{row['metric']}__fraction"] = float(row["fraction"])
    states, transitions = bounded_operator_surface_v19(depth=depth)
    ...
    rows.append({"scope": "operator", "metric": "state_count", "count": len(states), "total": len(states), "fraction": 1.0, "note": "reachable states with T_c rebuilt from spin_H_core_v5"})
    rows.append({"scope": "operator", "metric": "transition_count", "count": len(transitions), "total": len(transitions), "fraction": 1.0, "note": "nonzero operator entries with one core-v5-derived component"})
    ...
    rows.append({"scope": "core_dependency", "metric": "rebuilt_component_depends_on_spin_H_core_v5", "count": 1, "total": 1, "fraction": 1.0, "note": "T_c calls active_transport_lift_core_v5 and uses sigma_direct under sigma_family_holonomy_law"})
    rows.append({"scope": "core_dependency", "metric": "sigma_direct_materially_used", "count": 1, "total": 1, "fraction": 1.0, "note": "T_c uses sigma_update_v5 on sigma_direct and reads current_mode, fiber_mode, radial_mode, regressive_phase, and family_holonomy_class"})
    ...
    rows.append({"scope": "comparison_vs_v16", "metric": "state_count_change", "count": len(states) - int(old_metrics["state_count__count"]), "total": int(old_metrics["state_count__count"]), "fraction": (len(states) / max(old_metrics["state_count__count"], 1.0)) - 1.0, "note": "reachable state change vs prior T_c rebuild from spin_H_core_v4"})
    rows.append({"scope": "comparison_vs_v16", "metric": "transition_count_change", "count": len(transitions) - int(old_metrics["transition_count__count"]), "total": int(old_metrics["transition_count__count"]), "fraction": (len(transitions) / max(old_metrics["transition_count__count"], 1.0)) - 1.0, "note": "transition count change vs prior T_c rebuild from spin_H_core_v4"})
    rows.append({"scope": "comparison_vs_v16", "metric": "class_diversity_change", "count": len(target_class_counter) - int(old_metrics["distinct_class_identities_reached__count"]), "total": int(old_metrics["distinct_class_identities_reached__count"]), "fraction": (len(target_class_counter) / max(old_metrics["distinct_class_identities_reached__count"], 1.0)) - 1.0, "note": "class diversity change vs prior T_c rebuild from spin_H_core_v4"})
    rows.append({"scope": "comparison_vs_v16", "metric": "spin_diversity_change", "count": len(target_spin_counter) - int(old_metrics["distinct_spin_classes_reached__count"]), "total": int(old_metrics["distinct_spin_classes_reached__count"]), "fraction": (len(target_spin_counter) / max(old_metrics["distinct_spin_classes_reached__count"], 1.0)) - 1.0, "note": "spin diversity change vs prior T_c rebuild from spin_H_core_v4"})
    rows.append({"scope": "comparison_vs_v16", "metric": "radial_diversity_change", "count": len(radial_counter) - int(old_metrics["distinct_radial_classes_reached__count"]), "total": int(old_metrics["distinct_radial_classes_reached__count"]), "fraction": (len(radial_counter) / max(old_metrics["distinct_radial_classes_reached__count"], 1.0)) - 1.0, "note": "radial diversity change vs prior T_c rebuild from spin_H_core_v4"})
...
def write_operator_from_spinH_core_v5(
    rows: list[dict[str, object]],
    output_path: Path = OUTPUT_PATH_OPERATOR_FROM_CORE_V5,
) -> None:
...
__all__ = [
    ...
    "SparseLawfulOperatorV19",
    "bounded_operator_surface_v19",
    "coupled_torus_kick_component_v19",
