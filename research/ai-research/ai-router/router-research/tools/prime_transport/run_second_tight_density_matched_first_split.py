#!/usr/bin/env python3
"""Second tight density-matched family for first-splitting vs arrangement."""
...
def analyze_tuplet(name: str, offsets: tuple[int, ...]) -> dict[str, object]:
    ...
    return {
        ...
        "child_W": CHILD_W,
        "new_prime": NEW_PRIME,
        "parent_admissible_density": float(parent_bits.mean()),
...
def main() -> None:
    rows = [analyze_tuplet(name, offsets) for name, offsets in MATCHED_ROWS]
    y = np.asarray([float(row["first_visible_split_H"]) for row in rows], dtype=np.float64)
    predictors = {
        "num_first_splitting_classes": np.asarray([float(row["num_first_splitting_classes"]) for row in rows], dtype=np.float64),
        "total_extra_child_classes_from_first_split": np.asarray([float(row["total_extra_child_classes_from_first_split"]) for row in rows], dtype=np.float64),
        "fraction_first_splitting_classes": np.asarray([float(row["fraction_first_splitting_classes"]) for row in rows], dtype=np.float64),
        "gap_max": np.asarray([float(row["gap_max"]) for row in rows], dtype=np.float64),
        "gap_span": np.asarray([float(row["gap_span"]) for row in rows], dtype=np.float64),
    }
    ...
    summary_path = OUT_DIR / "visible_threshold_second_tight_density_matched_rows.csv"
    scores_path = OUT_DIR / "visible_threshold_second_tight_density_matched_scores.csv"
    write_csv(summary_path, rows)
    write_csv(scores_path, score_rows)

    best = score_rows[0]
    print(f"best_second_tight_matched_predictor,{best['predictor']}")
    print(f"best_second_tight_matched_spearman,{float(best['spearman_visible_threshold']):.12f}")
