#!/usr/bin/env python3
"""Eleventh operator formulation rebuilding T_r* directly from spin_H_core_v3."""
...
from geometry_native_operator_model_v13 import coupled_torus_kick_component_v13
from geometry_native_operator_model_v14 import (
    OUTPUT_PATH_OPERATOR_FROM_CORE_V3_LIFT,
    fiber_phase_lift_component_v14,
)
...
OUTPUT_PATH_OPERATOR_FROM_CORE_V3_RADIAL = Path(
    "/Users/adminamn/AI-Research/ai-router/router-research/results/"
    "prime_transport_recursive_system/prime_transport_operator_from_spinH_core_v3_radial.csv"
)


def _binary_spin_value_v15(word: tuple[int, ...]) -> int:
    return int("".join(str(int(bit)) for bit in word), 2)


def _projection_profile_map_v15(state: OperatorStateV10) -> dict[str, tuple[int, ...]]:
    core = active_transport_lift_core_v3(state)
    return dict(core.sigma.global_regressive_mode_index.generator_projection_profile)


def _orbit_profile_map_v15(
    state: OperatorStateV10,
) -> dict[str, tuple[tuple[int, ...], ...]]:
    core = active_transport_lift_core_v3(state)
    return dict(core.sigma.global_regressive_mode_index.generator_orbit_profile)


def _rotate_word_v15(word: tuple[int, ...], shift: int) -> tuple[int, ...]:
    if not word:
        return word
...
def radial_transport_component_v15(state: OperatorStateV10) -> OperatorStateV10:
    core = active_transport_lift_core_v3(state)
    profile_map = _projection_profile_map_v15(state)
    orbit_map = _orbit_profile_map_v15(state)

    source_tau = project_tau_v3(core)
    radial_projection = profile_map["radial_transport_unfolding"]
    radial_orbit = orbit_map["radial_transport_unfolding"]
    ...
    reference_orbit = radial_orbit[(core.sigma.radial_residue + core.h.holonomy_bit) % len(radial_orbit)]
    seed_reference = seed_orbit[(core.sigma.current_residue + core.sigma.fiber_residue) % len(seed_orbit)]
    rotated_projection = _rotate_word_v15(
        radial_projection,
        theta_weight + rho_weight + core.sigma.radial_residue + core.h.holonomy_bit,
    )
    inject_index = (
        target_r
        + target_phi
    ...
    target_bits = list(
        (
            int(rotated_projection[idx])
            ^ int(reference_orbit[idx])
            ^ int(seed_reference[idx])
        )
        for idx in range(state.spin_h.horizon)
    )
    target_bits[inject_index] = (
        (theta_weight + rho_weight + sigma_weight + h_weight + _binary_spin_value_v15(radial_projection))
        % 2
    )
    target_spin = NativeSpinHV1(
        horizon=state.spin_h.horizon,
        bits=tuple(int(bit) for bit in target_bits),
    )
    target_tau = NativeTauV3(
        ...
        lift_phase=(
            ...
            + core.rho.unfolding_load
            + target_phi
            + (_binary_spin_value_v15(reference_orbit) % 7)
            + (_binary_spin_value_v15(radial_projection) % 5)
        )
        % 12,
    )
...
class SparseLawfulOperatorV15:
    """Explicit sparse operator with T_c, T_z', and T_r* rebuilt from spin_H_core_v3."""
    ...
    def transitions(self, state: OperatorStateV10) -> tuple[OperatorTransitionV1, ...]:
        ...
        lift_target = fiber_phase_lift_component_v14(state)
        radial_target = radial_transport_component_v15(state)
        return (
    ...
    def _is_lawful(source: OperatorStateV10, target: OperatorStateV10, component: str) -> bool:
        ...
        if component == "coupled_torus_kick":
            return target == coupled_torus_kick_component_v13(source)
        ...
        if component == "fiber_phase_lift_spin_transport":
            expected = fiber_phase_lift_component_v14(source)
            return (
                target == expected
        ...
        if component == "radial_transport_unfolding":
            expected = radial_transport_component_v15(source)
            return (
                target == expected
                and target.b == source.b
                and target.r != source.r
...
def bounded_operator_surface_v15(
    depth: int = 8,
) -> tuple[tuple[OperatorStateV10, ...], tuple[OperatorTransitionV1, ...]]:
    operator = SparseLawfulOperatorV15()
...
def summarize_operator_from_spinH_core_v3_radial(depth: int = 8) -> list[dict[str, object]]:
    old = _summary_metrics_from_v14()
    states, transitions = bounded_operator_surface_v15(depth=depth)
    ...
    rows.append({"scope": "operator", "metric": "state_count", "count": len(states), "total": len(states), "fraction": 1.0, "note": "reachable states with T_r* rebuilt from spin_H_core_v3"})
    rows.append({"scope": "operator", "metric": "transition_count", "count": len(transitions), "total": len(transitions), "fraction": 1.0, "note": "nonzero operator entries with one additional core-v3-derived component"})
    ...
    rows.append({"scope": "core_dependency", "metric": "rebuilt_component_depends_on_spin_H_core_v3", "count": 1, "total": 1, "fraction": 1.0, "note": "T_r* calls active_transport_lift_core_v3 and reads sigma** global_regressive_mode_index"})
    rows.append({"scope": "core_dependency", "metric": "sigma_parent_mode_identity_materially_used", "count": 1, "total": 1, "fraction": 1.0, "note": "T_r* uses sigma** generator_projection_profile, generator_orbit_profile, derived mode orbit, and radial residue placement"})
    rows.append({"scope": "core_dependency", "metric": "theta_materially_used", "count": 1, "total": 1, "fraction": 1.0, "note": "T_r* uses theta.base_angle and theta.fiber_phase in radial phase transport and spin injection"})
    rows.append({"scope": "core_dependency", "metric": "rho_materially_used", "count": 1, "total": 1, "fraction": 1.0, "note": "T_r* uses rho.radial_target, radial_target_phi, radial_direction, and unfolding_load as the radial transport law itself"})
    rows.append({"scope": "core_dependency", "metric": "h_materially_used", "count": 1, "total": 1, "fraction": 1.0, "note": "T_r* uses h.recursive_phase, h.fiber_recursive_phase, h.radial_recursive_phase, and h.holonomy_bit in radial tau/spin transport"})

    rows.append({"scope": "comparison_vs_v14", "metric": "state_count_change", "count": len(states) - int(old["state_count__count"]), "total": int(old["state_count__count"]), "fraction": (len(states) / max(old["state_count__count"], 1.0)) - 1.0, "note": "reachable state change vs prior operator with older T_r*"})
    rows.append({"scope": "comparison_vs_v14", "metric": "transition_count_change", "count": len(transitions) - int(old["transition_count__count"]), "total": int(old["transition_count__count"]), "fraction": (len(transitions) / max(old["transition_count__count"], 1.0)) - 1.0, "note": "transition count change vs prior operator with older T_r*"})
    ...
    rows.append({"scope": "comparison_vs_v14", "metric": "radial_diversity_change", "count": len(radial_counter) - int(old["distinct_radial_classes_reached__count"]), "total": int(old["distinct_radial_classes_reached__count"]), "fraction": (len(radial_counter) / max(old["distinct_radial_classes_reached__count"], 1.0)) - 1.0, "note": "radial diversity change vs prior operator with older T_r*"})
...
def write_operator_from_spinH_core_v3_radial(
    rows: list[dict[str, object]],
    output_path: Path = OUTPUT_PATH_OPERATOR_FROM_CORE_V3_RADIAL,
) -> None:
...
__all__ = [
    ...
    "SparseLawfulOperatorV15",
    "bounded_operator_surface_v15",
