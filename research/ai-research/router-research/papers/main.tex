...
\title{%
  ...
  via Hopf-Base Sector Discretization}\\[4pt]
  \large An Extension of TurboQuant to Transformer Routing
}
...
HOPF/BASELINE ratio of $1.081$---identical to the PTB confirmed ratio---under identical
training conditions.
...
Our work extends TurboQuant from data compression to routing computation, and together
suggests that the angular structure of normalized embeddings has engineering consequences
...
\begin{table}[h]
\centering
\caption{Routing scaling laws. TRANS ORIG = canonical Hopf; PERM = structure-destroyed
...
\midrule
TRANS ORIG      & $\hat{e} = 2.957 \cdot K^{\mathbf{0.572}}$ & 0.963  & $K$=25--400 \\
...
TRANS ORIG achieves $3.0$--$4.9\times$ lower effective cost and $2.5$--$2.9\times$
...
The HOPF/BASELINE ratio of 1.081 is identical on PTB and WT2, the strongest possible
replication outcome.
...
\begin{enumerate}[leftmargin=1.4em,itemsep=2pt]
  \item Large-scale LM with Hopf-routed MoE (extends INC-0171 to production scale)
  \item Non-normalized embeddings (transformer hidden states without LayerNorm may
        activate radial shell structure)
  \item Combined Hopf routing + TurboQuant quantization (covers storage and compute
        in one pipeline)
  \item Attention routing via angular sector addressing of Q/K projections
\end{enumerate}
...
\begin{table}[h]
\centering
\caption{Key increments in the evidence chain. Full increment documents:
\texttt{docs/research/increments/INC\_*.md}}
\label{tab:evidence}
\small
\begin{tabular}{p{2.8cm}p{1.4cm}p{9.0cm}}
\toprule
Increment range     & Stage  & Finding \\
\midrule
INC-0143--0146      & Stg.~3 & Fixed H\textsuperscript{4} Hopf outperforms 100D K-means;
                               Gini advantage established \\
INC-0147            & Stg.~3 & Mechanism isolation: raw fiber $\alpha=(\theta_1+\theta_2)/2$
                               is primary mechanism (+20.6~pp) \\
INC-0148--0159      & Stg.~4/5 & Training-time scaling, EMA update rule, $K{=}250$--$1000$ \\
INC-0160--0165      & Stg.~7 & Hardware proxy: $3.0$--$4.9\times$ eff\_cost reduction,
                               $2.5$--$2.9\times$ LRU miss reduction \\
INC-0166--0168      & Stg.~7 & Norm invariance ($\Delta\alpha < 0.015$), shell hierarchy
                               excluded, canonical law \\
INC-0169            & Stg.~7 & Law freeze: $\hat{e} = 2.957 \cdot K^{0.572}$ (TRANS ORIG) \\
INC-0170            & Stg.~7 & Large-$K$: ratio $2.6$--$2.8\times$ at $K{=}600$--$5000$
                               (does not collapse) \\
INC-0171            & Stg.~6/7 & LM integration (PTB, 2 seeds): HOPF within 8\% PPL,
                               eff$_b{=}46/64$, $1.39\times$ vs DENSE \\
INC-0172 (killed)   & Control & Switch aux loss $\to$ near-uniform routing
                               (eff$_b{=}63$); confirms native concentration
                               as correct comparator \\
INC-0173            & Stg.~7 & WT2 replication (2 seeds): ratio $1.081$ (mean),
